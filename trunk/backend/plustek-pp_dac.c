 ...
 */
static void
dacP98AdjustGainAverage(pScanData ps)
{
	pUChar pDest, pSrce;
	ULong dw, dw1;
	UShort wSum;
 ...
 */
static void
dacP98FillDarkDAC(pScanData ps)
{
 ...
 */
static void
dacP98SetReadFBKRegister(pScanData ps)
{
 ...
 */
static UShort
dacP98CalDarkOff(pScanData ps, UShort wChDarkOff,
		 UShort wDACCompareHigh, UShort wDACOffset)
{
	UShort wTemp;

	if ((_CCD_518 == ps->Device.bCCDID)
	    || (_CCD_535 == ps->Device.bCCDID)) {
 ...
 */
static Bool
dacP98AdjustDAC(UShort DarkOff, UShort wHigh,
 ...
 */
static Bool
dacP98CheckChannelDarkLevel(pScanData ps)
{
 ...
 */
static void
dacP98FillChannelDarkLevelControl(pScanData ps)
{
	DataPointer p;
	...
	for (p.pw = (pUShort) (ps->pScanBuffer1 + dwPos + 1024), dwSum =
	     0, dw = 16; dw; dw--, p.pw++) {
		dwSum += (ULong) (*p.pw);
	}
	...
	for (p.pw = (pUShort) (ps->pScanBuffer1 + dwPos + 1024 * 2), dwSum =
	     0, dw = 16; dw; dw--, p.pw++) {
		dwSum += (ULong) (*p.pw);
	}
 ...
 */
static void
dacP98CheckLastGain(pScanData ps)
{
	DataPointer p;
	ULong dw;
	UShort w;
 ...
 */
static void
dacP98FillGainInitialRestRegister(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP98SetInitialGainRegister(pScanData ps)
{
 ...
 */
static void
dacP98SetRGBGainRegister(pScanData ps)
{
	...
	ps->AsicReg.RD_Motor0Control =
		_MotorOn + _MotorDirForward + _MotorHEightStep;
 ...
 */
static void
dacP98FillRGBMap(pUChar pBuffer)
{
	ULong dw, dw1;
 ...
 */
static void
dacP98DownloadMapTable(pScanData ps, pUChar pBuffer)
{
	Byte bAddr;
	ULong i;
	...
	IODataToRegister(ps, ps->RegScanControl,
			 (Byte) ((ps->AsicReg.
				  RD_ScanControl & 0xfc) | _SCAN_BYTEMODE));
 ...
 */
static void
dacP98DownloadShadingTable(pScanData ps, pUChar pBuffer, ULong size)
{
	...
	IODataToRegister(ps, ps->RegScanControl,
			 (Byte) (ps->AsicReg.
				 RD_ScanControl | _SCAN_12BITMODE));
 ...
 */
static void
dacP98SetInitialGainRAM(pScanData ps)
{
 ...
 */
static void
dacP98AdjustRGBGain(pScanData ps)
{
	int bCorrectTimes;
 ...
 */
static void
dacP98SetAdjustShadingRegister(pScanData ps)
{
	...
	ps->AsicReg.RD_ModeControl = _ModeScan;
	ps->AsicReg.RD_Motor0Control =
		_MotorOn + _MotorHEightStep + _MotorDirForward;
 ...
 */
static void
dacP98ReadShadingScanLine(pScanData ps)
{
	TimerDef timer;
	...
	while ((IOReadFifoLength(ps) < dwReadyLen) && !MiscCheckTimer(&timer)) {
		_DO_UDELAY(1);
	}
 ...
 */
static void
dacP98GainResize(pUShort pValue, UShort wResize)
{
	DataType Data;
	Byte bTemp;
 ...
 */
static void
dacP98SortHilightShadow(pScanData ps, pUShort pwData,
 ...
 */
static void
dacP98WriteBackToShadingRAM(pScanData ps)
{
	ULong dw;
		...
		for (dw = 0; dw < 5400; dw++) {
			...
			*pBuffer = ((pUShort) ps->pScanBuffer1)[dw] -
			...
			dacP98GainResize(pBuffer,
					 ps->Shade.pCcdDac->GainResize.Colors.
					 Red);
			pBuffer++;
			...
			*pBuffer =
				((pUShort) ps->pScanBuffer1)[dw + 5400 * 2] -
 ...
 */
static void
dacP98ShadingRunLoop(pScanData ps)
{
	int i;
	DataPointer p;
 ...
 */
static void
dacP98Adjust12BitShading(pScanData ps)
{
	DataPointer pd, pt;
	ULong dw, dw1, dwLoop;
	...
	for (dw1 = 0; dw1 < dwLoop; dw1++) {
		...
		if ((_Shading_32Times == ps->bShadingTimeFlag)
		    && ps->Shade.pHilight) {

			dacP98SortHilightShadow(ps,
						(pUShort) ps->pScanBuffer2, 0,
						0);
			dacP98SortHilightShadow(ps,
						(pUShort) ps->pScanBuffer2 +
						ps->dwShadingPixels,
						ps->dwHilightCh,
						ps->dwShadowCh);
		...
		pt.pdw = (pULong) (ps->pScanBuffer1 + 5400 * 4 +
				   dwADCPipeLine);
		...
		pt.pdw = (pULong) (ps->pScanBuffer1 + 5400 * 8 +
				   dwADCPipeLine);
		...
		if (IOReadFifoLength(ps) <= 2500)
			IORegisterDirectToScanner(ps,
						  ps->RegRefreshScanState);
	}
 ...
 */
static Bool
dacP98WaitForShading(pScanData ps)
{
	Byte oldLineControl;
 ...
 */
static void
dacP96FillWhole4kRAM(pScanData ps, pUChar pBuf)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP96FillChannelDarkOffset(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP96FillEvenOddControl(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP96ReadDataWithinOneSecond(pScanData ps, ULong dwLen, Byte bBlks)
{
	TimerDef timer;
 ...
 */
static void
dacP96GetEvenOddOffset(pUChar pb, pWordVal pwv)
{
	ULong dw;
	UShort we, wo;
 ...
 */
static void
dacP96SumAverageShading(pScanData ps, pUChar pDest, pUChar pSrce)
{
	ULong dw;
	UShort wLeft[6];
	UShort wSumL, wSumR;
...
/*  for (dw = 2772; dw; dw--, pSrce++, pDest++) */
	for (dw = ps->BufferSizePerModel - 6; dw; dw--, pSrce++, pDest++) {
		...
		*pDest = (Byte) (((UShort) * pSrce * 4 + wSumL + wSumR) / 16);
		...
		wSumR = wSumR - (UShort) * (pSrce + 1) + (UShort) * (pSrce +
								     7);
	}
}
 ...
 */
static void
dacP96WriteLinearGamma(pScanData ps, pUChar pBuf, ULong dwEntries, Byte bBank)
{
	ULong dw;
 ...
 */
static void
dacP96FillChannelShadingOffset(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP96GetHilightShadow(pScanData ps, pUChar pBuf, pUChar pChOff, pUChar pHigh)
{
	ULong dw;
 ...
 */
static void
dacP96ReadColorShadingLine(pScanData ps)
{
	...
	for (dw = 0; dw < (ULong) ps->BufferSizeBase * 3; dw++) {
		...
		Data.wOverlap.b1st =
			Data.wOverlap.b2nd =
			(Byte) (((pUShort) ps->pScanBuffer1)[dw] / 8);
 ...
 */
static void
dacP96SetShadingGainProc(pScanData ps, Byte bHigh, ULong dwCh)
{
	...
	case 0:		/* red */
		...
		pbDest = ps->pScanBuffer1 + ps->Offset70 +
	...
	case 1:		/* green */
		...
		pbDest = ps->pScanBuffer1 + ps->Offset70 +
			ps->ShadingBankSize + ps->Device.DataOriginX;
		bGainX2 = 4;
		bGainX4 = 0x0c;
		bMask = 0x33;
		break;
	...
	case 2:
		...
		pbDest = ps->pScanBuffer1 + ps->Offset70 +
			ps->ShadingBankSize * 2 + ps->Device.DataOriginX;
		bGainX2 = 0x10;
		bGainX4 = 0x30;
		bMask = 0x0f;
	}
	...
	if (!bGain) {
		/* GammaGain1 (ps) */
		for (dw = ps->BufferSizePerModel; dw;
		     dw--, pbSrce++, pbDest++)
			if (*pbSrce > bDark)
				*pbDest = (*pbSrce - bDark) * 4;
			else
				*pbDest = 0;
	} else if (bGain == bGainX2) {
		/* GammaGain2 */
		for (dw = ps->BufferSizePerModel; dw;
		     dw--, pbSrce++, pbDest++)
			if (*pbSrce > bDark)
 ...
 */
static void
dacP96FillShadingAndGammaTable(pScanData ps)
{
 ...
 */
static void
dacP96SetInitialGainRAM(pScanData ps)
{
	ULong dw, dw1;
 ...
 */
static void
dacP96Adjust10BitShading(pScanData ps)
{
	pULong pdw;
	ULong dw;
	...
	ps->Asic96Reg.RD_ShadingCorrectCtrl =
		_ShadingRCorrectX4 | _ShadingGCorrectX4 | _ShadingBCorrectX4;
...
*/
	...
	dacP96GetHilightShadow(ps,
			       ps->pPrescan16 + ps->BufferSizePerModel * 2,
			       &ps->Asic96Reg.RD_BlueChShadingOff,
			       &bBlueHigh);
 ...
 */
static Bool
dacP96003WaitForShading(pScanData ps)
{
	int bCorrectTimes;
	DataPointer p;
	ULong dw;
	...
	IOCmdRegisterToScanner(ps, ps->RegLedControl,
	...
	if (ps->GotoShadingPosition(ps)) {
		...
		IOCmdRegisterToScanner(ps, ps->RegModelControl2,
				       _Model2DirectOutPort);
		...
		for (bCorrectTimes = 4; bCorrectTimes >= 1; bCorrectTimes--) {
			...
			ps->Asic96Reg.RD_ShadingCorrectCtrl =
				_ShadingRCorrectX4 | _ShadingGCorrectX4 |
				_ShadingBCorrectX4;
			...
			ps->Asic96Reg.RD_MotorControl =
				(ps->IgnorePF | ps->
				 MotorOn | _MotorDirForward);

			ps->AsicReg.RD_Origin = 142;
			ps->AsicReg.RD_ModelControl =
				ps->Device.ModelCtrl | _ModelWhiteIs0;
			...
			/* ReadOneScanLine (); */
			dacP96ReadDataWithinOneSecond(ps, ps->OneScanLineLen,
						      11);
...
/* CHANGE: org was:	for (dw = 2560, b[w] = 0; dw; dw--, p.pb++) { */
				for (dw = (ps->OneScanLineLen / 3), b[w] = 0;
				     dw; dw--, p.pb++) {
				...
				if (b[w] < _GAIN_LOW)
					*(pbReg[w]) +=
						a_bCorrectTimesTable
						[bCorrectTimes - 1];
				else if (b[w] > _GAIN_P96_HIGH)
					*(pbReg[w]) -=
						a_bCorrectTimesTable
						[bCorrectTimes - 1];
			}
	...
	/*===================================================================*/
		...
		if (ps->fSonyCCD) {
			...
			for (p.pb =
			     ps->pPrescan16 + 32 + ps->FBKScanLineLenBase, w =
			     0, dw = 16; dw; dw--, p.pb++) {
				w += (UShort) * p.pb;
			}
			...
			for (p.pb =
			     ps->pPrescan16 + 32 + ps->FBKScanLineLenBase * 2,
			     w = 0, dw = 16; dw; dw--, p.pb++) {
				w += (UShort) * p.pb;
			}
			...
			dacP96GetEvenOddOffset(ps->pPrescan16 + 32 +
					       ps->FBKScanLineLenBase, &wv);
			ps->Asic96Reg.RD_GreenChEvenOff = wv.b1st;
			ps->Asic96Reg.RD_GreenChOddOff = wv.b2nd;

			dacP96GetEvenOddOffset(ps->pPrescan16 + 32 +
					       ps->FBKScanLineLenBase * 2,
					       &wv);
 ...
 */
static void
dacP96001ToSetShadingAddress(pScanData ps, pUChar pData)
{
	ps->OpenScanPath(ps);
 ...
 */
static void
dacP96001WriteBackToColorShadingRam(pScanData ps)
{
	...
	dacP96001ToSetShadingAddress(ps,
				     ps->pPrescan16 +
				     _BUF_SIZE_BASE_CONST * 2);
	...
	dacP96001ToSetShadingAddress(ps,
				     ps->pPrescan16 +
				     _BUF_SIZE_BASE_CONST * 4);
}

/**
 */
static void
dacP96001ModifyShadingColor(pByte pData, Byte bMul)
{
	UShort w;
	ULong dw;
 ...
 */
static Byte
dacP96001FBKReading(pScanData ps, Byte bValue,
		    Byte bReg, pByte pbSave, Bool bFBKModify)
{
	...
	while (_TRUE) {
		...
		*pbSave = bFBKTemp;
		...
		ps->AsicReg.RD_ModelControl =
			(_ModelMemSize32k96001 | _ModelDpi300 |
			 _ModelWhiteIs0);
 ...
 */
static Bool
dacP96001WaitForShading(pScanData ps)
{
	Bool bFBKModify;
	Byte bRSave;
	Byte bGSave;
	...
	if (ps->GotoShadingPosition(ps)) {
		...
		ps->a_nbNewAdrPointer[8] = ps->a_nbNewAdrPointer[24] = 0x30;
		...
		/* SetFBK */
		if ((IODataRegisterFromScanner(ps, ps->RegReadIOBufBus) &
		     0x0f) == 0x0f)
			bFBKModify = 0;
		else
			bFBKModify = 1;

		dacP96001FBKReading(ps, 0x10, ps->RegRedDCAdjust, &bRSave,
				    bFBKModify);
		dacP96001FBKReading(ps, 0x30, ps->RegGreenDCAdjust, &bGSave,
				    bFBKModify);
		dacP96001FBKReading(ps, 0x20, ps->RegBlueDCAdjust, &bBSave,
				    bFBKModify);

		ps->OpenScanPath(ps);
		IODataToRegister(ps, ps->RegRedDCAdjust, (Byte) (bRSave + 2));
		IODataToRegister(ps, ps->RegGreenDCAdjust,
				 (Byte) (bGSave + 2));
 ...
 */
static void
dacP98003GainOffsetToDAC(pScanData ps, Byte ch, Byte reg, Byte d)
{
 ...
 */
static void
dacP98003AdjustRGBGain(pScanData ps)
{
	ULong i;
	Byte bHi[3];
	...
	for (i = 10; i-- && !ps->Shade.fStop;) {
		...
		DacP98003FillToDAC(ps, &ps->Device.RegDACGain,
				   &ps->Shade.Gain);
		...
		if (ps->Shade.bIntermediate & _ScanMode_AverageOut)
			ps->AsicReg.RD_Origin =
				(UShort) ps->Device.DataOriginX >> 1;
		else
			ps->AsicReg.RD_Origin =
				(UShort) ps->Device.DataOriginX;
		...
		if (IOReadOneShadingLine
		    (ps, (pUChar) ps->Bufs.b1.pShadingRam, 2560)) {

			if (ps->DataInf.wPhyDataType <= COLOR_256GRAY) {

				bHi[1] = DacP98003SumGains((pUChar) ps->Bufs.
							   b1.pShadingRam +
							   2560, 2560);
				if (bHi[1])
					DacP98003AdjustGain(ps,
							    _CHANNEL_GREEN,
							    bHi[1]);
				else {
					ps->Shade.fStop = _FALSE;
				...
				bHi[0] = DacP98003SumGains((pUChar) ps->Bufs.
							   b1.pShadingRam,
							   2560);
				bHi[1] = DacP98003SumGains((pUChar) ps->Bufs.
							   b1.pShadingRam +
							   2560, 2560);
				bHi[2] = DacP98003SumGains((pUChar) ps->Bufs.
							   b1.pShadingRam +
							   5120, 2560);
				...
				if (!bHi[0] || !bHi[1] || !bHi[2]) {
					...
					DacP98003AdjustGain(ps, _CHANNEL_RED,
							    bHi[0]);
					DacP98003AdjustGain(ps,
							    _CHANNEL_GREEN,
							    bHi[1]);
					DacP98003AdjustGain(ps, _CHANNEL_BLUE,
							    bHi[2]);
				}
			}
		} else
 ...
 */
static UShort
dacP98003SumDarks(pScanData ps, pUShort data)
{
	UShort i, loop;
 ...
 */
static void
dacP98003AdjustShadingWaveform(pScanData ps)
{
	Byte b;
	...
	for (count = 32; count--;) {
		...
		IOReadOneShadingLine(ps,
				     ((pUChar) ps->Bufs.b1.pShadingRam) +
				     _SHADING_BEGINX, ps->Shade.shadingBytes);
		...
		if (ps->Shade.pHilight) {
			...
			if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
				...
				cp.red.usp =
					ps->Bufs.b1.pShadingRam +
					_SHADING_BEGINX;
				cp.green.usp =
					cp.red.usp + ps->AsicReg.RD_Pixels;
				cp.blue.usp =
					cp.green.usp + ps->AsicReg.RD_Pixels;
				pvar.pusrgb =
		...
		if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
			...
			pvar.pulrgb =
				(pRGBULongDef) ps->Bufs.b2.pSumBuf +
				_SHADING_BEGINX;

			for (var.dwValue =
			     (ULong) ps->AsicReg.RD_Pixels - _SHADING_BEGINX;
			     var.dwValue--;
			     pvar.pulrgb++, cp.red.usp++, cp.green.usp++,
			     cp.blue.usp++) {
	...
	if (ps->Shade.pHilight) {
		...
		if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
			...
			psum.pulrgb =
				(pRGBULongDef) ps->Bufs.b2.pSumBuf +
				_SHADING_BEGINX;
			pwsum = (pRGBUShortDef) ps->Bufs.b2.pSumBuf +
				_SHADING_BEGINX;
			pvar.pusrgb =
				(pRGBUShortDef) ps->Shade.pHilight +
				_SHADING_BEGINX;
			...
			for (var.dwValue =
				...
				for (b =
				     ps->Shade.skipHilight +
				     ps->Shade.skipShadow; b--;
				     pRGB += ps->AsicReg.RD_Pixels) {
				...
				pwsum->Red =
					(UShort) (psum.pulrgb->Red /
						  ps->Shade.dwDiv);
				pwsum->Green =
					(UShort) (psum.pulrgb->Green /
						  ps->Shade.dwDiv);
				pwsum->Blue =
					(UShort) (psum.pulrgb->Blue /
						  ps->Shade.dwDiv);
				psum.pulrgb++;
				pwsum++;
			}
		...
		if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
			...
			for (var.dwValue =
				...
				pwsum->Green =
					(UShort) (psum.pulrgb->Green >> 5);
				pwsum->Blue =
					(UShort) (psum.pulrgb->Blue >> 5);
				psum.pulrgb++;
				pwsum++;
			}
		} else {
	...
	if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
		...
		cp.green.usp =
			(pUShort) ps->Bufs.b2.pSumBuf + _SHADING_BEGINX;
 ...
 */
static void
dacP98003AdjustDark(pScanData ps)
{
	ULong i;
	UShort wDarks[3];
	...
	for (i = 16; i-- && !ps->Shade.fStop;) {
		...
		/* FillDarkToDAC() */
		DacP98003FillToDAC(ps, &ps->Device.RegDACOffset,
				   &ps->Shade.DarkDAC);

		IODataToRegister(ps, ps->RegModeControl, _ModeIdle);

		ps->AsicReg.RD_ScanControl =
			(_SCAN_12BITMODE + _SCAN_1ST_AVERAGE);
		IOSelectLampSource(ps);
		IODataToRegister(ps, ps->RegScanControl,
				 ps->AsicReg.RD_ScanControl);
		...
		/* read one shading line and work on it */
		if (IOReadOneShadingLine
		    (ps, (pUChar) ps->Bufs.b1.pShadingRam, 512 * 2)) {

			if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {

				wDarks[0] =
					dacP98003SumDarks(ps,
							  ps->Bufs.b1.
							  pShadingRam);
				wDarks[1] =
					dacP98003SumDarks(ps,
				...
				if (!wDarks[0] || !wDarks[1] || !wDarks[2]) {
					...
					ps->Shade.DarkOffset.wColors[0] =
						wDarks[0];
					ps->Shade.DarkOffset.wColors[1] =
						wDarks[1];
					ps->Shade.DarkOffset.wColors[2] =
						wDarks[2];
					(*(ps->Device).fnDACDark) (ps,
								   ps->Shade.
								   pCcdDac,
								   _CHANNEL_RED,
								   wDarks[0]);
	...
	if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {
		(*(ps->Device).fnDarkOffset) (ps, ps->Shade.pCcdDac,
					      _CHANNEL_RED);
		(*(ps->Device).fnDarkOffset) (ps, ps->Shade.pCcdDac,
					      _CHANNEL_GREEN);
		(*(ps->Device).fnDarkOffset) (ps, ps->Shade.pCcdDac,
 ...
 */
static Bool
dacP98003WaitForShading(pScanData ps)
{
	ULong i, tmp;
	Byte bScanControl;
 ...
 */
_LOC void
DacP98FillGainOutDirectPort(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
_LOC void
DacP98FillShadingDarkToShadingRegister(pScanData ps)
{
	pUChar pValue;
	Byte bReg;
	...
	for (bReg = ps->RegRedChDarkOffsetLow;
	     bReg <= ps->RegBlueChDarkOffsetHigh; bReg++, pValue++) {
 ...
 */
_LOC void
DacP96WriteBackToGammaShadingRAM(pScanData ps)
{
	...
	IODataToRegister(ps, ps->RegRedChShadingOffset,
			 ps->Asic96Reg.u28.RD_RedChShadingOff);
	IODataToRegister(ps, ps->RegGreenChShadingOffset,
			 (Byte) ((ULong) ps->Asic96Reg.u29.
				 RD_GreenChShadingOff * 96UL / 100UL));

	IODataToRegister(ps, ps->RegBlueChShadingOffset,
			 (Byte) ((ULong) ps->Asic96Reg.RD_BlueChShadingOff *
				 91UL / 100UL));
 ...
 */
_LOC void
DacP98003FillToDAC(pScanData ps, pRGBByteDef regs, pColorByte data)
{
	if (ps->DataInf.wPhyDataType > COLOR_256GRAY) {

		dacP98003GainOffsetToDAC(ps, _DAC_RED, regs->Red,
					 data->Colors.Red);
		dacP98003GainOffsetToDAC(ps, _DAC_GREENCOLOR, regs->Green,
					 data->Colors.Green);
		dacP98003GainOffsetToDAC(ps, _DAC_BLUE, regs->Blue,
					 data->Colors.Blue);
	} else {
 ...
 */
_LOC void
DacP98003AdjustGain(pScanData ps, ULong color, Byte hilight)
{
 ...
 */
_LOC Byte
DacP98003SumGains(pUChar pb, ULong pixelsLine)
{
	Byte bHilight, tmp;
