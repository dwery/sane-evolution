...
static void
print_buffer(const SANE_Byte * buffer, int size)
{
...
#define PRINT_BUFFER_SIZE (6 + NUM_COLUMNS * (3 + 1) + 1 + 1)
	...
	for (line = 0; line < ((size + NUM_COLUMNS - 1) / NUM_COLUMNS);
	     line++) {
		pp = line_str;
...
static void
kernel_get_vendor_product(int fd, const char *name, int *vendorID,
			  int *productID)
{
#if defined (__linux__)
	...
	if (ioctl(fd, SCANNER_IOCTL_PRODUCT, productID) == -1) {
		if (ioctl(fd, SCANNER_IOCTL_PRODUCT_OLD, productID) == -1)
			DBG(3,
			    "sanei_usb_get_vendor_product: ioctl (product) "
			    "of device %s failed: %s\n", name,
			    strerror(errno));
	}
#elif defined(__BEOS__)
...
#elif defined (__FreeBSD__) || defined (__DragonFly__)
	{
		...
		for (controller = 0;; controller++) {
			snprintf(buf, sizeof(buf) - 1, "/dev/usb%d",
				 controller);
...
sanei_usb_init(void)
{
	...
	for (prefix = prefixlist; *prefix; prefix += 2) {
		...
		while ((dir_entry = readdir(dir)) != 0) {
			...
			if (strncmp
			    (base_name, dir_entry->d_name,
			     strlen(base_name)) == 0) {
				if (strlen(dir_name) +
				    strlen(dir_entry->d_name) + 1 >
				    sizeof(devname))
					continue;
				sprintf(devname, "%s%s", dir_name,
					dir_entry->d_name);
				fd = -1;
#ifdef HAVE_RESMGR
				...
				vendor = -1;
				product = -1;
				kernel_get_vendor_product(fd, devname,
							  &vendor, &product);
				close(fd);
				...
				devices[dn].method =
					sanei_usb_method_scanner_driver;
				devices[dn].open = FALSE;
				DBG(4,
	...
	for (bus = usb_get_busses(); bus; bus = bus->next) {
		for (dev = bus->devices; dev; dev = dev->next) {
			...
			found = FALSE;
			for (interface = 0;
			     interface < dev->config[0].bNumInterfaces
			     && !found; interface++) {
				...
				case USB_CLASS_PER_INTERFACE:
					if (dev->config[0].
					    ...
					    num_altsetting == 0
					    || !dev->config[0].
					    interface[interface].altsetting) {
						DBG(1,
						    "sanei_usb_init: device 0x%04x/0x%04x doesn't "
			...
			DBG(4,
			    ...
			    dev->descriptor.idVendor,
			    dev->descriptor.idProduct, interface, devname);
			dn++;
			if (dn >= MAX_DEVICES)
				return;
		}
	...
	if (rc == 0 && ulNumDev) {
		for (ulDev = 1; ulDev <= ulNumDev; ulDev++) {
			rc = UsbQueryDeviceReport(ulDev, &ulBufLen, ucData);

			pDevDesc = (struct usb_device_descriptor *) ucData;
			pCfgDesc =
				(struct usb_config_descriptor *) (ucData +
								  sizeof
								  (struct
			...
			if (pDevDesc->idVendor == 0
			    || pDevDesc->idProduct == 0) {
				DBG(5,
				    "sanei_usb_init: device 0x%04x/0x%04x looks like a root hub\n",
				    pDevDesc->idVendor, pDevDesc->idProduct);
				continue;
			...
			devices[dn].interface_nr = interface;
			DBG(4,
			    "sanei_usb_init: found usbcalls device (0x%04x/0x%04x) as device number %s\n",
			    pDevDesc->idVendor, pDevDesc->idProduct,
			    devices[dn].devname);
			dn++;
			if (dn >= MAX_DEVICES)
...
SANE_Status
sanei_usb_get_vendor_product(int dn, int *vendor, int *product)
{
	int vendorID = 0;
	int productID = 0;
	...
	if (dn >= MAX_DEVICES || dn < 0) {
		DBG(1,
		    "sanei_usb_get_vendor_product: dn >= MAX_DEVICES || dn < 0\n");
...
#ifdef HAVE_LIBUSB
		vendorID =
			usb_device(devices[dn].libusb_handle)->descriptor.
			idVendor;
		productID =
			usb_device(devices[dn].libusb_handle)->descriptor.
			idProduct;
#else
		DBG(1,
		    "sanei_usb_get_vendor_product: libusb support missing\n");
	...
	if (!vendorID || !productID) {
		DBG(3,
		    "sanei_usb_get_vendor_product: device %d: Your OS doesn't "
		    "seem to support detection of vendor+product ids\n", dn);
		return SANE_STATUS_UNSUPPORTED;
	} else {
		DBG(3,
		    "sanei_usb_get_vendor_product: device %d: vendorID: 0x%04x, "
...
SANE_Status
sanei_usb_open(SANE_String_Const devname, int *dn)
{
	int devcount;
	SANE_Bool found = FALSE;
	int c, i, a;
	...
	if (devices[devcount].method == sanei_usb_method_libusb) {
		...
		if (!devices[devcount].libusb_handle) {
			...
			if (errno == EPERM) {
				DBG(1,
				    "Make sure you run as root or set appropriate "
				    "permissions\n");
				status = SANE_STATUS_ACCESS_DENIED;
			} else if (errno == EBUSY) {
				DBG(1,
				    "Maybe the kernel scanner driver claims the "
				    "scanner's interface?\n");
		...
		result = usb_set_configuration(devices[devcount].
					       libusb_handle,
					       dev->config[0].
					       bConfigurationValue);
		if (result < 0) {
			...
			if (errno == EPERM) {
				...
				DBG(3,
				    "Maybe the kernel scanner driver or usblp claims the "
		...
		for (c = 0; c < dev->descriptor.bNumConfigurations; c++) {
			...
			for (i = 0; i < dev->config[c].bNumInterfaces; i++) {
				...
				for (a = 0;
					...
					for (num = 0;
					     num < interface->bNumEndpoints;
					     num++) {
						struct usb_endpoint_descriptor
							*endpoint;
						int address, direction,
							transfer_type;
	...
	} else if (devices[devcount].method ==
		   sanei_usb_method_scanner_driver) {
		long int flag;
		...
		rc = UsbOpen(&dh,
			     ...
			     USB_ANY_PRODUCTVERSION, USB_OPEN_FIRST_UNUSED);
		...
		pDescHead =
			(struct usb_descriptor_header *) (pCurPtr +
							  pCfgDesc->bLength);
		/* Set the configuration */
		...
		DBG(5,
		    "UsbDeviceSetConfiguration parameters: dh = %p, bConfigurationValue = %d\n",
		...
		if (result) {
			SANE_Status status = SANE_STATUS_INVAL;
			DBG(1,
			    "sanei_usb_open: usbcalls complained on UsbDeviceSetConfiguration, rc= %d\n",
			    result);
			status = SANE_STATUS_ACCESS_DENIED;
			UsbClose(dh);
			return status;
		...
		for (pDescHead =
			...
			case USB_DT_ENDPOINT:
				...
				if (transfer_type ==
				    USB_ENDPOINT_TYPE_INTERRUPT) {
					DBG(5,
					    "sanei_usb_open: found interupt-%s endpoint (address %2x)\n",
					    direction ? "in" : "out",
					    address);
					if (direction) {	/* in */
						if (devices[devcount].
					   ...
					   USB_ENDPOINT_TYPE_BULK) {
					DBG(5,
					    "sanei_usb_open: found bulk-%s endpoint (address %2x)\n",
					    direction ? "in" : "out",
					    address);
					if (direction) {	/* in */
						if (devices[devcount].
				...
				/* ignore currently unsupported endpoints */
					...
					DBG(5,
					    "sanei_usb_open: ignoring %s-%s endpoint "
					    "(address: %d)\n",
					    transfer_type ==
					    USB_ENDPOINT_TYPE_CONTROL ?
...
sanei_usb_close(int dn)
{
...
#ifdef HAVE_LIBUSB
	{
		...
		usb_clear_halt(devices[dn].libusb_handle,
			       devices[dn].iso_in_ep);
		/* be careful, we don't know if we are in DATA0 stage now */
		usb_resetep(devices[dn].libusb_handle,
			    devices[dn].bulk_in_ep);
		usb_resetep(devices[dn].libusb_handle,
			    devices[dn].bulk_out_ep);
...
void
sanei_usb_set_timeout(int timeout)
{
#ifdef HAVE_LIBUSB
...
SANE_Status
sanei_usb_read_bulk(int dn, SANE_Byte * buffer, size_t * size)
{
	ssize_t read_size = 0;
...
#ifdef HAVE_LIBUSB
	{
		if (devices[dn].bulk_in_ep)
			...
			DBG(1,
			    "sanei_usb_read_bulk: can't read without a bulk-in "
			    "endpoint\n");
			return SANE_STATUS_INVAL;
		}
	}
#else /* not HAVE_LIBUSB */
	{
		...
		while (*size) {
			...
			DBG(5, "Entered usbcalls UsbBulkRead with dh = %p\n",
			    dh);
			DBG(5,
			    "Entered usbcalls UsbBulkRead with bulk_in_ep = 0x%02x\n",
			    devices[dn].bulk_in_ep);
			DBG(5,
			    "Entered usbcalls UsbBulkRead with interface_nr = %d\n",
			    devices[dn].interface_nr);
			DBG(5,
			    "Entered usbcalls UsbBulkRead with usbcalls_timeout = %d\n",
			    usbcalls_timeout);

			if (devices[dn].bulk_in_ep) {
				rc = UsbBulkRead(dh, devices[dn].bulk_in_ep,
						 devices[dn].interface_nr,
						 &ulToRead, buffer_ptr,
						 usbcalls_timeout);
...
SANE_Status
sanei_usb_write_bulk(int dn, const SANE_Byte * buffer, size_t * size)
{
...
#ifdef HAVE_LIBUSB
	{
		if (devices[dn].bulk_out_ep)
			...
			DBG(1,
			    "sanei_usb_write_bulk: can't write without a bulk-out "
			    "endpoint\n");
			return SANE_STATUS_INVAL;
		}
	}
#else /* not HAVE_LIBUSB */
	{
		...
		DBG(5,
		    "Entered usbcalls UsbBulkWrite with bulk_out_ep = 0x%02x\n",
		    devices[dn].bulk_out_ep);
		DBG(5,
		    "Entered usbcalls UsbBulkWrite with interface_nr = %d\n",
		    devices[dn].interface_nr);
		DBG(5,
		    "Entered usbcalls UsbBulkWrite with usbcalls_timeout = %d\n",
		    usbcalls_timeout);
		while (*size) {
			ULONG ulToWrite = (*size > MAX_RW) ? MAX_RW : *size;

			DBG(5,
			    "size requested to write = %lu, ulToWrite = %lu\n",
			    (unsigned long) *size, ulToWrite);
			if (devices[dn].bulk_out_ep) {
				rc = UsbBulkWrite(dh, devices[dn].bulk_out_ep,
						  devices[dn].interface_nr,
						  ulToWrite, (char *) buffer,
						  usbcalls_timeout);
				DBG(1, "sanei_usb_write_bulk: rc = %d\n", rc);
			} else {
				DBG(1,
				    "sanei_usb_write_bulk: can't read without a bulk-out endpoint\n");
				return SANE_STATUS_INVAL;
			}
			if (rc)
				return SANE_STATUS_INVAL;
...
sanei_usb_control_msg(int dn, int rtype, int req,
		      int value, int index, int len, SANE_Byte * data)
{
	if (dn >= MAX_DEVICES || dn < 0) {
		DBG(1,
		    "sanei_usb_control_msg: dn >= MAX_DEVICES || dn < 0, dn=%d\n",
		    dn);
		return SANE_STATUS_INVAL;
	}
...
#ifdef HAVE_LIBUSB
	{
		...
		result = usb_control_msg(devices[dn].libusb_handle, rtype,
					 req, value, index, (char *) data,
					 len, libusb_timeout);
		if (result < 0) {
			DBG(1,
...
#else /* not HAVE_LIBUSB */
	{
		...
		if (result < 0) {
			DBG(1,
			    "sanei_usb_control_msg: usbcalls complained: %d\n",
			    result);
			return SANE_STATUS_INVAL;
		}
		if ((rtype & 0x80) && debug_level > 10)
...
SANE_Status
sanei_usb_read_int(int dn, SANE_Byte * buffer, size_t * size)
{
	ssize_t read_size = 0;
#ifdef HAVE_LIBUSB
	SANE_Bool stalled = FALSE;
#endif
...
#ifdef HAVE_LIBUSB
	{
		if (devices[dn].int_in_ep)
			read_size =
				usb_interrupt_read(devices[dn].libusb_handle,
			...
			DBG(1,
			    "sanei_usb_read_int: can't read without an int "
			    "endpoint\n");
			return SANE_STATUS_INVAL;
		}
...
#else /* not HAVE_LIBUSB */
	{
		...
		DBG(5,
		    "Entered usbcalls UsbIrqStart with int_in_ep = 0x%02x\n",
		    devices[dn].int_in_ep);
		DBG(5,
		    "Entered usbcalls UsbIrqStart with interface_nr = %d\n",
		    devices[dn].interface_nr);
		DBG(5,
		    "Entered usbcalls UsbIrqStart with bytes to read = %u\n",
		    usNumBytes);

		if (devices[dn].int_in_ep) {
			rc = UsbIrqStart(dh, devices[dn].int_in_ep,
					 devices[dn].interface_nr, usNumBytes,
					 (char *) buffer, pUsbIrqStartHev);
...
sanei_usb_set_configuration(int dn, int configuration)
{
...
#else /* not __linux__ */
		DBG(5,
		    "sanei_usb_set_configuration: not supported on this OS\n");
		return SANE_STATUS_UNSUPPORTED;
#endif /* not __linux__ */
	} else if (devices[dn].method == sanei_usb_method_libusb)
#ifdef HAVE_LIBUSB
	{
		...
		result = usb_set_configuration(devices[dn].libusb_handle,
					       configuration);
		if (result < 0) {
			DBG(1,
	...
	{
		DBG(1,
		    "sanei_usb_set_configuration: libusb support missing\n");
		return SANE_STATUS_UNSUPPORTED;
	}
#endif /* not HAVE_LIBUSB */
	else {
...
sanei_usb_claim_interface(int dn, int interface_number)
{
...
#else /* not __linux__ */
		DBG(5,
		    "sanei_usb_claim_interface: not supported on this OS\n");
		return SANE_STATUS_UNSUPPORTED;
#endif /* not __linux__ */
	} else if (devices[dn].method == sanei_usb_method_libusb)
#ifdef HAVE_LIBUSB
	{
		...
		result = usb_claim_interface(devices[dn].libusb_handle,
					     interface_number);
		if (result < 0) {
			DBG(1,
...
sanei_usb_release_interface(int dn, int interface_number)
{
...
#else /* not __linux__ */
		DBG(5,
		    "sanei_usb_release_interface: not supported on this OS\n");
		return SANE_STATUS_UNSUPPORTED;
#endif /* not __linux__ */
	} else if (devices[dn].method == sanei_usb_method_libusb)
#ifdef HAVE_LIBUSB
	{
		...
		result = usb_release_interface(devices[dn].libusb_handle,
					       interface_number);
		if (result < 0) {
			DBG(1,
	...
	{
		DBG(1,
		    "sanei_usb_release_interface: libusb support missing\n");
		return SANE_STATUS_UNSUPPORTED;
	}
#endif /* not HAVE_LIBUSB */
	else {
...
sanei_usb_set_altinterface(int dn, int alternate)
{
...
#else /* not __linux__ */
		DBG(5,
		    "sanei_usb_set_altinterface: not supported on this OS\n");
		return SANE_STATUS_UNSUPPORTED;
#endif /* not __linux__ */
	} else if (devices[dn].method == sanei_usb_method_libusb)
#ifdef HAVE_LIBUSB
	{
		...
		result = usb_set_altinterface(devices[dn].libusb_handle,
					      alternate);
		if (result < 0) {
			DBG(1,
