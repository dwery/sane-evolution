...
/*
   $Id: leo.c,v 1.12 2005-09-19 19:57:48 fzago-guest Exp $
   Across FS-1130
*/
...
static SANE_Range y_range = { SANE_FIX(0), SANE_FIX(11.5 * SANE_MM_PER_INCH), 0 };
...
leo_identify_scanner(Leo_Scanner * dev)
{
	...
	for (i = 0; i < NELEMS(scanners); i++) {
		    ...
		    strcmp(dev->scsi_product, scanners[i].scsi_product) == 0)
		{
			...
			DBG(DBG_error,
			    "leo_identify_scanner: scanner supported\n");
...
static SANE_Status
leo_sense_handler(int scsi_fd, unsigned char *result,
		  void __sane_unused__ * arg)
{
	...
	if (len < 14) {
		DBG(DBG_error,
		    "leo_sense_handler: sense too short, no ASC/ASCQ\n");
...
get_filled_data_length(Leo_Scanner * dev, size_t * to_read)
{
	...
	*to_read = 0;
	...
	hexdump(DBG_info2, "get_filled_data_length return", dev->buffer,
		size);
...
attach_scanner(const char *devicename, Leo_Scanner ** devp)
{
	...
	if (leo_identify_scanner(dev) == FALSE) {
		DBG(DBG_error,
...
leo_fill_image(Leo_Scanner * dev)
{
	...
	while (dev->real_bytes_left) {
		...
		DBG(DBG_info,
		    "leo_fill_image: to read   = %ld bytes (bpl=%d)\n",
		    (long) size, dev->params.bytes_per_line);

		MKSCSI_READ_10(cdb, 0, 0, size);

		hexdump(DBG_info2, "leo_fill_image: READ_10 CDB", cdb.data,
			10);
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "leo_fill_image: cannot read from the scanner\n");
			return status;
		}
		...
		if (dev->scan_mode == LEO_COLOR) {
			...
			for (i = 0; i < nb_lines; i++) {
				...
				for (j = 0; j < dev->params.pixels_per_line;
				     j++) {
					*dest = src[j +
						    0 *
						    dev->params.
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	memcpy(dev->gamma_GRAY, gamma_init,
	       dev->opt[OPT_GAMMA_VECTOR_GRAY].size);
...
sane_start(SANE_Handle handle)
{
	...
	if (!(dev->scanning)) {
		...
		if (sanei_scsi_open
		    (dev->devicename, &(dev->sfd), leo_sense_handler,
		     dev) != 0) {
