...
static int
TransferControlMsg(TInstance * this,
		   int nReqType,
		   int nRequest,
		   int nValue,
		   int nIndex,
		   void *pBuffer, int cchBuffer, int cJiffiesTimeout)
{
	SANE_Status err;
	...
	err = sanei_usb_control_msg(this->hScanner,
				    nReqType,
				    nRequest,
				    nValue, nIndex, cchBuffer, pBuffer);
	if (err)
		return err;
	return cchBuffer;
}
...
static int
TransferBulkRead(TInstance * this,
		 int nEndPoint,
		 void *pBuffer, int cchMax, int cJiffiesTimeout)
{
	int err;
	size_t sz = cchMax;
	...
	err = sanei_usb_read_bulk(this->hScanner, pBuffer, &sz);
	if (err)
		return err;
	return sz;
}
...
__SM3600EXPORT__
	TState RegWrite(TInstance * this, int iRegister, int cb,
			unsigned long ulValue)
{
	...
	if (cb < 1 || cb > 4)
		return SetError(this, SANE_STATUS_INVAL,
				"unsupported control transfer size %d", cb);
	pchBuffer = malloc(cb);
	...
	if (!bOk) {
		...
		return SetError(this, SANE_STATUS_IO_ERROR,
				"error in reg out: %d,%d,%08X", iRegister, cb,
				ulValue);
	}
	...
	if (i < 0)
		return SetError(this, SANE_STATUS_IO_ERROR,
				"error during register write");
	return SANE_STATUS_GOOD;
}

__SM3600EXPORT__
	TState RegWriteArray(TInstance * this, int iRegister, int cb,
...
__SM3600EXPORT__
	TState MemReadArray(TInstance * this, int iAddress, int cb,
			    unsigned char *pchBuffer)
{
	int i;
	INST_ASSERT();
...
__SM3600EXPORT__
	TState RegCheck(TInstance * this, int iRegister, int cch,
			unsigned long ulValue)
{
	...
	if (cch < 1 || cch > 3)
		return SetError(this, SANE_STATUS_INVAL,
				"unsupported control transfer size %d", cch);
	...
	if (!pchBuffer || !pchTransfer) {
		...
		if (pchTransfer)
			free(pchTransfer);
		rcCode = SetError(this, SANE_STATUS_NO_MEM,
				  "no memory in RegCheck()");
	}
	bOk = true;
...
BulkRead(TInstance * this, FILE * fhOut, unsigned int cchBulk)
{
	int cchRead, rc;
	char *pchBuffer;
	INST_ASSERT();
...
__SM3600EXPORT__ int
BulkReadBuffer(TInstance * this,
	       unsigned char *puchBufferOut, unsigned int cchBulk)
{
	int cchRead, rc;
	char *pchBuffer;
	INST_ASSERT();
...
__SM3600EXPORT__ unsigned int
RegRead(TInstance * this, int iRegister, int cch)
{
	char *pchTransfer;
	int i;
	unsigned int n;
