...
static int sanei_pa4s2_dbg_init_called = FALSE;

#if (!defined __GNUC__ || __GNUC__ < 2 || \
     __GNUC_MINOR__ < (defined __cplusplus ? 6 : 4))

#define TEST_DBG_INIT() if (sanei_pa4s2_dbg_init_called == FALSE) \
                          {                                            \
                            DBG_INIT();                                \
                            DBG(6, "sanei_pa4s2: interface called for" \
                              " the first time\n");                    \
                            sanei_pa4s2_dbg_init_called = TRUE;   \
                          }
#else
...
static PortRec port[] = {
	{"0x378", 0x378, FALSE, FALSE, PA4S2_MODE_NIB,
	 {0, 0, 0}},
	{"0x278", 0x278, FALSE, FALSE, PA4S2_MODE_NIB,
	 {0, 0, 0}},
	{"0x3BC", 0x3BC, FALSE, FALSE, PA4S2_MODE_NIB,
	 {0, 0, 0}}
};
...
static const char *
pa4s2_libieee1284_errorstr(int error)
{
...
pa4s2_open(const char *dev, SANE_Status * status)
{
...

	{
		...
		if ((end == dev) || (*end != '\0')) {
			...
			DBG(1,
			    "pa4s2_open: `%s` is not a valid port number\n",
			    dev);
			DBG(6,
			    "pa4s2_open: the part I did not understand was ...`%s`\n",
			    end);
...
#if defined (HAVE_LIBIEEE1284)
	DBG(6, "pa4s2_open: name=%s in_use=TRUE\n", dev);
#else
	DBG(6, "pa4s2_open: base=0x%03lx in_use=TRUE\n", base);
#endif
	DBG(6, "pa4s2_open: enabled=FALSE mode=PA4S2_MODE_NIB\n");
	port[n].in_use = TRUE;
	port[n].enabled = FALSE;
	...
	if (ioperm(port[n].base, 5, 1)) {
		...
		DBG(1,
		    "pa4s2_open: cannot get io privilege for port 0x%03lx\n",
		    port[n].base);
...
sanei_pa4s2_scsi_pp_get_status(int fd, u_char * status)
{
	...
	*status =
		(stat & 0x2f) | ((stat & 0x10) << 2) | ((stat & 0x40) << 1) |
		((stat & 0x80) >> 3);
...
#else
	DBG(6,
	    "sanei_pa4s2_scsi_pp_reg_select: selecting register %u at 0x%03lx\n",
...
sanei_pa4s2_open(const char *dev, int *fd)
{
	...
	while (port[*fd].mode <= PA4S2_MODE_EPP) {
		...
		if ((port[*fd].mode == PA4S2_MODE_UNI) &&
		    ((sanei_pa4s2_interface_options &
		      SANEI_PA4S2_OPT_TRY_MODE_UNI) == 0)) {
		...
		if ((port[*fd].mode == PA4S2_MODE_EPP) &&
		    ((sanei_pa4s2_interface_options & SANEI_PA4S2_OPT_NO_EPP)
		     != 0)) {
	...
	if ((port[*fd].mode == PA4S2_MODE_UNI) &&
	    ((sanei_pa4s2_interface_options & SANEI_PA4S2_OPT_TRY_MODE_UNI) ==
	     0)) {
		port[*fd].mode--;
	}
...
sanei_pa4s2_enable(int fd, int enable)
{
	...
	if (enable == TRUE) {
		...
		if (ioperm(port[fd].base, 5, 1)) {
			DBG(1,
			    "sanei_pa4s2_enable: cannot get io privilege for port"
			    " 0x%03lx\n", port[fd].base);

			DBG(5,
			    "sanei_pa4s2_enable:: marking port[%d] as unused\n",
			    fd);
			port[fd].in_use = FALSE;
...
sanei_pa4s2_open(const char *dev, int *fd)
{
	...
	DBG(6,
	    "sanei_pa4s2_open: basically, this backend does only compile\n");
	DBG(6, "sanei_pa4s2_open: on x86 architectures. Furthermore it\n");
	DBG(6, "sanei_pa4s2_open: needs ioperm() and inb()/outb() calls.\n");
	DBG(6,
	    "sanei_pa4s2_open: alternativly it makes use of libieee1284\n");
...
sanei_pa4s2_enable(int fd, int enable)
{
	...
	DBG(6,
	    "sanei_pa4s2_enable: oops, I think there's someone going to\n");
...
sanei_pa4s2_writebyte(int fd, u_char reg, u_char val)
{
	...
	DBG(6,
	    "sanei_pa4s2_writebyte: whatever backend you're using, tell\n");
	DBG(6,
	    "sanei_pa4s2_writebyte: the maintainer his code has bugs...\n");
