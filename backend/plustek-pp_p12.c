 ...
 */
static void
p12ButtonSetup(pScanData ps, Byte nrOfButtons)
{
 ...
 */
static void
p12InitiateComponentModel(pScanData ps)
{
	/* preset some stuff and do the differences later */
	ps->Device.buttons = 0;
	ps->Device.Model1Mono =
		_BUTTON_MODE + _CCD_SHIFT_GATE + _SCAN_GRAYTYPE;
	...
	case _SCANNER2Button:
		...
		DBG(DBG_LOW,
		    "Seems we have a Genius Colorpage Vivid III V2\n");
 ...
 */
static void
p12SetupScannerVariables(pScanData ps)
{
 ...
 */
static void
p12SetupScanningCondition(pScanData ps)
{
	TimerDef timer;
	ULong channel;
	Byte bState;
	...
	if (ps->DataInf.wPhyDataType <= COLOR_TRUE24) {
		...
		ps->Scan.dwMaxReadFifo =
			ps->Scan.dwMinReadFifo =
			ps->DataInf.dwAppPixelsPerLine << 1;
	}
	...
	if (ps->DataInf.wPhyDataType == COLOR_BW)
		ps->AsicReg.RD_Pixels =
			(UShort) ps->DataInf.dwAsicBytesPerPlane;
	else
		ps->AsicReg.RD_Pixels =
			(UShort) ps->DataInf.dwAppPixelsPerLine;
...
/* CHECK: Replace by IOPutAll.... */
	...
	IODataToRegister(ps, ps->RegScanPosLow,
			 (_LOBYTE(ps->AsicReg.RD_Origin)));
	IODataToRegister(ps, ps->RegScanPosHigh,
			 (_HIBYTE(ps->AsicReg.RD_Origin)));
 ...
 */
static void
p12Init98003(pScanData ps, Bool shading)
{
	...
	ps->sCaps.dwFlag |=
		((ULong) (ps->Device.bCCDID | ps->Device.bPCBID) << 16);
	...
	IODataToRegister(ps, ps->RegModel1Control,
			 ps->AsicReg.RD_Model1Control);
 ...
 */
static void
p12InitializeAsicRegister(pScanData ps)
{
...
p12PutToIdleMode(pScanData ps)
{
	...
	for (i = 0; i < 13; i++) {
		...
		IODataToRegister(ps, p12CcdStop[i].bReg,
				 p12CcdStop[i].bParam);
	}
...
P12InitAsic(pScanData ps)
{
	...
	ps->CtrlReadHighNibble =
		_CTRL_GENSIGNAL + _CTRL_AUTOLF + _CTRL_STROBE;
 ...
 */
_LOC void
P12SetGeneralRegister(pScanData ps)
{
	...
	if (ps->Shade.bIntermediate & _ScanMode_AverageOut)
		...
		ps->AsicReg.RD_ModelControl =
			ps->Device.ModelCtrl | _ModelDpi600;
