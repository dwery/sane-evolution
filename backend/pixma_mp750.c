		       ...
		       unsigned flag)
{
	...
	mp->cb.buf[8] = 4 | flag;
	mp->cb.reslen =
		pixma_cmd_transaction(s, mp->cb.buf, 10, mp->cb.buf, 6);
...
read_image_block(pixma_t * s, uint8_t * data)
{
	...
	if (count == IMAGE_BLOCK_SIZE) {
		...
		if ((error = pixma_read(s->io, &temp, 0)) < 0) {
			PDBG(pixma_dbg
			     (1,
			      "WARNING:reading zero-length packet failed %d\n",
			      error));
		}
	}
	return count;
...
workaround_first_command(pixma_t * s)
{
	...
	if (error != 10) {
		if (error < 0) {
			...
			PDBG(pixma_dbg
			     (1,
			      "  Sending a dummy command failed: count = %d\n",
			      error));
		}
		return;
	}
...
mp750_fill_buffer(pixma_t * s, pixma_imagebuf_t * ib)
{
	...
	if (mp->state == state_warmup) {
		...
		check_status(s);
		 /*SIM*/ while (!is_calibrated(s) && --tmo >= 0) {
			...
			if (handle_interrupt(s, 1000) > 0) {
				block_size = 0;
				error = request_image_block(s, &block_size,
							    &info);
				 /*SIM*/ if (error < 0)
					return error;
			}
		...
		if (tmo < 0) {
			PDBG(pixma_dbg
			     (1,
			      "WARNING:Timed out waiting for calibration\n"));
		...
		if (is_warming_up(s) || !is_calibrated(s)) {
			PDBG(pixma_dbg
			     (1, "WARNING:Wrong status: wup=%d cal=%d\n",
...
mp750_get_status(pixma_t * s, pixma_device_status_t * status)
{
	...
	status->lamp =
		(is_warming_up(s)) ? PIXMA_LAMP_WARMING_UP : PIXMA_LAMP_OK;
	return 0;
}
