...
static SANE_Bool inhibit_clever_precal = FALSE;
static SANE_Bool inhibit_real_calib = FALSE;
...
ring_alloc(size_t initial_size, size_t bpl, size_t ppl)
{
	...
	if ((rb = (ring_buffer *) malloc(sizeof(*rb))) == NULL)
		return NULL;
	if ((buff =
	     (u_int8_t *) malloc(initial_size * sizeof(*buff))) == NULL) {
		free(rb);
		return NULL;
	}
	rb->base = buff;
...
ring_expand(ring_buffer * rb, size_t amount)
{
	...
	buff = (u_int8_t *) realloc(rb->base,
				    (rb->size + amount) * sizeof(*buff));
	if (buff == NULL)
...
sense_handler(int scsi_fd, u_char * sense, void *arg)
{
	...
	case 0x82:		/* SCANNER HARDWARE ERROR */
		if (sense[1] & 0x01)
			DBG(10, "sense:  ERR_CPURAMFAIL\n");
		if (sense[1] & 0x02)
			DBG(10, "sense:  ERR_SYSRAMFAIL\n");
		if (sense[1] & 0x04)
			DBG(10, "sense:  ERR_IMGRAMFAIL\n");
		if (sense[1] & 0x10)
			DBG(10, "sense:  ERR_CALIBRATE\n");
		if (sense[1] & 0x20)
			DBG(10, "sense:  ERR_LAMPFAIL\n");
		if (sense[1] & 0x40)
			DBG(10, "sense:  ERR_MOTORFAIL\n");
		if (sense[1] & 0x80)
			DBG(10, "sense:  ERR_FEEDERFAIL\n");
		if (sense[2] & 0x01)
			DBG(10, "sense:  ERR_POWERFAIL\n");
		if (sense[2] & 0x02)
			DBG(10, "sense:  ERR_ILAMPFAIL\n");
		if (sense[2] & 0x04)
			DBG(10, "sense:  ERR_IMOTORFAIL\n");
		if (sense[2] & 0x08)
			DBG(10, "sense:  ERR_PAPERFAIL\n");
		if (sense[2] & 0x10)
			DBG(10, "sense:  ERR_FILTERFAIL\n");
		return SANE_STATUS_IO_ERROR;
	case 0x83:		/* OPERATION ERROR */
		if (sense[1] & 0x01)
			DBG(10, "sense:  ERR_ILLGRAIN\n");
		if (sense[1] & 0x02)
			DBG(10, "sense:  ERR_ILLRES\n");
		if (sense[1] & 0x04)
			DBG(10, "sense:  ERR_ILLCOORD\n");
		if (sense[1] & 0x10)
			DBG(10, "sense:  ERR_ILLCNTR\n");
		if (sense[1] & 0x20)
			DBG(10, "sense:  ERR_ILLLENGTH\n");
		if (sense[1] & 0x40)
			DBG(10, "sense:  ERR_ILLADJUST\n");
		if (sense[1] & 0x80)
			DBG(10, "sense:  ERR_ILLEXPOSE\n");
		if (sense[2] & 0x01)
			DBG(10, "sense:  ERR_ILLFILTER\n");
		if (sense[2] & 0x02)
			DBG(10, "sense:  ERR_NOPAPER\n");
		if (sense[2] & 0x04)
			DBG(10, "sense:  ERR_ILLTABLE\n");
		if (sense[2] & 0x08)
			DBG(10, "sense:  ERR_ILLOFFSET\n");
		if (sense[2] & 0x10)
			DBG(10, "sense:  ERR_ILLBPP\n");
		return SANE_STATUS_IO_ERROR;
	default:
...
wait_ready(Microtek_Scanner * ms)
{
	...
	while ((status = sanei_scsi_cmd(ms->sfd, comm, 6, 0, 0))
		...
		if (retry > 5)
			return SANE_STATUS_IO_ERROR;	/* XXXXXXXX */
		retry++;
		sleep(3);
	}
	return SANE_STATUS_GOOD;
...
get_scan_status(Microtek_Scanner * ms,
		int * busy, int * bytes_per_line, int * lines)
{
...
read_scan_data(Microtek_Scanner * ms,
	       int lines, u_int8_t * buffer, size_t * bufsize)
{
...
download_gamma(Microtek_Scanner * ms)
{
	...
	if (comm == NULL) {
		DBG(23,
		    ".download_gamma:  couldn't allocate %d bytes for comm buffer!\n",
		    commsize);
		return SANE_STATUS_NO_MEM;
	}
	data = comm + 10;
    ...
    /***** Gamma by TABLE *****/
		...
		if (ms->val[OPT_GAMMA_BIND].w == TRUE) {
			...
			status = sanei_scsi_cmd(ms->sfd, comm, commsize, 0,
						0);
		} else {
			pl = 1;
			do {
				...
				default:
					DBG(23,
					    ".download_gamma:  uh, exceeded pl bound!\n");
					free(comm);
					return SANE_STATUS_INVAL;	/* XXXXXXXxx */
					break;
				}
				...
				status = sanei_scsi_cmd(ms->sfd, comm,
							commsize, 0, 0);
				pl++;
    ...
    /***** Gamma by SCALAR *****/
		...
		if (ms->val[OPT_GAMMA_BIND].w == TRUE) {
			double gamma =
				SANE_UNFIX(ms->val[OPT_ANALOG_GAMMA].w);
			for (i = 0; i < ms->gamma_entries; i++) {
				int val = (max_entry *
					   pow((double) i /
					       ((double) ms->gamma_entries -
						1.0),
					       1.0 / gamma));
				switch (ms->gamma_entry_size) {
				case 1:
...
init_options(Microtek_Scanner * ms)
{
	...
	sod[OPT_MODE].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	{
		...
		if (mode_list == NULL)
			return SANE_STATUS_NO_MEM;
		i = 0;
		if (ms->dev->info.modes & MI_MODES_COLOR)
			mode_list[i++] = M_COLOR;
		if (ms->dev->info.modes & MI_MODES_GRAY)
			mode_list[i++] = M_GRAY;
		if (ms->dev->info.modes & MI_MODES_HALFTONE)
			mode_list[i++] = M_HALFTONE;
		if (ms->dev->info.modes & MI_MODES_LINEART)
			mode_list[i++] = M_LINEART;
		mode_list[i] = NULL;
	...
	sod[OPT_HALFTONE_PATTERN].constraint_type =
	...
	sod[OPT_PREVIEW].size = sizeof(int);
	...
	sod[OPT_BRIGHTNESS].size = sizeof(int);
	sod[OPT_BRIGHTNESS].cap |=
		((ms->dev->info.
		  extra_cap & MI_EXCAP_OFF_CTL) ? 0 : SANE_CAP_INACTIVE);
	...
	sod[OPT_CONTRAST].size = sizeof(int);
	...
	sod[OPT_CUSTOM_GAMMA].desc =
		"How to specify gamma correction, if at all";
	...
	sod[OPT_ANALOG_GAMMA].size = sizeof(int);
	...
	sod[OPT_ANALOG_GAMMA_R].size = sizeof(int);
	...
	sod[OPT_ANALOG_GAMMA_G].size = sizeof(int);
	...
	sod[OPT_ANALOG_GAMMA_B].size = sizeof(int);
	...
	sod[OPT_GAMMA_VECTOR].size = ms->gamma_entries * sizeof(int);
	...
	sod[OPT_GAMMA_VECTOR_R].size = ms->gamma_entries * sizeof(int);
	...
	sod[OPT_GAMMA_VECTOR_G].size = ms->gamma_entries * sizeof(int);
	...
	sod[OPT_GAMMA_VECTOR_B].size = ms->gamma_entries * sizeof(int);
	...
	if (!(ms->do_real_calib)) {
		...
		val[OPT_CALIB_ONCE].w = FALSE;
	} else
		val[OPT_CALIB_ONCE].w = TRUE;
...
parse_inquiry(Microtek_Info * mi, unsigned char *result)
{
	...
	unsigned char result[0x60] = {
		0x06, 0x31, 0x23, 0x01, 0x5b, 0x00, 0x00, 0x00, 0x41, 0x47,
		0x46, 0x41, 0x20, 0x20, 0x20, 0x20,
		0x53, 0x74, 0x75, 0x64, 0x69, 0x6f, 0x53, 0x63, 0x61, 0x6e,
		0x20, 0x49, 0x49, 0x20, 0x20, 0x20,
		0x32, 0x2e, 0x33, 0x30, 0x53, 0x43, 0x53, 0x49, 0x20, 0x46,
		0x2f, 0x57, 0x56, 0x33, 0x2e, 0x31,
		0x20, 0x43, 0x54, 0x4c, 0x35, 0x33, 0x38, 0x30, 0x03, 0x4f,
		0x8c, 0xc5, 0x00, 0xee, 0x5b, 0x43,
		0x01, 0x01, 0x02, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x4a,
	...
	mi->scanner_firmware_ver_major =
		(SANE_Byte) ((result[2] & 0xf0) >> 4);
	...
	/* create the proper range constraints, given the doc size */
	{
		...
		mi->doc_y_range.max =
			SANE_FIX((float) mi->max_y / base_res_dpmm);
	...
	case 0x00:
		...
		break;
	case 0x01:
		mi->max_gamma_bit_depth = 10;
		mi->gamma_size = 2;
		break;
	case 0x02:
		mi->max_gamma_bit_depth = 12;
		mi->gamma_size = 2;
		break;
	case 0x03:
		mi->max_gamma_bit_depth = 16;
		mi->gamma_size = 2;
		break;
	default:
		mi->max_gamma_bit_depth = 0;
		mi->gamma_size = 0;
		DBG(15, "parse_inquiry:  Unknown gamma max val!  0x%x\n",
		    result[66]);
	}
	...
	if (!((mi->model_code == 0x66) || (mi->model_code == 0x63))) {
		...
		DBG(4,
		    "parse_inquiry:  Not an E6 -- pretend recal cannot be disabled.\n");
	}

	/* The E2 lies... */
	if (mi->model_code == 0x64) {
		DBG(4,
		    "parse_inquiry:  The E2 lies about it's 3-pass heritage.\n");
		mi->does_3pass = 1;
...
dump_inquiry(Microtek_Info * mi, unsigned char *result)
{
	...
	case MI_COLSEQ_PLANE:
		MDBG_ADD("plane-by-plane (3-pass)");
		break;
	case MI_COLSEQ_PIXEL:
		MDBG_ADD("pixel-by-pixel RGB");
		break;
	case MI_COLSEQ_RGB:
		MDBG_ADD("line-by-line, R-G-B sequence");
		break;
	case MI_COLSEQ_NONRGB:
		MDBG_ADD("line-by-line, non-sequential with headers");
		break;
	case MI_COLSEQ_2PIXEL:
		MDBG_ADD("2pixel-by-2pixel RRGGBB");
		break;
	default:
...
id_microtek(u_int8_t * result, char **model_string)
{
	...
	if (!(strncmp("MICROTEK", (char *) &(result[8]), 8)) || !(strncmp("MII SC31", (char *) &(result[8]), 8)) ||	/* the IISP */
		...
		switch (result[62]) {
		case 0x16:
			*model_string = "ScanMaker 600ZS";
			break;
		case 0x50:
			*model_string = "ScanMaker II/IIXE";
			break;
		case 0x51:
			*model_string = "ScanMaker 45t";
			break;
		case 0x52:
			*model_string = "ScanMaker 35t";
			break;
		case 0x54:
			*model_string = "ScanMaker IISP";
			break;
		case 0x55:
			*model_string = "ScanMaker IIER";
			break;
		case 0x56:
			*model_string = "ScanMaker A3t";
			break;
		case 0x57:
			*model_string = "ScanMaker IIHR";
			break;
		case 0x58:
			*model_string = "ScanMaker IIG";
			break;
		case 0x59:
			*model_string = "ScanMaker III";
			break;
		case 0x5A:
			*model_string = "Agfa StudioScan";
			break;
		case 0x5B:
			*model_string = "Agfa StudioScan II";
			break;
		case 0x5C:
			*model_string = "Agfa Arcus II";
			break;
		case 0x5f:
			*model_string = "ScanMaker E3";
			break;
		case 0x62:
		...
		case 0x63:
		case 0x66:
			*model_string = "ScanMaker E6";
			break;
		case 0x64:	/* and "Vobis RealScan" */
			*model_string = "ScanMaker E2";
			break;
		case 0x65:
			*model_string = "Color PageWiz";
			break;
		case 0xC8:
			*model_string = "ScanMaker 600ZS";
			break;
		...
		default:
			...
			if (response_data_format == 0x02) {
				DBG(15,
				    "id_microtek:  (uses new SCSI II command set)\n");
				if (DBG_LEVEL >= 15) {
					DBG(1, "\n");
					DBG(1, "\n");
					DBG(1, "\n");
					DBG(1,
					    "========== Congratulations! ==========\n");
					DBG(1,
					    "You appear to be the proud owner of a \n");
					DBG(1,
					    "brand-new Microtek scanner, which uses\n");
					DBG(1,
					    "a new SCSI II command set.            \n");
					DBG(1, "\n");
					DBG(1,
					    "Try the `microtek2' backend instead.  \n");
					DBG(1, "\n");
					DBG(1, "\n");
					DBG(1, "\n");
				}
	...
	DBG(15, "id_microtek:  not microtek:  %d, %d, %d\n",
	    ...
	    strncmp("        ", (char *) &(result[8]), 8), result[62]);
	return SANE_STATUS_INVAL;
}
...
attach_scanner(const char *devicename, Microtek_Device ** devp)
{
	...
	if (id_microtek(result, &model_string) != SANE_STATUS_GOOD) {
		DBG(15,
		    "attach_scanner:  device doesn't look like a Microtek scanner.");
		if (DBG_LEVEL >= 5)
			dump_suspect_inquiry(result);
		return SANE_STATUS_INVAL;
	}
...
/********************************************************************/
static SANE_Status
end_scan(Microtek_Scanner * s, SANE_Status ostat)
{
	SANE_Status status;
...
static int
comparo(const void *a, const void *b)
{
...
static void
sort_values(int *result, u_int8_t * scanline[], int pix)
{
	int i;
	for (i = 0; i < STRIPS; i++)
		result[i] = (scanline[i])[pix];
...
static void
calc_calibration(u_int8_t * caldata, u_int8_t * scanline[], int pixels)
{
	int i, j;
	int sorted[STRIPS];
...
static SANE_Status
do_real_calibrate(Microtek_Scanner * s)
{
	...
	if ((status = start_calibration(s)) != SANE_STATUS_GOOD)
		return status;
	...
	for (nleft = STRIPS, spot = 0;
	     nleft > 0; nleft -= ntoget, spot += buffsize) {
		ntoget = (nleft > nmax) ? nmax : nleft;
		buffsize = ntoget * 3 * linewidth;
		DBG(23,
		    "...nleft %d  toget %d  size %lu  spot %d  input+spot %p\n",
		    nleft, ntoget, (u_long) buffsize, spot, input + spot);
		if ((statusA =
		     read_scan_data(s, ntoget, input + spot, &buffsize))
		    != SANE_STATUS_GOOD) {
	...
	for (letter = 'R'; letter != 'X';) {
		...
		for (spot = 0, i = 0; spot < linewidth * STRIPS * 3;
		     spot += linewidth) {
			if (input[spot + 1] == letter) {
				DBG(23, "   found %d (at %d)\n", i, spot);
				if (i >= STRIPS) {
					DBG(23,
					    "WHOA!!!  %i have already been found!\n",
					    i);
					break;
				}
		...
		if ((status =
		     download_calibration(s, combuff, letter, linewidth))
		    != SANE_STATUS_GOOD) {
...
/********************************************************************/
static SANE_Status
do_precalibrate(SANE_Handle handle)
{
		...
		return status;
	{
		...
		s->y1 = 0;
		s->y2 = (s->resolution > s->dev->info.base_resolution) ?
	...
	if (s->dev->info.source_options & (MI_SRC_FEED_BT | MI_SRC_HAS_TRANS | MI_SRC_FEED_SUPP | MI_SRC_HAS_FEED)) {	/* ZZZZZZZZZZZ */
		if ((status = accessory(s)) != SANE_STATUS_GOOD)
			return status;
	}
	...
	if ((status = wait_ready(s)) != SANE_STATUS_GOOD)
		return status;
	if ((status = start_scan(s)) != SANE_STATUS_GOOD)
		return status;
	if ((statusA = get_scan_status(s, &busy,
				       &linewidth,
				       &lines)) != SANE_STATUS_GOOD) {
		DBG(10, "do_precalibrate:  get_scan_status fails\n");
	}
	if ((status = stop_scan(s)) != SANE_STATUS_GOOD)
		return status;
	if ((status = wait_ready(s)) != SANE_STATUS_GOOD)
		return status;
...
/********************************************************************/
static SANE_Status
finagle_precal(SANE_Handle handle)
{
	Microtek_Scanner *s = handle;
	SANE_Status status;
	int match;
	...
	if ((s->do_clever_precal) || (s->do_real_calib)) {
		if ((status =
		     compare_mode_sense(s, &match)) != SANE_STATUS_GOOD)
			return status;
		if (((s->do_real_calib) && (!s->calib_once)) ||	/* user want recal */
			...
			if (s->do_real_calib) {	/* do a real calibration if allowed */
				...
				if ((status =
				     do_precalibrate(s)) != SANE_STATUS_GOOD)
					return status;
			}
			...
			DBG(23,
			    "finagle_precal:  no precalibrate necessary.\n");
	}
	return SANE_STATUS_GOOD;
}
...
set_pass_parameters(SANE_Handle handle)
{
	...
	if (s->threepasscolor) {
		...
		default:
			...
			DBG(23,
			    "set_pass_parameters:  What?!? pass %d = filter?\n",
			    s->this_pass);
			break;
		}
	} else
...
/********************************************************************/
static SANE_Status
pack_flat_data(Microtek_Scanner * s, size_t nlines)
{
	SANE_Status status;
	ring_buffer *rb = s->rb;
	...
	if (s->doexpansion) {
		...
		if (!(s->multibit)) {
			for (line = 0; line < nlines; line++) {
				double x1, x2, n1, n2;
				for (i = 0, x1 = 0.0, x2 = s->exp_aspect, n1 =
				     0.0, n2 = floor(x2); i < rb->bpl; i++) {
					byte = 0;
					for (bit = 0;
					     bit < 8;
					     bit++, x1 = x2, n1 = n2, x2 +=
					     s->exp_aspect, n2 = floor(x2)) {
			...
			for (line = 0; line < nlines; line++) {
				double x1, x2, n1, n2;
				for (i = 0, x1 = 0.0, x2 = s->exp_aspect, n1 =
				     0.0, n2 = floor(x2); i < s->dest_ppl;
				     i++, x1 = x2, n1 = n2, x2 +=
				     s->exp_aspect, n2 = floor(x2)) {
					db[pos] = (x2 == n2) ? sb[(int) n1]
						: (int) (((double)
							  sb[(int) n1] * (n2 -
									  x1)
							  + (double) sb[(int)
...
pack_seqrgb_data(Microtek_Scanner * s, size_t nlines)
{
...

	{
		...
		DBG(23,
		    "pack_seq:  dr/ar: %lu/%lu  dg/ag: %lu/%lu  db/ab: %lu/%lu\n",
		    (u_long) dr, (u_long) ar, (u_long) dg, (u_long) ag,
		    (u_long) db, (u_long) ab);
		if ((dr > ar) || (dg > ag) || (db > ab)) {
			size_t increase = 0;
			if (dr > ar)
				increase = (dr - ar);
			if (dg > ag)
				increase = MAX(increase, (dg - ag));
			if (db > ab)
				increase = MAX(increase, (db - ab));
	...
	for (seg = 0, id = 0; seg < nlines * 3; seg++, id = (id + 1) % 3) {
		...
		if (s->doexpansion) {
			...
			for (i = 0, x1 = 0.0, x2 = s->exp_aspect, n1 =
			     0.0, n2 = floor(x2); i < s->dest_ppl;
			     i++, x1 = x2, n1 = n2, x2 += s->exp_aspect, n2 =
			     floor(x2)) {
				db[spot] =
					(x2 ==
					 n2) ? sb[(int) n1] : (int) (((double)
								      sb[(int)
									 n1] *
								      (n2 -
								       x1) +
		...
		case 1:
			rb->tail_green = spot;
			rb->green_extra += rb->ppl;
			break;
		case 2:
			rb->tail_blue = spot;
			rb->blue_extra += rb->ppl;
			break;
		}
	}
	...
	DBG(18, "pack_seq:  extra r: %lu  g: %lu  b: %lu\n",
	    (u_long) rb->red_extra,
	    (u_long) rb->green_extra, (u_long) rb->blue_extra);
...
pack_goofyrgb_data(Microtek_Scanner * s, size_t nlines)
{
	...
	/* prescan to decide if ring should be expanded */
	{
		...
		ab = rb->size - (rb->complete_count + rb->blue_extra * 3);
		DBG(23,
		    "pack_goofy:  dr/ar: %lu/%lu  dg/ag: %lu/%lu  db/ab: %lu/%lu\n",
		    (u_long) dr, (u_long) ar, (u_long) dg, (u_long) ag,
		    (u_long) db, (u_long) ab);
		/* >, or >= ???????? */
		if ((dr > ar) || (dg > ag) || (db > ab)) {
	...
	for (seg = 0; seg < nlines * 3; seg++) {
		...
		default:
			DBG(18,
			    "pack_goofy:  missing scanline RGB header!\n");
			return SANE_STATUS_IO_ERROR;
		}
	...
	}
	{
		...
		for (line = 0; line < nlines; line++) {
			for (p = 0; p < s->dest_ppl; p += 2) {
				...
				db[pos] = sb[0];
				if (++pos >= rb->size)
					pos = 0;	/* watch out for ringbuff end? */
				db[pos] = sb[2];
				if (++pos >= rb->size)
					pos = 0;
				db[pos] = sb[4];
...
read_from_scanner(Microtek_Scanner * s, int *nlines)
{
	...
	if (s->unscanned_lines > 0) {
		...
		DBG(18,
		    "read_from_scanner: gss busy, linewidth, remaining:  %d, %d, %d\n",
	...
	*nlines = MIN(remaining, s->max_scsi_lines);
	...
	if (*nlines > 0) {
		...
		status = read_scan_data(s, *nlines, s->scsi_buffer,
					&buffsize);
		if (status != SANE_STATUS_GOOD) {
...
pack_into_ring(Microtek_Scanner * s, int nlines)
{
	...
	case MS_LNFMT_FLAT:
		status = pack_flat_data(s, nlines);
		break;
	case MS_LNFMT_SEQ_RGB:
		status = pack_seqrgb_data(s, nlines);
		break;
	case MS_LNFMT_GOOFY_RGB:
		status = pack_goofyrgb_data(s, nlines);
		break;
	case MS_LNFMT_SEQ_2R2G2B:
		status = pack_seq2r2g2b_data(s, nlines);
		break;
	default:
...
/********************************************************************/
static int
pack_into_dest(SANE_Byte * dest_buffer, int dest_length,
	       ring_buffer * rb)
{
	int ret_length = MIN(rb->complete_count, dest_length);

	DBG(23, "pack_into_dest...\n");
	DBG(23, "pack_into_dest:  rl: %lu  sz: %lu  hc: %lu\n",
	    (u_long) ret_length, (u_long) rb->size,
	    (u_long) rb->head_complete);
	/* adjust for rollover!!! */
...
sane_open(const char * devicename, SANE_Handle * handle)
{
	...
	scanner->unit_type =
		(dev->info.
		 unit_type & MI_UNIT_PIXELS) ? MS_UNIT_PIXELS :
		MS_UNIT_18INCH;
	scanner->res_type =
		(dev->info.
		 res_step & MI_RESSTEP_1PER) ? MS_RES_1PER : MS_RES_5PER;
	scanner->midtone_support =
		(dev->info.
		 enhance_cap & MI_ENH_CAP_MIDTONE) ? TRUE : FALSE;
	scanner->paper_length =
		(scanner->unit_type ==
		 MS_UNIT_PIXELS) ? dev->info.
		max_y : (int) ((double) dev->info.max_y * 8.0 /
				    (double) dev->info.base_resolution);
	/*
	   (int)(SANE_UNFIX(dev->info.max_y) * dev->info.base_resolution) :
	   (int)(SANE_UNFIX(dev->info.max_y) * 8);
	   ZZZZZZZ */
	scanner->bright_r = 0;
	scanner->bright_g = 0;
	scanner->bright_b = 0;
	...
	if ((dev->info.extra_cap & MI_EXCAP_DIS_RECAL) &&
		...
		scanner->allow_calibrate = FALSE;
		scanner->do_real_calib = TRUE;
		scanner->do_clever_precal = FALSE;
	} else if ((dev->info.extra_cap & MI_EXCAP_DIS_RECAL) &&
		   (!(inhibit_clever_precal))) {
		DBG(23, "sane_open:  Clever precalibration enabled.\n");
		scanner->allow_calibrate = FALSE;
		scanner->do_real_calib = FALSE;
		scanner->do_clever_precal = TRUE;
	} else {
		DBG(23, "sane_open:  All calibration routines disabled.\n");
		scanner->allow_calibrate = TRUE;
		scanner->do_real_calib = FALSE;
		scanner->do_clever_precal = FALSE;
	}

	scanner->onepass = (dev->info.modes & MI_MODES_ONEPASS);
	scanner->allowbacktrack = TRUE;	/* ??? XXXXXXX */
	scanner->reversecolors = FALSE;
	scanner->fastprescan = FALSE;
	...
	if (dev->info.max_lookup_size) {
		...
		if ((scanner->gray_lut == NULL) ||
			...
			DBG(23,
			    "sane_open:  unable to allocate space for %d-entry LUT's;\n",
			    scanner->gamma_entries);
			DBG(23,
			    "            so, gamma tables now DISABLED.\n");
			free(scanner->gray_lut);
			free(scanner->red_lut);
			free(scanner->green_lut);
			free(scanner->blue_lut);
		}
		for (j = 0; j < scanner->gamma_entries;
		     j += scanner->gamma_entry_size) {
			v = (int)
				((double) j * (double) max_entry /
				 ((double) scanner->gamma_entries - 1.0) +
				 0.5);
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:{
		...
		case OPT_EXP_RES:
			if (val[option].w != *(int *) value) {
				val[option].w = *(int *) value;
				if (info)
					*info |= SANE_INFO_RELOAD_OPTIONS |
						SANE_INFO_RELOAD_PARAMS;
				if (val[OPT_EXP_RES].w) {
					sod[OPT_RESOLUTION].constraint.range =
						&(scanner->exp_res_range);
		...
		case OPT_CUSTOM_GAMMA:
			...
			if (!(strcmp(val[OPT_CUSTOM_GAMMA].s, M_NONE)) ||
				...
				sod[OPT_GAMMA_VECTOR].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_VECTOR_R].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_VECTOR_G].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_VECTOR_B].cap |=
					SANE_CAP_INACTIVE;
			}
			if (!(strcmp(val[OPT_CUSTOM_GAMMA].s, M_NONE)) ||
			    !(strcmp(val[OPT_CUSTOM_GAMMA].s, M_TABLE))) {
				sod[OPT_ANALOG_GAMMA].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_ANALOG_GAMMA_R].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_ANALOG_GAMMA_G].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_ANALOG_GAMMA_B].cap |=
					SANE_CAP_INACTIVE;
			}
	...
	if (!s->scanning) {
		...
		if (s->mode == MS_MODE_COLOR) {
			if (s->onepass) {
				/* regular one-pass */
				DBG(23,
				    "sane_get_parameters:  regular 1-pass color\n");
				s->threepasscolor = FALSE;
				s->onepasscolor = TRUE;
				s->color_seq = s->dev->info.color_sequence;
			} else {	/* 3-pass scanner */
				DBG(23,
				    "sane_get_parameters:  regular 3-pass color\n");
				s->threepasscolor = TRUE;
		...
		s->doexpansion = (s->expandedresolution
				  && !(s->dev->info.does_expansion));
		...
		if (SANE_OPTION_IS_ACTIVE(s->sod[OPT_BRIGHTNESS].cap)) {
			...
			if (s->val[OPT_BRIGHTNESS].w >= 0)
				s->bright_r =
					(SANE_Byte) (s->val[OPT_BRIGHTNESS].
						     w);
			else
				s->bright_r =
					(SANE_Byte) (0x80 |
						     (-s->val[OPT_BRIGHTNESS].
						      w));
#else
		...
		if (s->mode == MS_MODE_HALFTONE) {
			...
			while ((halftone_mode_list[i] != NULL) &&
			       (strcmp
				(halftone_mode_list[i],
				 s->val[OPT_HALFTONE_PATTERN].s)))
				i++;
			s->pattern =
				((i < s->dev->info.pattern_count) ? i : 0);
		} else
			s->pattern = 0;
...

		{
			 ...
			 */
			s->x1 = (int) (SANE_UNFIX(s->val[OPT_TL_X].w) *
					    units_per_mm + 0.5);
			s->y1 = (int) (SANE_UNFIX(s->val[OPT_TL_Y].w) *
					    units_per_mm + 0.5);
			s->x2 = (int) (SANE_UNFIX(s->val[OPT_BR_X].w) *
					    units_per_mm + 0.5);
			s->y2 = (int) (SANE_UNFIX(s->val[OPT_BR_Y].w) *
					    units_per_mm + 0.5);
		...
		s->params.last_frame = TRUE;	/* ?? XXXXXXXX */
	}
...
sane_start_guts(SANE_Handle handle)
{
	...
	status = sanei_scsi_open(s->dev->sane.name,
				 &(s->sfd), sense_handler, &(s->sense_flags));
	if (status != SANE_STATUS_GOOD) {
	...
	if ((status = scanning_frame(s)) != SANE_STATUS_GOOD)
		return end_scan(s, status);
	if (s->dev->info.source_options & (MI_SRC_FEED_BT | MI_SRC_HAS_TRANS | MI_SRC_FEED_SUPP | MI_SRC_HAS_FEED)) {	/* ZZZZZZZZZZZ */
		if ((status = accessory(s)) != SANE_STATUS_GOOD)
			return end_scan(s, status);
	...
	if ((s->unscanned_lines < 0) ||
		...
		DBG(10,
		    "sane_start:  get_scan_status returns weird line count %d\n",
		    s->unscanned_lines);
	...
	case MS_MODE_GRAY:
		if (s->bits_per_color < 8) {
			...
			s->pixel_bpl =
				linewidth * ((s->bits_per_color + 7) / 8);
			s->ppl = linewidth;
		}
		s->header_bpl = 0;
		s->planes = 1;
	...
	case MS_MODE_COLOR:
		...
		case MI_COLSEQ_PIXEL:
			s->pixel_bpl =
				linewidth * 3 * ((s->bits_per_color + 7) / 8);
			s->ppl = linewidth;
			s->header_bpl = 0;
			s->planes = 3;
			s->line_format = MS_LNFMT_FLAT;
	...
	if ((s->doexpansion) &&
		...
		s->dest_pixel_bpl =
			(int) ceil((double) s->pixel_bpl / s->exp_aspect);
		/*s->exp_aspect = 
	...
	if (s->max_scsi_lines < 1) {
		DBG(10,
		    "sane_start:  SCSI buffer smaller that one scan line!\n");
		return end_scan(s, SANE_STATUS_NO_MEM);
	}

	s->scsi_buffer =
		(u_int8_t *) malloc(SCSI_BUFF_SIZE * sizeof(u_int8_t));
	if (s->scsi_buffer == NULL)
		return SANE_STATUS_NO_MEM;
	       ...
	       int dest_length, int * ret_length)
{
	...
	*ret_length = 0;	/* default: no data */
	...
	if (s->cancel)
		return end_scan(s, SANE_STATUS_CANCELLED);
	/* we're not really scanning!... */
	if (!(s->scanning))
		return SANE_STATUS_INVAL;
	/* we are done scanning... */
	if (s->undelivered_bytes <= 0)
		return end_scan(s, SANE_STATUS_EOF);

	/* get more bytes if our ring is empty... */
	while (rb->complete_count == 0) {
		if ((status =
		     read_from_scanner(s, &nlines)) != SANE_STATUS_GOOD) {
...
sane_cancel(SANE_Handle handle)
{
	...
	ms->cancel = TRUE;
	if (!(ms->woe))
		end_scan(ms, SANE_STATUS_CANCELLED);
}
