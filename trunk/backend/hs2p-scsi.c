...
print_devtype(SANE_Byte devtype)
{
	...
	return (i >= 0
		&& i < NELEMS(devtypes)) ? devtypes[i] : "unknown-device";
}
static void
...
ScannerDump(HS2P_Scanner * s)
{
	...
	DBG(DBG_info,
	    "SANE Device: '%s' Vendor: '%s' Model: '%s' Type: '%s'\n",
	    sdev->name, sdev->vendor, sdev->model, sdev->type);

	DBG(DBG_info,
	    "Type: '%s' Vendor: '%s' Product: '%s' Revision: '%s'\n",
	...
	DBG(DBG_info, "Marker Recognition: %s\n",
	    info->
	    supports_markerrecognition ? "<supported>" : "<not supported>");
	DBG(DBG_info, "Size Recognition  : %s\n",
	    info->
	    supports_sizerecognition ? "<supported>" : "<not supported>");
	...
	DBG(DBG_info, "Service Mode is in Select %s Mode\n",
	    info->service_mode ==
	    0 ? "Self-Diagnostics" : "Optical Adjustment");
...
sense_handler(int scsi_fd, u_char * result, void *arg)
{
	...
	DBG(DBG_sense,
	    "sense_handler: result=%#x, sense=%#x, asc=%#x, ascq=%#x\n",
	...
	DBG(DBG_proc, "sense_handler %s: '%s'-'%s' '%s' return:%d\n",
	    sense_str, skey->meaning, skey->description,
	    ascq_key->description, status);
	return status;
}
...
mode_sense(int fd, MP * buf, SANE_Byte page_code)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "ERROR mode_sense: sanei_scsi_cmd error \"%s\"\n",
		    sane_strstatus(status));
		DBG(DBG_error,
		...
		DBG(DBG_info,
		    ">>>>> copying from msp to calling function's buf\n"
...
print_window_data(SWD * buf)
{
	...
	for (i = 0; i < 2; i++) {
		...
		DBG(DBG_info, "Byte29   = %#0x RIF=%d PaddingType=%d\n",
		    data->byte29, data->byte29 & 0x80, data->byte29 & 0x7);
		...
		DBG(DBG_info, "Ignored = %#x\n", data->ignored2);
		DBG(DBG_info,
		    "Byte42 = %#x MRIF=%d Filtering=%d GammaID=%d\n",
...
is_valid_endorser_character(char c)
{
	...
	if (i == 0x20)
		return TRUE;	/* space */
	if (i >= 0x41 && i <= 0x5A)
		return TRUE;	/* ABCDEFGHIJKLMNOPQRSTUVWXYZ <spaces> */
	if (i >= 0x61 && i <= 0x7A)
		return TRUE;	/* abcdefghijklmnopqrstuvwxyz <spaces> */
...
#if 0
static SANE_Status
set_halftone_mask(int fd, SANE_Byte halftone_id, void *buf, size_t * buf_size)
{
	static struct scsi_rs_scanner_cmd cmd;
	SANE_Status status;
...
trigger_scan(HS2P_Scanner * s)
{
	 ...
	 */
	scan.cmd.len = (s->val[OPT_DUPLEX].w == TRUE) ? 2 : 1;

	DBG(DBG_info, "trigger_scan: sending %d Window Id to scanner\n",
	    scan.cmd.len);
	status = sanei_scsi_cmd(s->fd, &scan, sizeof(scan.cmd) + scan.cmd.len,
				NULL, NULL);
...
hs2p_wait_ready(HS2P_Scanner * s)
{
	...
	while (1) {
		...
		default:
			...
			DBG(DBG_error,
			    "scsi_wait_ready: get datat status failed (%s)\n",
			    sane_strstatus(status));
			/* fall through */
...
connection_parameters(int fd, MP_CXN * settings, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		DBG(DBG_info,
		    ">> GET connection_parameters >> calling mode_sense\n");
		status = mode_sense(fd, (MP *) & buf,
				    (SANE_Byte) PAGE_CODE_CONNECTION);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		DBG(DBG_info,
		    ">> SET connection_parameters >> calling mode_select\n");
	    ...
	    SANE_Byte * adf_mode, SANE_Byte * mwt)
{
	...
	if (flag) {		/* GET */
		...
		status = mode_sense(fd, (MP *) & buf,
				    (SANE_Byte) PAGE_CODE_ADF_CONTROL);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "get_adf_control: MODE_SELECT failed\n");
			return (status);
		}
...
white_balance(int fd, int *val, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		...
		status = mode_sense(fd, (MP *) & buf,
				    (SANE_Byte) PAGE_CODE_WHITE_BALANCE);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
...
lamp_timer(int fd, int val, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		...
		if ((status =
		    ...
		    SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "get_lamp_timer: MODE_SELECT failed\n");
			return (-1);
		}
	} else {		/* SET */
		/* Fill in struct then hand off to mode_select */
		...
		if ((status =
		     mode_select(fd, (MP *) & buf)) != SANE_STATUS_GOOD) {
			DBG(DBG_error,
...
endorser_control(int fd, int *val, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		DBG(DBG_info,
		    ">> GET endorser control >> calling mode_sense\n");
		status = mode_sense(fd, (MP *) & buf,
				    (SANE_Byte) PAGE_CODE_ENDORSER_CONTROL);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		*val = buf.endorser_control & mask;
		...
		DBG(DBG_info,
		    ">> SET endorser control >> calling mode_select\n");
		/* Fill in struct then hand off to mode_select */
		memset(&buf, 0, sizeof(buf));	/* Fill struct with zeros */
...
scan_wait_mode(int fd, int val, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		DBG(DBG_info,
		    ">> GET scan_wait_mode >> calling mode_sense\n");
		status = mode_sense(fd, (MP *) & buf,
				    (SANE_Byte) PAGE_CODE_SCAN_WAIT_MODE);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		DBG(DBG_info,
		    ">> SET scan_wait_mode >> calling mode_sense\n");
		if ((status =
		     mode_select(fd, (MP *) & buf)) != SANE_STATUS_GOOD) {
...
service_mode(int fd, int val, SANE_Bool flag)
{
	...
	if (flag) {		/* GET */
		...
		status = mode_sense(fd, (MP *) & buf, (SANE_Byte)
				    PAGE_CODE_SERVICE_MODE_SELECT);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
