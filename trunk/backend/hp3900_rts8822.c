...
!= 0)
								...
								if (curve->
									...
									curve->step = (SANE_Int *) malloc(sizeof(SANE_Int) * curve->step_count);
									if (curve->step != NULL) {
...
static SANE_Int
Motor_Setup_Steps(struct st_device *dev, SANE_Byte * Regs, SANE_Int mysetting)
{
		...
		struct st_scanparams *scancfg, SANE_Int somevalue)
{
	...
	if ((Regs != NULL) && (scancfg != NULL)) {
		SANE_Int colormode, mymode, step_type, dummyline, myvalue,
			lf02c;
		SANE_Int step_size;
		  ...
		  struct st_hwdconfig *hwdcfg, SANE_Int bytes_per_line)
{
	...
	if ((Regs != NULL) && (hwdcfg != NULL)) {
		SANE_Int dots_count, myvalue, myvalue2, mem_available,
...
static SANE_Int
RTS_EEPROM_ReadByte(USB_Handle usb_handle, SANE_Int address, SANE_Byte * data)
{
	SANE_Int rst;
