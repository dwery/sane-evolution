...
static SANE_Status
cwrite(UMAX_Handle * scan, UMAX_Cmd cmd, size_t len,
       const unsigned char *data, UMAX_Status_Byte * s)
{
	SANE_Status res;
	UMAX_Status_Byte s0, s4;
      ...
      UMAX_Status_Byte * s)
{
	...
	if (len > 0) {
		CHK(sanei_pv8630_wait_byte
		    (scan->fd, PV8630_RSTATUS, UMAX_OK_WITH_MOTOR,
		     UMAX_STATUS_MASK, 2000));
		...
		while (len > 0) {
			...
			if (n < req) {
				DBG(1,
				    "qread: Expecting to read %lu, only got %lu\n",
				    (u_long) req, (u_long) n);
...
static SANE_Status
cwritev(UMAX_Handle * scan, UMAX_Cmd cmd, size_t len,
	const unsigned char *data, UMAX_Status_Byte * s)
{
	SANE_Status res;
...
umaxinit(UMAX_Handle * scan)
{
	...
	*p++ = 0xaa;
	...
	if (res != SANE_STATUS_GOOD) {
		DBG(4,
		    "umaxinit: Writing ramp 1 failed (is this a 2000U?)\n");
	}
	CHK(cwritev(scan, CMD_8, 35, opb1, &s));
	...
	if (res != SANE_STATUS_GOOD) {
		DBG(4,
		    "umaxinit: Writing ramp 2 failed (is this a 2000U?)\n");
	}
	...
	if (res != SANE_STATUS_GOOD) {
		DBG(4,
		    "umaxinit: Writing ramp 3 failed (is this a 2000U?)\n");
	}
	CHK(cwritev(scan, CMD_8, 35, opb1, &s));
...
move(UMAX_Handle * scan, int distance, UMAX_Speed fine)
{
	...
	scan->scanner_ypos +=
		(fine ==
		 UMAX_FINE ? distance : 2 * distance + (rev ? -1 : 1));
...
/* default gamma translation table */
	unsigned char ggamma[256] = {
		0x00, 0x06, 0x0A, 0x0D, 0x10, 0x12, 0x14, 0x17, 0x19, 0x1B,
		0x1D,
		0x1F, 0x21, 0x23, 0x24, 0x26, 0x28, 0x2A, 0x2B, 0x2D, 0x2E,
		0x30,
		0x31, 0x33, 0x34, 0x36, 0x37, 0x39, 0x3A, 0x3B, 0x3D, 0x3E,
		0x40,
		0x41, 0x42, 0x43, 0x45, 0x46, 0x47, 0x49, 0x4A, 0x4B, 0x4C,
		0x4D,
		0x4F, 0x50, 0x51, 0x52, 0x53, 0x55, 0x56, 0x57, 0x58, 0x59,
		0x5A,
		0x5B, 0x5C, 0x5E, 0x5F, 0x60, 0x61, 0x62, 0x63, 0x64, 0x65,
		0x66,
		...
		
	};
...
send_scan_parameters(UMAX_Handle * scan)
{
	...
	opbgo[19] = xend >> 4;
	opbgo[33] =
		0x33 + ((xend & 0x1000) >> 5) + ((scan->xskip & 0x1000) >> 6);
...
send_scan_parameters_2100U(UMAX_Handle * scan)
{
	...
	opbgo[19] = xend >> 4;
	opbgo[33] =
		0x23 + ((xend & 0x1000) >> 5) + ((scan->xskip & 0x1000) >> 6);
...
read_raw_strip_color(UMAX_Handle * scan)
{
  ...
  */
	...
	if (scan->maxh == -1) {
		DBG(10,
		    "read_raw_strip_color: filling buffer for the first time\n");
		...
		DBG(10,
		    "read_raw_strip_color: reading new rows into buffer\n");
		...
		CHK(read_raw_data
		    (scan, scan->p + hextra * linelen,
		     lines_to_read * linelen));
...
UMAX_start_scan(UMAX_Handle * scan)
{
	...
	CHK(find_zero(scan));
	CHK(move
	    (scan, scan->scanner_yorg - 232 - scan->scanner_ypos, UMAX_FINE));
...
UMAX_open_device(UMAX_Handle * scan, const char *dev)
{
	...
	if (sanei_usb_get_vendor_product(scan->fd, &vendor, &product) !=
	    SANE_STATUS_GOOD) {
		DBG(1,
		    "UMAX_open_device: sanei_usb_get_vendor_product failed\n");
