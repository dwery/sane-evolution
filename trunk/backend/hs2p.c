...
static char inquiry_data[255] = "HS2P scanner";
/*
...
is_device_supported(char *device)
{
	...
	return (hw == NULL) ? FALSE : TRUE;
}

static int
get_list_index(const char *list[], char *s)
{				/* sequential search */
	int i;
...
init_options(HS2P_Scanner * s)
{
	...
	if (s->hw->info.supports_MH == FALSE ||	/* MH  G3 1-D       */
	    ...
	    s->hw->info.supports_MMR == FALSE ||	/* MMR G4 2-D       */
	    s->hw->info.supports_MHB == FALSE) {	/* MH byte boundary */
	...
	DBG(DBG_info,
	    "INIT_OPTIONS: ul(x,y) = (%d,%d)    br(x,y) = (%d,%d)\n",
	...
	s->opt[OPT_ENDORSER_STRING].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	s->opt[OPT_ENDORSER_STRING].size =
		sizeof(s->hw->info.endorser_string);
	...
	s->val[OPT_SCAN_WAIT_MODE].w =
		(s->hw->info.scan_wait_mode) ? TRUE : FALSE;
	...
	s->opt[OPT_GAMMA_VECTOR_GRAY].size = GAMMA_LENGTH * sizeof(int);
	...
	s->opt[OPT_WHITE_BALANCE].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	s->opt[OPT_WHITE_BALANCE].constraint_type = SANE_CONSTRAINT_NONE;
	s->val[OPT_WHITE_BALANCE].w = FALSE;	/* F/T = Relative/Absolute White */

	/* 
	 * "Miscellaneous" GROUP: 
	 */
	s->opt[OPT_MISCELLANEOUS_GROUP].name = "";
	s->opt[OPT_MISCELLANEOUS_GROUP].title =
		SANE_TITLE_MISCELLANEOUS_GROUP;
	...
	s->opt[OPT_MISCELLANEOUS_GROUP].constraint_type =
...
attach(SANE_String_Const devname, HS2P_Device ** devp)
{
	...
	status = inquiry(fd, &ibuf, &buf_size, 0,
			 HS2P_INQUIRY_STANDARD_PAGE_CODE);
	if (status != SANE_STATUS_GOOD) {
	...
	DBG(DBG_info,
	    ...
	    ibuf.rmb_evpd, ibuf.version, ibuf.response_data_format,
	    ibuf.length, ibuf.byte7);

	if (ibuf.devtype != 6
	    || strncmp((char *) ibuf.vendor, "RICOH   ", 8) != 0) {
		DBG(DBG_warning,
		    ">>> attach: device is not a RICOH scanner\n");
		sanei_scsi_close(fd);
	...
	status = get_basic_measurement_unit(fd, &(dev->info.bmu),
					    &(dev->info.mud));
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    ">>> attach: get_basic_measurement_unit failed (%s)\n",
		    sane_strstatus(status));
	...
	if ((status =
	     get_connection_parameters(fd,
				       &(dev->info.cxn))) !=
	    SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    ">>> attach: get_connection_parameters failed\n");
	}
	...
	dev->info.hasADF = vbuf.adf_id == 0 ? FALSE : TRUE;
	dev->info.hasSimplex = vbuf.adf_id == 1 ? TRUE : FALSE;
	dev->info.hasDuplex = vbuf.adf_id == 2 ? TRUE : FALSE;
	dev->info.hasARDF = vbuf.adf_id == 3 ? TRUE : FALSE;

	/* end_id 0=none,1=Yes,2=reserved;  should always be 0 or 1 */
	dev->info.hasEndorser = vbuf.end_id == 1 ? TRUE : FALSE;
	for (i = 0; i < 20; i++)
	 ...
	 */
	dev->info.hasIPU =
		(vbuf.ipu_id & 0x01) == 0x01 ? TRUE : FALSE;
	dev->info.hasXBD =
		(vbuf.ipu_id & 0x02) == 0x02 ? TRUE : FALSE;


	/* Image Composition Byte is set to 0x37 (0011 0111) */
	dev->info.supports_lineart = (vbuf.imagecomposition & 0x01) == 0x01 ? TRUE : FALSE;	/* TRUE */
	dev->info.supports_dithering = (vbuf.imagecomposition & 0x02) == 0x02 ? TRUE : FALSE;	/* TRUE */
	dev->info.supports_errordiffusion = (vbuf.imagecomposition & 0x04) == 0x04 ? TRUE : FALSE;	/* TRUE */
	dev->info.supports_color = (vbuf.imagecomposition & 0x08) == 0x08 ? TRUE : FALSE;	/* FALSE */
	dev->info.supports_4bitgray = (vbuf.imagecomposition & 0x10) == 0x10 ? TRUE : FALSE;	/* TRUE */
	dev->info.supports_8bitgray = (vbuf.imagecomposition & 0x20) == 0x20 ? TRUE : FALSE;	/* TRUE */
	...
	*str = NULL;
	...
	dev->info.supports_whiteframing =
		((vbuf.imagedataprocessing[0] & 0x01) ==
		 0x01) ? TRUE : FALSE;
	dev->info.supports_blackframing =
		((vbuf.imagedataprocessing[0] & 0x02) ==
		 0x02) ? TRUE : FALSE;
	dev->info.supports_edgeextraction =
		((vbuf.imagedataprocessing[0] & 0x04) ==
		 0x04) ? TRUE : FALSE;
	dev->info.supports_noiseremoval =
		((vbuf.imagedataprocessing[0] & 0x08) ==
		 0x08) ? TRUE : FALSE;
	dev->info.supports_smoothing =
		((vbuf.imagedataprocessing[0] & 0x10) ==
		 0x10) ? TRUE : FALSE;
	dev->info.supports_linebolding =
		((vbuf.imagedataprocessing[0] & 0x20) ==
		 0x20) ? TRUE : FALSE;

	/* Compression Method is not supported for IS450
	 *                    is     supported for IS420  */
	dev->info.supports_MH =
		((vbuf.compression & 0x01) == 0x01) ? TRUE : FALSE;
	dev->info.supports_MR =
		((vbuf.compression & 0x02) == 0x02) ? TRUE : FALSE;
	dev->info.supports_MMR =
		((vbuf.compression & 0x04) == 0x04) ? TRUE : FALSE;
	dev->info.supports_MHB = ((vbuf.compression & 0x08) == 0x08) ? TRUE : FALSE;	/* MH Byte Boundary */
	...
	*str = NULL;
	...
	if (dev->info.resStdList[0] == 0) {	/* make a default standard resolutions for 200 and 300dpi */
		DBG(DBG_warning,
		    "attach: no standard resolutions reported\n");
	...
	if (dev->info.winWidth <= 0) {
		...
		DBG(DBG_warning,
		    "attach: invalid window width reported, using %d\n",
		    dev->info.winWidth);
	}
	if (dev->info.winHeight <= 0) {
		dev->info.winHeight = dev->info.resBasicY * 14;
		DBG(DBG_warning,
		    "attach: invalid window height reported, using %d\n",
		    dev->info.winHeight);
	}
	/* 4692 / 400 * 25.4 = 297 */
	...
	dev->info.overflow_support =
		((jbuf.functions & 0x01) == 0x01) ? TRUE : FALSE;
	dev->info.lineart_support =
		((jbuf.functions & 0x02) == 0x02) ? TRUE : FALSE;
	dev->info.dither_support =
		((jbuf.functions & 0x04) == 0x04) ? TRUE : FALSE;
	dev->info.grayscale_support =
		((jbuf.functions & 0x08) == 0x08) ? TRUE : FALSE;
...
parse_configuration_file(FILE * fp)
{
	...
	} else {		/*parse configuration file */
		for (linenumber = 0;
		     sanei_config_read(line, sizeof(line), fp);
		     linenumber++) {
			DBG(DBG_proc,
			...
			*(++t) = '\0';	/*trim trailing space */
			...
			if ((t = strstr(s, "scsi ")) != NULL) {
				...
				DBG(DBG_proc,
				    ...
				    linenumber, line);
				sanei_config_attach_matching_devices(t,
								     attach_one_scsi);
				...
				/* if(strstr(t,"FLAG_VALUE")!=NULL) FLAG_VALUE=TRUE; */
			} else {
				DBG(DBG_proc,
	  ...
	  SANE_Auth_Callback __sane_unused__ authorize)
{
	...
	if ((fp = sanei_config_open(HS2P_CONFIG_FILE)) != NULL) {
		...
		DBG(DBG_sane_init,
		    "> sane_init: No config file \"%s\" present!\n",
		    HS2P_CONFIG_FILE);
	}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		default:
			DBG(DBG_proc,
			    "sane_control_option: set_value %s [#%d]\n", name,
			    option);
		}
		...
		case OPT_CUSTOM_GAMMA:
			s->val[OPT_CUSTOM_GAMMA].w = *(int *) val;
		...
		case OPT_RESOLUTION:
			...
			s->val[option].w = *(int *) val;
			s->val[OPT_X_RESOLUTION].w = *(int *) val;
			s->val[OPT_Y_RESOLUTION].w = *(int *) val;
			return SANE_STATUS_GOOD;
		case OPT_SCAN_SOURCE:
			/* a string option */
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		width = (int) (SANE_UNFIX(s->val[OPT_BR_X].w) -
			       SANE_UNFIX(s->val[OPT_TL_X].w));
		length = (int) (SANE_UNFIX(s->val[OPT_BR_Y].w) -
		...
		DBG(DBG_proc,
		    "sane_get_parameters: scanning, so can't get params\n");
...
set_window_data(HS2P_Scanner * s, SWD * wbuf)
{
	...
	DBG(DBG_proc,
	    ">> set_window_data: sizeof(*wbuf)=%lu; window len=%lu\n",
	...
	if (xres < s->hw->info.resMinX || xres > s->hw->info.resMaxX) {
		DBG(DBG_error, "XRESOLUTION %d IS NOT WITHIN [%d, %d]\n",
		    xres, s->hw->info.resMinX, s->hw->info.resMaxX);
		return (SANE_STATUS_INVAL);
	}
	...
	if (yres < s->hw->info.resMinY || yres > s->hw->info.resMaxY) {
		DBG(DBG_error, "YRESOLUTION %d IS NOT WITHIN [%d, %d]\n",
		    yres, s->hw->info.resMinY, s->hw->info.resMaxY);
		return (SANE_STATUS_INVAL);
	}
	...
	if (strcmp(s->val[OPT_SCAN_SOURCE].s, scan_source_list[FB]) == 0) {	/* FB */
		xmax = 298;	/*mm */
		ymax = 432;
	} else {		/* ADF */
		xmax = 298;
	...
	if (number <= 0 || number > xmax) {
		DBG(DBG_error,
		    "NOT WITHIN BOUNDS: ulx=%ld width=%ld sum=%ld\n", ulx,
		    width, number);
		return (SANE_STATUS_INVAL);
	}
	number = uly + length;
	if (number <= 0 || number > ymax) {
		DBG(DBG_error,
		    "NOT WITHIN BOUNDS: uly=%ld length=%ld sum=%ld\n", uly,
		    length, number);
		return (SANE_STATUS_INVAL);
	}
	...
	nwin = (s->val[OPT_DUPLEX].w == TRUE) ? 2 : 1;
	for (i = 0; i < nwin; i++) {
		data = &(wbuf->data[i]);
		data->window_id = i;
		...
		if (s->image_composition == HALFTONE) {	/* Then let's use pattern selected by user */
			data->halftone_code =
				(get_halftone_code_id
				 (s->val[OPT_HALFTONE_CODE].s) ==
				 0) ? DITHER : ERROR_DIFFUSION;
			data->halftone_id =
				get_halftone_pattern_val(s->
							 val
		...
		data->byte29 |=
			(paddingtype
			 [get_paddingtype_id(s->val[OPT_PADDING_TYPE].s)].
			 val & 0x07);
		...
		if (s->hw->info.hasIPU) {
			...
			data->automatic_separation =
				get_auto_separation_val(s->val[OPT_AUTOSEP].
							s);
			/* Byte 50: Automatic Binarization */
			data->automatic_binarization =
				get_auto_binarization_val(s->val[OPT_AUTOBIN].
							  s);
...
{				/* begin scanning */
	...
	if (s->another_side) {
		...
		s->scanning = TRUE;
		s->cancelled = FALSE;
		s->another_side = FALSE;	/* This is side 2, so no more sides */
		DBG(DBG_proc, "<< sane_start\n");
		return (SANE_STATUS_GOOD);
	}
	...
	if ((status = sane_get_parameters(s, 0)) != SANE_STATUS_GOOD) {	/* get preliminary parameters */
	...
	if (s->val[OPT_CUSTOM_GAMMA].b) {	/* Custom Gamma needs to be sent to scanner */
		...
		s->val[OPT_CUSTOM_GAMMA].b = FALSE;
	}
	...
	if (mode != FLATBED) {
		if ((status =
		     read_adf_status(s->fd,
				     &adf_status)) != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sane_start: error with read_adf_status: %s\n",
			    sane_strstatus(status));
			return (status);
		}
	...
	s->another_side = (mode == DUPLEX) ? TRUE : FALSE;
	/* Number of bytes to read for one side of sheet */
...
sane_cancel(SANE_Handle handle)
{
	...
	if (s->scanning) {	/* if batchmode is enabled, then call set_window to abort the batch 
...
{
	DBG(DBG_proc,
	    ">> sane_set_io_mode (handle = %p, non_blocking = %d)\n", handle,
	    non_blocking);
