...
sanei_w_space(Wire * w, size_t howmuch)
{
	...
	if (w->buffer.curr + howmuch > w->buffer.end) {
		...
		case WIRE_ENCODE:
			...
			while (nbytes > 0) {
				nwritten =
					(*w->io.write) (fd, w->buffer.curr,
							nbytes);
				if (nwritten < 0) {
					DBG(1,
					    "sanei_w_space: ENCODE: write failed (%d)\n",
					    errno);
					w->status = errno;
					return;
				}
			...
			DBG(4,
			    "sanei_w_space: ENCODE: free buffer is now %lu\n",
			    (u_long) w->buffer.size);
			break;
		...
		case WIRE_DECODE:
			...
			if ((signed) left_over < 0) {
				DBG(1,
				    "sanei_w_space: DECODE: buffer underflow\n");
				return;
			}

			if (left_over) {
				DBG(4,
				    "sanei_w_space: DECODE: %lu bytes left in buffer\n",
				    (u_long) left_over);
				memmove(w->buffer.start, w->buffer.curr,
					left_over);
			}
			w->buffer.curr = w->buffer.start;
			...
			do {
				...
				if (nread <= 0) {
					DBG(2,
					    "sanei_w_space: DECODE: no data received (%d)\n",
					    errno);
					if (nread == 0)
						errno = EINVAL;
					w->status = errno;
		...
		case WIRE_FREE:
			DBG(4,
			    "sanei_w_space: FREE: doing nothing for free operation\n");
			break;
		}
	}
...
void
sanei_w_array(Wire * w, int *len_ptr, void **v,
	      WireCodecFunc w_element, size_t element_size)
{
	...
	if (w->direction == WIRE_FREE) {
		if (*len_ptr && *v) {
			DBG(4,
			    "sanei_w_array: FREE: freeing array (%d elements)\n",
			    *len_ptr);
			val = *v;
			for (i = 0; i < *len_ptr; ++i) {
				(*w_element) (w, val);
			...
			DBG(1,
			    "sanei_w_array: FREE: tried to free array but *len_ptr or *v "
			    "was NULL\n");
		...
		*len_ptr = len;
		if (len) {
			if (((unsigned int) len) > MAX_MEM
			    ...
			    MAX_MEM) {
				DBG(0,
				    "sanei_w_array: DECODE: maximum amount of allocated memory "
			...
			*v = malloc(len * element_size);
			if (*v == 0) {
				...
				DBG(1,
				    "sanei_w_array: DECODE: not enough free memory\n");
				w->status = ENOMEM;
				return;
			}
...
sanei_w_ptr(Wire * w, void **v, WireCodecFunc w_value, size_t value_size)
{
	...
	if (w->direction == WIRE_FREE) {
		if (*v && value_size) {
			...
			DBG(1,
			    "sanei_w_ptr: FREE: tried to free value but *v or value_size "
			    "was NULL\n");
	...
	if (!is_null) {
		if (w->direction == WIRE_DECODE) {
			DBG(4,
			    "sanei_w_ptr: DECODE: receive data pointed at\n");
			if (value_size > MAX_MEM) {
				DBG(0,
				    "sanei_w_ptr: DECODE: maximum amount of allocated memory "
				    "exceeded (limit: %u, new allocation: %lu, total: %lu bytes)\n",
				    MAX_MEM, (unsigned long) value_size,
				    (unsigned long) (w->allocated_memory +
						     value_size));
				w->status = ENOMEM;
				return;
			}
			...
			*v = malloc(value_size);
			if (*v == 0) {
				/* Malloc failed, so return an error. */
				DBG(1,
				    "sanei_w_ptr: DECODE: not enough free memory\n");
				w->status = ENOMEM;
				return;
			}
			w->allocated_memory += value_size;
...
sanei_w_range(Wire * w, SANE_Range * v)
{
	...
	if (w->direction != WIRE_FREE)
		DBG(4, "sanei_w_range: min/max/step = %f/%f/%f\n",
		    SANE_UNFIX(v->min), SANE_UNFIX(v->max),
		    SANE_UNFIX(v->quant));
}
...
sanei_w_device_ptr(Wire * w, SANE_Device ** v)
{
	...
	sanei_w_ptr(w, (void **) v, (WireCodecFunc) sanei_w_device,
		    sizeof(**v));
	if (w->direction != WIRE_FREE)
...
sanei_w_parameters(Wire * w, SANE_Parameters * v)
{
	...
	if (w->direction != WIRE_FREE)
		DBG(4,
		    ...
		    "%d/%d/%d/%d/%d/%d\n", v->format, v->last_frame,
		    v->bytes_per_line, v->pixels_per_line, v->lines,
		    v->depth);
}
...
sanei_w_set_dir(Wire * w, WireDirection dir)
{
	...
	if (w->direction == WIRE_DECODE && w->buffer.curr != w->buffer.end)
		DBG(1,
		    "sanei_w_set_dir: WARNING: will delete %lu bytes from buffer\n",
		    (u_long) (w->buffer.end - w->buffer.curr));
	flush(w);
	w->direction = dir;
