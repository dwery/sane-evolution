 ...
 */
static int dpi_ranges[] =
	{ 75, 100, 150, 200, 300, 400, 600, 800, 1200, 2400 };
...
static SANE_Bool m_fStart = FALSE;
...
usb_SetAsicDpiX(Plustek_Device * dev, u_short xdpi)
{
	...
	if (scaps->OpticDpi.x == 1200 &&
	    ...
	    xdpi < 150 && scanning->sParam.bDataType == SCANDATATYPE_BW) {
		xdpi = 150;
...
usb_SetAsicDpiY(Plustek_Device * dev, u_short ydpi)
{
	...
	if ((hw->motorModel == MODEL_Tokyo600) ||
		...
		/* return wDpi; */
	} else if (sCaps->wFlags & DEVCAPSFLAG_Adf
...
{
	sp->Size.dwValidPixels =
		sp->Size.dwPixels * sp->PhyDpi.x / sp->UserDpi.x;
	...
	if (sp->bBitDepth == 1) {
		...
		sp->Size.dwPhyPixels =
			(sp->Size.dwValidPixels + 1UL) & 0xfffffffeUL;
		sp->Size.dwPhyBytes =
			sp->Size.dwPhyPixels * sp->bChannels + 2UL;

		/* need to be adjusted fo CIS devices in color mode */
		if (usb_IsCISDevice(dev)
		    && (sp->bDataType == SCANDATATYPE_Color)) {
		...
		sp->Size.dwPhyBytes =
			sp->Size.dwPhyPixels * 2 * sp->bChannels + 2UL;
...
/* HEINER: check ADF stuff... */
	if (sp->bCalibration != PARAM_Gain &&
		...
		wDataPixelStart = 2550 * sCaps->OpticDpi.x / 300UL -
			(u_short) (m_dHDPIDivider * sp->Size.dwValidPixels +
				   0.5);
	else
#endif
	...
	if (sp->bCalibration == PARAM_Scan) {
		...
		if (sp->bDataType != SCANDATATYPE_Color)
			sp->Origin.y += (u_long) (300UL *
						  sCaps->bSensorDistance /
						  sCaps->OpticDpi.y);
	}

	sp->Origin.y =
		(u_short) ((u_long) sp->Origin.y * hw->wMotorDpi / 300UL);
...
usb_GetMCLKDiv(Plustek_Device * dev)
{
	...
	pixelsperline =
		(int) ((256 * regs[0x24] + regs[0x25] - 256 * regs[0x22] -
			regs[0x23])
		       * pixelbits / (hdpi * 8));
	mclkdiv =
		(int) ceil((double) MCLKDIV_SCALING * pixelsperline *
			   CRYSTAL_FREQ / ((double) 8. * m_wLineLength *
					   dev->transferRate));
...
usb_GetMCLKDivider(Plustek_Device * dev, ScanParam * pParam)
{
	...
	if (m_dHDPIDivider * m_dMCLKDivider >= 5.3 /*6 */ )
		...
		m_bIntTimeAdjust = ceil(5.3 /*6.0 */  /
					(m_dHDPIDivider * m_dMCLKDivider));
	...
	if (pParam->bCalibration == PARAM_Scan) {
		...
		if (pParam->bBitDepth > 8) {
			...
			while (pParam->Size.dwPhyBytes >
			       (m_dMCLKDivider * m_bCM * m_wLineLength / 6 *
				9 / 10) * (1 + m_bIntTimeAdjust)) {
				m_bIntTimeAdjust++;
			}
	...
	DBG(_DBG_INFO2,
	    "* Integration Time Adjust = %u (HDPI=%.3f,MCLKD=%.3f)\n",
...
usb_GetStepSize(Plustek_Device * dev, ScanParam * pParam)
{
	...
	if (m_bIntTimeAdjust != 0) {
		m_wStepSize =
			(u_short) (((u_long) pParam->PhyDpi.y *
				    m_wLineLength * m_bLineRateColor *
				    (m_bIntTimeAdjust +
				     1)) / (4 * hw->wMotorDpi *
					    m_bIntTimeAdjust));
		...
		m_wStepSize =
			(u_short) (((u_long) pParam->PhyDpi.y *
				    m_wLineLength * m_bLineRateColor) / (4 *
									 hw->
									 wMotorDpi));
	}
	...
	if (!_IS_PLUSTEKMOTOR(hw->motorModel)) {
		...
		if (md[idx].scan_lines_per_line > 1) {
			...
			if ((pParam->bBitDepth > 8) &&
				...
				DBG(_DBG_INFO2,
				    "* Line Skipping : 0x43=0x%02x, 0x44=0x%02x\n",
				    regs[0x43], regs[0x44]);
			}
		}
	} else {
...
usb_GetPauseLimit(Plustek_Device * dev, ScanParam * pParam)
{
	...
	m_dwPauseLimit = usb_max(usb_min(m_dwPauseLimit,
					 (u_long) ceil(pParam->Size.
						       dwTotalBytes /
						       1024.0)), 2);

	regs[0x4e] =
		(u_char) floor((m_dwPauseLimit * 512.0) /
			       (2.0 * hw->wDRAMSize));
	...
	DBG(_DBG_INFO2,
	    "* PauseLimit = %lu, [0x4e] = 0x%02x, [0x4f] = 0x%02x\n",
...
usb_GetScanLinesAndSize(Plustek_Device * dev, ScanParam * pParam)
{
	...
	pParam->Size.dwPhyLines =
		(u_long) ceil((double) pParam->Size.dwLines *
	...
	pParam->Size.dwTotalBytes =
		pParam->Size.dwPhyBytes * pParam->Size.dwPhyLines;
...
*/
		...
		usb_MotorSelect(dev, TRUE);
		usb_MotorOn(dev, TRUE);
	}
	...
	DBG(_DBG_INFO2,
	    "* FFStepSize = %u, [0x48] = 0x%02x, [0x49] = 0x%02x\n",
	...
	if (!usb_IsSheetFedDevice(dev)) {
		...
		if (pParam->bCalibration == PARAM_Scan
			...
			u_long lines =
				pParam->Size.dwPhyLines + scan->bLinesToSkip +
				scan->dwLinesDiscard + 5;
			u_short scansteps =
				(u_short) ceil((double) lines *
					       hw->wMotorDpi /
					       pParam->PhyDpi.y);
			DBG(_DBG_INFO, "* Scansteps=%u (%lu*%u/%u)\n",
			    scansteps, lines, hw->wMotorDpi,
			    pParam->PhyDpi.y);
	...
	_UIO(sanei_lm983x_write(dev->fd, 0x03, &regs[0x03], 3, TRUE));
	_UIO(sanei_lm983x_write
	     (dev->fd, 0x08, &regs[0x08], 0x7f - 0x08 + 1, TRUE));
...
usb_ScanBegin(Plustek_Device * dev, SANE_Bool auto_park)
{
	...
	if (_LM9831 == hw->chip) {
		...
		for (;;) {
			...
			if (m_bOldScanData) {
				...
				u_long dwBytesToRead =
					m_bOldScanData * hw->wDRAMSize * 4;
				u_char *pBuffer =
					malloc(sizeof(u_char) *
					       dwBytesToRead);

				DBG(_DBG_INFO,
				    "Flushing cache - %lu bytes (bOldScanData=%u)\n",
	...
	m_fStart = m_fFirst = TRUE;	/* Prepare to read */
	...
	DBG(_DBG_INFO2, ">>> INCH=%u, DOY=%u\n", inches,
	    sc->Normal.DataOrigin.y);
	if (inches > sc->Normal.DataOrigin.y)
		usb_WaitPos(dev, 150, FALSE);
...
usb_IsDataAvailableInDRAM(Plustek_Device * dev)
{
	...
	for (;;) {
		...
		_UIO(sanei_lm983x_read
		     (dev->fd, 0x01, a_bBand, 3, FALSE));
		...
		if (usb_IsEscPressed()) {
			DBG(_DBG_INFO,
			    "usb_IsDataAvailableInDRAM() - Cancel detected...\n");
			return FALSE;
		}
		...
		if (a_bBand[0] > m_bOldScanData) {
			...
			if (m_pParam->bSource != SOURCE_Reflection)
				...
				usleep(1000 *
				       (30 * regs[0x08] *
					dev->usbDev.Caps.OpticDpi.x / 600));
			else
				usleep(1000 *
				       (20 * regs[0x08] *
					dev->usbDev.Caps.OpticDpi.x / 600));
...
usb_ScanReadImage(Plustek_Device * dev, void *pBuf, u_long dwSize)
{
	...
	if (m_fFirst) {
		...
		sanei_lm983x_write(dev->fd, 0x48, &regs[0x48], 2, TRUE);
	}
/* HEINER: ADF */
#if 0
	if (ScanInf.m_fADF == 1 && Scanning.sParam.bCalibration == PARAM_Scan) {
		if (dwBytes) {
			...
			if (!dwBytes) {
				...
				ScanInf.m_fADF++;
				if (dwSize > dw)
					ScanReadImage((PBYTE) pBuf + dw,
						      dwSize - dw);
			}
			return fRet;
		} else if (!Hardware.SensorPaper()) {
			dwBytes =
				(Scanning.sParam.PhyDpi.y * 18 / 25) *
...
#endif
	res = sanei_lm983x_read(dev->fd, 0x00, (u_char *) pBuf, dwSize,
				FALSE);
