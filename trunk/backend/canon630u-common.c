...
static int
read_poll_flag(int fd, unsigned int addr, unsigned int mask, unsigned int val)
{
	SANE_Status status;
	byte result = 0;
...
static const byte seq002[] =
	{ /*r08 */ 0x04, /*300 dpi */ 0x1a, 0x00, 0x0d, 0x4c, 0x2f, 0x00,
	0x01,
/*r10 */ 0x07, 0x04, 0x05, 0x06, 0x07, 0x00, 0x00, 0x00, 0x03, 0x00,
	0x00, 0x00, 0x03, 0x25, 0x00,
	0x4b, /*r20 */ 0x15, 0xe0, /*data px start */ 0x00, 0x4b,
	/*data px end */ 0x14, 0x37, 0x15, 0x00
};

static const byte seq003[] =
	{ 0x02, 0x00, 0x00, /*lights out */ 0x03, 0xff, 0x00, 0x01, 0x03,
	0xff,
	0x00, 0x01, 0x03, 0xff, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01,
	0x01, 0x00, 0x00, 0x00, 0x00, 0x06,
	0x1d, 0x00, 0x13, 0x04, 0x1a, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00,
	0x57, 0x02, 0x00, 0x3c, 0x35, 0x94,
...
#define BUFSIZE 0xf000
	...
	while (line < s->height) {
		...
		numbytes =
			read_bulk_size(s->fd, datasize, 0, ptr,
				       BUFSIZE - level);
		if (numbytes < 0) {
...
scan(CANON_Handle * opt)
{
	...
	for (temp = 0; temp < 0x0400; temp++)
		...
		buf[temp] =
			(double) 255.0 *exp(log((temp + 0.5) / 1023.0) /
					    opt->gamma)
	+ 0.5;
...

	{
		...
		int stepsize, faststep = 0x0104, reverse = 0x28, phase, pwm =
			0x1f;
		switch (opt->resolution) {
		case 75:
			stepsize = 0x0106;
		...
		write_word(fd, SKIP_STEPS,
			   0x017a /* cal strip */  + opt->y1 * 2);
...
CANON_set_scan_parameters(CANON_Handle * scan,
			  ...
			  const int right,
			  const int bottom,
			  const int res, const int gain, const double gamma)
{
...
CANON_open_device(CANON_Handle * scan, const char *dev)
{
	...
	if (sanei_usb_get_vendor_product(scan->fd, &vendor, &product) !=
	    SANE_STATUS_GOOD) {
		DBG(1,
		    "CANON_open_device: sanei_usb_get_vendor_product failed\n");
	...
	if ((vendor != 0x04a9) || (product != 0x2204)) {
		DBG(1,
		    "CANON_open_device: incorrect vendor/product (0x%x/0x%x)\n",
		    vendor, product);
		sanei_usb_close(scan->fd);
		scan->fd = -1;
		return SANE_STATUS_UNSUPPORTED;
