...
static int Calib_test(struct st_device *dev, SANE_Byte * Regs,
			   struct st_calibration *myCalib,
...
static SANE_Byte data_bitget(SANE_Byte * address, int mask);
static void data_bitset(SANE_Byte * address, int mask, SANE_Byte data);
static int data_lsb_get(SANE_Byte * address, int size);
static void data_lsb_set(SANE_Byte * address, int data, int size);
static void data_msb_set(SANE_Byte * address, int data, int size);
static void data_wide_bitset(SANE_Byte * address, int mask,
			     int data);
...
static int RTS_Enable_CCD(struct st_device *dev, SANE_Byte * Regs,
			       int channels);

/* DMA management commands */
static int RTS_DMA_Cancel(struct st_device *dev);
static int RTS_DMA_CheckType(struct st_device *dev, SANE_Byte * Regs);
...
static int RTS_DMA_SetType(struct st_device *dev, SANE_Byte * Regs,
				SANE_Byte ramtype);
...
static int RTS_GetScanmode(struct st_device *dev, int scantype,
				int colormode, int resolution);
static int RTS_GetImage(struct st_device *dev, SANE_Byte * Regs,
			     struct st_scanparams *scancfg,
...
static int RTS_GetImage_GetBuffer(struct st_device *dev, double dSize,
				       SANE_Byte * buffer,
				       double *transferred);
static int RTS_GetImage_Read(struct st_device *dev, SANE_Byte * buffer,
...
static int RTS_ReadRegs(USB_Handle usb_handle, SANE_Byte * buffer);
static int RTS_WriteRegs(USB_Handle usb_handle, SANE_Byte * buffer);
...
static int RTS_Setup(struct st_device *dev, SANE_Byte * Regs,
			  struct st_scanparams *myvar,
			  struct st_hwdconfig *hwdcfg,
			  struct st_gain_offset *gain_offset);
static void RTS_Setup_Channels(struct st_device *dev, SANE_Byte * Regs,
			       struct st_scanparams *scancfg,
			       int mycolormode);
static void RTS_Setup_Coords(SANE_Byte * Regs, int iLeft, int iTop,
			     int width, int height);
static int RTS_Setup_Depth(SANE_Byte * Regs,
				struct st_scanparams *scancfg,
				int mycolormode);
...
static int RTS_Setup_Line_Distances(struct st_device *dev,
					 ...
					 int mycolormode,
					 int arrangeline);
static int RTS_Setup_Motor(struct st_device *dev, SANE_Byte * Regs,
				struct st_scanparams *myvar,
				int somevalue);
...
static void SetLock(USB_Handle usb_handle, SANE_Byte * Regs,
		    SANE_Byte Enable);
static int fn3330(struct st_device *dev, SANE_Byte * Regs,
...
static int Reading_NotifySize(struct st_device *dev, int data,
				   int size);
static int Reading_Start(struct st_device *dev);
static int Reading_Wait(struct st_device *dev,
			     SANE_Byte Channels_per_dot,
			     SANE_Byte Channel_size, int size,
			     int * last_amount, int seconds,
			     SANE_Byte op);

static int Read_Image(struct st_device *dev, int buffer_size,
			   SANE_Byte * buffer, int * transferred);
static int Read_ResizeBlock(struct st_device *dev, SANE_Byte * buffer,
				 int buffer_size,
				 int * transferred);
static int Read_Block(struct st_device *dev, int buffer_size,
			   SANE_Byte * buffer, int * transferred);
static int Read_NonColor_Block(struct st_device *dev,
				    SANE_Byte * buffer, int buffer_size,
				    SANE_Byte ColorMode,
				    int * transferred);
...
static int Refs_Counter_Save(struct st_device *dev, SANE_Byte data);
static int Refs_Detect(struct st_device *dev, SANE_Byte * Regs,
			    int resolution_x, int resolution_y);
static int Refs_Load(struct st_device *dev, int * left_leading,
			  int * start_pos);
static int Refs_Save(struct st_device *dev, int left_leading,
			  int start_pos);
static int Refs_Set(struct st_device *dev, SANE_Byte * Regs,
			 struct st_scanparams *myscan, int autoref);
...
static int PWM_GetDutyCycle(struct st_device *dev, int * data);
static int PWM_GetReady(struct st_device *dev, int resolution,
			     int * last_colour, double diff,
			     long totaltime, int interval,
			     int value);
static int PWM_SetDutyCycle(struct st_device *dev, int duty_cycle);
static int PWM_use(struct st_device *dev, int enable);

/* Fixed Pulse-Width Modulation functions */
static int FixedPWM_Get(struct st_device *dev, SANE_Byte type);
static int FixedPWM_Save(struct st_device *dev, int fixedpwm);
static int FixedPWM_SaveStatus(struct st_device *dev, SANE_Byte status);
...
static int GainOffset_IncCount(struct st_device *dev, int * arg1);
static int GainOffset_Save(struct st_device *dev, int * offset,
				SANE_Byte * gain);
static int GainOffset_SaveCount(struct st_device *dev, SANE_Byte data);

/* Gamma functions*/
static int Gamma_AllocTable(SANE_Byte * table);
static int Gamma_Apply(struct st_device *dev, SANE_Byte * Regs,
			    struct st_scanparams *scancfg,
			    struct st_hwdconfig *hwdcfg,
			    struct st_gammatables *mygamma);
static void Gamma_FreeTables(void);
static int Gamma_SendTables(struct st_device *dev, SANE_Byte * Regs,
				 SANE_Byte * gammatable, int size);
static int Gamma_GetTables(struct st_device *dev,
				SANE_Byte * Gamma_buffer);

/* Lamp functions */
static void Lamp_OverDrive(struct st_device *dev, int itime,
			   int lamp);
static SANE_Byte Lamp_GetGainMode(struct st_device *dev, int resolution,
				  SANE_Byte scantype);
static int Lamp_GetStatus(struct st_device *dev, SANE_Byte * flb_lamp,
			       SANE_Byte * tma_lamp);
static int Lamp_IsAtHome(struct st_device *dev, SANE_Byte * Regs);
static void Lamp_SetGainMode(struct st_device *dev, SANE_Byte * Regs,
			     int resolution, SANE_Byte gainmode);
static int Lamp_SetStatus(struct st_device *dev, SANE_Byte * Regs,
			       int turn_on, int lamp);
static int Lamp_SetStatus2(struct st_device *dev, int turnon,
				int lamp);
static int Lamp_SetTimer(struct st_device *dev, int minutes);
static int Lamp_SetupPwm(struct st_device *dev, int lamp);
static int Lamp_WaitForPWM(struct st_device *dev, int resolution,
				int lamp);
static int Lamp_Warmup(struct st_device *dev, SANE_Byte * Regs,
			    int lamp);

/* Motor functions */
static SANE_Byte *Motor_AddStep(SANE_Byte * steps, int * bwriten,
				int step);
static int Motor_Change(struct st_device *dev, SANE_Byte * buffer,
			     SANE_Byte value);
static int Motor_GetFromResolution(int resolution);
static int Motor_Move(struct st_device *dev, SANE_Byte * Regs,
			   struct st_motormove *mymotor,
			   struct st_motorpos *mtrpos);
static int Motor_ParkHome(struct st_device *dev, int bWait,
			       int movement);
static void Motor_Release(struct st_device *dev);
static int Motor_Relocate(struct st_device *dev, int speed,
			       int direction, int ypos);
static int Motor_Setup_Steps(struct st_device *dev, SANE_Byte * Regs,
				  int mysetting);

/* motorcurve functions */
static int Motorcurve_Equal(struct st_device *dev,
...
static struct st_motorcurve **Motorcurve_Parse(int * mtc_count,
					       int * buffer);

/* Functions to arrange scanned lines */
static int Arrange_Colour(struct st_device *dev, SANE_Byte * buffer,
			       int buffer_size, int * transferred);
static int Arrange_Compose(struct st_device *dev, SANE_Byte * buffer,
				int buffer_size, int * transferred);
static int Arrange_NonColour(struct st_device *dev, SANE_Byte * buffer,
				  int buffer_size,
				  int * transferred);

/* Composing RGB triplet functions */
static void Triplet_Gray(SANE_Byte * pPointer1, SANE_Byte * pPointer2,
			 SANE_Byte * buffer, int channels_count);
...
static void Triplet_Colour_HRes(SANE_Byte * pRed1, SANE_Byte * pGreen1,
				...
				SANE_Byte * buffer, int Width);
static void Triplet_Colour_LRes(int Width, SANE_Byte * Buffer,
				SANE_Byte * pChannel1, SANE_Byte * pChannel2,
				SANE_Byte * pChannel3);

/* Timing functions */
static int Timing_SetLinearImageSensorClock(SANE_Byte * Regs,
						 struct st_cph *cph);
...
static int Resize_Increase(SANE_Byte * to_buffer,
				int to_resolution, int to_width,
				SANE_Byte * from_buffer,
				int from_resolution,
				int from_width, int myresize_mode);
static int Resize_Decrease(SANE_Byte * to_buffer,
				int to_resolution, int to_width,
				SANE_Byte * from_buffer,
				int from_resolution,
				int from_width, int myresize_mode);
...
static int Calibrate_Malloc(struct st_cal2 *calbuffers,
				 ...
				 struct st_calibration *myCalib,
				 int somelength);
static int Calib_ReadTable(struct st_device *dev, SANE_Byte * table,
				int size, int data);
static int Calib_WriteTable(struct st_device *dev, SANE_Byte * table,
				 int size, int data);
...
static int Calib_PAGain(struct st_device *dev,
			     struct st_calibration_config *calibcfg,
			     int gainmode);
static int Calibration(struct st_device *dev, SANE_Byte * Regs,
			    struct st_scanparams *scancfg,
			    struct st_calibration *myCalib, int value);

/* function for white shading correction */
static int WShading_Calibrate(struct st_device *dev, SANE_Byte * Regs,
				   struct st_calibration *myCalib,
				   struct st_scanparams *scancfg);
static void WShading_Emulate(SANE_Byte * buffer, int * chnptr,
			     int size, int depth);

/* functions for shading calibration */
static int Shading_apply(struct st_device *dev, SANE_Byte * Regs,
			      struct st_scanparams *myvar,
			      struct st_calibration *myCalib);
static int Shading_black_apply(struct st_device *dev, SANE_Byte * Regs,
				    int channels,
				    struct st_calibration *myCalib,
				    struct st_cal2 *calbuffers);
static int Shading_white_apply(struct st_device *dev, SANE_Byte * Regs,
				    int channels,
				    struct st_calibration *myCalib,
				    struct st_cal2 *calbuffers);
...
static void Split_into_12bit_channels(SANE_Byte * destino,
				      SANE_Byte * fuente, int size);
static int Scan_Read_BufferA(struct st_device *dev,
				  int buffer_size, int arg2,
				  SANE_Byte * pBuffer,
				  int * bytes_transfered);

static int Get_Scanner_Buffer_Size(struct st_device *dev,
					SANE_Byte channels_per_dot,
					int channel_size);
static int Bulk_Operation(struct st_device *dev, SANE_Byte op,
			       int buffer_size, SANE_Byte * buffer,
			       int * transfered);
static int Get_PAG_Value(SANE_Byte scantype, SANE_Byte color);
...
static int SetMultiExposure(struct st_device *dev, SANE_Byte * Regs);
...
static int SetScanParams(struct st_device *dev, SANE_Byte * Regs,
			      struct st_scanparams *scancfg,
			      struct st_hwdconfig *hwdcfg);
static int IsScannerLinked(struct st_device *dev);

static int Read_FE3E(struct st_device *dev, SANE_Byte * destino);

static double get_shrd(double value, int desp);
static char get_byte(double value);
/*static int RTS8822_GetRegisters(SANE_Byte *buffer);*/
...
RTS_WriteRegs(USB_Handle usb_handle, SANE_Byte * buffer)
{
	...
	if (buffer != NULL)
		rst = Write_Buffer(usb_handle, 0xe800, buffer,
...
RTS_ReadRegs(USB_Handle usb_handle, SANE_Byte * buffer)
{
	...
	if (buffer != NULL)
		rst = Read_Buffer(usb_handle, 0xe800, buffer,
				  RT_BUFFER_LEN * sizeof(SANE_Byte));
...
Motorcurve_Get(struct st_device *dev, int motorcurve,
	       int direction, int itype)
{
...
static int
Motorcurve_Equal(struct st_device *dev, int motorsetting,
	...
	if ((crv1 != NULL) && (crv2 != NULL)) {
		if (crv1->step_count == crv2->step_count) {
			...
			if (crv1->step_count > 0) {
				...
				while ((a < crv1->step_count)
				       && (rst == TRUE)) {
					rst = (crv1->step[a] ==
					       crv2->step[a]) ? TRUE : FALSE;
					a++;
				}
			}
		}
...
Motorcurve_Parse(int * mtc_count, int * buffer)
{
...
!= 0)
								...
								if (curve->
									...
									curve->step = (int *) malloc(sizeof(int) * curve->step_count);
									if (curve->step != NULL) {
										memcpy(curve->step, buffer, sizeof(int) * curve->step_count);
...
static void
Motorcurve_Free(struct st_motorcurve **motorcurves, int * mtc_count)
{
...
Load_Scanmodes(struct st_device *dev)
{
	...
	while ((cfg_scanmode_get(dev->sensorcfg->type, a, &reg) == OK)
	       && (rst == OK)) {
		mode = (struct st_scanmode *)
			malloc(sizeof(struct st_scanmode));
		if (mode != NULL) {
			...
			mode->ctpc =
				((mode->ctpc + 1) * mode->multiexposure) - 1;

			dev->scanmodes =
				(struct st_scanmode **) realloc(dev->
								scanmodes,
								(dev->
								 scanmodes_count
								 +
...
Load_Config(struct st_device *dev)
{
	...
	if (dev->sensorcfg->type == CCD_SENSOR)
		...
		usbfile =
			(RTS_Debug->usbtype ==
			 USB20) ? T_RTINIFILE : T_USB1INIFILE;
	else			/* cis */
		usbfile =
			(RTS_Debug->usbtype ==
			 USB20) ? S_RTINIFILE : S_USB1INIFILE;
	...
	arrangeline2 =
		get_value(SCAN_PARAM, ARRANGELINE, FIX_BY_HARD, usbfile);

	binarythresholdh =
		get_value(PLATFORM, BINARYTHRESHOLDH, 0x80, usbfile);
	binarythresholdl =
		get_value(PLATFORM, BINARYTHRESHOLDL, 0x7f, usbfile);
...
LoadImagingParams(struct st_device *dev, int inifile)
{
	...
	arrangeline =
		get_value(SCAN_PARAM, ARRANGELINE, FIX_BY_HARD, inifile);
	...
	pixeldarklevel =
		get_value(CALI_PARAM, PIXELDARKLEVEL, 0x00ffff, inifile);

	binarythresholdh =
		get_value(PLATFORM, BINARYTHRESHOLDH, 0x80, inifile);
	binarythresholdl =
		get_value(PLATFORM, BINARYTHRESHOLDL, 0x7f, inifile);
	       ...
	       int buffer_size, int * transferred)
{
	...
	DBG(DBG_FNC,
	    "> Arrange_Colour(*buffer, buffer_size=%i, *transferred)\n",
	    buffer_size);
	...
	if (scn->imagebuffer == NULL) {
		...
		if (mydistance != 0) {
			...
			scn->bfsize =
				(scn->bfsize + (mydistance * 2) +
				 1) * line_size;
		} else
			scn->bfsize = line_size * 2;

		/*ab3c */
		space = (((scn->bfsize / line_size) * bytesperline) >
		...
		if (Read_Block
		    (dev, scn->bfsize, scn->imagebuffer, transferred) != OK)
			return ERROR;
		...
		for (c = CL_RED; c <= CL_BLUE; c++) {
			if (mydistance == 0) {
				...
				if (scn->arrange_hres == FALSE) {
					if ((((dev->sensorcfg->line_distance *
					       scan2.resolution_y) * 2) /
					     dev->sensorcfg->resolution) == 1)
						scn->arrange_orderchannel =
							TRUE;
				...
				/*ac32 */
				scn->desp[c] =
					(dev->sensorcfg->rgb_order[c] *
					 (mydistance * line_size)) +
					(scn->channel_size * c);
		...
		for (c = CL_RED; c <= CL_BLUE; c++) {
			if (scn->arrange_hres == TRUE) {
				scn->pColour2[c] =
					scn->imagebuffer + scn->desp2[c];
				scn->pColour1[c] =
					scn->imagebuffer + scn->desp1[c];
			} else
				scn->pColour[c] =
					scn->imagebuffer + scn->desp[c];
		}
	}
		...
		if (scn->arrange_orderchannel == FALSE) {
			if (scn->arrange_hres == TRUE)
				Triplet_Colour_HRes(scn->pColour1[CL_RED],
						    ...
						    scn->pColour2[CL_BLUE],
						    buffer,
						    line_size /
						    (scn->channel_size * 3));
			else
				Triplet_Colour_LRes(line_size /
						    (scn->channel_size * 3),
						    buffer,
						    scn->pColour[CL_RED],
			...
			Triplet_Colour_Order(dev, scn->pColour[CL_RED],
					     scn->pColour[CL_GREEN],
					     scn->pColour[CL_BLUE], buffer,
					     line_size / (scn->channel_size *
							  3));
...
RTS_Scanner_SetParams(struct st_device *dev, struct params *param)
{
	...
	if ((param->coords.width != 0) && (param->coords.height != 0)) {
		...
		scan.depth =
			(param->colormode == CM_LINEART) ? 8 : param->depth;
		...
		hwdcfg.highresolution =
			(scan.resolution_x > 1200) ? TRUE : FALSE;
		hwdcfg.sensorevenodddistance =
			dev->sensorcfg->evenodd_distance;
	      ...
	      struct st_scanparams *scancfg, struct st_hwdconfig *hwdcfg)
{
	...
	if (mymode == -1) {
		...
		fitres = Scanmode_fitres(dev, hwdcfg->scantype,
					 scancfg->colormode,
		...
		if (fitres != -1) {
			...
			fitres = Scanmode_maxres(dev, hwdcfg->scantype,
						 scancfg->colormode);
		}
		...
		mymode = RTS_GetScanmode(dev, hwdcfg->scantype, myvalue,
					 scan2.resolution_x);
		if (mymode == -1)
			return ERROR;
		...
		mycoords.width =
			...
			dev->Resize->resolution_x;
		mycoords.top =
			(scan2.resolution_y * scancfg->coord.top) /
			dev->Resize->resolution_y;
		mycoords.height =
		...
		case CM_GRAY:
			...
			dev->Resize->mode =
				(scan2.depth == 8) ? RSZ_GRAYL : RSZ_GRAYH;
			dev->Resize->bytesperline =
				dev->Resize->towidth * channel_size;
			break;
		case CM_LINEART:
			if (dev->scanmodes[mymode]->samplerate == PIXEL_RATE)
				dev->Resize->towidth *= 2;
		...
		default:	/*CM_COLOR */
			...
			dev->Resize->mode =
				(scan2.depth ==
				 8) ? RSZ_COLOURL : RSZ_COLOURH;
			dev->Resize->bytesperline =
				scancfg->coord.width * (channel_count *
							channel_size);
			break;
		}
	} else {
	...
	else if ((mycolormode == CM_GRAY)
		 && (hwdcfg->highresolution == FALSE))
		arrangeline2 = 0;
	...
	if (mycolormode == CM_LINEART) {
		bytesperline =
			(dev->scanmodes[mymode]->samplerate ==
			 PIXEL_RATE) ? mycoords.width / 4 : mycoords.width /
			8;
		...
		channel_size = (scan2.depth == 8) ? 1 : 2;
		bytesperline =
			mycoords.width * (channel_count * channel_size);
		imagewidth3 = bytesperline / channel_count;
		lineart_width = imagewidth3 / channel_size;
		line_size =
			bytesperline -
			(aditional_dots * (channel_count * channel_size));
		dev->Resize->fromwidth =
			line_size / (channel_count * channel_size);
	}
	...
	myvalue =
		(dev->Resize->type ==
		 RSZ_NONE) ? line_size : dev->Resize->bytesperline;
	...
	if (scan.colormode == CM_LINEART) {
		if (mycolormode == 3) {
			...
			scancfg->bytesperline =
				(scancfg->bytesperline + 7) / 8;
		}
	}
...
static int
GainOffset_IncCount(struct st_device *dev, int * arg1)
{
	SANE_Byte count;
	int rst;
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		if ((count >= 0x0f) || (GainOffset_Get(dev) != OK)) {
			offset[CL_BLUE] = offset[CL_GREEN] = offset[CL_RED] =
				0;
...
GainOffset_Get(struct st_device *dev)
{
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		if (RTS_EEPROM_ReadByte(dev->usb_handle, 0x76, &checksum) ==
		    OK) {
			rst = OK;

			/* read gain and offset values from EEPROM */
			for (a = CL_RED; a <= CL_BLUE; a++) {
				if (RTS_EEPROM_ReadWord
				    (dev->usb_handle, 0x70 + (2 * a), &data)
				    == ERROR) {
					rst = ERROR;
					break;
				} else
			...
			/* check checksum */
			checksum =
				_B0(checksum + offset[CL_GREEN] +
				    offset[CL_BLUE] + offset[CL_RED]);
		} else
			rst = ERROR;
	} else
		rst = ERROR;
		...
		int resolution)
{
	...
	DBG(DBG_FNC,
	    ...
	    dbg_scantype(sct, scantype), dbg_colour(scm, colormode),
	    resolution, rst);
		...
		int resolution)
{
	...
	if (rst == -1) {
		...
		if ((colormode == CM_LINEART) || (colormode == 3))
			rst = RTS_GetScanmode(dev, scantype, CM_GRAY,
					      resolution);
	}
...
FixedPWM_SaveStatus(struct st_device *dev, SANE_Byte status)
{
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		if (RTS_EEPROM_ReadByte(dev->usb_handle, 0x007b, &mypwm) ==
		    OK) {
			mypwm = (status ==
				 FALSE) ? mypwm & 0x7f : mypwm | 0x80;

			if (RTS_EEPROM_WriteByte
			    (dev->usb_handle, 0x007b, mypwm) == OK)
				rst = OK;
		}
	}
...
static int
FixedPWM_Save(struct st_device *dev, int fixedpwm)
{
	int rst;
	...
	/* check if chipset supports accessing eeprom */
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0)
		rst = RTS_EEPROM_WriteByte(dev->usb_handle, 0x7b,
					   ((fixedpwm << 2) | 0x80));
	else
		rst = OK;
...
FixedPWM_Get(struct st_device *dev, SANE_Byte type)
{
	...
	DBG(DBG_FNC, "> FixedPWM_Get(type=%s): %i\n",
	    dbg_scantype(sdebug, type), rst);

	return rst;
}

static int
Lamp_SetupPwm(struct st_device *dev, int lamp)
{
	int rst = OK;
...
static int
PWM_GetDutyCycle(struct st_device *dev, int * data)
{
	SANE_Byte a;
	int rst = ERROR;
...
static int
PWM_SetDutyCycle(struct st_device *dev, int duty_cycle)
{
	SANE_Byte *Regs;
	int rst;
	...
	if (Regs != NULL) {
		if (RTS_ReadRegs(dev->usb_handle, Regs) == OK) {
			...
			data_bitset(&dev->init_regs[0x148], 0x7f,
				    Regs[0x148]);
			data_bitset(&dev->init_regs[0x1e0], 0x3f,
				    Regs[0x1e0]);

			Write_Byte(dev->usb_handle, 0xe948, Regs[0x0148]);

			rst = Write_Byte(dev->usb_handle, 0xe9e0,
					 Regs[0x01e0]);
		}
...
static int
Motor_ParkHome(struct st_device *dev, int bWait, int movement)
{
	int rst = ERROR;
	SANE_Byte *Regs;
	...
	if (Regs != NULL) {
		...
		if (bWait != FALSE) {
			if (RTS_WaitScanEnd(dev, 0x3a98) != OK) {
				DBG(DBG_FNC,
				    " -> Motor_ParkHome: RTS_WaitScanEnd Timeout\n");
				rst = ERROR;	/* timeout */
			}
		} else {
	   ...
	   struct st_motormove *mymotor, struct st_motorpos *mtrpos)
{
	...
	if (cpRegs != NULL) {
		...
		if (mymotor->motorcurve != -1) {
			...
			crv = Motorcurve_Get(dev, mymotor->motorcurve,
					     ACC_CURVE, CRV_NORMALSCAN);
			if (crv != NULL)
				data_lsb_set(&cpRegs[0xe1],
					     crv->step[crv->step_count - 1],
					     3);

			DBG(DBG_FNC,
			    " -> Setting up stepper motor using motorcurve %i\n",
			    mymotor->motorcurve);
			v12dcf8 =
				Motor_Setup_Steps(dev, cpRegs,
						  mymotor->motorcurve);
...
Load_MotorCurves(struct st_device *dev)
{
	...
	if (mtc != NULL) {
		...
		dev->mtrsetting =
			Motorcurve_Parse(&dev->mtrsetting_count, mtc);
		if (dev->mtrsetting != NULL)
			rst = OK;
	}
...
static SANE_Byte *
Motor_AddStep(SANE_Byte * steps, int * bwriten, int step)
{
	steps = (SANE_Byte *) realloc(steps,
				      sizeof(SANE_Byte) * (*bwriten + 3));
	if (steps != NULL) {
...
static int
Motor_Setup_Steps(struct st_device *dev, SANE_Byte * Regs, int mysetting)
{
	int varx10, cont, last_acc_step, varx20, varx2b, stepbuffer_size,
		mystep, bwriten;
	...
	if (mycurve != NULL) {
		...
		if (mycurve->step_count > 0)
			for (cont = 0; cont < mycurve->step_count; cont++) {
				...
				varx20 += mystep + 1;
				steps = Motor_AddStep(steps, &bwriten,
						      mystep);
			}
	}
	...
	if (Motorcurve_Get(dev, mysetting, ACC_CURVE, CRV_SMEARING) != NULL) {
		...
		if (Motorcurve_Equal
		    (dev, mysetting, ACC_CURVE, CRV_SMEARING,
		     CRV_NORMALSCAN) == TRUE) {
			...
			mycurve =
				Motorcurve_Get(dev, mysetting, ACC_CURVE,
					       CRV_SMEARING);
			if (mycurve != NULL) {
				smearacccurvecount = mycurve->step_count;
				data_lsb_set(&Regs[0xe4],
					     mycurve->step[mycurve->
							   step_count - 1],
					     3);
				...
				if (mycurve->step_count > 0)
					for (cont = 0;
					     cont < mycurve->step_count;
					     cont++)
						steps = Motor_AddStep(steps,
								      &bwriten,
								      mycurve->
								      step
								      [cont]);
	...
	if (Motorcurve_Get(dev, mysetting, DEC_CURVE, CRV_SMEARING) != NULL) {
		...
		if (Motorcurve_Equal
		    (dev, mysetting, DEC_CURVE, CRV_SMEARING,
		     CRV_BUFFERFULL) == TRUE) {
			...
			data_lsb_set(&Regs[0x00f0],
				     data_lsb_get(&Regs[0x00ea], 3), 3);
		} else {
			/* set pointer to next step table */
			if (mycurve != NULL)
				stepbuffer_size += (mycurve->step_count * 3);
			data_wide_bitset(&Regs[0xfc], 0x3fff,
					 stepbuffer_size);

			/* set last step of dec.smearing table */
			mycurve =
				Motorcurve_Get(dev, mysetting, DEC_CURVE,
					       CRV_SMEARING);
			if (mycurve != NULL) {
	...
	if (Motorcurve_Get(dev, mysetting, DEC_CURVE, CRV_NORMALSCAN) != NULL) {
		...
		if (Motorcurve_Equal
		    (dev, mysetting, DEC_CURVE, CRV_NORMALSCAN,
		     CRV_BUFFERFULL) == TRUE) {
			...
			data_lsb_set(&Regs[0xed],
				     data_lsb_get(&Regs[0xea], 3), 3);
		} else if (Motorcurve_Equal
			   (dev, mysetting, DEC_CURVE, CRV_NORMALSCAN,
			    CRV_SMEARING) == TRUE) {
	...
	if (Motorcurve_Get(dev, mysetting, ACC_CURVE, CRV_PARKHOME) != NULL) {
		...
		if (Motorcurve_Equal
		    (dev, mysetting, ACC_CURVE, CRV_PARKHOME,
		     CRV_NORMALSCAN) == TRUE) {
	...
	if (Motorcurve_Get(dev, mysetting, DEC_CURVE, CRV_PARKHOME) != NULL) {
		/* parkhome curve enabled */
		if (Motorcurve_Equal
		    (dev, mysetting, DEC_CURVE, CRV_PARKHOME,
		     CRV_BUFFERFULL) == TRUE) {
...
static int
PWM_use(struct st_device *dev, int enable)
{
	SANE_Byte a, b;
	int rst = ERROR;
	...
	if (rst == OK) {
		if ((Read_Byte(dev->usb_handle, 0xfe3a, &data1) == OK)
			...
			if (enable != FALSE) {
				...
				data2 = (mode ==
					 0) ? data2 & 0x7f : data2 | 0x80;

				sscg = (data1 & 0xf3) |
					(((clock & 0x03) | 0x04) << 0x02);
				sscg = (sscg << 8) | data2;
			} else
...
RTS_Setup_RefVoltages(struct st_device *dev, SANE_Byte * Regs)
{
	...
	if (Regs != NULL) {
		...
		cfg_refvoltages_get(dev->sensorcfg->type, &vrts, &vrms,
				    &vrbs);
...
Init_Registers(struct st_device *dev)
{
	...
	if (dev->sensorcfg->type == CCD_SENSOR) {
		...
		dev->init_regs[0x9d] = dev->init_regs[0xa3] =
			dev->init_regs[0xa9] = 0;
		dev->init_regs[0x9e] = dev->init_regs[0xa4] =
			dev->init_regs[0xaa] = 0;
		dev->init_regs[0x9f] = dev->init_regs[0xa5] =
			dev->init_regs[0xab] = 0;
		dev->init_regs[0xa0] = dev->init_regs[0xa6] =
			dev->init_regs[0xac] = 0;
		dev->init_regs[0xa1] = dev->init_regs[0xa7] =
			dev->init_regs[0xad] =
...
RTS_WaitScanEnd(struct st_device *dev, int msecs)
{
	...
	if (Read_Byte(dev->usb_handle, 0xe800, &data) == OK) {
		...
		rst = OK;
		while (((data & 0x80) != 0) && (ticks > GetTickCount())
		       && (rst == OK)) {
...
static int
RTS_Enable_CCD(struct st_device *dev, SANE_Byte * Regs, int channels)
{
	int rst = ERROR;
...
static int
Lamp_SetTimer(struct st_device *dev, int minutes)
{
	...
	rst = Write_Word(dev->usb_handle, 0xe946,
...
GainOffset_Clear(struct st_device *dev)
{
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		for (a = CL_RED; a <= CL_BLUE; a++)
			RTS_EEPROM_WriteWord(dev->usb_handle, 0x70 + (2 * a),
					     0);
	...
	if ((buffer != NULL) && (size > 0)) {
		...
		if (RTS_DMA_Reset(dev) == OK) {
			...
			if (RTS_DMA_Enable_Read(dev, dmacs, size, options) ==
				...
				rst = Bulk_Operation(dev, BLK_READ, size,
						     buffer, &transferred);
			}
		}
	}
	      ...
	      int size, SANE_Byte * buffer)
{
	...
	if ((buffer != NULL) && (size > 0)) {
		...
		if (RTS_DMA_Reset(dev) == OK) {
			...
			if (RTS_DMA_Enable_Write(dev, dmacs, size, options) ==
			    OK) {
				int transferred;
				...
				if (check_buffer != NULL) {
					...
					Bulk_Operation(dev, BLK_WRITE, size,
						       buffer, &transferred);
					rst = OK;
				}
			}
	...
	if (Regs != NULL) {
		...
		if (out_buffer != NULL) {
			...
			if (in_buffer != NULL) {
				...
				for (a = 4; a >= 0; a--) {
					/* set ram type */
					if (RTS_DMA_SetType(dev, Regs, a) !=
					    OK)
						break;

					/* wait 1500 miliseconds */
					if (RTS_DMA_WaitReady(dev, 1500) ==
					    OK) {
						/* reset dma */
						RTS_DMA_Reset(dev);

						/* write buffer */
						RTS_DMA_Write(dev, 0x0004,
							      0x102, 2072,
							      out_buffer);

						/* now read buffer */
						RTS_DMA_Read(dev, 0x0004,
							     0x102, 2072,
							     in_buffer);
						...
						if (diff == FALSE) {
							...
							if (RTS_DMA_Write
								...
								do {
									...
									if (RTS_DMA_Write(dev, 0x0004, (_B0(c) << 0x11) >> 0x04, 0x20, out_buffer) == OK) {
										if (RTS_DMA_Read(dev, 0x0004, 0x0000, 0x20, in_buffer) == OK) {
											b = 0;
											diff = FALSE;
											while ((b < 0x20)
											       &&
...
RTS_DMA_SetType(struct st_device *dev, SANE_Byte * Regs, SANE_Byte ramtype)
{
	...
	if (Regs != NULL) {
		...
		if (Write_Byte(dev->usb_handle, 0xef08, Regs[0x708]) == OK) {
			...
			if (Write_Byte(dev->usb_handle, 0xef08, Regs[0x708])
			    == OK) {
				data_bitset(&Regs[0x708], 0x08, 1); /*----1---*/
				rst = Write_Byte(dev->usb_handle, 0xef08,
						 Regs[0x708]);
			}
		}
	}
	...
	if (Read_Byte(dev->usb_handle, 0xe800, &e800) == OK) {
		if (Read_Byte(dev->usb_handle, 0xe813, &e813) == OK) {
			...
			if (Write_Byte(dev->usb_handle, 0xe813, e813) == OK) {
				e800 |= 0x40;
				if (Write_Byte(dev->usb_handle, 0xe800, e800)
				    == OK) {
					e813 |= 0x40;
					if (Write_Byte
					    (dev->usb_handle, 0xe813,
					     e813) == OK) {
						e800 &= 0xbf;
						if (Write_Byte
						    (dev->usb_handle, 0xe800,
						     e800) == OK) {
							usleep(1000 * 100);
							e800 |= 0x80;
							ret = Write_Byte(dev->
	...
	if ((hp_gamma->table[CL_RED] != NULL) &&
		...
		for (C = 0; C < 256; C++) {
			if ((table != NULL)
				...
				hp_gamma->table[CL_GREEN][C] =
					table[0x100 + C];
				hp_gamma->table[CL_BLUE][C] =
					table[0x200 + C];
			} else {
	...
	if (hwdcfg->use_gamma_tables == FALSE) {
		...
		if (gammabuffer != NULL) {
			...
			for (c = 0; c < channels; c++)
				memcpy(gammabuffer + (c * table_size),
				       mygamma->table[c], table_size);

			/* send gamma buffer to scanner */
			Write_Byte(dev->usb_handle, 0xee0b,
				   Regs[0x060b] & 0xaf);
			rst = Gamma_SendTables(dev, Regs, gammabuffer,
					       buffersize);
...
Refs_Analyze_Pattern(struct st_scanparams *scancfg,
		     SANE_Byte * scanned_pattern, int * ler1,
		     int ler1order, int * ser1, int ser1order)
{
	int a, buffersize, buffer_pos, xpos, ypos, data, rst, cnt;
	...
	DBG(DBG_FNC,
	    ...
	    scancfg->depth, scancfg->coord.width, scancfg->coord.height,
	    ler1order, ser1order);
	...
	for (xpos = 0; xpos < scancfg->coord.width; xpos++) {
		for (a = 0; a < 20; a++) {
			data = _B0(scanned_pattern
				   [(scancfg->coord.width * a) + xpos]);
			if (scancfg->depth > 8)
				data = ((_B0
					 (scanned_pattern
					  [(scancfg->coord.width * a) + xpos +
	...
	data1 = (ser1order !=
		 0) ? buffer1[0] - buffer1[1] : buffer1[1] - buffer1[0];
	buffer2[0] = data1;
	...
	if ((scancfg->coord.width - 5) > 1) {
		...
		do {
			mydata1 =
				(ser1order !=
				 0) ? buffer1[buffer_pos] -
				buffer1[buffer_pos + 5] : buffer1[buffer_pos +
								  5] -
				buffer1[buffer_pos];
			...
			if ((mydata1 >= 0) && (mydata1 > data1)) {
				...
				d1 = abs(buffer2[buffer_pos] -
					 buffer2[buffer_pos - 1]);
	...
	if ((Regs != NULL) && (mytiming < dev->timings_count)) {
		...
		if (mt != NULL) {
			...
			if (dev->chipset->model == RTS8822L_02A) {
				if (mt->clampe == -1)
					data_lsb_set(&Regs[0x8d], mt->cphbp2e,
						     3);
			}
			...
			Regs[0x9b] &= 0xf0;
			Regs[0x9b] |=
				((get_byte(get_shrd(mt->adcclkp[0], 0x20))) &
				 0x0f);
			...
			Regs[0xc5] &= 0xe0;
			Regs[0xc5] |=
				((get_byte(get_shrd(mt->adcclkp[1], 0x20))) &
				 0x0f);

			/* bit(4) = bit(0) */
			Regs[0xc5] |= ((mt->adcclkp2e & 1) << 4);

			Timing_SetLinearImageSensorClock(&Regs[0x48],
							 &mt->cph[0]);
			Timing_SetLinearImageSensorClock(&Regs[0x52],
							 &mt->cph[1]);
			Timing_SetLinearImageSensorClock(&Regs[0x5c],
							 &mt->cph[2]);
			Timing_SetLinearImageSensorClock(&Regs[0x66],
							 &mt->cph[3]);
			Timing_SetLinearImageSensorClock(&Regs[0x70],
							 &mt->cph[4]);
			Timing_SetLinearImageSensorClock(&Regs[0x7a],
							 &mt->cph[5]);
		}
	}
}
...
static int
data_lsb_get(SANE_Byte * address, int size)
{
	int ret = 0;
...
static SANE_Byte
data_bitget(SANE_Byte * address, int mask)
{
	int desp = 0;
...
static void
data_bitset(SANE_Byte * address, int mask, SANE_Byte data)
{
...
static void
data_wide_bitset(SANE_Byte * address, int mask, int data)
{
...
static void
data_lsb_set(SANE_Byte * address, int data, int size)
{
	if ((address != NULL) && (size > 0) && (size < 5)) {
		int a;
		for (a = 0; a < size; a++) {
...
static void
data_msb_set(SANE_Byte * address, int data, int size)
{
	if ((address != NULL) && (size > 0) && (size < 5)) {
		int a;
...
{
	DBG(DBG_FNC,
	    "> Lamp_SetGainMode(*Regs, resolution=%i, gainmode=%i):\n",
	    resolution, gainmode);
		  ...
		  int buffer_size, int * transferred)
{
	...
	if (scn->imagebuffer == NULL) {
		if ((scn->arrange_hres == TRUE)
		    || (scan2.colormode == CM_LINEART)) {
			scn->bfsize =
				(scn->arrange_sensor_evenodd_dist +
				 1) * line_size;
			scn->imagebuffer =
				(SANE_Byte *) malloc(scn->bfsize *
						     sizeof(SANE_Byte));
			if (scn->imagebuffer != NULL) {
				if (Read_Block
				    (dev, scn->bfsize, scn->imagebuffer,
				     transferred)
				    == OK) {
					scn->channel_size =
						(scan2.depth == 8) ? 1 : 2;
	...
	if (rst == OK) {
		...
		while (lines_count > 0) {
			if (scan2.colormode == CM_LINEART)
				Triplet_Lineart(scn->pColour1[CL_RED],
						scn->pColour2[CL_RED], buffer,
						channels_count);
			else
				Triplet_Gray(scn->pColour1[CL_RED],
					     scn->pColour2[CL_RED], buffer,
					     channels_count);
			...
			rst = Read_Block(dev, line_size, scn->imagepointer,
					 transferred);
			if (rst != OK)
				break;
			...
			if (scn->arrange_hres == TRUE) {
				scn->desp2[CL_RED] =
					(line_size +
					 scn->desp2[CL_RED]) % scn->bfsize;
				scn->desp1[CL_RED] =
					(line_size +
					 scn->desp1[CL_RED]) % scn->bfsize;
...
static int
Resize_Decrease(SANE_Byte * to_buffer, int to_resolution,
		int to_width, SANE_Byte * from_buffer,
		...
		int myresize_mode)
{
	...
	DBG(DBG_FNC,
	    ...
	    to_resolution, to_width, from_resolution, from_width,
	    myresize_mode);
...
static int
Resize_Increase(SANE_Byte * to_buffer, int to_resolution,
		int to_width, SANE_Byte * from_buffer,
		int from_resolution, int from_width,
		int myresize_mode)
{
	...
	if (myresize_mode != RSZ_LINEART) {
		...
		if (channels > 0) {
			...
			for (mychannels = 0; mychannels < channels;
			     mychannels++) {
				pos = 0;
				rescount =
					(from_resolution / 2) + to_resolution;
				...
				if (to_width > 0) {
					for (mywidth = 0; mywidth < to_width;
					     mywidth++) {
...
Resize_Start(struct st_device *dev, int * transferred)
{
	...
	if (arrangeline2 == FIX_BY_SOFT) {
		...
		if (scan2.colormode == CM_COLOR)
			rst = Arrange_Colour(dev, rz->v3624, line_size,
					     transferred);
		else
			rst = Arrange_NonColour(dev, rz->v3624, line_size,
						transferred);
	} else
	...
	case RSZ_DECREASE:
		/* ff1b */
		Resize_Decrease(rz->v3628, rz->resolution_x, rz->towidth,
	...
	case RSZ_INCREASE:
		/* ff69 */
		rz->rescount = 0;
		Resize_Increase(rz->v3628, rz->resolution_x, rz->towidth,
				rz->v3624, scan2.resolution_x, rz->fromwidth,
				rz->mode);
		if (arrangeline2 == FIX_BY_SOFT) {
			/* ffb1 */
			if (scan2.colormode == CM_COLOR)
				rst = Arrange_Colour(dev, rz->v3624,
						     line_size, transferred);
			else
				rst = Arrange_NonColour(dev, rz->v3624,
							line_size,
							transferred);
		} else
		...
		/* fff2 */
		Resize_Increase(rz->v362c, rz->resolution_x, rz->towidth,
				rz->v3624, scan2.resolution_x, rz->fromwidth,
				rz->mode);
		break;
	}
		     ...
		     int size3)
{
	...
	DBG(DBG_FNC,
	    "> Resize_CreateBuffers(size1=%i, size2=%i, size3=%i): %i\n",
					...
					if (RTS_DMA_Enable_Write
						...
						if (Bulk_Operation
						    (dev, BLK_WRITE, size,
						     ...
						     &transferred) == OK) {
							/* Send size to read */
							if (RTS_DMA_Enable_Read(dev, 0x0000, size, first_table) == OK) {
								/* Retrieve data */
								if (Bulk_Operation(dev, BLK_READ, size, mybuffer, &transferred) == OK) {
									/* Check data */
...
RTS_Scanner_StopScan(struct st_device *dev, int wait)
{
	...
	if (Write_Buffer(dev->usb_handle, 0xee0a, &dev->init_regs[0x60a], 2)
	    == OK)
		Motor_Change(dev, dev->init_regs, 3);
	...
	if (wait == FALSE) {
		...
		if ((data & 0x02) == 0) {
			if (Lamp_IsAtHome(dev, dev->init_regs) == FALSE) {
				...
				Write_Byte(dev->usb_handle, 0x00,
					   dev->init_regs[0x00]);
				Motor_ParkHome(dev, TRUE,
					       dev->motorcfg->
					       parkhomemotormove);
			}
		}
		...
		if (Lamp_IsAtHome(dev, dev->init_regs) == FALSE)
			Motor_ParkHome(dev, TRUE,
				       dev->motorcfg->parkhomemotormove);
	}
...
RTS_ScanCounter_Inc(struct st_device *dev)
{
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		case RTS8822BL_03A:
			...
			if (RTS_EEPROM_ReadInteger
			    (dev->usb_handle, 0x21, &idata) == OK) {
				idata = data_swap_endianess(idata, 4) + 1;
				idata = data_swap_endianess(idata, 4);
				RTS_EEPROM_WriteInteger(dev->usb_handle, 0x21,
							idata);
			}
			break;
		default:
			...
			somebuffer[25] = cdata;
			RTS_EEPROM_WriteBuffer(dev->usb_handle, 0x21,
					       somebuffer, 0x1a);
			break;
		}
	}
...
static int
Read_Image(struct st_device *dev, int buffer_size, SANE_Byte * buffer,
	   int * transferred)
{
	int rst;
	...
	*transferred = 0;
	...
	if (dev->Resize->type == RSZ_NONE) {
		if (arrangeline == FIX_BY_SOFT) {
			switch (mycolormode) {
			case CM_COLOR:
				rst = Arrange_Colour(dev, buffer, buffer_size,
						     transferred);
				break;
			case 3:
				rst = Arrange_Compose(dev, buffer,
						      buffer_size,
						      transferred);
				break;
			default:
				rst = Arrange_NonColour(dev, buffer,
							buffer_size,
							transferred);
				break;
			}
		...
		int buffer_size, int * transferred)
{
	...
	DBG(DBG_FNC,
	    "+ Arrange_Compose(*buffer, buffer_size=%i, *transferred):\n",
	    buffer_size);
		...
		if (mydistance != 0) {
			scn->bfsize =
				(scn->arrange_hres ==
				 TRUE) ? scn->arrange_sensor_evenodd_dist : 0;
			scn->bfsize =
				line_size * (scn->bfsize + (mydistance * 2) +
					     1);
		} else
			scn->bfsize = line_size * 2;
		     ...
		     SANE_Byte * buffer, int Width)
{
	...
	while (Width > 0) {
		Value = data_lsb_get(pRed1,
				     Channel_size) + data_lsb_get(pGreen1,
								  Channel_size)
		...
		Value = data_lsb_get(pRed2,
				     Channel_size) + data_lsb_get(pGreen2,
								  Channel_size)
		      ...
		      SANE_Byte * buffer, int dots)
{
	...
	if (scan2.depth > 8) {
		...
		dots = dots / 2;
		while (dots > 0) {
			Value = min(data_lsb_get(pRed, 2) +
				    data_lsb_get(pGreen,
						 2) + data_lsb_get(pBlue, 2),
				    0xffff);

			if (v1600 != NULL)
				Value = (*(v1600 + (Value >> 8)) << 8) |
					_B0(Value);
		...
		while (dots > 0) {
			Value = min(((*myp1 + *(line_size + myp1)) / 2) +
				    *myp2 + *myp3, 0xff);

			*buffer =
				(v1600 ==
				 NULL) ? _B0(Value) : *(v1600 + Value);
		     ...
		     SANE_Byte * buffer, int dots)
{
	...
	/*bf59 */
	while (dots > 0) {
		Value = data_lsb_get(pRed,
				     Channel_size) + data_lsb_get(pGreen,
								  Channel_size)
...
static void
Triplet_Colour_LRes(int Width, SANE_Byte * Buffer,
		    SANE_Byte * pChannel1, SANE_Byte * pChannel2,
		    SANE_Byte * pChannel3)
{
	/*
	...
	DBG(DBG_FNC,
	    "> Triplet_Colour_LRes(Width=%i, *Buffer2, *p1, *p2, *p3):\n",
	    Width);
		 ...
		 int buffer_size, int * transferred)
{
	...
	if (rz->type == RSZ_DECREASE) {
		...
		do {
			...
			if (arrangeline2 == FIX_BY_SOFT) {
				if (scan2.colormode == CM_COLOR)
					...
					rst = Arrange_NonColour(dev,
								rz->v3624,
								line_size,
								transferred);
			} else
				rst = Read_Block(dev, line_size, rz->v3624,
						 transferred);

			/*f2df */
			Resize_Decrease(rz->v362c, rz->resolution_x,
					rz->towidth, rz->v3624,
					scan2.resolution_x, rz->fromwidth,
					rz->mode);
		...
		if (lfa68 != 0) {
			...
			Resize_Increase(temp, rz->resolution_x, rz->towidth,
					rz->v3624, scan2.resolution_x,
					rz->fromwidth, rz->mode);
		} else
			rst = OK;
	}
			  ...
			  int size)
{
	...
	DBG(DBG_FNC,
	    "> Split_into_12bit_channels(*destino, *fuente, size=%i\n", size);
...
Read_NonColor_Block(struct st_device *dev, SANE_Byte * buffer,
		    int buffer_size, SANE_Byte ColorMode,
		    int * transferred)
{
	...
	if (mybuffer != NULL) {
		...
		do {
			mysize4lines =
				(mysize <=
				 dev->Reading->Size4Lines) ? mysize : dev->
				Reading->Size4Lines;
			...
			if (ColorMode == CM_GRAY) {
				if (scan2.depth == 12) {
					...
					if (rst == OK) {
						...
						pBuffer +=
							LinesCount *
							block_bytes_per_line;
						while (LinesCount > 0) {
							Split_into_12bit_channels
								(mybuffer,
								 pImage,
								 line_size);
							pImage +=
				...
				/* LINEART */
				int desp;
				rst = Scan_Read_BufferA(dev, mysize4lines, 0,
							mybuffer,
							transferred);
				if (rst == OK) {
					pImage = mybuffer;
...
static int
Read_Block(struct st_device *dev, int buffer_size, SANE_Byte * buffer,
	   int * transferred)
{
	/*
...
*/
	...
	*transferred = 0;
	...
	if ((scan2.colormode != CM_COLOR) && (scan2.channel == 3)
		...
		/*6510 */
		return Read_NonColor_Block(dev, buffer, buffer_size,
					   scan2.colormode, transferred);
	}
	...
	if (readbuffer != NULL) {
		do {
			...
			if (scan2.depth == 12) {
				rst = Scan_Read_BufferA(dev, buffer_size, 0,
							readbuffer,
							transferred);
				if (rst == OK) {
					if (LinesCount > 0) {
...
Scan_Read_BufferA(struct st_device *dev, int buffer_size, int arg2,
		  SANE_Byte * pBuffer, int * bytes_transfered)
{
	int rst = OK;
	...
	if (pBuffer != NULL) {
		...
		while ((buffer_size > 0) && (rst == OK)
			...
			if (rd->Starting == TRUE) {
				...
				if (Read_Byte(dev->usb_handle, 0xe812, &data)
				    == OK) {
					data = data >> 6;
					if (data != 0)
				...
				rd->Channel_size = 1;
				if (Read_Byte(dev->usb_handle, 0xee0b, &data)
				    == OK)
					if (((data & 0x40) != 0)
					    && ((data & 0x08) == 0))
						rd->Channel_size = 2;
							...
							if (iAmount != 0) {
								    ...
								    ptBuffer)
								{
									...
									if ((ptDMABuffer + iAmount) >= (rd->DMABuffer + rd->DMABufferSize)) {
										int rest = iAmount - (rd->DMABufferSize - (ptDMABuffer - rd->DMABuffer));
			...
			if (rd->RDSize > 0) {
				...
				if ((rd->RDStart + iAmount) >=
					...
					memcpy(ptBuffer + rest, rd->DMABuffer,
					       iAmount - rest);
					rd->RDStart =
						rd->DMABuffer + (iAmount -
								 rest);
				} else {
					memcpy(ptBuffer, rd->RDStart,
					       iAmount);
					rd->RDStart += iAmount;
				}
			...
			int channel_size)
{
	...
	if (channel_size > 0) {
		...
		if (Read_Integer(dev->usb_handle, 0xef16, &myAmount) == OK)
			rst = ((channels_per_dot * 32) / channel_size) *
				myAmount;
	}
...
static int
Lamp_Warmup(struct st_device *dev, SANE_Byte * Regs, int lamp)
{
	int rst = OK;
...
Scan_Start(struct st_device *dev)
{
	...
	if (RTS_Enable_CCD(dev, dev->init_regs, 0x0f) == OK) {
		...
		int findref;
		int cl, lfaa0, ypos, xpos, runb1, gainmode;
		struct st_scanparams scancfg;
		struct st_hwdconfig hwdcfg;
		struct st_calibration myCalib;
		long tick;
		...
		while ((Lamp_IsAtHome(dev, Regs) == 0)
		       && (tick > GetTickCount()));
		...
		if (findref != FALSE) {
			...
			if (waitforpwm == TRUE) {
				Lamp_WaitForPWM(dev, scan.resolution_x,
						FLB_LAMP);
				waitforpwm = FALSE;
			}
			...
			if (Lamp_Warmup
			    (dev, Regs,
			     (scan.scantype ==
			      ST_NORMAL) ? FLB_LAMP : TMA_LAMP) == ERROR)
				return ERROR;
...
/* Calibrate White shading correction */
		if ((RTS_Debug->wshading == TRUE)
		    && (scan.scantype == ST_NORMAL))
			if (WShading_Calibrate(dev, Regs, &myCalib, &scancfg)
			    == OK)
				Motor_ParkHome(dev, TRUE,
					       dev->motorcfg->
					       parkhomemotormove);
		...
		if (RTS_Debug->calibrate != 0) {
			...
			if (scan.scantype == ST_NORMAL) {
				...
				if ((scan.scantype == ST_TA)
				    || (scan.scantype == ST_NEG)) {
					/*int ta_y_start; */
		...
		if ((scan.scantype >= ST_NORMAL) || (scan.scantype <= ST_NEG)) {
			...
			if ((dpi100Lumping == 0)
			    && (scancfg.resolution_x ==
				Scanmode_minres(dev, scan.scantype,
						scan.colormode)))
				myser = scan.ser / 2;
			else
				myser = scan.ser;
		...
		hwdcfg.sensorevenodddistance =
			dev->sensorcfg->evenodd_distance;
		hwdcfg.highresolution =
			(scancfg.resolution_x <= 1200) ? FALSE : TRUE;
		...
		if (RTS_Debug->calibrate != FALSE) {
			...
			hwdcfg.unk3 = 0;
			RTS_Setup(dev, Regs, &scancfg, &hwdcfg,
				  &calibdata->gain_offset);

			myCalib.shading_type = 0;
			myCalib.shadinglength =
				min(myCalib.shadinglength,
				    scan.shadinglength);

			if (scancfg.colormode != CM_COLOR) {
				if ((scancfg.channel > 0)
				    && (scancfg.channel < 3))
					myCalib.WRef[0] =
						myCalib.WRef[scancfg.channel];
			}
		...
		if (dev->motorcfg->changemotorcurrent != FALSE)
			Motor_Change(dev, Regs,
				     Motor_GetFromResolution(scancfg.
							     resolution_x));
		...
		struct st_scanparams *scancfg, int somevalue)
{
	...
	if ((Regs != NULL) && (scancfg != NULL)) {
		int colormode, mymode, step_type, dummyline, myvalue,
			lf02c;
		int step_size;
		...
		mymode = RTS_GetScanmode(dev, scantype, colormode,
					 scancfg->resolution_x);
		if (mymode == -1)
			return ERROR;
		...
		if (sm->motorcurve != -1) {
			if (sm->motorcurve < dev->mtrsetting_count) {
				struct st_motorcurve *ms =
					dev->mtrsetting[sm->motorcurve];
				ms->motorbackstep = sm->motorbackstep;
			}

			DBG(DBG_FNC,
			    " -> Setting up step motor using motorcurve %i\n",
			    sm->motorcurve);
			...
			if (mbs[1] >=
			    (smeardeccurvecount + smearacccurvecount))
				mbs[0] = mbs[1] - (smeardeccurvecount +
						   smearacccurvecount) + 2;
			else
				mbs[0] = 0;
			 ...
			 int arrangeline)
{
	...
	if (arrangeline == FIX_BY_HARD) {
		...
		mylinedistance =
			(dev->sensorcfg->line_distance *
			 scancfg->resolution_y) / dev->sensorcfg->resolution;
		...
		data_bitset(&Regs[0x14b], 0x3f,
			    mylinedistance + myevenodddist);
		data_bitset(&Regs[0x14c], 0x3f, mylinedistance * 2);
		data_bitset(&Regs[0x14d], 0x3f,
			    (mylinedistance * 2) + myevenodddist);
	} else {
		  ...
		  struct st_hwdconfig *hwdcfg, int bytes_per_line)
{
	...
	if ((Regs != NULL) && (hwdcfg != NULL)) {
		int dots_count, myvalue, myvalue2, mem_available,
			resolution_ratio, sensor_line_distance;
		...
		if ((hwdcfg->white_shading != FALSE)
		    && (hwdcfg->black_shading != FALSE)
		    && (hwdcfg->unk3 != 0))
		...
		case 3:	/* 3 channels per dot */
			...
			myvalue =
				(((sensor_line_distance + 1) * dots_count) +
				 v160c_block_size - 1) / v160c_block_size;
			myvalue2 = myvalue;
			mem_available =
				(mem_available - (myvalue * 3) + 2) / 3;
			...
			data_wide_bitset(&Regs[0x1c5], 0xfffff,
					 (myvalue / 16) + 1);
			break;
		case 2:	/* 2 channels per dot */
			...
			myvalue +=
				(((table_size * 3) + mem_available) / 16) + 1;
	...
	if ((scancfg != NULL) && (Regs != NULL)) {
		if ((mycolormode != CM_COLOR) && (mycolormode != 3)) {
			...
			if (scancfg->samplerate == LINE_RATE) {
				...
				if (scancfg->channel ==
				    dev->sensorcfg->rgb_order[1]) {
					/* mexpts[CL_RED] = mexpts[CL_GREEN] */
					data_lsb_set(&Regs[0x33],
						     data_lsb_get(&Regs[0x39],
								  3), 3);

					/* expts[CL_RED] = expts[CL_GREEN] */
					data_lsb_set(&Regs[0x36],
						     data_lsb_get(&Regs[0x3c],
								  3), 3);
				} else if (scancfg->channel ==
					   dev->sensorcfg->rgb_order[2]) {
					/* mexpts[CL_RED] = mexpts[CL_BLUE] */
					data_lsb_set(&Regs[0x33],
						     data_lsb_get(&Regs[0x3f],
								  3), 3);

					/* expts[CL_RED] = expts[CL_BLUE] */
					data_lsb_set(&Regs[0x36],
						     data_lsb_get(&Regs[0x42],
								  3), 3);
				}
			} else {
				/* e01 */
...
RTS_Setup(struct st_device *dev, SANE_Byte * Regs,
	  ...
	  struct st_gain_offset *gain_offset)
{
	int iLineDistance, lSMode, dummyline;
	int bytes_per_line, resolution_ratio;
	SANE_Byte mycolormode;
	struct st_scanmode *sm;
	...
	scantype = hwdcfg->scantype;
	lSMode = RTS_GetScanmode(dev, scantype, mycolormode,
				 scancfg->resolution_x);
	if (lSMode < 0)
		return ERROR;
	...
	dev->scanning->arrange_sensor_evenodd_dist =
		(hwdcfg->highresolution ==
		 FALSE) ? 0 : hwdcfg->sensorevenodddistance;

	/* set up line distances */
	iLineDistance =
		RTS_Setup_Line_Distances(dev, Regs, scancfg, hwdcfg,
					 mycolormode, arrangeline);
...
static void
RTS_Setup_Coords(SANE_Byte * Regs, int iLeft, int iTop,
		 int width, int height)
{
	DBG(DBG_FNC,
...
{
	SANE_Byte fake[] =
		{ 0x19, 0x15, 0x19, 0x64, 0x64, 0x64, 0x74, 0xc0, 0x74, 0xc0,
		0x6d,
	...
	if ((Regs != NULL) && (gain_offset != NULL)) {
		if (RTS_Debug->calibrate == FALSE) {
			...
			for (a = CL_RED; a <= CL_BLUE; a++) {
				/* Offsets */
				Regs[0x1a + (a * 4)] =
					_B0(gain_offset->edcg1[a]);
				Regs[0x1b + (a * 4)] =
					((gain_offset->
					  edcg1[a] >> 1) & 0x80) |
					(gain_offset->edcg2[a] & 0x7f);
				Regs[0x1c + (a * 4)] =
					_B0(gain_offset->odcg1[a]);
				Regs[0x1d + (a * 4)] =
					((gain_offset->
					  odcg1[a] >> 1) & 0x80) |
					(gain_offset->odcg2[a] & 0x7f);

				/* Variable Gain Amplifier */
				data_bitset(&Regs[0x14 + a], 0x1f,
					    gain_offset->vgag1[a]);
				data_bitset(&Regs[0x17 + a], 0x1f,
					    gain_offset->vgag2[a]);
			}
	...
	if ((calbuffers != NULL) && (Regs != NULL) && (myCalib != NULL)) {
		...
		if ((myshadinglength % somelength) != 0)
			calbuffers->tables_size =
				(myshadinglength >=
				 somelength) ? somelength * 2 : somelength;
		else
		...
		if (myshadinglength >= somelength) {
			...
			calbuffers->shadinglength3 =
				((myCalib->shadinglength * 2) / somelength) -
				1;
		} else
			calbuffers->shadinglength3 = 0;
		...
		for (pos = 0; pos < calbuffers->table_count; pos++) {
			calbuffers->tables[pos] =
				(USHORT *) malloc(calbuffers->tables_size *
						  sizeof(USHORT));
...
static int
Calib_WriteTable(struct st_device *dev, SANE_Byte * table, int size,
		 int data)
{
	int rst = ERROR;
	...
	if ((table != NULL) && (size > 0)) {
		...
		if (RTS_DMA_Reset(dev) == OK) {
			/* Send size to write */
			if (RTS_DMA_Enable_Write(dev, 0x0004, size, data) ==
			    OK)
				/* Send data */
				rst = Bulk_Operation(dev, BLK_WRITE, size,
						     table, &transferred);
		}
	}
...
static int
Calib_ReadTable(struct st_device *dev, SANE_Byte * table, int size,
		int data)
{
	int rst = ERROR;
	...
	if ((table != NULL) && (size > 0)) {
		...
		if (RTS_DMA_Reset(dev) == OK) {
			/* Send size to read */
			if (RTS_DMA_Enable_Read(dev, 0x0004, size, data) ==
			    OK)
				/* Retrieve data */
				rst = Bulk_Operation(dev, BLK_READ, size,
						     table, &transferred);
       ...
       int sensorchannelcolor, int * tablepos, int data)
{
	...
	if (calbuffers->table_count > 0) {
		do {
			...
			switch (schcolor) {
			case CM_LINEART:
				size = (((Regs[0x1bf] >> 1) & 3) << 0x10) |
					(Regs[0x1be] << 0x08) | Regs[0x1bd];
				value4 = (tablelength + size) | val_lineart;
				break;
			case CM_GRAY:
				size = ((Regs[0x1bf] & 1) << 0x10) |
					(Regs[0x1bc] << 0x08) | Regs[0x1bb];
			...
			if (Calib_ReadTable
			    (dev, (SANE_Byte *) calbuffers->table2,
			     calbuffers->tables_size, value4) != OK) {
				rst = ERROR;
				break;
			}

			memcpy(calbuffers->tables[a], calbuffers->table2,
			       tablepos[a]);
	...
	if (channels > 0) {
		...
		for (chnl = 0; chnl < channels; chnl++) {
			...
			for (pos = 0; pos <= 10; pos++) {
				...
				if (fn3730
				    (dev, calbuffers, Regs,
				     &myCalib->white_shading[chnl][myCalib->
								   first_position
								   - 1],
				     dev->sensorcfg->channel_color[chnl],
				     1) == OK) {
					rst = OK;
					break;
				}
	...
	if (channels > 0) {
		...
		for (chnl = 0; chnl < channels; chnl++) {
			...
			for (pos = 0; pos <= 10; pos++) {
				...
				if (fn3730
				    (dev, calbuffers, Regs,
				     &myCalib->black_shading[chnl][myCalib->
								   first_position
								   - 1],
				     dev->sensorcfg->channel_color[chnl],
				     0) == OK) {
	...
	if (rst == OK) {
		...
		if (myCalib->shading_enabled != FALSE) {
			...
			for (colour = 0; colour < channels; colour++) {
				...
				myfact = myCalib->WRef[colour] *
					myShadingBase;

				if (myCalib->shading_type == 2) {
					/*3bd8 */
					if ((myCalib->black_shading[colour] !=
					     NULL)
					    && (myCalib->
						white_shading[colour] !=
						NULL)) {
						for (pos =
						     myCalib->first_position -
						     1;
						     pos <
						     myCalib->shadinglength;
						     pos++) {
						...
						if (myCalib->
							...
							for (pos =
								...
								if (myCalib->
									...
									shadata = myfact / myCalib->black_shading[colour][pos];
						...
						if (myCalib->
							...
							for (pos = 0;
								...
								if (myCalib->
									...
									shadata = myfact / myCalib->white_shading[colour][pos];
		...
		if ((Regs[0x1cf] & 8) != 0)
			Shading_black_apply(dev, Regs, channels, myCalib,
					    &calbuffers);
		...
		if ((Regs[0x1cf] & 4) != 0)
			Shading_white_apply(dev, Regs, channels, myCalib,
					    &calbuffers);
		...
		if (rst == 0) {
			...
			rst = Write_Byte(dev->usb_handle, 0xee0b,
					 Regs[0x060b]);
		}
	}
	/*3fb5 */
...
static int
Bulk_Operation(struct st_device *dev, SANE_Byte op, int buffer_size,
	       SANE_Byte * buffer, int * transfered)
{
	...
	if (op != 0) {
		...
		do {
			...
			iBytesTransfered =
				Read_Bulk(dev->usb_handle, &buffer[iPos],
					  iTransferSize);
...
static int
Reading_NotifySize(struct st_device *dev, int data, int size)
{
	int rst;
...
Reading_Wait(struct st_device *dev, SANE_Byte Channels_per_dot,
	     SANE_Byte Channel_size, int size, int * last_amount,
	     int seconds, SANE_Byte op)
{
	int rst;
	...
	if (myAmount < size) {
		...
		while (cTimeout == FALSE) {
			myAmount =
				Get_Scanner_Buffer_Size(dev, Channels_per_dot,
							Channel_size);

			/* check special case */
			if (op == TRUE) {
				if (((myAmount + 0x450) > size)
				    || (RTS_IsExecuting(dev, &executing) ==
					FALSE))
					break;
			}
		       ...
		       char unsigned *buffer, double *transferred)
{
	...
	DBG(DBG_FNC,
	    "+ RTS_GetImage_GetBuffer(dSize=%f, buffer, transferred):\n",
	    dSize);
	...
	if (dSize > 0) {
		...
		do {
			...
			rst = ERROR;
			if (Reading_Wait
			    (dev, 0, 1, myLength * 2, NULL, 5, FALSE) == OK) {
				if (Reading_NotifySize(dev, 0, myLength * 2)
				    == OK)
					rst = Bulk_Operation(dev, BLK_READ,
							     myLength * 2,
							     &buffer[iPos],
							     &itransferred);
			}
		  ...
		  struct st_scanparams *scancfg, struct st_hwdconfig *hwdcfg)
{
	...
	if (buffer != NULL) {
		...
		if (exfn != 0) {
			double transferred;
			rst = RTS_GetImage_GetBuffer(dev, dSize, buffer,
						     &transferred);
		}
	     ...
	     int gainmode)
{
	...
	if ((scancfg->coord.width != 0) && (scancfg->coord.height != 0)) {
		...
		myscancfg = (struct st_scanparams *)
			malloc(sizeof(struct st_scanparams));
		if (myscancfg != NULL)
			memcpy(myscancfg, scancfg,
			       sizeof(struct st_scanparams));
		else
			return ERROR;

		/* Allocate space for low level config */
		hwdcfg = (struct st_hwdconfig *)
		...
		case ST_NORMAL:
			if (scan.resolution_x == 100) {
				...
				myRegs = (SANE_Byte *) malloc(RT_BUFFER_LEN *
							      sizeof
							      (SANE_Byte));
				if (myRegs != NULL) {
					bzero(myRegs,
...
static int
Refs_Detect(struct st_device *dev, SANE_Byte * Regs, int resolution_x,
	    int resolution_y)
{
	struct st_scanparams scancfg;
	struct st_gain_offset gain_offset;
	int rst, gainmode, pwmlamplevel_backup, C, ser1, ler1;
	SANE_Byte *image;

	DBG(DBG_FNC,
	    "+ Refs_Detect(*Regs, resolution_x=%i, resolution_y=%i):\n",
	...
	image = (SANE_Byte *)
		malloc((scancfg.coord.height * scancfg.coord.width) *
		       sizeof(SANE_Byte));
	if (image != NULL) {
		...
		if (RTS_GetImage
			...
			if (Refs_Analyze_Pattern
			    (&scancfg, image, &ler1, 1, &ser1, 0) == OK) {
	...
	DBG(DBG_FNC,
	    "- Refs_Detect: scan.startpos=%i, scan.leftleading=%i : %i\n",
	 ...
	 struct st_scanparams *myscan, int autoref)
{
	...
	if (refcfg.type != REF_NONE) {
		...
		case REF_AUTODETECT:
			...
			if (Refs_Detect
			    (dev, Regs, refcfg.resolution,
			     refcfg.resolution) == OK)
				Refs_Save(dev, scan.leftleading,
					  scan.startpos);
			else
				rst = ERROR;
		...
		case REF_TAKEFROMSCANNER:
			...
			if (Refs_Load(dev, &left_leading, &start_pos) ==
			    ERROR) {
				if (Refs_Detect
				    (dev, Regs, refcfg.resolution,
				     refcfg.resolution) == OK)
					Refs_Save(dev, left_leading,
						  start_pos);
				else
					rst = ERROR;
		...
		if (rst == OK) {
			...
			scan.startpos = (myscan->resolution_y < 1200) ?
				(scan.startpos - 12) * 4 : (scan.startpos -
							    24) * 4;

			if ((dpi100Lumping == 0)
			    && (myscan->resolution_x ==
				Scanmode_minres(dev, myscan->scantype,
						myscan->colormode)))
				scan.ser =
					((scan.leftleading +
					  refcfg.offset_x) * 200) / 2400;
			else
				scan.ser =
					((scan.leftleading +
					  refcfg.offset_x) *
					 myscan->resolution_x) / 2400;

			scan.ler =
				((scan.startpos +
				  refcfg.offset_y) * myscan->resolution_y) /
				2400;
...
static int
Lamp_SetStatus(struct st_device *dev, SANE_Byte * Regs, int turn_on,
	       int lamp)
{
...
static void
Lamp_OverDrive(struct st_device *dev, int itime, int lamp)
{
	long ticks;
	int pwmdutycycle;
	       ...
	       int * maximus, int * minimus, double *average)
{
	...
	if ((maximus != NULL) && (minimus != NULL) && (average != NULL)) {
		...
		Regs = (SANE_Byte *) malloc(RT_BUFFER_LEN *
					    sizeof(SANE_Byte));
		if (Regs != NULL) {
			...
			image = (SANE_Byte *)
				malloc(((scancfg.coord.width * 0x21) * 3) *
				       sizeof(SANE_Byte));
			if (image != NULL) {
				gainmode =
					Lamp_GetGainMode(dev,
							 resolution & 0xffff,
							 scan.scantype);
				if (RTS_GetImage
				    (dev, Regs, &scancfg, &gain_offset, image,
				     0, OP_STATIC_HEAD, gainmode) != ERROR) {
					...
					DBG(DBG_FNC,
					    " -> GetOneLineInfo: max r=%3i g=%3i b=%3i\n",
					    maximus[CL_RED],
					    maximus[CL_GREEN],
					    maximus[CL_BLUE]);
					DBG(DBG_FNC,
...
static int
PWM_GetReady(struct st_device *dev, int resolution,
	     int * last_colour, double diff, long totaltime,
	     int interval, int value)
{
	...
	while (GetTickCount() <= totaltime) {
		rst = GetOneLineInfo(dev, resolution, maximus, minimus,
				     average);
		if (rst == OK) {
...
static int
Lamp_WaitForPWM(struct st_device *dev, int resolution, int lamp)
{
...
static int
Motor_Relocate(struct st_device *dev, int speed, int direction,
	       int ypos)
{
	int rst;
	SANE_Byte *Regs;
	...
	if (Regs != NULL) {
		...
		mtrpos.coord_y = ypos;
		mtrpos.options =
			MTR_ENABLED | ((direction == MTR_BACKWARD) ?
				       MTR_BACKWARD : MTR_FORWARD);
		mtrpos.v12e448 = 0;
		mtrpos.v12e44c = 1;
...
static int
Lamp_SetStatus2(struct st_device *dev, int turnon, int lamp)
{
...
Calib_CreateFixedBuffers()
{
	...
	while ((channel < 3) && (ret == OK)) {
		...
		if (fixed_black_shading[channel] != NULL)
			bzero(fixed_black_shading[channel],
			      0x7f8 * sizeof(USHORT));
		else
			ret = ERROR;
		...
		if (fixed_white_shading[channel] != NULL)
			bzero(fixed_white_shading[channel],
			      0x7f8 * sizeof(USHORT));
		else
			ret = ERROR;
	...
	if (my14b4 != 0) {
		...
		while ((channel < 3) && (ret == OK)) {
			...
			if ((buffer->black_shading[channel] != NULL)
				...
				for (pos = 0; pos < buffer->shadinglength;
				     pos++) {
					buffer->black_shading[channel][pos] =
						0x00;
					buffer->white_shading[channel][pos] =
						0x4000;
				}
				ret = OK;
			} else
		 ...
		 int resolution, int bitmode)
{
	...
	calibcfg->WStripXPos =
		get_value(section, WSTRIPXPOS, 0, FITCALIBRATE);
	calibcfg->WStripYPos =
		get_value(section, WSTRIPYPOS, 0, FITCALIBRATE);
	calibcfg->BStripXPos =
		get_value(section, BSTRIPXPOS, 0, FITCALIBRATE);
	calibcfg->BStripYPos =
		get_value(section, WSTRIPYPOS, 0, FITCALIBRATE);
	...
	for (a = CL_RED; a <= CL_BLUE; a++) {
		...
		calibcfg->BRef[a] =
			get_value(section, BREFR + a, 10, FITCALIBRATE);
		calibcfg->OffsetEven1[a] =
			get_value(section, OFFSETEVEN1R + a, 256,
				  FITCALIBRATE);
		calibcfg->OffsetEven2[a] =
			get_value(section, OFFSETEVEN2R + a, 0, FITCALIBRATE);
		calibcfg->OffsetOdd1[a] =
			get_value(section, OFFSETODD1R + a, 256,
				  FITCALIBRATE);
	...
	calibcfg->CalibGain10n =
		get_value(section, CALIBGAIN10N, 1, FITCALIBRATE);
	calibcfg->CalibGain20n =
		get_value(section, CALIBGAIN20N, 0, FITCALIBRATE);
	calibcfg->CalibPAGOn =
		get_value(section, CALIBPAGON, 0, FITCALIBRATE);

	for (a = CL_RED; a <= CL_BLUE; a++) {
		calibcfg->OffsetAvgTarget[a] =
			_B0(get_value
			    (section, OFFSETAVGTARGETR + a, 0x0d,
			     FITCALIBRATE));
		calibcfg->PAG[a] =
			get_value(section, PAGR + a, 3, FITCALIBRATE);
		calibcfg->Gain1[a] =
			get_value(section, GAIN1R + a, 4, FITCALIBRATE);
		calibcfg->Gain2[a] =
			get_value(section, GAIN2R + a, 4, FITCALIBRATE);
		calibcfg->WShadingPreDiff[a] =
			get_value(section, WSHADINGPREDIFFR + a, -1,
				  FITCALIBRATE);
		calibcfg->BShadingPreDiff[a] =
			get_value(section, BSHADINGPREDIFFR + a, 2,
				  FITCALIBRATE);
	}

	calibcfg->GainHeight =
		get_value(section, GAINHEIGHT, 0x1e, FITCALIBRATE);
	calibcfg->GainTargetFactor =
		get_value(section, GAINTARGETFACTOR, 0x5a,
			  FITCALIBRATE) * 0.01;
	calibcfg->TotShading =
		get_value(section, TOTSHADING, 0, FITCALIBRATE);

	/* White shading */
	calibcfg->WShadingOn =
		get_value(section, WSHADINGON, 3, FITCALIBRATE);
	calibcfg->WShadingHeight =
		get_value(section, WSHADINGHEIGHT, 0x18, FITCALIBRATE);

	/* Black shading */
	calibcfg->BShadingOn =
		get_value(section, BSHADINGON, 2, FITCALIBRATE);
	      ...
	      int arg2, int gainmode)
{
	...
	if (myRegs == NULL) {
		...
		memcpy(myRegs, &calibdata->Regs,
		       sizeof(SANE_Byte) * RT_BUFFER_LEN);
	...
	image = (SANE_Byte *) malloc(sizeof(SANE_Byte) *
	...
	if (mywidth > 0) {
		...
		for (pos = mywidth; pos > 0; pos--) {
			...
			calibcfg->unk1[CL_RED] =
				max(calibcfg->unk1[CL_RED], lf14c);
			calibcfg->unk1[CL_GREEN] =
				min(calibcfg->unk1[CL_GREEN], lf148);
			calibcfg->unk1[CL_BLUE] =
				max(calibcfg->unk1[CL_BLUE], lf178);

			calibcfg->unk2[CL_RED] =
				min(calibcfg->unk1[CL_RED], lf154);
			calibcfg->unk2[CL_GREEN] =
				min(calibcfg->unk1[CL_GREEN], lf160);
			calibcfg->unk2[CL_BLUE] =
				min(calibcfg->unk1[CL_BLUE], lf138);
	...
	if (scancfg->colormode == CM_COLOR) {
		...
		for (a = CL_RED; a <= CL_BLUE; a++) {
			dvalue = ((((*prefs * (1 << scancfg->depth)) *
				    calibcfg->GainTargetFactor) *
				   0.00390625) / *pdval) * ((44 - *pgain)
							    / 40);
...
static int
GainOffset_Save(struct st_device *dev, int * offset, SANE_Byte * gain)
{
	int rst = OK;
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		if ((offset != NULL) && (gain != NULL)) {
			...
			for (a = CL_RED; a <= CL_BLUE; a++) {
				...
				rst = RTS_EEPROM_WriteWord(dev->usb_handle,
							   0x70 + (a * 2),
							   value);
			}

			if (rst == OK)
				rst = RTS_EEPROM_WriteByte(dev->usb_handle,
							   0x76, crc);
		} else
			rst = ERROR;
	}
	     ...
	     int gainmode)
{
	...
	Regs = (SANE_Byte *) malloc(RT_BUFFER_LEN * sizeof(SANE_Byte));
	if (Regs != NULL) {
		scancfg = (struct st_scanparams *)
			malloc(sizeof(struct st_scanparams));
		if (scancfg != NULL) {
			memcpy(Regs, &calibdata->Regs,
			       RT_BUFFER_LEN * sizeof(SANE_Byte));
			memcpy(scancfg, &calibdata->scancfg,
			       sizeof(struct st_scanparams));
			...
			if (scan.scantype == ST_NORMAL) {
				...
				scancfg->coord.left = scan.ser;
				scancfg->coord.width =
					(scancfg->sensorresolution * 17) / 2;
			} else {
				scancfg->coord.left = scan.ser + v0750;
				scancfg->coord.width =
					(scancfg->sensorresolution * 3) / 2;
			}
			...
			image = (SANE_Byte *)
				malloc((scancfg->coord.height * length) *
				       sizeof(SANE_Byte));
			if (image != NULL) {
				ret = RTS_GetImage(dev, Regs, scancfg,
						   &calibdata->gain_offset,
						   image, 0, OP_STATIC_HEAD,
						   gainmode);
				if (ret == OK) {
					/* 429c105 */
					int a;
					...
					ptr[CL_RED] = image;
					ptr[CL_GREEN] =
						image + bytes_to_next_colour;
					ptr[CL_BLUE] =
						image +
						(bytes_to_next_colour * 2);
						...
						for (pos = 0;
							...
							for (a = 0;
								...
								for (b =
								     ...
								     CL_BLUE;
								     b++)
									total[b] += *(ptr[b] + ((scancfg->coord.height - a) * length));
							}
...
static int
Refs_Load(struct st_device *dev, int * left_leading,
	  int * start_pos)
{
	int ret = OK;
	...
	*left_leading = 0;
	...
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		...
		if (RTS_EEPROM_ReadWord(dev->usb_handle, 0x6a, &data) == OK) {
			*left_leading = data;
			if (RTS_EEPROM_ReadWord(dev->usb_handle, 0x6c, &data)
			    == OK) {
				*start_pos = data;
				if (RTS_EEPROM_ReadWord
				    (dev->usb_handle, 0x6e, &data) == OK) {
					if ((_B0
					     (*start_pos + *left_leading +
					      data)) == 0x5a)
						ret = OK;
				}
			}
...
Refs_Save(struct st_device *dev, int left_leading, int start_pos)
{
	...
	DBG(DBG_FNC, "+ Refs_Save(left_leading=%i, start_pos=%i)\n",
	    left_leading, start_pos);

	/* check if chipset supports accessing eeprom */
	if ((dev->chipset->capabilities & CAP_EEPROM) != 0) {
		ret = ERROR;

		if (RTS_EEPROM_WriteWord(dev->usb_handle, 0x6a, left_leading)
		    == OK) {
			if (RTS_EEPROM_WriteWord
			    (dev->usb_handle, 0x6c, start_pos) == OK) {
				SANE_Byte data =
					_B0(0x5a -
					    (start_pos + left_leading));
				ret = RTS_EEPROM_WriteByte(dev->usb_handle,
							   0x6e, data);
			}
		}
	}
...
*/
	...
	if (channels_per_dot > 0) {
		for (channel = 0; channel < channels_per_dot; channel++) {
			avgtarget[channel] =
				calibcfg->OffsetAvgTarget[channel] << 8;
	...
	scanbuffer = (SANE_Byte *)
		malloc((scancfg.bytesperline * calibcfg->OffsetHeight) *
		       sizeof(SANE_Byte));
	if (scanbuffer == NULL)
		return ERROR;
	...
	do {
		if (channels_per_dot > 0) {
			for (channel = 0; channel < channels_per_dot;
			     channel++)
				dbValues[channel] =
					(40 /
					 (44 -
		...
		gainmode =
			Lamp_GetGainMode(dev, scancfg.resolution_x,
					 scan.scantype);
		if (RTS_GetImage
		    (dev, Regs, &scancfg, &calibdata->gain_offset, scanbuffer,
		     0, scan_options, gainmode) != OK) {
			free(scanbuffer);
			return ERROR;
		}
		...
		if (RTS_Debug->SaveCalibFile != FALSE) {
			...
			imgcount++;
			if (snprintf
			    (fname, 30, "adcoffset_rt%i.tiff", imgcount) > 0)
				dbg_tiff_save(fname, scancfg.coord.width,
					      scancfg.coord.height,
					      scancfg.depth, CM_COLOR,
					      scancfg.resolution_x,
					      scancfg.resolution_y,
					      scanbuffer,
					      scancfg.bytesperline *
						...
						if (lf104 <
							...
							for (dot = 0;
								...
								colour +=
									scanbuffer
									[mvgag
									 [(lf104 * channels_per_dot)] + (dot * (channels_per_dot * 2))];
						}
				...
				if (is_ready[channel] == FALSE) {
					...
					if (colour == 0) {
						...
						myesi = (colour >
							 avgtarget[channel]) ?
							0 : 1;
						colour = abs(avgtarget
							     [channel] -
							     colour);

						if (scancfg.resolution_x >
						    600) {
							/*c923 */
							if (wvalues
							    [channel + 3] ==
	...
	for (channel = 0; channel < 3; channel++) {
		poffseteven[channel] =
			(pedcg[channel] <
			 0x100) ? 0xff - pedcg[channel] : pedcg[channel];
		poffsetodd[channel] =
			(podcg[channel] <
			 0x100) ? 0xff - podcg[channel] : podcg[channel];
	}
...
*/
	...
	DBG(DBG_FNC,
	    "> Calib_WhiteShading3(*calibcfg, *myCalib, gainmode=%i)\n",
	    gainmode);
	...
	default:		/* CM_COLOR */
		...
		bytes_per_line = ((scancfg.depth + 7) / 8) * myWidth;
		lf168 = (scancfg.samplerate ==
			 LINE_RATE) ? scancfg.coord.width : 1;
	...
	scanbuffer = (SANE_Byte *)
		malloc(((scancfg.coord.height +
			 16) * bytes_per_line) * sizeof(SANE_Byte));
	if (scanbuffer == NULL) {
		free(buffer1);
		free(buffer2);
		return ERROR;
	...
	rst = RTS_GetImage(dev, myRegs, &scancfg, &calibdata->gain_offset,
			   scanbuffer, myCalib, 0x20000080, gainmode);
			...
			do {
				...
				if (myCalib->shading_enabled != FALSE) {
					...
					if ((scancfg.colormode != CM_GRAY)
						...
						sumatorio /= lf1a4[scancfg.
								   channel];

					sumatorio =
						min(sumatorio * 0x4000,
						    65535);

					if (myRegs[0x1bf] != 0x18)
						myCalib->
							black_shading[channel]
							[lf13c] |=
							(0x140 -
							 ((((myRegs[0x1bf] >>
							     3) & 3) *
		   ...
		   struct st_calibration *myCalib, int gainmode)
{
	...
	DBG(DBG_FNC,
	    "> Calib_BlackShading(*calibcfg, *myCalib, gainmode=%i)\n",
	    gainmode);
	...
	if (calibcfg->BShadingOn == -1) {
		...
		for (a = CL_RED; a <= CL_BLUE; a++) {
			...
			myst = (maxvalue >=
				myst) ? shadingprediff[a] : maxvalue;
		...
		while (b < bytes_per_line) {
			...
			myCalib->black_shading[channel][d] |=
	...
	myCalib->shading_enabled = FALSE;
	rst = RTS_GetImage(dev, myRegs, &scancfg, &calibdata->gain_offset,
			   buffer, myCalib, 0x101, gainmode);
	if (rst == ERROR) {
		/*8ac2 */
		free(buffer);
		memcpy(&calibdata->Regs, myRegs,
		       RT_BUFFER_LEN * sizeof(SANE_Byte));
		free(myRegs);
		return ERROR;
	}
	...
	if (calibcfg->BShadingOn == -2) {
		for (a = 0; a < 3; a++) {
			dbvalue[a] =
				(dbvalue[a] / scancfg.coord.width) +
				calibcfg->ShadingCut[a];
			if (dbvalue[a] < 0)
				dbvalue[a] = 0;
			smvalues[a] =
				min((int) (dbvalue[a] + 0.5) & 0xffff,
				    maxvalue);
		}
	...
	if (calibcfg.CalibPAGOn != 0) {
	 ...
	 /*ERROR*/} else {
		...
		if ((calibdata->scancfg.colormode != CM_GRAY)
			...
			for (a = CL_RED; a <= CL_BLUE; a++)
				calibdata->gain_offset.pag[a] =
					calibcfg.PAG[calibdata->scancfg.
						     channel];
		}
	}
...
/*==2*/
		...
		if ((v14b4 != 0) && (offset[CL_RED] != 0)
			...
			/* 3e84 */
			if ((calibcfg.CalibOffset10n > 0)
			    && (calibcfg.CalibOffset10n < 4)) {
				/*if (calibcfg.CalibOffset10n != 0) */
				if (calibcfg.CalibOffset10n == 3) {
					lf900 = Calib_AdcOffsetRT(dev,
								  &calibcfg,
								  1);
				} else {
					/* 3eb2 */
	...
	if (calibcfg.TotShading != 0) {
		...
		if (gainmode != 0) {
			if (calibcfg.BShadingOn != 0)
				lf900 = Calib_BlackShading(dev, &calibcfg,
							   myCalib, gainmode);
			...
			if ((lf900 != ERROR) && (calibcfg.WShadingOn != 0)) {
				...
				default:
					break;
				case 3:
					lf900 = Calib_WhiteShading_3(dev,
								     &calibcfg,
								     myCalib,
								     gainmode);
					break;
	...
	memcpy(&mitabla2, &calibdata->gain_offset,
	       sizeof(struct st_gain_offset));

	/*4424 */
	/* Park home after calibration */
	if (get_value(SCANINFO, PARKHOMEAFTERCALIB, TRUE, FITCALIBRATE) ==
	    FALSE)
		scan.ler -= calibcfg.WShadingHeight * (((dpi100Lumping == 0)
							&& (scancfg->
							    resolution_x ==
							    Scanmode_minres
							    (dev,
...
RTS_Setup_Gamma(SANE_Byte * Regs, struct st_hwdconfig *hwdcfg)
{
	...
	if ((hwdcfg != NULL) && (Regs != NULL)) {
		if (hwdcfg->use_gamma_tables != FALSE) {
			...
			/* set set table size */
			data_bitset(&Regs[0x1d0], 0x0f,
				    hwdcfg->gamma_tablesize);
			...
			data_wide_bitset(&Regs[0x1b8], 0x3fff,
					 table_size * 2);
...
static int
RTS_EEPROM_WriteByte(USB_Handle usb_handle, int address, SANE_Byte data)
{
	int rst;
...
static int
RTS_EEPROM_ReadByte(USB_Handle usb_handle, int address, SANE_Byte * data)
{
	int rst;
...
RTS_EEPROM_WriteBuffer(USB_Handle usb_handle, int address,
		       SANE_Byte * data, int size)
{
	int rst;

	DBG(DBG_FNC,
	    "+ RTS_EEPROM_WriteBuffer(address=%04x, data, size=%i):\n",
	    address, size);
...
static void
WShading_Emulate(SANE_Byte * buffer, int * chnptr, int size,
		 int depth)
{
	if ((wshading->rates != NULL) && (chnptr != NULL)) {
		if (*chnptr < wshading->count) {
			...
			while (pos < size) {
				/* get channel color */
				chncolor =
					data_lsb_get(buffer + pos, chnsize);
		   ...
		   struct st_scanparams *scancfg)
{
	...
	calibcfg = (struct st_calibration_config *)
	...
	Calib_LoadConfig(dev, calibcfg, scan.scantype,
			 myscancfg->resolution_x, myscancfg->depth);
	gainmode =
		Lamp_GetGainMode(dev, myscancfg->resolution_x, scan.scantype);
	...
	bytes_per_line =
		myscancfg->coord.width *
		(((myscancfg->colormode ==
		   CM_COLOR) ? 3 : 1) * ((myscancfg->depth > 8) ? 2 : 1));
	...
	pattern = (SANE_Byte *)
		malloc(((myscancfg->coord.height) * bytes_per_line) *
		       sizeof(SANE_Byte));
	if (pattern == NULL)
		return ERROR;
	...
	/* Scan image */
	myCalib->shading_enabled = FALSE;
	rst = RTS_GetImage(dev, myRegs, myscancfg, &myCalibTable, pattern,
	...
	if (rst != ERROR) {
		...
		if (avg_colors != NULL) {
			...
			for (x = 0; x < wshading->count; x++) {
				...
				data_lsb_set(avg_colors +
					     (x * bytes_per_channel), a,
					     bytes_per_channel);
			}
...
#ifdef developing
		{
			...
			chn = 0;
			for (x = 0;
			     x < myscancfg->coord.height * wshading->count;
			     x++) {
				data = data_lsb_get(pattern +
						    (x * bytes_per_channel),
						    bytes_per_channel);
	  ...
	  struct st_calibration *myCalib, struct st_scanparams *scancfg)
{
	...
	for (a = 0; a < 10; a++) {
		...
		Motor_Relocate(dev, dev->motorcfg->highspeedmotormove,
			       MTR_FORWARD, 5000);
		rst = RTS_GetImage(dev, myRegs, myscancfg, &myCalibTable,
				   scanbuffer, myCalib, 0x20000000, gainmode);
		Motor_ParkHome(dev, TRUE, dev->motorcfg->parkhomemotormove);
		       ...
		       struct st_scanparams *scancfg)
{
	...
	/* Scan image */
	myCalib->shading_enabled = FALSE;
	rst = RTS_GetImage(dev, myRegs, myscancfg, &myCalibTable, scanbuffer,
	...
	if (rst != ERROR) {
		...
		if (jkd_black != NULL) {
			...
			for (x = 0; x < bytes_per_line; x++) {
				...
				for (y = 0; y < myscancfg->coord.height + 16;
				     y++)
					sumatorio +=
						scanbuffer[x +
							   (bytes_per_line *
