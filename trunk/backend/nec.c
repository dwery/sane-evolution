		...
		if (sdat->model == PCIN500) {
			...
			case 0x02:	/* not ready */
				...
				case 0x80:
					...
					case 0x10:
						DBG(1,
						    "Scanner not ready: memory error\n");
						return SANE_STATUS_IO_ERROR;
					case 0x20:
						DBG(1,
						    "Scanner not ready: hardware error\n");
						return SANE_STATUS_IO_ERROR;
					case 0x30:
						DBG(1,
						    "Scanner not ready: optical error\n");
						return SANE_STATUS_IO_ERROR;
					case 0x40:
						DBG(1,
						    "Scanner not ready: optical error\n");
					...
					case 0x50:
						DBG(1,
						    "Scanner not ready: marker error\n");
						return SANE_STATUS_IO_ERROR;
					case 0x60:
						DBG(1,
						    "Scanner not ready: mechanical error\n");
						return SANE_STATUS_IO_ERROR;
					case 0x70:
						DBG(1,
						    "Scanner not ready: hardware error\n");
...
attach(const char *devnam, NEC_Device ** devp)
{
	...
	if (inquiry_data[0] == 6
	    && strncmp((char *) inquiry_data + 8, "NEC", 3) == 0) {
		if (strncmp((char *) inquiry_data + 16, "PC-IN500/4C", 11) ==
		    0)
	...
	DBG(11, "%02x %02x %02x %02x "
	    ...
	    msp.page_code,
	    msp.page_length,
	    msp.bmu, msp.res2, msp.mud[0], msp.mud[1], msp.res3, msp.res4);
#endif
...
init_options(NEC_Scanner * s)
{
	...
	init_string_option(s, SANE_NAME_SCAN_MODE, SANE_TITLE_SCAN_MODE,
			   SANE_DESC_SCAN_MODE, mode_list, OPT_MODE,
			   MODES_COLOR);

	/* half tone */
	init_string_option(s, SANE_NAME_HALFTONE_PATTERN,
			   SANE_TITLE_HALFTONE_PATTERN,
			   SANE_DESC_HALFTONE " (not support)", halftone_list,
			   OPT_HALFTONE, 0);
	...
	/* select resolution */
	if (s->dev->sensedat.model == PCIN500)
		init_string_option(s, "Resolution", "Resolution",
				   "Resolution", resolution_list_pcin500,
				   OPT_RESOLUTION_LIST,
				   RESOLUTION_MAX_PCIN500);
	else
		init_string_option(s, "Resolution", "Resolution",
				   "Resolution", resolution_list_pcinxxx,
				   OPT_RESOLUTION_LIST,
				   RESOLUTION_MAX_PCINXXX);
#endif

	/* x & y resolution */
	s->opt[OPT_RESOLUTION].name = SANE_NAME_SCAN_RESOLUTION;
	if (s->dev->sensedat.model == PCIN500)
		s->opt[OPT_RESOLUTION].title =
			SANE_TITLE_SCAN_RESOLUTION "(x 10)";
	else
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (strcmp(mode, M_LINEART) == 0) {
		...
		s->params.bytes_per_line =
			3 * (s->params.pixels_per_line + 7) / 8;
		s->params.depth = 8;
...
static SANE_Status
send_binary_g_table(NEC_Scanner * s, int * a, int dtq)
{
	SANE_Status status;
	unsigned int i, j;
...
sane_start(SANE_Handle handle)
{
...
#ifdef USE_FORK
	{
		...
		s->shmid = shmget(IPC_PRIVATE, sizeof(NEC_rdr_ctl)
				  + s->dev->info.buffers *
				  (sizeof(NEC_shmem_ctl) +
	...
	if (s->val[OPT_PREVIEW].w == FALSE) {
		s->res = s->val[OPT_RESOLUTION].w *
			s->dev->info.res_range.quant;
	} else {
		s->res = 75;
	}
		 ...
		 int * len)
{
...
#if 0
	{
		...
		DBG(20, "buffer_status: %i ",
		    bs.fdb[0] * 256 * 256 + bs.fdb[1] * 256 + bs.fdb[2]);
	}
#endif
