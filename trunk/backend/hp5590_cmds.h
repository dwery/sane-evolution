...
static SANE_Status hp5590_vendor_product_id(enum hp_scanner_types
					    ...
					    int * vendor_id,
					    int * product_id);
static SANE_Status hp5590_init_scanner(int dn,
				       struct scanner_info **info,
				       enum hp_scanner_types scanner_type);
static SANE_Status hp5590_power_status(int dn);
static SANE_Status hp5590_read_max_scan_count(int dn,
					      unsigned int *max_count);
static SANE_Status hp5590_select_source_and_wakeup(int dn,
						   enum scan_sources source,
						   SANE_Bool
						   extend_lamp_timeout);
static SANE_Status hp5590_stop_scan(int dn);
static SANE_Status hp5590_read_scan_count(int dn, unsigned int *count);
static SANE_Status hp5590_set_scan_params(int dn,
...
static SANE_Status hp5590_send_forward_calibration_maps(int dn);
static SANE_Status hp5590_send_reverse_calibration_map(int dn);
static SANE_Status hp5590_inc_scan_count(int dn);
static SANE_Status hp5590_start_scan(int dn);
static SANE_Status hp5590_read(int dn,
			       unsigned char *bytes,
			       unsigned int size, void *state);
static SANE_Status hp5590_read_buttons(int dn,
				       enum button_status *status);
static SANE_Status hp5590_read_part_number(int dn);
static SANE_Status hp5590_calc_pixel_bits(unsigned int dpi,
					  enum color_depths color_depth,
					  unsigned int *pixel_bits);
static SANE_Status hp5590_is_data_available(int dn);
static SANE_Status hp5590_reset_scan_head(int dn);
#endif /* HP5590_H */
/* vim: sw=2 ts=8
 */
