...
_UsbWriteControl(int fd, int iValue, int iIndex, void *pabData, int iSize)
{
	...
	if (fd != -1) {
		sanei_usb_control_msg(fd, requesttype, request, iValue,
				      iIndex, iSize, pabData);
	}
	/* No error checking? */
...
_UsbReadControl(int fd, int iValue, int iIndex, void *pabData, int iSize)
{
	...
	HP5400_DBG(DBG_MSG,
		   "Read: reqtype = 0x%02X, req = 0x%02X, value = %04X\n",
...
hp5400_open(const char *filename)
{
	...
	if (status != SANE_STATUS_GOOD) {
		HP5400_DBG(DBG_MSG,
			   "hp5400_open: can't get vendor/product ids: %s\n",
			   sane_strstatus(status));
		sanei_usb_close(fd);
		return -1;
	}
	...
	if ((iVendor != 0x03F0)
	    || ((iProduct != 0x1005) && (iProduct != 0x1105))) {
		HP5400_DBG(DBG_MSG,
			   "hp5400_open: vendor/product 0x%04X-0x%04X is not "
			   "supported\n", iVendor, iProduct);
		sanei_usb_close(fd);
		return -1;
	}
...
hp5400_command_verify(int iHandle, int iCmd)
{
	...
	if (iHandle < 0) {
		HP5400_DBG(DBG_ERR,
			   "hp5400_command_verify: invalid handle\n");
		return -1;
	}
	fd = iHandle;
	...
	if (abData[0] != (iCmd >> 8)) {
		HP5400_DBG(DBG_ERR,
			   ...
			   (int) (iCmd >> 8), (int) (iCmd & 0xff),
			   (int) abData[0], (int) abData[1]);
		       ...
		       void *buffer, int len)
{
	...
	if (iHandle < 0) {
		HP5400_DBG(DBG_ERR,
			   "hp5400_command_read_block: invalid handle\n");
		return -1;
	}
	fd = iHandle;
			  ...
			  int datalen, int block, char *data)
{
	...
	if (iHandle < 0) {
		HP5400_DBG(DBG_ERR,
			   "hp5400_bulk_command_write: invalid handle\n");
		return -1;
	}
	fd = iHandle;

	HP5400_DBG(DBG_MSG,
		   "bulk_command_write(%04X,<%d bytes>,<%d bytes>)\n", iCmd,
		   cmdlen, datalen);
