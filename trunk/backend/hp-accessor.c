...
hp_accessor_bool_set(HpAccessor this, HpData data, void *valp)
{
	...
	*datap = *(SANE_Bool *) valp == FALSE ? 0 : 1;
	return SANE_STATUS_GOOD;
}
...
{
	*(SANE_Fixed *) valp =
		*(SANE_Fixed *) hp_data_data(data, this->data_offset);
	return SANE_STATUS_GOOD;
}
...
hp_accessor_choice_set(HpAccessor _this, HpData data, void *valp)
{
	...
	for (choice = this->choices; choice; choice = choice->next) {
		...
		if (strcmp((const char *) valp, choice->name) == 0) {
			*(HpChoice *) hp_data_data(data, this->data_offset) =
				choice;
			return SANE_STATUS_GOOD;
		}
	}
				 ...
				 const HpDeviceInfo * info)
{
	if (optset) {
		int old_val =
			hp_accessor_choice_getint((HpAccessor) this, data);
		HpChoice choice;
		size_t count = 0;

		for (choice = this->choices; choice; choice = choice->next)
			if (sanei_hp_choice_isEnabled
			    (choice, optset, data, info))
			     ...
			     hp_bool_t may_change)
{
	...
	for (choice = choices; choice; choice = choice->next)
		count++;
	this = sanei_hp_alloc(sizeof(*this) +
			      (count + 1) * sizeof(*this->strlist));
	if (!this)
		return 0;
...
sanei_hp_accessor_gamma_vector_new(HpData data, unsigned length,
				   unsigned depth)
{
	_HpAccessorVector this = ((_HpAccessorVector)
				  sanei_hp_accessor_vector_new(data, length,
							       depth));
...
HpAccessor
sanei_hp_accessor_geometry_new(HpAccessor val, HpAccessor lim,
			       hp_bool_t is_br, HpAccessor resolution)
{
	static const struct hp_accessor_type_s type = {
