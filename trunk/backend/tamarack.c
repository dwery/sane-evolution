...
scan_area_and_windows(Tamarack_Scanner * s)
{
	...
	case THRESHOLDED:
		...
		dwp.wdb.thresh =
			1 + 2.55 * (SANE_UNFIX(s->val[OPT_THRESHOLD].w));
		break;
	case DITHERED:
		dwp.wdb.bpp = 1;
...
get_image_status(Tamarack_Scanner * s)
{
	...
	s->params.lines = result[11] | (result[10] << 8) | (result[9] << 16);
...
read_data(Tamarack_Scanner * s, SANE_Byte * buf, int lines, int bpl)
{
	...
	if (verbose)
		DBG(1, "Read took %d.%06d seconds.",
		    dt / 1000000, dt % 1000000);
	dt = 1000000 * nbytes / dt;
	if (verbose)
		DBG(1, "which is %d.%03d bytes per second.\n", dt, 0);
#endif
	return status;
}
...
init_options(Tamarack_Scanner * s)
{
	...
	s->opt[OPT_THRESHOLD].desc =
		"Threshold: below this level is black, above is white"
...
reader_process(void *scanner)
{
	...
	for (s->line = 0; s->line < s->params.lines;
	     s->line += lines_per_buffer) {
