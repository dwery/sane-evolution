...
static int res_list[] = { 6, 100, 200, 300, 600, 1200, 2400 };
...
struct hp5590_scanner
{
	...
	int transferred_image_size;
	void *bulk_read_state;
	SANE_Bool scanning;
};

static struct hp5590_scanner *scanners_list;
...
calc_image_params(struct hp5590_scanner *scanner,
		  ...
		  unsigned int *lines, unsigned int *image_size)
{
	...
	ret = hp5590_calc_pixel_bits(scanner->dpi, scanner->depth,
				     &_pixel_bits);
	...
	DBG(DBG_verbose, "%s: pixel_bits: %u, pixels_per_line: %u, "
	    ...
	    __FUNCTION__,
	    _pixel_bits, _pixels_per_line, _bytes_per_line, _lines,
	    _image_size);
	  ...
	  SANE_Auth_Callback __sane_unused__ authorize)
{
	...
	ret = hp5590_vendor_product_id(SCANNER_HP5550, &vendor_id,
				       &product_id);
	if (ret != SANE_STATUS_GOOD)
		return ret;
	...
	ret = hp5590_vendor_product_id(SCANNER_HP5590, &vendor_id,
				       &product_id);
	if (ret != SANE_STATUS_GOOD)
		return ret;
	...
	ret = hp5590_vendor_product_id(SCANNER_HP7650, &vendor_id,
				       &product_id);
	if (ret != SANE_STATUS_GOOD)
		return ret;
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	for (ptr = scanners_list;
	     ptr && strcmp(ptr->sane.name, devicename) != 0; ptr = ptr->next);
	...
	ptr->extend_lamp_timeout = FALSE;
	ptr->wait_for_button = FALSE;
	ptr->preview = FALSE;
	ptr->quality = 4;
	ptr->image_size = 0;
	ptr->scanning = FALSE;
	...
	*handle = ptr;
	...
	opts[HP5590_OPT_NUM].size = sizeof(int);
	...
	opts[HP5590_OPT_RESOLUTION].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	opts[HP5590_OPT_RESOLUTION].constraint_type =
	...
	opts[HP5590_OPT_LAMP_TIMEOUT].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_ADVANCED;
	...
	opts[HP5590_OPT_WAIT_FOR_BUTTON].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	opts[HP5590_OPT_WAIT_FOR_BUTTON].constraint_type =
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		DBG(DBG_proc, "%s: get option '%s' value\n", __FUNCTION__,
		    scanner->opts[option].name);

		if (option == HP5590_OPT_BR_X) {
			*(SANE_Fixed *) value =
				SANE_FIX(scanner->br_x * 25.4);
		}

		if (option == HP5590_OPT_BR_Y) {
			*(SANE_Fixed *) value =
				SANE_FIX(scanner->br_y * 25.4);
		}

		if (option == HP5590_OPT_TL_X) {
			*(SANE_Fixed *) value =
				SANE_FIX((scanner->tl_x) * 25.4);
		}

		if (option == HP5590_OPT_TL_Y) {
			*(SANE_Fixed *) value =
				SANE_FIX(scanner->tl_y * 25.4);
		}
		...
		if (option == HP5590_OPT_MODE) {
			...
			case DEPTH_BW:
				...
				memcpy(value, SANE_VALUE_SCAN_MODE_LINEART,
				       strlen(SANE_VALUE_SCAN_MODE_LINEART));
				break;
			case DEPTH_GRAY:
				memset(value, 0, scanner->opts[option].size);
				memcpy(value, SANE_VALUE_SCAN_MODE_GRAY,
				       strlen(SANE_VALUE_SCAN_MODE_GRAY));
				break;
			case DEPTH_COLOR_24:
				memset(value, 0, scanner->opts[option].size);
				memcpy(value, SANE_VALUE_SCAN_MODE_COLOR_24,
				       strlen(SANE_VALUE_SCAN_MODE_COLOR_24));
				break;
			case DEPTH_COLOR_48:
				memset(value, 0, scanner->opts[option].size);
				memcpy(value, SANE_VALUE_SCAN_MODE_COLOR_48,
				       strlen(SANE_VALUE_SCAN_MODE_COLOR_48));
				break;
			default:
				return SANE_STATUS_INVAL;
			}
		...
		if (option == HP5590_OPT_SOURCE) {
			...
			case SOURCE_FLATBED:
				memset(value, 0, scanner->opts[option].size);
				memcpy(value, SANE_VALUE_SCAN_SOURCE_FLATBED,
				       strlen
				       (SANE_VALUE_SCAN_SOURCE_FLATBED));
				break;
			case SOURCE_ADF:
				memset(value, 0, scanner->opts[option].size);
				memcpy(value, SANE_VALUE_SCAN_SOURCE_ADF,
				       strlen(SANE_VALUE_SCAN_SOURCE_ADF));
				break;
	...
	if (action == SANE_ACTION_SET_VALUE) {
		...
		if (option == HP5590_OPT_MODE) {
			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_MODE_LINEART) == 0) {
				scanner->depth = DEPTH_BW;
			}

			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_MODE_GRAY) == 0) {
				scanner->depth = DEPTH_GRAY;
			}

			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_MODE_COLOR_24) == 0) {
				scanner->depth = DEPTH_COLOR_24;
			}

			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_MODE_COLOR_48) == 0) {
		...
		if (option == HP5590_OPT_SOURCE) {
			range_y.max =
				SANE_FIX(scanner->info->max_size_y * 25.4);

			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_SOURCE_FLATBED) == 0) {
				scanner->source = SOURCE_FLATBED;
				range_x.max =
					SANE_FIX(scanner->info->max_size_x *
						 25.4);
				range_y.max =
					SANE_FIX(scanner->info->max_size_y *
						 25.4);
			...
			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_SOURCE_ADF) == 0) {
			...
			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_SOURCE_ADF_DUPLEX) ==
			    0) {
			...
			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_SOURCE_TMA_SLIDES) ==
			    0) {
				scanner->source = SOURCE_TMA_SLIDES;
				range_x.max =
					SANE_FIX(TMA_MAX_X_INCHES * 25.4);
				range_y.max =
					SANE_FIX(TMA_MAX_Y_INCHES * 25.4);
				scanner->br_x = TMA_MAX_X_INCHES * 25.4;
				scanner->br_y = TMA_MAX_Y_INCHES * 25.4;
			}
			if (strcmp
			    ((char *) value,
			     (char *) SANE_VALUE_SCAN_SOURCE_TMA_NEGATIVES) ==
			    0) {
...
sane_start(SANE_Handle handle)
{
	...
	if (scanner->scanning == TRUE
		...
		|| scanner->source == SOURCE_ADF_DUPLEX)) {
		DBG(DBG_verbose,
		    "%s: Scanner is scanning, check if more data is available\n",
		    __FUNCTION__);
	...
	ret = hp5590_set_scan_params(scanner->dn, scanner->info,
				     ...
				     (scanner->br_y -
				      scanner->tl_y) * scanner->dpi,
				     scanner->dpi, scanner->depth,
				     scanner->
				     preview ? MODE_PREVIEW : MODE_NORMAL,
				     scanner->source);
	if (ret != SANE_STATUS_GOOD) {
	...
	ret = calc_image_params(scanner, NULL, NULL,
				&bytes_per_line, NULL, &scanner->image_size);
	if (ret != SANE_STATUS_GOOD) {
		hp5590_reset_scan_head(scanner->dn);
		return ret;
	}
	...
	if (scanner->depth == DEPTH_COLOR_24
	    || scanner->depth == DEPTH_COLOR_48) {
		DBG(1,
		    "Color 24/48 bits: checking if image size is correctly "
		    "aligned on number of colors\n");
		if (bytes_per_line % 3) {
			DBG(DBG_err,
			    "Color 24/48 bits: image size doesn't lined up on number of colors (3) "
		...
		DBG(1,
		    "Color 24/48 bits: image size is correctly aligned on number of colors "
		    "(image size: %u, bytes per line %u)\n",
		    scanner->image_size, bytes_per_line);
		...
		if (scanner->image_size % bytes_per_line) {
			DBG(DBG_err,
			    "Color 24/48 bits: image size doesn't lined up on bytes per line "
			    "(image size: %u, bytes per line %u)\n",
			    scanner->image_size, bytes_per_line);
			hp5590_reset_scan_head(scanner->dn);
			return SANE_STATUS_INVAL;
		...
		DBG(1,
		    "Color 24/48 bits: image size correctly aligned on bytes per line "
...
/******************************************************************************/
static SANE_Status
convert_lineart(struct hp5590_scanner *scanner, SANE_Byte * data,
		int size)
{
	int i;
...
/******************************************************************************/
static SANE_Status
convert_to_rgb(struct hp5590_scanner *scanner, SANE_Byte * data,
	       int size)
{
	unsigned int pixels_per_line;
	...
	for (j = 0; j < lines; ptr += bytes_per_line, j++) {
		...
		for (i = 0; i < pixels_per_line; i++) {
			if (scanner->depth == DEPTH_COLOR_24) {
				...
				/* G */
				buf[i * 6 + 2] =
					ptr[2 * i + bytes_per_color + 1];
				buf[i * 6 + 3] = ptr[2 * i + bytes_per_color];
				/* B */
				buf[i * 6 + 4] =
					ptr[2 * i + bytes_per_color * 2 + 1];
				buf[i * 6 + 5] =
					ptr[2 * i + bytes_per_color * 2];
			}
		}
	  ...
	  int max_length, int * length)
{
	...
	DBG(DBG_proc, "%s, length %u, left %u\n",
	    __FUNCTION__, max_length, scanner->transferred_image_size);
	...
	if (!scanner->bulk_read_state) {
		ret = hp5590_low_init_bulk_read_state(&scanner->
						      bulk_read_state);
		if (ret != SANE_STATUS_GOOD) {
			scanner->scanning = FALSE;
			return ret;
	...
	*length = max_length;
	...
	ret = hp5590_read(scanner->dn, data, *length,
