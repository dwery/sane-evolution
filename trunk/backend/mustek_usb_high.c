		   ...
		   SANE_Word * dark_needed)
{
	...
	*dark_needed = cal->dark_needed;
	...
	cal->white_buffer =
		(SANE_Int *) malloc(cal->white_needed * cal->width *
				    sizeof(SANE_Int));
	if (!cal->white_buffer)
		return SANE_STATUS_NO_MEM;
...
usb_high_cal_i8o8_mono_calibrate(Calibrator * cal, void *src, void *target)
{
	...
	if (cal->gamma_table == NULL) {
		...
		for (i = 0; i < cal->width; i++) {
			...
			if (base < 0)
				base = 0;
			value = ((SANE_Word) (base) * k_white_level) /
				cal->k_white[i];
			if (value > 0x00ff)
				value = 0x00ff;
		...
		for (i = 0; i < cal->width; i++) {
			...
			if (base < 0)
				base = 0;
			value = ((SANE_Word) (base) * cal->k_white_level) /
				cal->k_white[i];
			if (value > 0x0fff)
				value = 0x0fff;
			gray_target[i] =
...
usb_high_cal_i4o1_calibrate(Calibrator * cal, void *src, void *target)
{
	...
	*local_target = 0;
	while (j < cal->width) {
		base = (SANE_Int) ((SANE_Word) (*local_src & 0xf0) << 4)
