...
static TState
ReadNextGrayLine(PTInstance this)
{
	int iWrite;
	int iDot;
	...
	while (iWrite < this->state.cxMax) {	/* max 1 time in reality */
		while (iWrite < this->state.cxMax &&
			...
			this->state.ppchLines[0][iWrite++] +=	/* add! */
				this->state.pchBuf[this->state.
						   iBulkReadPos++] << 4;
		if (iWrite < this->state.cxMax) {	/* we need an additional chunk */
			if (this->state.bLastBulk)
				return SANE_STATUS_EOF;
			this->state.cchBulk =
				BulkReadBuffer(this, this->state.pchBuf,
					       USB_CHUNK_SIZE);
			dprintf(DEBUG_SCAN,
				"bulk read: %d byte(s), line #%d\n",
				this->state.cchBulk, this->state.iLine);
			if (this->bWriteRaw)
				fwrite(this->state.pchBuf, 1,
				       this->state.cchBulk, this->fhScan);
			INST_ASSERT();
	...
	this->state.iLine++;
	iDot = 0;
	chBits = 0;		/* init pixelbuffer */
	for (nInterpolator = 50, iWrite = 0, iRead = 0;
	     iRead < this->state.cxMax; iRead++) {
		nInterpolator += this->state.nFixAspect;
		if (nInterpolator < 100)
			continue;	/* res. reduction */
		nInterpolator -= 100;
		if (iWrite >= this->state.cchLineOut)
			continue;
		...
		if (this->mode == gray)
			...
			if (this->mode == line)
				chBit = (this->state.ppchLines[0][iRead] <
					 LINE_THRESHOLD);
			else {
				short nError =
					this->state.ppchLines[0][iRead];
				/* printf("(%d)",nError); */
				if (nError >= 0xFF0) {
					nError -= 0xFF0;
					chBit = 0;
...
#define FASTDITHER
				...
				this->state.ppchLines[1][iRead + 1] +=
					(nError >> 1);
				this->state.ppchLines[1][iRead] += (nError >> 2);	/* 8/16 */
#else
				this->state.ppchLines[0][iRead + 1] +=
					(nError * 5) >> 4;
				this->state.ppchLines[1][iRead + 1] +=
					(nError * 8) >> 4;
	...
	/* cycle the history lines and clear the preread buffer */
	{
		...
		memset(this->state.ppchLines[1], 0,
		       (this->state.cxMax + 1) * sizeof(short));
	}
...
StartScanGray(TInstance * this)
{
	 ...
	 */
	RegWriteArray(this, R_ALL, NUM_SCANREGS, puchRegs);
	INST_ASSERT();
	RegWrite(this, R_SPOS, 2,
		 this->param.x / 2 + this->calibration.xMargin);
	INST_ASSERT();
	RegWrite(this, R_SLEN, 2, this->state.cyWindow);
	INST_ASSERT();
	RegWrite(this, R_SWID, 2, this->state.cxWindow);
	INST_ASSERT();
	RegWrite(this, R_STPS, 2, 0);
	INST_ASSERT();

	/* upload gamma table */
	RegWrite(this, 0x41, 1, 0x01);	/* gamma, gray */
	RegWrite(this, 0x40, 1, 0x20);	/* FIFO at   0x08000 */
	UploadGammaTable(this, 0, this->agammaY);
	INST_ASSERT();
	...
	for (i = 0; i < this->state.cBacklog; i++) {
		this->state.ppchLines[i] = calloc(this->state.cxMax + 1, sizeof(short));	/* 1 dummy at right edge */
	...
	this->state.cchLineOut = (this->mode == gray)
		? this->state.cxPixel : (this->state.cxPixel + 7) / 8;
	...
	RegWrite(this, R_CTL, 1, 0x39);
	INST_ASSERT();
	RegWrite(this, R_CTL, 1, 0x79);
	INST_ASSERT();
	RegWrite(this, R_CTL, 1, 0xF9);
	INST_ASSERT();
