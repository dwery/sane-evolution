			  ...
			  GT68xx_Scan_Parameters * params)
{
	...
	case SA_SCAN:
	{
		...
		if (strcmp(dev->model->command_set->name, "mustek-gt6816") !=
		    0)
			request->mbs = TRUE;	/* always go home for gt6801 scanners */
		if (request->use_ta) {
	...
	if (action == SA_SCAN && color) {
		...
		int max_ld =
			MAX(MAX(ld_shift_r, ld_shift_g), ld_shift_b);
		...
		params->ld_shift_double = 0;
		DBG(6,
		    "gt68xx_generic_setup_scan: overscan=%d, ld=%d/%d/%d\n",
	...
	if (action == SA_SCAN && xdpi >= model->optical_xdpi
		...
		DBG(6,
		    "gt68xx_generic_setup_scan: overscan=%d, ld double=%d\n",
		    overscan_lines, params->ld_shift_double);
	}
	...
	if (bits_per_line % 8) {	/* impossible */
		DBG(0,
		    "gt68xx_generic_setup_scan: BUG: unaligned bits_per_line=%d\n",
		    bits_per_line);
		return SANE_STATUS_INVAL;
	}
	...
	if (scan_bpl % 64) {	/* impossible */
		DBG(0,
		    "gt68xx_generic_setup_scan: BUG: unaligned scan_bpl=%d\n",
		    scan_bpl);
		return SANE_STATUS_INVAL;
	}
	...
	if (!request->calculate) {
		...
		if (scan_bpl > (16 * 1024)) {
			DBG(0,
			    "gt68xx_generic_setup_scan: scan_bpl=%d, too large\n",
			    scan_bpl);
		...
		if ((action != SA_SCAN)
		    && (strcmp(dev->model->command_set->name, "mustek-gt6816")
			== 0))
			motor_mode_2 |= 1 << 3;
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(3,
			    "gt68xx_generic_setup_scan: setup request failed: %s\n",
