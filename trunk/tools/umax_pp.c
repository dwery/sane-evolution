...
/* scanning parameters : defaults to preview (75 dpi color, full scan area) */
	...
	while (i < argc) {
		...
		if ((strcmp(argv[i], "-c") == 0)
			...
			if (color == 0) {
				fprintf(stderr,
					"unexpected color mode value <%s>\n",
					argv[i]);
				fprintf(stderr,
					"Must be RGB, RGB12, BW, or BW12\n");
				return 0;
			}
		}
		...
		if ((strcmp(argv[i], "-d") == 0)
			...
			if ((dpi < 75) || (dpi > 1200)) {
				fprintf(stderr,
					"dpi value has to be between 75 and 1200\n");
				return 0;
			}
			if ((dpi != 75)
			    && (dpi != 150)
		...
		if ((strcmp(argv[i], "-g") == 0)
			...
			if (i == (argc - 1)) {
				Usage(argv[0]);
				fprintf(stderr,
					"expected hex gain value ( ex: A59 )\n");
				return 0;
			}
			i++;
			found = 1;
		...
		if ((strcmp(argv[i], "-z") == 0)
			...
			if (i == (argc - 1)) {
				Usage(argv[0]);
				fprintf(stderr,
					"expected hex offset value ( ex: A59 )\n");
				return 0;
			}
			i++;
			found = 1;
		...
		if ((strcmp(argv[i], "-a") == 0)
			...
			if (i == (argc - 1)) {
				Usage(argv[0]);
				fprintf(stderr,
					"expected hex io port value ( ex: 3BC )\n");
				return 0;
			}
			i++;
			found = 1;
			if ((strlen(argv[i]) < 3) || (strlen(argv[i]) > 4)) {
				Usage(argv[0]);
				fprintf(stderr,
					"expected hex io port value ( ex: 378 )\n");
				return 0;
			}
	...
	if ((name == NULL) && (port == 0)) {
		...
		if (rc != 1) {
			...
			rc = 0;
			while ((ports[i] != NULL) && (rc != 1)) {
				rc = sanei_umax_pp_initPort(strtol
							    (ports[i], NULL,
							     16), NULL);
				i++;
			}
		...
		if (rc != 1) {
			fprintf(stderr,
				"failed to detect a valid device or port!\n");
			return 0;
		}
	} else {
		if (sanei_umax_pp_initPort(port, name) != 1) {
			if (port)
				fprintf(stderr,
					"failed to gain direct acces to port 0x%X!\n",
					port);
			else
				fprintf(stderr,
					"failed to gain acces to device %s!\n",
					name);
			return 0;
		}
	}
	if (trace) {
		printf("UMAX 610P/1220P/2000P scanning program version 6.4 starting ...\n");
