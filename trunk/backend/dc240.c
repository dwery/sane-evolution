...
init_dc240(DC240 * camera)
{
	...
	if ((camera->fd = open(camera->tty_name, O_RDWR)) == -1) {
		DBG(1,
		    "init_dc240: error: could not open %s for read/write\n",
		    camera->tty_name);
		return -1;
	}
	/*
...
/* NEW */
static int
read_data(int fd, SANE_Byte * buf, int sz)
{
	SANE_Byte ccsum;
	SANE_Byte rcsum;
	SANE_Byte c;
	...
	while (retries++ < 5) {
		...
		if (c != 1 && c != 0) {
			DBG(1,
			    "read_data: error: incorrect packet control byte: %02x\n",
			    c);
			return -1;
		}
		...
		if (n < sz || read(fd, &rcsum, 1) != 1) {
			DBG(2,
			    "read_data: warning: buffer underrun or no checksum\n");
			continue;
		}
...
end_of_data(int fd)
{
	...
	do {			/* loop until the camera isn't busy */
		if ((n = read(fd, &c, 1)) == -1) {
...
static int
change_res(int fd, SANE_Byte res)
{
	char f[] = "change_res";
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		case DC240_OPT_THUMBS:
			dc240_opt_thumbnails = !!*(int *) value;
		...
		case DC240_OPT_SNAP:
			...
			if (dc240_opt_snap) {
				...
				sod[DC240_OPT_LOWRES].cap &=
					~SANE_CAP_INACTIVE;
				/* and de-activate the image number selector */
				sod[DC240_OPT_IMAGE_NUMBER].cap |=
					SANE_CAP_INACTIVE;
			} else {
...
*/
		...
		case DC240_OPT_ERASE:
			dc240_opt_erase = !!*(int *) value;
			break;

		case DC240_OPT_AUTOINC:
			dc240_opt_autoinc = !!*(int *) value;
			break;
	...
	case SANE_ACTION_GET_VALUE:
		...
		case DC240_OPT_IMAGE_NUMBER:
			*(int *) value = Camera.current_picture_number;
			break;

		case DC240_OPT_THUMBS:
			*(int *) value = dc240_opt_thumbnails;
			break;

		case DC240_OPT_SNAP:
			*(int *) value = dc240_opt_snap;
			break;

		case DC240_OPT_LOWRES:
			*(int *) value = dc240_opt_lowres;
			break;

		case DC240_OPT_ERASE:
			*(int *) value = dc240_opt_erase;
			break;

		case DC240_OPT_AUTOINC:
			*(int *) value = dc240_opt_autoinc;
			break;
 ...
 */
	...
	if (dc240_opt_thumbnails) {
		...
		if (parms.pixels_per_line != 160 ||
			...
			DBG(1,
			    "sane_start: fixme! thumbnail image size is wrong\n");
			return SANE_STATUS_INVAL;
		}
	} else {
	...
	}
	{
		...
		cinfo.src =
			(struct jpeg_source_mgr *) (*cinfo.mem->
						    alloc_small) ((j_common_ptr) & cinfo, JPOOL_PERMANENT, sizeof(my_source_mgr));
	...
	Camera.scanning = TRUE;	/* don't overlap scan requests */
...
sane_cancel(SANE_Handle UNUSEDARG handle)
{
	...
	if (Camera.scanning) {
		...
		Camera.scanning = FALSE;	/* done with scan */
	} else
...
get_picture_info(PictureInfo * pic, int p)
{
	...
	pic->low_res = info_buf[3] == 0 ? TRUE : FALSE;
...
read_dir(char * dir)
{
	...
	if (entries > 1001) {
		DBG(1,
		    "%s: error: more than 999 pictures not supported yet\n",
		    f);
		return -1;
	}
	...
	while (dir_buf2 + 2 + CAMDIRENTRYSIZE * entries >=
	       (SANE_Byte *) next_buf) {
	...
	for (i = 0; i < entries; i++) {
		...
		if (dir_insert
		    ((struct cam_dirent *) &dir_buf2[2 + CAMDIRENTRYSIZE * i])
		    != 0) {
