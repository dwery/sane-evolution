...
static char inquiry_data[255] = "Bell+Howell scanner";
...
ScannerDump(BH_Scanner * s)
{
	...
	DBG(1, "Max bytes per scan-line: %d (%d pixels)\n",
	    info->lineMaxBytes, info->lineMaxBytes * 8);

	DBG(1, "Basic resolution (X/Y): %d/%d\n",
	    info->resBasicX, info->resBasicY);

	DBG(1, "Maximum resolution (X/Y): %d/%d\n",
	    info->resMaxX, info->resMaxY);

	DBG(1, "Minimum resolution (X/Y): %d/%d\n",
	    info->resMinX, info->resMinY);
	...
	DBG(1,
	    ...
	    (info->resBasicY) ? ((float) info->winHeight) /
	    info->resBasicY : 0.0);
...
read_barcode_data(BH_Scanner * s, FILE * fp)
{
	...
	s->barcode_not_found = FALSE;
	do {
		memset(&buf, 0, sizeof(buf));
		status = sanei_scsi_cmd(s->fd, &cmd, sizeof(cmd), &buf,
					&buf_size);
		if (status != SANE_STATUS_GOOD)
			break;
		if (s->barcode_not_found == TRUE)
			break;
		...
		fprintf(fp, " <orientation>%s</orientation>\n",
			print_orientation((int)
					  _2btol(buf.barcodeorientation)));
		fprintf(fp, " <location>\n  <tl><x>%d</x><y>%d</y></tl>\n",
...
read_icon_data(BH_Scanner * s)
{
	...
	DBG(3, "read_icon_data: windowwidth:%lu, windowlength:%lu\n",
	    _4btol(buf.windowwidth), _4btol(buf.windowlength));
	DBG(3,
	    "read_icon_data: iconwidth:%lu, iconlength:%lu, iconwidth(bytes):%lu\n",
	    _4btol(buf.iconwidth), _4btol(buf.iconlength),
	    _4btol(buf.iconwidthbytes));
	DBG(3, "read_icon_data: bitordering:%02x, icondatalen:%lu\n",
	    buf.bitordering, _4btol(buf.icondatalen));
...
mode_select_barcode_priority(BH_Scanner * s)
{
	...
	for (i = 0; i < NUM_SEARCH_BARS; i++) {
		...
		if ((select_cmd.mp.priority[i] = s->search_bars[i]) == 0)
			break;
	}
...
mode_select_barcode_param1(BH_Scanner * s)
{
	...
	_lto2b((int) _OPT_VAL_WORD_THOUSANDTHS(s, OPT_BARCODE_HMIN),
	       select_cmd.mp.minbarheight);
	select_cmd.mp.searchcount =
		_OPT_VAL_WORD(s, OPT_BARCODE_SEARCH_COUNT);
	select_cmd.mp.searchmode =
		get_barcode_search_mode(_OPT_VAL_STRING
					(s, OPT_BARCODE_SEARCH_MODE));
	_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_SEARCH_TIMEOUT),
	       select_cmd.mp.searchtimeout);
...
mode_select_barcode_param2(BH_Scanner * s)
{
	...
	status = sanei_scsi_cmd(s->fd, &select_cmd.cmd,
				sizeof(select_cmd.cmd), &select_cmd.mp, &len);

	if (status == SANE_STATUS_GOOD) {
		DBG(8,
		    "mode_select_barcode_param2: sensed values: relmax:%d barmin:%d barmax:%d\n",
		...
		if (_OPT_VAL_WORD(s, OPT_BARCODE_RELMAX) != 0) {
			_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_RELMAX),
			       select_cmd.mp.relmax);
		}
		if (_OPT_VAL_WORD(s, OPT_BARCODE_BARMIN) != 0) {
			_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_BARMIN),
			       select_cmd.mp.barmin);
		}
		if (_OPT_VAL_WORD(s, OPT_BARCODE_BARMAX) != 0) {
			_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_BARMAX),
			       select_cmd.mp.barmax);
		}

		DBG(8,
		    "mode_select_barcode_param2: param values: relmax:%d barmin:%d barmax:%d\n",
		    (int) _OPT_VAL_WORD(s, OPT_BARCODE_RELMAX),
		    (int) _OPT_VAL_WORD(s, OPT_BARCODE_BARMIN),
		    (int) _OPT_VAL_WORD(s, OPT_BARCODE_BARMAX));

		DBG(8,
		    "mode_select_barcode_param2: select values: relmax:%d barmin:%d barmax:%d\n",
		    (int) _2btol(select_cmd.mp.relmax),
		    (int) _2btol(select_cmd.mp.barmin),
		    (int) _2btol(select_cmd.mp.barmax));

		status = sanei_scsi_cmd(s->fd, &select_cmd,
					sizeof(select_cmd), 0, 0);
	}
...
mode_select_barcode_param3(BH_Scanner * s)
{
	...
	if (status == SANE_STATUS_GOOD) {
		...
		if (_OPT_VAL_WORD(s, OPT_BARCODE_CONTRAST) != 0) {
			_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_CONTRAST),
			       select_cmd.mp.barcodecontrast);
		}
		if (_OPT_VAL_WORD(s, OPT_BARCODE_PATCHMODE) != 0) {
			_lto2b(_OPT_VAL_WORD(s, OPT_BARCODE_PATCHMODE),
			       select_cmd.mp.patchmode);
		}
		...
		DBG(8,
		    "mode_select_barcode_param3: select values: contrast:%d patchmode:%d\n",
		    (int) _2btol(select_cmd.mp.barcodecontrast),
		    (int) _2btol(select_cmd.mp.patchmode));
...
set_window(BH_Scanner * s, SANE_Byte batchmode)
{
	...
	DBG(3, "set_window: sizeof(hdr) %d, sizeof(window): %d\n",
	    ...
	    (int) sizeof(set_window_cmd.window));
	...
	_lto4b((int) _OPT_VAL_WORD_THOUSANDTHS(s, OPT_TL_X),
	       set_window_cmd.window.ulx);
	_lto4b((int) _OPT_VAL_WORD_THOUSANDTHS(s, OPT_TL_Y),
	       set_window_cmd.window.uly);
	...
	if (_OPT_VAL_WORD(s, OPT_NEGATIVE) == TRUE) {
	...
	if (set_window_cmd.window.autoborder) {
					/*--- setting byte 46 of the window descriptor block only works with autoborder */
		rotation = get_rotation_id(_OPT_VAL_STRING(s, OPT_ROTATION));
		if (_OPT_VAL_WORD(s, OPT_DESKEW) == TRUE)
			deskew = BH_DESKEW_ENABLE;
		else
			deskew = BH_DESKEW_DISABLE;
		set_window_cmd.window.border_rotation = (rotation | deskew);
								    /*--- deskew assumes autoborder */
	}
	...
	if (set_window_cmd.window.remote == 0x00) {
		...
		set_window_cmd.window.acefunction =
			_OPT_VAL_WORD(s, OPT_ACE_FUNCTION);
		/* acesensitivity (ignored on non-ACE scanners) */
		set_window_cmd.window.acesensitivity =
			_OPT_VAL_WORD(s, OPT_ACE_SENSITIVITY);
	}
	...
	status = sanei_scsi_cmd(s->fd, &set_window_cmd,
				sizeof(set_window_cmd), 0, 0);
...
static SANE_Status
get_window(BH_Scanner * s, int * w, int * h, SANE_Bool backpage)
{
	SANE_Byte cmd[10];
	static struct
	{
	...
	while (1) {
		...
		get_window_data.window.windowid =
			(backpage == TRUE) ? 1 : 0;
			...
			*h = _4btol(get_window_data.window.windowlength);
			...
			if (autoborder) {
				...
				if (get_window_data.window.autoborder != 1 &&
					...
					DBG(5,
					    "waiting %d second[s], try: %d\n",
					    get_window_delay, i);
					sleep(get_window_delay);
					    /*--- page 4-5 of B&H Copiscan 8000 ESC OEM Tech Manual */
				...
				if (get_window_data.window.autoborder != 1) {
					DBG(1,
					    "Automatic Border Detection not done within %d tries\n",
					    BH_AUTOBORDER_TRIES);
					status = SANE_STATUS_IO_ERROR;
				}
...
get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (s->scanning) {
		...
		if (itemtype == BH_SCSI_READ_TYPE_FRONT) {
			DBG(3,
			    "get_parameters: sending GET WINDOW (front)\n");
			status = get_window(s, &w, &l, FALSE);
			if (status == SANE_STATUS_GOOD) {
				width = w;
				length = l;
			}
			...
			length = s->iconlength;
			   ...
			   itemtype <=
			   (BH_SCSI_READ_TYPE_FRONT + NUM_SECTIONS)) {
			/* a front section */
			...
			length = s->sections[sectnum -
					     1].length * res / 1000.0;
		} else if (itemtype > BH_SCSI_READ_TYPE_BACK
			   && itemtype <=
			   (BH_SCSI_READ_TYPE_BACK + NUM_SECTIONS)) {
			/* a back section */
			...
			length = s->sections[sectnum -
			    ...
			    && itemtype <=
			    (BH_SCSI_READ_TYPE_BACK_BARCODE + NUM_SECTIONS))
			   || (itemtype >= BH_SCSI_READ_TYPE_FRONT_BARCODE
			       && itemtype <=
			       (BH_SCSI_READ_TYPE_FRONT_BARCODE +
				NUM_SECTIONS))) {
			...
			length = -1;
			DBG(1,
			    "get_parameters: unrecognized read itemtype: %d\n",
			    itemtype);
		}
	...
	if (res <= 0 || width <= 0) {
		DBG(1,
		    "get_parameters:illegal parameters res=%d, width=%d, length=%d\n",
		    res, width, length);
	...
	DBG(1, "get_parameters: format=%d, pixels/line=%d, bytes/line=%d, "
	    ...
	    s->params.bytes_per_line, s->params.lines, res);
		...
		do {
			...
			while (f) {
				...
				if (strcmp(f, "front") == 0)
					...
					DBG(1,
					    "section_parse: ignoring unrecognized function "
					    "code '%s'\n", f);
...
setup_sections(BH_Scanner * s, const char *val)
{
	...
	if (strlen(val) > sizeof(buf) - 1) {
		...
		while (section != NULL && sectnum < NUM_SECTIONS) {
			if (!allblank(section)) {
				int res =
					_OPT_VAL_WORD(s, OPT_RESOLUTION);
				int format =
					get_compression_id(_OPT_VAL_STRING(s,
									   OPT_COMPRESSION));

				status = section_parse(section,
						       &s->sections[sectnum],
						       res, format);
				if (status != SANE_STATUS_GOOD) {
					DBG(1,
...
start_setup(BH_Scanner * s)
{
	...
	for (i = 0; i < s->num_sections; i++) {
		int flags = s->sections[i].flags;

		if (flags & BH_SECTION_FRONT_IMAGE)
			s->readlist[s->readcnt++] =
				BH_SCSI_READ_TYPE_FRONT + i + 1;
		if (flags & BH_SECTION_BACK_IMAGE)
			s->readlist[s->readcnt++] =
				BH_SCSI_READ_TYPE_BACK + i + 1;
	}
	...
	if (s->barcodes) {
		if (s->num_sections == 0) {
			...
			if (duplex == TRUE) {
				s->readlist[s->readcnt++] =
					BH_SCSI_READ_TYPE_BACK_BARCODE;
			}
		} else {
	...
	if (_OPT_VAL_WORD(s, OPT_BATCH) == TRUE) {
	...
	DBG(5, "start_setup: duplex=%s, barcodes=%s, patchcodes=%s, "
	    "icons=%s, batch=%s\n",
	    (duplex == TRUE) ? "yes" : "no",
	    (s->barcodes == TRUE) ? "yes" : "no",
	    (s->patchcodes == TRUE) ? "yes" : "no",
	    (s->icons == TRUE) ? "yes" : "no",
	...
	for (i = 0; i < s->num_sections; i++) {
		DBG(5, "start_setup:  "
		    ...
		    s->sections[i].flags,
		    s->sections[i].compressiontype,
		    s->sections[i].compressionarg);
	}
	DBG(5, "start_setup: read list length=%d\n", s->readcnt);
	for (i = 0; i < s->readcnt; i++) {
		DBG(5, "start_setup:  [%d] %s\n", i + 1,
		    print_read_type(s->readlist[i]));
	}
	...
	if (s->barcodes == TRUE) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "start_setup: mode_select_barcode_priority failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "start_setup: mode_select_barcode_param1 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "start_setup: mode_select_barcode_param2 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "start_setup: mode_select_barcode_param3 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
	}
...
start_scan(BH_Scanner * s)
{
	...
	if (s->readcnt > 1 && s->scanning == TRUE) {
		...
		if (s->readptr < s->readcnt) {
			...
			for (; s->readptr < s->readcnt; s->readptr++) {
				...
				DBG(3,
				    "start_scan: advance readlist(%d, %d)\n",
				    s->readptr, (int) itemtype);
				...
				if (!BH_HAS_IMAGE_DATA(itemtype)) {
					...
					strncpy(s->barfname, "/tmp/bhXXXXXX",
						sizeof(s->barfname));
					s->barfname[sizeof(s->barfname) - 1] =
						'\0';
					...
					if ((mktemp(s->barfname) == NULL) &&
						...
						if ((s->barf =
						     fopen(s->barfname,
							   "r")) == NULL) {
							DBG(1,
							    "sane_start: error opening barfile `%s'\n",
							    s->barfname);
							status = SANE_STATUS_IO_ERROR;
						}
					} else {
...
sense_handler(int scsi_fd, u_char * result, void *arg)
{
	...
	for (i = 0; i < 16; i++) {
		sprintf(print_sense + strlen(print_sense), "%02x ",
			result[i]);
	}
...
init_options(BH_Scanner * s)
{
	...
	if (s->hw->info.colorHalftone == FALSE) {
		s->opt[OPT_SCAN_MODE].size =
			max_string_size(scan_mode_min_list);
		s->opt[OPT_SCAN_MODE].constraint.string_list =
			scan_mode_min_list;
	}

	if (s->hw->info.comprG3_1D == FALSE ||
	    s->hw->info.comprG3_2D == FALSE ||
	    s->hw->info.comprG4 == FALSE) {
	...
	if (s->hw->info.canBorderRecog == FALSE) {
	...
	if (s->hw->info.canCheckADF == FALSE) {
		s->opt[OPT_CHECK_ADF].cap |= SANE_CAP_INACTIVE;
	}

	if (s->hw->info.canDuplex == FALSE) {
		s->opt[OPT_DUPLEX].cap |= SANE_CAP_INACTIVE;
	}

	if (s->hw->info.canADF == FALSE) {
	...
	s->opt[OPT_BARCODE_SEARCH_BAR].constraint.string_list =
		barcode_search_bar_list;
	s->opt[OPT_BARCODE_SEARCH_BAR].size =
		max_string_size(barcode_search_bar_list);
	s->val[OPT_BARCODE_SEARCH_BAR].s = strdup(barcode_search_bar_list[0]);

	/* Barcode search count (1-7, default 1). */
	s->opt[OPT_BARCODE_SEARCH_COUNT].name =
		SANE_NAME_BARCODE_SEARCH_COUNT;
	s->opt[OPT_BARCODE_SEARCH_COUNT].title =
		SANE_TITLE_BARCODE_SEARCH_COUNT;
	s->opt[OPT_BARCODE_SEARCH_COUNT].desc =
		SANE_DESC_BARCODE_SEARCH_COUNT;
	s->opt[OPT_BARCODE_SEARCH_COUNT].type = SANE_TYPE_INT;
	s->opt[OPT_BARCODE_SEARCH_COUNT].unit = SANE_UNIT_NONE;
	s->opt[OPT_BARCODE_SEARCH_COUNT].constraint_type =
		SANE_CONSTRAINT_RANGE;
	s->opt[OPT_BARCODE_SEARCH_COUNT].constraint.range =
		&barcode_search_count_range;
	...
	s->opt[OPT_BARCODE_SEARCH_MODE].title =
		SANE_TITLE_BARCODE_SEARCH_MODE;
	s->opt[OPT_BARCODE_SEARCH_MODE].desc = SANE_DESC_BARCODE_SEARCH_MODE;
	s->opt[OPT_BARCODE_SEARCH_MODE].type = SANE_TYPE_STRING;
	s->opt[OPT_BARCODE_SEARCH_MODE].size =
		max_string_size(barcode_search_mode_list);
	s->opt[OPT_BARCODE_SEARCH_MODE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	s->opt[OPT_BARCODE_SEARCH_MODE].constraint.string_list =
		barcode_search_mode_list;
	s->val[OPT_BARCODE_SEARCH_MODE].s =
		strdup(barcode_search_mode_list[0]);
	...
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].name =
		SANE_NAME_BARCODE_SEARCH_TIMEOUT;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].title =
		SANE_TITLE_BARCODE_SEARCH_TIMEOUT;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].desc =
		SANE_DESC_BARCODE_SEARCH_TIMEOUT;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].type = SANE_TYPE_INT;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].unit = SANE_UNIT_MICROSECOND;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].constraint_type =
		SANE_CONSTRAINT_RANGE;
	s->opt[OPT_BARCODE_SEARCH_TIMEOUT].constraint.range =
		&barcode_search_timeout_range;
	...
	s->opt[OPT_BARCODE_CONTRAST].constraint.range =
		&barcode_contrast_range;
	...
	s->opt[OPT_BARCODE_PATCHMODE].constraint.range =
		&barcode_patchmode_range;
	s->val[OPT_BARCODE_PATCHMODE].w = 0;

	if (s->hw->info.canSection == FALSE) {
		s->opt[OPT_SECTION].cap |= SANE_CAP_INACTIVE;
	}

	if (s->hw->info.canBarCode == FALSE) {
...
attach(const char *devnam, BH_Device ** devp)
{
...
#ifdef FAKE_INQUIRY
	if (fake_inquiry) {
		dev->info.canADF = FALSE;
		dev->info.colorBandW = TRUE;
		dev->info.colorHalftone = TRUE;
		dev->info.canWhiteFrame = TRUE;
		dev->info.canBlackFrame = TRUE;
		dev->info.canEdgeExtract = TRUE;
		dev->info.canNoiseFilter = TRUE;
		dev->info.canSmooth = TRUE;
		dev->info.canLineBold = TRUE;
		dev->info.comprG3_1D = TRUE;
		dev->info.comprG3_2D = TRUE;
		dev->info.comprG4 = TRUE;
		dev->info.canBorderRecog = TRUE;
		dev->info.canBarCode = TRUE;
		dev->info.canIcon = TRUE;
		dev->info.canSection = TRUE;
	...
	if (jbuf.standardres[0] & 0x80)
		appendStdList(&dev->info, 60);
	if (jbuf.standardres[0] & 0x40)
		appendStdList(&dev->info, 75);
	if (jbuf.standardres[0] & 0x20)
		appendStdList(&dev->info, 100);
	if (jbuf.standardres[0] & 0x10)
		appendStdList(&dev->info, 120);
	if (jbuf.standardres[0] & 0x08)
		appendStdList(&dev->info, 150);
	if (jbuf.standardres[0] & 0x04)
		appendStdList(&dev->info, 160);
	if (jbuf.standardres[0] & 0x02)
		appendStdList(&dev->info, 180);
	if (jbuf.standardres[0] & 0x01)
		appendStdList(&dev->info, 200);
	if (jbuf.standardres[1] & 0x80)
		appendStdList(&dev->info, 240);
	if (jbuf.standardres[1] & 0x40)
		appendStdList(&dev->info, 300);
	if (jbuf.standardres[1] & 0x20)
		appendStdList(&dev->info, 320);
	if (jbuf.standardres[1] & 0x10)
		appendStdList(&dev->info, 400);
	if (jbuf.standardres[1] & 0x08)
		appendStdList(&dev->info, 480);
	if (jbuf.standardres[1] & 0x04)
		appendStdList(&dev->info, 600);
	if (jbuf.standardres[1] & 0x02)
		appendStdList(&dev->info, 800);
	if (jbuf.standardres[1] & 0x01)
		appendStdList(&dev->info, 1200);
	...
	dev->info.batch_default = FALSE;
	dev->info.deskew_default = FALSE;
	dev->info.check_adf_default = FALSE;
	dev->info.duplex_default = FALSE;
	...
	if (fp) {
		...
		while (sanei_config_read(line, sizeof(line), fp)) {
			...
			if (strncmp(lp, "option", 6) == 0 &&
				...
				if (strncmp(lp, "disable-optional-frames", 23)
				    == 0) {
					DBG(1,
					    "sane_init: configuration option "
					    "'disable-optional-frames' set\n");
					disable_optional_frames = 1;
				} else if (strncmp(lp, "fake-inquiry", 12) ==
					   0) {
					DBG(1,
					    "sane_init: configuration option "
					    "'fake-inquiry' set\n");
		...
		sanei_config_attach_matching_devices("/dev/scanner",
						     attach_one);
	}
		    ...
		    void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		default:
			DBG(1,
			    "sane_control_option:invalid option number %d\n",
			    option);
			return SANE_STATUS_INVAL;
		}
		...
		case SANE_TYPE_INT:
			DBG(16,
			    "sane_control_option: set_value %s [#%d] to %d\n",
			    name, option, *(int *) val);
			break;

		case SANE_TYPE_FIXED:
			DBG(16,
			    "sane_control_option: set_value %s [#%d] to %f\n",
			    name, option, SANE_UNFIX(*(int *) val));
			break;

		case SANE_TYPE_STRING:
			DBG(16,
			    "sane_control_option: set_value %s [#%d] to %s\n",
		...
		case OPT_BR_Y:
			...
			if (s->val[option].w != *(int *) val) {
				if (info)
					*info |= SANE_INFO_RELOAD_PARAMS;

				if (get_paper_id
				    (_OPT_VAL_STRING(s, OPT_PAPER_SIZE)) !=
				    0) {
					if (info)
					...
					s->val[OPT_PAPER_SIZE].s =
						strdup(paper_list[0]);
				}
			}
			/* fall through */
		case OPT_RESOLUTION:
			if (info && s->val[option].w != *(int *) val)
				*info |= SANE_INFO_RELOAD_PARAMS;
			/* fall through */
		case OPT_TIMEOUT_ADF:
		case OPT_TIMEOUT_MANUAL:
		...
		case OPT_CONTROL_PANEL:
			...
			if (s->val[option].w != *(int *) val) {
				...
				if (*(int *) val == TRUE) {
					if (s->hw->info.canACE == TRUE) {
						s->opt[OPT_ACE_FUNCTION].
							cap |=
							SANE_CAP_INACTIVE;
						s->opt[OPT_ACE_SENSITIVITY].
							cap |=
							SANE_CAP_INACTIVE;
					} else {
						s->opt[OPT_BRIGHTNESS].cap |=
							SANE_CAP_INACTIVE;
						s->opt[OPT_THRESHOLD].cap |=
							SANE_CAP_INACTIVE;
					}
				} else {
					if (s->hw->info.canACE == TRUE) {
			...
			if (strcmp(s->val[option].s, (SANE_String) val)) {
				...
				if (get_scan_mode_id((SANE_String) val) != 0) {
					...
					if (s->val[OPT_COMPRESSION].s
					    && get_compression_id(s->
								  val
								  [OPT_COMPRESSION].
								  s) != 0) {
						free(s->val[OPT_COMPRESSION].
						     s);
		...
		case OPT_PAPER_SIZE:
			...
			if (strcmp(s->val[option].s, (SANE_String) val)) {
				...
				if (paper_id != 0) {
					...
					x_max = SANE_UNFIX(s->hw->info.
							   x_range.max);
					y_max = SANE_UNFIX(s->hw->info.
							   y_range.max);
					/* a dimension of 0.0 (or less) is replaced with the max value */
					x = (paper_sizes[paper_id].width <=
					     0.0) ? x_max :
		 paper_sizes[paper_id].width;
					y = (paper_sizes[paper_id].length <=
					     0.0) ? y_max :
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (params) {
		...
		*params = s->params;
		...
		DBG(1,
		    "get_parameters: format=%d, pixels/line=%d, bytes/line=%d, "
		    "lines=%d, dpi=%d\n", (int) s->params.format,
...
sane_start(SANE_Handle handle)
{
	...
	if (s->scanning == FALSE) {
		 ...
		 * acquired.  That is when scanning is set back to FALSE and
	  ...
	  int * len)
{
	...
	*len = nread;
	...
	return (maxlen != 0
		&& nread == 0) ? SANE_STATUS_EOF : SANE_STATUS_GOOD;
}
...
sane_set_io_mode(SANE_Handle handle, SANE_Bool non_blocking)
{
	...
	return (non_blocking ==
		1) ? SANE_STATUS_UNSUPPORTED : SANE_STATUS_GOOD;
#endif
}
...
sane_cancel(SANE_Handle handle)
{
	...
	if (s->scanning) {
		...
		if (_OPT_VAL_WORD(s, OPT_BATCH) == TRUE) {
			DBG(5,
			    "sane_cancel: calling set_window to abort batch\n");
			set_window(s, BH_BATCH_ABORT);
		}
	}
	s->scanning = FALSE;
