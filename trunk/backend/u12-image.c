 ...
 */
static SANE_Bool
fnReadOutScanner(U12_Device * dev)
{
...
static SANE_Bool
fnSamplePreview(U12_Device * dev)
{
 ...
 */
static void
fnDataDirect(U12_Device * dev, void *src, void *dest, u_long len)
{
	_VAR_NOT_USED(dev);
	memcpy(dest, src, len);
}
 ...
 */
static void
fnColorDirect(U12_Device * dev, void *pb, void *img, u_long len)
{
	SANE_Byte *src;
	RGBByteDef *dest;
	...
	for (len = dev->DataInf.dwAsicPixelsPerPlane; len;
	     len--, src++, dest++) {
 ...
 */
static void
fnColor42(U12_Device * dev, void *pb, void *img, u_long len)
{
	u_short *src;
	RGBUShortDef *dest;
	...
	for (i = dev->DataInf.dwAsicPixelsPerPlane; i; i--, src++, dest++) {
		...
		dest->Blue =
			(src[dev->DataInf.dwAsicPixelsPerPlane * 2]) << 4;
	}
}
 ...
 */
static void
u12image_SetupScanStateVariables(U12_Device * dev, u_long index)
{
	DataType var;
	...
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_TPA)) {
		...
		if (dev->DataInf.dwScanFlag & _SCANDEF_Transparency) {
			...
			dev->shade.wExposure =
				dev->scan.negScan[index].exposureTime;
			dev->shade.wXStep =
				dev->scan.negScan[index].xStepTime;
		}
	}
	...
	if (dev->DataInf.wPhyDataType >= COLOR_TRUE24) {
		...
		if (dev->DataInf.xyPhyDpi.y > 75U) {
			if (dev->f0_8_16) {
				dev->scan.gd_gk.wGreenDiscard =
					dev->DataInf.xyPhyDpi.y / 75U;
			} else {
				dev->scan.gd_gk.wGreenDiscard =
					dev->DataInf.xyPhyDpi.y / 150U;
			}
		} else {
		...
		dev->scan.bd_rk.wBlueDiscard = dev->scan.gd_gk.wGreenDiscard =
			0;
	}
}
 ...
 */
static void
u12image_GetImageInfo(U12_Device * dev, ImgDef * image)
{
	DBG(_DBG_INFO, "u12image_GetImageInfo()\n");

	dev->DataInf.xyPhyDpi.x = u12image_GetPhysDPI(dev, image, TRUE);
	dev->DataInf.xyPhyDpi.y = u12image_GetPhysDPI(dev, image, FALSE);
	...
	if (image->wDataType <= COLOR_BW) {
		...
		dev->DataInf.dwAppPhyBytesPerLine =
			...
			dev->DataInf.dwAsicBytesPerPlane =
			dev->DataInf.dwAsicPixelsPerPlane >> 3;
	} else {
		dev->DataInf.dwAsicBytesPerPlane =
			dev->DataInf.dwAsicPixelsPerPlane =
			dev->DataInf.dwAppPixelsPerLine;
	...
	case COLOR_TRUE24:
		...
		dev->DataInf.dwAsicBytesPerLine =
			dev->DataInf.dwAppPhyBytesPerLine =
			dev->DataInf.dwAppPixelsPerLine * 3;
	...
	case COLOR_TRUE42:
		...
		dev->DataInf.dwAsicBytesPerLine =
			dev->DataInf.dwAppPhyBytesPerLine =
			dev->DataInf.dwAppPixelsPerLine * 6;
 ...
 */
static int
imageSetupScanSettings(U12_Device * dev, ImgDef * img)
{
	u_short brightness;
 ...
 */
static SANE_Status
u12image_SetupScanSettings(U12_Device * dev, ImgDef * img)
{
	...
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_TPA)) {
		...
		dev->scan.dwScanOrigin =
			dev->adj.upNormal * 4 + _RFT_SCANNING_ORG;

	} else if (dev->DataInf.dwScanFlag & _SCANDEF_Transparency) {

		dev->scan.dwScanOrigin =
			dev->adj.upPositive * 4 + _POS_SCANNING_ORG;
	} else {
		dev->scan.dwScanOrigin =
			dev->adj.upNegative * 4 + _NEG_SCANNING_ORG;
	}
	...
	if (dev->DataInf.xyAppDpi.y <= 75) {
		...
		if (dev->DataInf.dwScanFlag & _SCANDEF_PREVIEW) {
			...
			if ((dev->DataInf.wPhyDataType < COLOR_TRUE24)
			    || dev->f0_8_16
			    || (dev->shade.
				intermediate & _ScanMode_AverageOut)) {
	...
	if (dev->DataInf.xyAppDpi.y == dev->DataInf.xyPhyDpi.y) {
		...
		if (dev->DataInf.dwScanFlag & _SCANDEF_PREVIEW) {
			...
			dev->DataInf.wYSum =
				dev->DataInf.xyPhyDpi.y -
				dev->DataInf.xyAppDpi.y;
		}
	}
	...
	if (dev->DataInf.wPhyDataType >= COLOR_TRUE24) {
		...
		if ((int) r < 16 || (int) g < 16) {
			...
			if (b > _SIZE_TOTAL_BUF_TPA) {
				DBG(_DBG_ERROR,
				    "Not that much FIFO memory available!\n");
 ...
 */
static SANE_Bool
u12image_DataIsReady(U12_Device * dev, void *buf)
{
	...
	if (dev->scan.DoSample(dev)) {
		...
		if (fnDataDirect != dev->scan.DataProcess) {
			(*dev->scan.DataProcess) (dev, buf,
						  (void *) (dev->scan.BufPut.
							    red.bp),
						  dev->DataInf.
 ...
 */
static SANE_Status
u12image_ReadOneImageLine(U12_Device * dev, void *buf)
{
	SANE_Byte b, state;
	TimerDef timer, t2;
	...
	do {
		...
		if (state & _SCANSTATE_STOP) {
			...
			if (u12io_GetFifoLength(dev) >=
			    dev->scan.dwMinReadFifo)
			...
			if (b >= dev->scan.bRefresh) {
				...
				u12io_RegisterToScanner(dev,
							REG_REFRESHSCANSTATE);
				dev->scan.oldScanState =
					u12io_GetScanState(dev);
	...
	u12motor_ToHomePosition(dev, TRUE);
 ...
 */
static void
u12image_PrepareScaling(U12_Device * dev)
{
	int step;
	double ratio;
	...
	if (dev->DataInf.xyAppDpi.x > dev->dpi_max_x) {
		...
		ratio = (double) dev->DataInf.xyAppDpi.x /
 ...
 */
static void
u12image_ScaleX(U12_Device * dev, SANE_Byte * ib, SANE_Byte * ob)
{
	SANE_Byte tmp;
	int ddax;
	u_long i, j, x;
	...
	if (0 == dev->scaleStep) {
		...
		x = 0;
		for (i = 0;
		     i < dev->DataInf.dwPhysBytesPerLine * dev->scaleStep;
		     i += dev->scaleStep) {
