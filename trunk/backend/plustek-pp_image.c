 ...
 */
static UShort
imageGetPhysDPI(pScanData ps, pImgDef pImgInf, Bool fDpiX)
{
	if (_IS_ASIC98(ps->sCaps.AsicID)) {
		...
		if (fDpiX) {
			...
			if (pImgInf->wDataType >= COLOR_TRUE24) {
				if (pImgInf->xyDpi.x >
				    ps->LensInf.rDpiX.wPhyMax)
					return ps->LensInf.rDpiX.wPhyMax;
				else
					return pImgInf->xyDpi.x;
			} else {
				if (pImgInf->xyDpi.x >
				    (ps->LensInf.rDpiX.wPhyMax * 2))
					return (ps->LensInf.rDpiX.wPhyMax *
						2);
				else
					return pImgInf->xyDpi.x;
 ...
 */
static void
fnP96GrayDirect(pScanData ps, pVoid pBuf, pVoid pImg, ULong bl)
{
	pUChar src, dest;
 ...
 */
static void
fnDataDirect(pScanData ps, pVoid pBuf, pVoid pImg, ULong bl)
{
	_VAR_NOT_USED(ps);
	memcpy(pBuf, pImg, bl);
}
 ...
 */
static void
fnHalftoneDirect0(pScanData ps, pVoid pb, pVoid pImg, ULong bL)
{
 ...
 */
static void
fnHalftoneDirect1(pScanData ps, pVoid pb, pVoid pImg, ULong bL)
{
	pUChar src, dest;
	UChar threshold;
	ULong dw;
 ...
 */
static void
fnP98ColorDirect(pScanData ps, pVoid pb, pVoid pImg, ULong bL)
{
	pUChar src;
	pRGBByteDef dest;
...
static void
fnP96ColorDirect(pScanData ps, pVoid pb, pVoid pImg, ULong bL)
{
	pUChar src;
	pRGBByteDef dest;
	...
	for (bL = ps->DataInf.dwAsicPixelsPerPlane; bL; bL--, dest++, src++) {
		...
		dest->Green =
			ps->pbMapGreen[src[ps->DataInf.dwAsicPixelsPerPlane]];
		dest->Blue =
			ps->
			pbMapBlue[src[ps->DataInf.dwAsicPixelsPerPlane * 2]];
	}
}
 ...
 */
static void
fnP98Color48(pScanData ps, pVoid pb, pVoid pImg, ULong bL)
{
	pUShort src;
	pRGBUShortDef dest;
 ...
 */
static int
imageP98SetupScanSettings(pScanData ps, pScanInfo pInf)
{
	UShort brightness;
	ULong xAdjust;
	...
	if (ps->DataInf.dwScanFlag & SCANDEF_BmpStyle) {
		ps->Scan.lBufferAdjust =
			-(Long) ps->DataInf.dwAppBytesPerLine;
	} else {
...
*/
	...
	if (ps->DataInf.siBrightness >= 0) {
		brightness = (short) ((long) (-(255 - _DEF_BW_THRESHOLD) *
					      ps->DataInf.siBrightness) /
				      127 + _DEF_BW_THRESHOLD);
	} else {
 ...
 */
static void
imageP98DoCopyBuffer(pScanData ps, pUChar pImage)
{
 ...
 */
static Bool
imageP98CopyToFilterBuffer(pScanData ps, pUChar pImage)
{
	if (ps->fDoFilter) {
 ...
 */
static void
imageP98UnSharpCompare(pScanData ps, Byte Center,
 ...
 */
static void
imageP98DoFilter(pScanData ps, pUChar pPut)
{
	ULong dw;
	Long dwNewValue;
	...
	if (ps->fDoFilter && (ps->DataInf.xyAppDpi.x) >= 600UL) {
		...
		/* DoUnsharpMask(); */
		for (dw = 0; dw < ps->DataInf.dwAsicBytesPerPlane - 2;
		     dw++, pPut++) {

			ps->dwDivFilter = ps->dwMul;

			dwNewValue = ((ULong) ps->pGet2[dw + 1]) * ps->dwMul;
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet1[dw], &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet1[dw + 1],
					       &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ...
					       &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet2[dw], &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet2[dw + 2],
					       &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet3[dw], &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
					       ps->pGet3[dw + 1],
					       &dwNewValue);
			imageP98UnSharpCompare(ps, ps->pGet2[dw + 1],
 ...
 */
static Bool
imageP98DataIsReady(pScanData ps)
{
	Byte b;
 ...
 */
static Bool
imageP98001ReadOneImageLine(pScanData ps)
{
	ULong dwFifoCounter;
	TimerDef timer;
 ...
 */
static void
imageP98GetInfo(pScanData ps, pImgDef pImgInf)
{
	...
	ps->DataInf.XYRatio =
		1000 * ps->DataInf.xyPhyDpi.y / ps->DataInf.xyPhyDpi.x;
	...
	if (pImgInf->wDataType <= COLOR_HALFTONE) {
		ps->DataInf.dwAsicPixelsPerPlane =
			(ps->DataInf.dwAppPixelsPerLine + 7UL) & 0xfffffff8UL;
		ps->DataInf.dwAppPhyBytesPerLine =
			ps->DataInf.dwAppBytesPerLine =
			ps->DataInf.dwAsicBytesPerLine =
			ps->DataInf.dwAsicBytesPerPlane =
			ps->DataInf.dwAsicPixelsPerPlane >> 3;
	} else {
		ps->DataInf.dwAsicBytesPerPlane =
			ps->DataInf.dwAsicPixelsPerPlane =
 ...
 */
	...
	case COLOR_TRUE24:
		...
		ps->DataInf.dwAsicBytesPerLine =
			ps->DataInf.dwAppPhyBytesPerLine =
			ps->DataInf.dwAppPixelsPerLine * 3;
	...
	case COLOR_TRUE48:
		...
		ps->DataInf.dwAsicBytesPerLine =
			ps->DataInf.dwAppPhyBytesPerLine =
			ps->DataInf.dwAppPixelsPerLine * 6;
 ...
 */
static void
imageP96GetInfo(pScanData ps, pImgDef pImgInf)
{
	...
	ps->DataInf.dwAsicBytesPerPlane =
		ps->DataInf.dwAsicPixelsPerPlane =
		(ULong) ps->DataInf.xyPhyDpi.x * pImgInf->crArea.cx /
		_MEASURE_BASE;
	...
	if (ps->DataInf.wPhyDataType == COLOR_TRUE24)
		ps->DataInf.dwAsicBytesPerLine =
			ps->DataInf.dwAsicBytesPerPlane * 3;
	else
		ps->DataInf.dwAsicBytesPerLine =
			ps->DataInf.dwAsicBytesPerPlane;
 ...
 */
static Bool
imageP96ReadOneImageLine(pScanData ps)
{
	Bool result = _FALSE;
...
*/
		if (bFifoCount >= ps->bMinReadFifo) {
			...
			for (; !(*ps->pCurrentColorRunTable &
				 (ps->RedDataReady | ps->
				  GreenDataReady | _BLUE_DATA_READY));
			...
			if (ps->pCurrentColorRunTable >
				...
				DBG(DBG_LOW,
				    "WARNING: pCurrentColorRunTab>pColorRunTable\n");
#endif
			...
			if (ps->DataInf.wPhyDataType == COLOR_TRUE24) {
				 ...
				 */
				if (*ps->pCurrentColorRunTable & ps->
				    b1stColor) {
					*ps->pCurrentColorRunTable &=
						ps->b1stMask;
					IOReadScannerImageData(ps,
							       ...
							       dwAsicBytesPerPlane);
					ps->pPutBufR +=
						ps->BufferSizePerModel;
					if (ps->pPutBufR == ps->pEndBufR)
						ps->pPutBufR = ps->pPrescan16;
				} else if (*ps->pCurrentColorRunTable & ps->
					   b2ndColor) {
					*ps->pCurrentColorRunTable &=
						ps->b2ndMask;
					IOReadScannerImageData(ps,
 ...
 */
static int
imageP96SetupScanSettings(pScanData ps, pScanInfo pInf)
{
 ...
 */
static Bool
imageP98003DataIsReady(pScanData ps)
{
	pUChar pb;
	...
	if (ps->Scan.DoSample(ps)) {
		...
		if (fnDataDirect != ps->Scan.DataProcess) {
			(*ps->Scan.DataProcess) (ps,
						 (pVoid) (ps->Scan.bp.
							  pMonoBuf),
						 (pVoid) (ps->Scan.BufPut.red.
							  bp),
						 ps->DataInf.
 ...
 */
static Bool
imageP98003ReadOneImageLine(pScanData ps)
{
	Byte b, state;
	TimerDef timer, t2;
	...
	do {
		...
		if (state & _SCANSTATE_STOP) {
			...
			if (IOReadFifoLength(ps) >= ps->Scan.dwMaxReadFifo) {
				...
				if (IOReadFifoLength(ps) >=
				    ps->Scan.dwMinReadFifo) {
					if (imageP98003DataIsReady(ps))
						return _TRUE;
				}
			}
 ...
 */
static void
imageP98003SetupScanStateVariables(pScanData ps, ULong index)
{
	DataType var;
	...
	if (!(ps->DataInf.dwScanFlag & SCANDEF_TPA)) {
		...
		if (((ps->IO.portMode == _PORT_BIDI) ||
			...
			ps->Shade.wExposure =
				nmlScan[ps->IO.portMode][index].exposureTime;
			ps->Shade.wXStep =
				nmlScan[ps->IO.portMode][index].xStepTime;
		} else {
			ps->Shade.wExposure =
				nmlScan[_PORT_EPP][index].exposureTime;
			ps->Shade.wXStep =
				nmlScan[_PORT_EPP][index].xStepTime;
		}
		...
		if (ps->DataInf.dwScanFlag & SCANDEF_Transparency) {
			...
			ps->Shade.wExposure =
				ps->Scan.negScan[index].exposureTime;
	...
	if (ps->DataInf.wPhyDataType == COLOR_BW)
		...
		if (ps->DataInf.wPhyDataType == COLOR_256GRAY)
			var.dwValue =
				xferSpeed[ps->IO.portMode].thresholdGray;
		else
			var.dwValue =
				xferSpeed[ps->IO.portMode].thresholdColor;
	}
	...
	if (var.dwValue && ps->DataInf.dwAsicBytesPerPlane > var.dwValue) {
		if ((var.dwValue << 1) > ps->DataInf.dwAsicBytesPerPlane)
			ps->Scan.dwInterval <<= 1;
		else if ((var.dwValue << 2) > ps->DataInf.dwAsicBytesPerPlane)
	...
	if (ps->DataInf.wPhyDataType >= COLOR_TRUE24) {
		...
		if (ps->DataInf.xyPhyDpi.y > 75U) {
			if (ps->Device.f0_8_16) {
				ps->Scan.gd_gk.wGreenDiscard =
					ps->DataInf.xyPhyDpi.y / 75U;
			} else {
				ps->Scan.gd_gk.wGreenDiscard =
					ps->DataInf.xyPhyDpi.y / 150U;
			}
		} else {
		...
		ps->Scan.bd_rk.wBlueDiscard = ps->Scan.gd_gk.wGreenDiscard =
			0;
	}
}
 ...
 */
static int
imageP98003SetupScanSettings(pScanData ps, pScanInfo pInf)
{
	...
	if (!(ps->DataInf.dwScanFlag & SCANDEF_TPA)) {
		...
		ps->Scan.dwScanOrigin =
			ps->Device.lUpNormal * 4 + _RFT_SCANNING_ORG;
	...
	if (ps->DataInf.xyAppDpi.y <= 75) {
		...
		if (ps->DataInf.dwVxdFlag & _VF_PREVIEW) {
			...
			if (!(ps->DataInf.dwScanFlag & SCANDEF_TPA) &&
			    ...
			    (ps->DataInf.wPhyDataType >= COLOR_TRUE24)) {
				ps->Shade.bIntermediate |=
					_ScanMode_AverageOut;
			}

			if ((ps->DataInf.wPhyDataType < COLOR_TRUE24)
			    || ps->Device.f0_8_16
			    || (ps->Shade.
				bIntermediate & _ScanMode_AverageOut)) {
	...
	if (ps->DataInf.xyAppDpi.y == ps->DataInf.xyPhyDpi.y) {
		...
		if (ps->DataInf.dwVxdFlag & _VF_PREVIEW) {
			...
			ps->DataInf.wYSum =
				ps->DataInf.xyPhyDpi.y -
				ps->DataInf.xyAppDpi.y;
		}
	}
	...
	if (ps->DataInf.wPhyDataType >= COLOR_TRUE24) {
		...
		if ((int) r < 16 || (int) g < 16) {
			...
			DBG(DBG_LOW,
			    "48Bit buffer request: len=%lu bytes, available=%lu\n",
...
/* CHECK: maybe remove this stuff */
	if (ps->DataInf.dwScanFlag & SCANDEF_Transparency) {
		...
		/* Reset shading Exposure Time & xStep Time */
		ps->Shade.wExposure =
			ps->Scan.negScan[ps->Scan.dpiIdx].exposureTime;
		ps->Shade.wXStep =
			ps->Scan.negScan[ps->Scan.dpiIdx].xStepTime;
	}
