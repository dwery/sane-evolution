		    ...
		    unsigned *size)
{
	...
	mf->cb.buf[10] = 0x06;
	mf->cb.reslen =
		pixma_cmd_transaction(s, mf->cb.buf, 11, mf->cb.buf, 8);
 ...
 */
	do {
		...
		error = read_image_block(s, mf->blkptr + mf->blk_len,
					 block_size);
		block_size = error;
		if (error < 0)
			return error;
		...
		if (n != 0) {
			if (s->param->channels != 1) {
				/* color */
				pack_rgb(mf->blkptr, n, mf->raw_width,
					 mf->lineptr);
			} else {
				/* grayscale */
				memcpy(mf->lineptr, mf->blkptr,
				       n * s->param->line_size);
			}
			lines_size = n * s->param->line_size;
			/* cull remainder and shift left */
			mf->blk_len -= block_size;
			memcpy(mf->blkptr, mf->blkptr + block_size,
			       mf->blk_len);
		}
	}
	while (n == 0);
