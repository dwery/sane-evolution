...
static int wWidth = 2570;	/*Scan area width */
static int wResolution = 300;	/*Resolution in DPI */
...
static int wVerticalResolution;
...
static const int ImageResolutionsList[] = {
...
static int
LengthForRes(int Resolution, int Length)
{
	switch (Resolution) {
	case 75:
		return Length / 4;
...
sane_get_devices(const SANE_Device *** device_list, SANE_Bool local_only)
{
	...
	if (DetectScanner() == 0) {	/*Device malfunction! */
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case 2:		/*Resolution */
		switch (action) {
		case SANE_ACTION_GET_VALUE:
			*((int *) value) = wResolution;
			return SANE_STATUS_GOOD;
		case SANE_ACTION_SET_VALUE:	/*info should be setted */
			wResolution = *((int *) value);
			if (info != NULL)
				*info = 0;
			return SANE_STATUS_GOOD;
		default:
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	parms.last_frame = TRUE;	/*For grayscale... */
...
sane_start(SANE_Handle handle)
{
	...
	if (bLastCalibration - bCalibration > 16) {	/*Lamp is not warm enouth */
	...
	if (PaperFeed(LINES_TO_FEED) == 0) {	/*Feed only for fixel lenght. Change it */
		...
		SwitchHardwareState(FLAGS_HW_INDICATOR_OFF |
				    FLAGS_HW_MOTOR_READY, 0);
	...
	*length = 0;
	...
	while (((bFuncResult = CallFunctionWithRetVal(0xB2)) & 0x20) == 0) {
		...
		*length = LengthForRes(wResolution, wWidth);
		...
		CallFunctionWithParameter(FUNCTION_SETUP_HARDWARE,
					  bHardwareState);
		return SANE_STATUS_GOOD;
	}
	return SANE_STATUS_EOF;
...
ReleasePaper()
{
	...
	if ((CallFunctionWithRetVal(0xB2) & 0x20) == 0) {	/*End of paper was not reached */
...
static void
TransferScanParameters(enumColorDepth enColor, int wResolution,
		       int wPixelsLength)
{
	int wRightBourder = (2570 + wPixelsLength) / 2 + 65;
	int wLeftBourder = (2570 - wPixelsLength) / 2 + 65;
...
static void
TurnOnPaperPulling(enumColorDepth enColor, int wResolution)
{
	switch (enColor) {
	case Drawing:
	case Halftone:
...
CalibrateScanElements()
{
	...
	for (Index = 0; Index < 3; Index++) {	/*For theree color components */
		...
		CallFunctionWithParameter(0xC6, 0xFF);
		CallFunctionWithParameter(0x92,
					  arUpTransferBorders[Index] | 0x80);
		...
		CallFunctionWithParameter(0x92,
					  arLowTransferBorders[Index] | 0x80);
	...
	for (Index = 0; Index < 3; Index++) {	/*Three color components */
		...
		for (CorrectionValue = 0x40; CorrectionValue != 0;
		     CorrectionValue >>= 2) {
			...
			WriteScannerRegister(CurrentUpSaveBorder,
					     Calibration);
...
ReadDataBlock(SANE_Byte * Buffer, int length)
{
	...
	ieee1284_data_dir(pl.portv[scanner_d], 1);	/*Reverse mode */
	ieee1284_frob_control(pl.portv[scanner_d], C1284_NINIT, C1284_NINIT);
	ieee1284_epp_read_data(pl.portv[scanner_d], 0, (char *) Buffer,
			       length);
}
