...
static const struct hp5590_model hp5590_models[] = {
	{
	 SCANNER_HP5550,
		    ...
		    enum hp_scanner_types scanner_type)
{
	...
	if (scanner_type != SCANNER_NONE) {
		unsigned int i;
		for (i = 0;
		     i < sizeof(hp5590_models) / sizeof(struct hp5590_model);
		     i++) {
			if (hp5590_models[i].scanner_type == scanner_type) {
				if (strcmp(id_buf, hp5590_models[i].vendor_id)
				    != 0) {
					DBG(DBG_err,
					    "%s: Vendor id mismatch for scanner HP%s - "
					    "required  '%s', got '%s'\n",
					    __FUNCTION__,
					    hp5590_models[i].model,
					    hp5590_models[i].vendor_id,
					    id_buf);
					return SANE_STATUS_INVAL;
				}
	...
	if (scanner_model) {
		...
		DBG(DBG_cmds,
		    "HP%s normal motor param: %u, max motor param: %u\n",
	...
	if (info) {
		*info = malloc(sizeof(struct scanner_info));
		...
		(*info)->max_size_x =
			(*info)->max_pixels_x * 1.0 / (*info)->max_dpi_x;
		(*info)->max_size_y =
			(*info)->max_pixels_y * 1.0 / (*info)->max_dpi_y;
		    ...
		    unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	ret = hp5590_cmd(dn,
			 CMD_VERIFY, CMD_EEPROM_WRITE, data, size, CORE_DATA);
	if (ret != SANE_STATUS_GOOD)
		return ret;
...
/******************************************************************************/
static SANE_Status
hp5590_read_scan_count(int dn, unsigned int *count)
{
...
/******************************************************************************/
static SANE_Status
hp5590_read_max_scan_count(int dn, unsigned int *max_count)
{
...
hp5590_read_eeprom_all_cmd(int dn)
{
	...
	ret = hp5590_read_eeprom(dn, 0x00,
				 (unsigned char *) eeprom, sizeof(eeprom));
	if (ret != SANE_STATUS_GOOD)
		return ret;
...
hp5590_read_part_number(int dn)
{
	...
	ret = hp5590_read_eeprom(dn, 0x1a, part_number, part_number_len);
	if (ret != SANE_STATUS_GOOD)
		return ret;
...
hp5590_is_data_available(int dn)
{
	...
	DBG(DBG_cmds, "%s: Data is %s\n",
	    __FUNCTION__,
	    data_available == TRUE ? "available" : "not available");

	return data_available ==
		TRUE ? SANE_STATUS_GOOD : SANE_STATUS_NO_DOCS;
}
...
/******************************************************************************/
static SANE_Status
hp5590_turnon_lamp(int dn, enum hp5590_lamp_state state)
{
...
hp5590_power_status(int dn)
{
	...
	DBG(DBG_cmds, "Power status: %s (%02x)\n",
	    power_resp.flags & POWER_FLAG_ON ? "on" : "off",
	    power_resp.flags);
...
/******************************************************************************/
static SANE_Status
hp5590_read_error_code(int dn, unsigned int *adf_flags)
{
...
hp5590_set_base_dpi(int dn,
		    struct scanner_info *scanner_info, unsigned int base_dpi)
{
	u_int16_t _base_dpi;
	SANE_Status ret;
	...
	if (base_dpi > scanner_info->max_dpi_x
		...
		DBG(DBG_err, "Base DPI too large "
		    ...
		    base_dpi,
		    scanner_info->max_dpi_x, scanner_info->max_dpi_y);
		return SANE_STATUS_INVAL;
	}
...
/******************************************************************************/
static SANE_Status
hp5590_set_color_map(int dn, unsigned int base_dpi)
{
...
/******************************************************************************/
static SANE_Status
hp5590_set_scan_area(int dn, struct scanner_info *scanner_info,
		     unsigned int top_x, unsigned int top_y,
		     unsigned int width, unsigned int height,
		     unsigned int dpi, enum color_depths color_depth,
		     enum scan_modes scan_mode, enum scan_sources scan_source)
{
	...
	if (scan_source == SOURCE_ADF_DUPLEX)
		scan_params.source |=
			SCAN_PARAMS_SOURCE_ADF | SCAN_PARAMS_SOURCE_DUPLEX;
	...
	DBG(DBG_cmds,
	    "Scanner top X: %u, top Y: %u, size X: %u, size Y: %u\n",
	    scanner_top_x, scanner_top_y, scanner_pixels_x, scanner_pixels_y);
...
hp5590_read_image_params(int dn)
{
	...
	DBG(DBG_cmds, "Line width: %u (%02x)\n",
	    ntohs(image_params.line_width), ntohs(image_params.line_width));
...
/******************************************************************************/
static SANE_Status
hp5590_set_scan_params(int dn, struct scanner_info *scanner_info,
		       unsigned int top_x, unsigned int top_y,
		       unsigned int width, unsigned int height,
		       unsigned int dpi, enum color_depths color_depth,
...
/******************************************************************************/
static SANE_Status
hp5590_read(int dn, unsigned char *bytes, unsigned int size, void *state)
{
	SANE_Status ret;
...
/******************************************************************************/
static SANE_Status
hp5590_read_buttons(int dn, enum button_status *status)
{
	...
	*status = BUTTON_NONE;
	...
	DBG(DBG_cmds,
	    "Power: %s, Scan: %s, Collect: %s, File: %s, Email: %s, Copy: %s, "
