...
attach(const char *devname, V4L_Device ** devp)
{
	...
	if (v4lfd != -1) {
		...
		if (!(VID_TYPE_CAPTURE & capability.type)) {
			DBG(1,
			    "attach: device %s can't capture to memory -- exiting\n",
			    devname);
			close(v4lfd);
			return SANE_STATUS_UNSUPPORTED;
		}
		DBG(2, "attach: found videodev `%s' on `%s'\n",
		    capability.name, devname);
		close(v4lfd);
	} else {
...
init_options(V4L_Scanner * s)
{
	...
	s->opt[OPT_CHANNEL].desc =
		"Selects the channel of the v4l device (e.g. television "
		"or video-in.";
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	while (sanei_config_read(dev_name, sizeof(dev_name), fp)) {
		...
		for (str = dev_name; *str && !isspace(*str) && *str != '#';
		     ++str);
		attach(dev_name, 0);
	}
	fclose(fp);
...
sane_open(SANE_String_Const devname, SANE_Handle * handle)
{
	...
	if (ioctl(s->fd, VIDIOCGCAP, &s->capability) == -1) {
		DBG(1,
		    "sane_open: ioctl (%d, VIDIOCGCAP,..) failed on `%s': %s\n",
	...
	if (VID_TYPE_OVERLAY & s->capability.type)
		DBG(5,
		    "sane_open: V4L device can overlay its image onto the frame "
		    "buffer\n");
	...
	if (VID_TYPE_FRAMERAM & s->capability.type)
		DBG(5,
		    "sane_open: V4L device overwrites frame buffer memory\n");
	...
	if (VID_TYPE_SUBCAPTURE & s->capability.type)
		DBG(5,
		    "sane_open: V4L device can capture parts of the image\n");
	...
	for (i = 0; i < max_channels; i++) {
		...
		DBG(5, "sane_open: channel %d (%s), tuners=%d, flags=0x%x, "
		    ...
		    channel.tuners, channel.flags, channel.type,
		    channel.norm);
	...
	DBG(5, "sane_open: brightness=%d, hue=%d, colour=%d, contrast=%d\n",
	    s->pict.brightness, s->pict.hue, s->pict.colour,
	    s->pict.contrast);
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		if (!SANE_OPTION_IS_SETTABLE(cap)) {
			DBG(1,
			    "sane_control_option: option is not settable\n");
			return SANE_STATUS_INVAL;
		}
		status = sanei_constrain_value(s->opt + option, val, info);
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_control_option: sanei_constarin_value failed: %s\n",
		...
		if (option >= OPT_TL_X && option <= OPT_BR_Y) {
			...
			if (-1 == ioctl(s->fd, VIDIOCGWIN, &s->window)) {
				DBG(1,
				    "sane_control_option: ioctl VIDIOCGWIN failed "
		...
		case OPT_TL_Y:
			break;
		case OPT_BR_X:
			s->window.width = *(int *) val;
			parms.pixels_per_line = *(int *) val;
			if (info)
				*info |= SANE_INFO_RELOAD_PARAMS;
			break;
		case OPT_BR_Y:
			s->window.height = *(int *) val;
			parms.lines = *(int *) val;
			if (info)
				*info |= SANE_INFO_RELOAD_PARAMS;
			break;
		case OPT_MODE:
			...
			break;
		case OPT_BRIGHTNESS:
			s->pict.brightness = *(int *) val *256;
			s->val[option].w = *(int *) val;
			break;
		case OPT_HUE:
			s->pict.hue = *(int *) val *256;
			s->val[option].w = *(int *) val;
			break;
		case OPT_COLOR:
			s->pict.colour = *(int *) val *256;
			s->val[option].w = *(int *) val;
			break;
		case OPT_CONTRAST:
			s->pict.contrast = *(int *) val *256;
			s->val[option].w = *(int *) val;
			break;
		case OPT_WHITE_LEVEL:
			s->pict.whiteness = *(int *) val *256;
			s->val[option].w = *(int *) val;
			break;
		case OPT_CHANNEL:
		{
			...
			for (i = 0; i < MAX_CHANNELS; i++) {
				if (strcmp(s->channel[i], val) == 0) {
					channel.channel = i;
					if (-1 ==
					    ioctl(s->fd, VIDIOCGCHAN,
						  &channel)) {
						DBG(1,
						    "sane_open: can't ioctl VIDIOCGCHAN %s: %s\n",
						    s->devicename,
						    strerror(errno));
						return SANE_STATUS_INVAL;
					}
					if (-1 ==
					    ioctl(s->fd, VIDIOCSCHAN,
						  &channel)) {
						DBG(1,
						    "sane_open: can't ioctl VIDIOCSCHAN %s: %s\n",
						    s->devicename,
						    strerror(errno));
						return SANE_STATUS_INVAL;
					}
		...
		if (option >= OPT_TL_X && option <= OPT_BR_Y) {
			if (-1 == ioctl(s->fd, VIDIOCSWIN, &s->window)) {
				DBG(1,
				    "sane_control_option: ioctl VIDIOCSWIN failed (%s)\n",
				    strerror(errno));
				/* return SANE_STATUS_INVAL; */
			}
			if (-1 == ioctl(s->fd, VIDIOCGWIN, &s->window)) {
				DBG(1,
				    "sane_control_option: ioctl VIDIOCGWIN failed (%s)\n",
				    strerror(errno));
				return SANE_STATUS_INVAL;
			}
		}
		if (option >= OPT_BRIGHTNESS && option <= OPT_WHITE_LEVEL) {
			if (-1 == ioctl(s->fd, VIDIOCSPICT, &s->pict)) {
				DBG(1,
				    "sane_control_option: ioctl VIDIOCSPICT failed (%s)\n",
				    strerror(errno));
				/* return SANE_STATUS_INVAL; */
			}
		}
		...
		if (!(cap & SANE_CAP_AUTOMATIC)) {
			DBG(1,
			    "sane_control_option: option can't be set automatically\n");
			return SANE_STATUS_INVAL;
		}
		switch (option) {
		case OPT_BRIGHTNESS:
...
sane_start(SANE_Handle handle)
{
	...
	if (-1 == ioctl(s->fd, VIDIOCGMBUF, &s->mbuf)) {
		s->is_mmap = FALSE;
		buffer = malloc(s->capability.maxwidth *
				s->capability.maxheight * s->pict.depth);
		if (0 == buffer)
			return SANE_STATUS_NO_MEM;
		...
		buffer = mmap(0, s->mbuf.size, PROT_READ | PROT_WRITE,
			      MAP_SHARED, s->fd, 0);
		if (buffer == (void *) -1) {
		...
		DBG(2,
		    "sane_start: mmappeded frame %d x %d with palette %d\n",
		    s->mmap.width, s->mmap.height, s->mmap.format);
		len = ioctl(s->fd, VIDIOCMCAPTURE, &s->mmap);
		if (len == -1) {
			DBG(1,
			    "sane_start: ioctl VIDIOCMCAPTURE failed: %s\n",
			    strerror(errno));
			return SANE_STATUS_INVAL;
		}
		...
		if (-1 == len) {
			DBG(1,
			    "sane_start: call to ioctl(%d, VIDIOCSYNC, ..) failed\n",
			    s->fd);
			return SANE_STATUS_INVAL;
		}
