...
_InitOptions(TScanner * s)
{
	...
	if (s->aGammaTableR == NULL) {	/* Not yet allocated */
		s->aGammaTableR =
			malloc(NUM_GAMMA_ENTRIES * sizeof(int));
		s->aGammaTableG =
			malloc(NUM_GAMMA_ENTRIES * sizeof(int));
		s->aGammaTableB =
			malloc(NUM_GAMMA_ENTRIES * sizeof(int));
...
sane_init(int * piVersion, SANE_Auth_Callback pfnAuth)
{
	...
	HP5400_DBG(DBG_MSG,
		   "sane_init: SANE hp5400 backend version %d.%d-%d (from %s)\n",
		    ...
		    void *pVal, int * pInfo)
{
	...
	case SANE_ACTION_GET_VALUE:
		...
		case optBRX:
		case optTLX:
			*(int *) pVal = s->aValues[n].w;	/* Not needed anymore  - s->aValues[optOffsetX].w; */
			HP5400_DBG(DBG_MSG,
		...
		case optBRY:
		case optTLY:
			*(int *) pVal = s->aValues[n].w;	/* Not needed anymore - - s->aValues[optOffsetY].w; */
			HP5400_DBG(DBG_MSG,
				   "sane_control_option: SANE_ACTION_GET_VALUE %d = %d\n",
				   n, *(int *) pVal);
			break;
	...
	case SANE_ACTION_SET_VALUE:
		...
		case optTLX:
			...
			s->aValues[n].w = *(int *) pVal;	/* Not needed anymore - + s->aValues[optOffsetX].w; */
			break;
		...
		case optTLY:
			info |= SANE_INFO_RELOAD_PARAMS;
			s->ScanParams.iLines = 0;	/* Forget actual image settings */
			s->aValues[n].w = *(int *) pVal;	/* Not needed anymore - + s->aValues[optOffsetY].w; */
			break;
		case optDPI:
			info |= SANE_INFO_RELOAD_PARAMS;
			s->ScanParams.iLines = 0;	/* Forget actual image settings */
...
#else
			(s->aValues[n].w) = min(1200, *(int *) pVal);
#endif
			break;
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * p)
{
	...
	if (s->ScanParams.iLines) {	/* Initialised by doing a scan */
		...
		p->lines =
			MM_TO_PIXEL(s->aValues[optBRY].w -
				    s->aValues[optTLY].w,
				    s->aValues[optDPI].w);
		p->pixels_per_line =
			MM_TO_PIXEL(s->aValues[optBRX].w -
				    s->aValues[optTLX].w,
				    s->aValues[optDPI].w);
...
sane_start(SANE_Handle h)
{
	...
	if (sane_get_parameters(h, &par) != SANE_STATUS_GOOD) {
		HP5400_DBG(DBG_MSG,
			   "Invalid scan parameters (sane_get_parameters)\n");
		return SANE_STATUS_INVAL;
	}
	...
	s->ScanParams.iHeight =
		MM_TO_PIXEL(s->aValues[optBRY].w - s->aValues[optTLY].w,
			    HW_LPI);
...
/*    FinishScan( &s->HWParams );        *** FinishScan called in sane_cancel */
	...
	while ((*len + s->ScanParams.iBytesPerLine <= maxlen)
		...
		CircBufferGetLine(s->HWParams.iXferHandle, &s->HWParams.pipe,
				  buffer);
