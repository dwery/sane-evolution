...
SANE_Status
sane_read(SANE_Handle handle, SANE_Byte * buf, SANE_Int maxlen,
	  SANE_Int * len)
{
	Microtek2_Scanner *ms = handle;
	SANE_Status status;
...
init_options(Microtek2_Scanner * ms, u_int8_t current_scan_source)
{
	...
	for (color = 0; color < 4; color++) {
		...
		DBG(100,
		    ...
		    (u_long) (tablesize * sizeof(SANE_Int)));
...
get_scan_parameters(Microtek2_Scanner * ms)
{
	...
	ms->y1_dots =
		(SANE_Int) (SANE_UNFIX(ms->val[OPT_TL_Y].w) * dpm + 0.5);
...
/*    ms->y1_dots -= 50;*/
	...
	if (ms->val[OPT_RESOLUTION_BIND].w == SANE_TRUE) {
		ms->x_resolution_dpi =
			(SANE_Int) (SANE_UNFIX(ms->val[OPT_RESOLUTION].w) +
				    0.5);
		ms->y_resolution_dpi =
			(SANE_Int) (SANE_UNFIX(ms->val[OPT_RESOLUTION].w) +
				    0.5);
		...
		ms->x_resolution_dpi =
			(SANE_Int) (SANE_UNFIX(ms->val[OPT_RESOLUTION].w) +
				    0.5);
		ms->y_resolution_dpi =
			(SANE_Int) (SANE_UNFIX(ms->val[OPT_Y_RESOLUTION].w) +
				    0.5);
	}
