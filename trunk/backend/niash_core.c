/*
  ...
  $Id: niash_core.c,v 1.8 2004-10-27 18:06:19 ullsig-guest Exp $
*/
...
static unsigned char abData0000[] = {
	0xfe, 0x9f, 0x58, 0x1b, 0x00, 0x03, 0xa4, 0x02, 0x63, 0x02, 0x33,
	0x02,
	0x0d, 0x02, 0xf0, 0x01,
	0xd8, 0x01, 0xc5, 0x01, 0xb5, 0x01, 0xa8, 0x01, 0x9d, 0x01, 0x93,
	0x01,
	0x8b, 0x01, 0x84, 0x01,
	0x7e, 0x01, 0x79, 0x01, 0x74, 0x01, 0x70, 0x01, 0x6d, 0x01, 0x69,
	0x01,
	0x67, 0x01, 0x64, 0x01,
	0x62, 0x01, 0x60, 0x01, 0x5f, 0x01, 0x5d, 0x01, 0x5c, 0x01, 0x5b,
	0x01,
	0x5a, 0x01, 0x59, 0x01,
	0x58, 0x01, 0x57, 0x01, 0x57, 0x01, 0x56, 0x01, 0x56, 0x01, 0x55,
	0x01,
	0x55, 0x01, 0x54, 0x01,
	0x54, 0x01, 0x54, 0x01, 0x54, 0x01, 0x53, 0x01, 0x53, 0x01, 0x53,
	0x01,
	0x53, 0x01, 0x52, 0x81
};
...
static unsigned char abData0400[] = {
	0xa4, 0x82, 0x00, 0x80, 0xa4, 0x82, 0xaa, 0x02, 0xc0, 0x02, 0xe8,
	0x02,
	0x3e, 0x03, 0xc8, 0x03,
	0x58, 0x1b, 0xfe, 0x9f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00,
	0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00
};
...
static void
WriteRegWord(int iHandle, unsigned char bReg, int wData)
{
...
InitNiashCommon(TScanParams * pParams, THWParams * pHWParams)
{
	...
	if (pHWParams->iReversedHead) {
		...
		/* left */
		WriteRegWord(iHandle, 0x10,
			     3 * (HW_PIXELS - pParams->iLeft) - 1);
	} else {
		  ...
		  SANE_Bool fReturn)
{
	...
	if (pabLine != NULL) {
		memcpy(pabLine,
		       &p->pabXferBuf[p->iCurLine * p->iBytesPerLine],
		       p->iBytesPerLine);
	}
	/* advance pointer */
...
XferBufferInit(int iHandle, TDataPipe * p)
{
	...
	for (i = 0; i < p->iSkipLines; i++) {
		XferBufferGetLine(iHandle, p, NULL, FALSE);
	}
}
...
/* gets an unscrambled line from the circular buffer. the first couple of lines contain garbage,
   if fReturn==TRUE, the head will return automatically on an end of scan */
STATIC SANE_Bool
	...
	for (iLineCount = 0; iLineCount < p->iScaleDownLpi; ++iLineCount) {
		...
		if (pabLine != NULL) {
			_UnscrambleLine(pabLine,
					&p->pabCircBuf[p->iRedLine *
						       p->iBytesPerLine],
					&p->pabCircBuf[p->iGrnLine *
						       p->iBytesPerLine],
					&p->pabCircBuf[p->iBluLine *
						       p->iBytesPerLine],
					p->iWidth * p->iScaleDownDpi,
...
{
	return CircBufferGetLineEx(iHandle, p, pabLine, iReversedHead,
				   FALSE);
}
	       ...
	       int iScaleDownDpi, int iScaleDownLpi)
{
	...
	p->pabCircBuf =
		(unsigned char *) malloc(p->iBytesPerLine *
					 p->iLinesPerCircBuf);
...
#define MAX_LINES_PER_XFERBUF 800
		...
		p->iLinesLeft =
			iHeight + p->iSkipLines + p->iLinesPerCircBuf +
			SAFETY_LINES;
		p->iLinesPerXferBuf = XFER_BUF_SIZE / p->iBytesPerLine;
		...
		DBG(DBG_MSG, "_iXFerSize = %d for %d transfer(s)\n",
		    ...
		    (p->iLinesLeft + p->iLinesPerXferBuf -
		     1) / p->iLinesPerXferBuf);
	}
...
CircBufferExit(TDataPipe * p)
{
	...
	if (p->pabCircBuf != NULL) {
		...
		DBG(DBG_ERR,
		    "CircBufferExit: Circular buffer not initialised!\n");
	}
}
	       ...
	       unsigned char *pabCalWhite)
{
	...
	for (i = 0; i < endWhiteY - startWhiteY + 1; i++) {
		CircBufferGetLine(iHandle, &DataPipe,
				  &abBuf[i * HW_PIXELS * 3], iReversedHead);
	}
	/* black level */
	bMinR = 255;
	bMinG = 255;
	...
	for (i = 0; i < HW_PIXELS; i++) {
		abWhite[i * 3 + 0] =
			_CalcAvg(&pabWhite[i * 3 + 0],
				 endWhiteY - startWhiteY + 1, HW_PIXELS * 3);
		abWhite[i * 3 + 1] =
			_CalcAvg(&pabWhite[i * 3 + 1],
				 endWhiteY - startWhiteY + 1, HW_PIXELS * 3);
		abWhite[i * 3 + 2] =
			_CalcAvg(&pabWhite[i * 3 + 2],
				 endWhiteY - startWhiteY + 1, HW_PIXELS * 3);
	}
