...
init_options(Coolscan_t * scanner)
{
	...
	scanner->opt[OPT_GAMMA_VECTOR].size =
		scanner->lutlength * sizeof(SANE_Word);
		    ...
		    SANE_Action action, void *val, SANE_Int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_PRESCAN:
			*(SANE_Word *) val =
				(scanner->prescan) ? SANE_TRUE : SANE_FALSE;
			return SANE_STATUS_GOOD;
		...
		case OPT_RGB_CONTROL:
			*(SANE_Word *) val =
				(scanner->
				 rgb_control) ? SANE_TRUE : SANE_FALSE;
			return SANE_STATUS_GOOD;

		case OPT_GAMMA_BIND:
			*(SANE_Word *) val =
				(scanner->
				 gamma_bind) ? SANE_TRUE : SANE_FALSE;
			return SANE_STATUS_GOOD;

		case OPT_ANALOG_GAMMA:
			*(SANE_Word *) val =
				(scanner->
				 analog_gamma_r) ? SANE_TRUE : SANE_FALSE;
			return SANE_STATUS_GOOD;

		case OPT_AVERAGING:
			*(SANE_Word *) val =
				(scanner->averaging) ? SANE_TRUE : SANE_FALSE;
			return SANE_STATUS_GOOD;
		...
		case OPT_TL_X:
			scanner->tlx =
				mmToIlu(SANE_UNFIX(*(SANE_Word *) val));
			if (info)
				*info |= SANE_INFO_RELOAD_PARAMS;
