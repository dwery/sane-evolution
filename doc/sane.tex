...
\code{\defn{SANE\_STATUS\_GOOD}}.  The version code of SANE,
\code{\defn{SANE\_CURRENT\_VERSION}}, is returned in the value pointed to by \code{version\_code}.
If that pointer is \code{NULL}, no version code is returned.
Argument \code{authorize} is either a pointer to a function that is invoked
when the backend requires authentication for a specific resource or \code{NULL}
