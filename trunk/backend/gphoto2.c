...
init_gphoto2(void)
{
	...
	if (!Cam_data.camera_name) {
		DBG(0,
		    "init_gphoto2: Camera name not specified in config file\n");
	...
	if (!Cam_data.port) {
		DBG(0,
		    "init_gphoto2: Camera port not specified in config file\n");
		return SANE_STATUS_INVAL;
	}
	...
	if (strcmp(Cam_data.port, "Browse") != 0) {
		CHECK_RET(port =
			  gp_port_info_list_lookup_path(il, Cam_data.port));
	...
	CHECK_RET(gp_camera_folder_list_folders
		  (camera, TopFolder, list, NULL));
	...
	if (SubDirs) {
		for (n = 0; n < gp_list_count(dir_list); n++) {
			...
			if (strchr((const char *) folder_list[n], ' ')) {
				*strchr((const char *) folder_list[n], ' ') =
					'\0';
			}
		}
		if (n == 0) {
...
sane_init(int * version_code, SANE_Auth_Callback UNUSEDARG authorize)
{
	...
	DBG(1,
	    "GPHOTO2 Backend $Id: gphoto2.c,v 1.32 2005-12-03 18:23:55 psfales-guest Exp $\n");
	...
	if (!fp) {
		...
		while (sanei_config_read(dev_name, sizeof(dev_name), fp)) {
			...
			if (strncmp(dev_name, "port=", 5) == 0) {
				...
				for (n = 0; n < entries; n++) {
					result = gp_port_info_list_get_info
						(list, n, &info);
					if (result < 0) {
						gp_port_info_list_free(list);
						return SANE_STATUS_INVAL;
					}
					if (strcmp(Cam_data.port, info.path)
					    == 0) {
						break;
					}
				}
				...
				sprintf(buf, "Image selection - %s",
					Cam_data.camera_name);

				CHECK_RET(gp_abilities_list_new(&al));
				CHECK_RET(gp_abilities_list_load(al, NULL));
				CHECK_RET(entries =
					  gp_abilities_list_count(al));
				...
				sod[GPHOTO2_OPT_IMAGE_SELECTION].title =
					strdup(buf);
			...
			} else if (strncmp(dev_name, "thumb_resolution=", 17)
				   == 0) {
				sscanf(&dev_name[17], "%dx%d", &thumb_width,
				       &thumb_height);
				DBG(20,
				    "Config file thumb_resolution=%ux%u\n",
...
*/
	...
	if (dumpinquiry) {
		...
		DBG(0, "Configuration support            : %s\n",
		    abilities.
		    operations & GP_OPERATION_CONFIG ? "yes" : "no");

		DBG(0, "Delete files on camera support   : %s\n",
		    abilities.
		    file_operations & GP_FILE_OPERATION_DELETE ? "yes" :
		    "no");
		DBG(0, "File preview (thumbnail) support : %s\n",
		    abilities.
		    file_operations & GP_FILE_OPERATION_PREVIEW ? "yes" :
		    "no");
		DBG(0, "File upload support              : %s\n",
		    abilities.
		    folder_operations & GP_FOLDER_OPERATION_PUT_FILE ? "yes" :
		    "no");
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		case GPHOTO2_OPT_IMAGE_NUMBER:
			if (*(int *) value <= Cam_data.pic_taken)
...
*/
		...
		case GPHOTO2_OPT_THUMBS:
			gphoto2_opt_thumbnails = !!*(int *) value;
...
*/
		...
		case GPHOTO2_OPT_SNAP:
			...
			case TRUE:
				gphoto2_opt_snap = TRUE;
				break;
			case FALSE:
				gphoto2_opt_snap = FALSE;
				break;
			default:
				return SANE_STATUS_INVAL;
			}
...
*/
				...
				sod[GPHOTO2_OPT_IMAGE_NUMBER].cap |=
					SANE_CAP_INACTIVE;
			} else {
				/* deactivate the resolution setting */
				sod[GPHOTO2_OPT_LOWRES].cap |=
					SANE_CAP_INACTIVE;
...
*/
		...
		case GPHOTO2_OPT_ERASE:
			gphoto2_opt_erase = !!*(int *) value;
			break;

		case GPHOTO2_OPT_AUTOINC:
			gphoto2_opt_autoinc = !!*(int *) value;
			break;
	...
	case SANE_ACTION_GET_VALUE:
		...
		case GPHOTO2_OPT_THUMBS:
			*(int *) value = gphoto2_opt_thumbnails;
			break;

		case GPHOTO2_OPT_SNAP:
			*(int *) value = gphoto2_opt_snap;
			break;

		case GPHOTO2_OPT_LOWRES:
			*(int *) value = gphoto2_opt_lowres;
			break;

		case GPHOTO2_OPT_ERASE:
			*(int *) value = gphoto2_opt_erase;
			break;

		case GPHOTO2_OPT_AUTOINC:
			*(int *) value = gphoto2_opt_autoinc;
			break;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	parms.last_frame = TRUE;	/* Have no idea what this does */
	*params = parms;
 ...
 */
	...
	Cam_data.scanning = TRUE;	/* don't overlap scan requests */
...
{
	if (Cam_data.scanning) {
		Cam_data.scanning = FALSE;	/* done with scan */
	} else
...
read_dir(char * dir, SANE_Bool read_files)
{
	...
	if (read_files) {
		CHECK_RET(gp_camera_folder_list_files
			  (camera, dir, dir_list, NULL));
	} else {
		CHECK_RET(gp_camera_folder_list_folders
			  (camera, dir, dir_list, NULL));
	}
	...
	if (gphoto2_opt_erase) {
		...
		for (i = 0; i < gp_list_count(dir_list); i++) {
			...
			if (strcmp(tfilename, filename) != 0) {
				CHECK_RET(gp_list_append
					  (tmp_list, tfilename, NULL));
			}
		}
 ...
 */
	...
	(*dest_mgr->put_pixel_rows) (&cinfo, dest_mgr, lines,
				     (char *) linebuffer);
