SANE_Status
sane_init(int * version_code, SANE_Auth_Callback UNUSEDARG authorize)
{
...
sane_open(SANE_String_Const devnam, SANE_Handle * handle)
{
	...
	if (s->hw->info.model == FS2710) {
		...
		for (j = 1; j < 4096; ++j) {	/* FS2710 needs inital gamma 2.0 */
...
sane_close(SANE_Handle handle)
{
	...
	if (s->val[OPT_EJECT_BEFOREEXIT].w) {
		if (s->fd == -1)
			sanei_scsi_open(s->hw->sane.name, &s->fd,
					sense_handler, s->hw);
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_GAMMA_VECTOR_B:
			...
			if (s->val[OPT_CUSTOM_GAMMA_BIND].w == TRUE)
			...
			if (s->hw->info.model != FS2710) {
				...
				status = get_density_curve_fs2710(s,
								  gamma_component,
								  gbuf,
								  &buf_size);
			...
			neg = (s->hw->info.is_filmscanner) ?
				strcmp(filmtype_list[1],
				       s->val[OPT_NEGATIVE].s)
				: s->val[OPT_HNEGATIVE].w;

			for (i = 0; i < 256; i++) {
				if (!neg)
					s->gamma_table[option -
						       OPT_GAMMA_VECTOR][i] =
						(int) gbuf[i];
				else
					s->gamma_table[option -
		...
		case OPT_HW_RESOLUTION_ONLY:
			if (s->val[option].w != *(int *) val) {
				...
				if (!s->val[option].w) {	/* use complete range */
					s->opt[OPT_X_RESOLUTION].
						constraint_type =
						SANE_CONSTRAINT_RANGE;
					s->opt[OPT_X_RESOLUTION].constraint.
						range =
						&s->hw->info.xres_range;
					s->opt[OPT_Y_RESOLUTION].
						constraint_type =
						SANE_CONSTRAINT_RANGE;
					s->opt[OPT_Y_RESOLUTION].constraint.
						range =
					...
					s->opt[OPT_X_RESOLUTION].constraint.
						word_list = s->xres_word_list;
					s->opt[OPT_Y_RESOLUTION].
						constraint_type =
						SANE_CONSTRAINT_WORD_LIST;
					s->opt[OPT_Y_RESOLUTION].constraint.
						word_list = s->yres_word_list;

					/* adjust resolutions */
					xres = s->xres_word_list[1];
					for (iPos = 0;
					     iPos < s->xres_word_list[0];
					     iPos++) {
						if (s->val[OPT_X_RESOLUTION].
						    w >=
						    s->xres_word_list[iPos +
								      1])
							xres = s->
								xres_word_list
								[iPos + 1];
					...
					for (iPos = 0;
					     iPos < s->yres_word_list[0];
					     iPos++) {
						if (s->val[OPT_Y_RESOLUTION].
						    w >=
						    s->yres_word_list[iPos +
								      1])
							yres = s->
								yres_word_list
								[iPos + 1];
		...
		case OPT_BIND_HILO:
			if (s->val[option].w != *(int *) val) {
				...
				if (info)
					*info |= SANE_INFO_RELOAD_OPTIONS;
				if (!s->val[option].w) {	/* don't bind */
					s->opt[OPT_HILITE_R].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_SHADOW_R].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_HILITE_B].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_SHADOW_B].cap &=
						~SANE_CAP_INACTIVE;

					s->opt[OPT_HILITE_G].title =
						SANE_TITLE_HIGHLIGHT_G;
					s->opt[OPT_HILITE_G].name =
						SANE_NAME_HIGHLIGHT_G;
					s->opt[OPT_HILITE_G].desc =
						SANE_DESC_HIGHLIGHT_G;

					s->opt[OPT_SHADOW_G].title =
						SANE_TITLE_SHADOW_G;
					s->opt[OPT_SHADOW_G].name =
						SANE_NAME_SHADOW_G;
					s->opt[OPT_SHADOW_G].desc =
						SANE_DESC_SHADOW_G;
				} else {	/* bind */
					s->opt[OPT_HILITE_R].cap |=
						SANE_CAP_INACTIVE;
		...
		case OPT_FLATBED_ONLY:
			s->val[option].w = *(int *) val;
			if (s->hw->adf.Status != ADF_STAT_NONE && s->val[option].w) {	/* switch on */
			...
			if (s->val[option].w) {	/* switch on */
				...
				s->opt[OPT_TPU_TRANSPARENCY].cap &=
					(s->hw->tpu.ControlMode ==
					 3) ? ~SANE_CAP_INACTIVE : ~0;
				s->opt[OPT_TPU_FILMTYPE].cap &=
					(s->hw->tpu.ControlMode ==
					 1) ? ~SANE_CAP_INACTIVE : ~0;
			} else {	/* switch off */
		...
		case OPT_MODE:
			...
			if (!strcmp(val, SANE_I18N("Lineart"))
				...
				s->opt[OPT_CUSTOM_GAMMA].cap |=
					SANE_CAP_INACTIVE;
				s->opt[OPT_CUSTOM_GAMMA_BIND].cap |=
					SANE_CAP_INACTIVE;
		...
		case OPT_NEGATIVE_TYPE:
			...
			if (s->val[option].s)
				free(s->val[option].s);
			s->val[option].s = strdup(val);
			for (i = 0; strcmp(val, negative_filmtype_list[i]);
			     i++);
		...
		case OPT_CALIBRATION_NOW:
			...
			if (status == SANE_STATUS_GOOD) {
				...
				DBG(1,
				    "calibration: cannot open device file\n");
			s->fd = -1;
			return status;
		...
		case OPT_SCANNER_SELF_DIAGNOSTIC:
			...
			if (status == SANE_STATUS_GOOD) {
				status = send_diagnostic(s->fd);
				{
					...
					DBG(21,
					    "SEND DIAGNOSTIC result: %s\n",
					    sane_strstatus(status));
					sanei_scsi_close(s->fd);
				}
			} else
				DBG(1,
				    "send diagnostic: cannot open device file\n");
			s->fd = -1;
			return status;
		...
		case OPT_RESET_SCANNER:
			...
			if (status == SANE_STATUS_GOOD) {
				...
				DBG(1,
				    "reset scanner: cannot open device file\n");
			s->fd = -1;
			return status;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		if (xres > 0 && yres > 0 && width > 0 && length > 0) {
			DBG(11,
			    "sane_get_parameters: width='%d', xres='%d', mud='%d'\n",
			    width, xres, s->hw->info.mud);
			s->params.pixels_per_line =
				width * xres / s->hw->info.mud;
			DBG(11,
			    "sane_get_parameters: length='%d', yres='%d', mud='%d'\n",
			    length, yres, s->hw->info.mud);
			s->params.lines = length * yres / s->hw->info.mud;
			DBG(11,
			    "sane_get_parameters: pixels_per_line='%d', lines='%d'\n",
	...
	DBG(11,
	    "sane_get_parameters: xres='%d', yres='%d', pixels_per_line='%d', "
...
/******* making a tempfile for 1200 dpi scanning of FB1200S ******/
	if (s->hw->info.model == FB1200) {
		...
		s->tmpfile =
			open(thistmpfile, O_RDWR | O_CREAT | O_EXCL, 0600);
...
/******************************************************************/
	...
	if (SANE_OPTION_IS_ACTIVE(s->opt[OPT_FOCUS_GROUP].cap)
		...
		if ((status = do_focus(s)) != SANE_STATUS_GOOD)
			return (status);
		if (s->val[OPT_AF_ONCE].w)
			s->AF_NOW = FALSE;
	}
#endif

	if (s->val[OPT_CUSTOM_GAMMA].w) {
		if ((status = do_gamma(s)) != SANE_STATUS_GOOD)
			return (status);
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "attach: GET SCAN MODE for scan control conditions failed\n");
		sanei_scsi_close(s->fd);
		return (SANE_STATUS_INVAL);
	}
	...
	if (s->hw->adf.Status != ADF_STAT_NONE) {
		DBG(3,
		    "attach: sending GET SCAN MODE for transparency unit\n");
		memset(ebuf, 0, sizeof(ebuf));
		buf_size = 12;
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "attach: GET SCAN MODE for transparency unit failed\n");
			sanei_scsi_close(s->fd);
			return (SANE_STATUS_INVAL);
		}
		for (i = 0; i < buf_size; i++)
	...
	if (!s->val[OPT_PREVIEW].w)
		wbuf[50] |= s->AE;	/* AE not during preview */
#endif
	wbuf[54] = 2;
	wbuf[57] = 1;
	wbuf[58] = 1;
	...
	if (s->hw->info.model == FB620) {	/* modification for FB620S */
		...
		wbuf[34] = (((600 < s->val[OPT_X_RESOLUTION].w)
			    ...
			    &&
			    (strcmp(s->val[OPT_MODE].s, SANE_I18N("Lineart"))
			     != 0))
			? 12 : s->bpp;
#endif
		wbuf[36] = 0;
	...
	if (s->hw->info.model == FB1200) {
		...
		DBG(3,
		    "sane_start: sending DEFINE SCAN MODE for scan control "
		    "conditions of FB1200\n");
		status = define_scan_mode(s->fd, SCAN_CONTROL_CON_FB1200,
					  wbuf);
	} else {
		wbuf[1] = 14;
		...
		status = define_scan_mode(s->fd, SCAN_CONTROL_CONDITIONS,
					  wbuf);
	}
	if (status != SANE_STATUS_GOOD) {
		DBG(1, "define scan mode failed: %s\n",
	...
	DBG(3,
	    "sane_start: sending GET SCAN MODE for scan control conditions\n");
	memset(ebuf, 0, sizeof(ebuf));
	buf_size = sizeof(ebuf);
	status = get_scan_mode(s->fd, SCAN_CONTROL_CONDITIONS, ebuf,
			       &buf_size);
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "sane_start: GET SCAN MODE for scan control conditions "
		    "failed\n");
		sanei_scsi_close(s->fd);
		return (SANE_STATUS_INVAL);
	}
	...
	if (s->hw->info.can_calibrate) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_start: GET_CALIBRATION_STATUS failed: %s\n",
			    sane_strstatus(status));
			sanei_scsi_close(s->fd);
			s->fd = -1;
			return (status);
		...
		if (cbuf[0] == 1 || cbuf[0] == 2 || cbuf[0] == 3) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "sane_start: EXECUTE_CALIBRATION failed\n");
				sanei_scsi_close(s->fd);
				s->fd = -1;
				return (status);
			}
			...
			buf_size = sizeof(cbuf);
			memset(cbuf, 0, buf_size);
			status = get_calibration_status(s->fd, cbuf,
							&buf_size);
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
...
static SANE_Status
sane_read_direct(SANE_Handle handle, SANE_Byte * buf, int max_len,
		 int * len)
{
	CANON_Scanner *s = handle;
	SANE_Status status;
...
static SANE_Status
read_fs2710(SANE_Handle handle, SANE_Byte * buf, int max_len,
	    int * len)
{
	CANON_Scanner *s = handle;
	SANE_Status status;
	...
	*len = 0;
	...
	if (!strcmp(s->val[OPT_MODE].s, SANE_I18N("Color"))) {
		if (max_len > s->auxbuf_len) {	/* extend buffer? */
			if (s->auxbuf_len > 0)
				free(s->auxbuf);
			s->auxbuf_len = max_len;
			if ((s->auxbuf =
			     (u_char *) malloc(2 * max_len)) == NULL) {
				DBG(1,
				    "sane_read buffer size insufficient\n");
				do_cancel(s);
				return SANE_STATUS_NO_MEM;
			}
		}
		...
		nread2 = 2 * nread;
		if ((status =
		     read_data(s->fd, s->auxbuf,
			       &nread2)) != SANE_STATUS_GOOD) {
			do_cancel(s);
			return (SANE_STATUS_IO_ERROR);
		}
		nread = nread2 / 2;
		...
		if ((status =
		     read_data(s->fd, buf, &nread)) != SANE_STATUS_GOOD) {
			do_cancel(s);
			return (SANE_STATUS_IO_ERROR);
		}
#if defined(WORDS_BIGENDIAN)
...
static SANE_Status
read_fb620(SANE_Handle handle, SANE_Byte * buf, int max_len,
	   int * len)
{
	CANON_Scanner *s = handle;
	SANE_Status status;
...
static SANE_Status
read_fb1200(SANE_Handle handle, SANE_Byte * buf, int max_len,
	    int * len)
{
	CANON_Scanner *s = handle;
	SANE_Status status;
	...
	int ncopy;
	u_char dbuf[28];
	size_t buf_size, nread, remain, nwritten, nremain, pos, pix,
		pixel_per_line, byte, byte_per_line /*, bit */ ;
	...
	if (s->buf_pos >= s->buf_used && s->bytes_to_read) {
      ...
      /**** save the primary scan data to tmpfile ****/
		...
		if ((int) s->bytes_to_read > s->params.bytes_per_line
		    * s->params.lines / 2) {
			...
			while (remain) {
				errno = 0;
				wres = write(s->tmpfile,
					     &s->inbuffer[nwritten], remain);
				if (wres == -1) {
					DBG(1,
					    "error write tmp file: %i, %s\n",
					    errno, strerror(errno));
					do_cancel(s);
					return (SANE_STATUS_NO_MEM);
				}
			...
			if ((int) s->bytes_to_read <=
			    s->params.bytes_per_line * s->params.lines / 2) {
				if ((int) s->bytes_to_read <
				    s->params.bytes_per_line *
				    s->params.lines / 2)
					DBG(1,
					    "warning: read more data for the primary scan "
					    "than expected\n");
      ...
      /** read an entire scan line from the primary scan **/
		...
		while (remain > 0) {
			...
			errno = 0;
			readres =
				read(s->tmpfile, &(firstimage[pos]), nremain);
			if (readres == -1) {
			...
			if (readres == 0) {
				DBG(1,
				    "0 byte read from temp file. premature EOF?\n");
				return (SANE_STATUS_INVAL);
		...
		if (!strcmp(s->val[OPT_MODE].s, SANE_I18N("Color"))) {
			...
			for (byte = 0; (int) byte < maxpix; byte++) {
				s->outbuffer[2 * byte] =
					primaryHigh[firstimage[byte]]
					| secondaryHigh[secondimage[byte]];
				s->outbuffer[2 * byte + 1] =
					primaryLow[firstimage[byte]]
				...
				for (bit = 0; bit < 4; bit++) {
					if (inmask ==
					    (secondimage[byte] & inmask))
						outbyte = outbyte | outmask;
					outmask = outmask >> 1;
					if (inmask ==
					    (firstimage[byte] & inmask))
						outbyte = outbyte | outmask;
					outmask = outmask >> 1;
					inmask = inmask >> 1;
				}
	  ...
	  int * len)
{
	...
	else if (s->hw->info.model == FB1200
...
SANE_Status
sane_get_select_fd(SANE_Handle UNUSEDARG handle, int UNUSEDARG * fd)
{
