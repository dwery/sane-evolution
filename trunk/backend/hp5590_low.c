...
hp5590_get_status(int dn)
{
	...
	if (status != 0x00) {
		DBG(DBG_err,
		    "%s: USB-in-USB: got non-zero device status (status %u)\n",
		    __FUNCTION__, status);
		return SANE_STATUS_DEVICE_BUSY;
	}
		   ...
		   int size, int core_flags)
{
	...
	if (requesttype & USB_DIR_IN) {
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(DBG_err,
			    "%s: USB-in-USB: error sending control message\n",
			    __FUNCTION__);
			return ret;
		}

		DBG(DBG_usb,
		    "%s: USB-in-USB: checking acknowledge for control message\n",
		    __FUNCTION__);
		...
		while (len) {
			...
			ret = sanei_usb_control_msg(dn,
						    USB_DIR_IN |
						    USB_TYPE_VENDOR,
						    core_flags & CORE_DATA ?
						    0x0c : 0x04, 0x90, 0x00,
						    next_packet_size, ptr);
			if (ret != SANE_STATUS_GOOD) {
				DBG(DBG_err,
				    "%s: USB-in-USB: error reading data\n",
				    __FUNCTION__);
				return ret;
			}
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(DBG_err,
			    "%s: USB-in-USB: error confirming data reception\n",
			    __FUNCTION__);
			return -1;
		}

		DBG(DBG_usb,
		    "%s: USB-in-USB: checking if confirmation was acknowledged\n",
		    __FUNCTION__);
		ret = hp5590_get_ack(dn);
		if (ret != SANE_STATUS_GOOD)
			return ret;
	...
	if (!(requesttype & USB_DIR_IN)) {
		...
		while (len) {
			...
			if (ret != SANE_STATUS_GOOD) {
				DBG(DBG_err,
				    "%s: USB-in-USB: error sending data\n",
				    __FUNCTION__);
				return ret;
			}
			...
			if (core_flags & CORE_DATA) {
				DBG(DBG_usb,
				    "%s: USB-in-USB: checking if data was accepted\n",
				    __FUNCTION__);
				ret = hp5590_get_ack(dn);
				if (ret != SANE_STATUS_GOOD)
					return ret;
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(DBG_err,
			    "%s: USB-in-USB: error getting response\n",
			    __FUNCTION__);
			return ret;
		}
		...
		if (core_flags & CORE_BULK_OUT) {
			...
			ret = sanei_usb_control_msg(dn,
						    USB_DIR_OUT |
						    USB_TYPE_VENDOR, 0x0c,
						    0x83, 0x00,
						    sizeof(bulk_flags),
						    &bulk_flags);
			if (ret != SANE_STATUS_GOOD) {
				DBG(DBG_err,
				    "%s: USB-in-USB: error sending bulk flags\n",
				    __FUNCTION__);
				return ret;
			}
			...
			DBG(DBG_usb,
			    "%s: USB-in-USB: checking confirmation for bulk flags\n",
			    __FUNCTION__);
			ret = hp5590_get_ack(dn);
			if (ret != SANE_STATUS_GOOD)
				return ret;
 ...
 */
static SANE_Status
hp5590_verify_last_cmd(int dn, unsigned int cmd)
{
 ...
 */
static SANE_Status
hp5590_cmd(int dn, unsigned int flags,
...
hp5590_low_init_bulk_read_state(void **state)
{
	...
	if (!bulk_read_state->buffer) {
		DBG(DBG_err, "%s: Memory allocation failed for %u bytes\n",
		    __FUNCTION__,
		    ALLOCATE_BULK_READ_PAGES * BULK_READ_PAGE_SIZE);
		return SANE_STATUS_NO_MEM;
	}
 ...
 */
static SANE_Status
hp5590_bulk_read(int dn, unsigned char *bytes, unsigned int size,
		 void *state)
{
	...
	if (bulk_read_state->initialized == 0) {
		DBG(DBG_err,
		    "%s: USB-in-USB: bulk read state not initialized\n",
		    __FUNCTION__);
		return SANE_STATUS_INVAL;
	}
	...
	if (size > bulk_read_state->buffer_size) {
		DBG(DBG_err,
		    "Data requested won't fit in the bulk read buffer "
	...
	while (bulk_read_state->bytes_available < size) {
		DBG(DBG_usb,
		    "%s: USB-in-USB: not enough data in buffer available "
		...
		if (bulk_read_state->total_pages == 1
			...
			/* Send bulk flags */
			DBG(DBG_usb,
			    "%s: USB-in-USB: sending USB-in-USB bulk flags\n",
			    __FUNCTION__);
			bulk_flags = 0x24;
			ret = sanei_usb_control_msg(dn,
						    USB_DIR_OUT |
			...
			DBG(DBG_usb,
			    "%s: USB-in-USB: sending control msg for bulk\n",
			    __FUNCTION__);
			ret = sanei_usb_control_msg(dn,
						    USB_DIR_OUT |
						    USB_TYPE_VENDOR, 0x04,
						    0x82, 0x00, sizeof(ctrl),
						    (unsigned char *) &ctrl);
			if (ret != SANE_STATUS_GOOD) {
				DBG(DBG_err,
				    "%s: USB-in-USB: error sending control msg\n",
				    __FUNCTION__);
				return ret;
			}
			...
			DBG(DBG_usb,
			    "%s: USB-in-USB: checking if control msg was accepted\n",
			    __FUNCTION__);
			ret = hp5590_get_ack(dn);
			if (ret != SANE_STATUS_GOOD)
				return ret;
		...
		if (ret != SANE_STATUS_GOOD) {
			if (ret == SANE_STATUS_EOF)
				return ret;
			DBG(DBG_err,
			    "%s: USB-in-USB: error during bulk read: %s\n",
		...
		if (next_portion != BULK_READ_PAGE_SIZE) {
			DBG(DBG_err, "%s: USB-in-USB: incomplete bulk read "
			    ...
			    __FUNCTION__, BULK_READ_PAGE_SIZE,
			    (u_long) next_portion);
			return SANE_STATUS_IO_ERROR;
		}
		...
		if (bulk_read_state->buffer_in_ptr >
		...
		if (bulk_read_state->buffer_in_ptr ==
		    bulk_read_state->buffer_end_ptr) {
			DBG(DBG_usb,
			    "%s: USB-in-USB: buffer wrapped while writing\n",
			    __FUNCTION__);
			bulk_read_state->buffer_in_ptr =
				bulk_read_state->buffer;
		}
	...
	if (bytes_until_buffer_end <= size) {
		...
		DBG(DBG_usb, "%s: USB-in-USB: reached bulk read buffer end\n",
		    __FUNCTION__);
		memcpy(bytes, bulk_read_state->buffer_out_ptr,
		       bytes_until_buffer_end);
		bulk_read_state->buffer_out_ptr = bulk_read_state->buffer;
		/* And second part (if any) */
		if (bytes_until_buffer_end < size) {
			DBG(DBG_usb,
			    "%s: USB-in-USB: giving 2nd buffer part\n",
			    __FUNCTION__);
		...
		if (bulk_read_state->buffer_out_ptr ==
		    bulk_read_state->buffer_end_ptr) {
			DBG(DBG_usb,
			    "%s: USB-in-USB: buffer wrapped while reading\n",
			    __FUNCTION__);
			bulk_read_state->buffer_out_ptr =
				bulk_read_state->buffer;
		}
 ...
 */
static SANE_Status
hp5590_bulk_write(int dn, int cmd, unsigned char *bytes,
		  unsigned int size)
{
	struct usb_in_usb_bulk_setup ctrl;
	SANE_Status ret;
	...
	while (len) {
		...
		DBG(DBG_usb,
		    "%s: USB-in-USB: checking if command was accepted\n",
		    __FUNCTION__);
		ret = hp5590_get_ack(dn);
		if (ret != SANE_STATUS_GOOD)
			return ret;
		...
		if (ret != SANE_STATUS_GOOD) {
			...
			if (ret == SANE_STATUS_EOF)
				break;
			DBG(DBG_err,
			    "%s: USB-in-USB: error during bulk write: %s\n",
			    __FUNCTION__, sane_strstatus(ret));
			return ret;
		}
