...
{
	SANE_Bool goto_shading_pos = TRUE;
	HWDef *hw = &dev->usbDev.HwSetting;

	switch (strip_state) {
	case 0:
		if (!usb_IsSheetFedDevice(dev)) {
			if (!usb_ModuleToHome(dev, TRUE)) {
				DBG(_DBG_ERROR,
...
{
	if (strip_state == 0)
		if (cano_PrepareToReadWhiteCal(dev, FALSE))
			return FALSE;
...
cano_DoCalibration(Plustek_Device * dev)
{
	...
	if (!scan->skipCoarseCalib) {
		...
		if (cano_PrepareToReadWhiteCal(dev, TRUE))
			return FALSE;
