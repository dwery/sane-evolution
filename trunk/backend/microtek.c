...
get_scan_status(Microtek_Scanner * ms,
		SANE_Int * busy, SANE_Int * bytes_per_line, SANE_Int * lines)
{
...
/********************************************************************/
static SANE_Int
pack_into_dest(SANE_Byte * dest_buffer, SANE_Int dest_length,
	       ring_buffer * rb)
{
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	scanner->midtone_support =
		(dev->info.
		 enhance_cap & MI_ENH_CAP_MIDTONE) ? SANE_TRUE : SANE_FALSE;
	scanner->paper_length =
		(scanner->unit_type ==
		 MS_UNIT_PIXELS) ? dev->info.
		max_y : (SANE_Int) ((double) dev->info.max_y * 8.0 /
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
...

		{
			 ...
			 */
			s->x1 = (SANE_Int) (SANE_UNFIX(s->val[OPT_TL_X].w) *
					    units_per_mm + 0.5);
			s->y1 = (SANE_Int) (SANE_UNFIX(s->val[OPT_TL_Y].w) *
					    units_per_mm + 0.5);
			s->x2 = (SANE_Int) (SANE_UNFIX(s->val[OPT_BR_X].w) *
					    units_per_mm + 0.5);
			s->y2 = (SANE_Int) (SANE_UNFIX(s->val[OPT_BR_Y].w) *
					    units_per_mm + 0.5);
			...
			if (s->unit_type == MS_UNIT_18INCH) {
				...
				widthpix = (SANE_Int) ((double)
						       (s->x2 - s->x1 +
							1) / 8.0 *
						       (double) s->
						       resolution);
				s->params.lines = (SANE_Int) ((double)
							      (s->y2 - s->y1 +
							       1) / 8.0 *
							      (double) s->
							      resolution);
