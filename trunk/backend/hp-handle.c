...
hp_handle_startReader(HpHandle this, HpScsi scsi)
{
	...
	this->reader_pid =
		sanei_thread_begin(this->
				   child_forked ? reader_process :
				   reader_thread, (void *) this);
	if (this->reader_pid != 0) {
		/* Here we are in the parent */
		sigprocmask(SIG_SETMASK, &old_set, 0);

		if (this->child_forked) {	/* After fork(), parent must close writing end of pipe */
			DBG(3,
			    "hp_handle_startReader: parent closes write end of pipe\n");
			close(this->pipe_write_fd);
			this->pipe_write_fd = -1;
		}
...
hp_handle_stopScan(HpHandle this)
{
	...
	if (this->reader_pid) {
		...
		if (!FAILED
		    (sanei_hp_scsi_new(&scsi, this->dev->sanedev.name))) {
			if (WIFSIGNALED(info)) {
				/*
	...
	*out8 = 0;
	...
	case HP_SCANMODE_GRAYSCALE:	/* Grayscale */
		...
		*scan_depth = data_width;
		...
		/* 8 bit output forced ? */
		if (*scan_depth > 8) {
			*out8 = sanei_hp_optset_output_8bit(this->dev->
							    options,
							    this->data);
	...
	case HP_SCANMODE_COLOR:	/* RGB */
		...
		*scan_depth = data_width / 3;
		...
		if ((*scan_depth > 8)
		    && (sanei_hp_device_probe(&compat, scsi) ==
			SANE_STATUS_GOOD)
		    && (compat & HP_COMPAT_PS))
			*soft_invert = 1;
...
sanei_hp_handle_saneoption(HpHandle this, int optnum)
{
	...
	return sanei_hp_optset_saneoption(this->dev->options, this->data,
					  optnum);
}

SANE_Status
sanei_hp_handle_control(HpHandle this, int optnum,
			SANE_Action action, void *valp, int * info)
{
	SANE_Status status;
	HpScsi scsi;
	hp_bool_t immediate;
...
sanei_hp_handle_getParameters(HpHandle this, SANE_Parameters * params)
{
	...
	if (this->cancelled) {
		DBG(1,
		    "sanei_hp_handle_getParameters: cancelled. Stop scan\n");
		RETURN_IF_FAIL(hp_handle_stopScan(this));
	}
	...
	if (!strcmp("C5100A", this->dev->sanedev.model)) {
		...
		if (!FAILED
			...
			RETURN_IF_FAIL(sanei_hp_scl_inquire
				       (scsi, SCL_NUMBER_OF_LINES, &p->lines,
					0, 0));
			sanei_hp_scsi_destroy(scsi, 0);
			*params = this->scan_params;
		}
...
sanei_hp_handle_startScan(HpHandle this)
{
	...
	status = sanei_hp_optset_download(this->dev->options, this->data,
					  scsi);
	...
	procdata->mirror_vertical =
		sanei_hp_optset_mirror_vert(this->dev->options, this->data,
					    scsi);
	DBG(1, "start: %s to mirror image vertically\n",
	    procdata->mirror_vertical ? "Request" : "No request");
	...
	if (adfscan) {
		...
		if ((sanei_hp_device_support_get(this->dev->sanedev.name,
		    ...
		    && (sanei_hp_device_support_get(this->dev->sanedev.name,
						    SCL_CHANGE_DOC, &minval,
						    &maxval)
			!= SANE_STATUS_GOOD)) {
			DBG(3,
			    "start: Request for ADF scan without support of unload doc\n");
			DBG(3,
			    "       and change doc. Seems to be something like a IIp.\n");
...
*/
			...
			DBG(3,
			    "start: Request for ADF scan without support of preload doc.\n");
			...
			DBG(3,
			    "start: Request for ADF scan with support of preload doc.\n");
		...
		if (can_check_paper) {
			if (sanei_hp_scl_inquire
			    (scsi, SCL_ADF_BIN, &adfstat, 0, 0)
			    != SANE_STATUS_GOOD) {
				DBG(1,
				    "start: Error checking if paper in ADF\n");
				sanei_hp_scsi_destroy(scsi, 0);
				return SANE_STATUS_UNSUPPORTED;
			}
			...
			if (adfstat != 1) {
				DBG(1,
				    "start: No paper in ADF bin. Finished.\n");
				sanei_hp_scsi_destroy(scsi, 0);
				return SANE_STATUS_NO_DOCS;
			}

			if (is_flatbed) {
				if (sanei_hp_scl_set(scsi, SCL_CHANGE_DOC, 0)
				    != SANE_STATUS_GOOD) {
					DBG(1,
					    "start: Error changing document\n");
					sanei_hp_scsi_destroy(scsi, 0);
					return SANE_STATUS_UNSUPPORTED;
				}
			}
	...
	DBG(1,
	    "start: %d pixels per line, %d bytes per line, %d lines high\n",
	    this->scan_params.pixels_per_line,
	    this->scan_params.bytes_per_line, this->scan_params.lines);
	procdata->bytes_per_line = (int) this->scan_params.bytes_per_line;
	if (procdata->out8) {
		procdata->bytes_per_line *= 2;
		DBG(1,
		    "(scanner will send %d bytes per line, 8 bit output forced)\n",
...
sanei_hp_handle_read(HpHandle this, void *buf, size_t * lengthp)
{
	...
	if ((nread = read(this->pipe_read_fd, buf, *lengthp)) < 0) {
		*lengthp = 0;
		if (errno == EAGAIN)
			return SANE_STATUS_GOOD;
		DBG(1,
		    "sanei_hp_handle_read: read from pipe: %s. Stop scan\n",
		    strerror(errno));
		hp_handle_stopScan(this);
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (status == SANE_STATUS_EOF) {
		...
		if (sanei_hp_scsi_new(&scsi, this->dev->sanedev.name) ==
		    SANE_STATUS_GOOD) {
			hpinfo = sanei_hp_device_info_get(this->dev->sanedev.
							  name);
...
sanei_hp_handle_setNonblocking(HpHandle this, hp_bool_t non_blocking)
{
	...
	if (this->cancelled) {
		DBG(3,
		    "sanei_hp_handle_setNonblocking: cancelled. Stop scan\n");
		RETURN_IF_FAIL(hp_handle_stopScan(this));
		return SANE_STATUS_CANCELLED;
	}

	if (fcntl(this->pipe_read_fd, F_SETFL, non_blocking ? O_NONBLOCK : 0)
	    < 0)
		return SANE_STATUS_IO_ERROR;
