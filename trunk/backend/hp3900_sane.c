...
static void bknd_constrains(TScanner * scanner, int source,
			    int type);
static SANE_Status bknd_depths(TScanner * scanner, int model);
static SANE_Status bknd_info(TScanner * scanner);
static SANE_Status bknd_models(TScanner * scanner);
static SANE_Status bknd_resolutions(TScanner * scanner, int model);
static SANE_Status bknd_sources(TScanner * scanner, int model);

/* convertions */
static void Color_Negative(SANE_Byte * buffer, int size, int depth);
static void Color_to_Gray(SANE_Byte * buffer, int size, int depth);
static void Gray_to_Lineart(SANE_Byte * buffer, int size,
			    int threshold);
static void Depth_16_to_8(SANE_Byte * from_buffer, int size,
			  SANE_Byte * to_buffer);

/* gamma functions */
static void gamma_apply(TScanner * s, SANE_Byte * buffer, int size,
			int depth);
...
/* reading buffers */
static SANE_Status img_buffers_alloc(TScanner * scanner, int size);
static SANE_Status img_buffers_free(TScanner * scanner);

static SANE_Status option_get(TScanner * scanner, int optid,
			      void *result);
static SANE_Status option_set(TScanner * scanner, int optid,
...
SANE_Status sane_control_option(SANE_Handle h, int n,
...
bknd_resolutions(TScanner * scanner, int model)
{
	...
	if (scanner != NULL) {
		...
		{
			int myres[] =
				{ 8, 50, 75, 100, 150, 200, 300, 600, 1200 };
		...
		{
			int myres[] =
				{ 10, 50, 75, 100, 150, 200, 300, 600, 1200,
				2400, 4800
			};
		...
		{
			int myres[] =
				{ 9, 50, 75, 100, 150, 200, 300, 600, 1200,
				2400
			};
...
bknd_info(TScanner * scanner)
{
	...
	if (scanner != NULL) {
		...
		scanner->aValues[opt_scancount].w =
			RTS_ScanCounter_Get(device);
...
img_buffers_alloc(TScanner * scanner, int size)
{
	...
	if (scanner != NULL) {
		...
		scanner->image =
			(SANE_Byte *) malloc(size * sizeof(SANE_Byte));
		if (scanner->image != NULL) {
			scanner->rest =
				(SANE_Byte *) malloc(size *
						     sizeof(SANE_Byte));
			if (scanner->rest != NULL)
...
set_ScannerModel(int proposed, int product, int vendor)
{
	...
	char *sdevname[9] =
		{ "Unknown", "HP3970", "HP4070", "HP4370", "UA4900", "HP3800",
		"HPG3010",
		"BQ5550", "HPG2710"
	};
	...
	if (proposed < 0) {
		if ((current < 0) || (current >= DEVSCOUNT)) {
			DBG(DBG_VRB,
			    " -> Unknown device. Defaulting to HP3970...\n");
...
static void
Color_Negative(SANE_Byte * buffer, int size, int depth)
{
	if (buffer != NULL) {
		int a;
...
static void
Depth_16_to_8(SANE_Byte * from_buffer, int size, SANE_Byte * to_buffer)
{
...
static void
Gray_to_Lineart(SANE_Byte * buffer, int size, int threshold)
{
...
static void
Color_to_Gray(SANE_Byte * buffer, int size, int depth)
{
...
gamma_create(TScanner * s, double gamma)
{
	...
	if (s != NULL) {
		...
		for (a = CL_RED; a <= CL_BLUE; a++) {
			s->aGammaTable[a] = malloc(65536 * sizeof(int));
			if (s->aGammaTable[a] == NULL) {
				rst = ERROR;
				break;
			}
...
static void
gamma_apply(TScanner * s, SANE_Byte * buffer, int size, int depth)
{
		...
		if ((s->aGammaTable[CL_RED] != NULL)
			...
			for (c = 0; c < size / dot_size; c++) {
				if (depth > 8) {
					*sColor =
						s->
						aGammaTable[CL_RED][*sColor];
					*(sColor + 1) =
						s->
						aGammaTable[CL_GREEN][*
								      (sColor
								       + 1)];
					*(sColor + 2) =
						s->
	...
	if (scanner != NULL) {
		...
		for (i = opt_begin; i < opt_count; i++) {
			...
			case opt_tlx:
				...
				pDesc->constraint_type =
					SANE_CONSTRAINT_RANGE;
				pDesc->constraint.range =
					&scanner->rng_horizontal;
				pDesc->cap =
			...
			case opt_scantype:
				...
				pDesc->size =
					max_string_size(scanner->
							list_sources);
				pDesc->constraint_type =
					SANE_CONSTRAINT_STRING_LIST;
				pDesc->constraint.string_list =
			...
			case opt_model:
				...
				pDesc->type = SANE_TYPE_STRING;
				pDesc->size =
					max_string_size(scanner->list_models);
				pDesc->constraint_type =
					SANE_CONSTRAINT_STRING_LIST;
				pDesc->constraint.string_list =
					scanner->list_models;
			...
			case opt_nogamma:
				...
				pDesc->title =
					SANE_I18N("Disable gamma correction");
				pDesc->desc =
					SANE_I18N
			...
			case opt_dbgimages:
				...
				pDesc->title =
					SANE_I18N("Save debugging images");
				pDesc->desc =
					SANE_I18N
			...
			case opt_chipid:
				...
				pDesc->desc =
					SANE_I18N("Shows the chipset ID");
				pDesc->type = SANE_TYPE_INT;
				pDesc->unit = SANE_UNIT_NONE;
				pDesc->constraint_type = SANE_CONSTRAINT_NONE;
				pDesc->cap =
			...
			case opt_button_5:
			{
				...
				sprintf(title, "Scanner button %d",
					i - opt_button_0);
...
_ReportDevice(TScannerModel * pModel, const char *pszDeviceName)
{
	...
	if (pNew != NULL) {
		...
		if (_pFirstSaneDev != NULL) {
			...
			for (pDev = _pFirstSaneDev; pDev->pNext;
			     pDev = pDev->pNext);
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (conf_fp) {
		...
		/* default */
		DBG(DBG_VRB,
		    "- %s not found. Looking for hardcoded usb ids ...\n",
		    HP3900_CONFIG_FILE);
		...
		if (rst == SANE_STATUS_GOOD) {
			...
			if (s != NULL) {
				...
				if (Init_Vars() == OK) {
					...
					set_ScannerModel(s->model, product,
							 vendor);
...
sane_get_option_descriptor(SANE_Handle h, int n)
{
...
option_set(TScanner * scanner, int optid, void *value, int * pInfo)
{
	...
	if (scanner != NULL) {
		if (scanner->fScanning == FALSE) {
			...
			case opt_scantype:
				if (strcmp(scanner->aValues[optid].s, value)
				    != 0) {
					struct st_coords *coords;
					int source;

					if (scanner->aValues[optid].s)
						free(scanner->aValues[optid].
						     s);

					scanner->aValues[optid].s =
						strdup(value);

					source = Get_Source(scanner->
							    aValues
							    [opt_scantype].s);
					coords = Constrains_Get(device,
								source);
					if (coords != NULL) {
						bknd_constrains(scanner,
								source, 0);
						bknd_constrains(scanner,
								source, 1);
						scanner->aValues[opt_tlx].w =
							0;
						scanner->aValues[opt_tly].w =
							0;
						scanner->aValues[opt_brx].w =
							coords->width;
						scanner->aValues[opt_bry].w =
							coords->height;
					}
			...
			case opt_model:
				if (strcmp(scanner->aValues[optid].s, value)
					...
					if (model != RTS_Debug->dev_model) {
						...
						bknd_colormodes(scanner,
								model);
						bknd_depths(scanner, model);
						bknd_resolutions(scanner,
								 model);
...
SANE_Status
sane_control_option(SANE_Handle h, int n, SANE_Action Action,
		    void *pVal, int * pInfo)
{
	TScanner *scanner;
	SANE_Status rst;
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * p)
{
	...
	if (s != NULL) {
		...
		frameformat =
			(colormode ==
			 CM_COLOR) ? SANE_FRAME_RGB : SANE_FRAME_GRAY;

		/* depth */
		depth = (colormode ==
			 CM_LINEART) ? 1 : s->aValues[opt_depth].w;
...
sane_start(SANE_Handle h)
{
	...
	if (s != NULL) {
		...
		if (!((source != ST_NORMAL)
		      && (RTS_isTmaAttached(device) == FALSE))) {
			/* Get depth */
			depth = s->aValues[opt_depth].w;

			/* Get color mode */
			colormode =
				Get_Colormode(s->aValues[opt_colormode].s);
			...
			if (s->cnv.colormode == CM_LINEART)
				...
				s->cnv.real_depth =
					(s->aValues[opt_realdepth].w ==
					 TRUE) ? TRUE : FALSE;

			/* use gamma? */
			RTS_Debug->EnableGamma =
				(s->aValues[opt_nogamma].w ==
				 TRUE) ? FALSE : TRUE;

			/* disable white shading correction? */
			RTS_Debug->wshading =
				(s->aValues[opt_nowshading].w ==
				 TRUE) ? FALSE : TRUE;

			/* skip warmup process? */
			RTS_Debug->warmup =
				(s->aValues[opt_nowarmup].w ==
				 TRUE) ? FALSE : TRUE;

			/* save debugging images? */
			RTS_Debug->SaveCalibFile =
				(s->aValues[opt_dbgimages].w ==
				 TRUE) ? TRUE : FALSE;
			...
			if (Translate_coords(&coords) == SANE_STATUS_GOOD) {
				...
				Set_Coordinates(source, res,
						&s->ScanParams.coords);

				/* emulating depth? */
				if ((s->cnv.real_depth == FALSE)
				    && (depth < 16)
				...
				if (RTS_Scanner_SetParams
				    (device, &s->ScanParams) == OK) {
					/* Start scanning process */
					if (RTS_Scanner_StartScan(device) ==
					    OK) {
						/* Allocate buffer to read one line */
						s->mylin = 0;
						rst = img_buffers_alloc(s,
									bytesperline);
					}
				}
			}
...
SANE_Status
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * len)
{
	SANE_Status rst = SANE_STATUS_GOOD;
	TScanner *s = (TScanner *) h;
	...
	if ((s != NULL) && (buf != NULL) && (len != NULL)) {
		...
		*len = 0;
		...
		if ((s->mylin == s->ScanParams.coords.height)
			...
			buffer = (SANE_Byte *) malloc(emul_maxlen *
						      sizeof(SANE_Byte));
