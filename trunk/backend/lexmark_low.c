...
static SANE_Status low_usb_bulk_write(int devnum,
				      SANE_Byte * cmd, size_t * size);
static SANE_Status low_usb_bulk_read(int devnum,
				     SANE_Byte * buf, size_t * size);
static SANE_Status low_write_all_regs(int devnum, SANE_Byte * regs);
static SANE_Bool low_is_home_line(unsigned char *buffer);
static SANE_Status low_get_start_loc(int resolution,
				     int * vert_start,
				     int * hor_start,
				     int offset, Lexmark_Device * dev);
static void low_rewind(Lexmark_Device * dev, SANE_Byte * regs);
static SANE_Status low_start_mvmt(int devnum);
static SANE_Status low_stop_mvmt(int devnum);
static SANE_Status low_clr_c6(int devnum);
static SANE_Status low_simple_scan(Lexmark_Device * dev,
				   SANE_Byte * regs,
				   int xoffset,
				   int pixels,
				   int yoffset, int lines, SANE_Byte ** data);
 ...
 */
static SANE_Status
rts88xx_read_reg(int devnum, int index, SANE_Byte * reg)
{
 ...
 */
static SANE_Status
rts88xx_write_reg(int devnum, int index, SANE_Byte * reg)
{
	SANE_Status status = SANE_STATUS_GOOD;
 ...
 */
static SANE_Status
rts88xx_write_regs(int devnum, int start, SANE_Byte * source,
		   int length)
{
	size_t size = 0;
	...
	if ((start + length > 0xb3) && (length > 1)) {
		size = 0xb3 - start;
		if (low_usb_bulk_write(devnum, source, &size) !=
		    SANE_STATUS_GOOD) {
			DBG(5,
			    "rts88xx_write_regs : write registers part 1 failed ...\n");
	...
	if (low_usb_bulk_write(devnum, source, &size) != SANE_STATUS_GOOD) {
		DBG(5,
		    "rts88xx_write_regs : write registers part 2 failed ...\n");
		return SANE_STATUS_IO_ERROR;
	}
 ...
 */
static SANE_Status
rts88xx_read_data(int devnum, size_t needed, SANE_Byte * data,
		  size_t * size)
{
...
static SANE_Status
rts88xx_commit(int devnum, SANE_Byte depth)
{
	SANE_Status status;
	SANE_Byte reg;
...
{
	SANE_Byte regs[14] =
		{ 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00,
		0x00, 0x00,
		0x00, 0x60
	};
	...
	if (color) {
		...
		for (y = 0; y < lines; y++) {
			for (x = 0; x < pixels; x += 2) {
				...
				fputc(data[y * pixels * 3 + x + 1 + pixels],
				      fdbg);
				fputc(data
				      [y * pixels * 3 + x + 1 + pixels * 2],
				      fdbg);
				fputc(data[y * pixels * 3 + x], fdbg);
				fputc(data[y * pixels * 3 + x + pixels],
				      fdbg);
				fputc(data[y * pixels * 3 + x + pixels * 2],
				      fdbg);
			}
		}
	} else {
...
SANE_Status
low_usb_bulk_write(int devnum, SANE_Byte * cmd, size_t * size)
{
	SANE_Status status;
	size_t cmd_size;
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(5,
		    ...
		    sane_strstatus(status), (u_long) * size,
		    (u_long) cmd_size);
...
SANE_Status
low_usb_bulk_read(int devnum, SANE_Byte * buf, size_t * size)
{
	SANE_Status status;
	size_t exp_size;
...
static SANE_Status
low_start_scan(int devnum, SANE_Byte * regs)
{
	SANE_Status status;
		...
		int pixels, int yoffset, int lines, SANE_Byte ** data)
{
	...
	*data = (SANE_Byte *) malloc(bpl * lines);
	...
	do {
		/* this block would deserve to be a function */
		status = rts88xx_read_data(dev->devnum, needed - read,
					   (*data) + read, &size);
		if (status != SANE_STATUS_GOOD)
			return status;
		read += size;
	...
	if (regs[0xc3] & 0x80) {
		i = 0;
		do {
			if (rts88xx_read_reg(dev->devnum, 0xb3, &reg) !=
			    SANE_STATUS_GOOD) {
				DBG(5,
				    "low_simple_scan: register read failed ...\n");
				return SANE_STATUS_IO_ERROR;
			}
			usleep(100000);
			i++;
...
sanei_lexmark_low_open_device(Lexmark_Device * dev)
{
	...
	if (shadow_regs[0] == 0x91) {
		...
		DBG(7, "startx=%d, endx=%d, pixels=%d, coef=%d, r2f=0x%02x\n",
		    sx, ex, ex - sx, dev->shadow_regs[0x7a],
		    shadow_regs[0x2f]);
	...
	if (shadow_regs[0xb0] == 0x2c
	    && dev->model.sensor_type == X1100_B2_SENSOR) {
...
/* This function writes the contents of the given registers to the 
     scanner. */
SANE_Status
low_write_all_regs(int devnum, SANE_Byte * regs)
{
	int i;
	SANE_Status status;
	size_t size;
...
low_is_home_line(unsigned char *buffer)
{
	/*
	   ...
	   transitions when we reach the end we return FALSE.
...
/* Move Forward without scanning: */
	...
	scan_head_moving = TRUE;
	while (scan_head_moving) {
#ifdef FAKE_USB
		scan_head_moving = FALSE;
#else
		cmd_size = 0x04;
		low_usb_bulk_write(devnum, pollstopmoving_command_block,
				   &cmd_size);
		cmd_size = 0x1;
...
sanei_lexmark_low_search_home_fwd(Lexmark_Device * dev)
{
	...
	if (ret_val)
		DBG(2,
		    "sanei_lexmark_low_search_home_fwd: !!!HOME POSITION!!!\n");
   ...
   dot. */
	...
	while (buffer_count < buffer_limit) {
		...
		while (!got_line) {
			...
			cmd_size = 4;
			status = low_usb_bulk_write(devnum, command4_block,
						    &cmd_size);
			if (status != SANE_STATUS_GOOD)
				return FALSE;
			cmd_size = 0x3;
			status = low_usb_bulk_read(devnum, poll_result,
						   &cmd_size);
			if (status != SANE_STATUS_GOOD)
				return FALSE;
			cmd_size = 4;
		...
		cmd_size = 4;
		status = low_usb_bulk_write(devnum, command5_block,
					    &cmd_size);
		if (status != SANE_STATUS_GOOD)
			return FALSE;
		/* Read it */
		...
		for (i = 0; i < no_of_buffers; i++) {
			...
			if (in_home_region) {
				...
				rewind(img);
				DBG(2,
				    "sanei_lexmark_low_search_home_bwd: offset=%d\n",
				    i);
				fprintf(img, "P5\n2500 %03d\n", i);
				fclose(img);
#endif
...
low_get_start_loc(int resolution, int * vert_start,
		  int * hor_start, int offset, Lexmark_Device * dev)
{
	int start_600;
...
low_set_scan_area(int res,
		  ...
		  int bry,
		  int offset,
		  SANE_Bool half_step, SANE_Byte * regs, Lexmark_Device * dev)
{
...
SANE_Status
sanei_lexmark_low_set_scan_regs(Lexmark_Device * dev, int offset,
				SANE_Bool calibrated)
{
	...
	if (yres == 75) {
		DBG(5,
		    "sanei_lexmark_low_set_scan_regs(): 75 DPI resolution\n");
	...
	if (yres == 150) {
		DBG(5,
		    "sanei_lexmark_low_set_scan_regs(): 150 DPI resolution\n");
	...
	if (yres == 300) {
		DBG(5,
		    "sanei_lexmark_low_set_scan_regs(): 300 DPI resolution\n");
	...
	if (yres == 600) {
		DBG(5,
		    "sanei_lexmark_low_set_scan_regs(): 600 DPI resolution\n");
	...
	if (yres == 1200) {
		DBG(5,
		    "sanei_lexmark_low_set_scan_regs(): 1200 DPI resolution\n");
	...
	if (calibrated) {
		...
		if (rts88xx_is_color(dev->shadow_regs)) {
			rts88xx_set_offset(dev->shadow_regs,
					   ...
					   dev->offset.green,
					   dev->offset.blue);
			rts88xx_set_gain(dev->shadow_regs, dev->gain.red,
					 dev->gain.green, dev->gain.blue);
		} else {
			rts88xx_set_offset(dev->shadow_regs,
					   dev->offset.gray,
					   dev->offset.gray,
					   dev->offset.gray);
			rts88xx_set_gain(dev->shadow_regs, dev->gain.gray,
					 dev->gain.gray, dev->gain.gray);
		}
	}
...
long
sanei_lexmark_low_read_scan_data(SANE_Byte * data, int size,
				 Lexmark_Device * dev)
{
	...
	if (strcmp(dev->val[OPT_MODE].s, SANE_VALUE_SCAN_MODE_COLOR) == 0)
		...
		isColourScan = FALSE;
		/* grayscale  mode */
		if (strcmp(dev->val[OPT_MODE].s, SANE_VALUE_SCAN_MODE_GRAY) ==
		    0)
			isGrayScan = TRUE;
	}
	...
	if (dev->transfer_buffer == NULL) {
		if (dev->bytes_remaining > 0) {
			...
			dev->transfer_buffer =
				(SANE_Byte *) malloc(MAX_XFER_SIZE);
			if (dev->transfer_buffer == NULL)
				return SANE_STATUS_NO_MEM;
			...
			cmd_size = 4;
			low_usb_bulk_write(dev->devnum, command1_block,
					   &cmd_size);

			/* Read: xfer_size  bytes */
			cmd_size = xfer_request;
			low_usb_bulk_read(dev->devnum, dev->transfer_buffer,
					  &cmd_size);
	...
	if (read_buffer_bytes_available(dev->read_buffer) >=
	    dev->bytes_in_buffer) {
		even_byte = TRUE;
		while (dev->bytes_in_buffer) {
			...
			/* Lineart Scan */
				...
				if (even_byte)
					read_buffer_add_bit_lineart(dev->
								    read_buffer,
								    dev->
								    read_pointer
								    + 1,
			...
			dev->read_pointer =
				dev->read_pointer + sizeof(SANE_Byte);
			dev->bytes_in_buffer--;
		}
	...
	if ((dev->bytes_remaining == 0)
	    && read_buffer_is_empty(dev->read_buffer)) {
		if (!dev->eof) {
			DBG(2,
			...
			DBG(2,
			    "ERROR: Why are we trying to set eof more than once?\n");
		}
	}
...
read_buffer_init(Lexmark_Device * dev, int bytesperline)
{
	...
	dev->read_buffer->empty = TRUE;
	dev->read_buffer->image_line_no = 0;
	dev->read_buffer->bit_counter = 0;
	dev->read_buffer->max_lineart_offset =
	...
	case RED:
		*(rb->writeptr + rb->red_offset) = *byte_pointer;
		if (rb->red_offset == rb->max_red_offset) {
			...
			rb->red_offset =
				rb->red_offset + (3 * sizeof(SANE_Byte));
		return SANE_STATUS_GOOD;
	case GREEN:
		*(rb->writeptr + rb->green_offset) = *byte_pointer;
		if (rb->green_offset == rb->max_green_offset) {
			...
			rb->green_offset =
				rb->green_offset + (3 * sizeof(SANE_Byte));
		return SANE_STATUS_GOOD;
	case BLUE:
		*(rb->writeptr + rb->blue_offset) = *byte_pointer;
		if (rb->blue_offset == rb->max_blue_offset) {
			...
			rb->blue_offset =
				rb->blue_offset + (3 * sizeof(SANE_Byte));
		return SANE_STATUS_GOOD;
	}
	return SANE_STATUS_GOOD;
}
	...
	if (rb->bit_counter == rb->max_lineart_offset) {
		...
		if (rb->gray_offset != rb->max_gray_offset) {
			...
			DBG(5, "  Bit Index: %d, Byte Index: %d. \n",
			    rb->bit_counter, rb->max_gray_offset);
			return SANE_STATUS_INVAL;
		}
		rb->image_line_no++;
...
sanei_lexmark_low_offset_calibration(Lexmark_Device * dev)
{
	...
	pixels = (dev->sensor->offset_endx -
		  dev->sensor->offset_startx) / regs[0x7a];
	...
	while (((i > 0) && (average > dev->sensor->offset_threshold))
		...
		status = low_simple_scan(dev, regs,
					 dev->sensor->offset_startx, pixels,
					 yoffset, lines, &data);
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
		...
		write_pnm_file(title, pixels, lines, rts88xx_is_color(regs),
			       data);
#endif
		average =
			average_area(regs, data, pixels, lines, &ra, &ga,
				     &ba);
	}
	if (i == 0) {
	...
	status = low_simple_scan(dev, regs, dev->sensor->offset_startx,
				 pixels, yoffset, lines, &data);
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "sanei_lexmark_low_offset_calibration: low_simple_scan failed!\n");
		if (data != NULL)
...
#ifdef DEEP_DEBUG
	write_pnm_file("offset-final.pnm", pixels, lines,
		       rts88xx_is_color(regs), data);
#endif
...
sanei_lexmark_low_gain_calibration(Lexmark_Device * dev)
{
	...
	while (((rts88xx_is_color(regs)
		...
		status = low_simple_scan(dev, regs, sx, pixels, yoffset,
					 lines, &data);
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
...
sanei_lexmark_low_shading_calibration(Lexmark_Device * dev)
{
	...
	/* execute scan */
	status = low_simple_scan(dev, regs, sx, pixels, yoffset, lines,
				 &data);
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
	...
	for (i = 0; i < pixels; i++) {
		...
		if (rts88xx_is_color(dev->shadow_regs)) {
			...
			dev->shading_coeff[i] =
				(rtarget /
				 (dev->shading_coeff[i] / (lines - yoffset)));
			...
			for (j = yoffset; j < lines; j++)
				dev->shading_coeff[i + pixels] +=
					data[i + j * bpl + pixels];
			dev->shading_coeff[i + pixels] =
				((gtarget / dev->shading_coeff[i + pixels]) *
				 (lines - yoffset));
			...
			dev->shading_coeff[i] =
				(rtarget / dev->shading_coeff[i]) * (lines -
								     yoffset);
		}
	}
...
#ifdef DEEP_DEBUG
	write_pnm_file("shading_bwd.pnm", pixels, lines,
		       rts88xx_is_color(regs), data);
#endif
	free(data);
