...
find_scanners(void)
{
	...
	PDBG(pixma_dbg
	     (3, "pixma_find_scanners() found %u devices\n", nscanners));
	dev_list =
		(const SANE_Device **) calloc(nscanners + 1,
					      sizeof(*dev_list));
	if (!dev_list)
		return;
...
static void
update_button_state(pixma_sane_t * ss, int * info)
{
...
static SANE_Bool
enable_option(pixma_sane_t * ss, int o, SANE_Bool enable)
{
	int save = SOD(o).cap;
	if (enable)
...
static void
clamp_value(pixma_sane_t * ss, int n, void *v, int * info)
{
...
static void
select_value_from_list(pixma_sane_t * ss, int n, void *v,
		       int * info)
{
	SANE_Option_Descriptor *sod = &SOD(n);
	int *va = (int *) v;
	const int *list = sod->constraint.word_list;
	int i, j, nmemb;

	nmemb = sod->size / sizeof(int);
	for (i = 0; i < nmemb; i++) {
		int value = va[i];
		int mindelta = abs(value - list[1]);
		int nearest = list[1];
		for (j = 2; j <= list[0]; j++) {
			int delta = abs(value - list[j]);
			if (delta < mindelta) {
				mindelta = delta;
				nearest = list[j];
			}
...
static SANE_Status
control_scalar_option(pixma_sane_t * ss, int n, SANE_Action a, void *v,
		      int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		case SANE_TYPE_FIXED:
			...
			else if (opt->sod.constraint_type ==
...
static SANE_Status
control_string_option(pixma_sane_t * ss, int n, SANE_Action a, void *v,
		      int * info)
{
	option_descriptor_t *opt = &(OPT_IN_CTX[n]);
...
static SANE_Status
control_option(pixma_sane_t * ss, int n,
	       SANE_Action a, void *v, int * info)
{
	int result, i;
	...
	case opt_custom_gamma:
		if (a == SANE_ACTION_SET_VALUE || a == SANE_ACTION_SET_AUTO) {
			if (enable_option
			    (ss, opt_gamma_table, OVAL(opt_custom_gamma).b))
				*info |= SANE_INFO_RELOAD_OPTIONS;
		}
		break;
	}
...
print_scan_param(int level, const pixma_scan_param_t * sp)
{
	...
	pixma_dbg(level,
		  "  line_size=%u image_size=%u channels=%u depth=%u\n",
...
#undef PIXEL
	...
	if (error < 0) {
		PDBG(pixma_dbg
		     (1, "BUG:calc_scan_param() failed %d\n", error));
...
init_option_descriptors(pixma_sane_t * ss)
{
	...
	if (cfg->cap & PIXMA_CAP_GAMMA_TABLE) {
		enable_option(ss, opt_custom_gamma, TRUE);
		sane_control_option(ss, opt_custom_gamma,
				    SANE_ACTION_SET_AUTO, NULL, NULL);
...
reader_loop(pixma_sane_t * ss)
{
	...
	if (OVAL(opt_button_controlled).b && ss->page_count == 0) {
		...
		pixma_dbg(1,
			  "==== To proceed, presse 'SCAN' or 'COLOR' button. "
...
start_reader_task(pixma_sane_t * ss)
{
	...
	if (pipe(fds) == -1) {
		PDBG(pixma_dbg
		     (1, "ERROR:start_reader_task():pipe() failed %s\n",
		      strerror(errno)));
		return PIXMA_ENOMEM;
	}
	ss->rpipe = fds[0];
	ss->wpipe = fds[1];
	ss->reader_stop = FALSE;
...
read_image(pixma_sane_t * ss, void *buf, unsigned size, int *readlen)
{
	...
	if (count == -1) {
		...
		if (!ss->cancel) {
			PDBG(pixma_dbg
			     (1, "WARNING:read_image():read() failed %s\n",
			      strerror(errno)));
		}
		close(ss->rpipe);
		ss->rpipe = -1;
	...
	if (ss->image_bytes_read > ss->sp.image_size) {
		PDBG(pixma_dbg
		     (1, "BUG:ss->image_bytes_read > ss->sp.image_size\n"));
	}
	if (ss->image_bytes_read >= ss->sp.image_size) {
		close(ss->rpipe);
		ss->rpipe = -1;
...
sane_open(SANE_String_Const name, SANE_Handle * h)
{
	...
	*h = NULL;
	...
	for (ss = first_scanner; ss; ss = ss->next) {
		if (strcmp(pixma_get_string(ss->s, PIXMA_STRING_ID), name) ==
		    0) {
	...
	if ((cfg->cap & PIXMA_CAP_EXPERIMENT) != 0) {
		...
		if (getenv_atoi("PIXMA_EXPERIMENT", 0) == 0) {
			pixma_dbg(1,
				  "Experimental SANE backend for %s is disabled "
...
sane_close(SANE_Handle h)
{
	...
	for (p = &first_scanner; *p && *p != (pixma_sane_t *) h;
	     p = &((*p)->next)) {
	}
	if (!(*p))
		return;
...
SANE_Status
sane_control_option(SANE_Handle h, int n,
...
sane_start(SANE_Handle h)
{
	...
	if (error >= 0) {
		ss->output_line_size =
			ss->sp.w * ss->sp.channels * (ss->sp.depth / 8);
...
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * len)
{
	...
	if (ss->cancel)
		return SANE_STATUS_CANCELLED;
	if ((ss->idle)
	    && ((ss->sp.source & (PIXMA_SOURCE_ADF | PIXMA_SOURCE_ADFDUP)) !=
		0))
		return SANE_STATUS_INVAL;
	if (!ss->scanning)
