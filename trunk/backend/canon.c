...
static SANE_Byte primaryHigh[256], primaryLow[256], secondaryHigh[256], secondaryLow[256];	/* modification for FB1200S */
...
sense_handler(int scsi_fd, u_char * result, void *arg)
{
	...
	DBG(11, "%s(%ld, %p, %p)\n", me, (long) scsi_fd, (void *) result,
	    (void *) arg);
	DBG(11,
	    "sense buffer: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x "
	...
	if (strncmp(dev->sane.vendor, "CANON", 5) != 0)
		return (status);
	...
	if (dev && dev->info.is_scsi2) {
		...
		case 0x05:
			...
			case 0x2c00:
				sense_str =
					SANE_I18N("command sequence error");
				status = SANE_STATUS_UNSUPPORTED;
				break;
			case 0x2c01:
				sense_str =
		...
		case 0x0b:
			...
			case 0x4900:
				sense_str =
					SANE_I18N("invalid message error");
				status = SANE_STATUS_UNSUPPORTED;
				break;
			case 0x8000:
...
/*   s->val[OPT_AF_NOW].w == TRUE; */
	...
	memset(ebuf, 0, sizeof(ebuf));
	buf_size = 32;
	status = get_scan_mode(fd, (u_char) ALL_SCAN_MODE_PAGES, ebuf,
			       &buf_size);
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "attach: GET SCAN MODE for scan control conditions failed\n");
	...
	if (!strncmp(str, "IX-27015", 8)) {	/* FS2700S */
		...
		dev->info.can_focus = TRUE;
		dev->info.can_autoexpose = TRUE;
		dev->info.can_calibrate = FALSE;
		dev->info.can_eject = TRUE;
		dev->info.can_mirror = TRUE;
		dev->info.is_scsi2 = TRUE;
		dev->info.is_filmscanner = TRUE;
		dev->info.has_fixed_resolutions = TRUE;
		...
		dev->adf.Status = ADF_STAT_NONE;
		dev->tpu.Status = TPU_STAT_NONE;
		dev->info.can_focus = FALSE;
		dev->info.can_autoexpose = FALSE;
		dev->info.can_calibrate = TRUE;
		dev->info.can_eject = FALSE;
		dev->info.can_mirror = FALSE;
		dev->info.is_scsi2 = TRUE;
		dev->info.is_filmscanner = FALSE;
		dev->info.has_fixed_resolutions = TRUE;
		...
		dev->info.is_scsi2 = TRUE;
		dev->info.is_filmscanner = FALSE;
		dev->info.has_fixed_resolutions = FALSE;
	}
...
init_options(CANON_Scanner * s)
{
	...
	s->opt[OPT_NEGATIVE_TYPE].desc =
		SANE_I18N("Selects the negative film type");
	s->opt[OPT_NEGATIVE_TYPE].type = SANE_TYPE_STRING;
	s->opt[OPT_NEGATIVE_TYPE].size =
		max_string_size(negative_filmtype_list);
	s->opt[OPT_NEGATIVE_TYPE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	s->opt[OPT_NEGATIVE_TYPE].constraint.string_list =
		negative_filmtype_list;
	...
	s->opt[OPT_SCANNING_SPEED].size =
		max_string_size(scanning_speed_list);
	s->opt[OPT_SCANNING_SPEED].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	s->opt[OPT_SCANNING_SPEED].constraint.string_list =
		scanning_speed_list;
	...
	s->opt[OPT_HW_RESOLUTION_ONLY].title =
		SANE_I18N("Hardware resolution");
	s->opt[OPT_HW_RESOLUTION_ONLY].desc =
		SANE_I18N("Use only hardware " "resolutions");
	s->opt[OPT_HW_RESOLUTION_ONLY].type = SANE_TYPE_BOOL;
	s->val[OPT_HW_RESOLUTION_ONLY].w = TRUE;
	...
	s->opt[OPT_AF_ONCE].desc =
		SANE_I18N("Do auto focus only once between " "ejects");
	...
	s->opt[OPT_MIRROR].cap |=
		(s->hw->info.can_mirror) ? 0 : SANE_CAP_INACTIVE;
	...
	s->val[OPT_CUSTOM_GAMMA_BIND].w = TRUE;
	...
	s->opt[OPT_AE].desc =
		SANE_I18N("Enable/disable the auto exposure feature");
	s->opt[OPT_AE].cap |=
		(s->hw->info.can_autoexpose) ? 0 : SANE_CAP_INACTIVE;
	s->opt[OPT_AE].type = SANE_TYPE_BOOL;
	s->val[OPT_AE].w = FALSE;
	...
	s->opt[OPT_CALIBRATION_NOW].desc =
		SANE_I18N("Execute calibration *now*");
	...
	s->opt[OPT_SCANNER_SELF_DIAGNOSTIC].title =
		SANE_I18N("Self diagnosis");
	s->opt[OPT_SCANNER_SELF_DIAGNOSTIC].desc =
		SANE_I18N("Perform scanner " "self diagnosis");
	...
	s->opt[OPT_SCANNER_SELF_DIAGNOSTIC].cap |=
		(s->hw->info.can_calibrate) ? 0 : SANE_CAP_INACTIVE;
	s->opt[OPT_SCANNER_SELF_DIAGNOSTIC].constraint_type =
	...
	s->opt[OPT_EJECT_AFTERSCAN].title =
		SANE_I18N("Eject film after each scan");
	s->opt[OPT_EJECT_AFTERSCAN].desc =
		SANE_I18N("Automatically eject the "
			  "film from the device after each scan");
	s->opt[OPT_EJECT_AFTERSCAN].cap |=
		(s->hw->info.can_eject) ? 0 : SANE_CAP_INACTIVE;
	s->opt[OPT_EJECT_AFTERSCAN].type = SANE_TYPE_BOOL;
	s->val[OPT_EJECT_AFTERSCAN].w = FALSE;

	/* eject before exit */
	s->opt[OPT_EJECT_BEFOREEXIT].name = "eject-before-exit";
	s->opt[OPT_EJECT_BEFOREEXIT].title =
		SANE_I18N("Eject film before exit");
	...
	s->opt[OPT_TPU_PN].cap |=
		(s->hw->tpu.Status ==
		 TPU_STAT_ACTIVE) ? 0 : SANE_CAP_INACTIVE;
	...
	s->val[OPT_TPU_DCM].s =
		strdup(tpu_dc_mode_list[s->hw->tpu.ControlMode]);
	...
	s->opt[OPT_TPU_FILMTYPE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	...
	s->opt[OPT_TPU_FILMTYPE].cap |=
		(s->hw->tpu.Status == TPU_STAT_ACTIVE
		 && s->hw->tpu.ControlMode == 1) ? 0 : SANE_CAP_INACTIVE;
...
do_focus(CANON_Scanner * s)
{
	...
	status = execute_auto_focus(s->fd, s->val[OPT_AF].w,
				    (s->scanning_speed == 0 && !s->RIF
				     && s->hw->info.model == CS2700),
