			 ...
			 int pixels_per_line, int delay_count)
{
	...
	if (!delay->mem_block) {
		DBG(3,
		    "gt68xx_delay_buffer_init: no memory for delay block\n");
...
static inline void
unpack_8_rgb(SANE_Byte * src, unsigned int *dst, int pixels_per_line)
{
...
static inline void
unpack_16_le_rgb(SANE_Byte * src, unsigned int *dst, int pixels_per_line)
{
			...
			unsigned int **buffer_pointers_return)
{
	...
	for (i = reader->params.double_column; i < reader->pixels_per_line;
	     i += 2)
		  ...
		  unsigned int **buffer_pointers_return)
{
	...
	buffer = DELAY_BUFFER_READ_PTR(&reader->g_delay);
	buffer_pointers_return[0] = buffer;
	unpack_12_le_mono(reader->pixel_buffer, buffer,
			  reader->pixels_per_line);
		       ...
		       GT68xx_Line_Reader ** reader_return)
{
	...
	*reader_return = NULL;
	...
	if (!reader) {
		DBG(3,
		    "gt68xx_line_reader_new: cannot allocate GT68xx_Line_Reader\n");
		return SANE_STATUS_NO_MEM;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
		    "gt68xx_line_reader_new: cannot allocate line buffers: %s\n",
	...
	if (!reader->params.color) {
		...
		if (reader->params.depth == 8) {
			if (dev->model->line_mode_color_order ==
				...
				if (reader->params.ld_shift_double > 0)
					reader->read =
						line_read_rgb_double_8_line_mode;
				else
					reader->read =
						line_read_rgb_8_line_mode;
			} else if (dev->model->line_mode_color_order ==
				   COLOR_ORDER_BGR)
			...
			if (dev->model->line_mode_color_order ==
				...
				if (reader->params.ld_shift_double > 0)
					reader->read =
						line_read_rgb_double_12_line_mode;
				else
					reader->read =
						line_read_rgb_12_line_mode;
			} else if (dev->model->line_mode_color_order ==
				   COLOR_ORDER_BGR)
			...
			if (dev->model->line_mode_color_order ==
				...
				if (reader->params.ld_shift_double > 0)
					reader->read =
						line_read_rgb_double_16_line_mode;
				else
					reader->read =
						line_read_rgb_16_line_mode;
			} else if (dev->model->line_mode_color_order ==
				   COLOR_ORDER_BGR)
	...
	if (!reader->pixel_buffer) {
		DBG(3,
		    "gt68xx_line_reader_new: cannot allocate pixel buffer\n");
		gt68xx_line_reader_free_delays(reader);
		free(reader);
		return SANE_STATUS_NO_MEM;
	}
	...
	status = gt68xx_device_read_prepare(reader->dev, image_size,
					    final_scan);
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
