...
static void
DBG_inq_nz(char *text, int flag)
{
	if (flag != 0) {
		DBG(DBG_inquiry, "%s", text);
	}
}
...
static void
DBG_sense_nz(char *text, int flag)
{
	if (flag != 0) {
		DBG(DBG_sense, "%s", text);
	}
}
...
static void
umax_print_inquiry(Umax_Device * dev)
{
	...
	if (dev->inquiry_len <= 0x8e) {
		DBG(DBG_inquiry,
		    "Inquiry block is unexpected short, should be at least 147 bytes\n");
	}
	...
	if (get_inquiry_fw_adjust_exposure_tf(inquiry_block)) {
		...
		DBG(DBG_inquiry,
		    "exposure time step units......: %d micro-sec\n", unit);
		DBG(DBG_inquiry,
		    "exposure time maximum.........: %d micro-sec\n",
		    unit * get_inquiry_exposure_time_max(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time minimum (LHG)...: %d micro-sec\n",
		    unit * get_inquiry_exposure_time_lhg_min(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time minimum color...: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_color_min(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default FB (LH).: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_lh_def_fb(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default UTA (LH): %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_lh_def_uta(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default FB gray.: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_gray_def_fb(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default UTA gray: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_gray_def_uta(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default FB red..: %d micro-sec\n",
		    unit * get_inquiry_exposure_time_def_r_fb(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default FB grn..: %d micro-sec\n",
		    unit * get_inquiry_exposure_time_def_g_fb(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default FB blue.: %d micro-sec\n",
		    unit * get_inquiry_exposure_time_def_b_fb(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default UTA red.: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_def_r_uta(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default UTA grn.: %d micro-sec\n",
		    unit *
		    get_inquiry_exposure_time_def_g_uta(inquiry_block));
		DBG(DBG_inquiry,
		    "exposure time default UTA blue: %d micro-sec\n",
		    unit *
	...
	if (get_inquiry_hda(inquiry_block)) {
		...
		DBG(DBG_inquiry,
		    "halftone pattern download max matrix %dx%d\n",
	...
	DBG_inq_nz
		("color ordering support....................: line without CCD distance\n",
	...
	DBG_inq_nz
		("color ordering support....................: line with CCD distance\n",
	...
	DBG(DBG_inquiry,
	    "optical resolution........................: %d dpi\n",
	    dev->inquiry_optical_res);
	DBG(DBG_inquiry,
	    "maximum x-resolution......................: %d dpi\n",
	    dev->inquiry_x_res);
	DBG(DBG_inquiry,
	    "maximum y-resolution......................: %d dpi\n",
	    dev->inquiry_y_res);
	...
	DBG(DBG_inquiry,
	    "FB maximum scan width.....................: %2.2f inch\n",
	    dev->inquiry_fb_width);
	DBG(DBG_inquiry,
	    "FB maximum scan length....................: %2.2f inch\n",
	...
	DBG(DBG_inquiry,
	    "UTA x-original point......................: %2.2f inch\n",
	    dev->inquiry_uta_x_off);
	DBG(DBG_inquiry,
	    "UTA y-original point......................: %2.2f inch\n",
	    dev->inquiry_uta_y_off);
	DBG(DBG_inquiry,
	    "UTA maximum scan width....................: %2.2f inch\n",
	    dev->inquiry_uta_width);
	DBG(DBG_inquiry,
	    "UTA maximum scan length...................: %2.2f inch\n",
	...
	DBG(DBG_inquiry,
	    "DOR optical resolution....................: %d dpi\n",
	    dev->inquiry_dor_optical_res);
	DBG(DBG_inquiry,
	    "DOR maximum x-resolution..................: %d dpi\n",
	    dev->inquiry_dor_x_res);
	DBG(DBG_inquiry,
	    "DOR maximum y-resolution..................: %d dpi\n",
	...
	DBG(DBG_inquiry,
	    "DOR x-original point......................: %2.2f inch\n",
	    dev->inquiry_dor_x_off);
	DBG(DBG_inquiry,
	    "DOR y-original point......................: %2.2f inch\n",
	    dev->inquiry_dor_y_off);
	DBG(DBG_inquiry,
	    "DOR maximum scan width....................: %2.2f inch\n",
	    dev->inquiry_dor_width);
	DBG(DBG_inquiry,
	    "DOR maximum scan length...................: %2.2f inch\n",
	...
	DBG(DBG_inquiry,
	    "lamp warmup maximum time..................: %d sec\n",
	...
	DBG(DBG_inquiry,
	    "window descriptor block length............: %d bytes\n",
...
static int
umax_cbhs_correct(int minimum, int cbhs, int maximum)
{
...
{				/* is called by sanei_scsi */
	...
	if (get_RS_error_code(result) != 0x70) {
		...
		case 3:
			DBG(DBG_error,
			    "=> ignored, sense handler does continue\n");
		}
	}
	...
	case 0x04:		/* hardware error */
		if (asc_ascq == 0x4000) {
			...
			if (len >= 0x13) {
				...
				DBG_sense_nz
					("   uta home sensor or motor error\n",
...
static void
umax_set_rgb_bind(Umax_Scanner * scanner)
{
	if ((scanner->val[OPT_RGB_BIND].w == FALSE) && (strcmp(scanner->val[OPT_MODE].s, COLOR_STR) == 0)) {	/* enable rgb options */
		if (scanner->device->inquiry_analog_gamma) {
			scanner->opt[OPT_ANALOG_GAMMA].cap |=
				SANE_CAP_INACTIVE;
			scanner->opt[OPT_ANALOG_GAMMA_R].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_ANALOG_GAMMA_G].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_ANALOG_GAMMA_B].cap &=
				~SANE_CAP_INACTIVE;
		}
		if (scanner->device->inquiry_highlight) {
			scanner->opt[OPT_HIGHLIGHT].cap |= SANE_CAP_INACTIVE;
			scanner->opt[OPT_HIGHLIGHT_R].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_HIGHLIGHT_G].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_HIGHLIGHT_B].cap &=
				~SANE_CAP_INACTIVE;
		}
	...
	if ((scanner->device->inquiry_exposure_adj)
		...
		if ((scanner->val[OPT_RGB_BIND].w == FALSE) && (!scanner->device->exposure_time_rgb_bind) && (strcmp(scanner->val[OPT_MODE].s, COLOR_STR) == 0)) {	/* enable rgb exposure time options */
			if (scanner->val[OPT_SELECT_CAL_EXPOSURE_TIME].w) {	/* exposure time setting for calibration enabled */
				scanner->opt[OPT_CAL_EXPOS_TIME].cap |=
					SANE_CAP_INACTIVE;
				scanner->opt[OPT_CAL_EXPOS_TIME_R].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_CAL_EXPOS_TIME_G].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_CAL_EXPOS_TIME_B].cap &=
					~SANE_CAP_INACTIVE;
			...
			scanner->opt[OPT_SCAN_EXPOS_TIME].cap |=
				SANE_CAP_INACTIVE;
			scanner->opt[OPT_SCAN_EXPOS_TIME_R].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_SCAN_EXPOS_TIME_G].cap &=
				~SANE_CAP_INACTIVE;
			scanner->opt[OPT_SCAN_EXPOS_TIME_B].cap &=
				~SANE_CAP_INACTIVE;
...
static int
umax_calculate_pixels(int scansize_pixel, int resolution, int resolution_base,
		      int coordinate_base)
...
{
	unsigned int intsize_inch, intsize_pixel, diffsize_pixel,
		missing_pixels, del_pixel_1, pix;
	int toomuch;
...
static int
umax_forget_line(Umax_Device * dev, int color)
...
/* returns 0 if line is ok, -1 if line has to be skipped */
{
	...
	if (opt_res != dev->y_resolution) {	/* are there any lines to skip ? */
		...
		forget = (dev->pixelline_del[color] * opt_res) / (opt_res -
								  dev->
								  y_resolution);
...
static void
umax_order_line_to_pixel(Umax_Device * dev, unsigned char *source, int color)
...
static void
umax_order_line(Umax_Device * dev, unsigned char *source)
{
	unsigned int CCD_distance = dev->CCD_distance * dev->scale_y;
	unsigned int length =
		(dev->scanlength * dev->scale_y * dev->relevant_optical_res) /
	...
	do {			/* search next valid line */
		...
		} else if (dev->pixelline_opt_res <
			   length * 3 - CCD_distance * 3) {
...
umax_get_pixel_line(Umax_Device * dev)
{
	...
	if (dev->pixelbuffer != NULL) {
		if ((dev->pixelline_ready[0] > dev->pixelline_written) &&
			...
			source = dev->pixelbuffer +
				dev->pixelline_read * dev->width_in_pixels *
				3;
...
static SANE_Status
umax_scsi_cmd(Umax_Device * dev, const void *src, size_t src_size, void *dst,
	      size_t * dst_size)
{
...
#ifdef UMAX_ENABLE_USB
	case SANE_UMAX_USB:
		return sanei_umaxusb_cmd(dev->sfd, src, src_size, dst,
					 dst_size);
		break;
#endif
			...
			int *buffersize)
{
	switch (dev->connection_type) {
	case SANE_UMAX_SCSI:
		return sanei_scsi_open_extended(devicename, &dev->sfd,
						handler, handler_arg,
						buffersize);
		break;
...
#ifdef UMAX_ENABLE_USB
	case SANE_UMAX_USB:
		return sanei_umaxusb_open_extended(devicename, &dev->sfd,
						   handler, handler_arg,
						   buffersize);
		break;
#endif
	       ...
	       SANEI_SCSI_Sense_Handler handler, void *handler_arg)
{
	switch (dev->connection_type) {
	case SANE_UMAX_SCSI:
		return sanei_scsi_open(devicename, &dev->sfd, handler,
				       handler_arg);
		break;
...
static SANE_Status
umax_scsi_req_enter(Umax_Device * dev, const void *src, size_t src_size,
		    void *dst, size_t * dst_size, void **idp)
{
	switch (dev->connection_type) {
	case SANE_UMAX_SCSI:
		return sanei_scsi_req_enter(dev->sfd, src, src_size, dst,
					    dst_size, idp);
		break;
...
static SANE_Status
umax_scsi_get_lamp_status(Umax_Device * dev, int *lamp_on)
{
	SANE_Status status;
	size_t size = 1;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_scsi_get_lamp_status: command returned status %s\n",
...
static SANE_Status
umax_scsi_set_lamp_status(Umax_Device * dev, int lamp_on)
{
	SANE_Status status;
	...
	status = umax_scsi_cmd(dev, set_lamp_status.cmd, set_lamp_status.size,
			       NULL, NULL);

	if (status) {
		DBG(DBG_error,
		    "umax_scsi_set_lamp_status: command returned status %s\n",
		    sane_strstatus(status));
	}
...
static SANE_Status
umax_set_lamp_status(SANE_Handle handle, int lamp_on)
{
	...
	if (umax_scsi_open
	    (scanner->device->sane.name, scanner->device, sense_handler,
	     scanner->device) != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "ERROR: umax_set_lamp_status: open of %s failed:\n",
...
static SANE_Status
umax_get_data_buffer_status(Umax_Device * dev)
{
	SANE_Status status;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_get_data_buffer_status: command returned status %s\n",
		    sane_strstatus(status));
	}
...
static void
umax_do_request_sense(Umax_Device * dev)
{
	...
	status = umax_scsi_cmd(dev, request_sense.cmd, request_sense.size,
			       dev->buffer[0], &size);
	if (status) {
		DBG(DBG_error,
		    "umax_do_request_sense: command returned status %s\n",
		    sane_strstatus(status));
	}
}
...
static SANE_Status
umax_wait_scanner(Umax_Device * dev)
{
	SANE_Status status;
	int cnt = 0;
	...
	do {
		...
		/* test unit ready */
		status = umax_scsi_cmd(dev, test_unit_ready.cmd,
...
static int
umax_grab_scanner(Umax_Device * dev)
{
	int status;
	...
	status = umax_scsi_cmd(dev, reserve_unit.cmd, reserve_unit.size, NULL,
			       NULL);

	if (status) {
		DBG(DBG_error,
		    "umax_grab_scanner: command returned status %s\n",
		    sane_strstatus(status));
	} else {
...
static int
umax_reposition_scanner(Umax_Device * dev)
{
	int status;
	int pause;

	pause = dev->pause_after_reposition +
		dev->pause_for_moving * (dev->upper_left_y +
					 dev->scanlength) /
		((dev->inquiry_fb_length * dev->y_coordinate_base));

	DBG(DBG_info2, "trying to reposition scanner ...\n");
	status = umax_scsi_cmd(dev, object_position.cmd, object_position.size,
			       NULL, NULL);
	if (status) {
		DBG(DBG_error,
		    "umax_reposition_scanner: command returned status %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (pause > 0) {	/* predefined time to wait (Astra 2400S) */
		...
		DBG(DBG_info,
		    "not waiting for finishing reposition scanner\n");
	}
...
static int
umax_give_scanner(Umax_Device * dev)
{
	int status;

	DBG(DBG_info2, "trying to release scanner ...\n");
	status = umax_scsi_cmd(dev, release_unit.cmd, release_unit.size, NULL,
			       NULL);
	if (status) {
		DBG(DBG_error,
		    "umax_give_scanner: command returned status %s\n",
		    sane_strstatus(status));
	} else {
...
static void
umax_send_gamma_data(Umax_Device * dev, void *gamma_data, int color)
{
	...
	if (dev->inquiry_gamma_DCF == 0) {	/* gamma format type 0 */
		...
		if (color == 1) {	/* one color */
			...
			status = umax_scsi_cmd(dev, dev->buffer[0],
					       send.size + 1026, NULL, NULL);
			if (status) {
				DBG(DBG_error,
				    "umax_send_gamma_data(DCF=0, one color): command returned status %s\n",
				    sane_strstatus(status));
			}
		} else {	/* three colors */
			...
			status = umax_scsi_cmd(dev, dev->buffer[0],
					       send.size + 3076, NULL, NULL);
			if (status) {
				DBG(DBG_error,
				    "umax_send_gamma_data(DCF=0, RGB): command returned status %s\n",
				    sane_strstatus(status));
			}
		}
		...
		status = umax_scsi_cmd(dev, dev->buffer[0], send.size + 258,
				       NULL, NULL);
		if (status) {
			DBG(DBG_error,
			    "umax_send_gamma_data(DCF=1): command returned status %s\n",
			    sane_strstatus(status));
		}
		...
		if ((dev->colormode == RGB) && (dev->three_pass != 0)) {	/* 3 pass color */
			set_DCF2_gamma_color(dest, dev->three_pass_color);
		}
		/* set color */
		if (color == 1) {
		...
		if (dev->bufsize >= color * length + gamma_DCF2.size) {
			...
			status = umax_scsi_cmd(dev, dev->buffer[0],
					       send.size + gamma_DCF2.size +
					       length * color, NULL, NULL);
			if (status) {
				DBG(DBG_error,
				    "umax_send_gamma_data(DCF=2): command returned status %s\n",
				    sane_strstatus(status));
			}
		} else {
			DBG(DBG_error,
			    "ERROR: too small scsi buffer (%d bytes) to send gamma data\n",
			    dev->bufsize);
		}
	} else {
		DBG(DBG_error,
		    "ERROR: unknown gamma download curve type for this scanner\n");
	}
}
...
static void
umax_send_data(Umax_Device * dev, void *data, int size, int datatype)
{
	unsigned char *dest;
	SANE_Status status;
	...
	status = umax_scsi_cmd(dev, dev->buffer[0], send.size + size, NULL,
			       NULL);
	if (status) {
...
static void
umax_send_halftone_pattern(Umax_Device * dev, void *data, int size)
{
...
static void
umax_send_shading_data(Umax_Device * dev, void *data, int size)
{
...
#ifndef UMAX_HIDE_UNUSED
static void
umax_send_gain_data(Umax_Device * dev, void *data, int size)
{
...
static SANE_Status
umax_queue_read_image_data_req(Umax_Device * dev, unsigned int length,
			       int bufnr)
{
	SANE_Status status;

	DBG(DBG_proc,
	    "umax_queue_read_image_data_req for buffer[%d], length = %d\n",
	    bufnr, length);
	...
	status = umax_scsi_req_enter(dev, sread.cmd, sread.size,
				     dev->buffer[bufnr],
				     &(dev->length_read[bufnr]),
				     &(dev->queue_id[bufnr]));
	if (status) {
		DBG(DBG_error,
		    "umax_queue_read_image_data_req: command returned status %s\n",
		    sane_strstatus(status));
		return -1;
	} else {
		DBG(DBG_info2,
		    "umax_queue_read_image_data_req: id for buffer[%d] is %p\n",
...
static int
umax_wait_queued_image_data(Umax_Device * dev, int bufnr)
{
	SANE_Status status;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_wait_queued_image_data: wait returned status %s\n",
		    sane_strstatus(status));
		return -1;
	}
...
static int
umax_read_data(Umax_Device * dev, size_t length, int datatype)
{
	SANE_Status status;
	...
	status = umax_scsi_cmd(dev, sread.cmd, sread.size, dev->buffer[0],
			       &length);
	if (status) {
...
static int
umax_read_shading_data(Umax_Device * dev, unsigned int length)
{
...
#ifndef UMAX_HIDE_UNUSED
static int
umax_read_gain_data(Umax_Device * dev, unsigned int length)
{
...
#ifndef UMAX_HIDE_UNUSED
static int
umax_read_image_data(Umax_Device * dev, unsigned int length)
{
...
static int
umax_correct_light(int light, int analog_gamma_byte)
{				/* correct highlight/shadow if analog gamma is set */
	double analog_gamma;
	analog_gamma = analog_gamma_table[analog_gamma_byte];
	return ((int) 255 *
		pow(((double) light) / 255.0, (1.0 / analog_gamma)) + .5);
}
...
static SANE_Status
umax_set_window_param(Umax_Device * dev)
{
	SANE_Status status;
	int num_dblocks = 1;	/* number of window descriptor blocks, usually 1 or 3 */
	unsigned char buffer_r[max_WDB_size], buffer_g[max_WDB_size],
	...
	set_WD_highlight(buffer_r,
			 umax_correct_light(dev->highlight_r,
					    dev->analog_gamma_r));
	set_WD_shadow(buffer_r,
		      umax_correct_light(dev->shadow_r, dev->analog_gamma_r));
	...
	case RGB:
		...
		if (dev->three_pass == 0) {	/* singlepass */
			num_dblocks = 3;

			if (dev->do_color_ordering != 0) {
				set_WD_line_arrangement(buffer_r,
							WD_line_arrengement_by_driver);

				if (dev->CCD_distance == 0) {
					set_WD_color_ordering(buffer_r,
							      WD_color_ordering_line_no_ccd);
				} else {
					set_WD_color_ordering(buffer_r,
							      WD_color_ordering_line_w_ccd);
				}
			}
			...
			set_WD_calibration_exposure_level(buffer_g,
							  dev->
							  exposure_time_calibration_g);
			set_WD_calibration_exposure_level(buffer_b,
							  dev->
							  exposure_time_calibration_b);
		} else {	/* threepass */
			set_WD_wid(buffer_r, 0);	/* window identifier */
			...
			case WD_wid_red:
				...
				set_WD_analog_gamma(buffer_r,
						    dev->analog_gamma_r);
				set_WD_highlight(buffer_r,
						 umax_correct_light(dev->
								    highlight_r,
								    dev->
								    analog_gamma_r));
				set_WD_shadow(buffer_r,
				...
				set_WD_scan_exposure_level(buffer_r,
							   dev->
							   exposure_time_scan_r);
				set_WD_calibration_exposure_level(buffer_r,
								  dev->
								  exposure_time_calibration_r);
				break;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_set_window_param: command returned status %s\n",
		    sane_strstatus(status));
	} else {
...
static void
umax_do_inquiry(Umax_Device * dev)
{
	size_t size;
	SANE_Status status;
	...
	status = umax_scsi_cmd(dev, inquiry.cmd, inquiry.size, dev->buffer[0],
			       &size);
	if (status) {
		DBG(DBG_error,
		    "umax_do_inquiry: command returned status %s\n",
		    sane_strstatus(status));
	}
...
static SANE_Status
umax_start_scan(Umax_Device * dev)
{
	int size = 1;
	SANE_Status status;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_start_scan: command returned status %s\n",
		    sane_strstatus(status));
	}
...
static SANE_Status
umax_do_calibration(Umax_Device * dev)
{
	SANE_Status status;
	...
	if ((status == SANE_STATUS_GOOD) && (dev->do_calibration != 0)) {	/* calibration by driver */
		...
		if (get_RS_SCC_condition_code(dev->buffer[0]) != 1) {
			DBG(DBG_warning,
			    "WARNING: missing information about shading-data\n");
			DBG(DBG_warning,
			    "         driver tries to guess missing values!\n");

			if ((dev->calibration_area !=
			     UMAX_CALIBRATION_AREA_CCD) && (!dev->batch_scan))
				/* calibration is done with image geometry and depth */
			{
				DBG(DBG_warning,
				    "         Calibration is done with selected image geometry and depth!\n");

				width = dev->scanwidth *
					dev->relevant_optical_res /
					dev->x_coordinate_base;

				if (dev->calibration_width_offset > -99999) {	/* driver or user (umax.conf) define an offset */
					width = width +
						dev->calibration_width_offset;
					DBG(DBG_warning,
					    "         Using calibration width offset of %d\n",
					    dev->calibration_width_offset);
				}
				...
				DBG(DBG_warning,
				    "         Calibration is done for each CCD pixel with full depth!\n");

				width = (int) (dev->inquiry_fb_width *
					       dev->inquiry_optical_res);
				...
				if (dev->batch_scan) {
					if (dev->calibration_width_offset_batch > -99999) {	/* driver or user (umax.conf) define an offset for batch scanning */
						width = width +
							dev->
							calibration_width_offset_batch;
						DBG(DBG_warning,
						    "         Using calibration width offset for batch scanning of %d\n",
						    dev->
						    calibration_width_offset_batch);
					}
				} else {	/* normal scan */
			...
			bytespp =
				get_RS_SCC_calibration_bytespp(dev->
							       buffer[0]);
			width = get_RS_SCC_calibration_bytesperline(dev->
								    buffer[0])
				/ bytespp;
		}
		...
		DBG(DBG_info,
		    "scanner sends %d lines with %d pixels and %d bytes/pixel\n",
		    lines, width, bytespp);

		if (width * bytespp > dev->bufsize) {
			DBG(DBG_error,
			    "ERROR: scsi buffer is to small for one shading line, calibration aborted\n");
			DBG(DBG_error,
			    "=> change umax.conf options scsi-buffer-size-min and scsi-buffer-size-max\n");
		...
		if (average == 0) {
			DBG(DBG_error,
			    "ERROR: could not allocate memory for averaging shading data: calibration aborted\n");
			return SANE_STATUS_NO_MEM;
		}
		...
		if (shading_data == 0) {
			DBG(DBG_error,
			    "ERROR: could not allocate memory for shading data: calibration aborted\n");
			return SANE_STATUS_NO_MEM;
		}

		if (bytespp == 1) {	/* 1 byte per pixel */
			DBG(DBG_info,
			    "calculating average value for 8 bit shading data!\n");
			...
			for (j = 0; j < width; j++) {
				shading_data[j] =
					(unsigned char) (average[j] / lines);
			}
		} else if (dev->low_byte_first) {	/* 2 bytes per pixel with low byte first */
			DBG(DBG_info,
			    "calculating average value for 16 bit shading data (low byte first)!\n");
			for (i = 0; i < lines; i++) {
				umax_read_shading_data(dev, width * bytespp);

				for (j = 0; j < width; j++) {
					average[j] +=
						(long) 256 *dev->buffer[0][2 *
									   j +
									   1]
					+ dev->buffer[0][2 * j];
				}
			...
			DBG(DBG_info,
			    "calculating average value for 16 bit shading data (high byte first)!\n");
			for (i = 0; i < lines; i++) {
				umax_read_shading_data(dev, width * bytespp);
		...
		if ((dev->invert_shading_data)) {	/* invert data */
			if (bytespp == 1) {
				...
				DBG(DBG_info,
				    "inverting 16 bit shading data\n");

				for (j = 0; j < width; j++) {
					value = shading_data[2 * j] +
						shading_data[2 * j + 1] * 256;
					value = 65535 - value;
					shading_data[2 * j] =
...
static void
umax_do_new_inquiry(Umax_Device * dev, size_t size)
{				/* call inquiry again if wrong length */
	SANE_Status status;
	...
	if (status) {
		DBG(DBG_error,
		    "umax_do_new_inquiry: command returned status %s\n",
		    sane_strstatus(status));
	}
}
...
static void
umax_correct_inquiry(Umax_Device * dev, char *vendor, char *product,
		     char *version)
{
	...
	if (!strncmp(vendor, "UMAX ", 5)) {
		if (!strncmp(product, "Astra 600S ", 11)) {
			int add_len =
				get_inquiry_additional_length(dev->buffer[0]);

			DBG(DBG_warning,
			    "setting up special options for %s\n", product);

			if (add_len == 0x8f) {
				DBG(DBG_warning,
				    " - correcting wrong inquiry data\n");
				...
				set_inquiry_color_order(dev->buffer[0],
							IN_color_ordering_line_w_ccd);
				set_inquiry_fb_uta_line_arrangement_mode(dev->
									 buffer
									 [0],
									 32);
				set_inquiry_CCD_line_distance(dev->buffer[0],
							      8);
				/* we should reset ADF-bit here too */

				if (dev->invert_shading_data == -1) {	/* nothing defined in umax.conf */
					DBG(DBG_warning,
					    " - activating inversion of shading data\n");
			...
			if (dev->gamma_lsb_padded == -1) {	/* nothing defined in umax.conf and not by backend */
				DBG(DBG_warning,
				    " - 16 bit gamma table is created lsb padded\n");
				dev->gamma_lsb_padded = 1;
			}

			if (!strncmp(version, "V1.5 ", 4)) {
				DBG(DBG_warning,
				    " - lamp control enabled for version %s\n",
				    version);
			...
			if (dev->calibration_area == -1) {	/* no calibration area defined in umax.conf */
				DBG(DBG_warning,
				    " - calibration by driver is done for each CCD pixel\n");
			...
			if (dev->connection_type == SANE_UMAX_USB) {
				DBG(DBG_warning,
				    " - disabling quality calibration for USB connection\n");
			...
			DBG(DBG_warning,
			    "setting up special options for %s\n", product);
			DBG(DBG_warning,
			    " - reposition_scanner waits until move of scan head has finished\n");
			...
			DBG(DBG_warning,
			    "setting up special options for %s\n", product);
			DBG(DBG_warning,
			    " - setting gamma download curve format to type 1\n");
			...
			DBG(DBG_warning,
			    "setting up special options for %s\n", product);
			DBG(DBG_warning,
			    " - setting gamma download curve format to type 0\n");
			...
			DBG(DBG_warning,
			    " - setting maximum calibration data lines to 66\n");
			set_inquiry_max_calibration_data_lines(dev->buffer[0],
							       66);

			if (dev->calibration_width_offset == -99999) {	/* no calibration-width-offset defined in umax.conf */
				dev->calibration_width_offset = -1;
				DBG(DBG_warning,
				    " - adding calibration width offset of %d pixels\n",
				    dev->calibration_width_offset);
			}
			...
			if (dev->calibration_width_offset_batch == -99999) {	/* no calibration-width-offset for batch scanning defined in umax.conf */
				...
				DBG(DBG_warning,
				    " - adding calibration width offset for batch scanning of %d pixels\n",
			...
			DBG(DBG_warning,
			    " - always set quality calibration\n");
...
static int
umax_identify_scanner(Umax_Device * dev)
{
	char vendor[10];
	char product[0x12];
	char version[6];
	...
	umax_do_inquiry(dev);	/* get inquiry */
	if (get_inquiry_periph_devtype(dev->buffer[0]) !=
	    IN_periph_devtype_scanner) {
		return 1;
	}
	/* no scanner */
	get_inquiry_vendor((char *) dev->buffer[0], vendor);
	vendor[8] = ' ';
	vendor[9] = '\0';
	get_inquiry_product((char *) dev->buffer[0], product);
	product[16] = ' ';
	product[17] = '\0';
	get_inquiry_version((char *) dev->buffer[0], version);
	version[4] = ' ';
	version[5] = '\0';
	...
	if (get_inquiry_additional_length(dev->buffer[0]) >= 0x8f) {
		...
		while (strncmp("END_OF_LIST", scanner_str[2 * i], 11) != 0) {	/* Now identify full supported scanners */
			if (!strncmp
			    (vendor, scanner_str[2 * i],
			     strlen(scanner_str[2 * i]))) {
				if (!strncmp
				    (product, scanner_str[2 * i + 1],
				     strlen(scanner_str[2 * i + 1]))) {
					umax_correct_inquiry(dev, vendor,
							     product,
							     version);
					return 0;
				}
		...
		/* not UMAX then abort */
		DBG(DBG_error0,
		    "WARNING: %s scanner %s version %s on device %s\n"
		...
		return 0;
	} else /* inquiry-data not complete */ if (!strncmp(vendor, "UMAX ", 5)) {	/* test UMAX-scanners with short inquiry */
		...
		for (i = 0; i < known_inquiry; i++) {
			...
			if (!strncmp
			    (product, inq_data.scanner,
			     strlen(inq_data.scanner))) {
				DBG(DBG_warning, "inquiry-block-length: %d\n",
				    get_inquiry_additional_length(dev->
								  buffer[0]) +
				    5);
				DBG(DBG_warning,
				...
				set_inquiry_length(dev->buffer[0],
						   inq_data.inquiry_len);
				umax_correct_inquiry(dev, vendor, product,
						     version);
		...
		DBG(DBG_error0,
		    "ERROR: %s scanner %s version %s on device %s\n"
...
static void
umax_trim_rowbufsize(Umax_Device * dev)
{
	unsigned int lines = 0;
...
static void
umax_calculate_exposure_time(Umax_Device * dev, int def, int *value)
{
	int level;
	...
	if ((*value)) {
		if ((*value) == -1) {
			...
			level = (*value) /
				dev->inquiry_exposure_time_step_unit;
			(*value) =
				inrange(dev->use_exposure_time_min, level,
					dev->inquiry_exposure_time_max);
		}
	}
}
...
static int
umax_check_values(Umax_Device * dev)
{
	...
	if (dev->uta != 0) {
		...
		if ((dev->inquiry_uta == 0) || (dev->inquiry_transavail == 0)) {
			DBG(DBG_error,
			    "ERROR: transparency mode not supported by scanner\n");
			return (1);
		}
	}
	...
	if (dev->adf != 0) {
		if (dev->inquiry_adf == 0) {
			DBG(DBG_error,
			    "ERROR: adf mode not supported by scanner\n");
			return (1);
		}
	}
	...
	if (dev->dor != 0) {
		if (dev->inquiry_dor == 0) {
			DBG(DBG_error,
			    "ERROR: double optical resolution not supported by scanner\n");
			return (1);
		}
	}
	...
	if (dev->module == WD_module_flatbed) {	/* flatbed mode */
		...
		inquiry_length =
			dev->inquiry_uta_y_off + dev->inquiry_uta_length;
	}
	...
	if (dev->dor != 0) {
		...
		inquiry_length =
			dev->inquiry_dor_y_off + dev->inquiry_dor_length;
	}
	...
	maxwidth =
		inquiry_width * dev->x_coordinate_base - dev->upper_left_x -
		1;
	...
	maxlength =
		inquiry_length * dev->y_coordinate_base - dev->upper_left_y -
		1;
	...
	dev->width_in_pixels =
		umax_calculate_pixels(dev->scanwidth, dev->x_resolution,
				      dev->relevant_optical_res *
				      dev->scale_x, dev->x_coordinate_base);

	dev->length_in_pixels =
		umax_calculate_pixels(dev->scanlength, dev->y_resolution,
				      dev->relevant_optical_res *
				      dev->scale_y, dev->y_coordinate_base);
	...
	case RGB:
		if (dev->three_pass) {	/* three (24bpp) or six (30bpp) bytes per pixel */
			dev->row_len =
				dev->width_in_pixels * dev->bytes_per_color;
		} else {
			dev->row_len =
				dev->width_in_pixels * 3 *
				dev->bytes_per_color;
		}
		break;
	}
	...
	if (dev->wdb_len > used_WDB_size) {
		DBG(DBG_warning,
		    "WARNING:window descriptor block too long, will be shortned!\n");
	...
	dev->threshold =
		umax_cbhs_correct(dev->inquiry_threshold_min, dev->threshold,
				  dev->inquiry_threshold_max);
	dev->contrast =
		umax_cbhs_correct(dev->inquiry_contrast_min, dev->contrast,
				  dev->inquiry_contrast_max);
	dev->brightness =
		umax_cbhs_correct(dev->inquiry_brightness_min,
				  dev->brightness,
				  dev->inquiry_brightness_max);

	dev->highlight_r =
		umax_cbhs_correct(dev->inquiry_highlight_min,
				  dev->highlight_r,
	...
	dev->shadow_r =
		umax_cbhs_correct(dev->inquiry_shadow_min, dev->shadow_r,
				  dev->inquiry_shadow_max - 1);
	dev->shadow_g =
		umax_cbhs_correct(dev->inquiry_shadow_min, dev->shadow_g,
				  dev->inquiry_shadow_max - 1);
	dev->shadow_b =
		umax_cbhs_correct(dev->inquiry_shadow_min, dev->shadow_b,
				  dev->inquiry_shadow_max - 1);
	...
	if (dev->inquiry_preview == 0) {
		if (dev->preview) {
			DBG(DBG_warning,
			    "WARNING: fast preview function not supported by scanner\n");
			dev->preview = 0;
		}
	}
	...
	if (dev->force_quality_calibration) {
		...
		if (dev->quality) {
			DBG(DBG_warning,
			    "WARNING: quality calibration not supported by scanner\n");
			dev->quality = 0;
		}
	} else {
		if (dev->preview != 0) {
			DBG(DBG_info,
			    "quality calibration disabled in preview mode\n");
	...
	if (dev->inquiry_lamp_ctrl == 0) {
		if (dev->c_density || dev->s_density) {
			DBG(DBG_warning,
			    "WARNING: scanner doesn't support lamp intensity control\n");
		}
	...
	if (dev->reverse != 0) {
		if ((dev->colormode == LINEART)
		    || (dev->colormode == HALFTONE)
		    || (dev->colormode == RGB_LINEART)
		    || (dev->colormode == RGB_HALFTONE)) {
			if (dev->inquiry_reverse == 0) {
				DBG(DBG_error,
				    "ERROR: reverse for bi-level-image not supported\n");
				return (1);
			}
		} else {
			dev->reverse = 0;
	...
	if (dev->reverse_multi != 0) {
		if ((dev->colormode == RGB) || (dev->colormode == GRAYSCALE)) {
			if (dev->inquiry_reverse_multi == 0) {
				DBG(DBG_error,
				    "ERROR: reverse for multi-level-image not supported\n");
				return (1);
			}
		} else {
			dev->reverse_multi = 0;
	...
	if (dev->inquiry_analog_gamma == 0) {
		if (dev->analog_gamma_r + dev->analog_gamma_g +
		    dev->analog_gamma_b != 0) {
			DBG(DBG_warning,
			    "WARNING: analog gamma correction not supported by scanner!\n");
		}
		dev->analog_gamma_r = dev->analog_gamma_g =
			dev->analog_gamma_b = 0;
	}
	...
	if ((dev->digital_gamma_r == 0) || (dev->digital_gamma_g == 0) ||
		...
		if (dev->inquiry_gamma_dwload == 0) {
			DBG(DBG_warning,
			    "WARNING: gamma download not available\n");
			dev->digital_gamma_r = dev->digital_gamma_g =
				dev->digital_gamma_b = 15;
		}
	}
	...
	if (((dev->inquiry_GIB | 1) & dev->gamma_input_bits_code) == 0) {
		DBG(DBG_warning,
		    "WARNING: selected gamma input bits not supported, gamma ignored\n");
	...
	if (((dev->inquiry_GOB | 1) & dev->bits_per_pixel_code) == 0) {
		DBG(DBG_error,
		    "ERROR: selected bits per pixel not supported\n");
		return (1);
	}
	...
	case RGB_LINEART:	/* ---------- RGB_LINEART ----------- */
		...
		if (dev->module == WD_module_flatbed) {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_l_fb_def;
		} else {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_l_uta_def;
		}

		if (dev->inquiry_lineart == 0) {
			DBG(DBG_error,
			    "ERROR: lineart mode not supported by scanner\n");
			return (1);
		}
		break;
	...
	case RGB_HALFTONE:	/* --------- RGB_HALFTONE---------- */
		...
		if (dev->module == WD_module_flatbed) {
			...
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_h_uta_def;
		}

		if (dev->inquiry_halftone == 0) {
			DBG(DBG_error,
			    "ERROR: halftone mode not supported by scanner\n");
			return (1);
		}
		break;
	...
	case GRAYSCALE:	/* ---------- GRAYSCALE ------------- */
		...
		if (dev->module == WD_module_flatbed) {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_g_fb_def;
		} else {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_g_uta_def;
		}

		if (dev->inquiry_gray == 0) {
			DBG(DBG_error,
			    "ERROR: grayscale mode not supported by scanner\n");
			return (1);
		}
		break;
	...
	case RGB:		/* ----------------- COLOR ---------- */
		...
		if (dev->module == WD_module_flatbed) {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_c_fb_def_r;
			dev->use_exposure_time_def_g =
				dev->inquiry_exposure_time_c_fb_def_g;
			dev->use_exposure_time_def_b =
				dev->inquiry_exposure_time_c_fb_def_b;
		} else {
			dev->use_exposure_time_def_r =
				dev->inquiry_exposure_time_c_uta_def_r;
			dev->use_exposure_time_def_g =
		...
		if (dev->inquiry_color == 0) {
			DBG(DBG_error,
			    "ERROR: color mode not supported by scanner\n");
			return (1);
		}
		...
		if (dev->inquiry_one_pass_color) {
			...
			if (dev->
			    inquiry_color_order & IN_color_ordering_pixel) {
				DBG(DBG_info,
				    "scanner uses color-pixel-ordering\n");
			} else if (dev->
				   inquiry_color_order &
				   IN_color_ordering_line_no_ccd) {
				dev->CCD_distance = 0;
				dev->do_color_ordering = 1;
				DBG(DBG_info,
				    "scanner uses color-line-ordering without CCD-distance\n");
			} else if (dev->
				   inquiry_color_order &
				   IN_color_ordering_line_w_ccd) {
				dev->CCD_distance =
					dev->inquiry_CCD_line_distance;
				dev->do_color_ordering = 1;
				...
				DBG(DBG_info,
				    "scanner uses color-line-ordering with CCD-distance of %d lines\n",
				    dev->CCD_distance);
			} else {
				DBG(DBG_error,
				    "ERROR: color-ordering-type not supported \n");
				return (1);
			}
		} else {
...
static void
umax_get_inquiry_values(Umax_Device * dev)
{
	unsigned char *inquiry_block;
	...
	get_inquiry_vendor((char *) inquiry_block, dev->vendor);
	dev->vendor[8] = '\0';
	get_inquiry_product((char *) inquiry_block, dev->product);
	dev->product[16] = '\0';
	get_inquiry_version((char *) inquiry_block, dev->version);
	dev->version[4] = '\0';
	...
	if (dev->inquiry_len <= 0x8f) {
		DBG(DBG_warning,
		    "WARNING: inquiry return block is unexpected short.\n");
	}

	dev->inquiry_uta = get_inquiry_sc_uta(inquiry_block);
	dev->inquiry_adf = get_inquiry_sc_adf(inquiry_block);

	dev->inquiry_one_pass_color =
		get_inquiry_sc_one_pass_color(inquiry_block);
	dev->inquiry_three_pass_color =
		get_inquiry_sc_three_pass_color(inquiry_block);
	...
	dev->inquiry_exposure_adj =
		get_inquiry_fw_adjust_exposure_tf(inquiry_block);
	dev->inquiry_exposure_time_step_unit =
		get_inquiry_exposure_time_step_unit(inquiry_block);
	dev->inquiry_exposure_time_max =
		get_inquiry_exposure_time_max(inquiry_block);

	/* --- lineart --- */
	dev->inquiry_exposure_time_l_min =
	...
	/* --- halftone --- */
	dev->inquiry_exposure_time_h_min =
		get_inquiry_exposure_time_lhg_min(inquiry_block);
	dev->inquiry_exposure_time_h_fb_def =
		get_inquiry_exposure_time_lh_def_fb(inquiry_block);
	dev->inquiry_exposure_time_h_uta_def =
	...
	/* --- grayscale --- */
	dev->inquiry_exposure_time_g_min =
		get_inquiry_exposure_time_lhg_min(inquiry_block);
	dev->inquiry_exposure_time_g_fb_def =
		get_inquiry_exposure_time_gray_def_fb(inquiry_block);
	dev->inquiry_exposure_time_g_uta_def =
	...
	/* --- color --- */
	dev->inquiry_exposure_time_c_min =
		get_inquiry_exposure_time_color_min(inquiry_block);
	dev->inquiry_exposure_time_c_fb_def_r =
	...
	dev->inquiry_reverse_multi =
		get_inquiry_sc_multi_image_reverse(inquiry_block);
	dev->inquiry_shadow = 1 - get_inquiry_sc_no_shadow(inquiry_block);
	dev->inquiry_highlight =
		1 - get_inquiry_sc_no_highlight(inquiry_block);
	...
	dev->inquiry_lens_cal_in_doc_pos =
		get_inquiry_manual_focus(inquiry_block);
	dev->inquiry_manual_focus = get_inquiry_manual_focus(inquiry_block);
	dev->inquiry_sel_uta_lens_cal_pos =
		get_inquiry_manual_focus(inquiry_block);

	dev->inquiry_gamma_dwload =
		get_inquiry_gamma_download_available(inquiry_block);
	...
	dev->inquiry_optical_res =
		100 * get_inquiry_max_opt_res(inquiry_block);
	...
	if (dev->inquiry_len > 0x95) {
		dev->inquiry_x_res +=
			get_inquiry_x_resolution_residue(inquiry_block);
	};
	...
	if (dev->inquiry_len > 0x96) {
		dev->inquiry_y_res +=
			get_inquiry_y_resolution_residue(inquiry_block);
	}


	/* optical resolution = [0x83] * 100 + [0xa0] , 0xa0 is not always defined */
	dev->inquiry_dor_optical_res =
		100 * get_inquiry_dor_max_opt_res(inquiry_block);
	if (dev->inquiry_len > 0xa0) {
		dev->inquiry_dor_optical_res +=
			get_inquiry_dor_optical_resolution_residue
			(inquiry_block);
	}

	/* x resolution = [0x84] * 100 + [0xa1] , 0xa1 is not always defined */
	dev->inquiry_dor_x_res =
		100 * get_inquiry_dor_max_x_res(inquiry_block);
	if (dev->inquiry_len > 0xa1) {
		dev->inquiry_dor_x_res +=
			get_inquiry_dor_x_resolution_residue(inquiry_block);
	}

	/* y resolution = [0x85] * 100 + [0xa2] , 0xa2 is not always defined */
	dev->inquiry_dor_y_res =
		100 * get_inquiry_dor_max_y_res(inquiry_block);
	if (dev->inquiry_len > 0xa2) {
		dev->inquiry_dor_y_res +=
			get_inquiry_dor_y_resolution_residue(inquiry_block);
	}
	...
	dev->inquiry_uta_length =
		(double) get_inquiry_uta_max_scan_length(inquiry_block) *
		0.01;
	dev->inquiry_uta_x_off =
		(double) get_inquiry_uta_x_original_point(inquiry_block) *
		0.01;
	dev->inquiry_uta_y_off =
		(double) get_inquiry_uta_y_original_point(inquiry_block) *
		0.01;

	dev->inquiry_dor_width =
		(double) get_inquiry_dor_max_scan_width(inquiry_block) * 0.01;
	dev->inquiry_dor_length =
		(double) get_inquiry_dor_max_scan_length(inquiry_block) *
		0.01;
	dev->inquiry_dor_x_off =
		(double) get_inquiry_dor_x_original_point(inquiry_block) *
		0.01;
	dev->inquiry_dor_y_off =
		(double) get_inquiry_dor_y_original_point(inquiry_block) *
		0.01;
	...
	dev->inquiry_fb_uta_color_arrangement =
		get_inquiry_fb_uta_line_arrangement_mode(inquiry_block);
	...
	dev->inquiry_adf_color_arrangement =
		get_inquiry_adf_line_arrangement_mode(inquiry_block);
...
static int
umax_calculate_analog_gamma(double value)
{
	int gamma;
	...
	if (gamma) {
		if ((analog_gamma_table[gamma - 1] +
		     analog_gamma_table[gamma]) / 2 > value) {
			gamma--;
		}
	}
...
static void
umax_output_image_data(Umax_Device * dev, FILE * fp,
		       unsigned int data_to_read, int bufnr)
{
	if (dev->do_color_ordering == 0) {	/* pixel ordering */
		...
		for (i = 0; i < lines; i++) {
			...
			if (pixelsource != NULL) {
				fwrite(pixelsource, bytes,
				       dev->width_in_pixels * 3, fp);
			}
		}
	}
}
...
static int
umax_reader_process(Umax_Device * dev, FILE * fp, unsigned int image_size)
{
	int status;
	int bytes = 1;
	int queue_filled = 0;
	...
	if (dev->do_color_ordering != 0) {
		...
		dev->pixelbuffer =
			malloc(dev->width_in_pixels * dev->pixelline_max *
			       bytes * 3);
	...
	do {
		if (data_left_to_queue) {
			data_to_queue =
				(data_left_to_queue <
				 dev->
				 row_bufsize) ? data_left_to_queue : dev->
				row_bufsize;

			/* umax_get_data_buffer_status(dev); */

			status = umax_queue_read_image_data_req(dev,
								data_to_queue,
								bufnr_queue);
			...
			if (status == -1) {	/* error */
				DBG(DBG_error,
				    "ERROR: umax_reader_process: unable to queue read image data request!\n");
				free(dev->pixelbuffer);
				dev->pixelbuffer = NULL;
				return (-1);
			}
			...
			DBG(DBG_read,
			    "umax_reader_process: read image data queued for buffer[%d] \n",
			    bufnr_queue);
		...
		if (queue_filled) {	/* queue filled, ok we can read data */
			...
			umax_output_image_data(dev, fp, data_to_read,
					       bufnr_read);

			data_left_to_read -= data_to_read;
			DBG(DBG_read,
			    "umax_reader_process: buffer of %d bytes read; %d bytes to go\n",
			...
			if (dev->length_read[bufnr_read] !=
			    dev->length_queued[bufnr_read]) {
				data_left_to_queue +=
					dev->length_queued[bufnr_read] -
					dev->length_read[bufnr_read];
			}
...
static void
umax_initialize_values(Umax_Device * dev)
{				/* called each time before setting scan-values *//* Initialize dev structure */
...
static void
umax_init(Umax_Device * dev)
{				/* umax_init is called once while driver-initialization */
	DBG(DBG_proc, "init\n");
	...
	if (dev->connection_type == SANE_UMAX_SCSI) {
		...
		DBG(DBG_info2,
		    "setting request_scsi_maxqueue = 1 for USB connection\n");
	...
	dev->calibration_width_offset_batch =
		umax_calibration_width_offset_batch;
	...
	if (umax_test_little_endian() == TRUE) {
...
static SANE_Status
do_cancel(Umax_Scanner * scanner)
{
	int pid, status;
	       ...
	       int connection_type)
{
	...
	if (dev->connection_type != SANE_UMAX_SCSI) {
		...
		if (sanei_umaxusb_open
		    (devicename, &dev->sfd, sense_handler,
		     dev) == SANE_STATUS_GOOD) {
			...
			if (dev->connection_type == SANE_UMAX_USB) {	/* we know it is not a scsi device: error */
				DBG(DBG_error,
				    "ERROR: attach_scanner: opening usb device %s failed\n",
				    devicename);
				free(dev);
				return SANE_STATUS_INVAL;
			}
			...
			DBG(DBG_info,
			    "attach_scanner: failed to open %s as usb device\n",
			    devicename);
		}
	}
#else
	...
	if (dev->connection_type != SANE_UMAX_USB) {	/* not an USB device, then try as SCSI */
		...
		if (sanei_scsi_open_extended
		    (devicename, &dev->sfd, sense_handler, dev,
		     (int *) &dev->bufsize) != 0) {
			DBG(DBG_error,
			    "ERROR: attach_scanner: opening scsi device %s failed\n",
			    devicename);
			free(dev);
			return SANE_STATUS_INVAL;
		}
		...
		if (dev->bufsize < 4096) {	/* < 4KB */
			DBG(DBG_error,
			    "ERROR: attach_scanner: sanei_scsi_open_extended returned too small scsi buffer\n");
			umax_scsi_close(dev);
			free(dev);
			return SANE_STATUS_NO_MEM;
		}
	...
	if (!dev->buffer[0]) {	/* malloc failed */
		DBG(DBG_error,
		    "ERROR: attach scanner: could not allocate buffer[0]\n");
		umax_scsi_close(dev);
		free(dev);
		return SANE_STATUS_NO_MEM;
	}
...
{				/* executed as a child process or as thread */
	...
	scanner->device->scsi_maxqueue =
		scanner->device->request_scsi_maxqueue;

	if (scanner->device->request_scsi_maxqueue > 1) {
		for (i = 1; i < SANE_UMAX_SCSI_MAXQUEUE; i++) {
			if (scanner->device->buffer[i]) {
				DBG(DBG_info,
				    "reader_process: freeing SCSI buffer[%d]\n",
				    i);
...
*/
	...
	scanner->opt[OPT_SOURCE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	scanner->opt[OPT_SOURCE].constraint.string_list = source_list;
	scanner->val[OPT_SOURCE].s = (char *) strdup(source_list[0]);
	...
	scanner->opt[OPT_X_RESOLUTION].constraint_type =
	...
	scanner->opt[OPT_Y_RESOLUTION].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->val[OPT_RESOLUTION_BIND].w = TRUE;
	...
	scanner->val[OPT_NEGATIVE].w = FALSE;
	...
	scanner->opt[OPT_GEOMETRY_GROUP].constraint_type =
	...
	scanner->opt[OPT_ENHANCEMENT_GROUP].constraint_type =
		SANE_CONSTRAINT_NONE;
	...
	scanner->opt[OPT_BIT_DEPTH].constraint_type =
	...
	scanner->val[OPT_WARMUP].w = FALSE;
	...
	scanner->val[OPT_RGB_BIND].w = FALSE;
	...
	scanner->opt[OPT_HIGHLIGHT_R].constraint.range =
		&percentage_range_100;
	...
	scanner->opt[OPT_HIGHLIGHT_G].constraint.range =
		&percentage_range_100;
	...
	scanner->opt[OPT_HIGHLIGHT_B].constraint.range =
		&percentage_range_100;
	...
	scanner->opt[OPT_ANALOG_GAMMA].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->opt[OPT_ANALOG_GAMMA_R].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->opt[OPT_ANALOG_GAMMA_G].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->opt[OPT_ANALOG_GAMMA_B].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->val[OPT_CUSTOM_GAMMA].w = TRUE;
	...
	scanner->opt[OPT_GAMMA_VECTOR].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->val[OPT_GAMMA_VECTOR].wa = scanner->gamma_table[0];
	scanner->opt[OPT_GAMMA_VECTOR].constraint.range =
		&scanner->output_range;
	scanner->opt[OPT_GAMMA_VECTOR].size =
		scanner->gamma_length * sizeof(int);
	...
	scanner->opt[OPT_GAMMA_VECTOR_R].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->val[OPT_GAMMA_VECTOR_R].wa = scanner->gamma_table[1];
	scanner->opt[OPT_GAMMA_VECTOR_R].constraint.range =
		&(scanner->gamma_range);
	...
	scanner->opt[OPT_GAMMA_VECTOR_G].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->val[OPT_GAMMA_VECTOR_G].wa = scanner->gamma_table[2];
	scanner->opt[OPT_GAMMA_VECTOR_G].constraint.range =
		&(scanner->gamma_range);
	...
	scanner->opt[OPT_GAMMA_VECTOR_B].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->val[OPT_GAMMA_VECTOR_B].wa = scanner->gamma_table[3];
	scanner->opt[OPT_GAMMA_VECTOR_B].constraint.range =
		&(scanner->gamma_range);
	scanner->opt[OPT_GAMMA_VECTOR_B].size =
		scanner->gamma_length * sizeof(int);

	/* halftone dimension */
	scanner->opt[OPT_HALFTONE_DIMENSION].name =
		SANE_NAME_HALFTONE_DIMENSION;
	scanner->opt[OPT_HALFTONE_DIMENSION].title =
		SANE_TITLE_HALFTONE_DIMENSION;
	scanner->opt[OPT_HALFTONE_DIMENSION].desc =
	...
	scanner->opt[OPT_ADVANCED_GROUP].constraint_type =
		SANE_CONSTRAINT_NONE;
	...
	scanner->opt[OPT_SELECT_EXPOSURE_TIME].name =
		SANE_NAME_SELECT_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_EXPOSURE_TIME].title =
		SANE_TITLE_SELECT_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_EXPOSURE_TIME].desc =
		SANE_DESC_SELECT_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_EXPOSURE_TIME].type = SANE_TYPE_BOOL;
	scanner->val[OPT_SELECT_EXPOSURE_TIME].w = FALSE;

	/* select calibration exposure time */
	scanner->opt[OPT_SELECT_CAL_EXPOSURE_TIME].name =
		SANE_UMAX_NAME_SELECT_CALIBRATON_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_CAL_EXPOSURE_TIME].title =
		SANE_UMAX_TITLE_SELECT_CALIBRATION_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_CAL_EXPOSURE_TIME].desc =
		SANE_UMAX_DESC_SELECT_CALIBRATION_EXPOSURE_TIME;
	scanner->opt[OPT_SELECT_CAL_EXPOSURE_TIME].type = SANE_TYPE_BOOL;
	scanner->val[OPT_SELECT_CAL_EXPOSURE_TIME].w = FALSE;
	...
	scanner->opt[OPT_CAL_EXPOS_TIME].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_CAL_EXPOS_TIME].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_CAL_EXPOS_TIME].w =
		scanner->device->inquiry_exposure_time_g_fb_def *
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_R].title =
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_R].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_CAL_EXPOS_TIME_R].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_CAL_EXPOS_TIME_R].w =
		scanner->device->inquiry_exposure_time_c_fb_def_r *
		scanner->device->inquiry_exposure_time_step_unit;
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_G].title =
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_G].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_CAL_EXPOS_TIME_G].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_CAL_EXPOS_TIME_G].w =
		scanner->device->inquiry_exposure_time_c_fb_def_g *
		scanner->device->inquiry_exposure_time_step_unit;
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_B].title =
	...
	scanner->opt[OPT_CAL_EXPOS_TIME_B].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_CAL_EXPOS_TIME_B].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_CAL_EXPOS_TIME_B].w =
		scanner->device->inquiry_exposure_time_c_fb_def_b *
		scanner->device->inquiry_exposure_time_step_unit;
	...
	scanner->opt[OPT_SCAN_EXPOS_TIME].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_SCAN_EXPOS_TIME].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_SCAN_EXPOS_TIME].w =
		scanner->device->inquiry_exposure_time_g_fb_def *
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->opt[OPT_SCAN_EXPOS_TIME].cap |= SANE_CAP_INACTIVE;

	/* scan exposure time red */
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].name =
		SANE_NAME_SCAN_EXPOS_TIME_R;
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].title =
		SANE_TITLE_SCAN_EXPOS_TIME_R;
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].desc =
	...
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_SCAN_EXPOS_TIME_R].w =
		scanner->device->inquiry_exposure_time_c_fb_def_r *
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->opt[OPT_SCAN_EXPOS_TIME_R].cap |= SANE_CAP_INACTIVE;

	/* scan exposure time green */
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].name =
		SANE_NAME_SCAN_EXPOS_TIME_G;
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].title =
		SANE_TITLE_SCAN_EXPOS_TIME_G;
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].desc =
	...
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_SCAN_EXPOS_TIME_G].w =
		scanner->device->inquiry_exposure_time_c_fb_def_g *
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->opt[OPT_SCAN_EXPOS_TIME_G].cap |= SANE_CAP_INACTIVE;

	/* scan exposure time blue */
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].name =
		SANE_NAME_SCAN_EXPOS_TIME_B;
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].title =
		SANE_TITLE_SCAN_EXPOS_TIME_B;
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].desc =
	...
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].constraint.range =
		&(scanner->exposure_time_range);
	scanner->val[OPT_SCAN_EXPOS_TIME_B].w =
		scanner->device->inquiry_exposure_time_c_fb_def_b *
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->opt[OPT_SCAN_EXPOS_TIME_B].cap |= SANE_CAP_INACTIVE;

	if (scanner->device->inquiry_exposure_adj == 0) {
		scanner->opt[OPT_SELECT_EXPOSURE_TIME].cap |=
			SANE_CAP_INACTIVE;
	}
	...
	scanner->opt[OPT_SELECT_LAMP_DENSITY].name =
		SANE_NAME_SELECT_LAMP_DENSITY;
	scanner->opt[OPT_SELECT_LAMP_DENSITY].title =
		SANE_TITLE_SELECT_LAMP_DENSITY;
	scanner->opt[OPT_SELECT_LAMP_DENSITY].desc =
		SANE_DESC_SELECT_LAMP_DENSITY;
	scanner->opt[OPT_SELECT_LAMP_DENSITY].type = SANE_TYPE_BOOL;
	scanner->val[OPT_SELECT_LAMP_DENSITY].w = FALSE;
	...
	scanner->opt[OPT_CAL_LAMP_DEN].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_CAL_LAMP_DEN].constraint.range =
		&percentage_range_100;
	...
	scanner->opt[OPT_SCAN_LAMP_DEN].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_SCAN_LAMP_DEN].constraint.range =
		&percentage_range_100;
	...
	if (scanner->device->inquiry_lamp_ctrl == 0) {
		scanner->opt[OPT_SELECT_LAMP_DENSITY].cap |=
			SANE_CAP_INACTIVE;
	}
	...
	scanner->opt[OPT_DISABLE_PRE_FOCUS].title =
		SANE_I18N("Disable pre focus");
	scanner->opt[OPT_DISABLE_PRE_FOCUS].desc =
		SANE_I18N("Do not calibrate focus");
	scanner->opt[OPT_DISABLE_PRE_FOCUS].type = SANE_TYPE_BOOL;
	scanner->val[OPT_DISABLE_PRE_FOCUS].w = FALSE;
	...
	scanner->opt[OPT_MANUAL_PRE_FOCUS].title =
		SANE_I18N("Manual pre focus");
	scanner->opt[OPT_MANUAL_PRE_FOCUS].desc = "";
	scanner->opt[OPT_MANUAL_PRE_FOCUS].type = SANE_TYPE_BOOL;
	scanner->val[OPT_MANUAL_PRE_FOCUS].w = FALSE;
	...
	scanner->opt[OPT_FIX_FOCUS_POSITION].title =
		SANE_I18N("Fix focus position");
	scanner->opt[OPT_FIX_FOCUS_POSITION].desc = "";
	scanner->opt[OPT_FIX_FOCUS_POSITION].type = SANE_TYPE_BOOL;
	scanner->val[OPT_FIX_FOCUS_POSITION].w = FALSE;
	...
	scanner->opt[OPT_LENS_CALIBRATION_DOC_POS].name =
		"lens-calibration-in-doc-position";
	scanner->opt[OPT_LENS_CALIBRATION_DOC_POS].title =
		SANE_I18N("Lens calibration in doc position");
	scanner->opt[OPT_LENS_CALIBRATION_DOC_POS].desc =
		SANE_I18N("Calibrate lens focus in document position");
	scanner->opt[OPT_LENS_CALIBRATION_DOC_POS].type = SANE_TYPE_BOOL;
	scanner->val[OPT_LENS_CALIBRATION_DOC_POS].w = FALSE;

	if (scanner->device->inquiry_lens_cal_in_doc_pos == 0) {
		scanner->opt[OPT_LENS_CALIBRATION_DOC_POS].cap |=
			SANE_CAP_INACTIVE;
	}

	/* 0mm holder focus position */
	scanner->opt[OPT_HOLDER_FOCUS_POS_0MM].name =
		"holder-focus-position-0mm";
	scanner->opt[OPT_HOLDER_FOCUS_POS_0MM].title =
		SANE_I18N("Holder focus position 0mm");
	scanner->opt[OPT_HOLDER_FOCUS_POS_0MM].desc =
		SANE_I18N("Use 0mm holder focus position instead of 0.6mm");
	scanner->opt[OPT_HOLDER_FOCUS_POS_0MM].type = SANE_TYPE_BOOL;
	scanner->val[OPT_HOLDER_FOCUS_POS_0MM].w = FALSE;

	if (scanner->device->inquiry_sel_uta_lens_cal_pos == 0) {
		scanner->opt[OPT_HOLDER_FOCUS_POS_0MM].cap |=
			SANE_CAP_INACTIVE;
	}
	...
	scanner->opt[OPT_LAMP_OFF_AT_EXIT].title =
		SANE_I18N("Lamp off at exit");
	scanner->opt[OPT_LAMP_OFF_AT_EXIT].desc =
		SANE_I18N("Turn off lamp when program exits");
	scanner->opt[OPT_LAMP_OFF_AT_EXIT].type = SANE_TYPE_BOOL;
	scanner->val[OPT_LAMP_OFF_AT_EXIT].w = FALSE;
	...
	scanner->val[OPT_BATCH_SCAN_LOOP].w = FALSE;
	...
	scanner->val[OPT_BATCH_SCAN_END].w = FALSE;
	...
	scanner->opt[OPT_BATCH_NEXT_TL_Y].constraint_type =
		SANE_CONSTRAINT_RANGE;
	scanner->opt[OPT_BATCH_NEXT_TL_Y].constraint.range =
	...
	scanner->opt[OPT_CALIB_MODE].desc =
		SANE_I18N("Define calibration mode");
	scanner->opt[OPT_CALIB_MODE].type = SANE_TYPE_STRING;
	scanner->opt[OPT_CALIB_MODE].size = max_string_size(calibration_list);
	scanner->opt[OPT_CALIB_MODE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	scanner->opt[OPT_CALIB_MODE].constraint.string_list =
		calibration_list;
	scanner->val[OPT_CALIB_MODE].s =
		(char *) strdup(calibration_list[0]);
...
static SANE_Status
attach_one_scsi(const char *name)
{
...
static SANE_Status
attach_one_usb(const char *name)
{
...
static SANE_Status
umax_test_configure_option(const char *option_str, char *test_name,
			   int *test_value, int test_min, int test_max)
/* returns with 1 if option was found, 0 if option was not found */
{
	...
	if (strncmp(option_str, test_name, strlen(test_name)) == 0) {
		value_str =
			sanei_config_skip_whitespace(option_str +
						     strlen(test_name));
		...
		if (end_ptr == value_str || errno) {
			DBG(DBG_error,
			    "ERROR: invalid value \"%s\" for option %s in %s\n",
			    value_str, test_name, UMAX_CONFIG_FILE);
		} else {
			if (value < test_min) {
				DBG(DBG_error,
				    "ERROR: value \"%d\" is too small for option %s in %s\n",
				    value, test_name, UMAX_CONFIG_FILE);
				value = test_min;
			} else if (value > test_max) {
				DBG(DBG_error,
				    "ERROR: value \"%d\" is too large for option %s in %s\n",
				    value, test_name, UMAX_CONFIG_FILE);
				value = test_max;
			}
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	while (sanei_config_read(config_line, sizeof(config_line), fp)) {
		...
		if (strncmp(config_line, "option", 6) == 0) {
			option_str =
				sanei_config_skip_whitespace(config_line + 6);

			if (umax_test_configure_option
			    (option_str, "scsi-maxqueue", &umax_scsi_maxqueue,
			     1, SANE_UMAX_SCSI_MAXQUEUE));
			else if (umax_test_configure_option
				 (option_str, "scsi-buffer-size-min",
				  &umax_scsi_buffer_size_min, 4096, 1048576));
			else if (umax_test_configure_option
				 (option_str, "scsi-buffer-size-max",
				  &umax_scsi_buffer_size_max, 4096, 1048576));
			else if (umax_test_configure_option
				 (option_str, "preview-lines",
				  &umax_preview_lines, 1, 65535));
			else if (umax_test_configure_option
				 (option_str, "scan-lines", &umax_scan_lines,
				  1, 65535));
			else if (umax_test_configure_option
				 (option_str, "handle-bad-sense-error",
				  &umax_handle_bad_sense_error, 0, 3));
			else if (umax_test_configure_option
				 (option_str, "execute-request-sense",
				  &umax_execute_request_sense, 0, 1));
			else if (umax_test_configure_option
				 (option_str, "force-preview-bit-rgb",
				  &umax_force_preview_bit_rgb, 0, 1));
			else if (umax_test_configure_option
				 (option_str, "slow-speed", &umax_slow, -1,
				  1));
			else if (umax_test_configure_option
				 (option_str, "care-about-smearing",
				  &umax_smear, -1, 1));
			else if (umax_test_configure_option
				 (option_str, "calibration-full-ccd",
				  &umax_calibration_area, -1, 1));
			else if (umax_test_configure_option
				 (option_str,
				  "calibration-width-offset-batch",
				  &umax_calibration_width_offset_batch,
				  -99999, 65535));
			else if (umax_test_configure_option
				 (option_str, "calibration-width-offset",
				  &umax_calibration_width_offset, -99999,
				  65535));
			else if (umax_test_configure_option
				 (option_str, "calibration-bytes-pixel",
				  &umax_calibration_bytespp, -1, 2));
			else if (umax_test_configure_option
				 (option_str, "exposure-time-rgb-bind",
				  &umax_exposure_time_rgb_bind, -1, 1));
			else if (umax_test_configure_option
				 (option_str, "invert-shading-data",
				  &umax_invert_shading_data, -1, 1));
			else if (umax_test_configure_option
				 (option_str, "lamp-control-available",
				  &umax_lamp_control_available, 0, 1));
			else if (umax_test_configure_option
				 (option_str, "gamma-lsb-padded",
				  &umax_gamma_lsb_padded, -1, 1));
			else if (umax_test_configure_option
				 (option_str, "connection-type",
				  &umax_connection_type, 1, 2));
			else {
				DBG(DBG_error,
				    "ERROR: unknown option \"%s\" in %s\n",
				    option_str, UMAX_CONFIG_FILE);
			}
			continue;
		} else if (strncmp(config_line, "scsi", 4) == 0) {
			DBG(DBG_info,
			    "sanei_config_attach_matching_devices(%s)\n",
			    config_line);
			sanei_config_attach_matching_devices(config_line,
							     attach_one_scsi);
			continue;
		} else if (strncmp(config_line, "usb", 3) == 0) {
#ifdef UMAX_ENABLE_USB
			DBG(DBG_info,
			    "sanei_usb_attach_matching_devices(%s)\n",
			    config_line);
			sanei_usb_attach_matching_devices(config_line,
							  attach_one_usb);
#else
			DBG(DBG_info,
			    "USB not supported, ignoring config line: %s\n",
			    config_line);
#endif
			continue;
		}
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	if (devicename[0]) {	/* search for devicename */
		...
		DBG(DBG_sane_info,
		    "sane_open: no devicename, opening first device\n");
	...
	for (j = 0; j < scanner->gamma_length; ++j) {	/* gamma_table[0] : converts GIB to GOB */
		scanner->gamma_table[0][j] =
			j * scanner->device->max_value /
	...
	scanner->exposure_time_range.min =
		scanner->device->inquiry_exposure_time_c_min *
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->exposure_time_range.quant =
		scanner->device->inquiry_exposure_time_step_unit;
	scanner->exposure_time_range.max =
		scanner->device->inquiry_exposure_time_max *
		scanner->device->inquiry_exposure_time_step_unit;
...
sane_get_option_descriptor(SANE_Handle handle, int option)
{
	Umax_Scanner *scanner = handle;
...
static void
umax_set_max_geometry(Umax_Scanner * scanner)
{
	...
	if (scanner->val[OPT_DOR].w) {
		...
		scanner->device->y_dpi_range.max =
			SANE_FIX(scanner->device->inquiry_dor_y_res);
	} else if ((strcmp(scanner->val[OPT_SOURCE].s, FLB_STR) == 0)
		   || (strcmp(scanner->val[OPT_SOURCE].s, ADF_STR) == 0)) {
		...
		scanner->device->y_dpi_range.max =
			SANE_FIX(scanner->device->inquiry_y_res);
	...
	DBG(DBG_info, "x_range     = [%f .. %f]\n",
	    SANE_UNFIX(scanner->device->x_range.min),
	    SANE_UNFIX(scanner->device->x_range.max));
	DBG(DBG_info, "y_range     = [%f .. %f]\n",
	    SANE_UNFIX(scanner->device->y_range.min),
...
SANE_Status
sane_control_option(SANE_Handle handle, int option, SANE_Action action,
		    void *val, int * info)
{
	Umax_Scanner *scanner = handle;
	SANE_Status status;
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_HALFTONE_PATTERN:
			memcpy(val, scanner->val[option].wa,
		...
		if (!SANE_OPTION_IS_SETTABLE(cap)) {
			DBG(DBG_error,
			    "could not set option, not settable\n");
			return SANE_STATUS_INVAL;
		}

		status = sanei_constrain_value(scanner->opt + option, val,
					       info);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "could not set option, invalid value\n");
			return status;
		}
		...
		case OPT_HOLDER_FOCUS_POS_0MM:
		case OPT_LAMP_OFF_AT_EXIT:
			scanner->val[option].w = *(int *) val;
			return SANE_STATUS_GOOD;

		case OPT_DOR:
		{
			if (scanner->val[option].w != *(int *) val) {
				scanner->val[option].w = *(int *) val;	/* update valure for umax_set_max_geometry */
		...
		case OPT_RESOLUTION_BIND:
		{
			if (scanner->val[option].w != *(int *) val) {
				...
				if (scanner->val[option].w == FALSE) {	/* don't bind */
					scanner->opt[OPT_Y_RESOLUTION].cap &=
						~SANE_CAP_INACTIVE;
					scanner->opt[OPT_X_RESOLUTION].title =
						SANE_TITLE_SCAN_X_RESOLUTION;
					scanner->opt[OPT_X_RESOLUTION].name =
						SANE_NAME_SCAN_X_RESOLUTION;
					scanner->opt[OPT_X_RESOLUTION].desc =
		...
		case OPT_SELECT_CAL_EXPOSURE_TIME:
		{
			if (scanner->val[option].w != *(int *) val) {
				...
				if (scanner->val[OPT_SELECT_EXPOSURE_TIME].
				    w == FALSE) {
					scanner->
						opt
					...
					scanner->opt[OPT_CAL_EXPOS_TIME].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_CAL_EXPOS_TIME_R].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_CAL_EXPOS_TIME_G].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_CAL_EXPOS_TIME_B].
						cap |= SANE_CAP_INACTIVE;

					scanner->opt[OPT_SCAN_EXPOS_TIME].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_SCAN_EXPOS_TIME_R].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_SCAN_EXPOS_TIME_G].
						cap |= SANE_CAP_INACTIVE;
					scanner->opt[OPT_SCAN_EXPOS_TIME_B].
						cap |= SANE_CAP_INACTIVE;
					...
					if ((strcmp(scanner->val[OPT_MODE].s, COLOR_STR) != 0) || (scanner->val[OPT_RGB_BIND].w == TRUE) || (scanner->device->exposure_time_rgb_bind)) {	/* RGB bind */
						if (scanner->
						    val
						    [OPT_SELECT_CAL_EXPOSURE_TIME].
						    w) {
		...
		case OPT_SELECT_LAMP_DENSITY:
		{
			if (scanner->val[option].w != *(int *) val) {
				...
				if (scanner->val[option].w == FALSE) {
					scanner->opt[OPT_CAL_LAMP_DEN].cap |=
						SANE_CAP_INACTIVE;
					scanner->opt[OPT_SCAN_LAMP_DEN].cap |=
						SANE_CAP_INACTIVE;
				} else {
					scanner->opt[OPT_CAL_LAMP_DEN].cap &=
						~SANE_CAP_INACTIVE;
		...
		case OPT_GAMMA_VECTOR_G:
		case OPT_GAMMA_VECTOR_B:
			memcpy(scanner->val[option].wa, val,
			       scanner->opt[option].size);
			return SANE_STATUS_GOOD;
		...
		case OPT_SOURCE:
		{
			...
			scanner->val[option].s = (char *) strdup(val);	/* update string for umax_set_max_geometry */

			DBG(DBG_info,
			    "sane_control_option: set SOURCE = %s\n",
			    (char *) val);
			umax_set_max_geometry(scanner);
...
#endif
		{
			...
			scanner->val[option].s = (char *) strdup(val);
			return SANE_STATUS_GOOD;
		}
		...
		{
			w = *(int *) val;
			if (w == scanner->val[OPT_CUSTOM_GAMMA].w) {
				return SANE_STATUS_GOOD;
			}
			...
			if (w) {	/* use custom_gamma_table */
				...
				if ((strcmp(mode, LINEART_STR) == 0) ||
					...
					scanner->opt[OPT_GAMMA_VECTOR_R].
						cap &= ~SANE_CAP_INACTIVE;
					scanner->opt[OPT_GAMMA_VECTOR_G].
						cap &= ~SANE_CAP_INACTIVE;
					scanner->opt[OPT_GAMMA_VECTOR_B].
						cap &= ~SANE_CAP_INACTIVE;
				}
		...
		case OPT_MODE:
		{
			...
			scanner->opt[OPT_CUSTOM_GAMMA].cap |=
				SANE_CAP_INACTIVE;
			scanner->opt[OPT_GAMMA_VECTOR].cap |=
				SANE_CAP_INACTIVE;
			scanner->opt[OPT_GAMMA_VECTOR_R].cap |=
			...
			scanner->opt[OPT_HALFTONE_DIMENSION].cap |=
				SANE_CAP_INACTIVE;
			...
			halftoning = (strcmp(val, HALFTONE_STR) == 0
				      || strcmp(val,
						COLOR_HALFTONE_STR) == 0);

			if (halftoning || strcmp(val, LINEART_STR) == 0 || strcmp(val, COLOR_LINEART_STR) == 0) {	/* one bit modes */
				if (scanner->device->inquiry_reverse) {
					scanner->opt[OPT_NEGATIVE].cap &=
						~SANE_CAP_INACTIVE;
				}

				if (halftoning) {	/* halftoning modes */
					...
					if (scanner->device->
					    inquiry_highlight) {
						scanner->opt[OPT_HIGHLIGHT].
							cap &=
							~SANE_CAP_INACTIVE;
					}

					if (scanner->device->inquiry_shadow) {
						scanner->opt[OPT_SHADOW].
							cap &=
							~SANE_CAP_INACTIVE;
					}
...
/* disable halftone pattern download options */
#if 0
					scanner->opt[OPT_HALFTONE_DIMENSION].
						cap &= ~SANE_CAP_INACTIVE;

					if (scanner->
					    val[OPT_HALFTONE_DIMENSION].w) {
						scanner->
							opt
					...
					if (scanner->
					    val[OPT_SELECT_EXPOSURE_TIME].w ==
					    TRUE) {
						scanner->
							opt
							[OPT_CAL_EXPOS_TIME].
					...
					scanner->exposure_time_range.min =
						scanner->device->
						inquiry_exposure_time_h_min *
						scanner->device->
						inquiry_exposure_time_step_unit;
				} else {	/* lineart modes */
					...
					scanner->exposure_time_range.min =
						scanner->device->
						inquiry_exposure_time_l_min *
						scanner->device->
						inquiry_exposure_time_step_unit;
				}
			} else {	/* multi-bit modes(gray or color) */
				scanner->opt[OPT_BIT_DEPTH].cap &=
					~SANE_CAP_INACTIVE;

				if (scanner->device->inquiry_highlight) {
					scanner->opt[OPT_HIGHLIGHT].cap &=
						~SANE_CAP_INACTIVE;
				}

				if (scanner->device->inquiry_shadow) {
					scanner->opt[OPT_SHADOW].cap &=
						~SANE_CAP_INACTIVE;
				}
				...
				if (scanner->device->inquiry_gamma_dwload) {
					...
					scanner->val[OPT_CUSTOM_GAMMA].w =
						FALSE;
				}
				...
				if (strcmp(val, COLOR_STR) == 0) {
					if ((scanner->device->
					     inquiry_analog_gamma)
					    || (scanner->device->
						inquiry_highlight)
					    || (scanner->device->
						inquiry_shadow)
					    || (scanner->device->
						inquiry_exposure_adj)) {
						scanner->opt[OPT_RGB_BIND].
							cap &=
							~SANE_CAP_INACTIVE;
					}
	...
	if (scanner->device->sfd == -1) {	/* first call, don`t run this routine again on multi frame or multi image scan */
		...
		if (strcmp(scan_source, UTA_STR) == 0) {
			if ((scanner->device->inquiry_uta != 0)
				...
				DBG(DBG_error,
				    "ERROR: Transparency Adapter not available\n");
				umax_scsi_close(scanner->device);
				return SANE_STATUS_INVAL;
			}
			...
			if (strcmp(scan_source, ADF_STR) == 0) {
				if ((scanner->device->inquiry_adf)
				    && (scanner->device->inquiry_adfmode)) {
					scanner->device->adf = 1;
				} else {
					DBG(DBG_error,
					    "ERROR: Automatic Document Feeder not available\n");
					umax_scsi_close(scanner->device);
					return SANE_STATUS_INVAL;
				}
			} else {
		...
		scanner->device->reverse = scanner->device->reverse_multi =
			scanner->val[OPT_NEGATIVE].w;

		scanner->device->threshold =
			P_100_TO_255(scanner->val[OPT_THRESHOLD].w);
		scanner->device->brightness =
			P_200_TO_255(scanner->val[OPT_BRIGHTNESS].w);
		scanner->device->contrast =
			P_200_TO_255(scanner->val[OPT_CONTRAST].w);

		scanner->device->batch_scan =
			(scanner->val[OPT_BATCH_SCAN_START].w
		...
		scanner->device->fix_focus_position =
			scanner->val[OPT_FIX_FOCUS_POSITION].w;
		scanner->device->lens_cal_in_doc_pos =
			scanner->val[OPT_LENS_CALIBRATION_DOC_POS].w;
		scanner->device->disable_pre_focus =
			scanner->val[OPT_DISABLE_PRE_FOCUS].w;
		scanner->device->holder_focus_pos_0mm =
			scanner->val[OPT_HOLDER_FOCUS_POS_0MM].w;
		scanner->device->manual_focus =
			scanner->val[OPT_MANUAL_PRE_FOCUS].w;

		scanner->device->analog_gamma_r =
			scanner->device->analog_gamma_g =
			scanner->device->analog_gamma_b =
			umax_calculate_analog_gamma(SANE_UNFIX
						    (scanner->
						     val[OPT_ANALOG_GAMMA].
						     w));
		...
		if (scanner->val[OPT_SELECT_EXPOSURE_TIME].w == TRUE) {
		...
		if (scanner->val[OPT_SELECT_LAMP_DENSITY].w == TRUE) {
			scanner->device->c_density =
				P_100_TO_254(scanner->val[OPT_CAL_LAMP_DEN].
					     w);
			scanner->device->s_density =
				P_100_TO_254(scanner->val[OPT_SCAN_LAMP_DEN].
					     w);
		}
		...
		if (strcmp(mode, LINEART_STR) == 0) {
			...
			if (scanner->val[OPT_RGB_BIND].w == FALSE) {
				scanner->device->analog_gamma_r =
					umax_calculate_analog_gamma(SANE_UNFIX
								    (scanner->
								     val
				...
				scanner->device->highlight_r =
					P_100_TO_255(scanner->
						     val[OPT_HIGHLIGHT_R].w);
				scanner->device->highlight_g =
					P_100_TO_255(scanner->
						     val[OPT_HIGHLIGHT_G].w);
				scanner->device->highlight_b =
					P_100_TO_255(scanner->
						     val[OPT_HIGHLIGHT_B].w);
				...
				if ((scanner->val[OPT_SELECT_EXPOSURE_TIME].
					...
					scanner->device->
						exposure_time_scan_r =
						scanner->
						val[OPT_SCAN_EXPOS_TIME_R].w;
					scanner->device->
						exposure_time_scan_g =
						scanner->
						val[OPT_SCAN_EXPOS_TIME_G].w;
					scanner->device->
						exposure_time_scan_b =
						scanner->
						val[OPT_SCAN_EXPOS_TIME_B].w;
				}
			}
		}
		...
		if (scanner->device->force_preview_bit_rgb != 0) {	/* in RGB-mode set preview bit, eg. for UMAX S6E */
			if (scanner->device->colormode == RGB) {
				DBG(DBG_sane_info,
				    "setting preview bit = 1 (option force-preview-bit-rgb)\n");
				scanner->device->preview = TRUE;
			}
		}
#ifdef UMAX_CALIBRATION_MODE_SELECTABLE
		if (strcmp(scanner->val[OPT_CALIB_MODE].s, CALIB_MODE_0000) ==
		    0) {
		...
		scanner->device->upper_left_x = ((int)
						 (SANE_UNFIX
						  (scanner->val[OPT_TL_X].w) *
						  xbasedots)) & 65534;
		scanner->device->upper_left_y = ((int)
						 (SANE_UNFIX
						  (scanner->val[OPT_TL_Y].w) *
						  ybasedots)) & 65534;

		scanner->device->scanwidth = ((int)
					      ((SANE_UNFIX
						(scanner->val[OPT_BR_X].w -
						 scanner->val[OPT_TL_X].w)) *
					       xbasedots)) & 65534;
		scanner->device->scanlength = ((int)
					       ((SANE_UNFIX
						 (scanner->val[OPT_BR_Y].w -
						  scanner->val[OPT_TL_Y].w)) *
						ybasedots)) & 65534;
#endif

		scanner->device->upper_left_x =
			(int) (SANE_UNFIX(scanner->val[OPT_TL_X].w) *
			       xbasedots);
		scanner->device->upper_left_y =
			(int) (SANE_UNFIX(scanner->val[OPT_TL_Y].w) *
			       ybasedots);

		scanner->device->scanwidth =
			(int) ((SANE_UNFIX
				(scanner->val[OPT_BR_X].w -
				 scanner->val[OPT_TL_X].w)) * xbasedots);
		scanner->device->scanlength =
			(int) ((SANE_UNFIX
				(scanner->val[OPT_BR_Y].w -
				 scanner->val[OPT_TL_Y].w)) * ybasedots);
		...
		if (scanner->device->dor != 0) {	/* dor mode active */
			DBG(DBG_info,
			    "substracting DOR x-origin-offset from upper left x\n");
		...
		/* set exposure times */
		if (scanner->device->inquiry_exposure_adj) {
			umax_calculate_exposure_time(scanner->device,
						     scanner->device->
						     use_exposure_time_def_r,
						     &scanner->device->
						     exposure_time_calibration_r);
			umax_calculate_exposure_time(scanner->device,
						     scanner->device->
						     use_exposure_time_def_g,
						     &scanner->device->
						     exposure_time_calibration_g);
			umax_calculate_exposure_time(scanner->device,
						     scanner->device->
						     use_exposure_time_def_b,
						     &scanner->device->
						     exposure_time_calibration_b);
			...
			scanner->device->exposure_time_calibration_r =
				...
				scanner->device->exposure_time_scan_r =
				scanner->device->exposure_time_scan_g =
				scanner->device->exposure_time_scan_b = 0;
		}
		...
		DBG(DBG_sane_info, "shadow                  = %d %d %d\n",
		    scanner->device->shadow_r,
		    scanner->device->shadow_g, scanner->device->shadow_b);
		...
		if (scanner->val[OPT_PREVIEW].w) {	/* preview mode */
			scanner->device->lines_max =
				scanner->device->request_preview_lines;
		} else {	/* scan mode */

			scanner->device->lines_max =
				scanner->device->request_scan_lines;
		}
...
#ifdef HAVE_SANEI_SCSI_OPEN_EXTENDED
		{
			...
			if (scsi_bufsize == 0) {	/* no scsi buffer size, take scanner buffer size */
				scsi_bufsize =
					scanner->device->inquiry_vidmem;
			}

			if (scsi_bufsize < scanner->device->scsi_buffer_size_min) {	/* make sure buffer has at least minimum size */
				scsi_bufsize =
					scanner->device->scsi_buffer_size_min;
			} else if (scsi_bufsize > scanner->device->scsi_buffer_size_max) {	/* make sure buffer does not exceed maximum size */
				scsi_bufsize =
					scanner->device->scsi_buffer_size_max;
			}

			if (umax_scsi_open_extended
			    (scanner->device->sane.name, scanner->device,
			     sense_handler, scanner->device,
			     (int *) &scsi_bufsize) != 0) {
				DBG(DBG_error,
				    "ERROR: sane_start: open failed\n");
				scanner->scanning = FALSE;
				return SANE_STATUS_INVAL;
			}
			...
			DBG(DBG_info,
			    "sane_start: umax_scsi_open_extended returned scsi buffer size = %d\n",
			    scsi_bufsize);

			if (scsi_bufsize < scanner->device->width_in_pixels) {	/* print warning when buffer is smaller than one scanline */
				DBG(DBG_warning,
				    "WARNING: sane_start: scsi buffer is smaller than one scanline\n");
			}

			if (scsi_bufsize != scanner->device->bufsize) {
				DBG(DBG_info,
				    "sane_start: buffer size has changed, reallocating buffer\n");

				if (scanner->device->buffer[0]) {
					DBG(DBG_info,
					    "sane_start: freeing SCSI buffer[0]\n");
		...
		if (umax_scsi_open
		    (scanner->device->sane.name, scanner->device,
		     sense_handler, scanner->device) != SANE_STATUS_GOOD) {
			scanner->scanning = FALSE;
			DBG(DBG_error,
			    "ERROR: sane_start: open of %s failed:\n",
			    scanner->device->sane.name);
			return SANE_STATUS_INVAL;
		}
...
/* halftone pattern download is not ready in this version */
		...
		if ((strcmp(mode, HALFTONE_STR) == 0)
		    || (strcmp(mode, COLOR_HALFTONE_STR) == 0)) {
			umax_send_halftone_pattern(scanner->device,
						   (char *) &(scanner->
							      halftone_pattern
							      [0]),
						   scanner->
	...
	/* ------------ end of first call -------------- */
	/* send gammacurves */
	if (scanner->val[OPT_CUSTOM_GAMMA].w == TRUE) {
		if (strcmp(mode, COLOR_STR) == 0) {
			if (scanner->device->three_pass == 0) {	/* one pass color scan */
				...
				gamma = malloc((size_t)
					       (3 * scanner->gamma_length *
						scanner->output_bytes));
				if (gamma == NULL) {
					DBG(DBG_warning,
					    "WARNING: not able to allocate memory for gamma table, gamma ignored !!!\n");
				} else {
					dest = 0;
					...
					DBG(DBG_sane_info,
					    "sending 3 * %d bytes of gamma data for RGB\n",
					    scanner->gamma_length *
					    scanner->output_bytes);

					umax_send_gamma_data(scanner->device,
							     &gamma[0], 3);
				...
				if (gamma == NULL) {
					DBG(DBG_warning,
					    "not able to allocate memory for gamma table, gamma ignored !!!\n");
				} else {
					dest = 0;
					color = scanner->device->
						three_pass_color;

					for (i = 0; i < scanner->gamma_length;
					     i++) {
						value = scanner->
							gamma_table[color][i];

						if (scanner->output_bytes ==
						    2) {
							gamma[dest++] =
								scanner->
								gamma_table[0]
					...
					DBG(DBG_sane_info,
					    "sending %d bytes of gamma data for color %d\n",
					    scanner->gamma_length *
					    scanner->output_bytes, color);
			...
			if (gamma == NULL) {
				...
				DBG(DBG_sane_info,
				    "sending %d bytes of gamma data for gray\n",
				    scanner->gamma_length *
				    scanner->output_bytes);
...
SANE_Status
sane_read(SANE_Handle handle, SANE_Byte * buf, int max_len,
	  int * len)
{
	Umax_Scanner *scanner = handle;
	ssize_t nread;
...
sane_set_io_mode(SANE_Handle handle, SANE_Bool non_blocking)
{
	...
	if (fcntl
	    (scanner->pipe_read_fd, F_SETFL,
	     non_blocking ? O_NONBLOCK : 0) < 0) {
		return SANE_STATUS_IO_ERROR;
	}
	return SANE_STATUS_GOOD;
}
