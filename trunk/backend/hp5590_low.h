...
#define CORE_BULK_OUT		1 << 2	/* CORE bulk operation - prepare for bulk OUT
					 * transfer
					 */
static SANE_Status hp5590_cmd(int dn,
			      unsigned int flags,
			      unsigned int cmd, unsigned char *data,
			      unsigned int size, unsigned int core_flags);
static SANE_Status hp5590_bulk_read(int dn,
				    unsigned char *bytes,
				    unsigned int size, void *state);
static SANE_Status hp5590_bulk_write(int dn, int cmd,
				     unsigned char *bytes, unsigned int size);
static SANE_Status hp5590_get_status(int dn);
