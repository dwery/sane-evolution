...
set_line_adjust(Mustek_pp_Handle * dev)
{
	...
	DBG(5, "set_line_adjust: ppl %u (%u), adjust %u, skip %u\n",
	    dev->params.pixels_per_line, (dev->bottomX - dev->topX),
	    adjustline, priv->adjustskip);


	sanei_pa4s2_writebyte(dev->fd, 6, 0x11);
	sanei_pa4s2_writebyte(dev->fd, 5,
			      (adjustline + priv->adjustskip) >> 8);
	sanei_pa4s2_writebyte(dev->fd, 6, 0x21);
	sanei_pa4s2_writebyte(dev->fd, 5,
			      (adjustline + priv->adjustskip) & 0xFF);
...
static void
read_line_101x(Mustek_pp_Handle * dev, SANE_Byte * buf, int pixel,
	       int RefBlack, SANE_Byte * calib, int * gamma)
{
		       ...
		       int RefBlack)
{
	...
	for (ctr = 0; ctr < pixel; ctr++) {
		...
		sum = lbuf[0][ctr] + lbuf[1][ctr] + lbuf[2][ctr] +
			lbuf[3][ctr];
...
find_black_side_edge_101x(Mustek_pp_Handle * dev)
{
	...
	for (ctr = 0; ctr < 20; ctr++) {
		...
		priv->ref_black = priv->ref_red = priv->ref_green =
			priv->ref_blue = buf[0];
...
max_color_levels_101x(Mustek_pp_Handle * dev)
{
	...
	for (ctr = 0; ctr < 32; ctr++) {
		...
		if (dev->mode == MODE_COLOR) {
			...
			read_average_line_101x(dev, rbuf[ctr],
			...
			read_average_line_101x(dev, gbuf[ctr],
					       dev->params.pixels_per_line,
					       priv->ref_green);
			...
			read_average_line_101x(dev, bbuf[ctr],
					       dev->params.pixels_per_line,
					       priv->ref_blue);
...
find_black_top_edge_101x(Mustek_pp_Handle * dev)
{
	...
	do {
		...
		read_line_101x(dev, buf, CCD300_MAXHSIZE, priv->ref_black,
			       NULL, NULL);
...
get_color_line_101x(Mustek_pp_Handle * dev, SANE_Byte * buf)
{
	...
	do {
		...
		if (!gored && !goblue && !gogreen) {
			...
			reset_bank_count(dev);
			if (priv->ccd_line >=
			    (priv->line_step >> SANE_FIXED_SCALE_SHIFT))
				priv->redline =
					++priv->redline % priv->green_offs;
			if (priv->ccd_line >=
			    priv->blue_offs +
			    (priv->line_step >> SANE_FIXED_SCALE_SHIFT))
				priv->blueline =
					++priv->blueline % priv->blue_offs;
			continue;
		}
		...
		if (priv->ccd_line >= priv->green_offs && gogreen) {
			...
			for (ctr = 0; ctr < dev->params.pixels_per_line;
			     ctr++) {
				*dest = *src++;
				dest += 3;
			}
		...
		if (gogreen) {
			...
			if (gored || goblue) {
				set_ccd_channel_101x(dev,
						     CCD300_CHANNEL_GREEN);
				set_sti(dev);
				wait_bank_change(dev, priv->bank_count, 1);
			}

			read_line_101x(dev, priv->green,
				       dev->params.pixels_per_line,
...
motor_backward_1013(Mustek_pp_Handle * dev)
{
	...
	priv->motor_phase =
		(priv->motor_phase == 0 ? 3 : priv->motor_phase - 1);
	    ...
	    const char * name, SANE_Attach_Callback attach)
{
	...
	if (asic != CCD300_ASIC1013 && asic != CCD300_ASIC1015) {
		DBG(2,
		    "ccd300_init: scanner not recognized (unknown ASIC id %#02x)\n",
		    asic);
		return SANE_STATUS_INVAL;
	}
	...
	DBG(3,
	    "ccd_init: found scanner on port ``%s'' (ASIC id %#02x, CCD %d)\n",
	    port, asic, ccd);
...
ccd300_open(char * port, int caps, int * fd)
{
	...
	if (caps & ~(CAP_NOTHING | CAP_INVERT | CAP_LAMP_OFF)) {
		DBG(1,
		    "ccd300_open: called with unknonw capabilities (%#02x)\n",
		    caps);
		return SANE_STATUS_INVAL;
	}
	      ...
	      const char * optval)
{
	...
	if (!strcmp(optname, "bw")) {
		...
		if (!optval) {
			DBG(1,
			    "ccd300_config: missing value for option ``bw''\n");
			return SANE_STATUS_INVAL;
		}
		...
		if (!optval) {
			DBG(1,
			    "ccd300_config: missing value for option ``waitbank''\n");
			return SANE_STATUS_INVAL;
		}
		...
		if (!optval) {
			DBG(1,
			    "ccd300_config: missing value for option ``top''\n");
			return SANE_STATUS_INVAL;
		}
...
ccd300_start(SANE_Handle handle)
{
	...
	move_motor(dev, priv->top + dev->topY -
		   (dev->mode == MODE_COLOR ? priv->green_offs : 0),
		   TRUE);
	...
	if (dev->mode == MODE_COLOR) {
		int failed = FALSE, cnt;

		priv->line_step = SANE_FIX(300.0 / (float) dev->res);
		priv->rdiff = priv->line_step;
		priv->bdiff =
			priv->rdiff +
			(priv->blue_offs << SANE_FIXED_SCALE_SHIFT);
		priv->gdiff =
			priv->rdiff +
			(priv->green_offs << SANE_FIXED_SCALE_SHIFT);
		...
		if ((priv->red == NULL) || (priv->blue == NULL)
			...
			DBG(1,
			    "ccd300_start: not enough memory for ld buffers\n");
			return SANE_STATUS_NO_MEM;
		}
		...
		for (cnt = 0; cnt < priv->green_offs; cnt++)
			if ((priv->red[cnt] =
			     malloc(dev->params.pixels_per_line)) == NULL)
				failed = TRUE;
