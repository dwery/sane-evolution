...
/* $Id: snapscan-scsi.c,v 1.58 2006-09-03 10:00:11 oliver-guest Exp $
...
static SnapScan_Model
snapscani_get_model_id(char *model_str, int fd, SnapScan_Bus bus_type)
{
	 ...
	 */
	if ((bus_type == USB) &&
	    (sanei_usb_get_vendor_product(fd, &vendor_id, &product_id) ==
	     SANE_STATUS_GOOD)) {
		DBG(DL_MINOR_INFO,
...
sense_handler(int scsi_fd, u_char * result, void *arg)
{
	...
	if ((result[0] & 0x80) == 0) {
		...
		DBG(DL_DATA_TRACE,
		    "%s: sense key: 0x%02x, asc: 0x%02x, ascq: 0x%02x, i1: 0x%02x, i2: 0x%02x\n",
...
static SANE_Status
snapscan_cmd(SnapScan_Bus bus, int fd, const void *src,
...
remove_trailing_space(char *s)
{
	...
	for (position = strlen(s);
	     position > 0 && ' ' == s[position - 1]; position--) {
		/* dummy */ ;
	}
	s[position] = 0;
}
...
static void
check_range(int *v, SANE_Range r)
{
	if (*v < r.min)
		*v = r.min;
	if (*v > r.max)
...
static SANE_Status
mini_inquiry(SnapScan_Bus bus, int fd, char *vendor, char *model)
{
...
/* TODO: Remove */
static char *
snapscani_debug_data(char *str, const SANE_Byte * data, int len)
{
	char tmpstr[10];
	int i;
...
inquiry(SnapScan_Scanner * pss)
{
	...
	status = snapscan_cmd(pss->pdev->bus,
			      pss->fd,
			      pss->cmd,
			      INQUIRY_LEN, pss->buf, &pss->read_bytes);
	CHECK_STATUS(status, me, "snapscan_cmd");
	...
	/* record current parameters */
	{
		...
		pss->ms_per_line =
			atof(exptime) * (float) pss->buf[INQUIRY_SCAN_SPEED];
	...
	case PERFECTION3490:
		if (pss->firmware_loaded) {
			...
			DBG(DL_DATA_TRACE,
			    "%s: Epson additional inquiry data:\n%s\n", me,
			    tmpstr);
	...
	default:
	{
		...
		signed char g =
			(pss->buf[INQUIRY_G2R_DIFF] & 0x80) ? -(pss->
								buf
								[INQUIRY_G2R_DIFF]
								& 0x7F) :
			pss->buf[INQUIRY_G2R_DIFF];
		signed char b =
			(pss->buf[INQUIRY_B2R_DIFF] & 0x80) ? -(pss->
								buf
								[INQUIRY_B2R_DIFF]
								& 0x7F) :
		...
		DBG(DL_DATA_TRACE,
		    ...
		    me, pss->chroma,
		    pss->chroma_offset[R_CHAN],
		    pss->chroma_offset[G_CHAN], pss->chroma_offset[B_CHAN]);
	}
		break;
	}

	pss->actual_res = u_char_to_u_short(pss->buf + INQUIRY_OPT_RES);
	...
	if ((pss->pdev->model == PERFECTION2480)
		...
		pss->bytes_per_line +=
			pss->buf[INQUIRY_BYTE_PER_LINE_MSB] << 16;
	pss->lines =
...
test_unit_ready(SnapScan_Scanner * pss)
{
	...
	DBG(DL_CALL_TRACE, "%s\n", me);
	status = snapscan_cmd(pss->pdev->bus, pss->fd, cmd, sizeof(cmd), NULL,
			      NULL);
	CHECK_STATUS(status, me, "snapscan_cmd");
	return status;
}
...
send(SnapScan_Scanner * pss, u_char dtc, u_char dtcq)
{
	...
	case DTC_HALFTONE:	/* halftone mask */
		...
		default:
			DBG(DL_MAJOR_ERROR,
			    "%s: bad halftone data type qualifier 0x%x\n", me,
			    dtcq);
	...
	case DTC_GAMMA2:
		...
		default:
			DBG(DL_MAJOR_ERROR,
			    "%s: bad gamma data type qualifier 0x%x\n", me,
			    dtcq);
			return SANE_STATUS_INVAL;
		}
		break;
	...
	status = snapscan_cmd(pss->pdev->bus, pss->fd, pss->buf,
			      SEND_LENGTH + tl, NULL, NULL);
	CHECK_STATUS(status, me, "snapscan_cmd");
	return status;
}
...
static SANE_Status
send_calibration_5150(SnapScan_Scanner * pss)
{
	...
	*(buf + 8) = length & 0xff;
	...
	status = snapscan_cmd(pss->pdev->bus, pss->fd, buf,
			      SEND_LENGTH + length, NULL, NULL);
	free(buf);
	return status;
}
...
set_window(SnapScan_Scanner * pss)
{
	...
	pc[SET_WINDOW_P_PADDING_TYPE] =
		0x00 /*| (pss->negative ? 0x00 : 0x80) */ ;
	...
	if (pss->pdev->model != ACER300F
	    ...
	    && pss->pdev->model != PRISA310 && pss->pdev->model != PRISA610) {
	...
	do {
		...
		if (status == SANE_STATUS_DEVICE_BUSY) {
			DBG(DL_MINOR_INFO,
			    "%s: waiting for scanner to warm up\n", me);
...
scan(SnapScan_Scanner * pss)
{
	...
	zero_buf(pss->cmd, MAX_SCSI_CMD_LEN);
	pss->cmd[0] = SCAN;
	status = snapscan_cmd(pss->pdev->bus, pss->fd, pss->cmd, SCAN_LEN,
			      NULL, NULL);
	CHECK_STATUS(status, me, "snapscan_cmd");
	return status;
}
			...
			/* first additional sense byte contains time to wait */
		{
			...
			if (delay > 0) {
				...
				/* This seems to happen for Epson scanners. Return
				   ...
				   next command... */
				DBG(DL_CALL_TRACE,
				    "%s: No timeout specified, returning immediately\n",
				    me);
				return SANE_STATUS_GOOD;
			}
		}
...
static SANE_Status
calibrate_epson(SnapScan_Scanner * pss)
{
	u_char *buf, *pbuf;
	int *bins;
	...
	if (!bins) {
		DBG(DL_MAJOR_ERROR,
		    "%s: out of memory allocating bins, %ld bytes.", me,
		    (u_long) num_bins * sizeof(int));
	...
	if (!buf) {
		DBG(DL_MAJOR_ERROR,
		    "%s: out of memory allocating calibration, %ld bytes.",
		    me, (u_long) expected_read_bytes);
		free(bins);
		return SANE_STATUS_NO_MEM;
	}
	...
	for (pass = 0; pass < 2; pass++) {
		...
		for (i = 0; i < cal_lines; i += loop_inc) {
			...
			if (cal_lines - i > loop_inc)
				expected_read_bytes =
					loop_inc * (num_bins * bytes_per_bin);
			else
				expected_read_bytes =
					(cal_lines -
			...
			status = snapscan_cmd(pss->pdev->bus, pss->fd,
					      pss->cmd, READ_LEN, buf,
					      &read_bytes);

			if (status != SANE_STATUS_GOOD) {
				DBG(DL_MAJOR_ERROR,
				    "%s: %s command failed: %s\n", me,
				    "read_cal_2480", sane_strstatus(status));
				free(bins);
				free(buf);
				return status;
			}
			...
			if (read_bytes != expected_read_bytes) {
				DBG(DL_MAJOR_ERROR,
				    "%s: read %lu of %lu calibration data\n",
				    me, (u_long) read_bytes,
				    (u_long) expected_read_bytes);
				free(bins);
				free(buf);
			...
			for (j = 0;
			     j <
			     (int) expected_read_bytes / (num_bins *
							  bytes_per_bin);
			     j++) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DL_MAJOR_ERROR, "%s: %s command failed: %s\n", me,
			    "send_cal_2480", sane_strstatus(status));
			free(bins);
			free(buf);
			return status;
		}
...
static SANE_Status
read_calibration_data(SnapScan_Scanner * pss, void *buf, u_char num_lines)
{
...
static SANE_Status
calibrate(SnapScan_Scanner * pss)
{
	int r;
	int c;
	u_char *buf;
	...
	if (line_length) {
		...
		if (num_lines > NUM_CALIBRATION_LINES) {
			...
			DBG(DL_MAJOR_ERROR,
			    "%s: scsi request size underflow (< %d bytes)",
			    me, line_length);
			return SANE_STATUS_IO_ERROR;
		}
		...
		if (!buf) {
			DBG(DL_MAJOR_ERROR,
			    "%s: out of memory allocating calibration, %d bytes.",
			    me, num_lines * line_length);
			return SANE_STATUS_NO_MEM;
		}

		DBG(DL_MAJOR_ERROR,
		    "%s: reading calibration data (%d lines)\n", me,
		    num_lines);
...
static SANE_Status
download_firmware(SnapScan_Scanner * pss)
{
	...
	if (status == SANE_STATUS_GOOD) {
		...
		if (fd == NULL) {
			...
			DBG(DL_INFO, "Size of firmware: %lu\n",
			    (u_long) bufLength);
			pCDB = (unsigned char *) malloc(bufLength +
							cdbLength);
			...
			*(pCDB + 8) = bufLength & 0xff;
			...
			status = snapscan_cmd(pss->pdev->bus, pss->fd, pCDB,
					      bufLength + cdbLength, NULL,
					      NULL);
			pss->firmware_loaded = TRUE;
