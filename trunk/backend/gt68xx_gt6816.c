			 ...
			 SANE_Byte * data, int size)
{
	...
	for (addr = 0; addr < size; addr += block_size) {
		...
		RIE(gt68xx_device_memory_read
		    (dev, addr, block_size, check_buf));
		if (memcmp(block, check_buf, block_size) != 0) {
			DBG(3,
			    "gt6816_download_firmware: mismatch at block 0x%0x\n",
			    addr);
...
SANE_Status
gt6816_lamp_control(GT68xx_Device * dev, SANE_Bool fb_lamp, SANE_Bool ta_lamp)
{
	GT68xx_Packet req;
