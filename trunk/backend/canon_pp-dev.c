...
static void
DBG(int level, const char *format, ...)
{
	va_list args;
...
static const struct scanner_hardware_desc
  ...
  hw_fb320p = { "FB320P", 2, 2, 3508, 2552, 0 }, hw_fb330p = {
"FB330P", 2, 2, 3508, 0, 1}, hw_fb620p = {
"FB620P", 3, 3, 7016, 5104, 0}, hw_fb630p = {
"FB630P", 3, 3, 7016, 0, 1}, hw_n640p = {
"N640P", 3, 3, 7016, 0, 1}, hw_n340p = {
"N340P", 2, 2, 3508, 0, 1},

	/* A few generic scanner descriptions for aliens */
	hw_alien600 = {
"Unknown 600dpi", 3, 3, 7016, 0, 1}, hw_alien300 = {
"Unknown 300dpi", 2, 2, 3508, 0, 1}, hw_alien = {
"Unknown (600dpi?)", 3, 3, 7016, 0, 1};
...
static unsigned char cmd_readinfo[] =
	{ 0xf3, 0x20, 0, 0, 0, 0, 0, 0, 0x0c, 0 };

/* Scan init command: Always followed immediately by command cmd_scan */
static unsigned char cmd_initscan[] =
	{ 0xde, 0x20, 0, 0, 0, 0, 0, 0, 0x2e, 0 };
...
static unsigned char cmd_buf_status[] =
	{ 0xf3, 0x21, 0, 0, 0, 0, 0, 0, 0x06, 0 };

/* Request a block of image data */
static unsigned char cmd_packet_req[] =
	{ 0xd4, 0x20, 0, 0, 0, 0, 0, 0x09, 0x64, 0 };
...
static unsigned char cmd_calblack[] =
	{ 0xf8, 0x20, 0, 0, 0, 0, 0, 0x4a, 0xc4, 0 };

/* Clear the existing gamma table and create a new one */
static unsigned char cmd_cleargamma[] =
	{ 0xc5, 0x20, 0, 0, 0, 0, 0, 0, 0, 0 };

/* Read back the gamma table values */
static unsigned char cmd_readgamma[] =
	{ 0xf6, 0x20, 0, 0, 0, 0, 0, 0, 0x20, 0 };

/* Reads COLOUR (R,G or B) calibration image */
static unsigned char cmd_calcolour[] =
	{ 0xf9, 0x20, 0, 0, 0, 0, 0, 0x4a, 0xc4, 0 };
...
static unsigned char cmd_setgamma[] =
	{ 0xe6, 0x20, 0, 0, 0, 0, 0, 0, 0x20, 0 };

#if 0
/* Something about RGB gamma/gain values? Not currently used by this code */
static unsigned char command_14[32] = { 0x2, 0x0, 0x3, 0x7f,
	0x2, 0x0, 0x3, 0x7f,
	0x2, 0x0, 0x3, 0x7f,
	0, 0, 0, 0,
	0x12, 0xd1, 0x14, 0x82,
 ...
 */
static int
safe_write(int fd, const char *p, unsigned long len)
{
	int diff;
...
static int
safe_read(int fd, char *p, unsigned long len)
{
	int diff;
	unsigned long total = 0;
...
int
sanei_canon_pp_init_scan(scanner_parameters * sp, scan_parameters * scanp)
{
	...
	case 0:		/* greyscale; 10 bits per pixel */
		expected_scanline_size = scanp->width * 1.25;
		break;
	case 1:		/* true-colour; 30 bits per pixel */
		expected_scanline_size = scanp->width * 3.75;
		break;
	default:
	...
	true_scanline_size =
		(buffer_info_block[0] << 8) | buffer_info_block[1];
	/* The scanner's idea of the number of scanlines in total */
	true_scanline_count =
		(buffer_info_block[2] << 8) | buffer_info_block[3];
	...
	if ((expected_scanline_size != true_scanline_size)
		...
		DBG(10, "expected: %i bytes wide, %i scanlines tall.\n",
		    expected_scanline_size, expected_scanline_count);
...
int
sanei_canon_pp_initialise(scanner_parameters * sp, int mode)
{
	...
	while (cur_id->id) {
		if (!strncmp
		    (sp->id_string + 8, cur_id->id, strlen(cur_id->id)))
			break;
		cur_id++;
	}
...
/* Shut scanner down */
int
sanei_canon_pp_close_scanner(scanner_parameters * sp)
{
...
int
sanei_canon_pp_load_weights(const char *filename, scanner_parameters * sp)
{
	int fd;
	...
	if ((ret < 0) || strcmp(buffer, header) != 0) {
		DBG(1,
		    "Calibration file header is wrong, recalibrate please\n");
		close(fd);
		return -2;
	}
	...
	if (safe_read(fd, (char *) (sp->blackweight), cal_data_size) < 0) {
		DBG(1,
		    "Error reading black calibration data, recalibrate?\n");
		close(fd);
		return -6;
	}
	...
	if (safe_read(fd, (char *) sp->greenweight, cal_data_size) < 0) {
		DBG(1,
		    "Error reading green calibration data, recalibrate?\n");
		close(fd);
		return -8;
	}
...
static void
convert_to_rgb(image_segment * dest, unsigned char *src,
...
int
sanei_canon_pp_read_segment(image_segment ** dest, scanner_parameters * sp,
			    scan_parameters * scanp, int scanline_number,
...
static void
convdata(unsigned char *srcbuffer, unsigned char *dstbuffer,
	 int width, int mode)
...
static int
adjust_output(image_segment * image, scan_parameters * scanp,
	      scanner_parameters * scannerp)
...
int
sanei_canon_pp_calibrate(scanner_parameters * sp, char *cal_file)
{
	...
	databuf =
		malloc(scanline_size * scanline_count * calibration_reads *
		       3);
	...
	for (readnum = 0; readnum < calibration_reads; readnum++) {
		...
		if (send_command
		    (sp->port, command_buffer, 10, 100000, 5000000)) {
	...
	DBG(20,
	    "  * Snoozing for 15 seconds while the scanner calibrates...");
	usleep(15000000);
	DBG(40, "done.\n");
	...
	if (cal_file != NULL) {
		...
		if (safe_write(outfile, header, strlen(header) + 1) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		if (safe_write
		    (outfile, (const char *) &fileversion, sizeof(int)) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		...
		if (safe_write(outfile, (char *) &(sp->scanheadwidth),
			...
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		if (safe_write
		    (outfile, (char *) (sp->blackweight),
		     sp->scanheadwidth * sizeof(long)) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		if (safe_write
		    (outfile, (char *) (sp->redweight),
		     sp->scanheadwidth * sizeof(long)) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		if (safe_write
		    (outfile, (char *) (sp->greenweight),
		     sp->scanheadwidth * sizeof(long)) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
		if (safe_write
		    (outfile, (char *) (sp->blueweight),
		     sp->scanheadwidth * sizeof(long)) < 0)
			DBG(10, "Write error on calibration file %s",
			    cal_file);
...
column_sum(image_segment * image, int x)
...
static int
scanner_setup_params(unsigned char *buf, scanner_parameters * sp,
		     scan_parameters * scanp)
{
...
int
sanei_canon_pp_abort_scan(scanner_parameters * sp)
{
...
int
sanei_canon_pp_adjust_gamma(scanner_parameters * sp)
{
...
int
sanei_canon_pp_sleep_scanner(struct parport *port)
{
...
int
sanei_canon_pp_detect(struct parport *port, int mode)
{
	/*int caps; */
...
static int
send_command(struct parport *port, unsigned char *buf, int bufsize,
	     int delay, int timeout)
