...
init_options(PINT_Scanner * s)
{
	...
	s->val[OPT_RESOLUTION].w =
		(s->hw->scanio.scan_x_resolution >
		 s->hw->scanio.scan_y_resolution) ? s->hw->scanio.
		scan_x_resolution : s->hw->scanio.scan_y_resolution;
	...
	x1 = SANE_FIX((s->hw->scanio.scan_x_origin + s->hw->scanio.scan_width)
		      / TWELVEHUNDS_PER_MM);
	y1 = SANE_FIX((s->hw->scanio.scan_y_origin +
		       s->hw->scanio.scan_height)
		      / TWELVEHUNDS_PER_MM);
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		if (ioctl(s->fd, SCIOCGET, &scanio) < 0) {
			DBG(1, "getting scanner state failed: %s",
			    strerror(errno));
			return SANE_STATUS_INVAL;
		}
		...
		if (ioctl(s->fd, SCIOCSET, &scanio) < 0) {
			DBG(1, "setting scan parameters failed: %s",
			    strerror(errno));
			return SANE_STATUS_INVAL;
		}
		if (ioctl(s->fd, SCIOCGET, &scanio) < 0) {
			DBG(1, "getting scan parameters failed: %s",
			    strerror(errno));
			return SANE_STATUS_INVAL;
		}
		...
		s->params.bytes_per_line =
			(scanio.scan_bits_per_pixel *
			 scanio.scan_pixels_per_line + 7) / 8;
		s->params.lines = scanio.scan_lines;
		s->params.depth = (scanio.scan_image_mode == SIM_COLOR) ?
			scanio.scan_bits_per_pixel /
			3 : scanio.scan_bits_per_pixel;
...
sane_start(SANE_Handle handle)
{
	...
	DBG(1, "%d pixels per line, %d bytes, %d lines high, dpi=%d\n",
	    ...
	    s->params.lines, s->val[OPT_RESOLUTION].w);
