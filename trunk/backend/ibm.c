...
/* next line by mf */
	...
	if (!((strncmp((char *) ibuf.vendor, "IBM", 3) == 0
		...
		DBG(1,
		    "attach: device \"%s\" doesn't look like a scanner I know\n",
		    devnam);
		sanei_scsi_close(fd);
		return (SANE_STATUS_INVAL);
	}
	...
	memset(&mup, 0, sizeof(mup));
	status = mode_sense(fd, (struct mode_pages *) &mup,
			    PC_CURRENT | MEASUREMENTS_PAGE);
	if (status != SANE_STATUS_GOOD) {
	...
	strncpy(str + sizeof(ibuf.revision), (char *) ibuf.revision,
		sizeof(ibuf.revision));
	...
	if (wbuf.image_comp == IBM_GRAYSCALE
	    || wbuf.image_comp == IBM_DITHERED_MONOCHROME) {
...
init_options(Ibm_Scanner * s)
{
	...
	s->val[OPT_MODE].s =
		strdup(mode_list[s->hw->info.image_mode_default]);
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		if (action == SANE_ACTION_SET_VALUE) {
			...
			status = sanei_constrain_value(s->opt + option, val,
						       info);
			if (status != SANE_STATUS_GOOD)
				return status;
			...
			case OPT_BR_Y:
				...
				if (strcmp
				    (s->val[OPT_PAPER].s,
				     paper_list[IBM_PAPER_USER_DEFINED]) !=
				    0) {
					if (info)
			...
			case OPT_PAPER:
				...
				if (strcmp(s->val[OPT_PAPER].s, "User") != 0) {
					s->val[OPT_TL_X].w = 0;
					s->val[OPT_TL_Y].w = 0;
					if (strcmp(s->val[OPT_PAPER].s, "A3")
					    == 0) {
						s->val[OPT_BR_X].w =
							PAPER_A3_W;
						s->val[OPT_BR_Y].w =
