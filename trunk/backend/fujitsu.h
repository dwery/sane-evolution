...
SANE_Status sane_get_select_fd(SANE_Handle h, int * fdp);

const SANE_Option_Descriptor *sane_get_option_descriptor(SANE_Handle handle,
							 int option);

SANE_Status sane_control_option(SANE_Handle handle, int option,
...
SANE_Status sane_read(SANE_Handle handle, SANE_Byte * buf, int max_len,
		      int * len);
...
static SANE_Status get_pixelsize(struct fujitsu *s, int *, int *, int *,
				 int *);
...
static SANE_Status copy_3091(struct fujitsu *s, unsigned char *buf, int len,
			     int side);
static SANE_Status copy_buffer(struct fujitsu *s, unsigned char *buf, int len,
			       int side);

static SANE_Status read_from_buffer(struct fujitsu *s, SANE_Byte * buf,
				    int max_len, int * len,
				    int side);
