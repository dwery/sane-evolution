...
hp_GetOpenDevice(const char *devname, HpConnect connect, int *pfd)
{
	...
	for (iCount = 0; iCount < HP_MAX_OPEN_FD; iCount++) {
		if (!asHpOpenFd[iCount].devname)
			continue;
		if ((strcmp(asHpOpenFd[iCount].devname, devname) == 0)
		    && (asHpOpenFd[iCount].connect == connect)) {
			if (pfd)
				*pfd = asHpOpenFd[iCount].fd;
			DBG(3,
			    "hp_GetOpenDevice: device %s is open with fd=%d\n",
			    devname, asHpOpenFd[iCount].fd);
...
hp_AddOpenDevice(const char *devname, HpConnect connect, int fd)
{
	...
	iKeepOpen = 0;
	switch (connect) {
	case HP_CONNECT_SCSI:
		iKeepOpen = iKeepOpenSCSI;
		break;
	case HP_CONNECT_PIO:
		iKeepOpen = iKeepOpenPIO;
		break;
	case HP_CONNECT_USB:
		iKeepOpen = iKeepOpenUSB;
		break;
	case HP_CONNECT_DEVICE:
		iKeepOpen = iKeepOpenDevice;
		break;
	...
	for (iCount = 0; iCount < HP_MAX_OPEN_FD; iCount++) {
		if (!asHpOpenFd[iCount].devname) {	/* Is this entry free ? */
			...
			DBG(3,
			    "hp_AddOpenDevice: added device %s with fd=%d\n",
			    devname, fd);
			asHpOpenFd[iCount].connect = connect;
...
hp_nonscsi_write(HpScsi this, hp_byte_t * data, size_t len, HpConnect connect)
{
	...
	case HP_CONNECT_USB:	/* Not supported */
		loc_len = len;
		status = sanei_usb_write_bulk((SANE_Int) this->fd, data,
					      &loc_len);
		n = loc_len;
		break;
...
static SANE_Status
hp_nonscsi_read(HpScsi this, hp_byte_t * data, size_t * len,
		HpConnect connect, int UNUSEDARG isResponse)
{
	int n = -1;
	...
	if (retries < 0) {	/* Read environment */
		...
		if (eptr != NULL) {
			if (sscanf(eptr, "%d", &retries) != 1)
				retries = 1;	/* Restore default */
			else if (retries < 0)
				retries = 0;	/* Allow no retries here */
		}
	}
	...
	for (;;) {		/* Retry on EOF */
		...
		case HP_CONNECT_USB:
			status = sanei_usb_read_bulk((SANE_Int) this->fd,
						     (SANE_Byte *) data, len);
			n = *len;
			break;
...
hp_nonscsi_open(const char *devname, int *fd, HpConnect connect)
{
	...
	case HP_CONNECT_DEVICE:
		...
		if (lfd < 0) {
			DBG(1,
			    "hp_nonscsi_open: open device %s failed (%s)\n",
			    devname, strerror(errno));
			status = (errno ==
				  EACCES) ? SANE_STATUS_ACCESS_DENIED :
				SANE_STATUS_INVAL;
		} else
...
sanei_hp_nonscsi_new(HpScsi * newp, const char *devname, HpConnect connect)
{
	...
	*newp = new;
	...
	if (!iAlreadyOpen)
		hp_AddOpenDevice(devname, connect, new->fd);
...
sanei_hp_get_connect(const char *devname)
{
	...
	if (!info) {
		DBG(1,
		    "sanei_hp_get_connect: Could not get info for %s. Assume SCSI\n",
		    devname);
		connect = HP_CONNECT_SCSI;
	} else if (!(info->config_is_up)) {
		DBG(1,
		    "sanei_hp_get_connect: Config not initialized for %s. Assume SCSI\n",
		    devname);
		connect = HP_CONNECT_SCSI;
	} else {
	...
	if ((connect == HP_CONNECT_SCSI) && !got_connect_type) {
		...
		if (maybe_usb) {
			...
			if (print_warning) {
				...
				DBG(1,
				    "  Device %s assumed to be SCSI, but device name\n",
				    devname);
				DBG(1,
				    "  looks like USB. Will continue with USB.\n");
				DBG(1,
				    "  If you really want it as SCSI, add the following\n");
				DBG(1,
				    "  to your file .../etc/sane.d/hp.conf:\n");
				DBG(1, "    %s\n", devname);
				DBG(1, "      option connect-scsi\n");
				DBG(1,
				    "  The same warning applies to other device names containing\n");
				DBG(1,
				    "  \"usb\", \"uscanner\" or \"ugen\".\n");
			}
...
hp_scsi_flush(HpScsi this)
{
	...
	*this->bufp++ = 0;
	...
	connect = sanei_hp_scsi_get_connect(this);
	if (connect == HP_CONNECT_SCSI)
		return sanei_scsi_cmd(this->fd, this->buf,
				      HP_SCSI_CMD_LEN + len, 0, 0);
	else
		return hp_nonscsi_write(this, this->buf + HP_SCSI_CMD_LEN,
					len, connect);
}
...
hp_scsi_scl(HpScsi this, HpScl scl, int val)
{
	...
	count = sprintf((char *) this->bufp, "\033*%c%d%c", group, val,
			param);
	this->bufp += count;
...
hp_scsi_read_slow(HpScsi this, void *dest, size_t * len)
{
	...
	while (leftover > 0) {	/* Until we got all the bytes */
		...
		if ((status != SANE_STATUS_GOOD) || (one != 1)) {
			DBG(250,
			    "hp_scsi_read_slow: Reading byte %d: status=%s, len=%d\n",
			    (int) (next_dest - start_dest),
			    sane_strstatus(status), (int) one);
		}

		if (status != SANE_STATUS_GOOD)
			break;	/* Finish on error */
	...
	if (connect == HP_CONNECT_SCSI) {
		...
		if (*len <= 32) {	/* Is it a candidate for reading bytewise ? */
			...
			info = sanei_hp_device_info_get
				(sanei_hp_scsi_devicename(this));
			if ((info != NULL) && (info->config_is_up)
			    && info->config.dumb_read)
				read_bytewise = 1;
		}

		if (!read_bytewise) {
			static hp_byte_t read_cmd[6] =
				{ 0x08, 0, 0, 0, 0, 0 };
			read_cmd[2] = *len >> 16;
			read_cmd[3] = *len >> 8;
			read_cmd[4] = *len;
			...
			RETURN_IF_FAIL(sanei_scsi_cmd(this->fd, read_cmd,
						      sizeof(read_cmd), dest,
						      len));
		} else {
			RETURN_IF_FAIL(hp_scsi_read_slow(this, dest, len));
		}
	} else {
		RETURN_IF_FAIL(hp_nonscsi_read
			       (this, dest, len, connect, isResponse));
	}
...
hp_get_simulation_map(const char *devname, const HpDeviceInfo * info)
{
	...
	sim_brightness =
		sanei_hp_device_simulate_get(devname, SCL_BRIGHTNESS);
		  ...
		  hp_bool_t invert)
{
	...
	if (lowbyte_first) {
		while (count--) {
			tmp = ((((unsigned int) data[0]) << 8) |
			       ((unsigned int) data[1])) & mask;
		  ...
		  int outfd, hp_bool_t use_imgbuf)
{
	...
	tsz = (HP_TMP_BUF_SIZE <=
	       0) ? procdata->bytes_per_line : HP_TMP_BUF_SIZE;
	...
	if (procdata->mirror_vertical || use_imgbuf) {
		...
		if (!ph->image_buf) {
			...
			ph->image_buf_size = 0;
			DBG(1,
			    "process_scanline_init: Not enough memory to mirror image\n");
		} else
...
process_data_write(PROCDATA_HANDLE * ph, unsigned char *data, int nbytes)
{
	...
	if (signal_caught
	    || (write(ph->outfd, ph->wr_buf, ph->wr_buf_size) !=
		ph->wr_buf_size)) {
	...
	while (nbytes > ph->wr_buf_size) {
		if (signal_caught
		    || (write(ph->outfd, data, ph->wr_buf_size) !=
			ph->wr_buf_size)) {
			DBG(1, "process_data_write: write failed: %s\n",
		 ...
		 int bytes_per_line)
{
	...
	if (ph->image_buf) {
		...
		if (ph->image_ptr + out_bytes_per_line - 1
			...
			DBG(1,
			    "process_scanline: would exceed image buffer\n");
		}
	} else {		/* Save scanlines in a bigger buffer. */
		/* Otherwise we will get performance problems */

		RETURN_IF_FAIL(process_data_write
			       (ph, linebuf, out_bytes_per_line));
	}
	return SANE_STATUS_GOOD;
}
...
process_data(PROCDATA_HANDLE * ph, unsigned char *read_ptr, int nread)
{
	...
	if (ph->tmp_buf_len > 0) {	/* Something left ? */
		...
		if (nread < bytes_left) {	/* All to buffer ? */
			memcpy(ph->tmp_buf + ph->tmp_buf_len, read_ptr,
			       nread);
		...
		read_ptr += bytes_left;
		nread -= bytes_left;
		RETURN_IF_FAIL(process_scanline
			       (ph, ph->tmp_buf, ph->tmp_buf_size));
		ph->tmp_buf_len = 0;
	}
	while (nread > 0) {
		if (nread >= ph->tmp_buf_size) {
			RETURN_IF_FAIL(process_scanline
				       (ph, read_ptr, ph->tmp_buf_size));
...
process_data_flush(PROCDATA_HANDLE * ph)
{
	...
	if (ph->wr_left != ph->wr_buf_size) {	/* Something in write buffer ? */
		...
		if (signal_caught
		    || (write(ph->outfd, ph->wr_buf, nbytes) != nbytes)) {
	...
	if (ph->image_buf) {
		...
		num_lines =
			((int) (image_len + bytes_per_line - 1)) /
			bytes_per_line;
		...
		if (procdata->mirror_vertical) {
			...
			while (num_lines > 0) {
				if (signal_caught
					...
					DBG(1,
					    "process_data_finish: write from memory failed: %s\n",
					    signal_caught ? "signal caught" :
					    strerror(errno));
...
#define HP_PIPEBUF	32768
	...
	if (procdata->startscan) {
		for (;;) {
			...
			if (signal_caught)
				goto quit;
			sanei_hp_scl_inquire(this, SCL_FRONT_BUTTON, &val, 0,
					     0);
			if (val)
				break;
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "do_read: Error starting scan in reader process\n");
			goto quit;
		}
	}
	...
	if (enable_requests) {	/* Issue SCSI-requests ? */
		while (count > 0 || reqs_completed < reqs_issued) {
			while (count > 0 && reqs_issued < reqs_completed + 2) {
				...
				DBG(3,
				    "do_read: entering request to read %lu bytes\n",
				    (unsigned long) req->len);

				status = sanei_scsi_req_enter(this->fd,
							      req->cmd, 6,
							      req->data,
							      &req->len,
							      &req->id);
				if (status != SANE_STATUS_GOOD) {
					DBG(1,
					    "do_read: Error from scsi_req_enter: %s\n",
					    sane_strstatus(status));
					goto quit;
				}
				if (signal_caught)
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "do_read: Error from scsi_req_wait: %s\n",
				    sane_strstatus(status));
				goto quit;
			}
			if (signal_caught)
			...
			status = process_data(ph, (unsigned char *) req->data,
					      (int) req->len);
			if (status != SANE_STATUS_GOOD) {
		...
		if (!read_buf) {
			DBG(1,
			    "do_read: not enough memory for read buffer\n");
			goto quit;
		}
		...
		while (count > 0) {
			...
			status = process_data(ph, (unsigned char *) read_buf,
					      (int) nread);
			if (status != SANE_STATUS_GOOD) {
				DBG(1, "do_read: Error in process_data\n");
				goto quit;
	...
	if (image_buf)
		sanei_hp_free(image_buf);
	if (read_buf)
		sanei_hp_free(read_buf);
	    ...
	    void *valp, size_t * lengthp)
{
	...
	if (memcmp(buf, expect, count) != 0) {
		DBG(1,
		    "scl_inq: malformed response: expected '%s', got '%.*s'\n",
		    expect, count, buf);
		return SANE_STATUS_IO_ERROR;
	}
	buf += count;

	if (buf[0] == 'N') {	/* null response */
	...
	if (sscanf(buf, "%d%n", &val, &count) != 1) {
		DBG(1,
		    "scl_inq: malformed response: expected int, got '%.8s'\n",
		    buf);
		return SANE_STATUS_IO_ERROR;
	}
	buf += count;
	...
	if (*buf++ != expect_char) {
		DBG(1,
		    "scl_inq: malformed response: expected '%c', got '%.4s'\n",
		    expect_char, buf - 1);
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (!lengthp)
		*(int *) valp = val;	/* Get integer value */
		...
		if (val > (int) *lengthp) {
			DBG(1,
			    "scl_inq: inquiry returned %d bytes, expected <= %lu\n",
			   ...
			   char **bufhp)
{
	...
	RETURN_IF_FAIL(hp_scsi_scl
		       (scsi, SCL_UPLOAD_BINARY_DATA, SCL_INQ_ID(scl)));
	...
	if (memcmp(buf, expect, count) != 0) {
		DBG(1,
		    "scl_upload_binary: malformed response: expected '%s', got '%.*s'\n",
		    expect, count, buf);
		return SANE_STATUS_IO_ERROR;
	}
	buf += count;
...
sanei_hp_scl_inquire(HpScsi scsi, HpScl scl, int *valp, int *minp, int *maxp)
{
	...
	if (minp)
		RETURN_IF_FAIL(_hp_scl_inq(scsi, scl,
					   SCL_INQUIRE_MINIMUM_VALUE, minp,
					   0));
	if (maxp)
		RETURN_IF_FAIL(_hp_scl_inq(scsi, scl,
					   SCL_INQUIRE_MAXIMUM_VALUE, maxp,
					   0));
	return SANE_STATUS_GOOD;
}
...
hp_scl_get_bounds(HpScsi scsi, HpScl scl, int *minp, int *maxp)
{
	...
	RETURN_IF_FAIL(_hp_scl_inq
		       (scsi, scl, SCL_INQUIRE_MINIMUM_VALUE, minp, 0));
			  ...
			  int *minp, int *maxp, int *valp)
{
	...
	RETURN_IF_FAIL(_hp_scl_inq
		       (scsi, scl, SCL_INQUIRE_MINIMUM_VALUE, minp, 0));
	RETURN_IF_FAIL(_hp_scl_inq
		       (scsi, scl, SCL_INQUIRE_MAXIMUM_VALUE, maxp, 0));
...
hp_scl_strerror(int errnum)
{
	...
	if (errnum >= 0
	    && errnum < (int) (sizeof(errlist) / sizeof(errlist[0])))
		return errlist[errnum];
	else
		switch (errnum) {
		case 1024:
			return "ADF Paper Jam";
		case 1025:
			return "Home Position Missing";
		case 1026:
...
sanei_hp_scl_errcheck(HpScsi scsi)
{
	...
	status = sanei_hp_scl_inquire(scsi, SCL_CURRENT_ERROR_STACK, &nerrors,
				      0, 0);
	if (!FAILED(status) && nerrors)
		status = sanei_hp_scl_inquire(scsi, SCL_OLDEST_ERROR, &errnum,
					      0, 0);
	if (FAILED(status)) {
