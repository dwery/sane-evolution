 ...
 */
static SANE_Status
u12_initDev(U12_Device * dev, int handle, int vendor)
{
	int i;
	SANE_Status res;
	TimerDef timer;
 ...
 */
static SANE_Bool
u12if_IsDeviceSupported(U12_Device * dev)
{
	int i;
 ...
 * @return TRUE if a matching device has been found or
 *         FALSE if nothing supported found...
 */
static SANE_Bool
usbDev_autodetect(int * vendor, int * product)
{
	int i;
	int p, v;
...
u12if_open(U12_Device * dev)
{
	...
	if (!strcmp(dev->name, "auto")) {
		...
		if (dev->usbId[0] == '\0') {
			...
			sanei_usb_find_devices(vendor, product,
					       u12if_usbattach);
 ...
 */
static SANE_Status
u12if_startScan(U12_Device * dev)
{
 ...
 */
static SANE_Status
u12if_stopScan(U12_Device * dev)
{
	DBG(_DBG_INFO, "u12if_stopScan()\n");

#if 0
	u12motor_ToHomePosition(dev, FALSE);
#else
#if 0
 ...
 */
static SANE_Status
u12if_readLine(U12_Device * dev, SANE_Byte * line_buffer)
{
	SANE_Status res;
 ...
 */
static SANE_Status
u12if_SetupBuffer(U12_Device * dev)
{
	void *buffer;
	...
	dev->bufs.TpaBuf.pb =
		&((SANE_Byte *) dev->bufs.b2.pSumBuf)[_SIZE_SHADING_SUM_BUF];
