...
static SANE_Status attach_one_net(const char * devname);
...
attach(const char *name, Epson_Device * *devp, int type)
{
	...
	dev->color_shuffle = FALSE;
	dev->extension = FALSE;
	dev->use_extension = FALSE;

	dev->need_color_reorder = FALSE;
	dev->need_double_vertical = FALSE;
	...
	dev->resolution_list =
		malloc((dev->res_list_size + 1) * sizeof(int));
	...
	*(dev->resolution_list) = dev->res_list_size;
	...
	/* establish defaults */
	dev->need_reset_on_source_change = FALSE;
	...
	if (e2_model(s, "GT-8200") || e2_model(s, "Perfection1650")
		...
		dev->cmd->feed = 0;
		dev->cmd->set_focus_position = 0;
		dev->need_reset_on_source_change = TRUE;
	}
...
static void
setOptionState(Epson_Scanner * s, SANE_Bool state, int option,
	       SANE_Bool * change)
{
	if (state)
...
e2_set_extended_scanning_parameters(Epson_Scanner * s)
{
	...
	s->invert_image = FALSE;	/* default: do no invert the image */
...
static void
e2_copy_image_data(Epson_Scanner * s, SANE_Byte * data, int max_length,
		   int * length)
{
