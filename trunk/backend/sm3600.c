...
InitOptions(TInstance * this)
{
	...
	for (iOpt = optCount; iOpt != optLast; iOpt++) {
		...
		static const SANE_Range *aRangesXY[] =
			{ &rangeXmm, &rangeYmm, &rangeXmm, &rangeYmm };
...
SANE_Status
sane_init(int * version_code, SANE_Auth_Callback authCB)
{
	int i;
	...
	for (i = 0; aScanners[i].idProduct; i++) {
		sanei_usb_find_devices(SCANNER_VENDOR, aScanners[i].idProduct,
				       sm_usb_attach);
	}
	return SANE_STATUS_GOOD;
}
...
sane_get_option_descriptor(SANE_Handle handle, int iOpt)
{
...
SANE_Status
sane_control_option(SANE_Handle handle, int iOpt,
		    SANE_Action action, void *pVal, int * pnInfo)
{
	int cap;
	SANE_Status rc;
	TInstance *this;
	...
	case SANE_ACTION_GET_VALUE:
		...
		case optGammaB:
			...
			memcpy(pVal, this->aoptVal[iOpt].wa,
			       this->aoptDesc[iOpt].size);
			break;
		default:
			return SANE_STATUS_INVAL;
		}
	...
	case SANE_ACTION_SET_VALUE:
		if (!SANE_OPTION_IS_SETTABLE(cap))
			return SANE_STATUS_INVAL;
		rc = sanei_constrain_value(this->aoptDesc + iOpt, pVal,
					   pnInfo);
		...
		case optGammaB:
			...
			memcpy(this->aoptVal[iOpt].wa, pVal,
			       this->aoptDesc[iOpt].size);
			break;
		default:
			return SANE_STATUS_INVAL;
...
SetupInternalParameters(TInstance * this)
{
	...
	this->param.nBrightness =
		(int) (this->aoptVal[optBrightness].
		       w >> SANE_FIXED_SCALE_SHIFT);
	this->param.nContrast =
		(int) (this->aoptVal[optContrast].
		       w >> SANE_FIXED_SCALE_SHIFT);
#else
...
#endif
	this->param.x =
		(int) (SANE_UNFIX(this->aoptVal[optTLX].w) * 1200.0 / 25.4);
	this->param.y =
		(int) (SANE_UNFIX(this->aoptVal[optTLY].w) * 1200.0 / 25.4);
	this->param.cx =
		(int) (SANE_UNFIX
		       (this->aoptVal[optBRX].w -
			this->aoptVal[optTLX].w) * 1200.0 / 25.4) + 1;
	this->param.cy =
		(int) (SANE_UNFIX
		       (this->aoptVal[optBRY].w -
			this->aoptVal[optTLY].w) * 1200.0 / 25.4) + 1;
...
sane_start(SANE_Handle handle)
{
	...
	if (!rc)
		rc = DoInit(this);	/* oopsi, we should initalise :-) */
	if (!rc && !this->bOptSkipOriginate)
		rc = DoOriginate(this, true);
	if (!rc)
		rc = DoJog(this, this->calibration.yMargin);
	if (rc)
		return rc;
...
{
	h = h;
	if (m == TRUE)	/* no non-blocking-mode */
		return SANE_STATUS_UNSUPPORTED;
	return SANE_STATUS_GOOD;
}
