...
hp4200_init_registers(HP4200_Scanner * s)
{
	...
	setreg(s, 0x43,
	       RAM_SIZE_128 |
	       SRAM_DRIVER_CURRENT(3) | SRAM_BANDWIDTH_8 |
	...
	setreg(s, 0x54,
	       NON_REVERSING_EXTRA_LINES(0) | FIRST_LINE_TO_PROCESS(0));
		    ...
		    size_t * byte_count)
{
	...
	*byte_count = 0;
	...
	while (to_read) {
		...
		really_read = chunk;
		if ((status =
		     sanei_usb_read_bulk(s->fd, buffer,
					 &really_read)) != SANE_STATUS_GOOD) {
		...
		if (really_read > to_read) {
			DBG(DBG_error,
			    "USB stack read more bytes than requested!\n");
...
compute_datalink_bandwidth(HP4200_Scanner * s)
{
...

	{
		...
		step_size =
			compute_fastfeed_step_size(s->hw_parms.
						   crystal_frequency, s->mclk,
						   s->hw_parms.
						   scan_bar_max_speed,
						   s->hw_parms.
						   motor_full_steps_per_inch,
						   color_mode);
...

	{
		...
		if (!buffer) {
			DBG(DBG_error,
			    "compute_datalink_bandwidth: malloc failed\n");
			return 0;
		}
		bytes_read_total = 0;
...
write_gamma(HP4200_Scanner * s)
{
	...
	for (color = 0; color < 3; color++) {
		...
		if (retval != 0) {
			DBG(DBG_error,
			    "error: color %d has bad gamma table\n", color);
		}
#ifdef DEBUG
		else
		    ...
		    int *offset, int *max_gain, int *min_offset)
{
	...
	if (min == 0) {
		if (*offset < 31) {
			...
			DBG(DBG_error,
			    "error: max static has pixel value == 0\n");
			return -1;
		}
	} else {
		       ...
		       unsigned char AFE_operation, int m)
{
	...
	case MODEA_1_CHANNEL:
		bytes_per_line =
			(pixels_per_line / pixels_per_byte) + status_bytes;
		break;
	default:
...
compute_pause_limit(hardware_parameters_t * hw_parms, int bytes_per_line)
{
	...
	pause_limit =
		hw_parms->SRAM_size - coef_size - (bytes_per_line / 1024) - 1;
...
read_required_bytes(HP4200_Scanner * s, int required, SANE_Byte * buffer)
{
	...
	while (required) {
		...
		while (to_read) {
			...
			sanei_pv8630_write_byte(s->fd, PV8630_REPPADDRESS,
						0x00);
			sanei_pv8630_prep_bulkread(s->fd, chunk);
			really_read = chunk;
			if ((status =
...
do_coarse_calibration(HP4200_Scanner * s, struct coarse_t *coarse)
{
	...
	data_pixels_end =
		(int) (data_pixels_start + s->hw_parms.scan_area_width);
	...
	do {
		...
		setreg(s, 0x07, 0x00);
		{
			unsigned int i;
			min_red = max_red =
				(cal_line[0] * 256 + cal_line[1]) >> 2;
			min_green = max_green =
				(cal_line[2] * 256 + cal_line[3]) >> 2;
			min_blue = max_blue =
				(cal_line[4] * 256 + cal_line[5]) >> 2;
			for (i = 6; i < (s->hw_parms.scan_area_width * 3 * 2);
			     i += 6) {
				int value;
				...
				if (value > max_red)
					max_red = value;
				value = cal_line[i + 2] * 256 + cal_line[i +
									 3];
				value >>= 2;
				if (value > max_green)
					max_green = value;
				value = cal_line[i + 4] * 256 + cal_line[i +
									 5];
				value >>= 2;
				if (value > max_blue)
					max_blue = value;
...
#ifdef DEBUG
			fprintf(stderr,
				"max_red:%d max_green:%d max_blue:%d\n",
				max_red, max_green, max_blue);
			fprintf(stderr,
				"min_red:%d min_green:%d min_blue:%d\n",
				min_red, min_green, min_blue);
#endif

			if (first_time) {
				first_time = 0;
				compute_first_gain_offset(s->hw_parms.
							  target_value.red,
							  max_red, min_red,
							  &red_gain,
							  &red_offset,
				...
				retval = compute_gain_offset(s->hw_parms.
							     target_value.red,
							     max_red, min_red,
							     &red_gain,
							     &red_offset,
				...
				if (retval < 0)
					break;
				retval |=
					compute_gain_offset(s->hw_parms.
							    target_value.
							    green, max_green,
							    min_green,
							    &green_gain,
			...
			fprintf(stderr, "%d, %d, %d   %d, %d, %d\n", red_gain,
				green_gain, blue_gain, red_offset,
...
compute_corr_code(int average, int min_color, int range, int target)
{
	...
	if (value > 0)
		corr_code =
			(int) (range *
			       ((double) target / (double) value - 1.0) +
			       0.5);
	else
		corr_code = 0;
	if (corr_code < 0)
...
do_fine_calibration(HP4200_Scanner * s, struct coarse_t *coarse)
{
	...
	data_pixels_end =
		(int) (data_pixels_start +
		       s->user_parms.image_width * hdpi_div);
	data_pixels_end = min(data_pixels_end, line_end - 20);
...
#if 0
	{
		...
		do {
			for (i = 3; (i + 6) < (line_length * 3); i += 3) {
				average[i] =
					(2 * average[i - 3] + average[i] +
					 2 * average[i + 3]) / 5;
				average[i + 1] =
					(2 * average[i - 2] + average[i + 1] +
					 2 * average[i + 4]) / 5;
				average[i + 2] =
					(2 * average[i - 1] + average[i + 2] +
					 2 * average[i + 5]) / 5;
			}
			j++;
		}
...
#endif
	{
		...
		/* do fine calibration */
		{
			...
			min_white_red = min_white_green = min_white_blue =
				0x3ff;
			...
			for (i = 0, j = 0; i < (line_length * 3); i += 3, j++) {
				corr_red[j] = compute_corr_code(average[i],
								...
								red);
				corr_green[j] =
					compute_corr_code(average[i + 1],
							  coarse->min_green,
							  range,
							  s->hw_parms.
							  target_value.green);
				corr_blue[j] =
					compute_corr_code(average[i + 2],
							  coarse->min_blue,
							  range,
							  s->hw_parms.
							  target_value.blue);
...
#ifdef DEBUG
			{
				...
				for (i = 0; i < line_length; i++) {
					fprintf(kaka, "%d %d %d %d %d %d ",
						corr_red[i], corr_green[i],
						corr_blue[i], average[3 * i],
						average[3 * i + 1],
						average[3 * i + 2]);
					fprintf(kaka,
						"%d %d %d  %d %d %d  %d %d %d ",
...
#endif
			{
				...
				memset(red_gain_offset, 0,
				       sizeof(red_gain_offset));
				memset(green_gain_offset, 0,
				       sizeof(green_gain_offset));
				memset(blue_gain_offset, 0,
				       sizeof(blue_gain_offset));
				for (i = 0, j =
				     (data_pixels_start -
				      active_pixels_start) * 2;
				     i < line_length; i++, j += 2) {
					if (use_six_eight_bits) {
						red_gain_offset[j] =
							(coarse->
...
prepare_for_a_scan(HP4200_Scanner * s)
{
	...
	line_rate_color = 1;
	step_size =
		(s->user_parms.vertical_resolution * line_end *
		 line_rate_color) / (4 *
		   ...
		   int num_bytes, int image_line_size, int status_bytes)
{
	...
	biggest_upper_block_size =
		cb->size - (cb->buffer_position - cb->buffer);
	...
	cb->can_consume += upper_block_size +
		status_bytes *
		(((bytes_to_be_a_entire_line +
		   upper_block_size) / image_line_size) - 1);
	...
	if (lower_block_size > 0) {
		memcpy(buf + biggest_upper_block_size, cb->buffer,
		       lower_block_size);
	...
	while (to_consume) {
		...
		if (cb->pixel_position == image_width) {
			if (scanner_buffer->num_bytes >= status_bytes) {
				...
				cb->red_idx =
					(cb->red_idx + 1) % cb->num_lines;
				cb->green_idx =
					(cb->green_idx + 1) % cb->num_lines;
				cb->blue_idx =
					(cb->blue_idx + 1) % cb->num_lines;
				cb->current_line++;
			} else
				break;
		}
		...
		for (i = cb->pixel_position * 3; to_consume_now >= 3;
			...
			cb->buffer_ptrs[cb->red_idx][i] =
				scanner_buffer->data_ptr[0];
			cb->buffer_ptrs[cb->green_idx][i + 1] =
				scanner_buffer->data_ptr[1];
			cb->buffer_ptrs[cb->blue_idx][i + 2] =
				scanner_buffer->data_ptr[2];
...
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * len)
{
	...
	*len = 0;
	...
	to_copy_now =
		min(s->ciclic_buffer.good_bytes, bytes_to_copy_to_frontend);
	...
	while (bytes_to_copy_to_frontend) {
		...
		while ((s->scanner_buffer.num_bytes > 3)
		       && bytes_to_copy_to_frontend) {
			ciclic_buffer_consume(&(s->ciclic_buffer),
					      &(s->scanner_buffer),
					      s->user_parms.image_width,
					      s->runtime_parms.status_bytes);
			to_copy_now =
				min(s->ciclic_buffer.good_bytes,
				    bytes_to_copy_to_frontend);

			if (to_copy_now > 0) {
				ciclic_buffer_copy(&(s->ciclic_buffer), buf,
						   to_copy_now,
						   s->runtime_parms.
						   image_line_size,
						   s->runtime_parms.
...
init_options(HP4200_Scanner * s)
{
	...
	s->opt[OPT_NUM_OPTS].size = sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_R].size = 1024 * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_G].size = 1024 * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_B].size = 1024 * sizeof(int);
...
SANE_Status
sane_get_select_fd(SANE_Handle h, int * fd)
{
