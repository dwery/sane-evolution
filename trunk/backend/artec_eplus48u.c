...
static int memory_read_value = 0x200c;	/**< Memory read - wValue */
static int memory_write_value = 0x200b;	/**< Memory write - wValue */
static int send_cmd_value = 0x2010;	/**< Send normal command - wValue */
static int send_cmd_index = 0x3f40;	/**< Send normal command - wIndex */
static int recv_res_value = 0x2011;	/**< Receive normal result - wValue */
static int recv_res_index = 0x3f00;	/**< Receive normal result - wIndex */
static int send_small_cmd_value = 0x2012;	/**< Send small command - wValue */
static int send_small_cmd_index = 0x3f40;	/**< Send small command - wIndex */
...
static int bitdepth_list[] = {
	2, 8, 16
};

static int bitdepth_list2[] = {
	1, 8
};
...
download_firmware_file(Artec48U_Device * chip)
{
	...
	if (status == SANE_STATUS_GOOD) {
		...
		if (size == -1) {
			XDBG((2,
			      "Error getting size of firmware file \"%s\"\n",
			      chip->firmware_path));
			status = SANE_STATUS_INVAL;
		}
	}
...
init_calibrator(Artec48U_Scanner * s)
{
	...
	s->shading_buffer_white[0] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
	s->shading_buffer_black[0] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
	s->shading_buffer_white[1] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
	s->shading_buffer_black[1] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
	s->shading_buffer_white[2] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
	s->shading_buffer_black[2] = (unsigned int *) malloc(5120 * s->dev->epro_mult * sizeof(unsigned int));	/*epro */
...
finish_shading_buffer(Artec48U_Scanner * s, SANE_Bool white)
{
	...
	for (c = 0; c < (30720 * s->dev->epro_mult) - 5; c += 6) {	/*epro */
		i = (int) shading_buffer[c] +
			((int) shading_buffer[c + 1] << 8);
		max_r += i;
		i = (int) shading_buffer[c + 2] +
			((int) shading_buffer[c + 3] << 8);
		max_g += i;
		i = (int) shading_buffer[c + 4] +
			((int) shading_buffer[c + 5] << 8);
		max_b += i;
	}
}
...
static void
finish_offset_buffer(Artec48U_Scanner * s, int *avg_r, int *avg_g, int *avg_b)
{
	unsigned int i, j, cnt, c, div;
	unsigned int min_r;
	unsigned int min_g;
 ...
 * @return The function returns TRUE if the option has been found,
			...
			if (_INT == what) {
				...
				*((int *) result) = *((int *) def);
				if (*name) {
					...
					name = sanei_config_get_string(name,
								       &tmp2);
					if (tmp2) {
						*((int *) result) =
							strtol(tmp2, 0, 0);
 ...
 *                devicename
 * @return The function returns TRUE if the devicename has been found,
 *         if not, it returns FALSE
 */
static SANE_Bool
			   ...
			   SANE_Byte * data, int size)
{
	...
	int block_size = MAX_DOWNLOAD_BLOCK_SIZE;
	...
	for (addr = 0; addr < size; addr += block_size) {
		...
		status = artec48u_device_memory_write(dev, addr, block_size,
						      block);
		if (status != SANE_STATUS_GOOD)
			return status;
		status = artec48u_device_memory_read(dev, addr, block_size,
						     check_buf);
		if (status != SANE_STATUS_GOOD)
			return status;
		    ...
		    Artec48U_Scan_Parameters * params)
{
	...
	if (!calculate_only) {
		...
		if (action == SA_SCAN) {
			artec48u_calculate_shading_buffer(s, pixel_x0,
							  pixel_xs + pixel_x0,
							  xdpi, color);
			artec48u_generic_set_exposure_time(s->dev,
							   &(s->dev->
							     artec_48u_exposure_params));
			artec48u_generic_set_afe(s->dev,
						 &(s->dev->
						   artec_48u_afe_params));
...
artec48u_device_activate(Artec48U_Device * dev)
{
	...
	dev->xdpi_offset = SANE_FIX(dev->xdpi_offset *
				    SANE_MM_PER_INCH / dev->optical_xdpi);
	dev->ydpi_offset = SANE_FIX(dev->ydpi_offset *
				    SANE_MM_PER_INCH / dev->optical_ydpi);
...
artec48u_device_memory_write(Artec48U_Device * dev,
			     int addr, int size, SANE_Byte * data)
{
...
artec48u_device_generic_req(Artec48U_Device * dev,
			    int cmd_value, int cmd_index,
			    int res_value, int res_index,
			  ...
			  Artec48U_Packet res)
{
	...
	return artec48u_device_generic_req(dev,
					   ...
					   recv_small_res_index, fixed_cmd,
					   res);
}
...
artec48u_device_read_prepare(Artec48U_Device * dev, size_t expected_count)
{
	...
	if (!dev->read_buffer) {
		XDBG((3,
		      "%s: not enough memory for the read buffer (%lu bytes)\n",
		      function_name,
		      (unsigned long) dev->requested_buffer_size));
		return SANE_STATUS_NO_MEM;
	}
...
static SANE_Status
artec48u_device_read(Artec48U_Device * dev, SANE_Byte * buffer, size_t * size)
{
	...
	while (left_to_read > 0) {
		if (dev->read_bytes_in_buffer == 0) {
			...
			status = artec48u_device_read_raw(dev,
							  dev->read_buffer,
							  &raw_block_size);
			if (status != SANE_STATUS_GOOD) {
		...
		if (transfer_size > 0) {
			memcpy(buffer, dev->read_buffer + dev->read_pos,
			       transfer_size);
...
static inline void
unpack_8_mono(SANE_Byte * src, unsigned int *dst, int pixels_per_line)
{
		 ...
		 unsigned int **buffer_pointers_return)
{
	...
	status = artec48u_device_read(reader->dev, reader->pixel_buffer,
				      &size);
	if (status != SANE_STATUS_GOOD)
		return status;
		  ...
		  unsigned int **buffer_pointers_return)
{
	...
	buffer = DELAY_BUFFER_READ_PTR(&reader->g_delay);
	buffer_pointers_return[0] = buffer;
	unpack_16_le_mono(reader->pixel_buffer, buffer,
...
artec48u_line_reader_init_delays(Artec48U_Line_Reader * reader)
{
	...
	reader->delays_initialized = TRUE;
...
artec48u_line_reader_free_delays(Artec48U_Line_Reader * reader)
{
	...
	if (reader->delays_initialized) {
		...
		reader->delays_initialized = FALSE;
	}
}
			 ...
			 Artec48U_Line_Reader ** reader_return)
{
	...
	*reader_return = NULL;
	...
	reader = (Artec48U_Line_Reader *)
		malloc(sizeof(Artec48U_Line_Reader));
	if (!reader) {
...
{
	DECLARE_FUNCTION_NAME("artec48u_scanner_read_line") SANE_Status
		status;
	int i, j, c;
		...
		for (i = c - 1; i >= 0; i--) {
			for (j = 0; j < 3; j++) {
				...
				new_value =
					(double) (value -
						  s->
						  shading_buffer_black[j][i])
					* 65535.0 /
					(double) (s->
						  shading_buffer_white[j][i] -
						  s->
						  shading_buffer_black[j][i]);
				if (new_value < 0)
					new_value = 0;
...
artec48u_scanner_internal_start_scan(Artec48U_Scanner * s)
{
	...
	if (status != SANE_STATUS_GOOD) {
		XDBG((2,
		      "%s: artec48u_scanner_wait_for_positioning error: %s\n",
		      function_name, sane_strstatus(status)));
		return status;
	}
	...
	for (repeat_count = 0; repeat_count < 30 * 10; ++repeat_count) {
		...
		if (status != SANE_STATUS_GOOD) {
			XDBG((2,
			      "%s: artec48u_device_read_scanned_data error: %s\n",
			      function_name, sane_strstatus(status)));
			return status;
		}
		if (ready)
				     ...
				     Artec48U_Scan_Parameters * params)
{
	...
	if (action == SA_SCAN)
		status = artec48u_setup_scan(s, request, action, FALSE,
					     params);
	if (status != SANE_STATUS_GOOD) {
	...
	if (status != SANE_STATUS_GOOD) {
		XDBG((2,
		      "%s: artec48u_scanner_internal_start_scan failed: %s\n",
		      function_name, sane_strstatus(status)));
		return status;
	}
...
artec48u_scanner_start_scan(Artec48U_Scanner * s,
			    ...
			    Artec48U_Scan_Parameters * params)
{
	return artec48u_scanner_start_scan_extended(s, request, SA_SCAN,
						    params);
}
...
init_options(Artec48U_Scanner * s)
{
	...
	s->opt[OPT_GAMMA].size = sizeof(int);
	...
	s->opt[OPT_DEFAULT_ENHANCEMENTS].constraint_type =
	...
	s->opt[OPT_CALIBRATE].desc =
		SANE_I18N
		("If enabled, the device will be calibrated before the "
	...
	s->val[OPT_CALIBRATE_SHADING].w = FALSE;
#ifdef ARTEC48U_USE_BUTTONS
	...
	for (cnt = 0; cnt < 65536; cnt++) {
		if (bright < 0.0)
			s->brightness_array[cnt] =
				(int) (((double) cnt * (65535.0 + bright)) /
				       65535.0);
		else
			s->brightness_array[cnt] =
				(int) ((double) cnt +
				       ((65535.0 -
					 (double) cnt) * bright) / 65535.0);
...
calculate_contrast(Artec48U_Scanner * s)
{
	...
	for (cnt = 0; cnt < 65536; cnt++) {
		if (contr < 0.0) {
			...
			val = (int) (32769.0 *
				     pow((double) (val ? val : 1) / 32769.0,
					 (32769.0 + contr) / 32769.0));
			s->contrast_array[cnt] =
				(cnt > 32769) ? (65535 - val) : val;
			...
			p = ((int) contr ==
			     32769) ? 32769.0 : 32769.0 / (32769.0 - contr);
			val = (int) (32769.0 *
				     pow((double) val / 32769.0, p));
			s->contrast_array[cnt] =
				(cnt > 32639) ? (65535 - val) : val;
			if (s->contrast_array[cnt] > 65535)
				s->contrast_array[cnt] = 65535;
			if (s->contrast_array[cnt] < 0)
				s->contrast_array[cnt] = 0;
...
calibrate_scanner(SANE_Handle handle)
{
	...
	if ((s->val[OPT_CALIBRATE].w == TRUE) &&
		...
		while (finish == 0) {
			...
			artec48u_setup_scan(s, &(s->request),
			...
			for (c = 0; c < s->dev->shading_lines_b; c++) {
				artec48u_scanner_read_line(s, buffer_pointers,
							   FALSE);
			...
			XDBG((1, "avg_r: %i, avg_g: %i, avg_b: %i\n",
			      avg_black[0], avg_black[1], avg_black[2]));
			/*adjust offset */
			for (c = 0; c < 3; c++) {
				if (c == 0) {
			...
			artec48u_scanner_stop_scan(s);
			finish_exposure_buffer(s, &avg_white[0],
					       &avg_white[1], &avg_white[2]);
			s->scanning = FALSE;
			XDBG((1, "avg_r: %i, avg_g: %i, avg_b: %i\n",
			      avg_white[0], avg_white[1], avg_white[2]));
			for (c = 0; c < 3; c++) {
				if (c == 0) {
   ...
   *******************************/
	...
	for (c = 0; c < s->dev->shading_lines_b; c++) {
		artec48u_scanner_read_line(s, buffer_pointers, FALSE);
		add_to_shading_buffer(s, buffer_pointers);
	}
	artec48u_scanner_stop_scan(s);
	finish_shading_buffer(s, FALSE);
	s->scanning = FALSE;
...
reader_process(void *data)
{
	...
	if (sigaction(SIGTERM, &act, 0) < 0)
		XDBG((2,
		      "(child) reader_process: sigaction(SIGTERM,...) failed\n"));

	act.sa_handler = usb_reader_process_sigterm_handler;
	if (sigaction(SIGUSR1, &act, 0) < 0)
		XDBG((2,
		      "(child) reader_process: sigaction(SIGUSR1,...) failed\n"));
	...
	while (s->lines_to_read > 0) {
		if (cancelRead == TRUE) {
			XDBG((2,
			      "(child) reader_process: cancelRead == TRUE\n"));
			s->scanning = FALSE;
			s->eof = FALSE;
		...
		if (s->scanning != TRUE) {
			XDBG((2,
			      "(child) reader_process: scanning != TRUE\n"));
			return SANE_STATUS_CANCELLED;
		}
		status = artec48u_scanner_read_line(s, s->buffer_pointers,
						    TRUE);
		if (status != SANE_STATUS_GOOD) {
			XDBG((2,
			      "(child) reader_process: scanner_read_line failed\n"));
			return SANE_STATUS_IO_ERROR;
		}
		copy_scan_line(s);
		s->lines_to_read -= 1;
		bytes_written =
			write(fd, s->line_buffer,
			      s->sane_params.bytes_per_line);

		if (bytes_written < 0) {
			XDBG((2,
			      "(child) reader_process: write returned %s\n",
			      strerror(errno)));
			s->eof = FALSE;
			return SANE_STATUS_IO_ERROR;
		}
...
load_calibration_data(Artec48U_Scanner * s)
{
	...
	if (strlen(filename) <
	    (PATH_MAX - 1 - strlen("artec48ushading_black")))
	...
	strcpy(filename, path);
	if (strlen(filename) <
	    (PATH_MAX - 1 - strlen("artec48ushading_white")))
	...
	if (!f)
		return SANE_STATUS_INVAL;
	/*read values */
	cnt = fread(&s->dev->artec_48u_afe_params,
		    sizeof(Artec48U_AFE_Parameters), 1, f);
	if (cnt != 1) {
		fclose(f);
	...
	if (!f)
		return SANE_STATUS_INVAL;
	/*read values */
	cnt = fread(&s->dev->artec_48u_exposure_params,
...
save_calibration_data(Artec48U_Scanner * s)
{
	...
	if (chmod(filename, mode) != 0)
		return SANE_STATUS_INVAL;
	/*read values */
	cnt = fwrite(&s->dev->artec_48u_afe_params,
		     sizeof(Artec48U_AFE_Parameters), 1, f);
	if (cnt != 1) {
		fclose(f);
	...
	if (chmod(filename, mode) != 0)
		return SANE_STATUS_INVAL;
	/*read values */
	cnt = fwrite(&s->dev->artec_48u_exposure_params,
		     sizeof(Artec48U_Exposure_Parameters), 1, f);
	if (cnt != 1) {
		fclose(f);
	...
	if (devicename[0]) {
		for (dev = first_dev; dev; dev = dev->next) {
			if (strcmp(dev->sane.name, devicename) == 0) {
				XDBG((2,
				      "sane_open: found matching device %s\n",
				      dev->sane.name));
				break;
			}
		}
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	XDBG((8,
	      ...
	      (void *) handle, option, action, (void *) value,
	      (void *) info));
	...
	case SANE_ACTION_SET_VALUE:
		...
		case OPT_RESOLUTION:
			if (s->dev->is_epro != 0) {
				if ((s->val[option].w == 1200)
				    && (*(int *) value < 1200)) {
					s->opt[OPT_BIT_DEPTH].constraint.
						word_list = bitdepth_list;
					*info |= SANE_INFO_RELOAD_OPTIONS;
				} else if ((s->val[option].w < 1200)
					   && (*(int *) value == 1200)) {
					s->opt[OPT_BIT_DEPTH].constraint.
						word_list = bitdepth_list2;
					if (s->val[OPT_BIT_DEPTH].w > 8)
						s->val[OPT_BIT_DEPTH].w = 8;
					*info |= SANE_INFO_RELOAD_OPTIONS;
				}
			}
			s->val[option].w = *(int *) value;
			if (info) {
		...
		case OPT_DEFAULT_ENHANCEMENTS:
			s->val[OPT_GAMMA].w = SANE_FIX(s->dev->gamma_master);
			if (strcmp(s->val[OPT_SCAN_MODE].s, mode_list[2]) ==
			    0) {
				s->val[OPT_GAMMA_R].w =
					SANE_FIX(s->dev->gamma_r);
				s->val[OPT_GAMMA_G].w =
					SANE_FIX(s->dev->gamma_g);
				s->val[OPT_GAMMA_B].w =
					SANE_FIX(s->dev->gamma_b);
			}
			s->val[OPT_BRIGHTNESS].w = 0;
			s->val[OPT_CONTRAST].w = 0;
			if (info)
	...
	case SANE_ACTION_GET_VALUE:
		...
		case OPT_BUTTON_STATE:
			status = artec48u_check_buttons(s->dev,
							&button_state);
			if (status == SANE_STATUS_GOOD) {
...
/*  int scan_mode;*/
	...
	s->request.color = TRUE;
	if ((strcmp(str, mode_list[0]) == 0)
	    || (strcmp(str, mode_list[1]) == 0))
		s->request.color = FALSE;
	else
		s->request.color = TRUE;
...
/*DBG(1, "sane_get_params: scan_mode %i\n",scan_mode);*/
	...
	s->params.lineart = FALSE;
	if (s->params.color == TRUE) {
		...
		if (strcmp(str, mode_list[0]) == 0) {
			...
			s->params.lineart = TRUE;
		}
	}
	if ((resx == 1200) && (s->dev->is_epro == 0)) {
		if (params->depth == 1)
			params->bytes_per_line =
				(s->params.pixel_xs * 2 + 7) / 8;
		else
...
sane_start(SANE_Handle handle)
{
	...
	if ((s->calibrated != TRUE)
	    || (s->val[OPT_CALIBRATE].w == TRUE)) {
	...
	if ((s->params.ydpi == 1200) && (s->dev->is_epro == 0)) {
		if (s->request.color == TRUE) {
			s->line_buffer =
				(SANE_Byte *) malloc(s->params.scan_bpl * 8);
		} else {
			s->line_buffer =
				(SANE_Byte *) malloc(s->params.scan_bpl * 4);
			/*lineart ? */
			if (strcmp(s->val[OPT_SCAN_MODE].s, mode_list[0]) ==
			    0)
				s->lineart_buffer =
					(SANE_Byte *) malloc(s->params.
							     pixel_xs * 2);
		}
	} else {
		if (s->request.color == TRUE)
			...
			s->line_buffer =
				(SANE_Byte *) malloc(s->params.scan_bpl * 2);
			/*lineart ? */
			if (strcmp(s->val[OPT_SCAN_MODE].s, mode_list[0]) ==
			    0)
				s->lineart_buffer =
	...
	while (sanei_config_read(str, sizeof(str), fp)) {
		...
		if (0 == strncmp(str, "option", 6)) {
			...
			decodeVal(str, "masterGamma", _FLOAT,
				  &gamma_master_default, &gamma_m);
			decodeVal(str, "redGamma", _FLOAT, &gamma_r_default,
				  &gamma_r);
			decodeVal(str, "greenGamma", _FLOAT, &gamma_g_default,
				  &gamma_g);
			decodeVal(str, "blueGamma", _FLOAT, &gamma_b_default,
				  &gamma_b);
			decodeVal(str, "redOffset", _BYTE,
				  &afe_params.r_offset,
				  &default_afe_params.r_offset);
			decodeVal(str, "greenOffset", _BYTE,
				  &afe_params.g_offset,
				  &default_afe_params.g_offset);
			decodeVal(str, "blueOffset", _BYTE,
				  &afe_params.b_offset,
				  &default_afe_params.b_offset);

			decodeVal(str, "redExposure", _INT,
				  &exp_params.r_time,
				  &default_exp_params.r_time);
			decodeVal(str, "greenExposure", _INT,
				  &exp_params.g_time,
				  &default_exp_params.g_time);
			decodeVal(str, "blueExposure", _INT,
				  &exp_params.b_time,
				  &default_exp_params.b_time);

			decodeVal(str, "modelString", _STRING, model_string,
				  model_string);
			decodeVal(str, "vendorString", _STRING, vendor_string,
				  vendor_string);

			decodeVal(str, "artecFirmwareFile", _STRING,
			...
			if (temp[0] != 0) {
				...
				XDBG((3, "      vendor: %s\n",
				      vendor_string));
				XDBG((3, "      model: %s\n", model_string));
				sanei_usb_attach_matching_devices(temp,
								  attach_one_device);
			}
			/*save config line in temp */
			strcpy(temp, str);
		} else if (0 == strncmp(str, "device", 6)) {
			if (TRUE == decodeDevName(str, devName)) {
				if (devName[0] != 0)
					sanei_usb_attach_matching_devices
						(devName, attach_one_device);
				temp[0] = 0;
			}
		} else {
			/* ignore other stuff... */
