...
cap_data[] = {
	...
	, {
		"A6000C", "flatbed", 8.3, 14, 8, 55, 8192,
 ...
 */
	...
	, {
		"AT6", "flatbed",
			...
			ARTEC_FLAG_CALIBRATE_RGB |
			ARTEC_FLAG_RGB_LINE_OFFSET |
			ARTEC_FLAG_RGB_CHAR_SHIFT | ARTEC_FLAG_OPT_CONTRAST |
 ...
 */
	ARTEC_FLAG_SEPARATE_RES |
			...
			"50,100,200,300", "50,100,200,300,600"}
	, {
		"AT12", "flatbed", 8.5, 11, 12, 67, 32768,
 ...
 */
	...
	, {
		"AM12S", "flatbed", 8.26, 11.7, 12, 67, ARTEC_MAX_READ_SIZE,
/* calibration works slower so disabled
   ARTEC_CALIBRATE_DARK_WHITE |
 */
/* gamma not working totally correct yet.
 ...
 */
	ARTEC_FLAG_RGB_LINE_OFFSET |
			...
			ARTEC_FLAG_MBPP_NEGATIVE |
			ARTEC_FLAG_ONE_PASS_SCANNER,
			"50,100,300,600", "50,100,300,600,1200"}
,};
...
static char *
artec_skip_whitespace(char *str)
{
	while (isspace(*str))
		++str;
	return str;
...
artec_str_list_to_word_list(int ** word_list_ptr, char * str)
{
	...
	if ((str == NULL) || (strlen(str) == 0)) {
		...
		word_list = (int *) malloc(sizeof(int));
		if (word_list == NULL)
	...
	word_list = (int *) calloc(comma_count + 1, sizeof(int));
...
sense_handler(int fd, u_char * sense, void *arg)
{
	...
	case 0x70:		/* ALWAYS */
		...
		case 0x02:
			DBG(2,
			    "sense:  Not Ready, target can not be accessed\n");
			return SANE_STATUS_IO_ERROR;
		case 0x03:
			DBG(2,
			    "sense:  Medium Error, paper jam or misfeed during ADF\n");
			return SANE_STATUS_IO_ERROR;
		case 0x04:
		...
		case 0x05:
			DBG(2,
			    "sense:  Illegal Request, bad parameter in command block\n");
			return SANE_STATUS_IO_ERROR;
		case 0x06:
...
abort_scan(SANE_Handle handle)
{
	...
	data[2] =
		0x02 | ((s->val[OPT_TRANSPARENCY].w == TRUE) ? 0x04 :
			0x00) | ((s->val[OPT_ADF].w ==
				  TRUE) ? 0x00 : 0x01);
	...
	data[2] = ((s->val[OPT_TRANSPARENCY].w == TRUE) ? 0x04 : 0x00) |
		((s->val[OPT_ADF].w == TRUE) ? 0x00 : 0x01);
	sanei_scsi_cmd(s->fd, comm, 6 + comm[4], 0, 0);
...
artec_reverse_line(SANE_Handle handle, SANE_Byte * data)
{
	...
	if (s->params.format == SANE_FRAME_RGB) {	/* RGB format */
		...
		if (s->params.depth == 8) {	/* 256 color gray-scale */
			for (from = tmp_buf, to = data + len; to >= data;
			     to--, from++) {
				*to = *from;
			}
			...
			for (from = tmp_buf, to = data + len; to >= data;
			     to--, from++) {
				*to = (((*from & 0x01) << 7) |
				       ((*from & 0x02) << 5) | ((*from & 0x04)
								<< 3) |
				       ((*from & 0x08) << 1) | ((*from & 0x10)
								>> 1) |
				       ((*from & 0x20) >> 3) | ((*from & 0x40)
								>> 5) |
				       ((*from & 0x80) >> 7));
			}
		}
...
#if 0
static SANE_Status
artec_byte_rgb_to_line_rgb(SANE_Byte * data, int len)
{
	SANE_Byte tmp_buf[32768];	/* max dpi 1200 * 8.5 inches * 3 = 30600 */
	int count, from;
...
static SANE_Status
artec_line_rgb_to_byte_rgb(SANE_Byte * data, int len)
{
	SANE_Byte tmp_buf[32768];	/* max dpi 1200 * 8.5 inches * 3 = 30600 */
	int count, to;
...
static SANE_Status
artec_buffer_line_offset(SANE_Handle handle, int line_offset,
			 SANE_Byte * data, size_t * len)
{
	...
	if (tmp_line_buf == NULL) {
		...
		if (tmp_line_buf == NULL) {
			DBG(1,
			    "couldn't allocate memory for temp line buffer\n");
			return (SANE_STATUS_NO_MEM);
		}
		...
		if (line_buffer == NULL) {
			DBG(1,
			    "couldn't allocate memory for line buffer pointers\n");
			return (SANE_STATUS_NO_MEM);
		}

		for (count = 0; count < r_buf_lines; count++) {
			line_buffer[count] =
				malloc((*len) * sizeof(SANE_Byte));
			if (line_buffer[count] == NULL) {
				DBG(1,
				    "couldn't allocate memory for line buffer %d\n",
				    count);
				return (SANE_STATUS_NO_MEM);
			}
		}
	...
	if (r_buf_lines > 0) {
		if (cur_line > r_buf_lines) {
			...
			if (s->hw->flags & ARTEC_FLAG_RGB_CHAR_SHIFT) {
				...
				memcpy(tmp_line_buf + width,
				       &line_buffer[line_offset][width],
				       width);
			} else {
...
artec_read_gamma_table(SANE_Handle handle)
{
	...
	if ((!strcmp(s->hw->sane.model, "AT12")) ||
		...
		return (sanei_scsi_cmd
			(s->fd, write_6, 10 + 8 + s->gamma_length, 0, 0));
	} else {
		return (sanei_scsi_cmd
			(s->fd, write_6, 10 + 9 + s->gamma_length, 0, 0));
	}
}
#endif
...
artec_send_gamma_table(SANE_Handle handle)
{
	...
	if (!s->val[OPT_CUSTOM_GAMMA].w) {
		...
		if ((!strcmp(s->hw->sane.model, "AT12")) ||
		    (!strcmp(s->hw->sane.model, "AM12S"))) {
			return (sanei_scsi_cmd
				(s->fd, write_6, 10 + 8 + s->gamma_length, 0,
				 0));
		} else {
			return (sanei_scsi_cmd
				(s->fd, write_6, 10 + 9 + s->gamma_length, 0,
				 0));
		}
	}
}
...
artec_set_scan_window(SANE_Handle handle)
{
	...
	if (s->hw->flags & ARTEC_FLAG_HALFTONE_PATTERN) {
		data[35] = artec_get_str_index(halftone_pattern_list, s->val[OPT_HALFTONE_PATTERN].s);	/* halftone pattern */
	}
	...
	data[48] = artec_get_str_index(filter_type_list, s->val[OPT_FILTER_TYPE].s);	/* filter mode */
...
artec_software_rgb_calibrate(SANE_Handle handle, SANE_Byte * buf, int lines)
{
	...
	for (line = 0; line < lines; line++) {
		...
		for (loop = 0; loop < s->params.pixels_per_line; loop++) {
			if ((DBG_LEVEL == 100) && (loop < 100)) {
				DBG(100,
				    "  %2d-%4d R (%4d,%4d): %d * %5.2f = %d\n",
				    line, loop, i, offset, buf[i],
				    s->
				    soft_calibrate_data[ARTEC_SOFT_CALIB_RED]
				    [offset],
				    (int) (buf[i] *
					   s->
					   soft_calibrate_data
					   [ARTEC_SOFT_CALIB_RED][offset]));
			}
			buf[i] = buf[i] *
				s->soft_calibrate_data[ARTEC_SOFT_CALIB_RED]
				[offset];
			i++;
			...
			buf[i] = buf[i] *
				s->soft_calibrate_data[ARTEC_SOFT_CALIB_GREEN]
				[offset];
			i++;
			...
			if ((DBG_LEVEL == 100) && (loop < 100)) {
				DBG(100,
				    ...
				    i, offset, buf[i],
				    s->
				    soft_calibrate_data[ARTEC_SOFT_CALIB_BLUE]
				    [offset],
				    (int) (buf[i] *
					   s->
					   soft_calibrate_data
					   [ARTEC_SOFT_CALIB_BLUE][offset]));
			}
			buf[i] = buf[i] *
				s->soft_calibrate_data[ARTEC_SOFT_CALIB_BLUE]
				[offset];
			i++;
...
artec_calibrate_shading(SANE_Handle handle)
{
	...
	size_t len;
	int save_x_resolution;
	int save_pixels_per_line;
	int i;
	...
	if (s->hw->flags & ARTEC_FLAG_CALIBRATE_RGB) {
		...
		for (i = 0; i < 2592; i++) {
			s->soft_calibrate_data[ARTEC_SOFT_CALIB_RED][i] =
				255.0 /
				((buf[i] + buf[i + 2592] + buf[i + 5184] +
				  buf[i + 7776]) / 4);
			if (DBG_LEVEL == 100) {
				DBG(100,
				    "   %4d: 255.0 / (( %3d + %3d + %3d + %3d ) / 4 ) = %5.2f\n",
				    i, buf[i], buf[i + 2592], buf[i + 5184],
				    buf[i + 7776],
				    s->
				    soft_calibrate_data[ARTEC_SOFT_CALIB_RED]
				    [i]);
...
artec_get_cap_data(ARTEC_Device * dev, int fd)
{
	...
	if (dev->support_cap_data_retrieve) {	/* DB */
		...
		dev->width =
			((float) ((cap_buf[55] << 8) | cap_buf[56])) / 1000;
		dev->height =
			((float) ((cap_buf[57] << 8) | cap_buf[58])) / 1000;
	...
	DBG(9, "  X Range(mm) : %d-%d\n",
	    dev->x_range.min, (int) (SANE_UNFIX(dev->x_range.max)));
	DBG(9, "  Y Range(mm) : %d-%d\n",
	    dev->y_range.min, (int) (SANE_UNFIX(dev->y_range.max)));
	...
	DBG(9, "  Calib Method: %s\n",
	    ...
	    dev->
	    flags & ARTEC_FLAG_CALIBRATE_DARK_WHITE ? "white/black" : "N/A");
...
attach(const char *devname, ARTEC_Device ** devp)
{
	...
	if (result[0] != 0x6) {
		DBG(1,
		    "attach: device doesn't look like a scanner at all.\n");
		sanei_scsi_close(fd);
		return (SANE_STATUS_INVAL);
	}
	...
	if ((result[36] == 0x00) &&
		...
		DBG(6,
		    "Found BlackWidow BW4800SP scanner, setting up like AT3\n");
	...
	if ((strncmp(result + 8, "        ", 8) == 0) &&
		...
		DBG(6,
		    "Found Plustek 19200S scanner, setting up like AM12S\n");
	...
	if ((strncmp(result + 8, "ULTIMA", 6) != 0) &&
		...
		DBG(1,
		    "attach: device doesn't look like a Artec/ULTIMA scanner\n");
	...
	*t = '\0';
	...
	if (strncmp(result + 36, "ULTIMA  ", 8) == 0) {
		...
		dev->support_cap_data_retrieve = TRUE;
	} else {
		DBG(5,
		    "scanner does NOT support read capability data function\n");
		dev->support_cap_data_retrieve = FALSE;
	}
...
init_options(ARTEC_Scanner * s)
{
	...
	s->val[OPT_RESOLUTION_BIND].w = TRUE;
	...
	s->opt[OPT_PREVIEW].size = sizeof(int);
	...
	s->opt[OPT_GRAY_PREVIEW].size = sizeof(int);
	s->val[OPT_GRAY_PREVIEW].w = FALSE;
	...
	s->opt[OPT_PIXEL_AVG].desc =
		"Enable HardWare Pixel Averaging function";
	s->opt[OPT_PIXEL_AVG].type = SANE_TYPE_BOOL;
	s->val[OPT_PIXEL_AVG].w = FALSE;
	...
	s->opt[OPT_EDGE_ENH].desc =
		"Enable HardWare Lineart Line Edge Enhancement";
	s->opt[OPT_EDGE_ENH].type = SANE_TYPE_BOOL;
	s->val[OPT_EDGE_ENH].w = FALSE;
	...
	s->opt[OPT_GAMMA_VECTOR].size = s->gamma_length * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_R].size = s->gamma_length * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_G].size = s->gamma_length * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_B].size = s->gamma_length * sizeof(int);
	...
	s->val[OPT_TRANSPARENCY].w = FALSE;
	...
	s->opt[OPT_QUALITY_CAL].desc =
		"Perform hardware calibration on every scan";
	...
	s->opt[OPT_SOFTWARE_CAL].desc =
		"Perform software quality calibration in "
		"addition to hardware calibration";
	s->opt[OPT_SOFTWARE_CAL].type = SANE_TYPE_BOOL;
	s->val[OPT_SOFTWARE_CAL].w = FALSE;
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	while (sanei_config_read(dev_name, sizeof(dev_name), fp)) {
		...
		if ((strncmp(cp, "vendor", 6) == 0) && isspace(cp[6])) {
			...
			DBG(5,
			    "sane_init: Forced vendor string '%s' in %s.\n",
...
sane_open(const char * devicename, SANE_Handle * handle)
{
	...
	for (j = 0; j < s->gamma_length; ++j) {
		s->gamma_table[0][j] =
			j * (s->gamma_length - 1) / s->gamma_length;
	}
	...
	*handle = s;
	...
	if (s->hw->flags & ARTEC_FLAG_CALIBRATE) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "error opening scanner for initial calibration: %s\n",
			    sane_strstatus(status));
			s->fd = -1;
			return status;
		}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_MODE:
		{
			...
			s->val[option].s = (char *) strdup(val);
			...
			if (strcmp(val, "Lineart") == 0) {
				...
				if (s->hw->
				    flags & ARTEC_FLAG_ENHANCE_LINE_EDGE)
					s->opt[OPT_EDGE_ENH].cap &=
						~SANE_CAP_INACTIVE;
			} else if (strcmp(val, "Halftone") == 0) {
				/* Halftone mode */
				if (s->hw->
				    flags & ARTEC_FLAG_HALFTONE_PATTERN)
				...
				if (!
				    (s->hw->
				     flags & ARTEC_FLAG_MBPP_NEGATIVE)) {
					s->opt[OPT_NEGATIVE].cap |=
						SANE_CAP_INACTIVE;
				}
			} else if (strcmp(val, "Color") == 0) {
				/* Color mode */
				s->opt[OPT_FILTER_TYPE].cap |=
					SANE_CAP_INACTIVE;
				s->opt[OPT_SOFTWARE_CAL].cap &=
					~SANE_CAP_INACTIVE;
				if (!
				    (s->hw->
				     flags & ARTEC_FLAG_MBPP_NEGATIVE)) {
		...
		case OPT_RESOLUTION_BIND:
			if (s->val[option].w != *(int *) val) {
				...
				if (s->val[option].w == FALSE) {	/* don't bind */
					s->opt[OPT_Y_RESOLUTION].cap &=
						~SANE_CAP_INACTIVE;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		if ((s->val[OPT_RESOLUTION_BIND].w == TRUE) ||
		    (s->val[OPT_PREVIEW].w == TRUE)) {
...
sane_start(SANE_Handle handle)
{
	...
	if ((strcmp(s->mode, "Color") != 0) ||
		...
		if (s->hw->flags & ARTEC_FLAG_SENSE_HANDLER) {
			status = sanei_scsi_open(s->hw->sane.name, &s->fd,
						 sense_handler, (void *) s);
		} else {
			status = sanei_scsi_open(s->hw->sane.name, &s->fd, 0,
						 0);
		}
	...
	DBG(9, "%d pixels per line, %d bytes, %d lines high, xdpi = %d, "
	    ...
	    s->params.lines, s->x_resolution, s->y_resolution,
...
static SANE_Status
artec_sane_read(SANE_Handle handle, SANE_Byte * buf, int max_len,
		int * len)
{
	ARTEC_Scanner *s = handle;
	SANE_Status status;
	...
	*len = 0;
	...
	remaining_rows =
		(s->bytes_to_read + s->params.bytes_per_line -
		 1) / s->params.bytes_per_line;
	...
	while ((rows_read < max_ret_rows) && (rows_read < remaining_rows)) {
		DBG(50, "top of while loop, rr = %lu, mrr = %lu, rem = %lu\n",
		    (u_long) rows_read, (u_long) max_ret_rows,
		    (u_long) remaining_rows);

		if (s->bytes_to_read - bytes_read <=
		    s->params.bytes_per_line * max_read_rows) {
		...
		DBG(50,
		    ...
		    (u_long) rows_available, s->params.lines,
		    s->params.bytes_per_line);
		DBG(50, "bytes_to_read = %lu, max_len = %d, max_rows = %lu\n",
		    (u_long) s->bytes_to_read, max_len,
		    (u_long) max_ret_rows);
		DBG(50,
		    "nread = %lu, lread = %lu, bytes_read = %lu, rows_read = %lu\n",
		    (u_long) nread, (u_long) lread, (u_long) bytes_read,
		    (u_long) rows_read);
		...
		if ((strcmp(s->mode, "Color") == 0) &&
			...
			for (line = 0; line < lread; line++) {
				...
				artec_buffer_line_offset(s, s->line_offset,
							 line_buf, &nread);

				if (nread > 0) {
					if (s->hw->
					    flags & ARTEC_FLAG_RGB_CHAR_SHIFT)
					{
					...
					if (s->hw->
					    flags & ARTEC_FLAG_IMAGE_REV_LR) {
						artec_reverse_line(s,
								   line_buf);
					}

					/* do software calibration if necessary */
					if (s->val[OPT_SOFTWARE_CAL].w) {
						artec_software_rgb_calibrate
							(s, line_buf, 1);
					}
			...
			if ((s->val[OPT_SOFTWARE_CAL].w) &&
			    (strcmp(s->mode, "Color") == 0)) {
				artec_software_rgb_calibrate(s, temp_buf,
							     lread);
			}
	  ...
	  int * len)
{
	...
	if (bytes_in_buf != 0) {
		bytes_to_copy =
			max_len < bytes_in_buf ? max_len : bytes_in_buf;
	} else {
		status = artec_sane_read(s, temp_buf, s->hw->max_read_size,
					 len);
