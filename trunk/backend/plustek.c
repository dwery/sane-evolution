...
reader_process(void *args)
{
	...
	if ((int) status < 0) {
		DBG(_DBG_ERROR,
		    "reader_process: read failed, status = %i, errno %i\n",
		    (int) status, lerrn);
...
initGammaSettings(Plustek_Scanner * s)
{
	...
	for (i = 0; i < 4; i++) {
		...
		switch (i) {
		case 1:
			gamma = s->hw->adj.rgamma;
			break;
		case 2:
			gamma = s->hw->adj.ggamma;
			break;
		case 3:
			gamma = s->hw->adj.bgamma;
			break;
		default:
			gamma = s->hw->adj.graygamma;
			break;
		}
...
init_options(Plustek_Scanner * s)
{
	...
	s->opt[OPT_LAMPSWITCH].desc =
		SANE_I18N("Manually switching the lamp(s).");
	s->opt[OPT_LAMPSWITCH].type = SANE_TYPE_BOOL;
	s->val[OPT_LAMPSWITCH].w = FALSE;

	s->opt[OPT_LOFF4DARK].name = "lamp-off-during-dcal";
	s->opt[OPT_LOFF4DARK].title =
		SANE_I18N("Lamp off during dark calibration");;
	s->opt[OPT_LOFF4DARK].desc =
		SANE_I18N
	...
	s->opt[OPT_CACHECAL].desc =
		SANE_I18N("Enables or disables calibration data cache.");
	...
	s->opt[OPT_CALIBRATE].size = sizeof(int);
	...
	s->opt[OPT_SPEEDUP].desc =
		SANE_I18N("Enables or disables speeding up sensor movement.");
	...
	s->opt[OPT_LAMPOFF_TIMER].desc =
		SANE_I18N("Lampoff-time in seconds.");
	...
	s->opt[OPT_OVR_REDOFS].desc =
		SANE_I18N("Red offset value of the AFE");
	...
	s->opt[OPT_OVR_GREENGAIN].desc =
		SANE_I18N("Green gain value of the AFE");
	...
	s->opt[OPT_OVR_GREENOFS].desc =
		SANE_I18N("Green offset value of the AFE");
	...
	s->opt[OPT_OVR_BLUEGAIN].desc =
		SANE_I18N("Blue gain value of the AFE");
	...
	s->opt[OPT_OVR_BLUEOFS].desc =
		SANE_I18N("Blue offset value of the AFE");
	...
	s->opt[OPT_OVR_RED_LOFF].desc =
		SANE_I18N("Defines red lamp off parameter");
	...
	s->opt[OPT_OVR_GREEN_LOFF].desc =
		SANE_I18N("Defines green lamp off parameter");
	...
	s->opt[OPT_OVR_BLUE_LOFF].desc =
		SANE_I18N("Defines blue lamp off parameter");
...
attach(const char *dev_name, CnfDef * cnf, Plustek_Device ** devp)
{
	...
	handle = usbDev_open(dev, usbDevs, FALSE);
	if (handle < 0) {
	 ...
	 */
	dev->res_list = (int *)
		calloc((((dev->usbDev.Caps.OpticDpi.x * 16) - _DEF_DPI) / 25 +
			1), sizeof(int));
	...
	dev->res_list_size = 0;
	for (cntr = _DEF_DPI; cntr <= (dev->usbDev.Caps.OpticDpi.x * 16);
	     cntr += 25) {
		dev->res_list_size++;
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	while (sanei_config_read(str, sizeof(str), fp)) {
		...
		if (0 == strncmp(str, "option", 6)) {
			...
			ival = -1;
			decodeVal(str, "warmup", _INT, &config.adj.warmup,
				  &ival);
			decodeVal(str, "lampOff", _INT, &config.adj.lampOff,
				  &ival);
			decodeVal(str, "lOffOnEnd", _INT,
				  &config.adj.lampOffOnEnd, &ival);
			decodeVal(str, "posShadingY", _INT,
				  &config.adj.posShadingY, &ival);
			decodeVal(str, "tpaShadingY", _INT,
				  &config.adj.tpaShadingY, &ival);
			decodeVal(str, "negShadingY", _INT,
				  &config.adj.negShadingY, &ival);
			decodeVal(str, "red_gain", _INT, &config.adj.rgain,
				  &ival);
			decodeVal(str, "green_gain", _INT, &config.adj.ggain,
				  &ival);
			decodeVal(str, "blue_gain", _INT, &config.adj.bgain,
				  &ival);
			decodeVal(str, "red_offset", _INT, &config.adj.rofs,
				  &ival);
			decodeVal(str, "green_offset", _INT, &config.adj.gofs,
				  &ival);
			decodeVal(str, "blue_offset", _INT, &config.adj.bofs,
				  &ival);
			decodeVal(str, "red_lampoff", _INT,
				  &config.adj.rlampoff, &ival);
			decodeVal(str, "green_lampoff", _INT,
				  &config.adj.glampoff, &ival);
			decodeVal(str, "blue_lampoff", _INT,
				  &config.adj.blampoff, &ival);

			ival = 0;
			decodeVal(str, "enableTPA", _INT,
				  &config.adj.enableTpa, &ival);
			decodeVal(str, "cacheCalData", _INT,
				  &config.adj.cacheCalData, &ival);
			decodeVal(str, "altCalibration", _INT,
				  &config.adj.altCalibrate, &ival);
			decodeVal(str, "skipCalibration", _INT,
				  &config.adj.skipCalibration, &ival);
			decodeVal(str, "skipFine", _INT, &config.adj.skipFine,
				  &ival);
			decodeVal(str, "skipFineWhite", _INT,
				  &config.adj.skipFineWhite, &ival);
			decodeVal(str, "skipDarkStrip", _INT,
				  &config.adj.skipDarkStrip, &ival);
			decodeVal(str, "incDarkTarget", _INT,
				  &config.adj.incDarkTgt, &ival);
			decodeVal(str, "invertNegatives", _INT,
				  &config.adj.invertNegatives, &ival);
			decodeVal(str, "disableSpeedup", _INT,
				  &config.adj.disableSpeedup, &ival);

			decodeVal(str, "posOffX", _INT, &config.adj.pos.x,
				  &ival);
			decodeVal(str, "posOffY", _INT, &config.adj.pos.y,
				  &ival);

			decodeVal(str, "negOffX", _INT, &config.adj.neg.x,
				  &ival);
			decodeVal(str, "negOffY", _INT, &config.adj.neg.y,
				  &ival);

			decodeVal(str, "tpaOffX", _INT, &config.adj.tpa.x,
				  &ival);
			decodeVal(str, "tpaOffY", _INT, &config.adj.tpa.y,
				  &ival);

			decodeVal(str, "mov", _INT, &config.adj.mov, &ival);

			dval = 1.0;
			decodeVal(str, "grayGamma", _FLOAT,
				  &config.adj.graygamma, &dval);
			decodeVal(str, "redGamma", _FLOAT, &config.adj.rgamma,
				  &dval);
			decodeVal(str, "greenGamma", _FLOAT,
				  &config.adj.ggamma, &dval);
			decodeVal(str, "blueGamma", _FLOAT,
			...
			if (config.devName[0] != '\0') {
				...
				if (first_dev != NULL) {
					DBG(_DBG_WARNING,
					    "section contains no device name,"
					    " ignored!\n");
				}
			}
		...
		} else if (TRUE == decodeDevName(str, config.devName)) {
			continue;
		}
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_GET_VALUE:
		...
		case OPT_EXT_MODE:
			strcpy((char *) value,
			       s->opt[option].constraint.string_list[s->
								     val
								     [option].
								     w]);
			break;
	...
	case SANE_ACTION_SET_VALUE:
		...
		optval = NULL;
		if (SANE_CONSTRAINT_STRING_LIST ==
		    s->opt[option].constraint_type) {
		...
		case OPT_CALIBRATE:
			if (s->calibrating) {
				...
				s->w_pipe = -1;
				s->reader_pid =
					sanei_thread_begin(do_calibration, s);
				s->calibrating = TRUE;
		...
		case OPT_CUSTOM_GAMMA:
			...
			if (TRUE == s->val[option].w) {
				DBG(_DBG_INFO,
				    "Using custom gamma settings.\n");
				...
				DBG(_DBG_INFO,
				    "NOT using custom gamma settings.\n");
				initGammaSettings(s);
		...
		case OPT_LAMPOFF_TIMER:
			s->val[option].w = (*(int *) value);
			adj->lampOff = (*(int *) value);
			usb_CheckAndCopyAdjs(dev);
			break;

		case OPT_WARMUPTIME:
			s->val[option].w = (*(int *) value);
			adj->warmup = (*(int *) value);
			usb_CheckAndCopyAdjs(dev);
			break;
		...
		case OPT_OVR_RED_LOFF:
			s->val[option].w = (*(int *) value);
			adj->rlampoff = (*(int *) value);
			break;
		case OPT_OVR_GREEN_LOFF:
			s->val[option].w = (*(int *) value);
			adj->glampoff = (*(int *) value);
			break;
		case OPT_OVR_BLUE_LOFF:
			s->val[option].w = (*(int *) value);
			adj->blampoff = (*(int *) value);
			break;
		...
		case OPT_MODE:
			s->val[option].w =
				optval -
				s->opt[option].constraint.string_list;
			scanmode = getScanMode(s);
		...
		case OPT_EXT_MODE:{
			...
			if (s->val[option].w == 0) {
				...
				if (s->val[option].w == 1) {
					...
					s->val[OPT_TL_X].w =
						SANE_FIX(_DEFAULT_TP_TLX);
					s->val[OPT_TL_Y].w =
						SANE_FIX(_DEFAULT_TP_TLY);
					s->val[OPT_BR_X].w =
						SANE_FIX(_DEFAULT_TP_BRX);
					s->val[OPT_BR_Y].w =
						SANE_FIX(_DEFAULT_TP_BRY);
					...
					s->val[OPT_TL_X].w =
						SANE_FIX(_DEFAULT_NEG_TLX);
					s->val[OPT_TL_Y].w =
						SANE_FIX(_DEFAULT_NEG_TLY);
					s->val[OPT_BR_X].w =
						SANE_FIX(_DEFAULT_NEG_BRX);
					s->val[OPT_BR_Y].w =
						SANE_FIX(_DEFAULT_NEG_BRY);
				}
				s->opt[OPT_MODE].constraint.string_list =
					&mode_list[2];
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	 ...
	 */
	if ((NULL == params) || (s->scanning != TRUE)) {
...
local_sane_start(Plustek_Scanner * s, int scanmode)
{
	 ...
	 */
	dev->fd = usbDev_open(dev, NULL, TRUE);
	if (dev->fd < 0) {
	...
	case 1:
		crop.ImgDef.dwFlag |= SCANDEF_Transparency;
		break;
	case 2:
		crop.ImgDef.dwFlag |= SCANDEF_Negative;
		break;
	default:
		break;
	}
	...
	if (scanmode <= COLOR_GRAY16) {
		usbDev_setMap(dev, s->gamma_table[0], s->gamma_length,
			      _MAP_MASTER);
	} else {
		usbDev_setMap(dev, s->gamma_table[1], s->gamma_length,
			      _MAP_RED);
		usbDev_setMap(dev, s->gamma_table[2], s->gamma_length,
			      _MAP_GREEN);
		usbDev_setMap(dev, s->gamma_table[3], s->gamma_length,
			      _MAP_BLUE);
	}
...
sane_set_io_mode(SANE_Handle handle, SANE_Bool non_blocking)
{
	...
	if (fcntl(s->r_pipe, F_SETFL, non_blocking ? O_NONBLOCK : 0) < 0) {
		DBG(_DBG_ERROR,
		    "ERROR: could not set to non-blocking mode !\n");
		return SANE_STATUS_IO_ERROR;
	}
