...
typedef enum
{ ltHome, ltUnknown, ltBed, ltError } TLineType;
...
GetLineType(TInstance * this)
{
	...
	while (i < MAX_PIXEL_PER_SCANLINE && iHole < 3) {
		int c;
		while (i < MAX_PIXEL_PER_SCANLINE
		       && puchBuffer[i] > BLACK_HOLE_GRAY)
			i++;	/* not very black */
		c = 0;
...
FakeCalibration(TInstance * this)
{
	...
	if (!this->calibration.achStripeY) {
		this->calibration.achStripeY =
			calloc(1, MAX_PIXEL_PER_SCANLINE);
		if (!this->calibration.achStripeY)
			return SetError(this, SANE_STATUS_NO_MEM,
					"no memory for calib Y");
	}
...
static int
CompareProc(const unsigned char *p1, const unsigned char *p2)
{
	return *p1 - *p2;
}
#endif
...
DoCalibration(TInstance * this)
{
...
#ifdef SM3600_CALIB_USE_RMS
	for (i = 0; i < MAX_PIXEL_PER_SCANLINE; i++)
		this->calibration.achStripeY[i] =
			(unsigned char) (int) sqrt(aulSum[i] / cStripes);
#endif
#ifdef SM3600_CALIB_USE_MEDIAN
	...
	for (i = 0; i < MAX_PIXEL_PER_SCANLINE; i++) {
		...
		qsort(auchRow, cStripes, sizeof(unsigned char),
		      (TQSortProc) CompareProc);
		this->calibration.achStripeY[i] = auchRow[(cStripes - 1) / 2];
	}
#endif
#ifdef SM3600_CALIB_APPLY_HANNING_WINDOW
	memcpy(auchHanning, this->calibration.achStripeY,
	       sizeof(auchHanning));
	for (i = 1; i < MAX_PIXEL_PER_SCANLINE - 1; i++)
		this->calibration.achStripeY[i] = (unsigned char)
			((2 * (int) auchHanning[i] + auchHanning[i - 1] +
			  auchHanning[i + 1]) / 4);
#endif
...
__SM3600EXPORT__ TState
DoOriginate(TInstance * this, TBool bStepOut)
{
	TLineType lt;
	if (this->bVerbose)
	...
	DoJog(this, 1);
	INST_ASSERT();		/* Correction for 1 check line */
...
__SM3600EXPORT__ TState
DoJog(TInstance * this, int nDistance)
{
	int cSteps;
	int nSpeed, nRest;
	...
	cSteps = (nDistance > 0) ? nDistance : -nDistance;
	{
		unsigned char uchRegs2587[] = {
			...
			0x00 /*0x28 */ , 0x00 /*0x29 */ , 0x4E /*0x2A */ ,
			0xF0 /*0x2B */ , 0x00 /*0x2C */ , 0x00 /*0x2D */ ,
			0x4E /*0x2E */ , 0x88 /*R_CCAL */ ,
			0x88 /*R_CCAL2 */ ,
			0x84 /*R_CCAL3 */ , 0xEA /*R_LEN */ ,
			0x24 /*R_LENH */ ,
			0x63 /*0x34 */ , 0x29 /*0x35 */ , 0x00 /*0x36 */ ,
			0x00 /*0x37 */ , 0x00 /*0x38 */ , 0x00 /*0x39 */ ,
			0x00 /*0x3A */ , 0x00 /*0x3B */ , 0xFF /*0x3C */ ,
	...
	if (cSteps > 600) {
		nRest = cSteps;
		for (nSpeed = 0x9800; nRest > 600 && nSpeed >= 0x4000;
		     nSpeed -= 0x800) {
			nRest = RegRead(this, R_POS, 2);
			usleep(100);
			/* perhaps 40C0 is the fastest possible value */
			RegWrite(this, 0x47, 2,
				 nSpeed > 0x4000 ? nSpeed : 0x40C0);
		}
	}
	INST_ASSERT();
	usleep(100);
