...
static char *hp_backend_revision = "$Revision: 1.21 $";
/* Changes:
...
hp_device_config_add(const char *devname)
{
	...
	if (config) {
		...
		DBG(3,
		    "hp_device_config_add: No configuration found for device %s.\n\tUseing default\n",
		    devname);
...
sanei_hp_device_info_get(const char *devname)
{
	...
	do {
		...
		DBG(1,
		    "hp_device_info_get: device %s not configured. Using default\n",
		    devname);
...
sanei_hp_device_get(const char *devname)
{
	...
	for (ptr = global.device_list; ptr; ptr = ptr->next)
		if (strcmp
		    (sanei_hp_device_sanedevice(ptr->dev)->name,
		     devname) == 0)
			return ptr->dev;
...
hp_get_dev(const char *devname, HpDevice * devp)
{
	...
	if (hp_connect == HP_CONNECT_SCSI)
		connect = "scsi";
	else if (hp_connect == HP_CONNECT_DEVICE)
		connect = "device";
	else if (hp_connect == HP_CONNECT_PIO)
		connect = "pio";
	else if (hp_connect == HP_CONNECT_USB)
		connect = "usb";
	else if (hp_connect == HP_CONNECT_RESERVE)
		connect = "reserve";
	else
		connect = "unknown";
...
hp_attach_matching_devices(HpDeviceConfig * config, const char *devname)
{
	...
	if (strncmp(devname, "usb", 3) == 0) {
		...
		DBG(1,
		    "hp_attach_matching_devices: usb attach matching \"%s\"\n",
		    devname);
		if (!usb_initialized) {
			sanei_usb_init();
			usb_initialized = 1;
...
hp_read_config(void)
{
	...
	DBG(1,
	    "hp_read_config: hp backend v%s/%s starts reading config file\n",
	...
	if ((fp = sanei_config_open(HP_CONFIG_FILE)) != 0) {
		while (sanei_config_read(buf, sizeof(buf), fp)) {
			...
			if ((strcmp(arg1, "option") == 0) && (nargs >= 2)) {
				if (strcmp(arg2, "connect-scsi") == 0) {
				...
				} else if (strcmp(arg2, "connect-device") ==
					   0) {
					...
					config->got_connect_type = 1;
					config->use_scsi_request = 0;
				} else if (strcmp(arg2, "connect-reserve") ==
					   0) {
					...
					config->use_scsi_request = 0;
					   ...
					   (arg2,
					    "disable-scsi-request") == 0) {
					config->use_scsi_request = 0;
				} else if (strcmp
					   (arg2,
					    "enable-image-buffering") == 0) {
					config->use_image_buffering = 1;
				} else if (strcmp(arg2, "dumb-read") == 0) {
					config->dumb_read = 1;
				} else {
					DBG(1,
					    "hp_read_config: Invalid option %s\n",
					    arg2);
				}
		...
		if (cu_device[0] != '\0') {	/* Did we work on a device ? */
			memcpy(hp_global_config_get(), &dev_config,
			       sizeof(dev_config));
			DBG(1, "hp_read_config: attach %s\n", cu_device);
			hp_attach_matching_devices(hp_global_config_get(),
						   cu_device);
			cu_device[0] = '\0';
		}
		fclose(fp);
