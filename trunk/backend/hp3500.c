		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	*info = 0;
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_RESOLUTION:
			*(int *) val = scanner->resolution;
			return SANE_STATUS_GOOD;

		case OPT_TL_X:
			*(int *) val = scanner->request_mm.left;
			return SANE_STATUS_GOOD;

		case OPT_TL_Y:
			*(int *) val = scanner->request_mm.top;
			return SANE_STATUS_GOOD;

		case OPT_BR_X:
			*(int *) val = scanner->request_mm.right;
			return SANE_STATUS_GOOD;

		case OPT_BR_Y:
			*(int *) val = scanner->request_mm.bottom;
			return SANE_STATUS_GOOD;
		...
		switch (option) {
		case OPT_RESOLUTION:
			if (scanner->resolution == *(int *) val) {
				return SANE_STATUS_GOOD;
			}
			scanner->resolution = (*(int *) val);
			calculateDerivedValues(scanner);
			*info |= SANE_INFO_RELOAD_PARAMS;
			return SANE_STATUS_GOOD;

		case OPT_TL_X:
			if (scanner->request_mm.left == *(int *) val)
				return SANE_STATUS_GOOD;
			scanner->request_mm.left = *(int *) val;
			calculateDerivedValues(scanner);
			if (scanner->actual_mm.left !=
			    scanner->request_mm.left)
				*info |= SANE_INFO_INEXACT;
			*info |= SANE_INFO_RELOAD_PARAMS;
			return SANE_STATUS_GOOD;

		case OPT_TL_Y:
			if (scanner->request_mm.top == *(int *) val)
				return SANE_STATUS_GOOD;
			scanner->request_mm.top = *(int *) val;
			calculateDerivedValues(scanner);
		...
		case OPT_BR_X:
			if (scanner->request_mm.right == *(int *) val) {
				return SANE_STATUS_GOOD;
			}
			scanner->request_mm.right = *(int *) val;
			calculateDerivedValues(scanner);
			if (scanner->actual_mm.right !=
			    scanner->request_mm.right)
				*info |= SANE_INFO_INEXACT;
			*info |= SANE_INFO_RELOAD_PARAMS;
			return SANE_STATUS_GOOD;

		case OPT_BR_Y:
			if (scanner->request_mm.bottom == *(int *) val) {
				return SANE_STATUS_GOOD;
			}
			scanner->request_mm.bottom = *(int *) val;
			calculateDerivedValues(scanner);
			if (scanner->actual_mm.bottom !=
			    scanner->request_mm.bottom)
				*info |= SANE_INFO_INEXACT;
			*info |= SANE_INFO_RELOAD_PARAMS;
			return SANE_STATUS_GOOD;
			...
			for (i = 0; scan_mode_list[i]; ++i) {
				if (!strcmp
				    ((char const *) val,
				     scan_mode_list[i])) {
					DBG(10,
					    "Setting scan mode to %s (request: %s)\n",
					    scan_mode_list[i],
					    (char const *) val);
					scanner->mode = i;
					return SANE_STATUS_GOOD;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	params->format =
		(scanner->mode ==
		 HP3500_COLOR_SCAN) ? SANE_FRAME_RGB : SANE_FRAME_GRAY;
...
init_options(struct hp3500_data *scanner)
{
	...
	if (!scan_mode_list[0]) {
		scan_mode_list[HP3500_COLOR_SCAN] =
			SANE_VALUE_SCAN_MODE_COLOR;
		scan_mode_list[1] = 0;
#if 0
		scan_mode_list[HP3500_GRAY_SCAN] = SANE_VALUE_SCAN_MODE_GRAY;
		scan_mode_list[HP3500_LINEART_SCAN] =
...
do_cancel(struct hp3500_data *scanner)
{
	if (scanner->reader_pid != -1) {
		...
		if (sanei_thread_kill(scanner->reader_pid) == 0) {
			...
			sanei_thread_waitpid(scanner->reader_pid,
					     &exit_status);
		}
...
rt_execute_commands(void)
{
	...
	result = sanei_usb_write_bulk(udh, /* 0x02, */ command_buffer,
				      &bytes);
		...
		if (result == SANE_STATUS_GOOD) {
			...
			for (i = 0, readptr = readbuf;
				...
				memcpy(command_readmem_outstanding[i],
				       readptr,
				       command_readbytes_outstanding[i]);
			}
		}
	}
		 ...
		 int bytes, void const *data_, int readbytes, void *readdata)
{
	...
	*buffer++ = count;
	...
	if (readbytes) {
		command_readbytes_outstanding[command_reads_outstanding] =
			readbytes;
		command_readmem_outstanding[command_reads_outstanding] =
			readdata;
...
{
	rt_queue_command(command, reg, count, bytes, data, readbytes,
			 readdata);
...
rt_set_register_immediate(int reg, int bytes, void *data)
{
	if (reg < 0xb3 && reg + bytes > 0xb3) {
		...
		if (rt_set_register_immediate(reg, bytes_in_first_block, data)
		    < 0
		    || rt_set_register_immediate(0xb4,
						 bytes -
						 bytes_in_first_block - 1,
						 (char *) data +
						 bytes_in_first_block + 1) <
		    0)
			return -1;
...
rt_write_sram(int bytes, void *data_)
{
	...
	while (bytes > 0) {
		...
		if (rt_send_command_immediate
		    (RTCMD_WRITESRAM, 0, now, now, data, 0, 0) < 0)
			return -1;
		bytes -= now;
		data += now;
	}
...
rt_read_sram(int bytes, void *data_)
{
	...
	while (bytes > 0) {
		...
		if (rt_send_command_immediate
		    (RTCMD_READSRAM, 0, bytes, 0, 0, bytes, data) < 0)
			return -1;
		bytes -= now;
		data += now;
	}
...
rt_detect_sram(unsigned *totalbytes, unsigned char *r93setting)
{
	...
	for (i = 0; i < 16; ++i) {
		...
		if (rt_set_sram_page(pagesetting) < 0 ||
			...
			return -1;
		if (i) {
			if (rt_set_sram_page(0) < 0
			    || rt_read_sram(32, read_data) < 0)
				return -1;
		       ...
		       int enable, int frequency, int offduty)
{
	...
	regs[0x3b] =
		(regs[0x3b] & 0x80) | ((frequency & 0x7) << 4) | (offduty &
								  0x0f);
...
rt_nvram_enable_write(int addressingbits, int enable, unsigned char stdbits)
{
	...
	if (addressingbits > 1)
		rt_nvram_set_addressing_bits(cmdbuffer + 8, 0,
					     addressingbits - 1, stdbits);
...
rt_nvram_write(int block, int location, char const *data, int bytes)
{
	...
	while (bytes--) {
		...
		rt_nvram_set_addressing_bits(address_bits, location,
					     addressingbits, stdbits);
		rt_nvram_set_addressing_bits(data_bits, *data++, 8, stdbits);

		if (rt_nvram_enable_controller(1) < 0 ||
		    rt_send_command_immediate(RTCMD_NVRAMCONTROL, 0, cmdsize,
...
rt_nvram_read(int block, int location, unsigned char *data, int bytes)
{
	...
	while (bytes--) {
		...
		if (rt_nvram_enable_controller(1) < 0 ||
		    rt_send_command_immediate(RTCMD_NVRAMCONTROL, 0x1d,
					      cmdsize, cmdsize, cmdbuffer, 0,
					      0) < 0)
			return -1;
		...
		for (i = 0; i < 8; ++i) {
			...
			if (rt_send_command_immediate
			    (RTCMD_NVRAMCONTROL, 0x1d, 2, 2, readbit_command,
			     0, 0) < 0
			    || rt_read_register_immediate(0x10, 1, &r) < 0)
				return -1;
			if (r & 0x80)
...
dump_registers(unsigned char const *regs)
{
	...
	DBG(5, "    Scanning distance:               %u\n",
	    get_lsbfirst_int(regs + 0x62, 2) - get_lsbfirst_int(regs + 0x60,
								2));
	DBG(5, "    Direction:                       %s\n",
	...
	if (!regs[0x7a])
		DBG(5,
		    "    Horizontal resolution:           Denominator is zero!\n");
	else
	...
	if (regs[0x7a]) {
		...
		pixels = (long) (get_lsbfirst_int(regs + 0x62, 2) -
				 get_lsbfirst_int(regs + 0x60,
						  2)) *
			(long) (get_lsbfirst_int(regs + 0x6c, 2) -
	       ...
	       detailed_calibration_data const *detailed_calib_data)
{
	...
	buffered_rows =
		red_green_offset + green_blue_offset + intra_channel_offset +
		1;
	...
	for (i = j = 0; i < channels; ++i) {
		if (i == 1)
			...
			j += green_blue_offset;
		if (merged_channels)
			channel_data[i][1 - oddfirst] =
				row_buffer + rowbytes * j + i;
		else
			channel_data[i][1 - oddfirst] =
				row_buffer + rowbytes * j + width * i;
		channel_data[i][oddfirst] =
			channel_data[i][1 - oddfirst] +
			rowbytes * intra_channel_offset;
	}

	while (((n = rt_get_available_bytes()) > 0 || rt_is_moving() > 0)
	       && !cancelled_scan) {
		if (n == 1
		    && (rt_is_moving() || rt_get_available_bytes() != 1))
			n = 0;
		if (n > 0) {
			char buffer[0xffc0];
						...
						if (!rows_to_begin
							...
							for (i = 0;
								...
								for (j = 0;
								     ...
								     channels;
								     ++j) {
									unsigned pix = (unsigned char) channel_data[j][i & 1][i];

									if (detailed_calib_data) {
										unsigned char const *calib_start = detailed_calib_data->channeldata[j] + 2 * detailed_calib_data->resolution_divisor * i / (merged_channels ? channels : 1);
										pix = constrain((int) pix - (int) calib_start[0], 0, 255);
										pix = constrain(pix * calib_start[1] / 0x40, 0, 255);
									}
									*outnow++
										=
										pix;
							...
							for (i = 0;
								...
								for (j = 0;
									...
									channel_data
										...
										+=
										rowbytes;
									if (channel_data[i][j] - row_buffer >= rowbuffer_bytes)
										channel_data
											[i]
											[j]
											-=
		 ...
		 detailed_calibration_data const *detailed_calib_data)
{
	...
	rt_set_channel(regs,
		       (colour ==
			HP3500_COLOR_SCAN) ? RT_CHANNEL_ALL :
		       RT_CHANNEL_GREEN);
	rt_set_single_channel_scanning(regs,
				       (colour ==
					HP3500_LINEART_SCAN) ? 1 : 0);
...
accumfunc(struct dcalibdata *dcd, int bytes, char *data)
{
	...
	while (bytes > 0) {
		if (dcd->firstrowdone)
			dcd->buffers[dcd->channelnow][dcd->pixelnow -
						      dcd->pixelsperrow] = *c;
		if (++dcd->channelnow >= 3) {
			dcd->channelnow = 0;
	     ...
	     unsigned colour, rts8801_callback cbfunc, void *param)
{
	...
	do {
		...
		for (i = 0; i < 3; ++i)
			calib_info[i * 3] = calib_info[i * 3 + 1] =
				aiBestOffset[i];

		cd.buffer = calibbuf;
		cd.space = sizeof(calibbuf);
		DBG(30, "Commencing scan for initial calibration pass\n");
		rts8801_fullscan(iCalibX, iCalibY, iCalibWidth, 2, 600,
				 resolution, HP3500_COLOR_SCAN,
				 (rts8801_callback) storefunc, &cd,
		...
		for (i = 0; i < 3; ++i) {
			...
			if (aiBestOffset[i] >= 255)
				continue;
			sum = sum_channel(calibbuf + iCalibOffset + i,
					  iCalibPixels, 0);
	...
	rts8801_fullscan(iCalibX + 2100, iCalibY, iCalibWidth, 2, 600,
			 resolution, HP3500_COLOR_SCAN,
			 (rts8801_callback) storefunc, &cd, calib_info,
	...
	do {
		...
		dcdt.buffers[1] =
			(tdetail_buffer + w * detailed_calibration_rows);
		dcdt.buffers[2] =
			(dcdt.buffers[1] + w * detailed_calibration_rows);
		dcdt.pixelsperrow = w;
		dcdt.pixelnow = dcdt.channelnow = dcdt.firstrowdone = 0;
		rts8801_fullscan(x, 4, w, detailed_calibration_rows + 1,
				 resolution, resolution, HP3500_COLOR_SCAN,
				 (rts8801_callback) accumfunc, &dcdt,
				 calib_info, RTS8801_F_SUPPRESS_MOVEMENT, -1,
				 -1, -1, -1, 0);
		for (i = 0; i < 3; ++i) {
			int largest = 1;

			for (j = 0; j < w; ++j) {
				int val = calcmedian(dcdt.buffers[i], j, w,
						     detailed_calibration_rows);
	...
	if (colour != HP3500_LINEART_SCAN) {
		...
		detail_buffer =
			(unsigned char *) malloc(w * 3 *
						 detailed_calibration_rows);

		dcd.buffers[0] = detail_buffer;
		dcd.buffers[1] =
			(detail_buffer + w * detailed_calibration_rows);
		dcd.buffers[2] =
			(dcd.buffers[1] + w * detailed_calibration_rows);
		dcd.pixelsperrow = w;
	...
	for (i = 0; i < 3; ++i) {
		int idx =
			(i == 0) ? red_calibration_offset : (i ==
							     1) ?
			green_calibration_offset : blue_calibration_offset;
	...
	rts8801_fullscan(x, y, w, h, resolution, resolution, colour, cbfunc,
			 param, calib_info, 0, red_calibration_offset,
			 green_calibration_offset, blue_calibration_offset,
			 end_calibration_offset, &detailed_calib_data);
