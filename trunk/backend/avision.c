...
static Avision_HWEntry Avision_Device_List[] = {
	...
	{"AVISION", "AV122",
	 ...
	 AV_DOES_KEEP_WINDOW | AV_DOES_KEEP_GAMMA | AV_REAR_OFFSET},
	...
	{"AVISION", "AV610",
	 0x0638, 0x0a19,
	 "Avision", "AV610",
	 AV_GRAY_CALIB_BLUE | AV_ACCEL_TABLE | AV_NO_64BYTE_ALIGN |
	 AV_INT_STATUS | AV_INT_BUTTON},
	/* status="good" */
	...
	{"AVISION", "AV600U",
	 0x0638, 0x0A13,
	 "Avision", "AV600U",
	 AV_MULTI_CALIB_CMD | AV_ADF_BGR_ORDER_INVERT | AV_SOFT_SCALE |
	 AV_INT_STATUS | AV_NO_BUTTON},
	/* comment="1 pass, 600 dpi" */
	/* status="good" */
	...
	{"KODAK", "i40 Scanner",
	 0x040a, 0x6002,
	 "Kodak", "i40",
	 AV_LIGHT_CHECK_BOGUS | AV_NO_GAMMA | AV_NO_TUNE_BOTTOM |
	 AV_INT_BUTTON},
	/* status="basic" */
...
static SANE_Bool disable_gamma_table = FALSE;

/* disable the calibration */
static SANE_Bool disable_calibration = FALSE;
static SANE_Bool force_calibration = FALSE;

/* force scanable areas to ISO(DIN) A4/A3 */
static SANE_Bool force_a4 = FALSE;
static SANE_Bool force_a3 = FALSE;
...
static void
debug_print_raw(int dbg_level, char *info, const uint8_t * data, size_t count)
{
	size_t i;
	...
	for (i = 0; i < count; ++i) {
		DBG(dbg_level,
		    "  [%lu] %1d%1d%1d%1d%1d%1d%1d%1db %3oo %3dd %2xx\n",
		    (u_long) i, BIT(data[i], 7), BIT(data[i], 6), BIT(data[i],
								      5),
		    BIT(data[i], 4), BIT(data[i], 3), BIT(data[i], 2),
		    BIT(data[i], 1), BIT(data[i], 0), data[i], data[i],
		    data[i]);
	}
}

static void
debug_print_hex_raw(int dbg_level, char *info, const uint8_t * data,
		    size_t count)
{
	int address = 0;
	char text[16 * 3 + 1];
...
static void
debug_print_nvram_data(int dbg_level, char *func, nvram_data * nvram)
{
...
static void
debug_print_avdimen(int dbg_level, char *func, Avision_Dimensions * avdimen)
{
	DBG(dbg_level, "%s: hw_xres: %d, hw_yres: %d, line_difference: %d\n",
	    func, avdimen->hw_xres, avdimen->hw_yres,
	    avdimen->line_difference);

	DBG(dbg_level, "%s: tlx: %ld, tly: %ld, brx: %ld, bry: %ld\n",
	    func, avdimen->tlx, avdimen->tly, avdimen->brx, avdimen->bry);

	DBG(dbg_level,
	    "%s: hw_pixel_per_line: %d, hw_lines: %d, hw_bytes_per_line: %d\n",
	    func, avdimen->hw_pixels_per_line, avdimen->hw_lines,
	    avdimen->hw_bytes_per_line);
...
static void
debug_print_params(int dbg_level, char *func, SANE_Parameters * params)
{
...
static void
debug_print_calib_format(int dbg_level, char *func, uint8_t * result)
{
...
static void
debug_print_accel_info(int dbg_level, char *func, uint8_t * result)
{
	...
	DBG(dbg_level, "%s: [16]    ability:%s%s\n",
	    func,
	    BIT(result[16],
		0) ? " TWO_BYTES_PER_ELEM" : " SINGLE_BYTE_PER_ELEM",
...
static void
debug_print_window_descriptor(int dbg_level, char *func,
			      command_set_window_window * window)
{
	...
	debug_print_raw(dbg_level + 1, "window_descriptor: \n",
			...
			sizeof(*window) - sizeof(window->header));
...
static int
write_pnm_header(FILE * f, color_mode m, int depth, int width, int height)
{
...
sense_handler(int fd, u_char * sense, void *arg)
{
	...
	case 0x0b:
		...
		break;
	default:
		sprintf(textbuf, "got unknown sense code 0x%02x",
			(int) sense_key);
		text = textbuf;
	}
...
#define ADDITIONAL_SENSE(asc,ascq,txt)			\
    case ( (asc << 8) + ascq): text = txt; break
		...
		switch ((asc << 8) + ascq) {
			/* normal */
			ADDITIONAL_SENSE(0x00, 0x00,
					 "No additional sense information");
			ADDITIONAL_SENSE(0x00, 0x06,
					 "I/O process terminated");
			ADDITIONAL_SENSE(0x15, 0x01,
					 "Mechanical positioning error");

			ADDITIONAL_SENSE(0x15, 0x02,
					 "Flatbed Home Sensor Error (OKI only");
			ADDITIONAL_SENSE(0x15, 0x03,
					 "ADF Home Sensor Error (OKI only)");
			ADDITIONAL_SENSE(0x15, 0x04, "Lock Error (OKI only)");

			ADDITIONAL_SENSE(0x1a, 0x00,
					 "parameter list lenght error");

			ADDITIONAL_SENSE(0x20, 0x00, "Invaild command");
			ADDITIONAL_SENSE(0x24, 0x00, "Invaild field in CDB");
			ADDITIONAL_SENSE(0x25, 0x00,
					 "Logical unit not supported");
			ADDITIONAL_SENSE(0x26, 0x00,
					 "Invaild field in parameter list");
			ADDITIONAL_SENSE(0x26, 0x01,
					 "parameter not supported");
			ADDITIONAL_SENSE(0x26, 0x02,
					 "parameter value invalid");
			ADDITIONAL_SENSE(0x29, 0x00,
					 "Power-on, reset or bus device reset occurred");
			ADDITIONAL_SENSE(0x2c, 0x02,
					 "Invaild combination of window specified");
			ADDITIONAL_SENSE(0x2f, 0x00,
					 "Command cleared by another initiator");

			ADDITIONAL_SENSE(0x3D, 0x00,
					 "Invalid Bit in Identify Message");

			ADDITIONAL_SENSE(0x43, 0x00, "Message error");
			ADDITIONAL_SENSE(0x44, 0x00,
					 "Internal target failure");
			ADDITIONAL_SENSE(0x44, 0x01,
					 "Flatbed DRAM Error(OKI only)");
			ADDITIONAL_SENSE(0x44, 0x02,
					 "ADF DRAM Error(OKI only)");
			...
			ADDITIONAL_SENSE(0x60, 0x01,
					 "Flatbed Lamp error (Oki only)");
			ADDITIONAL_SENSE(0x60, 0x02,
					 "ADF lamp error (Oki only)");
			ADDITIONAL_SENSE(0x62, 0x00,
			...
			ADDITIONAL_SENSE(0x80, 0x05,
					 "Multi-feed (AV220,Kodak)");
			ADDITIONAL_SENSE(0x80, 0x06,
					 "ADF prefeeding (OKI only)");
			ADDITIONAL_SENSE(0x80, 0x07,
					 "Fltabed cover open (OKI only)";
					 status = SANE_STATUS_COVER_OPEN);
			ADDITIONAL_SENSE(0x80, 0x08,
					 "FW module doesn't match with scanner");
			ADDITIONAL_SENSE(0x80, 0x09,
					 "Papers fed from multiple trays (DM272)");
			ADDITIONAL_SENSE(0x80, 0x0A, "ADF Paper Start");
			ADDITIONAL_SENSE(0x80, 0x0B,
					 "Multiple ADF paper End and Start");
			ADDITIONAL_SENSE(0x80, 0x0C,
					 "Multiple ADF paper End");

			/* film scanner */
			ADDITIONAL_SENSE(0x81, 0x00,
					 "ADF/MFP front door open";
					 status = SANE_STATUS_COVER_OPEN);
			ADDITIONAL_SENSE(0x81, 0x01,
					 "ADF holder cartrige open";
					 status = SANE_STATUS_COVER_OPEN);
			...
			ADDITIONAL_SENSE(0x81, 0x04, "ADF film end";
					 status = SANE_STATUS_NO_DOCS);
			ADDITIONAL_SENSE(0x81, 0x05,
					 "ADF forward feed error");
			...
			ADDITIONAL_SENSE(0xA0, 0x01,
					 "Filter Positioning Error");

			ADDITIONAL_SENSE(0x90, 0x00,
					 "Scanner busy (FW busy)");

		default:
			sprintf(textbuf,
				"Unknown sense code asc: 0x%02x, ascq: 0x%02x",
				(int) asc, (int) ascq);
			text = textbuf;
		}
...
#undef ADDITIONAL_SENSE
		...
		if (sense_key == 0x05) {
			if (sense[15] & (1 << 7)) {
				if (sense[15] & (1 << 6))
					DBG(1,
					    "sense_handler: error in command parameter\n");
				else
					DBG(1,
					    "sense_handler: error in data parameter\n");

				DBG(1,
				    "sense_handler: error in parameter byte: %d, %x\n",
				    get_double(&(sense[16])),
				    get_double(&(sense[16])));
				...
				if (sense[15] & (1 << 3))
					...
					DBG(1,
					    "sense_handler: bit pointer invalid\n");
			}
		}
	}
...
static SANE_Status
avision_open(const char *device_name,
	     Avision_Connection * av_con,
	     SANEI_SCSI_Sense_Handler sense_handler, void *sense_arg)
{
...
static SANE_Status
avision_open_extended(const char *device_name,
		      Avision_Connection * av_con,
		      SANEI_SCSI_Sense_Handler sense_handler,
		      void *sense_arg, int *buffersize)
{
	if (av_con->connection_type == AV_SCSI) {
		return sanei_scsi_open_extended(device_name,
						&(av_con->scsi_fd),
						sense_handler, sense_arg,
						buffersize);
	} else {
...
static SANE_Bool
avision_is_open(Avision_Connection * av_con)
{
	if (av_con->connection_type == AV_SCSI) {
...
static SANE_Status
avision_cmd(Avision_Connection * av_con,
...
#define max_usb_size 256 * 1024	/* or 0x10000, used by AV Windows driver during background raster read, ... ? */
		...
		if (cmd_size < min_usb_size) {
			DBG(1,
			    "filling command to have a length of 10, was: %lu\n",
			    (u_long) cmd_size);
			memcpy(enlarged_cmd, m_cmd, cmd_size);
			memset(enlarged_cmd + cmd_size, 0,
			       min_usb_size - cmd_size);
			m_cmd = enlarged_cmd;
		...
		if (status != SANE_STATUS_GOOD || count != cmd_size) {
			DBG(3,
			    "=== Got error %d trying to write, wrote: %ld. ===\n",
			    status, (long) count);
			...
			if (status != SANE_STATUS_GOOD) {	/* == SANE_STATUS_EOF) */
				...
				if (status != SANE_STATUS_GOOD) {
					DBG(3,
					    "=== Got error %d trying to read status. ===\n",
					    status);
					return SANE_STATUS_IO_ERROR;
				} else
					goto write_usb_cmd;
		...
		if (status == SANE_STATUS_GOOD && dst != NULL
		    && *dst_size > 0) {
			out_count = 0;
			...
			while (out_count < *dst_size) {
				...
				DBG(8, "try to read %lu bytes\n",
				    (u_long) count);
				status = sanei_usb_read_bulk(av_con->usb_dn,
							     &(m_dst
							       [out_count]),
							     &count);
				DBG(8, "read %lu bytes\n", (u_long) count);
		...
		status = avision_usb_status(av_con, /*retry */ 1,
					    status_timeout);
		...
		if (status == SANE_STATUS_INVAL) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(3,
				    "=== Got error %d trying to request sense! ===\n",
				    status);
			} else {
				...
				status = avision_usb_status(av_con, 1,
							    status_timeout);
				...
				if (status != SANE_STATUS_GOOD
					...
					DBG(3,
					    "=== Got error %d trying to read sense! ===\n",
					    status);
				else {
					/* read complete -> call our sense handler */
					status = sense_handler(-1,
							       sense_buffer,
							       0);
				}
			}	/* end read sense data */
...
match_color_mode(Avision_Device * dev, char * name)
{
	...
	for (i = 0; i < AV_COLOR_MODE_LAST; ++i) {
		if (dev->color_list[i] != 0
		    && strcmp(dev->color_list[i], name) == 0) {
			DBG(3, "match_color_mode: found at %d mode: %d\n", i,
			    dev->color_list_num[i]);
...
{
	return s->hw->scanner_type == AV_SHEETFEED
		|| (s->hw->scanner_type == AV_FLATBED
		    && s->source_mode_dim == AV_ADF_DIM);
...
match_source_mode(Avision_Device * dev, char * name)
{
	...
	for (i = 0; i < AV_SOURCE_MODE_LAST; ++i) {
		if (dev->source_list[i] != 0
		    && strcmp(dev->source_list[i], name) == 0) {
			DBG(3, "match_source_mode: found at %d mode: %d\n", i,
			    dev->source_list_num[i]);
...
compute_parameters(Avision_Scanner * s)
{
	...
	if (dev->hw->feature_type & AV_SOFT_SCALE) {
		...
		const int *hw_res_list =
			...
			AV_ASIC_C5 ? hw_res_list_c5 : hw_res_list_generic;

		for (hw_res = hw_res_list;
		     *hw_res && *hw_res < s->avdimen.xres; ++hw_res)
			/* just iterate */ ;
		s->avdimen.hw_xres = *hw_res;

		for (hw_res = hw_res_list;
		     *hw_res && *hw_res < s->avdimen.yres; ++hw_res)
			/* just iterate */ ;
		s->avdimen.hw_yres = *hw_res;

		DBG(3, "sane_compute_parameters: soft scale, hw res: %dx%d\n",
		    s->avdimen.hw_xres, s->avdimen.hw_yres);

		if (!s->avdimen.hw_xres || !s->avdimen.hw_yres) {
			DBG(1,
			    "sane_compute_parameters: no matching HW res for: %dx%d\n",
			    s->avdimen.xres, s->avdimen.yres);
			return SANE_STATUS_INVAL;
		}
	} else {
	...
	DBG(3,
	    "sane_compute_parameters: tlx: %f, tly: %f, brx: %f, bry: %f\n",
	...
	if (color_mode_is_color(s->c_mode) &&
		...
		s->avdimen.line_difference =
			(dev->inquiry_line_difference * s->avdimen.hw_yres) /
		...
		/* limit bry + line_difference to real scan boundary */
		{
			...
			if (s->avdimen.bry + 2 * s->avdimen.line_difference >
			    y_max) {
				DBG(1,
				    "sane_compute_parameters: bry limitted!\n");
				s->avdimen.bry =
					y_max -
	...
	if (s->avdimen.interlaced_duplex
	    && dev->hw->feature_type & AV_REAR_OFFSET) {
	...
	s->avdimen.hw_pixels_per_line -=
		s->avdimen.hw_pixels_per_line % boundary;
	...
	if (s->avdimen.interlaced_duplex)
		s->avdimen.hw_lines -=
			s->avdimen.hw_lines % dev->read_stripe_size;

	s->params.pixels_per_line =
		s->avdimen.hw_pixels_per_line * s->avdimen.xres /
		s->avdimen.hw_xres;
	s->params.lines =
		s->avdimen.hw_lines * s->avdimen.xres / s->avdimen.hw_xres;
	...
	case AV_THRESHOLDED:
		...
		s->avdimen.hw_bytes_per_line =
			s->avdimen.hw_pixels_per_line / 8;
	...
	case AV_GRAYSCALE16:
		s->params.format = SANE_FRAME_GRAY;
		s->avdimen.hw_bytes_per_line =
			s->avdimen.hw_pixels_per_line * 2;
	...
	case AV_TRUECOLOR:
		...
		s->avdimen.hw_bytes_per_line =
			s->avdimen.hw_pixels_per_line * 3;
	...
	case AV_TRUECOLOR16:
		s->params.format = SANE_FRAME_RGB;
		s->avdimen.hw_bytes_per_line =
			s->avdimen.hw_pixels_per_line * 3 * 2;
...
wait_ready(Avision_Connection * av_con, int delay)
{
	...
	for (try = 0; try < 10; ++try) {
		...
		status = avision_cmd(av_con, test_unit_ready,
				     sizeof(test_unit_ready), 0, 0, 0, 0);
		sleep(delay);
...
wait_4_light(Avision_Scanner * s)
{
	...
	for (try = 0; try < 90; ++try) {
		...
		if (result == 1) {
			...
			DBG(3,
			    "wait_4_light: scanner marked as returning bogus values in device-list!!\n");
			return SANE_STATUS_GOOD;
		} else {
...
get_firmware_status(Avision_Connection * av_con)
{
	...
	status = avision_cmd(av_con, &rcmd, sizeof(rcmd), 0, 0, &result,
			     &size);
	if (status != SANE_STATUS_GOOD || size != sizeof(result)) {
...
static SANE_Status
get_power_save_time(Avision_Scanner * s, int * time)
{
	SANE_Status status;
	nvram_data nvram;
...
get_accessories_info(Avision_Scanner * s)
{
	...
	char *adf_model[] = { "Origami", "Oodles", "HP9930", "unknown" };
	...
	status = avision_cmd(&s->av_con, &rcmd, sizeof(rcmd), 0, 0, result,
			     &size);
	if (status != SANE_STATUS_GOOD || size != sizeof(result)) {
...
get_button_status(Avision_Scanner * s)
{
	...
	if (!(dev->hw->feature_type & AV_INT_BUTTON)) {
		...
		DBG(5, "==> (interrupt read) going down ...\n");
		status = sanei_usb_read_int(s->av_con.usb_dn,
					    (uint8_t *) & result, &size);
		DBG(5, "==> (interrupt read) got: %ld\n", (long) size);

		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "get_button_status: interrupt read failed (%s)\n",
			    sane_strstatus(status));
			return SANE_STATUS_GOOD;
		}

		if (size < sizeof(result))
			memset((char *) result.buttons + size, 0,
			       sizeof(result) - size);
		...
		if (size == 1) {	/* AV 210, AV 610 */
			DBG(1,
			    "get_button_status: just one byte, filling the rest\n");

			if (result.press_state > 0) {
				debug_print_raw(6,
						"get_button_status: raw data\n",
						(uint8_t *) & result, size);
			...
			DBG(1,
			    "get_button_status: zero buttons  - filling values ...\n");
...
#define add_token(format,value) do {				     \
      ...
      message += n > 0 ? n : 0;					     \
		...
		if (result.press_state >> 7) {	/* AV220 et.al. bit 6 is long/short press? */
			...
			const unsigned int buttons_pressed =
				result.press_state & 0x7F;
			...
			if (!(dev->hw->feature_type & AV_INT_BUTTON)) {
				...
				DBG(3,
				    "get_button_status: resetting status\n");
				...
				status = avision_cmd(&s->av_con, &scmd,
						     sizeof(scmd),
						     &button_reset,
						     sizeof(button_reset), 0,
						     0);

				if (status != SANE_STATUS_GOOD) {
					DBG(1,
					    "get_button_status: send failed (%s)\n",
					    sane_strstatus(status));
					return status;
				}
			}
			...
			for (i = 0; i < buttons_pressed; ++i) {
				...
				DBG(3,
				    "get_button_status: button %d pressed\n",
				    button);
				if (button >= dev->inquiry_buttons) {
					DBG(1,
					    "get_button_status: button %d not allocated as not indicated in inquiry\n",
					    button);
				} else {
					const char *label =
						string_for_button(s, button);
					if (label)
						add_token("%s", label);
					else
...
get_frame_info(Avision_Scanner * s)
{
	...
	DBG(3, "get_frame_info: [3]  Exposures (if APS): %s\n",
	    ((i =
	      (BIT(result[3], 3) << 1) + BIT(result[2],
					     2)) == 0) ? "Unknown" : (i ==
								      1) ?
	    "15" : (i == 2) ? "25" : "40");
	DBG(3, "get_frame_info: [3]  Film Type (if APS): %s\n",
	    ((i =
	      (BIT(result[1], 3) << 1) + BIT(result[0],
					     2)) == 0) ? "Unknown" : (i ==
								      1) ?
	    "B&W Negative" : (i == 2) ? "Color slide" : "Color Negative");
...
static SANE_Status
set_frame(Avision_Scanner * s, int frame)
{
	struct
	{
		struct command_send cmd;
	...
	if (frame != 0xff && (frame < 1 || frame > dev->frame_range.max)) {
		DBG(1,
		    "set_frame: Illegal frame (%d) requested (min=1, max=%d)\n",
       ...
       Avision_Device ** devp)
{
	...
	DBG(1,
	    "attach: Inquiry gives mfg=%s, model=%s, product revision=%s.\n",
	    mfg, model, rev);
	...
	while (Avision_Device_List[model_num].real_mfg != NULL ||
		...
		if (Avision_Device_List[model_num].scsi_mfg) {
			...
			if (strcmp
			    (mfg,
			     Avision_Device_List[model_num].scsi_mfg) == 0)
				++matches;
		}
		if (Avision_Device_List[model_num].scsi_model) {
			...
			if (strcmp
			    (model,
			     Avision_Device_List[model_num].scsi_model) == 0)
				++matches;
		}
		...
		if ((attaching_hw == &(Avision_Device_List[model_num]) &&
		     matches == match_count) || matches == 2) {
			DBG(1,
			    "attach: Scanner matched entry: %d: \"%s\", \"%s\", 0x%x, 0x%x\n",
			    model_num,
	...
	if (!found) {
		...
		DBG(0,
		    "attach: To: rene@exactcode.de (the Avision backend author)\n");
	...
	dev->sane.vendor =
		dev->hw->real_mfg ? dev->hw->real_mfg : strdup(mfg);
	dev->sane.model =
		dev->hw->real_model ? dev->hw->real_model : strdup(model);
	...
	DBG(3, "attach: [36]    Bitfield:%s%s%s%s%s%s%s color plane\n",
	    ...
	    BIT(result[36], 3) ? " IS_NOT_FLATBED" : "", s);
	...
	DBG(3, "attach: [51]    ESA2:%s%s%s%s%s%s%s%s\n",
	    ...
	    BIT(result[51],
		1) ? " NEW_CAL_METHOD_3x3_MATRIX_(NO_GAMMA_TABLE)" : "",
	    BIT(result[51], 0) ? " ADF_MIRRORS_IMAGE" : "");
	DBG(3, "attach: [52]    ESA3:%s%s%s%s%s%s%s%s\n", BIT(result[52], 7) ? " GRAY_WHITE" : "", BIT(result[52], 6) ? " SUPPORTS_GAIN_CONTROL" : "", BIT(result[52], 5) ? " SUPPORTS_TET" : "",	/* "Text Enhanced Technology" */
	...
	DBG(3, "attach: [61]    bits per channel:%s%s%s%s%s%s%s\n",
	    ...
	    BIT(result[61], 4) ? " 8" : "",
	    BIT(result[61], 3) ? " 10" : "",
	    BIT(result[61], 2) ? " 12" : "", BIT(result[61], 1) ? " 16" : "");

	DBG(3, "attach: [62]    scanner type:%s%s%s%s%s%s\n", BIT(result[62], 7) ? " Flatbed" : "", BIT(result[62], 6) ? " Roller (ADF)" : "", BIT(result[62], 5) ? " Flatbed (ADF)" : "", BIT(result[62], 4) ? " Roller" : "",	/* does not feed multiple pages, AV25 */
	...
	DBG(3, "attach: [94]    ESA5:%s%s%s%s%s%s%s%s\n", BIT(result[94], 7) ? " IGNORE_LINE_DIFFERENCE_FOR_ADF" : "", BIT(result[94], 6) ? " NEEDS_SW_LINE_COLOR_PACK" : "", BIT(result[94], 5) ? " SUPPORTS_DUPLEX_SCAN" : "", BIT(result[94], 4) ? " INTERLACED_DUPLEX_SCAN" : "", BIT(result[94], 3) ? " SUPPORTS_TWO_MODE_ADF_SCANS" : "", BIT(result[94], 2) ? " SUPPORTS_TUNE_SCAN_LENGTH" : "", BIT(result[94], 1) ? " SUPPORTS_SWITCH_STRIP_FOR_DESKEW" : "",	/* Kodak i80 only */
	    BIT(result[94],
		0) ? " SEARCHES_LEADING_SIDE_EDGE_BY_FIRMWARE" : "");

	DBG(3, "attach: [95]    ESA6:%s%s%s%s%s%s%s%s\n", BIT(result[95], 7) ? " SUPPORTS_PAPER_SIZE_AUTO_DETECTION" : "", BIT(result[95], 6) ? " SUPPORTS_DO_HOUSEKEEPING" : "",	/* Kodak i80 only */
	    BIT(result[95], 5) ? " SUPPORTS_PAPER_LENGTH_SETTING" : "",	/* AV220, Kodak */
	    BIT(result[95], 4) ? " SUPPORTS_PRE_GAMMA_LINEAR_CORRECTION" : "", BIT(result[95], 3) ? " SUPPORTS_PREFEEDING" : "",	/*  OKI S9800 */
	...
	DBG(3, "attach: [128]    ESA7:%s%s%s%s%s%s%s%s\n",
	    ...
	    BIT(result[128],
		3) ? " SUPPORTS_READ_WRITE_ABILITY_PARAMETER" : "",
	...
	dev->inquiry_duplex_interlaced = BIT(result[62], 2)
		|| BIT(result[94], 4);
	...
	if (Avision_Device_List[model_num].
	    feature_type & AV_DOES_NOT_KEEP_WINDOW)
	...
	if (Avision_Device_List[model_num].
	    feature_type & AV_DOES_NOT_KEEP_GAMMA)
	...
	if (Avision_Device_List[model_num].
	    feature_type & AV_ADF_BGR_ORDER_INVERT)
	...
	if (dev->inquiry_optical_res > dev->inquiry_max_res) {
		DBG(1,
		    "Inquiry optical resolution > max_resolution, adjusting!\n");
	...
	if (dev->inquiry_max_res == 0) {
		DBG(1,
		    "Inquiry max resolution is invalid, using 1200 dpi!\n");
	...
	DBG(1,
	    "attach: max channels per pixel: %d, max bits per channel: %d\n",
	...
	if (dev->inquiry_background_raster) {
		dev->inquiry_background_raster_pixel =
			get_double(&(result[85])) * dev->inquiry_optical_res /
			AVISION_BASE_RES;
	}
	...
	/* check if x/y ranges are valid :-((( */
	{
		...
		for (mode = AV_NORMAL_DIM; mode < AV_SOURCE_MODE_DIM_LAST;
		     ++mode) {
			if (dev->inquiry_x_ranges[mode] != 0
			    && dev->inquiry_y_ranges[mode] != 0) {
				DBG(3,
				    "attach: x/y-range for mode %d is valid!\n",
				    mode);
				if (force_a4) {
					DBG(1,
				...
				DBG(1,
				    "attach: x/y-range for mode %d is invalid! Using a default.\n",
				    mode);
...
send_tune_scan_length(Avision_Scanner * s)
{
	...
	if (dev->hw->feature_type & AV_NO_TUNE_BOTTOM) {
		DBG(1,
		    "send_tune_scan_length: tune bottom disabled due device list\n");
		return SANE_STATUS_GOOD;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "send_tune_scan_length: send bottom/right failed (%s)\n",
		    sane_strstatus(status));
		return status;
	}
...
get_calib_data(Avision_Scanner * s, uint8_t data_type,
	       uint8_t * calib_data, size_t calib_size)
{
	SANE_Status status;
	uint8_t *calib_ptr;
	       ...
	       uint8_t * dark_data, uint8_t * white_data)
{
	...
	if (BIT(format->ability1, 2)) {
		...
		for (i = 0; i < elements_per_line; ++i) {
			uint16_t value_orig =
				get_double_le(white_data + i * 2);
			uint16_t value_new = value_orig;

			value_new &= 0xffc0;
			value_new |=
				(get_double_le(dark_data + i * 2) >> 10) &
				0x3f;

			DBG(100,
			    "set_calib_data: element %d, dark difference %d\n",
		...
		/* one command (most scanners) */
	{
		...
		if (!conv_out_data) {
			...
			for (channel = 0; channel < 3; ++channel) {
				...
				uint16_t *casted_avg_data =
					(uint16_t *) white_data;

				DBG(3,
				    "set_calib_data_calibration: channel: %i\n",
				    channel);
				...
				DBG(3,
				    "set_calib_data: sending %i bytes now\n",
				    conv_out_size);
				...
				if (status != SANE_STATUS_GOOD) {
					DBG(3,
					    "set_calib_data: send_data failed (%s)\n",
					    sane_strstatus(status));
					/* not return immediately to free mem at the end */
				}
			  ...
			  struct calibration_format *format, uint8_t * data)
{
	...
	if (format->channels == 1) {
		...
		rgb_map_value[0] = rgb_map_value[1] = rgb_map_value[2] =
			rgb_map_value[1];
	}
			   ...
			   struct calibration_format *format, uint8_t * data)
{
	...
	for (i = 0; i < 3; ++i) {
		if (mst[i] == INVALID_WHITE_SHADING) {	/* mst[i] > MAX_WHITE_SHADING) */
			DBG(3,
			    "compute_white_shading_data: target %d invaild (%x) using inquiry (%x)\n",
			    i, mst[i], inquiry_mst);
			mst[i] = inquiry_mst;
		}
		/* some firmware versions seems to return the bytes swapped? */
			...
			DBG(3,
			    "compute_white_shading_data: target %d: bytes swapped.\n",
			    i);
		}
		if (mst[i] < DEFAULT_WHITE_SHADING / 2) {
			DBG(3,
			    "compute_white_shading_data: target %d: too low (%d) usind default (%d).\n",
	...
	for (i = 0; i < elements_per_line; ++i) {
		...
		result = ((int) mst[i % 3] * WHITE_MAP_RANGE /
			  (tmp_data + 0.5));
...
normal_calibration(Avision_Scanner * s)
{
	...
	if (calib_format.flags != 1) {
		DBG(1,
		    "normal_calibration: Scanner claims no calibration needed -> skipped!\n");
		return SANE_STATUS_GOOD;
	}
	...
	if (BIT(calib_format.ability1, 3)) {
		...
		status = get_calib_data(s, 0x66, calib_tmp_data,
					calib_data_size);
		...
		s->dark_avg_data =
			sort_and_average(&calib_format, calib_tmp_data);
		if (!s->dark_avg_data) {
			free(calib_tmp_data);
			return SANE_STATUS_NO_MEM;
		}
	...
	status = get_calib_data(s, read_type, calib_tmp_data,
				calib_data_size);
	...
	if (s->dark_avg_data) {
		...
		DBG(1,
		    "normal_calibration: dark data present - decreasing white average data\n");
...
send_gamma(Avision_Scanner * s)
{
	...
	if (dev->inquiry_asic_type != AV_ASIC_OA980)
		invert_table = (s->c_mode == AV_THRESHOLDED)
			|| (s->c_mode == AV_DITHERED);
	...
	for (color = 0; color < 3 && status == SANE_STATUS_GOOD; ++color) {
		...
		for (j = 0; j < 256; ++j) {	/* little table index */
			...
			v1 = (brightness_contrast_func
			      (brightness, contrast, v1));
			v2 = (brightness_contrast_func
			      (brightness, contrast, v2));
		...
		/* fill the gamma table - (e.g.) if 11bit (old protocol) table */
		{
			...
			if (i < gamma_table_raw_size) {
				DBG(4,
				    "send_gamma: (old protocol) - filling the table.\n");
...
#define INT_PART 10
	...
	int i, a_i;
	double c5_matrix[] = { 1.3042, -0.1475, -0.1567,
	...
	status = avision_cmd(&s->av_con, &scmd, sizeof(scmd), &matrix,
			     sizeof(matrix), 0, 0);
	return status;
}
...
get_acceleration_info(Avision_Scanner * s, struct acceleration_info *info)
{
	...
	if (status != SANE_STATUS_GOOD || size != sizeof(result)) {
		DBG(1,
		    "get_acceleratoin_info: read accel. info failed (%s)\n",
		    sane_strstatus(status));
		return status;
	}
...
send_acceleration_table(Avision_Scanner * s)
{
	...
	do {
		...
		if (accel_info.table_count == 0) {
			DBG(3,
			    "send_acceleration_table: device does not need tables\n");
			return SANE_STATUS_GOOD;
		}
		...
		if (accel_info.target_speed > accel_info.start_speed ||
			...
			DBG(1,
			    "send_acceleration_table: table does not look right.\n");
			return SANE_STATUS_INVAL;
		}

		if (accel_info.ability != 0) {
			DBG(1,
			    "send_acceleration_table: ability non-zero - insert code\n");
			return SANE_STATUS_INVAL;
		}
		   ...
		   constructed this way. "Works for me" -ReneR */
		{
			...
			uint16_t accel_steps =
				accel_info.total_steps -
				accel_info.stable_steps + 1;
			...
			while ((up_lim - low_lim) > 0.0001) {
				...
				table_data[i++] =
					(uint8_t) accel_info.start_speed;

				while (now_count != accel_info.target_speed) {
					now_count_f =
						now_count_f - (now_count_f -
							       accel_info.
							       target_speed) *
						mid;
					now_count =
						(uint16_t) (now_count_f +
							    0.5);
			...
			debug_print_hex_raw(5,
					    "send_acceleration_table: first pass:\n",
					    table_data,
					    accel_info.total_steps);
			...
			/* maybe post fix-up */
			{
				...
				i = 0;
				if (((table_total * accel_info.table_units) %
				     accel_info.base_units) == 0)
					add_count = 0;
				else
					add_count = (accel_info.base_units -
						     ((table_total *
						       accel_info.
						       table_units) %
						      accel_info.base_units))
						/ accel_info.table_units;

				/* add_count should not be bigger than 255 */
				if (add_count > 255) {
					DBG(1,
					    "send_acceleration_table: add_count limitted, was: %d\n",
					    add_count);
					add_count = 255;
				}
				for (i = 0;
				     i < accel_info.total_steps - 1
				     && add_count > 0; i++) {
					uint16_t temp_count =
						255 - table_data[i];
					temp_count =
		...
		status = avision_cmd(&s->av_con, &scmd, sizeof(scmd),
				     (char *) table_data,
				     accel_info.total_steps, 0, 0);
		if (status != SANE_STATUS_GOOD) {
			DBG(3,
			    "send_acceleration_table: send_data failed (%s)\n",
			    sane_strstatus(status));
		}
...
set_window(Avision_Scanner * s)
{
	...
	paralen =
		sizeof(cmd.window.avision) - sizeof(cmd.window.avision.type);
	...
	set_quad(cmd.window.descriptor.ulx,
		 s->avdimen.tlx * base_dpi_abs / s->avdimen.hw_xres);
	set_quad(cmd.window.descriptor.uly,
		 s->avdimen.tly * base_dpi_abs / s->avdimen.hw_yres);

	set_quad(cmd.window.descriptor.width,
		 s->avdimen.hw_pixels_per_line * base_dpi_rel /
		 s->avdimen.hw_xres + 1);
	line_count =
	...
	if (s->avdimen.interlaced_duplex) {
		DBG(2,
		    "set_window: interlaced duplex scan, doubled line count\n");
		line_count *= 2;
	}
	...
	if (dev->inquiry_new_protocol
	    && !(dev->hw->feature_type & AV_FUJITSU)) {
	...
	if (dev->inquiry_background_raster)
		cmd.window.avision.type.normal.background_lines =
			s->val[OPT_BACKGROUND].w;
	    ...
	    s->source_mode, s->source_mode_dim);
	{
		...
		if (adf_mode) {
			...
			if (dev->inquiry_duplex_interlaced) {
				DBG(3,
				    "set_window: interlaced duplex type\n");
				cmd.window.avision.type.normal.bitset3 |=
					(adf_mode << 3);
			} else {	/* HP 2-pass duplex */

				if (adf_mode) {	/* if duplex */
					DBG(3,
					    "set_window: non-interlaced duplex type (HP)\n");
					/* MIRR 0x04 | FLIP 0x02 | DPLX 0x01 ... */
					cmd.window.avision.type.normal.
						bitset3 |= 7;
				}
			}
		}
	...
	if (!(dev->hw->feature_type & AV_FUJITSU)) {
		...
		if (dev->scanner_type == AV_FILM) {
			set_double(cmd.window.avision.type.normal.
				   r_exposure_time, 0x016d);
			set_double(cmd.window.avision.type.normal.
				   g_exposure_time, 0x016d);
			set_double(cmd.window.avision.type.normal.
				   b_exposure_time, 0x016d);
		}
	}
	...
	if (color_mode_is_color(s->c_mode)) {
		...
		if (dev->hw->feature_type & AV_FASTER_WITH_FILTER)
			cmd.window.avision.bitset1 |=
				AVISION_FILTER_GREEN << 3;
		else
			cmd.window.avision.bitset1 |=
				AVISION_FILTER_NONE << 3;
	}
...
get_background_raster(Avision_Scanner * s)
{
	...
	DBG(3,
	    "get_background_raster: native raster pixels: %d, raster bytes_per_line: %d\n",
	...
	background = s->background_raster =
		realloc(s->background_raster,
			size * (s->avdimen.interlaced_duplex ? 2 : 1));
	if (!background)
		return SANE_STATUS_NO_MEM;
	...
	if (s->avdimen.interlaced_duplex
	    && dev->hw->feature_type & AV_2ND_LINE_INTERLACED) {
		set_triple(rcmd.transferlen, bytes_per_line);

		for (i = 0;
		     i <
		     s->val[OPT_BACKGROUND].w *
		     (s->avdimen.interlaced_duplex ? 2 : 1); ++i) {
			uint8_t *dst_raster =
				background + bytes_per_line * (i / 2 +
							       (i % 2) *
							       s->
							       val
							       [OPT_BACKGROUND].
							       w);
			DBG(3,
			    "get_background_raster: interlaced line: %d, %lu bytes\n",
			    i, (u_long) bytes_per_line);

			size = bytes_per_line;
			read_size = size;
			status = avision_cmd(&s->av_con, &rcmd, sizeof(rcmd),
					     0, 0, dst_raster, &read_size);
			if (status != SANE_STATUS_GOOD || read_size != size) {
				DBG(1,
				    "get_background_raster: read raster failed (%s)\n",
				    sane_strstatus(status));
				return status;
			}
		}
		...
		for (i = 0; i < (s->avdimen.interlaced_duplex ? 2 : 1); ++i) {
			...
			DBG(3,
			    "get_background_raster: stripe: %d, %lu bytes\n",
			    i, (u_long) size);
	...
	if (dev->inquiry_needs_line_pack) {
		...
		for (l = 0; l < lines; ++l) {
			...
			memcpy(background + (bytes_per_line * l), tmp_data,
			       bytes_per_line);
		}
	...
	for (i = 0; debug && i < (s->avdimen.interlaced_duplex ? 2 : 1); ++i) {
		...
		write_pnm_header(f,
				 (color_mode_is_color(s->c_mode) ?
				  AV_TRUECOLOR : AV_GRAYSCALE), 8,
				 bytes_per_line / bpp,
				 s->val[OPT_BACKGROUND].w);

		fwrite(raster, 1, bytes_per_line * s->val[OPT_BACKGROUND].w,
		       f);
		fclose(f);
	}
	...
	if (s->avdimen.hw_xres != s->avdimen.xres) {
		...
		for (l = 0; l < lines; ++l) {
			...
			for (x = 0; x < s->params.pixels_per_line; ++x) {
				const double bx =
					(-1.0 +
					 s->avdimen.hw_pixels_per_line) * x /
					s->params.pixels_per_line;
	...
	if (debug) {
		for (i = 0; i < (s->avdimen.interlaced_duplex ? 2 : 1); ++i) {
			...
			write_pnm_header(f,
					 (color_mode_is_color(s->c_mode) ?
					  AV_TRUECOLOR : AV_GRAYSCALE), 8,
					 s->params.bytes_per_line / bpp,
					 s->val[OPT_BACKGROUND].w);

			fwrite(raster, 1,
			       s->params.bytes_per_line *
			       s->val[OPT_BACKGROUND].w, f);
			fclose(f);
		}
	}
...
{
	char cmd[] = { AVISION_SCSI_RESERVE_UNIT, 0, 0, 0, 0, 0 };
	SANE_Status status;
...
{
	char cmd[] = { AVISION_SCSI_RELEASE_UNIT, 0, 0, 0, 0, 0 };
	SANE_Status status;
...
start_scan(Avision_Scanner * s)
{
	...
	if (s->val[OPT_PREVIEW].w == TRUE
	    && s->hw->inquiry_asic_type != AV_ASIC_C7) {
...
read_data(Avision_Scanner * s, SANE_Byte * buf, size_t * count)
{
	...
	status = avision_cmd(&s->av_con, &rcmd, sizeof(rcmd), 0, 0, buf,
			     count);
...
init_options(Avision_Scanner * s)
{
	...
	dev->x_range.max =
		SANE_FIX((int) dev->inquiry_x_ranges[s->source_mode_dim]);
	dev->x_range.quant = 0;
	dev->y_range.max =
		SANE_FIX((int) dev->inquiry_y_ranges[s->source_mode_dim]);
	dev->y_range.quant = 0;
	...
	s->opt[OPT_OVERSCAN_TOP].desc =
		"The top overscan controls the additional area to scan before the paper is detected.";
	...
	s->opt[OPT_OVERSCAN_BOTTOM].desc =
		"The bottom overscan controls the additional area to scan after the paper end was detected.";
	...
	s->opt[OPT_BACKGROUND].desc =
		"The background raster controls the additional background lines to scan before the paper is feed thru the scanner.";
	...
	s->opt[OPT_GAMMA_VECTOR].size = 256 * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_R].size = 256 * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_G].size = 256 * sizeof(int);
	...
	s->opt[OPT_GAMMA_VECTOR_B].size = 256 * sizeof(int);
	...
	s->opt[OPT_POWER_SAVE_TIME].desc =
	...
	s->opt[OPT_MESSAGE].desc =
		"This text contains device specific options controlled ny the user on the scanner hardware.";
...
reader_process(void *data)
{
	...
	if (deinterlace == NONE ||
		...
		if (dev->hw->feature_type & AV_NO_START_SCAN) {
			DBG(1,
			    "reader_process: start_scan skipped due to device-list!\n");
		} else {
			status = start_scan(s);
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "reader_process: start_scan faild: %s\n",
				    sane_strstatus(status));
				return status;
			}
		}
		...
		if (dev->hw->feature_type & AV_ACCEL_TABLE)
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "reader_process: send_acceleration_table failed: %s\n",
				    sane_strstatus(status));
				return status;
			}
			}
	...
	if (deinterlace != NONE) {
		if (!s->duplex_rear_valid) {	/* create new file for writing */
			DBG(3,
			    "reader_process: opening duplex rear file for writing.\n");
			...
			DBG(3,
			    "reader_process: opening duplex rear file for reading.\n");
	...
	if (dev->inquiry_background_raster && s->val[OPT_BACKGROUND].w) {
		...
		fwrite(background, s->params.bytes_per_line,
		       s->val[OPT_BACKGROUND].w, fp);
	}
	...
	while (exit_status == SANE_STATUS_GOOD
		...
		while (!s->duplex_rear_valid &&
			...
			DBG(5,
			    "reader_process: processed_bytes: %lu, total_size: %lu\n",
			    (u_long) processed_bytes, (u_long) total_size);
			DBG(5, "reader_process: this_read: %lu\n",
			    (u_long) this_read);

			sigprocmask(SIG_BLOCK, &sigterm_set, 0);
			status = read_data(s, stripe_data + stripe_fill,
					   &this_read);
			...
			if (status == SANE_STATUS_EOF
				...
				if (!already_eof) {
					DBG(5,
					    "reader_process: first EOF on stripe interlace: hiding.\n");
					status = SANE_STATUS_GOOD;
					already_eof = 1;
				}
			}
			...
			if (raw_fp && this_read > 0)
				fwrite(stripe_data + stripe_fill, this_read,
				       1, raw_fp);

			if (status == SANE_STATUS_EOF || this_read == 0) {
				DBG(1,
				    "reader_process: read_data failed due to EOF\n");
				exit_status = SANE_STATUS_EOF;
			}

			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "reader_process: read_data failed with status: %d\n",
				    status);
				exit_status = status;
			}
		...
		if (s->duplex_rear_valid) {
			...
			DBG(5,
			    "reader_process: virtual processed_bytes: %lu, total_size: %lu\n",
			    (u_long) processed_bytes, (u_long) total_size);
			DBG(5, "reader_process: virtual this_read: %lu\n",
			    (u_long) this_read);

			got = fread(stripe_data + stripe_fill, 1, this_read,
				    rear_fp);
			stripe_fill += got;
			processed_bytes += got;
			if (got != this_read)
		...
		if (deinterlace != NONE && !s->duplex_rear_valid) {
			...
			unsigned int absline =
				(processed_bytes -
				 stripe_fill) / s->avdimen.hw_bytes_per_line;
			unsigned int abslines =
				absline +
				useful_bytes / s->avdimen.hw_bytes_per_line;
			uint8_t *ptr = stripe_data;
			for (; absline < abslines; ++absline) {
				DBG(9,
				    "reader_process: deinterlacing line %d\n",
				    absline);
				/* interlaced? save the back data to the rear buffer */
				if ((deinterlace == STRIPE && absline % (lines_per_stripe * 2) >= lines_per_stripe) || (deinterlace == HALF && absline >= total_size / s->avdimen.hw_bytes_per_line / 2) || (deinterlace == LINE && absline & 0x1)) {	/* last bit equals % 2 */
					DBG(9,
					    "reader_process: saving rear line %d to temporary file.\n",
					    absline);
					fwrite(ptr,
					       s->avdimen.hw_bytes_per_line,
					       1, rear_fp);
			...
			DBG(9,
			    "reader_process: after deinterlacing: useful_bytes: %d, stripe_fill: %d\n",
			    useful_bytes, stripe_fill);
		}
		...
		if ((s->source_mode_dim == AV_ADF_DIM
		     && dev->inquiry_adf_need_mirror)
		    || (s->source_mode == AV_ADF_DUPLEX
			&& dev->inquiry_adf_need_mirror_rear
			&& s->page % 2 == 0)) {
			if ((s->c_mode != AV_TRUECOLOR)
			    || (s->c_mode == AV_TRUECOLOR
				&& dev->inquiry_adf_bgr_order)) {
		...
		if (s->c_mode == AV_GRAYSCALE12 ||
			...
			uint8_t *end_ptr =
				begin_ptr + s->avdimen.hw_bytes_per_line;
			uint8_t *line_ptr;
			...
			while (begin_ptr < end_ptr) {
				...
				if (dark_avg_data)
					dark_avg =
						get_double_le(dark_avg_data);
				if (white_avg_data)
					white_avg =
						get_double_le(white_avg_data);

				line_ptr = begin_ptr;
				for (l = 0; l < lines; ++l) {
					double v = (double)
						get_double_le(line_ptr) *
						scale;
					uint16_t v2;
					if (0)
						v = (v -
		...
		if (s->avdimen.hw_xres == s->avdimen.xres && s->avdimen.hw_yres == s->avdimen.yres) {	/* No scaling */
			...
			for (lines =
				...
				if (deinterlace != NONE) {
					...
					if ((!s->duplex_rear_valid
						...
						s->avdimen.rear_offset)) {
						DBG(7,
						    "reader_process: skip due read offset line: %d\n",
						    line);
						continue;
					}
				}
				fwrite(src, s->avdimen.hw_bytes_per_line, 1,
				       fp);
				++line;
			}
			...
			while (1) {
				...
				if (deinterlace != NONE) {
					if (!s->duplex_rear_valid
					    && syy >= s->avdimen.hw_lines) {
						DBG(7,
						    "reader_process: skip due past intended front page lines: %d\n",
						    sy);
						break;
					} else if (s->duplex_rear_valid) {
						/* the beginning is to be skipped, accessed thru offset */
						DBG(7,
						    "reader_process: rear_offset adjusting source: %d\n",
						    sy);
				...
				if (sy >= hw_line_end || syy >= hw_line_end) {
					DBG(3,
					    "reader_process: source line %d-%d not yet avail\n",
					    sy, syy);
					break;
				}
				...
				if (sy < -1) {
					DBG(1,
					    "reader_process: need more history: %d???\n",
					    sy);
					sy = -1;
				}

				DBG(8,
				    "reader_process: out line: %d <- from: %d-%d\n",
				    line, sy, syy);

				for (x = 0; x < s->params.pixels_per_line;
				     ++x) {
					const double bx =
						(-1.0 +
						 s->avdimen.
						 hw_pixels_per_line) * x /
						s->params.pixels_per_line;
					const int sx = (int) floor(bx);
					const int xdist =
						(int) ((bx - sx) * 256);
				...
				fwrite(ip_data, s->params.bytes_per_line, 1,
				       fp);
				++line;
			}
			/* copy one line of history for the next pass */
			memcpy(ip_history,
			       out_data + useful_bytes -
			       s->avdimen.hw_bytes_per_line,
			       s->avdimen.hw_bytes_per_line);
		}
		...
		if (stripe_fill > 0)
			memcpy(stripe_data, stripe_data + useful_bytes,
			       stripe_fill);
	...
	if (exit_status == SANE_STATUS_EOF && line < s->params.lines) {
		...
		memset(out_data, gray_mode ? 0xff : 0x00,
		       s->params.bytes_per_line);
	...
	if (deinterlace != NONE && s->duplex_rear_valid) {
		DBG(1,
		    "reader_process: virtual duplex scan - no device cleanup!\n");
	} else {
		...
		if (dev->inquiry_new_protocol && dev->scanner_type == AV_FILM) {
			...
			if (status != SANE_STATUS_GOOD)
				DBG(1,
				    "reader_process: object position go-home failed!\n");
		}
	}
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (fp <= (FILE *) 0) {
		...
		while (sanei_config_read(line, sizeof(line), fp)) {
			...
			if (!word) {
				DBG(1,
				    "sane_init: config file line %d: could not be parsed\n",
				    linenumber);
				continue;
			}
			...
			if (strcmp(word, "option") == 0) {
				...
				if (strcmp(word, "disable-gamma-table") == 0) {
					DBG(3,
					    "sane_init: config file line %d: disable-gamma-table\n",
					    linenumber);
					disable_gamma_table = TRUE;
				} else if (strcmp(word, "disable-calibration")
					   == 0) {
					DBG(3,
					    "sane_init: config file line %d: disable-calibration\n",
					    linenumber);
					disable_calibration = TRUE;
				} else if (strcmp(word, "force-calibration")
					   == 0) {
					DBG(3,
					    "sane_init: config file line %d: force-calibration\n",
					    linenumber);
					force_calibration = TRUE;
				} else if (strcmp(word, "force-a4") == 0) {
					DBG(3,
					    "sane_init: config file line %d: enabling force-a4\n",
					    linenumber);
					force_a4 = TRUE;
				} else if (strcmp(word, "force-a3") == 0) {
					DBG(3,
					    "sane_init: config file line %d: enabling force-a3\n",
					    linenumber);
					force_a3 = TRUE;
				} else if (strcmp(word, "static-red-calib") ==
					   0) {
					DBG(3,
					    "sane_init: config file line %d: static red calibration\n",
					    linenumber);
					static_calib_list[0] = TRUE;
				} else if (strcmp(word, "static-green-calib")
					   == 0) {
					DBG(3,
					    "sane_init: config file line %d: static green calibration\n",
					    linenumber);
					static_calib_list[1] = TRUE;
				} else if (strcmp(word, "static-blue-calib")
					   == 0) {
					DBG(3,
					    "sane_init: config file line %d: static blue calibration\n",
					    linenumber);
					static_calib_list[2] = TRUE;
				} else
					DBG(1,
					    "sane_init: config file line %d: options unknown!\n",
					    linenumber);
			} else if (strcmp(word, "usb") == 0) {
				DBG(2,
				    "sane_init: config file line %d: trying to attach USB:`%s'\n",
				    linenumber, line);
				...
				DBG(2,
				    "sane_init: config file line %d: trying to attach SCSI: %s'\n",
				    linenumber, line);
				...
				DBG(1,
				    "sane_init: config file line %d: OBSOLETE !! use the scsi keyword!\n",
				    linenumber);
				DBG(1,
				    "sane_init:   (see man sane-avision for details): trying to attach SCSI: %s'\n",
				    line);
	...
	while (Avision_Device_List[model_num].scsi_mfg != NULL ||
		...
		if (attaching_hw->scsi_mfg != NULL)
			sanei_scsi_find_devices(attaching_hw->scsi_mfg,
						attaching_hw->scsi_model,
						NULL, -1, -1, -1, -1,
						attach_one_scsi);

		if (attaching_hw->usb_vendor != 0
		    && attaching_hw->usb_product != 0) {
			DBG(1,
			    "sane_init: Trying to find USB device %x %x ...\n",
			...
			if (sanei_usb_find_devices(attaching_hw->usb_vendor,
			    ...
			    SANE_STATUS_GOOD) {
				DBG(1,
				    "sane_init: error during USB device detection!\n");
			}
		}
		++model_num;
...
sane_open(const char * devicename, SANE_Handle * handle)
{
	...
	if (devicename[0]) {
		...
		if (dev) {
			status = attach(devicename,
					dev->connection.connection_type,
					&dev);
			if (status != SANE_STATUS_GOOD)
				return status;
		}
	...
	*handle = s;
	...
	if (!avision_is_open(&s->av_con)) {
		...
		status = avision_open_extended(s->hw->sane.name, &s->av_con,
					       sense_handler, 0,
					       &(dev->scsi_buffer_size));
#else
		status = avision_open(s->hw->sane.name, &s->av_con,
				      sense_handler, 0);
#endif
		if (status != SANE_STATUS_GOOD) {
	...
	if (dev->inquiry_duplex_interlaced) {
		...
		strncpy(s->duplex_rear_fname, "/tmp/avision-rear-XXXXXX",
			PATH_MAX);

		if (!mktemp(s->duplex_rear_fname)) {
			DBG(1,
			    "sane_open: failed to generate temporary fname for duplex scans\n");
			return SANE_STATUS_NO_MEM;
		} else {
			DBG(1,
			    "sane_open: temporary fname for duplex scans: %s\n",
			    s->duplex_rear_fname);
		}
	}
	...
	if (dev->scanner_type == AV_FILM) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_open: set scan window command failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		...
		if (!(dev->hw->feature_type & AV_NO_CALIB)) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "sane_open: perform calibration failed: %s\n",
				    sane_strstatus(status));
				return status;
			}
		}
		...
		if (dev->scanner_type == AV_FILM) {
			...
			if (status != SANE_STATUS_GOOD)
				DBG(1,
				    "reader_open: object position go-home failed!\n");
		}
...
sane_control_option(SANE_Handle handle, int option,
		    SANE_Action action, void *val, int * info)
{
	Avision_Scanner *s = handle;
	Avision_Device *dev = s->hw;
	SANE_Status status;
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_SOURCE:
			...
			s->source_mode =
				match_source_mode(dev, s->val[option].s);
			s->source_mode_dim =
...
sane_start(SANE_Handle handle)
{
	...
	if (s->page > 0 && dev->inquiry_keeps_window) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_start: set scan window command failed: %s\n",
			    sane_strstatus(status));
	...
	if (dev->inquiry_new_protocol) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_start: get pixel size command failed: %s\n",
			    sane_strstatus(status));
			goto stop_scanner_and_return;
		}
	...
	if (disable_calibration) {
		DBG(1,
		    "sane_start: calibration disabled in config - skipped!\n");
		goto calib_end;
	}
	...
	if (disable_gamma_table) {
		DBG(1,
		    "sane_start: gamma-table disabled in config - skipped!\n");
		goto gamma_end;
	}

	if (dev->hw->feature_type & AV_NO_GAMMA) {
		DBG(1,
		    "sane_start: gamma table skipped due to device-list!!\n");
		goto gamma_end;
	}
	...
	if (dev->inquiry_tune_scan_length && is_adf_scan(s)) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_start: tune_scan_length command failed: %s\n",
			    sane_strstatus(status));
			goto stop_scanner_and_return;
		}
	}
	...
	if (s->page == 0 && dev->inquiry_background_raster) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_start: get background raster command failed: %s\n",
			    sane_strstatus(status));
			goto stop_scanner_and_return;
		}
	}
