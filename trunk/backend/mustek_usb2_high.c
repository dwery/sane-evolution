...
#ifdef SANE_UNUSED
static SANE_Bool MustScanner_AdjustOffset(int nTimes, SANE_Bool * bDirection,
					  SANE_Byte * bOffset,
					  SANE_Byte * bLastMin,
					  SANE_Byte * bLastOffset,
					  unsigned short *wMinValue,
					  SANE_Byte * bOffsetUpperBound,
					  SANE_Byte * bOffsetLowerBound,
					  unsigned short wStdMinLevel,
					  unsigned short wStdMaxLevel);
static SANE_Bool MustScanner_SecondAdjustOffset(int nTimes,
						SANE_Bool * bDirection,
						SANE_Byte * bOffset,
						SANE_Byte * bLastMin,
						SANE_Byte * bLastOffset,
						unsigned short *wMinValue,
...
MustScanner_Init()
{
	...
	if (STATUS_GOOD != Asic_Initialize(&g_chip)) {
		DBG(DBG_FUNC,
		    "MustScanner_Init: Asic_Initialize return error\n");
		return FALSE;
	}
...
***********************************************************************/
static SANE_Bool
MustScanner_AdjustOffset(int nTimes, SANE_Bool * bDirection,
			 ...
			 unsigned short wStdMaxLevel)
{
	...
	if (nTimes == 1) {
		if (*wMinValue > *bLastMin) {
			if (*wMinValue > wStdMaxLevel
			    && *bLastMin > wStdMaxLevel) {
				*bDirection = !(*bDirection);
				return TRUE;
			}

			if (*wMinValue < wStdMinLevel
			    && *bLastMin < wStdMinLevel)
				return TRUE;
		}
	...
	if (nTimes > 1) {
		if (*wMinValue > *bLastMin) {
			...
			*bOffset =
				(*bOffsetUpperBound + *bOffsetLowerBound) / 2;
...
***********************************************************************/
static SANE_Bool
MustScanner_SecondAdjustOffset(int nTimes, SANE_Bool * bDirection,
			       SANE_Byte * bOffset, SANE_Byte * bLastMin,
			       SANE_Byte * bLastOffset,
			       unsigned short *wMinValue,
			       SANE_Byte * bOffsetUpperBound,
	...
	if (!isOrderInvert) {
		for (; TotalXferLines < wWantedTotalLines;) {
			...
			if (GetScannedLines() > g_wtheReadyLines) {
				wRLinePos =
					g_wtheReadyLines % g_wMaxScanLines;
				wGLinePos =
					(g_wtheReadyLines -
					 g_wLineDistance) % g_wMaxScanLines;
				wBLinePos =
					(g_wtheReadyLines -
					 g_wLineDistance * 2) %
					g_wMaxScanLines;
				...
				for (i = 0; i < g_SWWidth; i++) {
					...
					wBTempData +=
						*(g_lpReadImageHead +
						  ...
						  i * 6 + 5) << 8;
					*(lpLine + i * 6 + 0) =
						LOBYTE(g_pGammaTable
						       [wRTempData]);
					*(lpLine + i * 6 + 1) =
						HIBYTE(g_pGammaTable
						       [wRTempData]);
					*(lpLine + i * 6 + 2) =
						LOBYTE(g_pGammaTable
						       [wGTempData + 65536]);
...
***********************************************************************/
static SANE_Bool
MustScanner_GetRgb48BitLine1200DPI(SANE_Byte * lpLine,
				   SANE_Bool isOrderInvert,
				   unsigned short *wLinesCount)
{
	unsigned short wWantedTotalLines;
...
***********************************************************************/
static SANE_Bool
MustScanner_GetRgb24BitLine1200DPI(SANE_Byte * lpLine,
				   SANE_Bool isOrderInvert,
				   unsigned short *wLinesCount)
{
	SANE_Byte *lpTemp;
							...
							(unsigned
							 char) (*
								(g_pGammaTable
								 +
								 (unsigned
								  short) ((byRed << 4) | QBET4(byBlue, byGreen))));
						*(lpLine + i * 3 + 1) =
							(unsigned
							 char) (*
								(g_pGammaTable
								 + 4096 +
								 (unsigned
								  short) ((byGreen << 4) | QBET4(byRed, byBlue))));
						*(lpLine + i * 3 + 2) =
							(unsigned
							 char) (*
								(g_pGammaTable
								 + 8192 +
								 (unsigned
								  short) ((byBlue << 4) | QBET4(byGreen, byRed))));
#else
						*(lpLine + i * 3 + 0) =
							(unsigned char) byRed;
						*(lpLine + i * 3 + 1) =
			...
			for (i = 0; i < g_SWWidth; i++) {
				...
				wTempData +=
					*(g_lpReadImageHead +
					  ...
					  1) << 8;
				*(lpLine + i * 2 + 0) =
					LOBYTE(g_pGammaTable[wTempData]);
				*(lpLine + i * 2 + 1) =
					HIBYTE(g_pGammaTable[wTempData]);
			}
					...
					dwTempData +=
						(unsigned
						 int) (*(g_lpReadImageHead +
							 ...
							 2 + 1) << 8);
					dwTempData =
						g_pGammaTable[dwTempData >>
							      1];
					*(lpLine + i * 2 + 0) =
						LOBYTE((unsigned short)
						       dwTempData);
					*(lpLine + i * 2 + 1) =
						HIBYTE((unsigned short)
						       dwTempData);
					i++;
					if (i >= g_SWWidth) {
						break;
			    ...
			    unsigned short wResolution)
{
	...
	wSecData =
		(unsigned short *) malloc(sizeof(unsigned short) * g_nSecNum);
	if (wSecData == NULL) {
		return;
	} else {
	...
	*lpMaxValue = wSecData[0];
	...
	if (wDarkSecData == NULL) {
		...
		memset(wDarkSecData, 0,
		       g_nDarkSecNum * sizeof(unsigned short));
	}
...
MustScanner_ReadDataFromScanner(void *dummy)
{
	...
	while (wTotalReadImageLines < wWantedLines && g_lpReadImageHead) {
		if (!isWaitImageLineDiff) {
			...
			if (STATUS_GOOD !=
			    Asic_ReadImage(&g_chip, lpReadImage,
					   wScanLinesThisBlock)) {
				DBG(DBG_FUNC,
...
ModifyLinePoint(SANE_Byte * lpImageData,
		SANE_Byte * lpImageDataBefore,
		unsigned int dwBytesPerLine,
		unsigned int dwLinesCount, unsigned short wPixDistance,
		unsigned short wModPtCount)
{
	unsigned short i = 0;
	unsigned short j = 0;
	...
	for (i = wModPtCount; i > 0; i--) {
		for (j = 0; j < wPixDistance; j++) {
			...
			*(lpImageData + (dwWidth - i) * wPixDistance + j) =
				(*
				 (lpImageData +
				  (dwWidth - i - 1) * wPixDistance + j) +
				 *(lpImageDataBefore +
				   (dwWidth - i) * wPixDistance + j)) / 2;
			/*modify other lines */
			for (wLines = 1; wLines < dwLinesCount; wLines++) {
				unsigned int dwBytesBefor =
					(wLines - 1) * dwBytesPerLine;
				unsigned int dwBytes =
					wLines * dwBytesPerLine;
				*(lpImageData + dwBytes +
				  (dwWidth - i) * wPixDistance + j) =
		(*
