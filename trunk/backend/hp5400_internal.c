...
WarmupLamp(int iHandle)
{
	...
	do {
		...
		couldRead =
			hp5400_command_read_noverify(iHandle, 0xc500, 0x02,
						     dataVerify);
		     ...
		     const int *pabGammaB)
{
	...
	for (i = 0; i < 3; i++) {
		...
		hp5400_bulk_command_write(iHandle, 0x2A01 + i, cmd, 3,
					  2 * 65536, 65536, (void *) buffer);
	}
	free(buffer);
	       ...
	       int bpp, int iMisAlignment, int blksize, int iTransferSize)
{
	...
	HP5400_DBG(DBG_MSG,
		   ...
		   p->linelength, p->linelength, p->pixels, p->pixels,
		   p->roff, p->roff, p->goff, p->goff, p->boff, p->boff, bpp,
		   iMisAlignment);
}
...
/*  HP5400_DBG(DBG_MSG, "CircBufferGetLine:\n");   */
	...
	while (p->bufstart + maxoff >= p->bufend) {	/* Not enough data in buffer */
		...
		res = hp5400_bulk_read_block(iHandle, CMD_INITBULK3, cmd,
					     sizeof(cmd), buftmp + p->bufend,
					     p->blksize);
		if (res != p->blksize) {
			HP5400_DBG(DBG_ERR,
				   "*** ERROR: Read returned %d. FATAL.\n",
				   res);
			return -1;
		}
#ifdef IMAGE_DEBUG
		fwrite(buftmp + p->bufend, p->blksize, 1, temp);
#endif
		p->bufend += p->blksize;
	}
		...
		for (j = 0; j < xsize; j++) {
			for (k = 0; k < planes; k++) {
				if (bpp == 1) {
					...
					buf[j * planes * 2 + k * 2 + 0] =
						p[k][2 * j + 0];
					buf[j * planes * 2 + k * 2 + 1] =
						p[k][2 * j + 1];
				}
			}
		}
...
hp5400_test_scan_response(struct ScanResponse *resp, struct ScanRequest *req)
{
	...
	HP5400_DBG(DBG_MSG, "  transfersize=%d   htonl-> %d\n",
		   resp->transfersize, htonl(resp->transfersize));
	      ...
	      unsigned int **array)
{
	...
	if (InitScan2(SCAN_TYPE_CALIBRATION, req, &HWParams, &res, 0, code) !=
	    0)
	    ...
	    const char *filename)
{
	...
	if (imageFile) {
		...
		DecodeImage(imageFile, planes, bpp,
			    planes * bpp * params->iWidth, params->iHeight,
			    filename);
		fclose(imageFile);
	}
#endif
...
/* Offset 40 */
	...
	0x00,
	0x52, 0x65, 0x61, 0x64,
	0x79, 0x00, 0x53, 0x63, 0x61, 0x6E, 0x6E, 0x65, 0x72, 0x20, 0x4C,
	0x6F,
	0x63, 0x6B, 0x65, 0x64,
	0x00, 0x45, 0x72, 0x72, 0x6F, 0x72, 0x00, 0x43, 0x61, 0x6E, 0x63,
	0x65,
	0x6C, 0x69, 0x6E, 0x67,
	0x14, 0x00, 0x50, 0x6F, 0x77, 0x65, 0x72, 0x53, 0x61, 0x76, 0x65,
	0x20,
	0x4F, 0x6E, 0x00, 0x53,
};
...
static char UISetup2[] = {
	...
	0x63, 0x61, 0x6E, 0x6E, 0x69, 0x6E, 0x67, 0x14, 0x00, 0x41, 0x44,
	0x46,
	0x20, 0x70, 0x61, 0x70,
	0x65, 0x72, 0x20, 0x6A, 0x61, 0x6D, 0x00, 0x43, 0x6F, 0x70, 0x69,
	0x65,
	0x73, 0x00, 0x00,
};
...
InitScanner(int iHandle)
{
	...
	if (hp5400_command_write(iHandle, 0xF10B, sizeof(UISetup1), UISetup1)
	    < 0) {
	...
	if (hp5400_command_write(iHandle, 0xF10C, sizeof(UISetup2), UISetup2)
	    < 0) {
...
HP5400_SANE_STATIC int
InitScan(enum ScanType scantype, TScanParams * pParams, THWParams * pHWParams)
{
	struct ScanRequest req;
	struct ScanResponse res;
	int ret;
	...
	req.flags1 =
		htons((scantype == SCAN_TYPE_CALIBRATION) ? 0x0000 : 0x0080);
	req.flags2 =
		htons((scantype ==
		       SCAN_TYPE_CALIBRATION) ? 0x0010 : (scantype ==
							  SCAN_TYPE_PREVIEW) ?
		      0x0000 :
...
/*  SetDefaultGamma( pHWParams->iXferHandle ); ** Must be done by caller */
	...
	ret = InitScan2(scantype, &req, pHWParams, &res,
			pParams->iColourOffset, 0x40);
	  ...
	  int iColourOffset, int code)
{
	...
	{			/* Try to set it to make scan succeed, URB 53 *//*  0x1B01 => 0x40  */
		...
		if (hp5400_command_write
		    (iHandle, CMD_STOPSCAN, sizeof(flag), &flag) < 0) {
	...
	{			/* Try to set it to make scan succeed, URB 55  */
		...
		if (hp5400_command_write
		    (iHandle, CMD_UNKNOWN3, sizeof(data), data) < 0) {
	...
	{			/* Try to set it to make scan succeed, URB 59 */
		char flag = 0x04;
		if (hp5400_command_write
		    (iHandle, CMD_UNKNOWN2, sizeof(flag), &flag) < 0) {
	...
	{			/* URB 67 *//* Reference DPI = 300 currently */
		...
		if (hp5400_command_write
		    (iHandle, CMD_SETDPI, sizeof(dpi), &dpi) < 0) {
			HP5400_DBG(DBG_MSG, "failed to set dpi\n");
			return -1;
		}
	}

	if (scantype != SCAN_TYPE_CALIBRATION) {	/* Setup scan offsets - Should only apply to non-calibration scans */
		short offsets[2];
	...
	{			/* Try to set it to make scan succeed, URB 71 */
		...
		if (hp5400_command_write
		    (iHandle, CMD_STARTSCAN, sizeof(flag), &flag) < 0) {
			HP5400_DBG(DBG_MSG, "failed to set gamma flag\n");
			return -1;
		}
	}

	if (hp5400_command_read(iHandle, CMD_SCANRESPONSE, sizeof(res), &res)
	    < 0) {
	...
	HP5400_DBG(DBG_MSG,
		   ...
		   htonl(res.transfersize), htonl(res.xsize),
		   htons(res.ysize));
	...
	if (htonl(res.transfersize) == 0) {
		HP5400_DBG(DBG_MSG,
			   "Hmm, size is zero. Obviously a problem. Aborting...\n");
		return -1;
	}

	{
		...
		float pixels =
			((float) htons(req->lenx) *
			 (float) htons(req->leny)) *
			((float) htons(req->dpix) *
			 (float) htons(req->dpiy)) / ((float) HW_LPI *
						      (float) HW_LPI);
		int bpp =
			(int) ((float) htonl(res.transfersize) / pixels +
			       0.5);
		...
		HP5400_DBG(DBG_MSG,
			   "bpp = %d / ( (%d * %d) * (%d * %d) / (%d * %d) ) = %d\n",
			   htonl(res.transfersize), htons(req->lenx),
			   htons(req->leny), htons(req->dpix),
			   htons(req->dpiy), HW_LPI, HW_LPI, bpp);
...
HP5400Open(THWParams * params, const char *filename)
{
	...
	while (!versionMatched && (i < numVersions)) {
		if (!strncmp
		    (szVersion + 1, MatchVersions[i].strVersion,
		     strlen(MatchVersions[i].strVersion) - 4)) {
			versionMatched = 1;
		}
		i++;
	}
...
#else
	HP5400_DBG(DBG_MSG,
		   "Warning, Version match is disabled. Version is '%s'\n",
		   szVersion);
...
main(int argc, char *argv[])
{
	...
	if ((argc == 6) && (!strcmp(argv[1], "-decode"))) {
		...
		if (temp) {
			...
			DecodeImage(temp, planes, bpp, planes * bpp * width,
				    height, argv[5]);
			fclose(temp);
		}
		return 1;
