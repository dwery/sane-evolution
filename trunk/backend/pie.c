...
pie_get_inquiry_values(Pie_Device * dev, unsigned char *buffer) 
{
		...
		(double) (get_inquiry_trans_x2(buffer) -
			  get_inquiry_trans_x1(buffer)) /
		dev->inquiry_pixel_resolution;
	dev->inquiry_trans_length =
		(double) (get_inquiry_trans_y2(buffer) -
			   get_inquiry_trans_y1(buffer)) /
		dev->inquiry_pixel_resolution;
...
pie_identify_scanner(Pie_Device * dev, int sfd) 
{
	...
	if (get_inquiry_periph_devtype(inquiry_block) !=
...
pie_get_cal_info(Pie_Device * dev, int sfd) 
{
		...
		sanei_scsi_cmd(sfd, buffer, swrite.size + size, NULL, NULL);
			...
			sanei_scsi_cmd(sfd, sread.cmd, sread.size, buffer,
				...
				malloc(sizeof(struct Pie_cal_info) *
					dev->cal_info_count);
			for (i = 0; i < dev->cal_info_count; i++)
				 {
				dev->cal_info[i].cal_type =
					buffer[8 + i * buffer[5]];
...
init_options(Pie_Scanner * scanner) 
{
	...
	scanner->opt[OPT_RESOLUTION].constraint.range =
	...
	scanner->opt[OPT_TL_X].constraint.range =
	...
	scanner->opt[OPT_BR_X].constraint.range =
		&(scanner->device->x_range);
...
pie_power_save(Pie_Scanner * scanner, int time) 
{
		...
		sanei_scsi_cmd(scanner->sfd, buffer, swrite.size + size,
				NULL, NULL);
...
pie_perform_cal(Pie_Scanner * scanner, int cal_index) 
{
		...
		while (rcv_lines--)
			...
			sanei_scsi_cmd(scanner->sfd, sread.cmd, sread.size,
					rcv_buffer, &size);
		...
		sanei_scsi_cmd(scanner->sfd, send_buffer, swrite.size + size,
				NULL, NULL);
...
pie_dwnld_gamma(Pie_Scanner * scanner) 
{
			...
			pie_dwnld_gamma_one(scanner, FILTER_RED,
					     scanner->gamma_table[1]);
		if (status)
			return status;
		status =
			pie_dwnld_gamma_one(scanner, FILTER_GREEN,
					     scanner->gamma_table[2]);
		if (status)
			return status;
		status =
			pie_dwnld_gamma_one(scanner, FILTER_BLUE,
...
pie_mode_select(Pie_Scanner * scanner) 
{
		...
		sanei_scsi_cmd(scanner->sfd, buffer, smode.size + size, NULL,
				NULL);
...
pie_get_params(Pie_Scanner * scanner) 
{
		...
		sanei_scsi_cmd(scanner->sfd, param.cmd, param.size, buffer,
				&size);
		...
		scanner->filter_offset1 =
			get_param_scan_filter_offset1(buffer);
		scanner->filter_offset2 =
			get_param_scan_filter_offset2(buffer);
...
pie_grab_scanner(Pie_Scanner * scanner) 
{
		...
		sanei_scsi_cmd(scanner->sfd, reserve_unit.cmd,
...
pie_give_scanner(Pie_Scanner * scanner) 
{
		...
		sanei_scsi_cmd(scanner->sfd, release_unit.cmd,
...
pie_reader_process_indexed(Pie_Scanner * scanner, FILE * fp) 
{
		...
		do
				...
				sanei_scsi_cmd(scanner->sfd, sread.cmd,
						sread.size, buffer, &size);
...
pie_reader_process(Pie_Scanner * scanner, FILE * fp) 
{
		...
		do
			...
			for (i = scanner->params.pixels_per_line; i > 0;
			       i--)
			...
			fwrite(reorder, 1, scanner->params.bytes_per_line,
				fp);
			}
		
		else
			 {
			fwrite(buffer, 1, scanner->params.bytes_per_line,
				fp);
		    ...
		    void *val, int * info) 
{
		...
		case OPT_GAMMA_VECTOR:
				...
				SANE_CAP_INACTIVE;
					...
					scanner->opt[OPT_HALFTONE_PATTERN].
						cap &= ~SANE_CAP_INACTIVE;
...
sane_set_io_mode(SANE_Handle handle, SANE_Bool non_blocking) 
{
	...
	if (fcntl(scanner->pipe, F_SETFL, non_blocking ? O_NONBLOCK : 0) <
	      0)
