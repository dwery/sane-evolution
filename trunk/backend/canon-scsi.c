...
request_sense(int fd, void *buf, size_t * buf_size)
{
	...
	cmd[0] = 0x03;
	cmd[4] = 14;
	status = sanei_scsi_cmd2(fd, cmd, sizeof(cmd), NULL, 0, buf,
				 buf_size);
...
set_adf_mode(int fd, u_char priority)
{
	...
	status = sanei_scsi_cmd2(fd, cmd, sizeof(cmd), &priority, 1, NULL,
				 NULL);
		  ...
		  int transfer_data_type)
{
	...
	status = sanei_scsi_cmd2(fd, cmd, sizeof(cmd), buf, *buf_size, NULL,
				 NULL);
...
set_density_curve_fs2710(SANE_Handle handle, int component, u_char * buf)
{
	...
	for (i = 1, hi = *buf++, p = &s->gamma_map[component][0]; i <= 256;
	     i++) {
		lo = hi;
