...
usage(char *msg)
{
	...
	fprintf(stderr,
		"\t-f: force opening devname as SCSI even if it looks "
		"like USB\n");
...
scanner_do_scsi_inquiry(unsigned char *buffer, int sfd)
{
	...
	status = sanei_scsi_cmd(sfd, inquiry.cmd, inquiry.size, buffer,
				&size);
			      ...
			      char *devicename)
{
	...
	printf("found SCSI %s \"%s %s %s\" at %s\n",
	       devtype <
	       NELEMS(devtypes) ? devtypes[devtype] : "unknown device",
...
check_scsi_file(char *file_name)
{
	...
	if (strstr(file_name, "usb")
		...
		if (force) {
			if (verbose > 1)
				printf("checking %s even though it looks like a USB device...", file_name);
		} else {
			if (verbose > 1)
...
check_usb_file(char *file_name)
{
	...
	if (!strstr(file_name, "usb")
		...
		if (force) {
			if (verbose > 1)
				printf("checking %s even though doesn't look like a " "USB device...", file_name);
		} else {
			if (verbose > 1)
	...
	if (result != SANE_STATUS_GOOD) {
		...
		if (result == SANE_STATUS_GOOD) {
			...
			if (verbose > 1)
				printf(" open ok, but vendor and product could NOT be " "identified\n");
...
get_libusb_string_descriptor(struct usb_device *dev, int index)
{
	...
	if (sd->bLength < 2 || sd->bDescriptorType != USB_DT_STRING) {
		usb_close(handle);
		return 0;
	}
	...
	if (usb_control_msg(handle,
			    USB_ENDPOINT_IN + USB_TYPE_STANDARD +
			    USB_RECIP_DEVICE, USB_REQ_GET_DESCRIPTOR,
			    (USB_DT_STRING << 8) + index, 0, buffer, size,
			    1000) < 0) {
		usb_close(handle);
		return 0;
	}
...
{
	return get_libusb_string_descriptor(dev,
...
check_libusb_device(struct usb_device *dev, SANE_Bool from_file)
{
	...
	if (verbose > 2) {
		...
		printf("<device descriptor of 0x%04x/0x%04x at %s:%s",
		       d->idVendor, d->idProduct, dev->bus->dirname,
		       dev->filename);
		if (vendor || product) {
		...
		printf("iManufacturer         %d (%s)\n", d->iManufacturer,
		       ...
		       ? get_libusb_string_descriptor(dev,
						      d->iManufacturer) : "");
		printf("iProduct              %d (%s)\n", d->iProduct,
		       d->iProduct ? get_libusb_string_descriptor(dev,
								  d->
								  iProduct) :
		       "");
		printf("iSerialNumber         %d (%s)\n", d->iSerialNumber,
		       d->iSerialNumber ? get_libusb_string_descriptor(dev,
								       d->
								       iSerialNumber)
		       : "");
		printf("bNumConfigurations    %d\n", d->bNumConfigurations);

		for (config_nr = 0; config_nr < d->bNumConfigurations;
		     config_nr++) {
			struct usb_config_descriptor *c =
				&dev->config[config_nr];
			int interface_nr;
			...
			for (interface_nr = 0;
			     interface_nr < c->bNumInterfaces;
			     interface_nr++) {
				struct usb_interface *interface =
					&c->interface[interface_nr];
				int alt_setting_nr;
				...
				for (alt_setting_nr = 0;
					...
					printf("   iInterface         %d (%s)\n", i->iInterface, i->iInterface ? get_libusb_string_descriptor(dev, i->iInterface) : "");
					for (ep_nr = 0;
					     ep_nr < i->bNumEndpoints;
					     ep_nr++) {
						...
						printf("    bLength           %d\n", e->bLength);
						printf("    bDescriptorType   %d\n", e->bDescriptorType);
						printf("    bEndpointAddress  0x%02X (%s 0x%02X)\n", e->bEndpointAddress, e->bEndpointAddress & USB_ENDPOINT_DIR_MASK ? "in" : "out", e->bEndpointAddress & USB_ENDPOINT_ADDRESS_MASK);
	...
	for (interface_nr = 0;
	     interface_nr < dev->config[0].bNumInterfaces && is_scanner <= 0;
	     interface_nr++) {
		...
		case USB_CLASS_PER_INTERFACE:
			if (dev->config[0].interface[interface_nr].
			    num_altsetting == 0
			    || !dev->config[0].interface[interface_nr].
			    altsetting)
				break;
			switch (dev->config[0].interface[interface_nr].
				altsetting[0].bInterfaceClass) {
			case USB_CLASS_VENDOR_SPEC:
			case USB_CLASS_PER_INTERFACE:
			case 16:	/* data? */
				++is_scanner;
		...
		for (i = 0; i < adapter->NumberOfBuses; i++) {
			...
			while (1) {
				...
				if (inquiry->InquiryDataLength >= 5 &&
					...
					char device_name[20];
					sprintf(device_name, "h%db%dt%dl%d",
						hca, inquiry->PathId,
						inquiry->TargetId,
						inquiry->Lun);
					dev_list[dev_list_index] =
						strdup(device_name);
					dev_list_index++;
				}
...
static SANE_Status
AddToSCSIDeviceList(const char *dev)
{
...
check_mustek_pp_device(void)
{
	...
	while (devices[ctr] != NULL) {
		...
		if (verbose > 1)
			printf("checking %s (SCSI emulation)...",
			       devices[ctr]);
	...
	while (sanei_config_read(line, PATH_MAX, parsefile)) {
		...
		case 'I':
			...
			if (parse_num("Alt=", line, 10, &number)) {
				...
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as].
					bInterfaceNumber = current_if;
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as].
					bAlternateSetting = current_as;
			}
			if (parse_num("#EPs=", line, 10, &number))
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as].bNumEndpoints =
					number;
			dev->config[current_config].interface[current_if].
				altsetting[current_as].endpoint =
				calloc(number,
				       sizeof(struct
		...
		case 'E':
			...
			if (parse_num("Atr=", line, 16, &number))
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as]
					.endpoint[current_ep].bmAttributes =
					number;
			if (parse_num("MxPS=", line, 10, &number))
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as]
					.endpoint[current_ep].wMaxPacketSize =
					number;
			if (parse_num("Ivl=", line, 10, &number))
				dev->config[current_config].
					interface[current_if].
					altsetting[current_as]
					.endpoint[current_ep].bInterval =
					number;
			break;
		case 'S':
		case 'B':
...
main(int argc, char **argv)
{
	...
	if (ap < argv + argc) {
		...
		static char *default_dev_list[] = {
			...
			"/dev/gsc12", "/dev/gsc13", "/dev/gsc14",
			"/dev/gsc15",
#elif defined(__sun)
			"/dev/scg0a", "/dev/scg0b", "/dev/scg0c",
			"/dev/scg0d",
			"/dev/scg0e", "/dev/scg0f", "/dev/scg0g",
			"/dev/scg1a", "/dev/scg1b", "/dev/scg1c",
			"/dev/scg1d",
			"/dev/scg1e", "/dev/scg1f", "/dev/scg1g",
			"/dev/scg2a", "/dev/scg2b", "/dev/scg2c",
			"/dev/scg2d",
			...
			"/dev/pass0", "/dev/pass1", "/dev/pass2",
			"/dev/pass3",
			"/dev/pass4", "/dev/pass5", "/dev/pass6",
			"/dev/pass7",
#elif defined(__FreeBSD__)
			"/dev/uk0", "/dev/uk1", "/dev/uk2", "/dev/uk3",
			"/dev/uk4",
			"/dev/uk5", "/dev/uk6",
	...
	if (device_found) {
		if (verbose > 0)
			printf("  # Your SCSI scanner was detected. It may or may not be " "supported by SANE. Try\n  # scanimage -L and read the backend's " "manpage.\n");
	} else {
		if (verbose > 0)
			printf("  # No SCSI scanners found. If you expected something different, " "make sure that\n  # you have loaded a kernel SCSI driver for your SCSI " "adapter.\n");
		if (!check_sg()) {
			if (verbose > 0)
				printf("  # Also you need support for SCSI Generic (sg) in your " "operating system.\n  # If using Linux, try \"modprobe " "sg\".\n");
		}
	}
	if (verbose > 0)
		printf("\n");
	...
	if (device_found) {
		if (libusb_device_found) {
			if (verbose > 0)
				printf("  # Your USB scanner was (probably) detected. It " "may or may not be supported by\n  # SANE. Try scanimage " "-L and read the backend's manpage.\n");
		} else if (verbose > 0)
			printf("  # Your USB scanner was detected. It may or may not " "be supported by\n  # SANE. Try scanimage -L and read the " "backend's manpage.\n");
		if (unknown_found && verbose > 0)
			printf("  # `UNKNOWN vendor and product' means that there seems to be a " "scanner at this\n  # device file but the vendor and product ids " "couldn't be identified.\n  # Currently identification only works " "with Linux versions >= 2.4.8. You may\n  # need to configure your " "backend manually, see the backend's manpage.\n");
	} else {
		if (verbose > 0)
			printf("  # No USB scanners found. If you expected something different, " "make sure that\n  # you have loaded a kernel driver for your USB host " "controller and have setup\n  # the USB system correctly. " "See man sane-usb for details.\n");
#ifndef HAVE_LIBUSB
		if (verbose > 0)
			printf("  # SANE has been built without libusb support. This may be a " "reason\n  # for not detecting USB scanners. Read README for " "more details.\n");
#endif
	}
