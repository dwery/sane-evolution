 ...
 */
static void
ioP96InitialSetCurrentSpeed(pScanData ps)
{
	...
	case COLOR_256GRAY:
		if (COLOR_256GRAY == ps->DataInf.wAppDataType) {
			...
			ps->bCurrentSpeed =
				(Byte) (ps->a_wGrayInitTime[ps->IO.portMode] /
					ps->wLinesPer64kTime);
			if (!ps->bCurrentSpeed)
				ps->bCurrentSpeed = 1;

			if ((ps->DataInf.dwAsicPixelsPerPlane >= 1500)
			    && (ps->bCurrentSpeed == 1))
				ps->bCurrentSpeed = 2;
			...
			if (_PORT_SPP != ps->IO.portMode) {
				...
				else if (ps->DataInf.dwAsicPixelsPerPlane <=
					 1720)
	...
	case COLOR_TRUE24:
		ps->bCurrentSpeed =
			(Byte) (ps->a_wColorInitTime[ps->IO.portMode] /
				ps->wLinesPer64kTime);
	...
	if (ps->DataInf.xyAppDpi.y <= ps->wMinCmpDpi) {
		...
		if (ps->DataInf.xyAppDpi.y <= 100) {
			...
			if (ps->DataInf.dwAsicBytesPerPlane > 1400)
				pModeDiff =
					a_tabDiffParam + _ColorEpp100_1400;
		} else {
			if (ps->DataInf.xyAppDpi.y <= 150) {
				pModeType += 2;
				...
				if (ps->DataInf.xyAppDpi.y <= 300) {
					...
					pModeDiff =
						a_tabDiffParam +
						_ColorEpp300_1200;
					if (ps->DataInf.dwAsicBytesPerPlane <=
					    1200)
						pModeDiff--;
					else {
						if (ps->DataInf.
 ...
 */
static void
fnSppLineArtSpeed(pScanData ps)
{
	...
	if (ps->DataInf.xyAppDpi.y > ps->wMinCmpDpi) {
		...
		if (ps->DataInf.xyAppDpi.y > 100) {
			...
			if (ps->DataInf.xyAppDpi.y > 150) {
				pModeType++;
				pModeDiff =
					a_tabDiffParam + _ColorSpp300_2000;
 ...
 */
static void
fnBppLineArtSpeed(pScanData ps)
{
	...
	if (ps->DataInf.xyAppDpi.y > ps->wMinCmpDpi) {
		...
		if (ps->DataInf.xyAppDpi.y > 100) {
			...
			if (ps->DataInf.xyAppDpi.y > 150) {
				pModeType++;
				pModeDiff =
					a_tabDiffParam + _ColorBpp300_1600;
				if (ps->DataInf.xyAppDpi.y > 300) {
					pModeType++;
					pModeDiff =
						a_tabDiffParam +
 ...
 */
static void
ioP98SppNegativeProcs(pScanData ps)
{
	...
	if (ps->DataInf.dwScanFlag & SCANDEF_Negative) {
		if (ps->AsicReg.RD_LineControl == 144)
			pModeDiff += 4;
		else if (ps->AsicReg.RD_LineControl == 192)
			pModeDiff += 7;
	}
}
 ...
 */
static void
ioP98EppNegativeProcs(pScanData ps)
{
	if (ps->DataInf.dwScanFlag & SCANDEF_Negative)
 ...
 */
static void
ioP98BppNegativeProcs(pScanData ps)
{
	if (ps->DataInf.dwScanFlag & SCANDEF_Negative) {
 ...
 */
static void
ioControlLampOnOff(pScanData ps)
{
	Byte lampStatus;
	...
	if (_IS_ASIC98(ps->sCaps.AsicID)) {
		...
		if (ps->DataInf.
		    dwScanFlag & (SCANDEF_Transparency + SCANDEF_Negative)) {
			ps->bLampOn = 0;
		} else {
 ...
 */
static void
ioP98InitialSetCurrentSpeed(pScanData ps)
{
	...
	if (ps->DataInf.dwScanFlag & SCANDEF_TPA) {
		...
		case _PORT_SPP:
			ioP98SppNegativeProcs(ps);
			break;
		case _PORT_BIDI:
			ioP98BppNegativeProcs(ps);
			break;

		default:
			ioP98EppNegativeProcs(ps);
			break;
		}
	} else {
 ...
 */
_LOC Byte
IOSetToMotorRegister(pScanData ps)
{
	ps->OpenScanPath(ps);
 ...
 */
_LOC Byte
IOGetScanState(pScanData ps, Bool fOpenned)
{
 ...
 */
_LOC Byte
IOGetExtendedStatus(pScanData ps)
{
	Byte b;
 ...
 */
_LOC void
IOGetCurrentStateCount(pScanData ps, pScanState pScanStep)
{
	...
	if ((_NO_BASE != ps->sCaps.wIOBase) &&
		...
		if (_ASIC_IS_98003 != ps->sCaps.AsicID) {
			for (dw = _SCANSTATE_BYTES,
			     pdwTable = (pULong) ps->a_wMoveStepTable; dw;
			     dw--, pdwTable++) {
				*pdwTable = 0x10001;
			}

			memset(ps->a_bColorByteTable, 0,
 ...
 */
_LOC void
IOSetXStepLineScanTime(pScanData ps, Byte b)
{
 ...
 */
_LOC void
IOSetToMotorStepCount(pScanData ps)
{
	ULong dw;
	pUChar pb;
	TimerDef timer;
	...
	for (dw = _SCANSTATE_BYTES, pb = ps->a_nbNewAdrPointer; dw;
	     dw--, pb++)
 ...
 */
_LOC void
IOSelectLampSource(pScanData ps)
{
 ...
 */
_LOC Bool
IOReadOneShadingLine(pScanData ps, pUChar pBuf, ULong len)
{
	TimerDef timer;
...
IOReadFifoLength(pScanData ps)
{
	...
	Data.dwOverlap.w1st.b1st =
		IODataFromRegister(ps, ps->Scan.bFifoSelect);

	IODataToRegister(ps, ps->RegBitDepth, _BIT8_15);
	Data.dwOverlap.w1st.b2nd =
		IODataFromRegister(ps, ps->Scan.bFifoSelect);

	IODataToRegister(ps, ps->RegBitDepth, _BIT16_20);
	Data.dwOverlap.w2nd.b1st =
		(IODataFromRegister(ps, ps->Scan.bFifoSelect) & 0x0f);
 ...
 */
_LOC void
IOPutOnAllRegisters(pScanData ps)
{
	pUChar pValue;
	Byte bReg;
	...
	if (_IS_ASIC98(ps->sCaps.AsicID)) {
		...
		if (_ASIC_IS_98003 == ps->sCaps.AsicID)
			...
			IODataToRegister(ps, ps->RegModeControl,
					 (_ModeScan + _ModeFifoRSel));
 ...
 */
_LOC void
IOReadColorData(pScanData ps, pUChar pBuf, ULong len)
{
