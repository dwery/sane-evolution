...
static SANE_Status set_lamp_brightness(struct Rts8891_Device *dev, int level);
...
sane_get_devices(const SANE_Device *** device_list, SANE_Bool local_only)
{
	...
	*device_list = devlist;
	...
	for (device = first_device; dev_num < num_devices;
	     device = device->next) {
...
sane_open(SANE_String_Const name, SANE_Handle * handle)
{
	...
	if (name[0] == 0 || strncmp(name, "rts8891", 7) == 0) {
		DBG(DBG_info,
		    "sane_open: no device requested, using default\n");
		if (first_device) {
			device = first_device;
			DBG(DBG_info,
			    "sane_open: device %s used as default device\n",
			    device->file_name);
		}
	} else {
	...
	if (device->initialized == SANE_FALSE) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sane_open: initialize_device failed\n");
			DBG(DBG_proc, "sane_open: exit on error\n");
			return status;
		}
#else
...
set_automatic_value(Rts8891_Scanner * s, int option, SANE_Int * myinfo)
{
	...
	case OPT_RESOLUTION:
		...
		dpi_list =
			(SANE_Word *) s->opt[OPT_RESOLUTION].constraint.
			word_list;
		min = 65536;
	...
	default:
		DBG(DBG_warn,
		    "set_automatic_value: can't set unknown option %d\n",
		    option);
	}
		 ...
		 SANE_Int * myinfo)
{
	...
	case OPT_GAMMA_VECTOR_B:
		/* sanity checks */
		for (i = 0;
		     i < (int) (s->opt[option].size / sizeof(SANE_Word));
		     i++) {
			if (((SANE_Int *) val)[i] < 0
			    || ((SANE_Int *) val)[i] > 255) {
				DBG(2,
		...
		/* then alloc memory */
		s->val[option].wa =
			(SANE_Word *) malloc(256 * sizeof(SANE_Word));
	...
	default:
		DBG(DBG_warn,
		    "set_option_value: can't set unknown option %d\n",
		    option);
	}
	return status;
}
...
get_option_value(Rts8891_Scanner * s, int option, void *val)
{
	...
	default:
		DBG(DBG_warn,
		    "get_option_value: can't get unknown option %d\n",
		    option);
	}
	return SANE_STATUS_GOOD;
}
...
sane_start(SANE_Handle handle)
{
	...
	do {
		...
		if (status != SANE_STATUS_GOOD) {
			...
			DBG(DBG_error,
			    "sane_start: failed to find origin!\n");
			return status;
		}
		...
		if (changed) {
			if (dev->sensor == SENSOR_TYPE_BARE) {
				DBG(DBG_info,
				    "sane_start: sensor changed to type 'xpa'!\n");
				dev->sensor = SENSOR_TYPE_XPA;
			} else {
				DBG(DBG_info,
				    "sane_start: sensor changed to type 'bare'!\n");
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to do dark calibration!\n");
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to do gain calibration!\n");
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to do offset calibration!\n");
		return status;
	}
	...
	dev->scanned_data =
		(SANE_Byte *) malloc(dev->data_size + dev->lds_max +
				     dev->ripple);
	...
	sanei_rts88xx_set_offset(dev->regs, dev->red_offset,
				 dev->green_offset, dev->blue_offset);
	sanei_rts88xx_set_gain(dev->regs, dev->red_gain, dev->green_gain,
			       dev->blue_gain);

	/* step 6: shading calibration */
	status = shading_calibration(dev,
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to do shading calibration!\n");
		return status;
	}
	...
	if (dev->ydpi > dev->model->min_ydpi
		...
		if (status != SANE_STATUS_GOOD) {
			sanei_usb_release_interface(dev->devnum, 0);
			DBG(DBG_error,
			    "sane_start: failed to move to scan area!\n");
			return status;
		}
	}
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to write scan registers!\n");
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		sanei_usb_release_interface(dev->devnum, 0);
		DBG(DBG_error,
		    "sane_start: failed to send calibration data!\n");
		return status;
	}
...
compute_parameters(Rts8891_Scanner * scanner)
{
	...
	/* gray modes */
	dev->threshold =
		(255 * SANE_UNFIX(scanner->val[OPT_THRESHOLD].w)) / 100;
	...
	/* hardware values */
	dev->xstart =
		((SANE_UNFIX(dev->model->x_offset) +
		  tl_x) * dev->xdpi) / MM_PER_INCH;
	dev->ystart =
		((SANE_UNFIX(dev->model->y_offset) +
		  tl_y) * dev->ydpi) / MM_PER_INCH;
	...
	scanner->to_send =
		scanner->params.bytes_per_line * scanner->params.lines;
	...
	if (scanner->params.format == SANE_FRAME_RGB
		...
		dev->lds_r
			=
			((dev->model->ld_shift_r * dev->ydpi) /
			 dev->model->max_ydpi)
			* dev->bytes_per_line;
		dev->lds_g
			=
			((dev->model->ld_shift_g * dev->ydpi) /
			 dev->model->max_ydpi)
			* dev->bytes_per_line;
		dev->lds_b
			=
			((dev->model->ld_shift_b * dev->ydpi) /
			 dev->model->max_ydpi)
			* dev->bytes_per_line;
	...
	if (dev->xdpi < dev->model->max_ydpi) {
		dev->data_size =
			(PREFERED_BUFFER_SIZE / dev->bytes_per_line) *
			(dev->bytes_per_line);
	} else {
		dev->data_size =
			(((PREFERED_BUFFER_SIZE / 2) - dev->lds_max -
			  dev->ripple) / dev->bytes_per_line) *
			dev->bytes_per_line;
	}
	...
	*len = 0;
	...
	if (dev->read == 0
		...
		if (status == SANE_STATUS_DEVICE_BUSY) {
			DBG(DBG_io,
			    "sane_read: no data currently available\n");
			return SANE_STATUS_GOOD;
		}
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sane_read: failed to read data from scanner\n");
			return status;
		}
	...
	if (scanner->params.format == SANE_FRAME_GRAY
	    && scanner->emulated_gray == SANE_FALSE) {
		DBG(DBG_error0,
		    "sane_read: unimplemented native gray scanning\n");
		return SANE_STATUS_INVAL;
		...
		while (dev->current < dev->end && *len < max_len
			...
			if (dev->xdpi == dev->model->max_xdpi) {	/* at max xdpi, data received is distorted and ydpi is half of xdpi */
				if (scanner->emulated_gray == SANE_TRUE) {
					/* in emulated gray mode we are allways reading 3 bytes of raw data */
					/* at a time                                                        */
					switch (((scanner->sent * 3) %
						 dev->bytes_per_line) % 6) {
					case 0:
					case 1:
					case 2:
						val = dev->current[dev->
				...
				if ((dev->current -
				     dev->start) % dev->bytes_per_line == 0)
					dev->current += dev->bytes_per_line;
			}
		}
	}
	...
	if (dev->current >= dev->end
	    && (scanner->params.format == SANE_FRAME_RGB
		|| scanner->emulated_gray == SANE_TRUE)) {
		memcpy(dev->scanned_data,
		       dev->end - (dev->lds_max + dev->ripple),
...
attach_Rts8891(const char *devicename)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "attach_Rts8891: couldn't open device `%s': %s\n",
		    devicename, sane_strstatus(status));
		return status;
	} else {
		DBG(DBG_info,
		    "attach_Rts8891: device `%s' successfully opened\n",
		    devicename);
	}
...
init_options(struct Rts8891_Scanner *scanner)
{
	...
	scanner->opt[OPT_THRESHOLD].constraint.range =
		&threshold_percentage_range;
	...
	for (i = OPT_BUTTON_1; i <= OPT_BUTTON_11; i++) {
		...
		if (idx < model->buttons) {
			...
			scanner->opt[i].desc =
				SANE_I18N("This option reflects the status "
					  "of a scanner button.");
			scanner->opt[i].cap =
...
find_origin(struct Rts8891_Device *dev, SANE_Bool * changed)
{
	...
	if (control != 0) {
		DBG(DBG_warn,
		    "find_origin: unexpected control value 0x%02x\n",
		    control);
	}
	...
	sanei_rts88xx_set_scan_area(dev->regs, starty, starty + height,
				    startx, startx + width);
	...
	if (DBG_LEVEL > DBG_io2) {
		write_gray_data(image, "find_origin_ysobel.pnm", width,
				height);
	}
	...
	if (sum > 11) {
		...
		do {
			status = sanei_rts88xx_read_reg(dev->devnum,
							CONTROL_REG, &reg);
		}
		while ((reg & 0x08) == 0x08);
	}
...
initialize_device(struct Rts8891_Device *dev)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "initialize_device: failed to read lamp status!\n");
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (!(control & 0x02)) {
		if (park_head(dev) != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "initialize_device: failed to park head!\n");
			return SANE_STATUS_IO_ERROR;
		}
	}
...
init_device(struct Rts8891_Device *dev)
{
	...
	SANE_Byte nv_cmd1[21] =
		{ 0x28, 0x38, 0x28, 0x38, 0x08, 0x18, 0x28, 0x38, 0x28, 0x38,
	0x28, 0x38,
	...
	if (control != 0x04) {
		DBG(DBG_warn,
		    "init_device: expected control=0x04, got 0x%02x\n",
		    control);
	}
	...
	if (DBG_LEVEL > DBG_io2) {
		...
		for (i = 0; i < dev->reg_count; i++)
			sprintf(message + strlen(message), "0x%02x",
				dev->regs[i]);
	...
	for (i = 0; i < 2072; i++) {
		if (buffer[i] != 0) {
			DBG(DBG_error,
			    "init_device: memory at %d is not 0x00 (0x%02x)\n",
			    i, buffer[i]);
			return SANE_STATUS_IO_ERROR;
		}
	}
	...
	do {
		...
		/* write it to memory */
		sanei_rts88xx_set_mem(dev->devnum, 0x00, page * 16, 32,
				      buffer);
	...
	if (control != 0) {
		DBG(DBG_warn,
		    "init_device: unexpected control value 0x%02x\n",
		    control);
	}
	sanei_rts88xx_reset_lamp(dev->devnum, dev->regs);
...
*/
	...
	if (control != 0x80) {
		DBG(DBG_warn,
		    "detect_device: expected reg[b0]=0x80, got 0x%02x\n",
		    control);
		/* TODO : fail here ? */
	}
	...
	if (control != 0x04) {
		DBG(DBG_warn,
		    "detect_device: expected control=0x04, got 0x%02x\n",
		    control);
	...
	if (control != 0x00) {
		DBG(DBG_warn,
		    "detect_device: expected LINK_REG=0x00, got 0x%02x\n",
		    control);
		/* TODO : fail here ? */
	}
...
/* red, green and blue offset, within a 't'op and 'b'ottom value */
	...
	sanei_rts88xx_set_scan_area(dev->regs, 1, 2, 4,
				    4 + CALIBRATION_WIDTH);
	...
	do {
		...
		/* do scan */
		status = rts8891_simple_scan(dev->devnum, dev->regs,
					     dev->reg_count, 0x02,
...
/* intervals for dichotomy */
	...
	do {
		...
		/* scan on line in RGB */
		status = rts8891_simple_scan(dev->devnum, dev->regs,
					     dev->reg_count, 0x02, length,
					     image);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gain_calibration: failed scan data\n");
			return status;
		}
		...
		global = average_area(SANE_TRUE, image, width, 1, &ra, &ga,
				      &ba);
		DBG(DBG_info,
		...
		if (abs(ra - RED_GAIN_TARGET) < GAIN_MARGIN) {
			...
			trg = rg;
			brg = rg;
		} else {	/* average is higher than the desired value */
...
offset_calibration(struct Rts8891_Device *dev, int light)
{
	...
	do {
		DBG(DBG_info,
		    "offset_calibration: trying offsets=(%d,%d,%d) ...\n", ro,
		    go, bo);
		sanei_rts88xx_set_offset(dev->regs, ro, go, bo);
		sanei_rts88xx_set_status(dev->devnum, dev->regs, 0x20, light);
		rts8891_simple_scan(dev->devnum, dev->regs, dev->reg_count,
				    0x02, CALIBRATION_SIZE, image);
...
shading_calibration(struct Rts8891_Device *dev, SANE_Bool color, int light)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "shading_calibration: failed scan shading area\n");
		free(dev->shading_data);
		dev->shading_data = NULL;
		free(image);
		return status;
...
#define BLUE_SHADING_TARGET_CODE  2700000
	...
	for (i = 0; i < width; i++) {
		...
		if (gamma_r[dev->shading_data[i * 3]] < 5)
			...
			value = RED_SHADING_TARGET_CODE /
				gamma_r[dev->shading_data[i * 3]];
		...
		if (gamma_r[dev->shading_data[i * 3 + 1]] < 5)
			...
			value = GREEN_SHADING_TARGET_CODE /
				gamma_g[dev->shading_data[i * 3 + 1]];
		val = (SANE_Byte) (value / 256);
		if (val == 0xaa)
			val++;
		...
		if (gamma_r[dev->shading_data[i * 3 + 2]] < 5)
			...
			value = BLUE_SHADING_TARGET_CODE /
				gamma_b[dev->shading_data[i * 3 + 2]];
		val = (SANE_Byte) (value / 256);
		if (val == 0xaa)
			val++;
	...
	if (size > RTS88XX_MAX_XFER_SIZE) {
		status = sanei_rts88xx_write_mem(dev->devnum,
						 RTS88XX_MAX_XFER_SIZE, 6,
						 calibration);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		status = sanei_rts88xx_write_mem(dev->devnum, size, 0,
						 calibration +
						 RTS88XX_MAX_XFER_SIZE);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		status = sanei_rts88xx_write_mem(dev->devnum, size, 6,
						 calibration);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
...
write_scan_registers(struct Rts8891_Scanner *scanner)
{
	...
	if (scanner->params.format == SANE_FRAME_GRAY
	    && (scanner->dev->model->
		flags & RTS8891_FLAG_EMULATED_GRAY_MODE) == 0) {
		DBG(DBG_warn,
	...
	if (control != 0) {
		DBG(DBG_error0,
		    "write_scan_registers: scanner is not idle!\n");
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error0,
		    "write_scan_registers: failed to write registers\n");
	}
	return status;
}
...
park_head(struct Rts8891_Device *dev)
{
	...
	if (reg != 0x20) {
		DBG(DBG_warn,
		    "park_head: unexpected controler value 0x%02x\n", reg);
	}
...
update_button_status(struct Rts8891_Scanner *scanner)
{
	...
	for (i = 0; i < scanner->dev->model->buttons; i++) {
		if (mask & (1 << i)) {
			...
			DBG(DBG_io2,
			    "update_button_status: setting button %d to TRUE\n",
			    i + 1);
		}
	}
	return status;
...
set_lamp_state(struct Rts8891_Scanner *scanner, int on)
{
	...
	status = sanei_rts88xx_write_reg(scanner->dev->devnum, LAMP_REG,
					 &reg);
