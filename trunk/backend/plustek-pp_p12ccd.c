 ...
 */
static void
fnCCDInitWolfson3797(pScanData ps)
{
	if (ps->Shade.bIntermediate & _ScanMode_Mono)
		ps->Shade.pCcdDac->DarkDAC.Colors.Green = 0xcc;
	else if (ps->Shade.bIntermediate & _ScanMode_AverageOut)
 ...
 */
static void
fnCCDInitSamsung3797(pScanData ps)
{
 ...
 */
static void
fnCCDInitWolfson3799(pScanData ps)
{
 ...
 */
static void
fnCCDInitWolfson548(pScanData ps)
{
	if (!(ps->Shade.bIntermediate & _ScanMode_AverageOut)) {
 ...
 */
static void
fnCCDInitSamsung3777(pScanData ps)
{
	if (!(ps->DataInf.dwScanFlag & SCANDEF_Negative)) {
 ...
 */
static void
fnCCDInitSamsung3799(pScanData ps)
{
	if (!(ps->DataInf.dwScanFlag & SCANDEF_TPA)) {
 ...
 */
static void
fnCCDInitESIC3799(pScanData ps)
{
	if (!(ps->DataInf.dwScanFlag & SCANDEF_Negative)) {
 ...
 */
static void
fnDarkOffsetWolfson3797(pScanData ps, pDACTblDef pDacTbl, ULong dwCh)
{
...
#if 0
static void
fnDarkOffsetWolfson3799(pScanData ps, pDACTblDef pDacTbl, ULong dwCh)
{
	if (ps->Shade.DarkOffset.wColors[dwCh] >
	    pDacTbl->DarkOffSub.wColors[dwCh])
		ps->Shade.DarkOffset.wColors[dwCh] -=
			pDacTbl->DarkOffSub.wColors[dwCh];
	else
		ps->Shade.DarkOffset.wColors[dwCh] = 0;
}
 ...
 */
static void
fnDarkOffsetSamsung3777(pScanData ps, pDACTblDef pDacTbl, ULong dwCh)
{
	ps->Shade.DarkOffset.wColors[dwCh] +=
		pDacTbl->DarkOffSub.wColors[dwCh];
}
 ...
 */
static void
fnDarkOffsetSamsung3797(pScanData ps, pDACTblDef pDacTbl, ULong dwCh)
{
	if (ps->Shade.DarkOffset.wColors[dwCh] >
	    pDacTbl->DarkOffSub.wColors[dwCh])
		ps->Shade.DarkOffset.wColors[dwCh] -=
 ...
 */
static void
fnDarkOffsetSamsung3799(pScanData ps, pDACTblDef pDacTbl, ULong dwCh)
{
	if (ps->Shade.DarkOffset.wColors[dwCh] >
	    pDacTbl->DarkOffSub.wColors[dwCh])
		ps->Shade.DarkOffset.wColors[dwCh] -=
 ...
 */
static void
fnDACDarkWolfson(pScanData ps, pDACTblDef pDacTbl,
 ...
 */
static void
fnDACDarkSamsung(pScanData ps, pDACTblDef pDacTbl,
		 ULong dwCh, UShort wDarkest)
{
	UShort w;
	...
	if (wDarkest > pDacTbl->DarkCmpHi.wColors[dwCh]) {
		...
		if (wDarkest)
			...
			w = ps->Shade.wDarkLevels +
				(UShort) ps->Shade.DarkDAC.bColors[dwCh];
 ...
 */
_LOC void
P12InitCCDandDAC(pScanData ps, Bool shading)
{
	UShort w;
	pDACTblDef pDAC_CCD;
