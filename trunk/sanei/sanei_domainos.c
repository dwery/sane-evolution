...
static void
DomainErrorCheck(status_$t status, const char *message)
{
	...
	if (status.all) {
		DBG(0, "Unrecoverable Domain/OS Error 0x%08x:  %s\n",
		    status.all, message);
		error_$find_text(status, &subsystem, &subsystem_length,
				 &module, &module_length, &code,
				 &code_length);
		if (subsystem_length && module_length && code_length)
			DBG(0, "%.*s (%.*s/%.*s)\n", code_length, code,
			    subsystem_length, subsystem, module_length,
			    module);
		exit(EXIT_FAILURE);
	}
}
...
unable to do so. */
pfm_$fh_func_val_t
FaultHandler(pfm_$fault_rec_t * FaultStatusPtr)
{
	status_$t status;
...
DomainSCSIOpen(void)
{
	...
	scsi_$acquire((char *) com->open_path, len, &scsi_handle,
		      &com->CommandStatus);
	if (com->CommandStatus.all != status_$ok) {
		/* we have a failure, return an error code, and generate debug output */
		DBG(1,
		    "DomainSCSIOpen: acquire failed, Domain/OS status is %08x\n",
		...
		DBG(2, "DomainSCSIOpen: Data block name will be '%s'\n",
		    com->open_path);
		DataBasePtr =
			ms_$crmapl(com->open_path, strlen(com->open_path), 0,
				   DomainMaxDataSize + DomainSenseSize,
				   ms_$cowriters, &com->CommandStatus);
		...
		/* Wire the buffer */
		scsi_$wire(scsi_handle, (void *) DataBasePtr,
			   DomainMaxDataSize + DomainSenseSize,
			   &com->CommandStatus);
		if (com->CommandStatus.all == status_$ok) {
			...
			DBG(1,
			    "Buffer wire failed, Domain/OS status is %08x\n",
			    com->CommandStatus.all);
			error_$print(com->CommandStatus);
			return;
		}
	...
	if (fd >= num_alloced) {
		...
		memset((char *) DomainFdInfo + old_size, 0,
		       new_size - old_size);
		assert(DomainFdInfo);
	}
	...
	DomainFdInfo[fd].DomainSensePtr =
		((char *) DataBasePtr) + DomainMaxDataSize;
	com->fd = fd;
}


static void
DomainSCSIClose(void)
{
	DomainFdInfo[com->fd].in_use = 0;
	DBG(1, "sanei_scsi_close:  fd=%d\n", com->fd);
	/* Unwire the buffer */
	scsi_$unwire(DomainFdInfo[com->fd].scsi_handle,
		     DomainFdInfo[com->fd].DomainSCSIPtr,
		     DomainMaxDataSize + DomainSenseSize, true,
		     &com->CommandStatus);
	...
	ms_$unmap(DomainFdInfo[com->fd].DomainSCSIPtr,
		  DomainMaxDataSize + DomainSenseSize, &com->CommandStatus);
...
DomainSCSIWait(void)
{
	...
	wait_index =
		scsi_$wait(DomainFdInfo[com->fd].scsi_handle,
			   DomainScsiTimeout, true,
			   DomainFdInfo[com->fd].op_id, 1, status_list,
			   &return_count, &com->CommandStatus);
	...
	if (com->CommandStatus.all == status_$ok) {
		...
		if (wait_index != scsi_device_advance) {
			...
			for (count = 0; count < return_count; count++) {
				...
				case scsi_condition_met:
					...
					break;
				case scsi_rsv1:
					ascii_op_status = "scsi_rsv1";
					break;
				case scsi_busy:
					ascii_op_status = "scsi_busy";
					break;
				case scsi_rsv2:
					ascii_op_status = "scsi_rsv2";
					break;
				case scsi_rsv3:
					ascii_op_status = "scsi_rsv3";
					break;
				case scsi_rsv4:
					ascii_op_status = "scsi_rsv4";
					break;
				case scsi_intermediate_good:
					ascii_op_status =
						"scsi_intermediate_good";
					break;
				case scsi_rsv5:
					ascii_op_status = "scsi_rsv5";
					break;
				case scsi_intermediate_condition_met:
					ascii_op_status =
						"scsi_intermediate_condition_met";
					break;
				case scsi_rsv6:
					ascii_op_status = "scsi_rsv6";
					break;
				case scsi_reservation_conflict:
					...
					break;
				case scsi_rsv7:
					ascii_op_status = "scsi_rsv7";
					break;
				case scsi_rsv8:
					ascii_op_status = "scsi_rsv8";
					break;
				case scsi_rsv9:
					ascii_op_status = "scsi_rsv9";
					break;
				case scsi_undefined_status:
					ascii_op_status =
						"scsi_undefined_status";
					break;
				default:
					ascii_op_status = "unknown";
					break;
				}
				DBG(2,
				    " list[%d]: op=%x  cmd_status=%08x, status=%s\n",
				    count, status_list[count].op,
				...
				case status_$ok:
					...
					case scsi_check_condition:
					{
						...
						scsi_$do_command_2
							(DomainFdInfo
							 ...
							 &sense_op_id,
							 &sense_status);
						DomainErrorCheck(sense_status,
								 "Executing sense command");
						scsi_$wait(DomainFdInfo
							   [com->fd].
							   scsi_handle,
							   DomainScsiTimeout,
							   true, sense_op_id,
						...
						DBG(2,
						    "Sense information:  Error code=%02x, ASC=%02x, ASCQ=%02x\n",
						    ((u_char *)
						     DomainFdInfo[com->fd].
						     DomainSensePtr)[0],
						    ((char *)
						...
						if ((((char *)
							...
							/*   Its also very ugly */
							DBG(2,
							    "Shortening destination length by %x bytes\n",
							    *(int
							      *) (((char *)
								   DomainFdInfo
								   [com->fd].
					...
					default:
						...
						DBG(0,
						    "Unrecoverable Domain/OS scsi handler error:  status=%08x\n",
						    status_list[count].
						    op_status);
						exit(EXIT_FAILURE);
					}
				...
				case scsi_$hdwr_failure:	/* received when both scanners were active */
					com->CommandStatus =
						status_list[count].cmd_status;
					break;
				default:
					DBG(0,
					    "Unrecoverable DomainOS scsi handler error:  status=%08x\n",
					    status_list[count].cmd_status.
					    all);
					error_$print(status_list[count].
						     cmd_status);
					exit(EXIT_FAILURE);
				}
			}
		...
		if (com->direction == scsi_read) {
			...
			for (return_count = 0; return_count < com->dst_size;
			     return_count++)
				DBG(3, "%02X%c",
				    ((unsigned char *) DomainFdInfo[com->fd].
				     DomainSCSIPtr)[return_count],
				    (return_count % 16) == 15 ? '\n' : ' ');
			DBG(3, "\n");
		}
	} else {
...
DomainSCSIEnter(void)
{
	...
	scsi_$do_command_2(DomainFdInfo[com->fd].scsi_handle, com->cdb,
			   com->cdb_size, DomainFdInfo[com->fd].DomainSCSIPtr,
			   com->buf_size, com->direction,
			   &DomainFdInfo[com->fd].op_id, &com->CommandStatus);
	if (com->CommandStatus.all == status_$ok) {
		/* success, indicate status */
...
sanei_DomainOS_init(char *path)
{
	...
	DBG(1, "Starting Domain SANE Server, common area path = '%s'\n",
	    path);
	com = ms_$mapl(path, strlen(path), 0,
	...
	if (length_mapped < sizeof(struct DomainServerCommon)) {
		DBG(0,
		    "Error - can't open common area '%s' to required length\n",
		    path);
	...
	DomainErrorCheck(status,
			 "Can't advance ResultReady EC after startup");
...
/*   pfm_$inhibit();*/
	...
	FaultHandle =
		pfm_$establish_fault_handler(pfm_$all_faults, 0, FaultHandler,
					     &status);
	...
	do {
		...
		do {
			index = ec2_$wait_svc(CommandAvailablePtr,
					      &CommandTriggerValue, 1,
					      &status);
		}
		while (status.all == ec2_$wait_quit);
		DomainErrorCheck(status,
				 "Error waiting on CommandAvailable EC");
		assert(index == 1);
		...
		case Open:
			...
			DomainErrorCheck(status,
					 "Can't advance CommandAccepted EC on open");
			break;
		case Close:
			DomainSCSIClose();
			ec2_$advance(&com->CommandAccepted, &status);
			DomainErrorCheck(status,
					 "Can't advance CommandAccepted EC on close");
			break;
		case Enter:
			DomainSCSIEnter();
			ec2_$advance(&com->CommandAccepted, &status);
			DomainErrorCheck(status,
					 "Can't advance CommandAccepted EC on enter");
			break;
		case Exit:
			done = 1;
			...
			ec2_$advance(&com->CommandAccepted, &status);
			DomainErrorCheck(status,
					 "Can't advance CommandAccepted EC on exit");
			break;
		default:
