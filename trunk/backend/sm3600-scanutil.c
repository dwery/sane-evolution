...
void
DBG(int nLevel, const char *szFormat, ...)
{
	szFormat++;
}
#endif

__SM3600EXPORT__ void
debug_printf(unsigned long ulType, const char *szFormat, ...)
{
	va_list ap;
	if ((ulDebugMask & ulType) != ulType)
		return;
	if (*szFormat == '~')
		szFormat++;
	else
...
__SM3600EXPORT__ int
SetError(TInstance * this, int nError, const char *szFormat, ...)
{
	va_list ap;
	if (this->nErrorState)
		return 0;	/* do not overwrite error state */
...
FreeState(TInstance * this, TState nReturn)
{
	...
	if (this->state.pchLineOut)
		free(this->state.pchLineOut);
	if (this->state.pchBuf)
		free(this->state.pchBuf);
			 ...
			 int cchMax, int *pcchRead)
{
	...
	*pcchRead = 0;
	...
	while (this->state.iReadPos + cchMax > this->state.cchLineOut) {
		...
		memcpy(achOut,
		       this->state.pchLineOut + this->state.iReadPos, cch);
	...
	if (!cchMax)
		return SANE_STATUS_GOOD;	/* now everything fits! */
	(*pcchRead) += cchMax;
	memcpy(achOut, this->state.pchLineOut + this->state.iReadPos, cchMax);
...
GetAreaSize(TInstance * this)
{
	...
	this->state.cxMax =
		this->state.cxPixel * 100 / this->state.nFixAspect;
...
__SM3600EXPORT__ void
ResetCalibration(TInstance * this)
{
...
__SM3600EXPORT__
	TState InitGammaTables(TInstance * this, int nBrightness,
			       int nContrast)
{
	long i;
	long lOffset;
...
DoScanFile(TInstance * this)
{
	...
	rc = InitGammaTables(this, this->param.nBrightness,
	...
	while (!rc) {
		...
		if (cch > 0 && this->fhScan && cch <= APP_CHUNK_SIZE) {
			if (this->pchPageBuffer) {
				...
				if (this->bVerbose)
					fprintf(stderr,
						"ichPageBuffer:%d, cch:%d, cchPageBuffer:%d\n",
						this->ichPageBuffer, cch,
						this->cchPageBuffer);
#endif
				CHECK_ASSERTION(this->ichPageBuffer + cch <=
						this->cchPageBuffer);
				memcpy(this->pchPageBuffer +
				       this->ichPageBuffer, achBuf, cch);
				this->ichPageBuffer += cch;
