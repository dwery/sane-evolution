...
#define MM_TO_PIXEL(mm, res)	(SANE_UNFIX(mm) * (float )res / SANE_MM_PER_INCH)
#define PIXEL_TO_MM(px, res)	(SANE_FIX((float )(px * SANE_MM_PER_INCH / (res / 10)) / 10.0))
...
attach(const char *devname)
{
	...
	if (num_devices > 0) {
		memcpy(dev + 1, devlist,
		       sizeof(Umax_PP_Descriptor) * (num_devices));
		free(devlist);
	}
	...
	if (mdl > 610) {	/* Astra 1220, 1600 and 2000 */
		dev->max_res = 1200;
		dev->ccd_res = 600;
		dev->max_h_size = 5100;
	...
	if (ports != NULL) {
		...
		while (ports[i] != NULL) {
			if (rc != SANE_STATUS_GOOD) {
				DBG(3,
				    "umax_pp_try_ports: trying port `%s'\n",
				    ports[i]);
				rc = attach(ports[i]);
				if (rc != SANE_STATUS_GOOD)
					DBG(3,
					    "init: couldn't attach to port `%s'\n",
					    ports[i]);
				else
					DBG(3,
					    "init: attach to port `%s' successfull\n",
					    ports[i]);
			}
			free(ports[i]);
			i++;
 ...
 * if safe set to TRUE, no direct hardware access
 *
 */
static int
umax_pp_auto_attach(int safe)
{
	char **ports;
	int rc = SANE_STATUS_INVAL;
	...
	if ((safe != TRUE) && (rc != SANE_STATUS_GOOD)) {
...
init_options(Umax_PP_Device * dev)
{
	...
	dev->opt[OPT_PREVIEW].size = sizeof(int);
	...
	dev->opt[OPT_GRAY_PREVIEW].size = sizeof(int);
	dev->opt[OPT_GRAY_PREVIEW].unit = SANE_UNIT_NONE;
	dev->val[OPT_GRAY_PREVIEW].w = FALSE;
	...
	dev->opt[OPT_LAMP_CONTROL].size = sizeof(int);
	dev->opt[OPT_LAMP_CONTROL].unit = SANE_UNIT_NONE;
	dev->val[OPT_LAMP_CONTROL].w = TRUE;
	...
	dev->opt[OPT_UTA_CONTROL].size = sizeof(int);
	dev->opt[OPT_UTA_CONTROL].unit = SANE_UNIT_NONE;
	dev->val[OPT_UTA_CONTROL].w = TRUE;
	dev->opt[OPT_UTA_CONTROL].cap |=
		SANE_CAP_ADVANCED | SANE_CAP_INACTIVE;
	...
	dev->opt[OPT_GAMMA_VECTOR].size = 256 * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_R].size = 256 * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_G].size = 256 * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_B].size = 256 * sizeof(int);
	...
	dev->val[OPT_MANUAL_GAIN].w = FALSE;
	...
	dev->opt[OPT_BLUE_OFFSET].desc =
		SANE_I18N("Sets blue channel offset");
	dev->opt[OPT_BLUE_OFFSET].type = SANE_TYPE_INT;
	dev->opt[OPT_BLUE_OFFSET].cap |=
		SANE_CAP_INACTIVE | SANE_CAP_ADVANCED;
		 ...
		 long int fallback)
{
	...
	if ((strncmp(string, name, len) == 0) && isspace(string[len])) {
		...
		*value = strtol(string, &end, 0);
		...
		if (end == string || errno) {
			DBG(2,
			    "init: invalid value `%s`, using fallback '%ld'\n",
			    string, fallback);
			*value = fallback;
		}
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (version_code != NULL)
		*version_code =
			SANE_VERSION_CODE(V_MAJOR, V_MINOR, UMAX_PP_BUILD);

	DBG(3, "init: SANE v%s, backend v%d.%d.%d-%s\n", VERSION, V_MAJOR,
	    V_MINOR, UMAX_PP_BUILD, UMAX_PP_STATE);
	...
	if (fp == NULL) {
		DBG(2,
		    "init: no configuration file, using default `port %s'\n",
	...
	while (sanei_config_read(dev_name, sizeof(dev_name), fp)) {
		...
		if (strncmp(cp, "option", 6) == 0 && isspace(cp[6])) {
			...
			if (parse_int_option(cp, "buffer", &val, buf_size)) {
				if (val < 8192) {
					DBG(2,
					    "init: invalid value `%s`, falling back to %ld\n",
					    cp, buf_size);
				...
				if (num_devices == 0) {
					DBG(3,
					    "init: setting global option buffer to %ld\n",
					    val);
					buf_size = val;
				} else {
					DBG(3,
					    "init: setting buffer to %ld for device %s\n",
				...
				DBG(3,
				    "init: setting global option astra to %ld\n",
				    val);
				...
				DBG(3,
				    "init: setting global option red-gain to %ld\n",
				    val);
				red_gain = val;
			} else if (parse_int_option
				   (cp, "green-gain", &val, 6)) {
				if ((val < 0) || (val > 15)) {
					val = 6;
					DBG(2,
				...
				DBG(3,
				    "init: setting global option green-gain to %ld\n",
				    val);
				green_gain = val;
			} else if (parse_int_option
				   (cp, "blue-gain", &val, 12)) {
				...
				DBG(3,
				    "init: setting global option blue-gain to %ld\n",
				    val);
				blue_gain = val;
			} else if (parse_int_option
				   (cp, "red-offset", &val, 10)) {
				if ((val < 0) || (val > 15)) {
					val = 10;
					DBG(2,
					    "init: invalid value `%s`, falling back to %ld\n",
				...
				DBG(3,
				    "init: setting global option red-offset to %ld\n",
				    val);
				red_offset = val;
			} else if (parse_int_option
				   (cp, "green-offset", &val, 10)) {
				if ((val < 0) || (val > 15)) {
					val = 10;
					DBG(2,
					    "init: invalid value `%s`, falling back to %ld\n",
				...
				DBG(3,
				    "init: setting global option green-offset to %ld\n",
				    val);
				green_offset = val;
			} else if (parse_int_option
				   (cp, "blue-offset", &val, 10)) {
				if ((val < 0) || (val > 15)) {
					val = 10;
					DBG(2,
					    "init: invalid value `%s`, falling back to %ld\n",
				...
				DBG(2,
				    "init: don't know what to do with option `%s'\n",
				    cp);
			...
			if (*cp) {
				...
				if (strcmp(cp, "safe-auto") == 0) {
					...
					if (umax_pp_auto_attach(TRUE) !=
					    SANE_STATUS_GOOD)
						DBG(2,
						    "init: safe-auto attach failed !");
				} else if (strcmp(cp, "auto") == 0) {
					/* try every port/device we can find */
					if (umax_pp_auto_attach(FALSE) !=
					    SANE_STATUS_GOOD) {
						DBG(2,
						    "init: auto attach failed !");
					}
				} else {
			...
			if (num_devices == 0)
				DBG(2,
				    "init: 'name' only allowed after 'port'\n");
			else {
			...
			if (num_devices == 0)
				DBG(2,
				    "init: 'model' only allowed after 'port'\n");
			else {
			...
			if (num_devices == 0)
				DBG(2,
				    "init: 'vendor' only allowed after 'port'\n");
			else {
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	case UMAX1220P_TRANSPORT_FAILED:
		if (name == NULL) {
			DBG(1,
			    "failed to init transport layer on port 0x%03X\n",
			    prt);
		} else {
			DBG(1,
			    "failed to init transport layer on device %s\n",
			    name);
		}
		return SANE_STATUS_IO_ERROR;
	...
	case UMAX1220P_SCANNER_FAILED:
		if (name == NULL) {
			DBG(1,
			    "failed to initialize scanner on port 0x%03X\n",
			    prt);
		} else {
	...
	if (dev->buf == NULL) {
		DBG(2,
		    "open: not enough memory for scan buffer (%lu bytes)\n",
...
sane_close(SANE_Handle handle)
{
	...
	if (dev->val[OPT_LAMP_CONTROL].w == TRUE) {
		rc = sanei_umax_pp_lamp(0);
		if (rc == UMAX1220P_TRANSPORT_FAILED) {
			DBG(1,
			    "close: switch off gain failed (ignored....)\n");
		}
	}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_GAMMA_VECTOR_B:
			...
			for (i = 0;
			     i < dev->opt[option].size / sizeof(int);
			     i++) {
				if (((int *) val)[i] < 0
				    || ((int *) val)[i] > 255) {
					DBG(2,
					    "Value at index %d out of range\n",
					    i);
					return SANE_STATUS_INVAL;
				}
			}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(2,
			    "control_option: constrain_value failed (%s)\n",
			    sane_strstatus(status));
			DEBUG();
			return status;
		}
		...
		case OPT_BLUE_OFFSET:
			...
			if (dev->val[OPT_BR_Y].w < dev->val[OPT_TL_Y].w) {
				...
				DBG(16,
				    "control_option: swapping Y coordinates\n");
			}
			if (strcmp(dev->val[OPT_MODE].s, "Color") == 0) {
				dpi = (int) (SANE_UNFIX
					     (dev->val[OPT_RESOLUTION].w));
				if (dev->val[OPT_TL_Y].w <
				    2 * umax_pp_get_sync(dpi)) {
					DBG(16,
					    "control_option: correcting TL_Y coordinates\n");
					dev->val[OPT_TL_Y].w =
						2 * umax_pp_get_sync(dpi);
					if (info)
						*info |= SANE_INFO_INEXACT;
		...
		case OPT_BR_X:
			...
			if (dpi >= 600) {
				if (dev->val[option].w & 0x03) {
					if (info)
						*info |= SANE_INFO_INEXACT;
					dev->val[option].w =
						dev->val[option].w & 0xFFFC;
					*(int *) val =
						dev->val[option].w;
					DBG(16,
					    "control_option: rounding X to %d\n",
					    *(int *) val);
				}
			}
			/* sanity check */
			if (dev->val[OPT_BR_X].w < dev->val[OPT_TL_X].w) {
				...
				if (info)
					*info |= SANE_INFO_INEXACT;
				DBG(16,
				    "control_option: swapping X coordinates\n");
			}
			return SANE_STATUS_GOOD;
		...
		case OPT_RESOLUTION:
			...
			if (dpi >= 600) {
				dev->val[OPT_TL_X].w =
					dev->val[OPT_TL_X].w & 0xFFFC;
				dev->val[OPT_BR_X].w =
					dev->val[OPT_BR_X].w & 0xFFFC;
			}
		...
		case OPT_CUSTOM_GAMMA:
			...
			if (w == TRUE) {
				...
				if ((strcmp(mode, "Grayscale") == 0)
				    || (strcmp(mode, "Lineart") == 0)) {
					dev->opt[OPT_GAMMA_VECTOR].cap &=
						~SANE_CAP_INACTIVE;
					sanei_umax_pp_gamma(NULL,
							    dev->
							    val
							    [OPT_GAMMA_VECTOR].
							    wa, NULL);
		...
		case OPT_MODE:
		{
			...
			if (dev->val[OPT_CUSTOM_GAMMA].w == TRUE) {
			...
			if (dev->val[OPT_MANUAL_OFFSET].w == TRUE) {
				if ((strcmp(val, "Grayscale") == 0)
				    || (strcmp(val, "Lineart") == 0))
					dev->opt[OPT_GRAY_OFFSET].cap &=
						~SANE_CAP_INACTIVE;
			...
			if (dev->val[OPT_MANUAL_GAIN].w == TRUE) {
				if ((strcmp(val, "Grayscale") == 0)
				    || (strcmp(val, "Lineart") == 0))
					dev->opt[OPT_GRAY_GAIN].cap &=
						~SANE_CAP_INACTIVE;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (dev->val[OPT_MANUAL_OFFSET].w == TRUE) {
		if (dev->color != UMAX_PP_MODE_COLOR) {
			dev->red_offset = 0;
			dev->green_offset =
				(int) (dev->val[OPT_GRAY_OFFSET].w);
			dev->blue_offset = 0;
		} else {
			dev->red_offset = (int) (dev->val[OPT_RED_OFFSET].w);
			dev->green_offset =
				(int) (dev->val[OPT_GREEN_OFFSET].w);
			dev->blue_offset =
				(int) (dev->val[OPT_BLUE_OFFSET].w);
		}
	} else {
		dev->red_offset = 6;
		dev->green_offset = 6;
	...
	if (dpi >= 600) {
		...
		if (remain) {
			DBG(64,
			    "sane_get_parameters: %d-%d -> remain is %d\n",
	...
	if (dev->val[OPT_PREVIEW].w == TRUE) {
		...
		if (dev->val[OPT_GRAY_PREVIEW].w == TRUE) {
...
sane_start(SANE_Handle handle)
{
	...
	if (dev->state == UMAX_PP_STATE_CANCELLED) {
		DBG(2,
		    "sane_start: checking if scanner is parking head .... \n");
	...
	if ((sanei_umax_pp_getastra() < 1210)
	    && (dev->color == UMAX_PP_MODE_COLOR)) {
		rc = sanei_umax_pp_read(2 * delta * dev->tw * dev->bpp,
					dev->tw, dev->dpi, 0,
	  ...
	  int * len)
{
	...
	*len = 0;
	...
	if ((dev->buflen == 0) || (dev->bufread >= dev->buflen)) {
		...
		if (dev->color == UMAX_PP_MODE_COLOR) {
			delta = umax_pp_get_sync(dev->dpi);
			rc = sanei_umax_pp_read(length, dev->tw, dev->dpi,
						last,
						dev->buf + UMAX_PP_RESERVE);
		} else
			rc = sanei_umax_pp_read(length, dev->tw, dev->dpi,
		...
		if (dev->color == UMAX_PP_MODE_LINEART) {
			...
			nl = dev->buflen / ll;
			DBG(64,
			    "sane_read: reordering %ld bytes of data (lines=%d)\n",
			    length, nl);
			lbuf = (SANE_Byte *) malloc(dev->bufsize +
						    UMAX_PP_RESERVE);
			if (lbuf == NULL) {
				DBG(1,
				    "sane_read: couldn't allocate %ld bytes\n",
			...
			if (!last)
				memcpy(lbuf + UMAX_PP_RESERVE -
				       2 * delta * ll,
				       dev->buf + UMAX_PP_RESERVE +
				       dev->buflen - 2 * delta * ll,
				       2 * delta * ll);
			free(dev->buf);
			dev->buf = lbuf;
		}
	...
	if (dev->color == UMAX_PP_MODE_COLOR)
		memcpy(buf, dev->buf + dev->bufread + UMAX_PP_RESERVE,
		       length);
	else
...
sane_cancel(SANE_Handle handle)
{
	...
	if (dev->state == UMAX_PP_STATE_SCANNING) {
		...
		/* STATE_CANCELLED */
		DBG(2,
		    "cancel: checking if scanner is still parking head .... \n");
