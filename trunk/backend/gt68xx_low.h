...
#define HIBYTE(w) ((SANE_Byte)(((int)(w) >> 8) & 0xFF))
...
{
	int vendor;		/**< USB vendor identifier */
	int product;		/**< USB product identifier */
...
struct GT68xx_Command_Set
{
   ...
   */
	...
	int memory_read_value;	/**< Memory read - wValue */
	int memory_write_value;	/**< Memory write - wValue */
	int send_cmd_value;	/**< Send normal command - wValue */
	int send_cmd_index;	/**< Send normal command - wIndex */
	int recv_res_value;	/**< Receive normal result - wValue */
	int recv_res_index;	/**< Receive normal result - wIndex */
	int send_small_cmd_value;	/**< Send small command - wValue */
	int send_small_cmd_index;	/**< Send small command - wIndex */
	int recv_small_res_value;	/**< Receive small result - wValue */
	int recv_small_res_index;	/**< Receive small result - wIndex */
   ...
   * - #TRUE - the firmware is already loaded.
   * - #FALSE - the firmware is not loaded.
   */
   ...
   * - #TRUE - the external power supply is connected, or the scanner does
   *   not need external power.
   * - #FALSE - the external power supply is not connected, so the scanner
   *   will not work.
   */
   ...
   * - #TRUE - the transparency adapter is connected.
   * - #FALSE - the transparency adapter is not connected.
   *
   * @return
   ...
   *
   * @param dev Device object.
   * @param fb_lamp #TRUE turns on the flatbed lamp.
   * @param ta_lamp #TRUE turns on the transparency adapter lamp.
   *
   * @return
   ...
   * - #TRUE  - the scanner carriage is still moving.
   * - #FALSE - the scanner carriage has stopped.
   *
   * @return
...
struct GT68xx_Model
{
   ...
   *
   * This flag must be set to TRUE if the structure is dynamically
  ...
  /** @name Scanner model parameters */
	...
	int flags;	/* Which hacks are needed for this scanner? */
	/*@} */
};
 ...
 *
 * @return
 * - #TRUE  - device is configured and can be activated.
 * - #FALSE - device is not configured; attempt to activate it will fail.
 */
 ...
 */
static SANE_Status
gt68xx_device_memory_write(GT68xx_Device * dev, int addr,
			   int size, SANE_Byte * data);
 ...
 */
static SANE_Status
gt68xx_device_memory_read(GT68xx_Device * dev, int addr,
			  int size, SANE_Byte * data);
 ...
 * - #TRUE  - the TA is connected.
 * - #FALSE - the TA is not connected.
 *
 * @return
 * - #SANE_STATUS_GOOD        - success.
 ...
 * - #TRUE  - the scanner is ready to send data.
 * - #FALSE - the scanner is not ready (e.g., the carriage has not reached
