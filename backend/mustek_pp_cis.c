...
static void
cis_move_motor(Mustek_PP_CIS_dev * dev, SANE_Int steps)
{				/* steps @ maxres */
		     ...
		     SANE_Int pixels, SANE_Int first, SANE_Int last)
{
	...
	memset((void *) &max, 0,
	       3 * MUSTEK_PP_CIS_MAX_H_PIXEL * sizeof(SANE_Byte));
	memset((void *) &sum, 0,
	       3 * MUSTEK_PP_CIS_MAX_H_PIXEL * sizeof(SANE_Int));
...
cis_return_home(Mustek_PP_CIS_dev * dev, SANE_Bool nowait)
{
	...
	if (nowait == SANE_FALSE)
		Mustek_PP_1015_wait_bit(dev, MA1015R_MOTOR,
					MA1015B_MOTOR_HOME, SANE_TRUE, 1000);
}
...
cis_attach(SANE_String_Const port,
	   ...
	   SANE_Attach_Callback attach, SANE_Int driverNo, SANE_Int info)
{
	int fd;
	SANE_Status status;
	u_char asic;
...
SANE_Status
cis600_drv_init(SANE_Int options, SANE_String_Const port,
...
SANE_Status
cis1200_drv_init(SANE_Int options, SANE_String_Const port,
		 SANE_String_Const name, SANE_Attach_Callback attach)
{
	if (options != CAP_NOTHING)
		return SANE_STATUS_INVAL;
...
SANE_Status
cis1200p_drv_init(SANE_Int options, SANE_String_Const port,
		  SANE_String_Const name, SANE_Attach_Callback attach)
{
	if (options != CAP_NOTHING)
		return SANE_STATUS_INVAL;
...
******************************************************************************/
void
cis_drv_capabilities(SANE_Int info, SANE_String * model,
...
******************************************************************************/
SANE_Status
cis_drv_open(SANE_String port, SANE_Int caps, SANE_Int * fd)
{
	SANE_Status status;
