...
struct device_s
{
	...
	int width;
	int height;
	int optionw[OPTION_MAX];
	uint32_t conf_data[512];
	uint32_t packet_data[512];
};


static void do_cancel(struct device_s *dev);
...
/* Init the whole structure with default values */
	...
	dev->optiond[RES_OFFSET].type = SANE_TYPE_INT;
	dev->optiond[RES_OFFSET].size = sizeof(int);
	dev->optiond[RES_OFFSET].cap =
	...
	dev->optiond[X1_OFFSET].size = sizeof(int);
	dev->optiond[X1_OFFSET].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	dev->optiond[Y1_OFFSET].size = sizeof(int);
	dev->optiond[Y1_OFFSET].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	dev->optiond[X2_OFFSET].size = sizeof(int);
	dev->optiond[X2_OFFSET].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	dev->optiond[Y2_OFFSET].size = sizeof(int);
	dev->optiond[Y2_OFFSET].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	dev->optiond[BRIGH_OFFSET].size = sizeof(int);
	...
	dev->optiond[CONTR_OFFSET].size = sizeof(int);
	...
	dev->optiond[COLOR_OFFSET].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	dev->optiond[COLOR_OFFSET].constraint_type =
		 ...
		 SANE_Bool __sane_unused__ local_only)
{
	...
	for (i = 0, iter = devlist_head; i < devlist_count;
	     i++, iter = iter->next) {
...
sane_get_option_descriptor(SANE_Handle h, int option)
{
	struct device_s *dev = (struct device_s *) h;
...
static SANE_Status
getvalue(SANE_Handle h, int option, void *v)
{
	struct device_s *dev = (struct device_s *) h;

	if (option != COLOR_OFFSET)
		*((int *) v) = dev->optionw[option];
	else {
		strcpy((char *) v,
...
static SANE_Status
setvalue(SANE_Handle h, int option, void *value, int * info)
{
	struct device_s *dev = (struct device_s *) h;
	...
	switch (option) {
	case X1_OFFSET:
		dev->optionw[option] = *((int *) value);
		s_unit = (int)
			round((dev->optionw[option] / ((double) MAX_X_S))
			      * MAX_X_H);
		s_unit_2 = (int)
			round((dev->optionw[X2_OFFSET] / ((double) MAX_X_S))
			      * MAX_X_H);
		if (abs(s_unit_2 - s_unit) < MIN_SCAN_ZONE)
			s_unit = s_unit_2 - MIN_SCAN_ZONE;
		dev->optionw[option] =
			round((s_unit / ((double) MAX_X_H)) * MAX_X_S);
		if (info)
	...
	case X2_OFFSET:
		...
		s_unit = (int)
			round((dev->optionw[option] / ((double) MAX_X_S))
			      * MAX_X_H);
		s_unit_2 = (int)
			round((dev->optionw[X1_OFFSET] / ((double) MAX_X_S))
			      * MAX_X_H);
		if (abs(s_unit_2 - s_unit) < MIN_SCAN_ZONE)
			s_unit = s_unit_2 + MIN_SCAN_ZONE;
		dev->optionw[option] =
	...
	case Y1_OFFSET:
		...
		s_unit = (int)
			round((dev->optionw[option] / ((double) MAX_Y_S))
			      * MAX_Y_H);

		s_unit_2 = (int)
			round((dev->optionw[Y2_OFFSET] / ((double) MAX_Y_S))
			      * MAX_Y_H);
		if (abs(s_unit_2 - s_unit) < MIN_SCAN_ZONE)
			s_unit = s_unit_2 - MIN_SCAN_ZONE;

		dev->optionw[option] =
			round((s_unit / ((double) MAX_Y_H)) * MAX_Y_S);
		if (info)
			*info |= SANE_INFO_INEXACT;
		break;
	case Y2_OFFSET:
		...
		s_unit_2 = (int)
			round((dev->optionw[Y1_OFFSET] / ((double) MAX_Y_S))
			      * MAX_Y_H);
		if (abs(s_unit_2 - s_unit) < MIN_SCAN_ZONE)
			s_unit = s_unit_2 + MIN_SCAN_ZONE;
...
SANE_Status
sane_control_option(SANE_Handle h, int option,
		    SANE_Action a, void *v, int * i)
{
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * p)
{
	...
	p->format =
		dev->optionw[COLOR_OFFSET] ==
		RGB ? SANE_FRAME_RGB : SANE_FRAME_GRAY;
	p->last_frame = TRUE;
	p->depth = 8;
...
send_pkt(int command, int data_size, struct device_s *dev)
{
	...
	sanei_usb_write_bulk(dev->dn, (unsigned char *) dev->packet_data,
			     &size);
}
...
send_conf(struct device_s *dev)
{
	...
	y1 = (int) round((dev->optionw[Y1_OFFSET] / ((double) MAX_Y_S)) *
			 MAX_Y_H);
	y2 = (int) round((dev->optionw[Y2_OFFSET] / ((double) MAX_Y_S)) *
			 MAX_Y_H);
	x1 = (int) round((dev->optionw[X1_OFFSET] / ((double) MAX_X_S)) *
			 MAX_X_H);
	x2 = (int) round((dev->optionw[X2_OFFSET] / ((double) MAX_X_S)) *
			 MAX_X_H);
	...
	sanei_usb_write_bulk(dev->dn, (unsigned char *) dev->conf_data,
			     &size);
}
...
get_data(struct device_s *dev)
{
		...
		if (size) {
			if (ntohl(dev->packet_data[0]) == MAGIC_NUMBER) {
				if (ntohl(dev->packet_data[1]) == PKT_DATA)
					break;
				if (ntohl(dev->packet_data[1]) ==
				    PKT_END_DATA) {
					dev->status = STATUS_IDLE;
					DBG(100,
					    "End of scan encountered on device %s\n",
					    dev->devname);
	...
	do {
		...
		case GRAY_LAYER:
			...
			memcpy(dev->buffer + dev->write_offset_r, buffer,
			       size);
...
sane_start(SANE_Handle h)
{
	...
	if ((size = wait_ack(dev, NULL))) {
		sanei_usb_read_bulk(dev->dn, (unsigned char *) dev->conf_data,
				    &size);
	}
