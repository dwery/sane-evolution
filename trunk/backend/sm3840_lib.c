		  ...
		  unsigned char *bytes, int size, int timeout)
{
	...
	ep = ep;
	my_size = size;
	status = sanei_usb_write_bulk((SANE_Int) dev, (SANE_Byte *) bytes,
				      &my_size);
		 ...
		 unsigned char *bytes, int size, int timeout)
{
	...
	ep = ep;
	my_size = size;
	status = sanei_usb_read_bulk((SANE_Int) dev, (SANE_Byte *) bytes,
				     &my_size);
	if (status != SANE_STATUS_GOOD)
		my_size = -1;
	return my_size;
...
idle_ab(usb_dev_handle * udev)
{
	...
	unsigned char buff[8] =
		{ 0x64, 0x65, 0x16, 0x17, 0x64, 0x65, 0x44, 0x45 };
	for (i = 0; i < 8; i++) {
		len = usb_control_msg(udev, 0x40, 0x0c, 0x0090, 0x0000,
				      buff + i, 0x0001, wr_timeout);
	}
}
	    ...
	    unsigned char **save_color_remap)
{
	...
	if (reset) {
		...
		*save_scan_line = scan_line =
			(unsigned char *) calloc(linelen, 1);
		if (dpi == 1200)
			*save_dpi1200_remap = dpi1200_remap =
				(unsigned char *) calloc(linelen,
							 DPI1200SHUFFLE);
		else
	...
	while (1) {		/* We'll exit inside the loop... */
		len = usb_bulk_read(udev, 1, scan_line, linelen, rd_timeout);
		if (dpi == 1200) {
			ptrcur = dpi1200_remap +
				(linelen * (i % DPI1200SHUFFLE));
			ptrprev =
				dpi1200_remap +
				(linelen *
				 ((i -
				   (DPI1200SHUFFLE - 2)) % DPI1200SHUFFLE));
		} else {
			ptrcur = scan_line;
			ptrprev = NULL;
		}
		if (gray) {
			...
			memcpy(color_remap + linelen * lineptr, scan_line,
			       linelen);

			outline++;
			if (outline > bufflines) {
				int redline =
					(outline + red_delay) % bufflines;
				int greenline =
					(outline + green_delay) % bufflines;
				int blueline =
					(outline + blue_delay) % bufflines;
				unsigned char *rp, *gp, *bp;

				rp = color_remap + linelen * redline;
				gp = color_remap + linelen * greenline +
					1 * pixsize;
				bp = color_remap + linelen * blueline +
					2 * pixsize;
		...
		if (dpi != 1200) {
			...
			if (i > (DPI1200SHUFFLE + blue_delay)) {
				...
				*save_i = i;
				if (bpp16)
					fix_endian_short((unsigned short *)
							 storeline, scanpix);
				return;
	     ...
	     int scanpix, int scanlines, int gray, char *head, int bpp16)
{
	...
	storeline =
		(unsigned char *) malloc(scanpix * (gray ? 1 : 3) *
					 (bpp16 ? 2 : 1));
	...
	for (i = 0; i < scanlines; i++) {
		record_line((i == 0) ? 1 : 0, udev, storeline, dpi, scanpix,
			    gray, bpp16, &save_i, &save_scan_line,
			    &save_dpi1200_remap, &save_color_remap);
		fwrite(storeline, scanpix * (gray ? 1 : 3) * (bpp16 ? 2 : 1),
		       1, fp);
	}
	fclose(fp);
	if (save_scan_line)
...
record_mem(usb_dev_handle * udev, unsigned char **dest, int bytes)
{
	...
	*dest = mem;
	do {
		len = usb_bulk_read(udev, 1, buff,
				    bytes > 65536 ? 65536 : bytes,
				    rd_timeout);
		if (len > 0) {
			memcpy(mem, buff, len);
			bytes -= len;
...
select_pixels(unsigned short *map, int dpi, int start, int end)
{
	...
	for (i = 0; i < end; i++) {
		...
		if (x < 0
		    || x > ((dpi == 1200) ? RAWPIXELS1200 : RAWPIXELS600))
			DBG(2, "ERR %d\n", x);
		else
			map[x] |= SCANMAP;
	}
...
set_lamp_timer(usb_dev_handle * udev, int timeout_in_mins)
{
	write_regs(udev, 7, 0xa8, 0x80, 0x83, 0xa2, 0x85, 0xc8, 0x83, 0x82,
		   0x85, 0xaf, 0x83, 0x00, 0x93, 0x00);
	write_regs(udev, 3, 0xa8, timeout_in_mins * 2, 0x8c, 0x08, 0x94,
		   0x00);
	idle_ab(udev);
...
download_lut8(usb_dev_handle * udev, int dpi, int incolor)
{
	...
	case 600:
		...
		if (!incolor) {
			...
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x20, 0xb2,
				   0x07, 0xb3, 0xff, 0xb4, 0x2f, 0xb5, 0x07);
			write_vctl(udev, 0x0c, 0x0002, 0x1000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 4096, wr_timeout);
		} else {
			calculate_lut8(color, 2, lut);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x10, 0xb2,
				   0x07, 0xb3, 0xff, 0xb4, 0x1f, 0xb5, 0x07);
			write_vctl(udev, 0x0c, 0x0002, 0x1000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 4096, wr_timeout);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x20, 0xb2,
				   0x07, 0xb3, 0xff, 0xb4, 0x2f, 0xb5, 0x07);
			write_vctl(udev, 0x0c, 0x0002, 0x1000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 4096, wr_timeout);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x30, 0xb2,
				   0x07, 0xb3, 0xff, 0xb4, 0x3f, 0xb5, 0x07);
			write_vctl(udev, 0x0c, 0x0002, 0x1000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 4096, wr_timeout);
		}
		break;
	...
	default:
		...
		if (!incolor) {
			...
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x40, 0xb2,
				   0x06, 0xb3, 0xff, 0xb4, 0x5f, 0xb5, 0x06);
			write_vctl(udev, 0x0c, 0x0002, 0x2000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 8192, wr_timeout);
		} else {
			calculate_lut8(color, 1, lut);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x20, 0xb2,
				   0x06, 0xb3, 0xff, 0xb4, 0x3f, 0xb5, 0x06);
			write_vctl(udev, 0x0c, 0x0002, 0x2000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 8192, wr_timeout);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x40, 0xb2,
				   0x06, 0xb3, 0xff, 0xb4, 0x5f, 0xb5, 0x06);
			write_vctl(udev, 0x0c, 0x0002, 0x2000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 8192, wr_timeout);
			write_regs(udev, 6, 0xb0, 0x00, 0xb1, 0x60, 0xb2,
				   0x06, 0xb3, 0xff, 0xb4, 0x7f, 0xb5, 0x06);
			write_vctl(udev, 0x0c, 0x0002, 0x2000, 0x00);
			len = usb_bulk_write(udev, 2, lut, 8192, wr_timeout);
		}
		break;
...
fix_endian_short(unsigned short *data, int count)
{
	...
	for (i = 0; i < count; i++)
		data[i] =
			((data[i] >> 8) & 0x00ff) | ((data[i] << 8) & 0xff00);
}
