...
artec_str_list_to_word_list(SANE_Word ** word_list_ptr, SANE_String str)
{
	...
	word_list = (SANE_Word *) calloc(comma_count + 1, sizeof(SANE_Word));
...
abort_scan(SANE_Handle handle)
{
	...
	data[2] =
		0x02 | ((s->val[OPT_TRANSPARENCY].w == SANE_TRUE) ? 0x04 :
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_RESOLUTION_BIND:
			if (s->val[option].w != *(SANE_Word *) val) {
				...
				if (s->val[option].w == SANE_FALSE) {	/* don't bind */
...
static SANE_Status
artec_sane_read(SANE_Handle handle, SANE_Byte * buf, SANE_Int max_len,
		SANE_Int * len)
{
	ARTEC_Scanner *s = handle;
	SANE_Status status;
