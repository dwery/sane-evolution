...
/*
   $Id: teco1.c,v 1.16 2005-09-19 19:57:48 fzago-guest Exp $
...
teco_identify_scanner(Teco_Scanner * dev)
{
...
#ifdef sim
	{
		...
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x24, 0x00, 0x00, 0x10, 0x44,
			0x46,
			0x2D, 0x36, 0x30, 0x30, 0x4D, 0x20, 0x20, 0x20, 0x20,
			0x20,
			0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
			0x20,
			0x20, 0x20, 0x31, 0x2E, 0x31, 0x37, 0x31, 0x2E, 0x31,
			0x37,
			0x02
		};
#endif
		...
		/* vm4542 */
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x30, 0x00, 0x00, 0x10, 0x52,
			0x45, 0x4c, 0x49,
			0x53, 0x59, 0x53, 0x20, 0x52, 0x45, 0x4c, 0x49, 0x20,
			0x34, 0x38,
			0x33, 0x30, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
			0x31, 0x2e,
			0x30, 0x33, 0x31, 0x2e, 0x30, 0x33, 0x02, 0x00, 0x54,
			0x45, 0x43,
...
static SANE_Status
teco_sense_handler(int __sane_unused__ scsi_fd, unsigned char *result,
...
teco_send_gamma(Teco_Scanner * dev)
{
	...
	if (dev->val[OPT_CUSTOM_GAMMA].w) {
		...
		if (dev->scan_mode == TECO_GRAYSCALE) {
			...
			/* Color */
			for (i = 0; i < GAMMA_LENGTH; i++) {
				param.gamma[0 * GAMMA_LENGTH + i] =
					dev->gamma_R[i];
				param.gamma[1 * GAMMA_LENGTH + i] =
					dev->gamma_G[i];
				param.gamma[2 * GAMMA_LENGTH + i] =
					dev->gamma_B[i];
				param.gamma[3 * GAMMA_LENGTH + i] = 0;
			}
		}
		...
		if (dev->scan_mode == TECO_BW) {
			...
			unsigned int threshold =
				dev->val[OPT_THRESHOLD].w * (GAMMA_LENGTH /
							     256);
