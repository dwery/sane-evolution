...
sanei_rts88xx_write_reg(SANE_Int devnum, SANE_Int index, SANE_Byte * reg)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_write_reg: bulk write failed\n");
		return status;
	}
			 ...
			 SANE_Byte * source, SANE_Int length)
{
	...
	if (DBG_LEVEL > DBG_io) {
		...
		DBG(DBG_io,
		    "sanei_rts88xx_write_regs : write_regs(0x%02x,%d)=%s\n",
	...
	if ((start + length > 0xb3) && (length > 1)) {
		...
		size += 4;
		if (sanei_usb_write_bulk(devnum, buffer, &size) !=
		    SANE_STATUS_GOOD) {
			DBG(DBG_error,
			...
			SANE_Byte * dest, SANE_Int length)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_read_regs: failed to write header\n");
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_read_regs: failed to read data\n");
		return status;
	}
	if (size != (size_t) length) {
		DBG(DBG_warn,
		    "sanei_rts88xx_read_regs: read got only %lu bytes\n",
		    (u_long) size);
	}
	if (DBG_LEVEL >= DBG_io) {
		for (i = 0; i < size; i++)
		...
		DBG(DBG_io,
		    "sanei_rts88xx_read_regs: read_regs(0x%02x,%d)=%s\n",
		    start, length, message);
	}
	return status;
}
...
sanei_rts88xx_reset_lamp(SANE_Int devnum, SANE_Byte * regs)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_reset_lamp: failed to get status\n");
		return status;
	}
		      ...
		      SANE_Byte ctrl2, SANE_Int length, SANE_Byte * value)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_set_mem: failed to write memory\n");
	}
	return status;
}
		      ...
		      SANE_Byte ctrl2, SANE_Int length, SANE_Byte * value)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_get_mem: failed to read memory\n");
	}
	return status;
}
...
sanei_rts88xx_nvram_ctrl(SANE_Int devnum, SANE_Int length, SANE_Byte * value)
{
	...
	if (DBG_LEVEL > DBG_io) {
		...
		DBG(DBG_io,
		    "sanei_rts88xx_nvram_ctrl : nvram_ctrl(0x00,%d)=%s\n",
		    length, message);
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_nvram_ctrl : write failed ...\n");
	}
	return status;
}
 ...
 */
SANE_Status
sanei_rts88xx_setup_nvram(SANE_Int devnum, SANE_Int length, SANE_Byte * value)
{
	SANE_Status status = SANE_STATUS_GOOD;
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_setup_nvram : failed step #1 ...\n");
		return status;
	}
	local[0] = 0x18;
	local[1] = 0x08;
	for (i = 0; i < 8; i++) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sanei_rts88xx_setup_nvram : failed loop #%d ...\n",
			    i);
			return status;
		}
...
sanei_rts88xx_data_count(SANE_Int devnum, SANE_Word * count)
{
	...
	*count = 0;
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_data_count : failed to write header\n");
		return status;
	}
	size = 3;
	...
	*count = result[0] + (result[1] << 8) + (result[2] << 16);
	DBG(DBG_io2,
	    "sanei_rts88xx_data_count: %d bytes available (0x%06x)\n", *count,
	    *count);
	return status;
}
...
sanei_rts88xx_wait_data(SANE_Int devnum, SANE_Bool busy, SANE_Word * count)
{
	...
	while (SANE_TRUE) {
		...
		if (*count != 0) {
			DBG(DBG_io,
			    "sanei_rts88xx_wait_data: %d bytes available\n",
			    *count);
			return SANE_STATUS_GOOD;
		}
			...
			unsigned char *dest)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_rts88xx_read_data: failed to write header\n");
	}
	read = 0;
	...
	while (len - read > 64) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sanei_rts88xx_read_data: failed to read data\n");
			return status;
		}
