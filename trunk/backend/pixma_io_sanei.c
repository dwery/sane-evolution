...
get_descriptor(int dn, int type, int descidx,
	       int index, int length, SANE_Byte * data)
{
...
read_serial_number(scanner_info_t * si)
{
	...
	if (iSerialNumber != 0) {
		...
		if (get_string_descriptor(usb, 0, 0, 4, unicode) !=
		    SANE_STATUS_GOOD)
			goto done;
		...
		if (len > (int) sizeof(unicode)) {
			len = sizeof(unicode);
			PDBG(pixma_dbg
			     (1, "WARNING:Truncated serial number\n"));
		}
		serial[8] = '_';
		for (i = 2; i < len; i += 2) {
	...
	for (i = 0; pixma_devices[i]; i++) {
		for (cfg = pixma_devices[i]; cfg->name; cfg++) {
			...
			while (j < nscanners) {
				PDBG(pixma_dbg
				     (3,
				      "pixma_collect_devices() found %s at %s\n",
				      cfg->name, si->devname));
				si->cfg = cfg;
...
pixma_write(pixma_io_t * io, const void *cmd, unsigned len)
{
	...
	if (count != len) {
		PDBG(pixma_dbg
		     (1, "WARNING:pixma_write(): count(%u) != len(%u)\n",
		      (unsigned) count, len));
		error = PIXMA_EIO;
	}
	if (error >= 0)
