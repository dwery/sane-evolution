...
/* 
	$Id: teco2.h,v 1.11 2005-07-29 20:27:12 gekl-guest Exp $
*/
...
#define mmToIlu(mm) (((mm) * dev->def->x_resolution_max) / SANE_MM_PER_INCH)
#define iluToMm(ilu) (((ilu) * SANE_MM_PER_INCH) / dev->def->x_resolution_max)
...
typedef struct Teco_Scanner
{
	...
	int gamma_GRAY[GAMMA_LENGTH];
	int gamma_R[GAMMA_LENGTH];
	int gamma_G[GAMMA_LENGTH];
	int gamma_B[GAMMA_LENGTH];
}
Teco_Scanner;
