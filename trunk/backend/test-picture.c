...
init_picture_buffer(Test_Device * test_device, SANE_Byte ** buffer,
		    size_t * buffer_size)
{
	int pattern_size = 0, pattern_distance = 0;
	int line_count, b_size;
	int lines = 0;
	int bpl = test_device->bytes_per_line;
	int ppl = test_device->pixels_per_line;
	SANE_Byte *b;
	...
	DBG(2, "(child) init_picture_buffer test_device=%p, buffer=%p, "
	    "buffer_size=%p\n", (void *) test_device, (void *) buffer,
	    (void *) buffer_size);

	if (strcmp(test_device->val[opt_test_picture].s, "Solid black") == 0
	    || strcmp(test_device->val[opt_test_picture].s,
		      "Solid white") == 0) {
		SANE_Byte pattern = 0;
		...
		if (!b) {
			DBG(1,
			    "(child) init_picture_buffer: couldn't malloc buffer\n");
		...
		if (strcmp
		    (test_device->val[opt_test_picture].s,
		     "Solid black") == 0) {
			DBG(3,
			    "(child) init_picture_buffer: drawing solid black test "
			    "picture %d bytes\n", b_size);
			...
			DBG(3,
			    "(child) init_picture_buffer: drawing solid white test "
			    "picture %d bytes\n", b_size);
			if (test_device->params.format == SANE_FRAME_GRAY
			    && test_device->params.depth == 1)
				pattern = 0x00;
	...
	if (strcmp(test_device->val[opt_test_picture].s, "Grid") == 0) {
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing grid test picture "
		...
		for (line_count = 0; line_count < lines; line_count++) {
			...
			for (x = 0; x < bpl; x += increment) {
				...
				if (test_device->params.depth == 1) {
					if (test_device->params.format ==
					    SANE_FRAME_GRAY
					    || (test_device->params.format >=
						SANE_FRAME_RED
						&& test_device->params.
	...
	if (test_device->params.format == SANE_FRAME_GRAY
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing b/w test picture "
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing 8 bit gray test picture "
		    "%d bytes, %d bpl, %d lines\n", b_size, bpl, lines);
		memset(b, 0x55, b_size);
		for (line_count = 0; line_count < lines; line_count++) {
			int x = pattern_distance;
			...
			while (x < bpl) {
				...
				if (x + width >= bpl)
					width = bpl - x;
				if (line_count >
				    (pattern_size + pattern_distance))
					color = 0xff -
						((x /
						  (pattern_size +
						   pattern_distance)) & 0xff);
				else
					color = (x /
						 (pattern_size +
						  pattern_distance)) & 0xff;
				memset(b + line_count * bpl + x, color,
				       width);
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing 16 bit gray test picture "
		    "%d bytes, %d bpl, %d lines\n", b_size, bpl, lines);
		memset(b, 0x55, b_size);
		for (line_count = 0; line_count < lines; line_count++) {
			int x = pattern_distance * 2;
			...
			while (x < bpl) {
				...
				if (x + width >= bpl)
					width = bpl - x;
				pattern_lo = ((line_count - pattern_distance)
					      % (pattern_size +
						 pattern_distance)) & 0xff;
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing color lineart test "
		...
		for (line_count = 0; line_count < lines; line_count++) {
			int x = 0;
			SANE_Byte color = 0, color_r = 0, color_g =
				0, color_b = 0;
			...
			while (x < bpl) {
				...
				for (x2 = 0; x2 < width; x2 += 3) {
					b[line_count * bpl + x + x2 + 0] =
						color_r;
					b[line_count * bpl + x + x2 + 1] =
						color_g;
					b[line_count * bpl + x + x2 + 2] =
						color_b;
				}
				if (line_count < lines / 2) {
					++color;
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing color lineart three-pass "
		...
		for (line_count = 0; line_count < lines; line_count++) {
			...
			while (x < bpl) {
				...
				for (x2 = 0; x2 < width; x2++) {
					if (test_device->params.format ==
					    SANE_FRAME_RED)
						b[line_count * bpl + x + x2] =
							color_r;
					else if (test_device->params.format ==
						 SANE_FRAME_GREEN)
						b[line_count * bpl + x + x2] =
							color_g;
					else
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing 8 bit color test picture "
		    "%d bytes, %d bpl, %d lines\n", b_size, bpl, lines);
		memset(b, 0x55, b_size);
		for (line_count = 0; line_count < lines; line_count++) {
			int x = pattern_distance * 3;
			...
			while (x < bpl) {
				...
				if ((line_count /
				     (pattern_size + pattern_distance)) & 1)
					color = 0xff -
						((x /
						  ((pattern_size +
						    pattern_distance) * 3))
				...
				if (line_count /
				    (pattern_size + pattern_distance) < 2) {
					color_r = color;
					color_g = 0;
					color_b = 0;
				} else if (line_count /
				...
				for (x1 = 0; x1 < width; x1 += 3) {
					b[line_count * bpl + x + x1 + 0] =
						color_r;
					b[line_count * bpl + x + x1 + 1] =
						color_g;
					b[line_count * bpl + x + x1 + 2] =
						color_b;
				}
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing 8 bit color three-pass "
		    "test picture %d bytes, %d bpl, %d lines\n", b_size, bpl,
		    lines);
		memset(b, 0x55, b_size);
		for (line_count = 0; line_count < lines; line_count++) {
		...
		if (buffer)
			*buffer = b;
		DBG(3,
		    "(child) init_picture_buffer: drawing 16 bit color three-pass "
		    "test picture %d bytes, %d bpl, %d lines\n", b_size, bpl,
		    lines);
		memset(b, 0x55, b_size);
		for (line_count = 0; line_count < lines; line_count++) {
