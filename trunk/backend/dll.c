...
#define PRELOAD_DECL(name)                                                            \
  ...
  extern void *BE_ENTRY(name,read) (SANE_Handle, SANE_Byte *, int, int *);                  \
  extern void *BE_ENTRY(name,cancel) (SANE_Handle);                \
  extern void *BE_ENTRY(name,set_io_mode) (SANE_Handle, SANE_Bool);           \
  extern void *BE_ENTRY(name,get_select_fd) (SANE_Handle, int *);
...
static const char *op_name[] = {
	"init", "exit", "get_devices", "open", "close",
...
#else
static const char *op_name[] = {
	"sane_init", "sane_exit", "sane_get_devices", "sane_open",
	"sane_close", "sane_get_option_descriptor",
	"sane_control_option", "sane_get_parameters", "sane_start",
	"sane_read", "sane_cancel",
...
load(struct backend *be)
{
	...
	image_id id = -1;
	int i, w;
	directory_which which[3] =
		{ B_USER_ADDONS_DIRECTORY, B_COMMON_ADDONS_DIRECTORY,
		B_BEOS_ADDONS_DIRECTORY
	};

	/* look for config files in SANE/conf */
	for (w = 0; (w < 3) && (id < 0)
	     && (find_directory(which[w], 0, true, path, PATH_MAX) == 0); w++) {
		strcat(path, "/SANE/");
		strcat(path, be->name);
		...
		for (i = 0; i < NUM_OPS; ++i) {
			...
			/* Look for the symbol */
			if ((get_image_symbol
			     (id, op_name[i], B_SYMBOL_TYPE_TEXT,
			      (void **) &op) < 0) || !op)
...
# error "Tried to compile unsupported DLL."
	...
	while (dir) {
		...
		snprintf(libname, sizeof(libname),
			 "%s/" PREFIX "%.2s%.5s" POSTFIX, dir, be->name,
			 strlen(be->name) >
			 7 ? (be->name) + strlen(be->name) - 5 : (be->name) +
			 2, V_MAJOR);
#else
		...
		/* Some platforms have two ways of storing their libraries, try both
		   postfixes */
		snprintf(libname, sizeof(libname),
			 "%s/" PREFIX "%s" ALT_POSTFIX, dir, be->name,
			 V_MAJOR);
		DBG(4, "load: trying to load `%s'\n", libname);
		fp = fopen(libname, "r");
		if (fp)
...
#elif defined(HAVE_NSLINKMODULE)
	{
		...
		if (NSCreateObjectFileImageFromFile(libname, &objectfile_img)
			...
			be->handle =
				NSLinkModule(objectfile_img, libname, mode);
...
# error "Tried to compile unsupported DLL."
	...
	for (i = 0; i < NUM_OPS; ++i) {
		...
		{
			NSSymbol *nssym =
				NSLookupSymbolInModule(be->handle, funcname);
			if (!nssym) {
				DBG(15, "dyld error: %s\n",
				    dyld_get_error_str());
			} else {
				op = (void *(*)(void))
					NSAddressOfSymbol(nssym);
			}
		}
#else
# error "Tried to compile unsupported DLL."
...
#elif defined(HAVE_SHL_LOAD)
			shl_findsym(be->handle, funcname, TYPE_UNDEFINED,
				    &op);
#elif defined(HAVE_NSLINKMODULE)
			{
				NSSymbol *nssym =
					NSLookupSymbolInModule(be->handle,
							       funcname);
				if (!nssym) {
					DBG(15, "dyld error: %s\n",
					    dyld_get_error_str());
...
sane_exit(void)
{
	...
	for (be = first_backend; be; be = next) {
		...
		if (be->loaded) {
			if (be->inited) {
				DBG(3,
				    "sane_exit: calling backend `%s's exit function\n",
				    be->name);
			...
			if (be->handle)
				unload_add_on((image_id) be->handle);
#else
#ifdef HAVE_DLL
...
# error "Tried to compile unsupported DLL."
	...
	if (NULL != devlist) {	/* Release memory allocated by sane_get_devices(). */
		int i = 0;
		while (devlist[i])
			free(devlist[i++]);
		free(devlist);
...
#define ASSERT_SPACE(n)                                                    \
  {                                                                        \
	...
	for (be = first_backend; be; be = be->next) {
		...
		status = (*(op_get_devs_t) be->op[OP_GET_DEVS]) (&be_list,
								 local_only);
		...
		for (i = 0; i < num_devs; ++i) {
			...
			for (alias = first_alias; alias != NULL;
			     alias = alias->next) {
				len = strlen(be->name);
				if (strlen(alias->oldname) <= len)
					continue;
				if (strncmp(alias->oldname, be->name, len) ==
				    0 && alias->oldname[len] == ':'
				    && strcmp(&alias->oldname[len + 1],
					      be_list[i]->name) == 0)
					break;
			}
			...
			if (alias) {
				...
				/* create a new device entry with a device name that is the
				   ...
				   name: */
				len = strlen(be->name) + 1 +
					strlen(be_list[i]->name);
				mem = malloc(sizeof(*dev) + len + 1);
				if (!mem)
					return SANE_STATUS_NO_MEM;
...
sane_get_option_descriptor(SANE_Handle handle, int option)
{
	...
	DBG(3, "sane_get_option_descriptor(handle=%p,option=%d)\n", handle,
	    option);
	return (*(op_get_option_desc_t) s->be->op[OP_GET_OPTION_DESC]) (s->
									handle,
									option);
}
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	return (*(op_ctl_option_t) s->be->op[OP_CTL_OPTION]) (s->handle,
							      option, action,
							      value, info);
}
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	DBG(3, "sane_get_parameters(handle=%p,params=%p)\n", handle,
	    (void *) params);
	return (*(op_get_params_t) s->be->op[OP_GET_PARAMS]) (s->handle,
							      params);
}
	  ...
	  int * length)
{
	...
	return (*(op_read_t) s->be->op[OP_READ]) (s->handle, data, max_length,
						  length);
}
...
sane_get_select_fd(SANE_Handle handle, int * fd)
{
	...
	return (*(op_get_select_fd_t) s->be->op[OP_GET_SELECT_FD]) (s->handle,
								    fd);
}
