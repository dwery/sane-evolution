...
/* $Id: snapscan-options.c,v 1.35 2006-01-06 20:59:17 oliver-guest Exp $
...
#define DEFAULT_HIGHQUALITY     FALSE
#define DEFAULT_BRIGHTNESS      0
#define DEFAULT_CONTRAST        0
#define DEFAULT_GAMMA           SANE_FIX(1.8)
#define DEFAULT_HALFTONE        FALSE
#define DEFAULT_NEGATIVE        FALSE
#define DEFAULT_THRESHOLD       50
#define DEFAULT_QUALITY         TRUE
#define DEFAULT_CUSTOM_GAMMA    FALSE
#define DEFAULT_GAMMA_BIND      FALSE
...
static char lpr_desc[] =
	SANE_I18N("Number of scan lines to request in a SCSI read. "
...
init_options(SnapScan_Scanner * ps)
{
	...
	po[OPT_COUNT].size = sizeof(int);
	po[OPT_COUNT].cap = SANE_CAP_SOFT_DETECT;
	{
		static SANE_Range count_range = { NUM_OPTS, NUM_OPTS, 0 };
	...
	po[OPT_SCANRES].size = sizeof(int);
	po[OPT_SCANRES].cap =
	...
	po[OPT_PREVIEW].size = sizeof(int);
	po[OPT_PREVIEW].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_AUTOMATIC;
	...
	po[OPT_HIGHQUALITY].desc =
		SANE_I18N("Highest quality but lower speed");
	po[OPT_HIGHQUALITY].type = SANE_TYPE_BOOL;
	po[OPT_HIGHQUALITY].unit = SANE_UNIT_NONE;
	po[OPT_HIGHQUALITY].size = sizeof(int);
	...
	case PRISA310:
	case PERFECTION3490:
		po[OPT_PREVIEW_MODE].constraint.string_list =
			preview_names_basic;
		break;
	default:
		po[OPT_PREVIEW_MODE].constraint.string_list =
	...
	po[OPT_SOURCE].constraint_type = SANE_CONSTRAINT_STRING_LIST;
	{
		...
		ps->source_s = (char *) strdup(src_flatbed);
	}
	...
	po[OPT_BIT_DEPTH].size = sizeof(int);
	...
	if ((!(ps->hconfig & HCFG_CAL_ALLOWED))
		...
		ps->val[OPT_QUALITY_CAL].b = FALSE;
	}

	if ((ps->pdev->model == PRISA5150) ||
	    (ps->pdev->model == STYLUS_CX1500)) {
		po[OPT_QUALITY_CAL].cap |= SANE_CAP_INACTIVE;
		ps->val[OPT_QUALITY_CAL].b = TRUE;
	}
	...
	po[OPT_GAMMA_GS].size = sizeof(int);
	po[OPT_GAMMA_GS].cap = SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	po[OPT_GAMMA_R].size = sizeof(int);
	po[OPT_GAMMA_R].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_INACTIVE;
	...
	po[OPT_GAMMA_G].size = sizeof(int);
	po[OPT_GAMMA_G].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_INACTIVE;
	...
	po[OPT_GAMMA_B].size = sizeof(int);
	po[OPT_GAMMA_B].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_INACTIVE;
	...
	po[OPT_GAMMA_VECTOR_GS].size = ps->gamma_length * sizeof(int);
	...
	po[OPT_GAMMA_VECTOR_R].size = ps->gamma_length * sizeof(int);
	...
	po[OPT_GAMMA_VECTOR_G].size = ps->gamma_length * sizeof(int);
	...
	po[OPT_GAMMA_VECTOR_B].size = ps->gamma_length * sizeof(int);
	...
	po[OPT_HALFTONE_PATTERN].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_INACTIVE;
	po[OPT_HALFTONE_PATTERN].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	...
	po[OPT_RGB_LPR].size = sizeof(int);
	po[OPT_RGB_LPR].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
		SANE_CAP_ADVANCED | SANE_CAP_AUTOMATIC;
	...
	po[OPT_GS_LPR].size = sizeof(int);
	po[OPT_GS_LPR].cap = SANE_CAP_SOFT_SELECT
		| SANE_CAP_SOFT_DETECT
		| SANE_CAP_ADVANCED | SANE_CAP_INACTIVE | SANE_CAP_AUTOMATIC;
...
static void
control_options(SnapScan_Scanner * pss)
{
	...
	if ((pss->mode == MD_COLOUR) ||
		...
		if (pss->val[OPT_CUSTOM_GAMMA].b) {
			if (pss->val[OPT_GAMMA_BIND].b) {
				pss->options[OPT_GAMMA_VECTOR_GS].cap &=
					~SANE_CAP_INACTIVE;
			} else {
				pss->options[OPT_GAMMA_VECTOR_R].cap &=
					~SANE_CAP_INACTIVE;
				pss->options[OPT_GAMMA_VECTOR_G].cap &=
					~SANE_CAP_INACTIVE;
				pss->options[OPT_GAMMA_VECTOR_B].cap &=
					~SANE_CAP_INACTIVE;
			}
		} else {
			pss->options[OPT_BRIGHTNESS].cap &=
				~SANE_CAP_INACTIVE;
			pss->options[OPT_CONTRAST].cap &= ~SANE_CAP_INACTIVE;
			if (pss->val[OPT_GAMMA_BIND].b) {
				pss->options[OPT_GAMMA_GS].cap &=
					~SANE_CAP_INACTIVE;
			} else {
				pss->options[OPT_GAMMA_R].cap &=
					~SANE_CAP_INACTIVE;
				pss->options[OPT_GAMMA_G].cap &=
					~SANE_CAP_INACTIVE;
				pss->options[OPT_GAMMA_B].cap &=
					~SANE_CAP_INACTIVE;
			}
		}
...
SANE_Status
sane_control_option(SANE_Handle h,
		    int n, SANE_Action a, void *v, int * i)
{
	...
	case SANE_ACTION_GET_VALUE:
		...
		case OPT_BRIGHTNESS:
			*(int *) v =
				pss->bright << SANE_FIXED_SCALE_SHIFT;
			break;
		case OPT_CONTRAST:
			*(int *) v =
				pss->contrast << SANE_FIXED_SCALE_SHIFT;
			break;
		...
		case OPT_THRESHOLD:
			*(int *) v =
				pss->threshold << SANE_FIXED_SCALE_SHIFT;
			break;
		case OPT_RGB_LPR:
			*(int *) v = pss->rgb_lpr;
			break;
	...
	case SANE_ACTION_SET_VALUE:
		...
		case OPT_MODE:
		{
			...
			if (strcmp(s, md_colour) == 0) {
				...
				pss->options[OPT_HALFTONE].cap |=
					SANE_CAP_INACTIVE;
				pss->options[OPT_HALFTONE_PATTERN].cap |=
					SANE_CAP_INACTIVE;
				pss->options[OPT_NEGATIVE].cap |=
					SANE_CAP_INACTIVE;
				pss->options[OPT_THRESHOLD].cap |=
					SANE_CAP_INACTIVE;
				pss->options[OPT_GS_LPR].cap |=
					SANE_CAP_INACTIVE;
				pss->options[OPT_RGB_LPR].cap &=
					~SANE_CAP_INACTIVE;
				...
				if (ht_cap && pss->halftone) {
					pss->options[OPT_HALFTONE_PATTERN].
	...
	case SANE_ACTION_SET_AUTO:
		...
		case OPT_SOURCE:
			...
			if (pss->source_s)
				free(pss->source_s);
			pss->source_s = (char *) strdup(src_flatbed);
			if (i)
				*i |= SANE_INFO_RELOAD_PARAMS |
					SANE_INFO_RELOAD_OPTIONS;
			break;
