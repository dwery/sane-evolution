...
snapscani_mutex_open(snapscan_mutex_t * sem_id, const char *dev)
{
	...
	if (strstr(dev, "libusb:") == dev) {
		if (sanei_usb_get_vendor_product_byname(dev, NULL, &pid) !=
		    SANE_STATUS_GOOD) {
			DBG(DL_MAJOR_ERROR,
			    "%s: could not obtain USB product ID for device %s\n",
			    me, dev);
			return 0;
		}
		...
		if (sscanf(dev, "libusb:%d:%d", &busnum, &devnum) != 2) {
			DBG(DL_MAJOR_ERROR,
			    "%s: could not parse device string: %s\n", me,
			    strerror(errno));
			return 0;
		}
		...
		DBG(DL_INFO,
		    "%s: using IPC key 0x%08x for device %s (pid 0x%04x, bus 0x%02x, dev 0x%02x)\n",
