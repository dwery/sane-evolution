...
filter_lower_end(int * buffer, int total_count,
		 int filter_count)
{
	int bound = total_count - 1;
	int left_count = total_count - filter_count;
	int temp = 0;
	int i, j;
	int sum = 0;
		  ...
		  int target_dark)
{
	DBG(5, "usb_high_cal_init: start, cal=%p, type=%d, target_white=%d "
	    "target_dark=%d\n", (void *) cal, type, target_white,
	    target_dark);
	cal->is_prepared = FALSE;
	cal->k_white = NULL;
	cal->k_dark = NULL;
	/* Working Buffer */
	cal->white_line = NULL;
...
usb_high_cal_prepare(Calibrator * cal, int max_width)
{
	...
	cal->k_white = (int *) malloc(max_width * sizeof(int));
	if (!cal->k_white)
	...
	cal->k_dark = (int *) malloc(max_width * sizeof(int));
	if (!cal->k_dark)
		return SANE_STATUS_NO_MEM;

	cal->max_width = max_width;

	cal->is_prepared = TRUE;
		   ...
		   int * dark_needed)
{
	...
	*dark_needed = cal->dark_needed;
	...
	cal->white_buffer =
		(int *) malloc(cal->white_needed * cal->width *
				    sizeof(int));
	if (!cal->white_buffer)
		return SANE_STATUS_NO_MEM;
...
usb_high_cal_evaluate_white(Calibrator * cal, double factor)
{
	...
	for (i = 0; i < cal->width; i++) {
		for (j = 0; j < cal->white_needed; j++) {
			*(buffer + j) =
				*(cal->white_buffer + j * cal->width + i);
		}
		average =
...
usb_high_cal_evaluate_calibrator(Calibrator * cal)
{
	...
	if (cal->white_line == NULL) {
		DBG(3,
		    "usb_high_cal_evaluate_calibrator: white_line==NULL\n");
		return FALSE;
	}
	...
	for (i = 0; i < cal->width; i++) {
		...
		if (average <= 0)
			...
			average = 4095;
		cal->k_white[i] = (int) (average);
		cal->k_dark[i] = (int) (cal->dark_line[i]);
	}
	free(cal->dark_line);
	cal->dark_line = NULL;
	free(cal->white_line);
...
SANE_Status
usb_high_cal_fill_in_dark(Calibrator * cal, int major, int minor,
			  void *dark_pattern)
{
	...
	case I8O8RGB:
	case I8O8MONO:
		return usb_high_cal_i8o8_fill_in_dark(cal, major, minor,
						      dark_pattern);
		break;
	case I4O1MONO:
		return usb_high_cal_i4o1_fill_in_dark(cal, major, minor,
						      dark_pattern);
		break;
	}
...
usb_high_cal_i8o8_mono_calibrate(Calibrator * cal, void *src, void *target)
{
	...
	if (cal->gamma_table == NULL) {
		int k_white_level = cal->k_white_level >> 4;
		for (i = 0; i < cal->width; i++) {
			base = (int) ((int) (gray_src[i]) << 4)
				- (int) (cal->k_dark[i]);
			if (base < 0)
				base = 0;
			value = ((int) (base) * k_white_level) /
				cal->k_white[i];
			if (value > 0x00ff)
				value = 0x00ff;
		...
		for (i = 0; i < cal->width; i++) {
			...
			if (base < 0)
				base = 0;
			value = ((int) (base) * cal->k_white_level) /
				cal->k_white[i];
			if (value > 0x0fff)
				value = 0x0fff;
			gray_target[i] =
				(SANE_Byte) (cal->gamma_table[value]);
		}
	}
...
usb_high_scan_init(Mustek_Usb_Device * dev)
{
	...
	dev->get_line = NULL;
	dev->backtrack = NULL;
	dev->is_adjusted_rgb_600_power_delay = FALSE;
	dev->is_adjusted_mono_600_power_delay = FALSE;
	dev->is_adjusted_rgb_300_power_delay = FALSE;
	dev->is_adjusted_mono_300_power_delay = FALSE;
	dev->is_evaluate_pixel_rate = FALSE;
	...
	dev->is_cis_detected = FALSE;
	dev->is_sensor_detected = FALSE;
...
usb_high_scan_prepare(Mustek_Usb_Device * dev)
{
	...
	dev->image_buffer =
		(SANE_Byte *) malloc(dev->init_bytes_per_strip * 3);
	if (!dev->image_buffer)
		return SANE_STATUS_NO_MEM;
...
usb_high_scan_turn_power(Mustek_Usb_Device * dev, SANE_Bool is_on)
{
	...
	if (is_on) {
		if (dev->is_open) {
			DBG(3,
			    "usb_high_scan_turn_power: wanted to turn on power, "
			    "but scanner already open\n");
			return SANE_STATUS_INVAL;
		}
		RIE(usb_low_open(dev->chip, dev->device_name));
		dev->is_open = TRUE;
		RIE(usb_low_turn_peripheral_power(dev->chip, TRUE));
		RIE(usb_low_turn_lamp_power(dev->chip, TRUE));
	} else {
		if (!dev->is_open) {
			DBG(3,
			    "usb_high_scan_turn_power: wanted to turn off power, "
			    "but scanner already closed\n");
			return SANE_STATUS_INVAL;
		}
		RIE(usb_low_turn_lamp_power(dev->chip, FALSE));
...
usb_high_scan_wait_carriage_home(Mustek_Usb_Device * dev)
{
	...
	if (status != SANE_STATUS_GOOD) {
		RIE(usb_low_set_ccd_width
		    (dev->chip, dev->init_min_expose_time));
		RIE(usb_mid_motor_prepare_home(dev->chip));
		do {
			status = usb_low_get_home_sensor(dev->chip);
			if (status != SANE_STATUS_GOOD)
	...
	/* No Motor & Forward */
	RIE(usb_low_move_motor_home(dev->chip, FALSE, FALSE));
...
usb_high_scan_hardware_calibration(Mustek_Usb_Device * dev)
{
	...
	case 1:		/* CIS */
		...
		case RGB24EXT:
			if (usb_mid_sensor_is600_mode(dev->chip, dev->x_dpi)) {
				...
				dev->red_rgb_600_pga =
					dev->init_red_rgb_600_pga;
				dev->green_rgb_600_pga =
					dev->init_green_rgb_600_pga;
				dev->blue_rgb_600_pga =
					dev->init_blue_rgb_600_pga;
				RIE(usb_high_scan_adjust_rgb_600_power_delay
				    (dev));
			} else {
				dev->expose_time = dev->init_expose_time;
				dev->red_rgb_300_pga =
					dev->init_red_rgb_300_pga;
				dev->green_rgb_300_pga =
					dev->init_green_rgb_300_pga;
				dev->blue_rgb_300_pga =
					dev->init_blue_rgb_300_pga;
				RIE(usb_high_scan_adjust_rgb_300_power_delay
				    (dev));
			}
			break;
		case GRAY8EXT:
			if (usb_mid_sensor_is600_mode(dev->chip, dev->x_dpi)) {
				...
				RIE(usb_high_scan_adjust_mono_600_power_delay
				    (dev));
			} else {
				dev->expose_time = dev->init_expose_time;
				dev->mono_300_pga = dev->init_mono_300_pga;
				RIE(usb_high_scan_evaluate_pixel_rate(dev));
				RIE(usb_high_scan_adjust_mono_300_power_delay
				    (dev));
			}
			break;
		default:
	...
	case 3:		/* CCD */
		...
		case GRAY8EXT:
			...
			dev->is_adjusted_mono_600_offset = FALSE;
			/* RIE(usb_high_scan_adjust_mono_600_offset (dev); fixme */
	...
	default:
		dev->expose_time = dev->init_expose_time;
		dev->red_rgb_600_power_delay =
			dev->init_red_rgb_600_power_delay;
		dev->green_rgb_600_power_delay =
			dev->init_green_rgb_600_power_delay;
		dev->blue_rgb_600_power_delay =
			dev->init_blue_rgb_600_power_delay;
		dev->red_mono_600_power_delay =
			dev->init_red_mono_600_power_delay;
		dev->green_mono_600_power_delay =
			dev->init_green_mono_600_power_delay;
		dev->blue_mono_600_power_delay =
			dev->init_blue_mono_600_power_delay;
		dev->red_rgb_600_pga = dev->init_red_rgb_600_pga;
		dev->green_rgb_600_pga = dev->init_green_rgb_600_pga;
		dev->blue_rgb_600_pga = dev->init_blue_rgb_600_pga;
		dev->mono_600_pga = dev->init_mono_600_pga;
		dev->red_rgb_300_power_delay =
			dev->init_red_rgb_300_power_delay;
		dev->green_rgb_300_power_delay =
			dev->init_green_rgb_300_power_delay;
		dev->blue_rgb_300_power_delay =
			dev->init_blue_rgb_300_power_delay;
		dev->red_mono_300_power_delay =
			dev->init_red_mono_300_power_delay;
		dev->green_mono_300_power_delay =
			dev->init_green_mono_300_power_delay;
		dev->blue_mono_300_power_delay =
			dev->init_blue_mono_300_power_delay;
...
usb_high_scan_detect_sensor(Mustek_Usb_Device * dev)
{
	if (dev->is_sensor_detected) {
		DBG(5,
		    "usb_high_scan_detect_sensor: sensor already detected\n");
		return SANE_STATUS_GOOD;
	}
	dev->is_sensor_detected = TRUE;

	switch (dev->chip->scanner_type) {
	case MT_600CU:
		dev->chip->sensor = ST_CANON300;
		dev->chip->motor = MT_600;
		dev->is_cis_detected = TRUE;
		DBG(4,
		    "usb_high_scan_detect_sensor: sensor=Canon 300 dpi, motor="
		    "600 dpi\n");
		break;
	case MT_1200USB:
	...
	case MT_1200CU:	/* need to check if it's a 300600 or 600 dpi sensor */
	{
		...
		RIE(usb_low_turn_peripheral_power(dev->chip, TRUE));
		RIE(usb_low_enable_motor(dev->chip, TRUE));	/* Enable Motor */
		RIE(usb_low_turn_lamp_power(dev->chip, TRUE));
		RIE(usb_low_invert_image(dev->chip, FALSE));
		RIE(usb_low_set_image_dpi(dev->chip, TRUE, SW_P6P6));
		...
		if (r_temp > 50) {
			...
			DBG(4,
			    "usb_high_scan_detect_sensor: sensor=Canon 300/600 dpi, "
			    "motor=1200 dpi\n");
	...
	default:
		DBG(5,
		    "usb_high_scan_detect_sensor: I don't know this scanner type "
			 ...
			 int width)
{
	...
	if (!dev->is_cis_detected) {
		...
		usb_mid_front_enable(dev->chip, TRUE);
	...
	case GRAY8EXT:
		dev->bytes_per_row = dev->width;
		upper_bound =
			((y * 600) / dev->y_dpi) + dev->init_j_lines + 4;
		/* fixme */
		break;
	default:
		upper_bound =
	...
	if (usb_mid_sensor_is600_mode(dev->chip, dev->x_dpi)) {
		/* in 600dpi */
		left_bound =
			(x * 600 / dev->x_dpi) + dev->init_skips_per_row_600;
		dev->skips_per_row =
			(((left_bound % 32) * dev->x_dpi + 300) / 600);
	} else {
		/* in 300dpi */
		left_bound =
			(x * 300 / dev->x_dpi) + dev->init_skips_per_row_300;
		dev->skips_per_row =
			(((left_bound % 32) * dev->x_dpi + 150) / 300);
	}
...
usb_high_scan_init_asic(Mustek_Usb_Device * dev, Sensor_Type sensor)
{
	...
	case ST_TOSHIBA600:
		...
		bank_size = BS_16K;
		DBG(5,
		    "usb_high_scan_init_asic: sensor is set to TOSHIBA600\n");
		break;
	case ST_CANON300:
		ccd_dpi = 232;
		select = 232;
		...
		bank_size = BS_4K;
		DBG(5,
		    "usb_high_scan_init_asic: sensor is set to CANON300\n");
		break;
	case ST_CANON300600:
		ccd_dpi = 232;
		select = 232;
		...
		bank_size = BS_16K;
		DBG(5,
		    "usb_high_scan_init_asic: sensor is set to CANON300600\n");
		break;
	case ST_CANON600:
		ccd_dpi = 232;
		select = 232;
		...
		bank_size = BS_16K;
		DBG(5,
		    "usb_high_scan_init_asic: sensor is set to CANON600\n");
		break;
	...
	RIE(usb_low_set_test_sram_mode(dev->chip, FALSE));
			       ...
			       int length)
{
	...
	*target = (max + min) / 2;
	...
	while (*target != min) {
		RIE(usb_high_scan_evaluate_max_level
		    (dev, dev->init_powerdelay_lines, length, &max_level));
		...
		} else if (max_level == threshold) {	/* Found. */
	...
	/* Fail... */
	if (max == max_max || min == min_min) {	/* Boundary check */
	...
	} else {		/* Fail... will always on mimnum side, make it darker */
		target++;
		*signal_state = SS_DARKER;
	}
...
usb_high_scan_adjust_rgb_600_power_delay(Mustek_Usb_Device * dev)
{
	...
	RIE(usb_mid_front_set_green_offset
	    (dev->chip, dev->init_green_offset));
	...
	dev->is_adjusted_rgb_600_power_delay = TRUE;
...
usb_high_scan_adjust_mono_600_power_delay(Mustek_Usb_Device * dev)
{
	...
	dev->is_adjusted_mono_600_power_delay = TRUE;
...
usb_high_scan_adjust_mono_600_exposure(Mustek_Usb_Device * dev)
{
	...
	dev->is_adjusted_mono_600_exposure = TRUE;
...
{
	DBG(5,
	    "usb_high_scan_adjust_mono_600_skips_per_row: start (dev = %p)\n",
	    dev);
...
usb_high_scan_adjust_rgb_300_power_delay(Mustek_Usb_Device * dev)
{
	...
	RIE(usb_high_scan_bssc_power_delay
	    (dev, &usb_low_set_red_pd, &signal_state,
	     ...
	     dev->init_max_power_delay, dev->adjust_length_300));
	dev->is_adjusted_rgb_300_power_delay = TRUE;
...
usb_high_scan_adjust_mono_300_power_delay(Mustek_Usb_Device * dev)
{
	...
	dev->is_adjusted_mono_300_power_delay = TRUE;
...
usb_high_scan_evaluate_pixel_rate(Mustek_Usb_Device * dev)
{
	...
	/* fixme: new for CCD */
	dev->pixel_rate = 2000;
	dev->is_evaluate_pixel_rate = TRUE;
...
usb_high_scan_calibration_rgb_24(Mustek_Usb_Device * dev)
{
	...
	DBG(5, "usb_high_scan_calibration_rgb_24: start, dev=%p\n",
	    (void *) dev);
	if (dev->is_cis_detected) {
		RIE(usb_mid_motor_prepare_calibrate_rgb
		    (dev->chip, dev->y_dpi));
		RIE(usb_low_turn_lamp_power(dev->chip, TRUE));
		minor_average = 2;
	} else {
	...
	RIE(usb_high_cal_setup
	    (dev->red_calibrator, 1, minor_average, 8, dev->width,
	     &white_need, &dark_need));
	...
	RIE(usb_high_cal_embed_gamma
	    (dev->green_calibrator, dev->gamma_table));
	RIE(usb_high_cal_setup
	    (dev->green_calibrator, 1, minor_average, 8, dev->width,
	     &white_need, &dark_need));
	...
	RIE(usb_low_enable_motor(dev->chip, FALSE));
	RIE(usb_low_turn_lamp_power(dev->chip, FALSE));
...
usb_high_scan_step_forward(Mustek_Usb_Device * dev, int step_count)
{
	...
	/* Initialize */
	RIE(usb_low_set_ccd_width(dev->chip, dev->init_min_expose_time));
	RIE(usb_low_set_motor_direction(dev->chip, FALSE));
	RIE(usb_mid_motor_prepare_step(dev->chip, (int) step_count));
	/* Startup */
	RIE(usb_low_start_rowing(dev->chip));
	/* Wait for stop */
					   ...
					   SANE_Byte * ideal_blue_pd)
{
	...
	green_light_up =
		dev->expose_time - dev->green_rgb_600_power_delay * 64;
	...
	*ideal_green_pd =
		(SANE_Byte) ((ideal_expose_time - green_light_up) / 64);
	*ideal_blue_pd =
		(SANE_Byte) ((ideal_expose_time - blue_light_up) / 64);
					    ...
					    SANE_Byte * ideal_blue_pd)
{
	...
	max_light_up =
		dev->expose_time - dev->green_mono_600_power_delay * 64;
	transfer_time = (int) ((int) (dev->pixel_rate)
				     * (int) (dev->x_dpi) / 600);
	...
	*ideal_green_pd =
		(SANE_Byte) ((ideal_expose_time - max_light_up) / 64);
					   ...
					   SANE_Byte * ideal_blue_pd)
{
	...
	green_light_up =
		dev->expose_time - dev->green_rgb_300_power_delay * 64;
	...
	if (dev->chip->sensor == ST_CANON300600) {
		...
		ideal_expose_time = MAX(MAX(2624, max_light_up),	/* fixme? */
					usb_mid_motor_rgb_capability(dev->
								     chip,
								     dev->
								     y_dpi));
	} else {
					    ...
					    SANE_Byte * ideal_blue_pd)
{
	...
	max_light_up =
		dev->expose_time - dev->green_mono_300_power_delay * 64;
	transfer_time =
		(int) ((int) (dev->pixel_rate) *
			     (int) (dev->x_dpi) / 600);
	/* base on 600, but double it. */
				  ...
				  SANE_Bool is_order_invert)
{
	...
	RIE(usb_high_cal_calibrate(dev->green_calibrator,
				   dev->green + dev->skips_per_row,
				   line + 1));
	RIE(usb_high_cal_calibrate
	    (dev->blue_calibrator, dev->blue + dev->skips_per_row,
	     line + ((is_order_invert) ? 0 : 2)));
	RIE(usb_high_cal_calibrate
	    (dev->red_calibrator, dev->red + dev->skips_per_row,
...
usb_high_scan_backtrack_mono_8(Mustek_Usb_Device * dev)
{
	...
	if (dev->y_dpi >= 300) {
		RIE(usb_low_stop_rowing(dev->chip));
		RIE(usb_low_set_motor_direction(dev->chip, TRUE));
		RIE(usb_low_start_rowing(dev->chip));
		for (i = 0; i < dev->init_mono_8_back_track; i++) {
			RIE(usb_low_get_row
			    (dev->chip, dev->green, &lines_left));
		}
		usleep(100 * 1000);
		RIE(usb_low_stop_rowing(dev->chip));
		RIE(usb_low_set_motor_direction(dev->chip, FALSE));
