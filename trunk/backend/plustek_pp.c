...
static const char * mode_9800x_list[] = {
	SANE_VALUE_SCAN_MODE_LINEART,
	SANE_I18N("Halftone"),
	SANE_VALUE_SCAN_MODE_GRAY,
	SANE_VALUE_SCAN_MODE_COLOR,
...
static const char * ext_mode_list[] = {
	SANE_I18N("Normal"),
 ...
 */
static pModeParam
getModeList(Plustek_Scanner * scanner)
{
	pModeParam mp;
 ...
 */
static SANE_Status
limitResolution(Plustek_Device * dev)
{
 ...
 */
static SANE_Status
attach(const char *dev_name, pCnfDef cnf, Plustek_Device ** devp)
{
	int cntr;
	int result;
	int handle;
 ...
 */
static void
init_config_struct(pCnfDef cnf, SANE_Bool direct_io)
{
...
#ifdef _BACKEND_ENABLED
	init_config_struct(&config, TRUE);
#else
	init_config_struct(&config, FALSE);
#endif
 ...
 */
	...
	case 1:
		sinfo.ImgDef.dwFlag |= SCANDEF_Transparency;
		break;
	case 2:
		sinfo.ImgDef.dwFlag |= SCANDEF_Negative;
		break;
	default:
		break;
	}
	...
	if (scanmode <= COLOR_256GRAY || scanmode == COLOR_GRAY16) {
		s->hw->setMap(s->hw, s->gamma_table[0], s->gamma_length,
			      _MAP_MASTER);
	} else {
		s->hw->setMap(s->hw, s->gamma_table[1], s->gamma_length,
			      _MAP_RED);
		s->hw->setMap(s->hw, s->gamma_table[2], s->gamma_length,
			      _MAP_GREEN);
		s->hw->setMap(s->hw, s->gamma_table[3], s->gamma_length,
			      _MAP_BLUE);
	}
	/* work-around for USB... */
