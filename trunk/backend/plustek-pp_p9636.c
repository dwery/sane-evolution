 ...
 */
static void
p9636InitializeAsicRegister(pScanData ps)
{
 ...
 */
static void
p9636Init98001(pScanData ps, Bool shading)
{
	Byte bData;
	UShort w;
	ULong dwID;
	...
	case _CCD_3797:
		...
		if (ps->DataInf.dwScanFlag & SCANDEF_TPA) {
			if (ps->DataInf.dwScanFlag & SCANDEF_Transparency) {
				...
				ps->bsPreRedDAC = ps->bsPreGreenDAC =
	...
	case _CCD_3717:
		...
		ps->Shade.pCcdDac->DarkDAC.Colors.Green =
			((ps->
			  bSetScanModeFlag & _ScanMode_Mono) ? 0xd0 : 0xd6);
 ...
 */
	...
	case _CCD_518:
		...
		if (ps->DataInf.dwScanFlag & SCANDEF_TPA) {
			if (ps->DataInf.dwScanFlag & SCANDEF_Transparency) {
				...
				ps->wsDACOffsetRed = ps->wsDACOffsetGreen = ps->wsDACOffsetBlue = 0;	/* 0x70 */
			} else {
	...
	for (w = 0; w < ps->Device.wNumDACRegs; w++) {
		...
		IODataRegisterToDAC(ps,
				    WolfsonDAC8144[w].bReg,
				    WolfsonDAC8144[w].bParam);
	}
 ...
 */
static void
p9636SetupScannerVariables(pScanData ps)
{
 ...
 */
static void
p9636SetGeneralRegister(pScanData ps)
{
	...
	ps->AsicReg.RD_Motor0Control =
		ps->bHpMotor | (_MotorOn + _MotorDirForward);
 ...
 */
static void
p9636SetStartStopRegister(pScanData ps)
{
	...
	if (ps->DataInf.wPhyDataType < COLOR_256GRAY) {
		ps->AsicReg.RD_Pixels =
			(UShort) ps->DataInf.dwAsicBytesPerLine;
	} else {
		ps->AsicReg.RD_Pixels =
			(UShort) ps->DataInf.dwAsicPixelsPerPlane;
	}
 ...
 */
static void
p9636SetupScanningCondition(pScanData ps)
{
	ULong dw;
		...
		if (ps->DataInf.xyPhyDpi.y <= 150) {
			...
			if (ps->DataInf.xyPhyDpi.y <= 300) {
				...
				if (ps->DataInf.xyPhyDpi.y <= 600) {
					dw = ps->DataInf.
						dwAsicPixelsPerPlane * 4;
				} else {
					dw = ps->DataInf.
						dwAsicPixelsPerPlane * 8;
				}
			}
		}
 ...
 */
static void
p9636PutToIdleMode(pScanData ps)
{
	int i;
