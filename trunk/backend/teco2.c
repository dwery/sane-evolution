...
/*
   $Id: teco2.c,v 1.19 2008-01-12 14:25:50 gekl-guest Exp $
...
static const char * filter_color_list[] = {
	"Red",
	"Green",
	"Blue",
	NULL
...
teco_identify_scanner(Teco_Scanner * dev)
{
...
#ifdef sim
	{
		...
		/* vm3564 */
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x43, 0x00, 0x00, 0x10, 0x52,
			0x45,
			0x4c, 0x49, 0x53, 0x59, 0x53, 0x20, 0x41, 0x56, 0x45,
			0x43,
			0x20, 0x49, 0x49, 0x20, 0x53, 0x33, 0x20, 0x20, 0x20,
			0x20,
			0x20, 0x20, 0x31, 0x2e, 0x30, 0x37, 0x31, 0x2e, 0x30,
			0x37,
			0x00, 0x01, 0x54, 0x45, 0x43, 0x4f, 0x20, 0x56, 0x4d,
			0x33,
			0x35, 0x36, 0x34, 0x20, 0x00, 0x01, 0x01, 0x2c, 0x00,
			0x01,
			0x02, 0x58, 0x09, 0xf6, 0x0d, 0xaf, 0x01, 0x2c, 0x00,
			0x08,
			0x01, 0x00
		};
#endif
		...
		/* vm356A */
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x43, 0x00, 0x00, 0x10, 0x50,
			0x72,
			0x69, 0x6d, 0x61, 0x78, 0x20, 0x20, 0x4a, 0x65, 0x77,
			0x65,
			0x6c, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
			0x20,
			0x20, 0x20, 0x31, 0x2e, 0x30, 0x30, 0x31, 0x2e, 0x30,
			0x30,
			0x00, 0x01, 0x54, 0x45, 0x43, 0x4f, 0x20, 0x56, 0x4d,
			0x33,
			0x35, 0x36, 0x41, 0x20, 0x00, 0x01, 0x01, 0x2c, 0x00,
			0x01,
			0x02, 0x58, 0x09, 0xf6, 0x0d, 0xaf, 0x01, 0x2c, 0x00,
			0x08,
			0x10, 0x00
		...
		/* vm3575 */
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x43, 0x00, 0x00, 0x00, 0x20,
			0x20,
			0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x46, 0x6c, 0x61,
			0x74,
			0x62, 0x65, 0x64, 0x20, 0x53, 0x63, 0x61, 0x6e, 0x6e,
			0x65,
			0x72, 0x20, 0x31, 0x2e, 0x30, 0x33, 0x31, 0x2e, 0x30,
			0x33,
			0x00, 0x01, 0x54, 0x45, 0x43, 0x4f, 0x20, 0x56, 0x4d,
			0x33,
			0x35, 0x37, 0x35, 0x20, 0x00, 0x01, 0x01, 0x2c, 0x00,
			0x01,
			0x02, 0x58, 0x09, 0xf6, 0x0d, 0xaf, 0x01, 0x2c, 0x00,
			0x08,
			0x01, 0x00
		...
		unsigned char table[] = {
			...
			0x62, 0x65, 0x64, 0x20, 0x53, 0x63, 0x61, 0x6e, 0x6e,
			0x65,
			0x72, 0x20, 0x33, 0x2e, 0x30, 0x31, 0x33, 0x2e, 0x30,
			0x31,
			0x00, 0x01, 0x54, 0x45, 0x43, 0x4f, 0x20, 0x56, 0x4d,
			0x36,
			0x35, 0x38, 0x36, 0x20, 0x00, 0x01, 0x01, 0x2c, 0x00,
			0x01,
			0x02, 0x58, 0x09, 0xf6, 0x0d, 0xaf, 0x01, 0x2c, 0x00,
			0x08,
			0x01, 0x00
		...
		/* vm656A */
		unsigned char table[] = {
			0x06, 0x00, 0x02, 0x02, 0x43, 0x00, 0x00, 0x00, 0x52,
			0x45,
			0x4c, 0x49, 0x53, 0x59, 0x53, 0x20, 0x41, 0x50, 0x4f,
			0x4c,
			0x4c, 0x4f, 0x20, 0x45, 0x78, 0x70, 0x72, 0x65, 0x73,
			0x73,
			0x20, 0x36, 0x31, 0x2e, 0x30, 0x33, 0x31, 0x2e, 0x30,
			0x33,
			0x00, 0x01, 0x54, 0x45, 0x43, 0x4f, 0x20, 0x56, 0x4d,
			0x36,
			0x35, 0x36, 0x41, 0x00, 0x01, 0x01, 0x2c, 0x00, 0x01,
			0x02,
			0x58, 0x09, 0xf6, 0x0d, 0xaf, 0x01, 0x2c, 0x00, 0x08,
			0x01,
			0x00, 0x00
		};
#endif
	...
	for (i = 0; i < NELEMS(scanners); i++) {
		if (dev->scsi_type == scanners[i].scsi_type &&
		    strcmp(dev->scsi_teco_name,
			   scanners[i].scsi_teco_name) == 0) {

			DBG(DBG_error,
			    "teco_identify_scanner: scanner supported\n");
...
teco_set_window(Teco_Scanner * dev)
{
	...
	/* Lamp to use */
	i = get_string_list_index(filter_color_list,
				  dev->val[OPT_FILTER_COLOR].s);
...
teco_reset_window(Teco_Scanner * dev)
{
	...
	hexdump(DBG_info2, "CDB:", cdb.data, cdb.len);
	status = sanei_scsi_cmd2(dev->sfd, cdb.data, cdb.len, NULL, 0, NULL,
				 NULL);
		...
		case TECO_VM3575:
			...
			if (dev->scan_mode == TECO_GRAYSCALE) {
				...
				dev->params.bytes_per_line =
					dev->params.pixels_per_line * 3;
				dev->bytes_per_raster =
...
teco_wait_for_data(Teco_Scanner * dev)
{
	...
	for (i = 0; i < 60; i++) {
		...
		hexdump(DBG_info2, "teco_wait_for_data return", dev->buffer,
			size);
	...
	DBG(DBG_proc,
	    "teco_wait_for_data: scanner not ready to send data (%d)\n",
	    status);
...
teco_do_calibration(Teco_Scanner * dev)
{
	...
	if ((tmp_buf == NULL) || (tmp_min_buf == NULL)
	    || (tmp_max_buf == NULL)) {
		DBG(DBG_proc,
		    "teco_do_calibration: not enough memory (%ld bytes)\n",
		    (long) tmp_buf_size);
...
/*		default:
	...
	for (j = 0; j < dev->def->cal_length; j++) {
		...
		case TECO_VM6586:
			dev->buffer[6 * j + 0] =
				(tmp_buf[0 * dev->def->cal_length + j] >> 0) &
				0xff;
			dev->buffer[6 * j + 1] =
				(tmp_buf[0 * dev->def->cal_length + j] >> 8) &
				0xff;

			dev->buffer[6 * j + 2] =
				(tmp_buf[1 * dev->def->cal_length + j] >> 0) &
				0xff;
			dev->buffer[6 * j + 3] =
				(tmp_buf[1 * dev->def->cal_length + j] >> 8) &
				0xff;

			dev->buffer[6 * j + 4] =
				(tmp_buf[2 * dev->def->cal_length + j] >> 0) &
				0xff;
			dev->buffer[6 * j + 5] =
				(tmp_buf[2 * dev->def->cal_length + j] >> 8) &
				0xff;
			break;
		case TECO_VM3564:
		case TECO_VM356A:
			dev->buffer[3 * j + 0] =
				(tmp_buf[3 * j + 0] >> 0) & 0xff;

			dev->buffer[3 * j + 1] =
				(tmp_buf[3 * j + 1] >> 0) & 0xff;

			dev->buffer[3 * j + 2] =
				(tmp_buf[3 * j + 2] >> 0) & 0xff;
			break;
		}
	}
...
static SANE_Status
teco_sense_handler(int scsi_fd, unsigned char *result,
		   void __sane_unused__ * arg)
{
	...
	if (len < 14) {
		DBG(DBG_error,
		    "teco_sense_handler: sense too short, no ASC/ASCQ\n");
...
attach_scanner(const char *devicename, Teco_Scanner ** devp)
{
	...
	if (teco_identify_scanner(dev) == FALSE) {
		DBG(DBG_error,
	...
	if (dev->def->color_adjust[0].resolution != 0) {
		...
		dev->resolutions_list =
			malloc(sizeof(int) * (num_entries + 1));
		...
		for (i = 0; i < num_entries; i++) {
			dev->resolutions_list[i + 1] =
				dev->def->color_adjust[i].resolution;
		}
	} else {
...
teco_init_options(Teco_Scanner * dev)
{
	...
	dev->val[OPT_MODE].s = (char *) strdup("");	/* will be set later */
	...
	dev->opt[OPT_ENHANCEMENT_GROUP].constraint_type =
	...
	dev->val[OPT_CUSTOM_GAMMA].w = FALSE;
	...
	dev->opt[OPT_GAMMA_VECTOR_R].size = GAMMA_LENGTH * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_G].size = GAMMA_LENGTH * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_B].size = GAMMA_LENGTH * sizeof(int);
	...
	dev->opt[OPT_GAMMA_VECTOR_GRAY].size =
		GAMMA_LENGTH * sizeof(int);
	dev->opt[OPT_GAMMA_VECTOR_GRAY].constraint_type =
	...
	dev->opt[OPT_FILTER_COLOR].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_FILTER_COLOR].constraint.string_list = filter_color_list;
	dev->val[OPT_FILTER_COLOR].s =
		(char *) strdup(filter_color_list[0]);
	...
	dev->opt[OPT_PREVIEW].cap =
		SANE_CAP_SOFT_DETECT | SANE_CAP_SOFT_SELECT;
	dev->val[OPT_PREVIEW].w = FALSE;
...
teco_wait_scanner(Teco_Scanner * dev)
{
	...
	while (timeout > 0) {
		...
		/* test unit ready */
		hexdump(DBG_info2, "CDB:", cdb.data, cdb.len);
		status = sanei_scsi_cmd(dev->sfd, cdb.data, cdb.len, NULL,
					NULL);
...
teco_adjust_raster(Teco_Scanner * dev, size_t size_in)
{
	...
	for (raster = 0; raster < nb_rasters; raster++) {
		...
		if (dev->raster_num < color_adjust->color_shift) {
			...
			if ((dev->raster_real - dev->raster_num -
				...
				if (color_adjust->factor_x) {
					...
					color = COLOR_0;
					line = dev->line +
						color_adjust->color_shift - 1;
				}
			} else {
				if (color_adjust->factor_x) {
					color = COLOR_0;
					line = dev->line +
						color_adjust->color_shift;
				} else {
					color = COLOR_2;
					line = dev->line;
				}
		...
		assert(offset <=
		       (dev->image_size - dev->params.bytes_per_line));

		/* Copy the raster to the temporary image. */
		{
			int i;
			unsigned char *src =
				dev->buffer + raster * dev->raster_size;
		...
		DBG(DBG_info, "raster=%d, line=%d, color=%d\n",
		    dev->raster_num, dev->line + line, color);
...
teco_fill_image(Teco_Scanner * dev)
{
	...
	while (dev->real_bytes_left) {
		...
		if (size >
		    dev->image_size - dev->raster_ahead - dev->image_end) {
			size = dev->image_size - dev->raster_ahead -
				dev->image_end;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "teco_fill_image: cannot read from the scanner\n");
			return status;
		}
		...
		if (dev->scan_mode == TECO_COLOR &&
			...
			memcpy(dev->image + dev->image_end, dev->buffer,
			       size);
			dev->image_end += size;
		}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_MODE:
			...
			dev->val[OPT_MODE].s = (char *) strdup(val);
			...
			dev->opt[OPT_RESOLUTION].constraint_type =
				SANE_CONSTRAINT_RANGE;
			dev->opt[OPT_RESOLUTION].constraint.range =
				&dev->def->res_range;
			...
			if (strcmp
				...
				dev->depth = 8;
				dev->opt[OPT_DITHER].cap &=
					~SANE_CAP_INACTIVE;
				dev->opt[OPT_FILTER_COLOR].cap &=
					~SANE_CAP_INACTIVE;
				dev->opt[OPT_THRESHOLD].cap &=
					~SANE_CAP_INACTIVE;
			} else if (strcmp
				   (dev->val[OPT_MODE].s,
				    SANE_VALUE_SCAN_MODE_GRAY) == 0) {
				...
				case TECO_VM3564:
				case TECO_VM356A:
					dev->opt[OPT_WHITE_LEVEL_R].cap &=
						~SANE_CAP_INACTIVE;
					dev->opt[OPT_WHITE_LEVEL_G].cap &=
						~SANE_CAP_INACTIVE;
					dev->opt[OPT_WHITE_LEVEL_B].cap &=
						~SANE_CAP_INACTIVE;
					break;
				case TECO_VM3575:
				case TECO_VM6575:
				...
				case TECO_VM6575:
					...
					dev->opt[OPT_CUSTOM_GAMMA].cap &=
						~SANE_CAP_INACTIVE;
					if (dev->val[OPT_CUSTOM_GAMMA].w) {
						dev->opt[OPT_GAMMA_VECTOR_R].
							cap &=
							~SANE_CAP_INACTIVE;
						dev->opt[OPT_GAMMA_VECTOR_G].
							cap &=
							~SANE_CAP_INACTIVE;
						dev->opt[OPT_GAMMA_VECTOR_B].
							cap &=
							~SANE_CAP_INACTIVE;
					}
					break;
				...
				if (dev->resolutions_list != NULL) {
					...
					dev->opt[OPT_RESOLUTION].constraint.
						word_list =
						dev->resolutions_list;

					/* If the resolution isn't in the list, set a default. */
					for (i = 1;
					     i <= dev->resolutions_list[0];
					     i++) {
						if (dev->
						    resolutions_list[i] >=
						    dev->val[OPT_RESOLUTION].
						    w)
							break;
					}
		...
		case OPT_GAMMA_VECTOR_B:
		case OPT_GAMMA_VECTOR_GRAY:
			memcpy(dev->val[option].wa, val,
			       dev->opt[option].size);
			return SANE_STATUS_GOOD;

		case OPT_CUSTOM_GAMMA:
			dev->val[OPT_CUSTOM_GAMMA].w = *(int *) val;
			if (dev->val[OPT_CUSTOM_GAMMA].w) {
				...
				if (dev->scan_mode == TECO_GRAYSCALE) {
					...
					/* color mode */
					dev->opt[OPT_GAMMA_VECTOR_R].cap &=
						~SANE_CAP_INACTIVE;
					dev->opt[OPT_GAMMA_VECTOR_G].cap &=
						~SANE_CAP_INACTIVE;
					dev->opt[OPT_GAMMA_VECTOR_B].cap &=
						~SANE_CAP_INACTIVE;
				}
			} else {
				dev->opt[OPT_GAMMA_VECTOR_R].cap |=
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!(dev->scanning)) {
		...
		if (dev->val[OPT_PREVIEW].w == TRUE) {
			...
			dev->y_tl = 0;
			dev->x_br =
				mmToIlu(SANE_UNFIX(dev->def->x_range.max));
			dev->y_br =
				mmToIlu(SANE_UNFIX(dev->def->y_range.max));
		} else {
		...
		case TECO_COLOR:
			...
			if (dev->resolutions_list != NULL) {
				...
				dev->color_adjust =
					&dev->def->color_adjust[i];
			} else {
				dev->color_adjust =
					&dev->def->color_adjust[0];
			}
			break;
		}

		dev->params.lines =
			(dev->length * dev->y_resolution) /
			dev->def->x_resolution_max;
	}
...
sane_start(SANE_Handle handle)
{
	...
	if (!(dev->scanning)) {
		...
		if (sanei_scsi_open
		    (dev->devicename, &(dev->sfd), teco_sense_handler,
		     dev) != 0) {
		...
		if (dev->color_adjust) {
			dev->raster_ahead =
				(2 * dev->color_adjust->color_shift) *
				dev->params.bytes_per_line;
		} else {
			dev->raster_ahead = 0;
		}
