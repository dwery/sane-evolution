...
static ST400_Model st400_models[] = {
	{8, "SIEMENS", 16, "ST 400", 6, 0x200000UL, 65536UL, NULL, "Siemens",
	 "ST400", "flatbed scanner"},
...
static int
str_at_offset(char *str, size_t offset, unsigned char *data)
{
	size_t len;
...
st400_inquiry(int fd, ST400_Model ** modelP)
{
	struct
	{
		SANE_Byte cmd, lun, reserved[2], tr_len, ctrl;
	} scsi_cmd;
/*
	struct {
...
*/
	...
	status = sanei_scsi_cmd(fd, &scsi_cmd, sizeof(scsi_cmd), &inqdata,
				&inqlen);
	...
	for (model = st400_models; model->inq_vendor; model++) {
		if (str_at_offset
		    (model->inq_vendor, model->inq_voffset, inqdata.bytes)
		    && str_at_offset(model->inq_model, model->inq_moffset,
				     inqdata.bytes)) {
			*modelP = model;
			DBG(DINFO,
			    "found matching scanner model \"%s %s\" in list\n",
...
st400_set_window(ST400_Device * dev)
{
	...
	yoff = 6;
	th = (double) maxval(dev->model->bits) *
		SANE_UNFIX(dev->val[OPT_THRESHOLD]) / 100.0;
	...
	DBG(DSCSI,
	    "SCSI: sending SET_WINDOW (x=%hu y=%hu w=%hu h=%hu wy=%hu wh=%hu th=%d\n",
...
st400_read10(int fd, SANE_Byte * buf, size_t * lenP)
{
	struct
	{
		SANE_Byte cmd, lun, res[4], tr_len[3], ctrl;
	} scsi_cmd;
	SANE_Status status;
...
st400_config_do_option(char *opt, size_t linenum)
{
	...
	if (strncmp(opt, "maxread", 7) == 0 && isspace(opt[7])) {
	...
	} else if (strncmp(opt, "delay", 5) == 0 && isspace(opt[5])) {
		opt += 6;
		status = st400_config_get_single_arg(opt, &arg, linenum);
		if (status == SANE_STATUS_GOOD)
			st400_light_delay = (size_t) arg;
	} else if (strncmp(opt, "scanner_bufsize", 15) == 0
		   && isspace(opt[15])) {
		opt += 16;
		status = st400_config_get_single_arg(opt, &arg, linenum);
		if (status == SANE_STATUS_GOOD)
	...
	} else if (strncmp(opt, "scanner_bits", 12) == 0 && isspace(opt[12])) {
		opt += 13;
		status = st400_config_get_single_arg(opt, &arg, linenum);
		if (status == SANE_STATUS_GOOD)
			if (st400_devices)
				st400_devices->model->bits = arg;	/* FIXME */
	} else if (strncmp(opt, "scanner_maxread", 15) == 0
		   && isspace(opt[15])) {
		opt += 16;
		status = st400_config_get_single_arg(opt, &arg, linenum);
		if (status == SANE_STATUS_GOOD)
	...
	} else if (strncmp(opt, "scanner_resolutions", 19) == 0
		   && isspace(opt[19])) {
		opt += 20;
		st400_devices->model->dpi_list =
		...
		do {
			...
			if (status == SANE_STATUS_GOOD) {
				++i;
				st400_devices->model->dpi_list[i] =
					(int) arg;
			}
		...
		status = SANE_STATUS_GOOD;
	} else if (strncmp(opt, "dump_inquiry", 12) == 0) {
		st400_dump_data = 1;
	}
	if (st400_devices)
...
SANE_Status
sane_init(int * versionP, SANE_Auth_Callback authorize)
{
	FILE *fp;
	SANE_Status status;
		...
		while (sanei_config_read(line, sizeof(line), fp)) {
			...
			if (strncmp(str, "option", 6) == 0 && isspace(str[6])) {
				DBG(DCODE, "sane_init: config line <%s>\n",
				    line);
				status = st400_config_do_option(str + 7,
								linenum);
			} else {
				DBG(DCODE,
				    "sane_init: attaching device <%s>\n",
				    line);
	...
	if (status == SANE_STATUS_GOOD && st400_devices == NULL) {
		...
		sanei_config_attach_matching_devices(ST400_DEFAULT_DEVICE,
						     st400_attach_one);
	}
...
sane_get_devices(const SANE_Device *** devarrayP, SANE_Bool local_only)
{
	...
	if (!st400_status.array_valid) {
		if (st400_device_array) {
			DBG(DCODE,
			    "sane_get_devices: freeing old device array\n");
			free(st400_device_array);
		}
		st400_device_array =
			malloc((st400_num_devices +
				1) * sizeof(*st400_device_array));
		if (!st400_device_array)
			return SANE_STATUS_NO_MEM;
...
st400_reset_options(ST400_Device * dev)
{
	...
	dev->val[OPT_RESOLUTION] =
		dev->opt[OPT_RESOLUTION].constraint.word_list[1];
...
st400_init_options(ST400_Device * dev)
{
	...
	dev->opt[OPT_NUM_OPTS].size = sizeof(int);
	...
	dev->opt[OPT_RESOLUTION].size = sizeof(int);
	dev->opt[OPT_RESOLUTION].cap =
	...
	dev->opt[OPT_DEPTH].size = sizeof(int);
	...
	dev->opt[OPT_TL_X].size = sizeof(int);
	...
	dev->opt[OPT_TL_Y].size = sizeof(int);
	...
	dev->opt[OPT_BR_X].size = sizeof(int);
	...
	dev->opt[OPT_BR_Y].size = sizeof(int);
...
sane_get_option_descriptor(SANE_Handle handle, int optnum)
{
	ST400_Device *dev = handle;
...
SANE_Status
sane_control_option(SANE_Handle handle, int optnum,
		    SANE_Action action, void *valP, int * infoP)
{
	...
	DBG(DCODE, "sane_control_option(%p, %d, %d, %p, %p)\n",
	    (void *) handle, (int) optnum, (int) action, valP,
	    (void *) infoP);
	...
	case SANE_ACTION_SET_VALUE:
		if (!SANE_OPTION_IS_SETTABLE(dev->opt[optnum].cap))
			return SANE_STATUS_INVAL;
		status = sanei_constrain_value(&dev->opt[optnum], valP,
					       infoP);
		if (status != SANE_STATUS_GOOD)
			return status;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * paramsP)
{
	...
	if (!dev->status.scanning) {
		...
		if (dpi > 0.0 && width > 0.0 && height > 0.0) {
			...
			dev->x = SANE_UNFIX(dev->val[OPT_TL_X]) *
				dots_per_mm + 0.5;
			dev->y = SANE_UNFIX(dev->val[OPT_TL_Y]) *
				dots_per_mm + 0.5;
			dev->w = dev->params.pixels_per_line;
			dev->h = dev->params.lines;

			DBG(DVAR,
			    "parameters: bpl=%d, x=%hu, y=%hu, w=%hu, h=%hu\n",
			    (int) dev->params.bytes_per_line, dev->x, dev->y,
			    dev->w, dev->h);
		}
	}
...
sane_start(SANE_Handle handle)
{
	...
	if (!dev->buffer) {
		if (st400_maxread > 0)
			dev->bufsize = min(st400_maxread, (unsigned int)
					   sanei_scsi_max_request_size);
		else if (dev->model->maxread > 0)
			dev->bufsize = min(dev->model->maxread, (unsigned int)
					   sanei_scsi_max_request_size);
		else
	...
	if (dev->fd < 0) {
		status = sanei_scsi_open(dev->sane.name, &dev->fd,
					 st400_sense_handler, dev);
		if (status != SANE_STATUS_GOOD)
			goto return_error;
	}
...
SANE_Status
sane_get_select_fd(SANE_Handle handle, int * fdP)
{
