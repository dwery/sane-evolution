...
struct _SANE_Option
{
	SANE_Option_Descriptor *descriptor;
	  SANE_Status(*callback) (struct _SANE_Option * option,
				  SANE_Handle handle, SANE_Action action,
				  void *value, int * info);
};
...
static int getNumberOfOptions(void);	/* Forward declaration */
...
static int optionCalibrateValue = FALSE;
			...
			SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_GET_VALUE:
		*(int *) value = optionCalibrateValue;
		break;
	}
	return SANE_STATUS_GOOD;
}
...
static const int optionResolutionList[] = {
			 ...
			 SANE_Action action, void *value, int * info)
{
	...
	switch (action) {
	case SANE_ACTION_SET_AUTO:
		status = sanei_constrain_value(option->descriptor,
					       (void *) &autoValue, info);
	...
	case SANE_ACTION_SET_VALUE:
		*info |= SANE_INFO_RELOAD_PARAMS;
		optionResolutionValue = *(int *) value;
		break;
	case SANE_ACTION_GET_VALUE:
		*(int *) value = optionResolutionValue;
		break;
	}
	return SANE_STATUS_GOOD;
}
...
*/
static int optionGrayscaleValue = FALSE;
			...
			SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		*info |= SANE_INFO_RELOAD_PARAMS;
		...
		break;
	case SANE_ACTION_GET_VALUE:
		*(int *) value = optionGrayscaleValue;
		break;
	}
	return SANE_STATUS_GOOD;
}
...
*/
static SANE_Status
dispatch_control_option(SANE_Handle handle, int option,
			SANE_Action action, void *value, int * info)
{
	...
	if (action == SANE_ACTION_SET_VALUE) {
		status = sanei_constrain_value(op->descriptor, value,
					       &myinfo);
		if (status != SANE_STATUS_GOOD)
			return status;
	}
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	int w = SANE_UNFIX(optionBotRightXValue -
			   optionTopLeftXValue) / MM_IN_INCH *
		optionResolutionValue;
	int h = SANE_UNFIX(optionBotRightYValue -
			   optionTopLeftYValue) / MM_IN_INCH *
		optionResolutionValue;
...
sane_start(SANE_Handle handle)
{
	...
	res = CANON_set_scan_parameters(&scanner->scan, optionCalibrateValue,
#ifdef GRAY
