...
gt68xx_device_new(GT68xx_Device ** dev_return)
{
	...
	if (!dev) {
		DBG(3,
		    "gt68xx_device_new: couldn't malloc %lu bytes for device\n",
...
gt68xx_find_usb_device_entry(int vendor, int product)
{
...
gt68xx_device_identify(GT68xx_Device * dev)
{
	...
	if (entry) {
		...
		dev->model = NULL;
		DBG(3,
		    "gt68xx_device_identify: unknown USB device (vendor 0x%04x, "
		    "product 0x%04x)\n", vendor, product);
		return SANE_STATUS_INVAL;
	}
...
gt68xx_device_generic_req(GT68xx_Device * dev,
			  SANE_Byte request_type, int request,
			  ...
			  size_t res_size)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
		    "gt68xx_device_generic_req: writing command failed: %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
		    "gt68xx_device_generic_req: reading response failed: %s\n",
		    sane_strstatus(status));
		return status;
	}
				...
				SANE_Byte * data, int size)
{
	...
	if (dev->model->command_set->download_firmware)
		return (*dev->model->command_set->download_firmware) (dev,
								      data,
								      size);
	else
...
gt68xx_device_get_power_status(GT68xx_Device * dev, SANE_Bool * power_ok)
{
	...
	if (dev->model->command_set->get_power_status)
		return (*dev->model->command_set->get_power_status) (dev,
								     power_ok);
	else
		return SANE_STATUS_UNSUPPORTED;
}
...
gt68xx_device_get_ta_status(GT68xx_Device * dev, SANE_Bool * ta_attached)
{
	...
	if (dev->model->command_set->get_ta_status)
		return (*dev->model->command_set->get_ta_status) (dev,
								  ta_attached);
	else
		return SANE_STATUS_UNSUPPORTED;
}
			   ...
			   SANE_Bool ta_lamp)
{
	...
	if (dev->model->command_set->lamp_control)
		return (*dev->model->command_set->lamp_control) (dev, fb_lamp,
								 ta_lamp);
	else
		return SANE_STATUS_UNSUPPORTED;
}
...
gt68xx_device_move_relative(GT68xx_Device * dev, int distance)
{
	...
	if (dev->model->command_set->move_relative)
		return (*dev->model->command_set->move_relative) (dev,
								  distance);
	else
		return SANE_STATUS_UNSUPPORTED;
}
...
gt68xx_device_read_scanned_data(GT68xx_Device * dev, SANE_Bool * ready)
{
	...
	if (dev->model->command_set->read_scanned_data)
		return (*dev->model->command_set->read_scanned_data) (dev,
								      ready);
	else
		return SANE_STATUS_UNSUPPORTED;
}
			 ...
			 GT68xx_Scan_Parameters * params)
{
	...
	if (dev->model->command_set->setup_scan)
		return (*dev->model->command_set->setup_scan) (dev, request,
							       action,
							       params);
	else
		return SANE_STATUS_UNSUPPORTED;
				...
				GT68xx_Exposure_Parameters * params)
{
	...
	if (dev->model->command_set->set_exposure_time)
		return (*dev->model->command_set->set_exposure_time) (dev,
								      params);
	else
		return SANE_STATUS_UNSUPPORTED;
}
...
SANE_Status
gt68xx_device_read_raw(GT68xx_Device * dev, SANE_Byte * buffer, size_t * size)
{
	SANE_Status status;
...
gt68xx_device_set_read_buffer_size(GT68xx_Device * dev, size_t buffer_size)
{
	...
	if (dev->read_active) {
		DBG(3,
		    "gt68xx_device_set_read_buffer_size: BUG: read already "
		    "active\n");
		return SANE_STATUS_INVAL;
	}
...
gt68xx_reader_process(GT68xx_Device * dev)
{
	...
	while (read_bytes_left > 0) {
		...
		DBG(9,
		    "gt68xx_reader_process: buffer %d: trying to read %lu bytes "
		    "(%lu bytes left, line %d)\n", buffer_id,
		    (unsigned long) size, (unsigned long) read_bytes_left,
		    line);
...
gt68xx_device_read_start_fork(GT68xx_Device * dev)
{
	...
	status = shm_channel_new(dev->read_buffer_size, SHM_BUFFERS,
				 &dev->shm_channel);
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
...
gt68xx_device_read(GT68xx_Device * dev, SANE_Byte * buffer, size_t * size)
{
	...
	while (left_to_read > 0) {
		if (dev->read_bytes_in_buffer == 0) {
			...
			DBG(7,
			    "gt68xx_device_read: trying to read %ld bytes\n",
			    (long) raw_block_size);
#ifdef USE_FORK
			if (dev->shm_channel) {
				status = shm_channel_reader_get_buffer(dev->
								       shm_channel,
								       &buffer_id,
								       &buffer_addr,
								       &buffer_bytes);
				if (status == SANE_STATUS_GOOD
				    && buffer_addr != NULL) {
					DBG(9,
					    "gt68xx_device_read: buffer %d: get\n",
					    buffer_id);
					memcpy(dev->read_buffer, buffer_addr,
					       buffer_bytes);
					shm_channel_reader_put_buffer(dev->
								      shm_channel,
								      buffer_id);
					DBG(9,
					    "gt68xx_device_read: buffer %d: put\n",
					    buffer_id);
				}
			} else
#endif /* USE_FORK */
				status = gt68xx_device_read_raw(dev,
								dev->
								read_buffer,
								&raw_block_size);
			if (status != SANE_STATUS_GOOD) {
...
gt68xx_device_read_finish(GT68xx_Device * dev)
{
	...
	if (dev->reader_pid != 0) {
		...
		/* usleep (100000); */
		DBG(7,
		    "gt68xx_device_read_finish: trying to kill reader process\n");
...
gt68xx_device_check_result(GT68xx_Packet res, SANE_Byte command)
{
	...
	if (res[1] != command)
		DBG(5,
		    "gt68xx_device_check_result: warning: result was %2X %2X "
		    "(expected: %2X %2X)\n", res[0], res[1], 0, command);
	return SANE_STATUS_GOOD;
}
