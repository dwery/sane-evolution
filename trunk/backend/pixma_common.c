		      ...
		      void *data, unsigned expected_len)
{
	...
	if (error != (int) cmdlen) {
		if (error >= 0) {
			...
			PDBG(pixma_dbg
			     (1,
			      "ERROR:incomplete write, %u out of %u written\n",
	...
	do {
		...
		if (error == PIXMA_ETIMEDOUT)
			PDBG(pixma_dbg
			     (2, "No response yet. Timed out in %d sec.\n",
			      tmo));
	}
	while (error == PIXMA_ETIMEDOUT && --tmo != 0);
	if (error < 0) {
		PDBG(pixma_dbg
		     (1, "WARNING:Error in response phase. cmd:%02x%02x\n",
		      ((const uint8_t *) cmd)[0],
		      ((const uint8_t *) cmd)[1]));
		PDBG(pixma_dbg
		     (1,
...
pixma_close(pixma_t * s)
{
	...
	if (s->io) {
		if (s->scanning) {
			PDBG(pixma_dbg
			     (3,
			      "pixma_close():scanning in progress, call"
			      " finish_scan()\n"));
			s->ops->finish_scan(s);
		}
		s->ops->close(s);
...
pixma_read_image(pixma_t * s, void *buf, unsigned len)
{
	...
	if (s->underrun) {
		if (s->cur_image_size < s->param->image_size) {
			...
			PDBG(pixma_dbg
			     (3,
			      "pixma_read_image():completed (underrun detected)\n"));
			s->scanning = 0;
		}
	...
	while (ib.wptr != ib.wend) {
		...
		if (ib.rptr) {
			unsigned count =
				MIN(ib.rend - ib.rptr, ib.wend - ib.wptr);
...
pixma_check_scan_param(pixma_t * s, pixma_scan_param_t * sp)
{
	...
	case PIXMA_SOURCE_TPU:
		if ((s->cfg->cap & PIXMA_CAP_TPU) != PIXMA_CAP_TPU) {
			...
			PDBG(pixma_dbg
			     (1,
			      "WARNING: TPU unsupported, fallback to flatbed.\n"));
		}
		break;
	...
	case PIXMA_SOURCE_ADF:
		if ((s->cfg->cap & PIXMA_CAP_ADF) != PIXMA_CAP_ADF) {
			...
			PDBG(pixma_dbg
			     (1,
			      "WARNING: ADF unsupported, fallback to flatbed.\n"));
		}
		break;
	...
	case PIXMA_SOURCE_ADFDUP:
		if ((s->cfg->cap & PIXMA_CAP_ADFDUP) != PIXMA_CAP_ADFDUP) {
			...
			PDBG(pixma_dbg
			     (1,
			      "WARNING: ADF duplex unsupported, fallback to %d.\n",
			      sp->source));
		}
		break;
	}
...
pixma_fill_gamma_table(double gamma, uint8_t * table, unsigned n)
{
	...
	for (i = 0; (unsigned) i != n; i++) {
		table[i] =
			(int) (out_scale * pow(i * in_scale, r_gamma) + 0.5);
	}
}
