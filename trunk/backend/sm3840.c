	  ...
	  int * len)
{
	...
	if (!s->remaining) {
		if (!s->linesleft) {
			*len = 0;
			...
			/* Send lamp timeout */
			set_lamp_timer((usb_dev_handle *) s->udev,
				       s->sm3840_params.lamp);
	...
	if (s->sm3840_params.lineart || s->sm3840_params.halftone) {
		...
		*len = i / 8;
		...
		memcpy(buf, s->offset + s->line_buffer,
		       min(max_len, s->remaining));
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		s->sm3840_params.gray =
			(!strcasecmp(s->value[OPT_MODE].s, SANE_I18N("Gray")))
			? 1 : 0;
		s->sm3840_params.halftone =
			(!strcasecmp
			 (s->value[OPT_MODE].s,
			  SANE_I18N("Halftone"))) ? 1 : 0;
		s->sm3840_params.lineart =
			(!strcasecmp
			 (s->value[OPT_MODE].s,
			  SANE_I18N("Lineart"))) ? 1 : 0;
		...
		s->sm3840_params.top =
			sm3840_unit_convert(s->value[OPT_TL_Y].w);
		s->sm3840_params.left =
			sm3840_unit_convert(s->value[OPT_TL_X].w);
		s->sm3840_params.width =
			sm3840_unit_convert(s->value[OPT_BR_X].w) -
			s->sm3840_params.left;
		s->sm3840_params.height =
			sm3840_unit_convert(s->value[OPT_BR_Y].w) -
			s->sm3840_params.top;
		...
		s->sane_params.format =
			s->sm3840_params.
			gray ? SANE_FRAME_GRAY : SANE_FRAME_RGB;
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		if (status != SANE_STATUS_GOOD)
			return (status);
		status = sanei_constrain_value(s->options_list + option, val,
					       info);
		switch (option) {
...
initialize_options_list(SM3840_Scan * s)
{
	...
	s->options_list[OPT_BIT_DEPTH].constraint_type =
		SANE_CONSTRAINT_WORD_LIST;
	...
	s->options_list[OPT_LAMP_TIMEOUT].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	s->options_list[OPT_THRESHOLD].constraint_type =
		SANE_CONSTRAINT_RANGE;
