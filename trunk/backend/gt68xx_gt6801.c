			 ...
			 SANE_Byte * data, int size)
{
	...
	for (addr = 0; addr < size; addr += block_size) {
		...
		RIE(gt68xx_device_memory_read
		    (dev, 0x3f00, block_size, check_buf));
		...
		if ((check_buf[0] != 0) && (check_buf[1] != 0x40)) {
			DBG(3,
			    "gt6801_download_firmware: mismatch at block 0x%0x\n",
			    addr);
...
gt6801_get_power_status(GT68xx_Device * dev, SANE_Bool * power_ok)
{
	...
	/* I don't know what power_ok = FALSE looks like... */
...
SANE_Status
gt6801_lamp_control(GT68xx_Device * dev, SANE_Bool fb_lamp, SANE_Bool ta_lamp)
{
	if (!dev->model->is_cis) {
		GT68xx_Packet req;
...
gt6801_carriage_home(GT68xx_Device * dev)
{
	...
	if (dev->model->flags & GT68XX_FLAG_MOTOR_HOME) {
		...
		req[1] = 0x01;
		if ((status =
		     gt68xx_device_req(dev, req, req)) == SANE_STATUS_GOOD) {
