...
typedef struct
{
	...
	char vendor[9];	/* model name 8+1 */
	char product[17];	/* product name 16+1 */
	char revision[5];	/* revision 4+1 */
...
typedef struct HS2P_Scanner
{
	...
	int gamma_table[GAMMA_LENGTH];	/* Custom Gray Gamma Table */
...
_is_host_little_endian()
{
	...
	return (*firstbyte == 255) ? TRUE : FALSE;
}
#endif
