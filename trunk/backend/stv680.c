...
/*  $Id: stv680.c,v 1.1 2006-06-05 17:59:26 gekl-guest Exp $
...
stv680_reset_vidcam(Stv680_Vidcam * dev)
{
	...
	sizew = 0x00;		/* was 0 ? */
	status = sanei_usb_control_msg(dev->fd, 0x41, 0x0a, 0x0000, 0, sizew,
				       dev->windoww);
	if (status != SANE_STATUS_GOOD) {
		return status;
	}
	...
	sizew = 0x00;		/* was 0 */
	status = sanei_usb_control_msg(dev->fd, 0x41, 0x04, 0x0000, 0, sizew,
				       dev->windoww);
	if (status != SANE_STATUS_GOOD) {
		return status;
	}
	...
	status = sanei_usb_control_msg(dev->fd, 0xc1, 0x80, 0x0000, 0, sizer,
				       dev->windowr);
	if (status != SANE_STATUS_GOOD) {
		...
		DBG(DBG_proc,
		    "stv680_reset_vidcam: Camera reset to idle mode.\n");
	}
...
stv680_identify_vidcam(Stv680_Vidcam * dev)
{
	...
	for (i = 0; i < NELEMS(vidcams); i++) {
		if (vidcams[i].vendor == vendor
			...
			DBG(DBG_info,
			    "stv680_identify_vidcam: vidcam %x:%x is in list\n",
			    vendor, product);
			 ...
			 */
			sizer = 0x02;
			status = sanei_usb_control_msg(dev->fd, 0xc1, 0x88,
						       0x5678, 0, sizer,
						       dev->windowr);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
			...
			if ((dev->windowr[0] != 0x56)
			    || (dev->windowr[1] != 0x78)) {
				DBG(DBG_proc,
				    "STV(e): camera ping failed!!, checkvalue !=0x5678\n");
				sizer = 0x02;
				hexdump(DBG_info2, "urb12 window",
					dev->windowr, sizer);
				return FALSE;
			}
			sizer = 0x02;
			hexdump(DBG_info2, "urb12 ping data", dev->windowr,
				sizer);

			sizer = 0x10;
			status = sanei_usb_control_msg(dev->fd, 0xc1, 0x85,
						       0x0000, 0, sizer,
						       dev->windowr);
			if (status != SANE_STATUS_GOOD)
				return FALSE;

			hexdump(DBG_info2, "urbxx CMDID_GET_CAMERA_INFO",
				dev->windowr, sizer);
			...
			if (dev->SupportedModes == 0) {
				...
				if (dev->VGA)
					DBG(DBG_proc,
					    "STV(i): VGA is supported\n");
				if (dev->CIF)
					DBG(DBG_proc,
					    "STV(i): CIF is supported\n");
				if (dev->QVGA)
					DBG(DBG_proc,
					    "STV(i): QVGA is supported\n");
				if (dev->QCIF)
					DBG(DBG_proc,
					    "STV(i): QCIF is supported\n");
			}
			...
			DBG(DBG_proc, "STV(i): ASIC rev is %i.%i\n",
			    dev->windowr[2], dev->windowr[3]);
			DBG(DBG_proc, "STV(i): Sensor ID is %i.%i\n",
			    (dev->windowr[4]), (dev->windowr[5]));
			/* Hardware config  */
			...
			if (i & 1)
				DBG(DBG_proc,
				    "STV(i): Comms link is serial\n");
			else
				DBG(DBG_proc, "STV(i): Comms link is USB\n");
			if (i & 2)
				DBG(DBG_proc,
				    "STV(i): Flicker freq = 60 Hz\n");
			else
				DBG(DBG_proc,
				    "STV(i): Flicker freq = 50 Hz\n");
			if (i & 4)
			...
			sizer = 0x10;
			status = sanei_usb_control_msg(dev->fd, 0xc1, 0x86,
						       0x0000, 0, sizer,
						       dev->windowr);
			if (status != SANE_STATUS_GOOD) {
				return FALSE;
			}
			hexdump(DBG_info2, "urb25 CMDID_GET_IMAGE_INFO",
				dev->windowr, sizer);
	...
	DBG(DBG_error,
	    "stv680_identify_vidcam: exit this is not a STV680 exit\n");
	return FALSE;
}
...
stv680_vidcam_init(Stv680_Vidcam * dev)
{
	...
	while (dev->video_status == 0x04) {
		sizer = 0x08;
		status = sanei_usb_control_msg(dev->fd, 0xc1, 0x8d, 0x0000, 0,
					       sizer, dev->windowr);
		if (status != SANE_STATUS_GOOD)
			return status;

		hexdump(DBG_info2,
			"stv680_vidcam_init: urb13 CMDID_GET_USER_INFO",
			dev->windowr, sizer);

		dev->video_status = dev->windowr[1];
		if (dev->video_status == 0x02) {
			DBG(DBG_proc, "stv680_vidcam_init: status = video\n");
		} else if ((dev->video_status == 0x01)
			   || (dev->video_status == 0x08)) {
			...
			status = sanei_usb_control_msg(dev->fd, 0x41, 0x04,
						       0x0000, 0, 0, 0);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_info,
	...
	if (dev->video_status == 0x01 || dev->video_status == 0x08) {
		DBG(DBG_proc,
		    "stv680_vidcam_init: urb 21 CMDID_GET_COLDATA_SIZE\n");
		sizer = 0x02;
		status = sanei_usb_control_msg(dev->fd, 0xc1, 0x8a, 0x0000, 0,
					       sizer, dev->windowr);
		if (status != SANE_STATUS_GOOD)
			return status;
		...
		val = dev->windowr[0];
		hexdump(DBG_info2,
			"stv680_vidcam_init: urb21 CMDID_GET_COLDATA_SIZE",
			dev->windowr, sizer);
		...
		for (i = 0; i < val; i += 0x10) {
			DBG(DBG_proc,
			    ...
			    i, val);
			status = sanei_usb_control_msg(dev->fd, 0xc1, 0x8b,
						       (i << 8), 0, sizer,
						       dev->windowr);
			if (status != SANE_STATUS_GOOD)
				return status;
		...
		sizer = 0x12;
		status = sanei_usb_control_msg(dev->fd, 0x80, 0x06, 0x0100, 0,
					       sizer, dev->windowr);
		if (status != SANE_STATUS_GOOD)
			return status;
		...
		sizer = 0x12;
		hexdump(DBG_info2,
			"stv680_vidcam_init: CMDID_SET_IMAGE_INDEX",
			dev->windowr, sizer);
	...
	sizew = 0x0;
	status = sanei_usb_control_msg(dev->fd, 0x41, 0x09, dev->video_mode,
				       0, sizew, dev->windoww);
	if (status != SANE_STATUS_GOOD) {
...
attach_vidcam(const char * devicename, Stv680_Vidcam ** devp)
{
	...
	if (stv680_identify_vidcam(dev) == FALSE) {
		DBG(DBG_error,
		    "ERROR: attach_vidcam: vidcam-identification failed\n");
		stv680_free(dev);
		return SANE_STATUS_INVAL;
	}
	...
	if (dev->hw->color_adjust[0].resolution_x != 0) {
		...
		DBG(DBG_proc,
		    "attach_vidcam: make color resolution table \n");
...
stv680_fill_image(Stv680_Vidcam * dev)
{
	...
	while (dev->real_bytes_left) {
		...
		DBG(DBG_info,
		    ...
		    (unsigned long) (size_t) size,
		    (unsigned long) (size_t) dev->params.bytes_per_line);
...
#define AD(x, y, w) (((y)*(w)+(x))*3)
	...
	DBG(DBG_proc,
	    "stv680_bayer_unshuffle: w=%d, vw=%d, vh=%d, len=0x%lx\n", w, vw,
	    vh, (unsigned long) (size_t) size);
		...
		for (y = 0; y < vh; y++) {
			for (x = 0; x < vw; x++) {
				...
				*(dev->output + i) =
					red_g[*(dev->output + i)];
				*(dev->output + i + 1) =
					green_g[*(dev->output + i + 1)];
				*(dev->output + i + 2) =
					blue_g[*(dev->output + i + 2)];
			}
		}
	}
	...
	bright_red =
		(dev->val[OPT_BRIGHTNESS].w) +
		(dev->val[OPT_WHITE_LEVEL_R].w);
	bright_green =
		(dev->val[OPT_BRIGHTNESS].w) +
		(dev->val[OPT_WHITE_LEVEL_G].w);
	bright_blue =
		(dev->val[OPT_BRIGHTNESS].w) +
		(dev->val[OPT_WHITE_LEVEL_B].w);
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	while (sanei_config_read(line, sizeof(line), fp)) {
		...
		if (sscanf(line, "usb %i %i", &vendor, &product) == 2) {
			...
			/* Garbage. Ignore. */
			DBG(DBG_warning,
			    "bad configuration line: \"%s\" - ignoring.\n",
			    line);
		}
	}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_MODE:
			...
			} else if (strcmp(dev->val[OPT_MODE].s, COLOR_RGB_STR)
				   == 0) {
			...
			if (dev->resolutions_list != NULL) {
				...
				dev->opt[OPT_RESOLUTION].constraint.
					word_list = dev->resolutions_list;

				/* If the resolution isn't in the list, set a default. */
				for (i = 1; i <= dev->resolutions_list[0];
				     i++) {
				...
				if (i > dev->resolutions_list[0]) {
					/* Too big. Take lowest. */
					dev->val[OPT_RESOLUTION].w =
						dev->resolutions_list[1];
				} else {
					/* Take immediate superioir value. */
					dev->val[OPT_RESOLUTION].w =
						dev->resolutions_list[i];
				}
			}
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!(dev->scanning)) {
		...
		if (dev->resolutions_list != NULL) {
			...
			dev->y_resolution =
				dev->hw->color_adjust[i].resolution_y;
		}
		dev->subsample = 0;
	  ...
	  int * len)
{
	...
	*len = 0;
	...
	if (dev->image_begin == dev->image_end) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_info,
			    "sane_read: stv680_fill_image status NOK\n");
			return (status);
		}
	}
	...
	if ((dev->image_end - dev->image_begin) > size) {
		...
		DBG(DBG_proc,
		    "sane_read: size < dev->image_end - dev->image_begin\n");
	}
	...
	*len = dev->bytes_left;	/* needed */
	...
	if (dev->scan_mode != STV680_COLOR_RAW) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_info,
			    "sane_read: stv680_bayer_unshuffle status NOK\n");
			return (status);
		}
	} else {
