...
/* 
	$Id: ricoh-scsi.c,v 1.2 2003-12-27 17:48:38 hmg-guest Exp $
...
mode_sense(int fd, struct mode_pages *mp, SANE_Byte page_code)
{
	...
	status = sanei_scsi_cmd(fd, &cmd, sizeof(cmd), &select_data,
				&select_size);
...
get_data_status(int fd, struct scsi_status_desc *dbs)
{
	...
	if (status == SANE_STATUS_GOOD &&
	    (((unsigned int) _3btol(ssd.len)) <= sizeof(*dbs)
	     || _3btol(ssd.desc.filled) == 0)) {
...
ricoh_wait_ready_tur(int fd)
{
	...
	while (1) {
		...
		default:
			...
			DBG(1,
			    "scsi_wait_ready: test unit ready failed (%s)\n",
		...
		case SANE_STATUS_DEVICE_BUSY:
			...
			if (now.tv_sec - start.tv_sec >= MAX_WAITING_TIME) {
				DBG(1,
				    "ricoh_wait_ready: timed out after %lu seconds\n",
