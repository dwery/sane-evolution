...
usb_low_start_rowing(ma1017 * chip)
{
	...
	chip->total_lines =
		((SANE_Word) (chip->loop_count - 1)) * line_of_second +
		line_of_first;
	...
	while ((SANE_Word) bytes_total < byte_count) {
		...
		if ((SANE_Word) bytes_total != byte_count) {
			DBG(7, "usb_low_read_rows:  wanted %d, got %d "
			    ...
			    (SANE_Word) n, (SANE_Word) bytes_total);
		}
		n = MIN((byte_count - (SANE_Word) bytes_total),
			chip->max_block_size);
	}
