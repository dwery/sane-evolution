...
test_ready(int fd)
{
	...
	for (try = 0; try < 1000; ++try) {
		...
		status = sanei_scsi_cmd(fd, &scsi_test_ready,
					sizeof(scsi_test_ready), 0, 0);
...
start_scan(int fd, SANE_Bool cont)
{
	...
	scsi_start_scan.ctrl = (cont == TRUE) ? 0x80 : 0x00;

	DBG(1, "Starting scanner ...\n");
	return sanei_scsi_cmd(fd, &scsi_start_scan, sizeof(scsi_start_scan),
			      0, 0);
}
...
get_read_sizes(int fd, int *lines_available, int *bpl, int *total_lines)
{
	...
	status = sanei_scsi_cmd(fd, scsi_read, sizeof(scsi_read), &read_sizes,
				&size);
...
/* This function sets and sends the window for scanning */
{
	...
	int brightness =
		auto_bright ? 0 : (SANE_UNFIX(s->val[OPT_BRIGHTNESS].w)
				   * -1.27 + 128.5);
	...
	int bright_adjust =
		(SANE_UNFIX(s->val[OPT_BRIGHTNESS].w) * -20.0) / 100.0 + 20.0;

	/* ranges from 20 (little contrast) down to -20 = 235 (much contrast) */
	int contr_adjust =
		(SANE_UNFIX(s->val[OPT_CONTRAST].w) * -20.0) / 100.0;
	...
	set_size(cmd.wd.wlength, 2,
		 SANE_UNFIX(s->val[OPT_BR_Y].w - s->val[OPT_TL_Y].w)
		 * pixels_per_mm + 0.5);

	cmd.wd.bpp = s->bpp;

	if (s->mode == COLOR18BIT || s->mode == COLOR24BIT) {
		...
		DBG(3,
		    ...
		    s->exposure, cmd.wd.red_att, cmd.wd.blue_att,
		    cmd.wd.green_att, s->edge);
	} else {
		if (s->bpp == 1)
...
static SANE_Status
upload_dither_matrix(AgfaFocus_Scanner * s, int rows, int cols,
		     int *dither_matrix)
{
	struct
	{
...
#if 0
static SANE_Status
upload_tonecurve(AgfaFocus_Scanner * s, int color_type, int input, int output,
		 int dither_matrix[256])
{
	struct
	{
		Byte cmd;
	...
	for (i = 0; i < cols; ++i)
		for (j = 0; j < rows; ++j)
			set_size(cmd.wd.element[j + i * rows].data, 2,
				 dither_matrix[j + i * rows]);
...
#define ATTACH_SCSI_INQ_LEN 55
	...
	if (result[0] != 6 || strncmp((char *) result + 36, "AGFA0", 5)) {
		DBG(1,
		    "attach: device doesn't look like a Siemens 9036 scanner\n");
		return SANE_STATUS_INVAL;
	}
	...
	if (!strncmp((char *) result + 36, "AGFA01", 6)) {
		...
		free(dev);
		DBG(1,
		    "attach: device looks like an AGFA scanner, but wasn't recognised\n");
		return SANE_STATUS_INVAL;
	}
	dev->sane.type = "flatbed scanner";

	dev->transparent = result[45] & 0x80 ? TRUE : FALSE;
	dev->analoglog = result[46] & 0x80 ? TRUE : FALSE;
	dev->tos5 = result[46] & 0x05 ? TRUE : FALSE;
	dev->quality = result[47] & 0x40 ? TRUE : FALSE;
	dev->disconnect = result[47] & 0x80 ? TRUE : FALSE;
...
init_options(AgfaFocus_Scanner * s)
{
	...
	static const SANE_Range sharpen_range = { 0, 7, 1 };
	static const SANE_Range exposure_range = { 0, 100, 0 };
	...
	s->opt[OPT_AUTO_BRIGHTNESS].desc =
		"Turns on automatic brightness correction of "
	...
	s->val[OPT_AUTO_BRIGHTNESS].b = FALSE;
	...
	s->opt[OPT_AUTO_CONTRAST].desc =
		"Turns on automatic contrast correction of "
	...
	s->val[OPT_AUTO_CONTRAST].b = FALSE;

	/* brightness */
	s->opt[OPT_BRIGHTNESS].name = SANE_NAME_BRIGHTNESS;
	s->opt[OPT_BRIGHTNESS].title = SANE_TITLE_BRIGHTNESS;
	s->opt[OPT_BRIGHTNESS].desc =
	...
	if (s->hw->upload_user_defines) {
		s->opt[OPT_HALFTONE_PATTERN].constraint.string_list =
			halftone_upload_list;
		s->val[OPT_HALFTONE_PATTERN].s =
			strdup(halftone_upload_list[0]);
	} else {
		s->opt[OPT_HALFTONE_PATTERN].constraint.string_list =
			halftone_list;
	...
	s->opt[OPT_ATTENUATION_RED].desc =
		"Controls the red attenuation of the acquired image. "
	...
	s->opt[OPT_ATTENUATION_GREEN].desc =
		"Controls the green attenuation of the acquired image. "
		"Higher values mean less impact on scanned image.";
	...
	s->opt[OPT_ATTENUATION_BLUE].desc =
		"Controls the blue attenuation of the acquired image. "
		"Higher values mean less impact on scanned image.";
	...
	s->opt[OPT_QUALITY].desc =
		"Controls the calibration that will be done in the "
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (option >= NUM_OPTIONS
	    || !SANE_OPTION_IS_ACTIVE(s->opt[option].cap))
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_MODE:
			if (strcmp(s->val[option].s, (SANE_String) val)) {
				...
				if (strcmp(s->val[option].s, "Gray (6 bit)")
				    == 0)
					s->mode = GRAY6BIT;
				else if (strcmp
					 (s->val[option].s,
				...
				case LINEART:
					s->opt[OPT_HALFTONE_PATTERN].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_SHARPEN].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_EXPOSURE].cap |=
						SANE_CAP_INACTIVE;
					s->opt[OPT_ATTENUATION_RED].cap |=
						SANE_CAP_INACTIVE;
					s->opt[OPT_ATTENUATION_GREEN].cap |=
						SANE_CAP_INACTIVE;
					s->opt[OPT_ATTENUATION_BLUE].cap |=
						SANE_CAP_INACTIVE;
					s->opt[OPT_BRIGHTNESS].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_CONTRAST].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_AUTO_BRIGHTNESS].cap &=
						~SANE_CAP_INACTIVE;
					s->opt[OPT_AUTO_CONTRAST].cap &=
						~SANE_CAP_INACTIVE;
					break;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		s->exposure =
			((s->val[OPT_EXPOSURE].w * (255.0f - 80.0f)) /
			 100.0f) + 80.0f;
		s->r_att =
			(SANE_UNFIX(s->val[OPT_ATTENUATION_RED].w) * 20.0f) /
			100.0f;
		s->g_att =
			(SANE_UNFIX(s->val[OPT_ATTENUATION_GREEN].w) *
			 20.0f) / 100.0f;
		s->b_att =
			(SANE_UNFIX(s->val[OPT_ATTENUATION_BLUE].w) * 20.0f) /
			100.0f;
		s->tonecurve = 0;
		...
		case LINEART:
		{
			...
			if (strcmp(halftone, "1") == 0)
				...
				s->halftone = 2;
			else if (strcmp(halftone, "Round (Clustered dot 4x4)")
				 == 0)
				s->halftone = 3;
			else if (strcmp
				 (halftone,
				  "Diamond (Clustered dot 4x4)") == 0)
				s->halftone = 4;
		...
		/*s->params.bytes_per_line =
		   ...
		   / (8 / s->params.depth); */
	} else if (s->mode == COLOR18BIT || s->mode == COLOR24BIT)
...
reader_process(void *scanner)
{
	...
	status = get_read_sizes(s->fd, &s->lines_available, &bytes_per_line,
				&total_lines);
	if (status != SANE_STATUS_GOOD) {
	...
	if (!s->lines_available || !bytes_per_line || !total_lines
	...
	while (lines_read < s->params.lines) {
		...
		DBG(1, "Requesting %d lines, in scanner: %d, total: %d\n",
		    lines, s->lines_available, s->params.lines);
...
sane_start(SANE_Handle handle)
{
	...
	if (s->pass == 0) {
...

		{
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "open: upload_dither_matrix() failed: %s\n",
				    sane_strstatus(status));
				release_unit(s->fd);
				sanei_scsi_close(s->fd);
				s->fd = -1;
