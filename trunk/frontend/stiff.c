...
write_ifd(FILE * fptr, IFD * ifd, int motorola)
{
	...
	if (motorola)
		putc('M', fptr), putc('M', fptr);
	else
		putc('I', fptr), putc('I', fptr);
...
write_tiff_bw_header(FILE * fptr, int width, int height, int resolution)
{
	...
	/* image width */
	add_ifd_entry(ifd, 256,
		      (width > 0xffff) ? IFDE_TYP_LONG : IFDE_TYP_SHORT, 1,
		      width);
	/* image length */
	add_ifd_entry(ifd, 257,
		      (height > 0xffff) ? IFDE_TYP_LONG : IFDE_TYP_SHORT, 1,
		      height);
	/* bits per sample */
		       ...
		       int resolution, const char *icc_profile)
{
	...
	if (icc_profile) {
		...
		if (!icc_file) {
			...
			icc_len =
				16777216 * fgetc(icc_file) +
				65536 * fgetc(icc_file) +
				256 * fgetc(icc_file) + fgetc(icc_file);
			rewind(icc_file);
		}
	}
			...
			int resolution, const char *icc_profile)
{
	...
	default:
		if (depth == 1)
			write_tiff_bw_header(stdout, width, height,
					     resolution);
		else
