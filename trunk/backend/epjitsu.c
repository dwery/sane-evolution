...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (fp) {
		...
		while (sanei_config_read(line, PATH_MAX, fp)) {
			...
			if ((strncmp("firmware", lp, 8) == 0)
			    && isspace(lp[8])) {
				lp += 8;
				lp = sanei_config_skip_whitespace(lp);
				DBG(15, "sane_init: firmware '%s'\n", lp);
				strncpy((char *) global_firmware_filename, lp,
					PATH_MAX);
				...
				DBG(5,
				    "sane_init: config line \"%s\" ignored.\n",
				    lp);
			}
		}
		fclose(fp);
...
load_fw(struct scanner *s)
{
	...
	ret = do_cmd(s, 0, cmd, cmdLen, buf, FIRMWARE_LENGTH, NULL, 0);
	if (ret) {
...
get_ident(struct scanner *s)
{
	...
	ret = do_cmd(s, 0, cmd, cmdLen, NULL, 0, in, &inLen);
...
void
update_block_totals(struct scanner *s)
{
...
change_params(struct scanner *s)
{
	...
	if (s->model == MODEL_S300) {
		...
		if (s->resolution_x == 150) {
			...
			s->setWindowCoarseCalLen =
				sizeof(setWindowCoarseCal_S300_150);

			s->setWindowFineCal = setWindowFineCal_S300_150;
			s->setWindowFineCalLen =
				sizeof(setWindowFineCal_S300_150);

			s->setWindowSendCal = setWindowSendCal_S300_150;
			s->setWindowSendCalLen =
				sizeof(setWindowSendCal_S300_150);

			s->sendCal1Header = sendCal1Header_S300_150;
			s->sendCal1HeaderLen =
				sizeof(sendCal1Header_S300_150);

			s->sendCal2Header = sendCal2Header_S300_150;
			s->sendCal2HeaderLen =
				sizeof(sendCal2Header_S300_150);
...
*/
		...
		/* FIXME: too short? */
			...
			s->setWindowCoarseCalLen =
				sizeof(setWindowCoarseCal_FI60F_300);

			s->setWindowFineCal = setWindowFineCal_FI60F_300;
			s->setWindowFineCalLen =
				sizeof(setWindowFineCal_FI60F_300);

			s->setWindowSendCal = setWindowSendCal_FI60F_300;
			s->setWindowSendCalLen =
				sizeof(setWindowSendCal_FI60F_300);
			...
			s->sendCal2HeaderLen =
				sizeof(sendCal2Header_FI60F_300);
	...
	s->coarsecal.total_bytes =
		s->coarsecal.width_bytes * s->coarsecal.height + 8;

	s->darkcal.total_pix = s->darkcal.width_pix * s->darkcal.height;
	s->darkcal.total_bytes =
		s->darkcal.width_bytes * s->darkcal.height + 8;

	s->lightcal.total_pix = s->lightcal.width_pix * s->lightcal.height;
	s->lightcal.total_bytes =
		s->lightcal.width_bytes * s->lightcal.height + 8;
...
sane_start(SANE_Handle handle)
{
	...
	if (!s->started) {
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(5,
			    "sane_start: ERROR: failed to teardown buffers\n");
			sane_cancel((SANE_Handle) s);
			return SANE_STATUS_NO_MEM;
		}
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(5,
			    "sane_start: ERROR: failed to change_params\n");
			sane_cancel((SANE_Handle) s);
			return SANE_STATUS_NO_MEM;
		}
		...
		if (ret != SANE_STATUS_GOOD) {
			DBG(5,
			    "sane_start: ERROR: failed to setup buffers\n");
			sane_cancel((SANE_Handle) s);
			return SANE_STATUS_NO_MEM;
		}
...
setup_buffers(struct scanner *s)
{
	...
	if (!s->coarsecal.buffer) {
		DBG(5,
		    "setup_buffers: ERROR: failed to setup coarse cal buffer\n");
		return SANE_STATUS_NO_MEM;
	}
	...
	if (!s->darkcal.buffer) {
		DBG(5,
		    "setup_buffers: ERROR: failed to setup fine cal buffer\n");
		return SANE_STATUS_NO_MEM;
	}
	...
	if (!s->sendcal.buffer) {
		DBG(5,
		    "setup_buffers: ERROR: failed to setup send cal buffer\n");
		return SANE_STATUS_NO_MEM;
	}
	...
	if (!s->block.buffer) {
		DBG(5,
		    "setup_buffers: ERROR: failed to setup block buffer\n");
		return SANE_STATUS_NO_MEM;
	}
	...
	if (s->source != SOURCE_ADF_BACK) {
		...
		if (!s->front.buffer) {
			DBG(5,
			    "setup_buffers: ERROR: failed to setup front buffer\n");
			return SANE_STATUS_NO_MEM;
		}
	}
	...
	if (s->source == SOURCE_ADF_DUPLEX || s->source == SOURCE_ADF_BACK) {
		...
		if (!s->back.buffer) {
			DBG(5,
			    "setup_buffers: ERROR: failed to setup back buffer\n");
			return SANE_STATUS_NO_MEM;
		}
	}
...
coarsecal(struct scanner *s)
{
	...
	while (try++ < 1) {
		...
		ret = do_cmd(s, 0, pay, payLen, NULL, 0, stat, &statLen);
		if (ret) {
...
finecal(struct scanner *s)
{
	...
	for (j = 0; j < s->darkcal.width_bytes; j++) {
		...
		for (i = 0; i < s->darkcal.height; i++) {
			dtotal +=
				s->darkcal.buffer[i * s->darkcal.width_bytes +
						  j];
		}
	...
	if (DBG_LEVEL >= 15) {
		...
		fprintf(foo, "P5\n%d\n%d\n255\n", s->darkcal.width_bytes,
			s->darkcal.height * 2 + 2);
		fwrite(s->darkcal.buffer, s->darkcal.total_bytes - 8, 1, foo);
		fwrite(s->lightcal.buffer, s->lightcal.total_bytes - 8, 1,
		       foo);
...
set_window(struct scanner *s, int window)
{
	...
	ret = do_cmd(s, 0, payload, paylen, NULL, 0, stat, &statLen);
	if (ret) {
	...
	if (s->scan.rx_bytes != s->scan.total_bytes) {
		...
		if (!s->block.rx_bytes) {
			...
			if (s->model == MODEL_S300) {
				...
				if (ret) {
					DBG(5,
					    "sane_read: error sending d3 cmd\n");
					return ret;
				}
				if (stat[0] != 6) {
					DBG(5,
					    "sane_read: cmd bad status?\n");
					return SANE_STATUS_IO_ERROR;
				}
			}
		}
		...
		if (s->block.rx_bytes == s->block.total_bytes) {
			...
			if (s->model == MODEL_S300) {
				...
				if (ret) {
					DBG(5,
					    "sane_read: error sending 43 cmd\n");
					return ret;
				}
				...
				if (ret) {
					DBG(5,
					    "sane_read: cant copy to front/back\n");
					return ret;
				}
			}
...
read_from_scanner(struct scanner *s, struct transfer *tp)
{
	...
	ret = do_cmd(s, 0,
		     NULL, 0, NULL, 0, tp->buffer + tp->rx_bytes, &bytes);
	...
	DBG(10, "read_from_scanner: finish rB:%lu len:%lu\n",
	    (unsigned long) (tp->total_bytes - tp->rx_bytes),
	    (unsigned long) bytes);
...
fill_frontback_buffers_S300(struct scanner *s)
{
	...
	case MODE_COLOR:
		...
		if (s->source != SOURCE_ADF_BACK) {
			...
			for (i = 0; i < s->block.rx_bytes - 8;
			     i += s->block.width_bytes) {
