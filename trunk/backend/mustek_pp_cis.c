...
Mustek_PP_1015_reg_r_name(Mustek_PP_1015R_reg id)
{
	static const char *names[4] =
		{ "ASIC", "SCAN_VAL", "MOTOR", "BANK_COUNT" };
...
Mustek_PP_1015_reg_w_name(Mustek_PP_1015R_reg id)
{
	static const char *names[4][4] = {
		...
		{"BYTE_COUNT_HB", "BYTE_COUNT_LB", "SKIP_COUNT",
		 "EXPOSE_TIME"},
		{"SRAM_SOURCE_PC", "MOTOR_CONTROL", "UNKNOWN_42",
		 "UNKNOWN_82"},
		{"POWER_ON_DELAY", "CCD_TIMING", "CCD_TIMING_ADJ",
		 "RIGHT_BOUND"}
	};
...
#define Mustek_PP_1015_RING_BUFFER_SIZE 50
	...
	if (index >= Mustek_PP_1015_RING_BUFFER_SIZE)
		index = 0;
...
Mustek_PP_1015_wait_bit(Mustek_PP_CIS_dev * dev, Mustek_PP_1015R_reg reg,
			Mustek_PP_1015R_bit bit, SANE_Bool on,
			unsigned period)
{
	SANE_Byte tmp;
	SANE_Byte mask, val;
	...
	while (dev->desc->state != STATE_CANCELLED) {
		...
		sprintf(&hl_next_line[0], "%s wait_bit(%s, %s, %d): %s %s;\n",
			cis_indent, Mustek_PP_1015_reg_r_name(reg),
			Mustek_PP_1015_bit_name(bit), on ? 1 : 0,
			Mustek_PP_1015_show_val(mask),
			Mustek_PP_1015_show_val(tmp));
		M1015_FLUSH_HL;
#endif
		val = ((on == TRUE) ? tmp : ~tmp) & mask;
		...
		if (tries > 50000) {
#ifdef M1015_LOG_HL
			sprintf(&hl_next_line[0],
				"%s wait_bit(%s, %s, %d): failed;\n",
				cis_indent, Mustek_PP_1015_reg_r_name(reg),
				Mustek_PP_1015_bit_name(bit), on ? 1 : 0);
			M1015_FLUSH_HL;
#endif
			DBG(2,
			    "Mustek_PP_1015_wait_bit: failed (reg %d, bit %d, on: %d)\n",
			    reg, bit, on ? 1 : 0);
			return FALSE;
		}
	}
...
#ifdef M1015_LOG_LL
	fprintf(M1015_LOG_1, "\tread_reg(%s, %d);\n",
		Mustek_PP_1015_reg_r_name(reg), tries);
#endif

#ifdef M1015_TRACE_REGS
	dev->CIS.regs.in_regs[reg & 0x03] = tmp;
#endif
	return dev->desc->state != STATE_CANCELLED ? TRUE : FALSE;
}
...
static void
Mustek_PP_1015_write_reg(Mustek_PP_CIS_dev * dev, Mustek_PP_1015W_reg reg,
			 SANE_Byte val)
{
...
#ifdef M1015_LOG_HL
	sprintf(&hl_next_line[0],
		"%s write_reg3(%s, 0x%02X, 0x%02X, 0x%02X);\n", cis_indent,
		Mustek_PP_1015_reg_w_name(reg), val1, val2, val3);
	M1015_FLUSH_HL;
#endif
}
 ...
 *****************************************************************************/
static void
Mustek_PP_1015_write_reg_start(Mustek_PP_CIS_dev * dev,
			       Mustek_PP_1015W_reg reg)
{
	SANE_Byte regBank = (reg & 0xF0) >> 4;
	SANE_Byte regNo = (reg & 0x0F);
...
{
	return (int) ((dist * dev->CIS.hw_hres) / dev->desc->dev->maxres +
		      0.5);
}
...
{
	return (int) ((dist * dev->CIS.hw_vres) / dev->desc->dev->maxres +
		      0.5);
}
#endif
...
{
	return (int) ((dist * dev->CIS.cisRes) / dev->desc->dev->maxres +
		      0.5);
}
...
{
	return (int) ((dist * dev->desc->dev->maxres) / dev->CIS.cisRes +
		      0.5);
}
...
{
	return (int) ((dist * dev->desc->dev->maxres) / dev->CIS.hw_vres +
		      0.5);
}
#endif
...
{
	dev->bank_count =
		(Mustek_PP_1015_read_reg(dev, MA1015R_BANK_COUNT) & 0x7);
	if (dev->CIS.use8KBank)
		dev->bank_count >>= 1;
}
...
cis_set_sti(Mustek_PP_CIS_dev * dev)
{
	...
	dev->bank_count++;
	dev->bank_count &= (dev->CIS.use8KBank == TRUE) ? 3 : 7;
}
...
cis_wait_bank_change(Mustek_PP_CIS_dev * dev, int bankcount)
{
	...
	while ((dev->bank_count != bankcount)
	       && (diff < MUSTEK_PP_CIS_WAIT_BANK));

	if (dev->bank_count != bankcount
	...
	return dev->bank_count == bankcount ? TRUE : FALSE;
}
...
cis_config_ccd(Mustek_PP_CIS_dev * dev)
{
	...
	if (dev->CIS.res != 0)
		dev->CIS.hres_step =
			SANE_FIX((float) dev->CIS.hw_hres /
				 (float) dev->CIS.res);
	...
	if (dev->CIS.setParameters) {
		dev->CIS.channel = dev->desc->mode == MODE_COLOR ?
			MUSTEK_PP_CIS_CHANNEL_RED :
			MUSTEK_PP_CIS_CHANNEL_GRAY;
	} else {
	...
	if (dev->CIS.setParameters == TRUE) {
		...
		DBG(4, "cis_config_ccd: Skip count: %d (cis res: %d)\n",
		    skipCount, dev->CIS.cisRes);
		skipCount += (int) (2.0 / 25.4 * dev->CIS.cisRes);
		DBG(4, "cis_config_ccd: Skip count: %d\n", skipCount);

		Mustek_PP_1015_write_reg(dev, MA1015W_SKIP_COUNT,
					 skipCount / 32);
	...
	if (dev->CIS.setParameters == TRUE) {
		...
		Mustek_PP_1015_write_reg3(dev, MA1015W_RED_REF, 0x96, 0x96,
					  0x96);
		dev->CIS.adjustskip = max2hw_hres(dev, skipCount);
		byteCount =
			max2hw_hres(dev, skipCount + dev->CIS.imagebytes) + 2;
		dev->CIS.setParameters = FALSE;
	} else {
...
cis_wait_motor_stable(Mustek_PP_CIS_dev * dev)
{
	...
	SANE_Bool ret = Mustek_PP_1015_wait_bit(dev, MA1015R_MOTOR,
						MA1015B_MOTOR_STABLE,
						FALSE, 0);
#ifdef HAVE_SYS_SELECT_H
...
static void
cis_move_motor(Mustek_PP_CIS_dev * dev, int steps)
{				/* steps @ maxres */
	...
	while (quadSteps-- > 0 && dev->desc->state != STATE_CANCELLED) {
		...
		Mustek_PP_1015_write_reg(dev, MA1015W_MOTOR_CONTROL,
					 quadStep);
	}
...
cis_read_line_low_level(Mustek_PP_CIS_dev * dev, SANE_Byte * buf,
			int pixel, SANE_Byte * calib_low,
			SANE_Byte * calib_hi, int * gamma)
{
	u_char color;
	...
	if (dev->CIS.hw_hres == dev->CIS.res) {
		...
		int step = SANE_FIX(1) - dev->CIS.hres_step;
		...
		do {
			...
			if (++calctr >= dev->calib_pixels) {
				...
				DBG(3,
				    "cis_read_line_low_level: calibration overshoot\n");
			}
...
static SANE_Bool
cis_read_line(Mustek_PP_CIS_dev * dev, SANE_Byte * buf, int pixel,
	      SANE_Bool raw)
{
	if (!dev->CIS.dontIncRead)
		CIS_INC_READ(dev);
	else
		dev->CIS.dontIncRead = FALSE;
...
cis_get_next_line(Mustek_PP_CIS_dev * dev, SANE_Byte * buf)
{
	...
	int ctr, channel, first, last, stride, ignore, step =
		dev->CIS.line_step;
	SANE_Byte gotline;
	...
	do {
		...
		for (channel = first; channel <= last; ++channel) {
			if (!cis_read_line
			    (dev, tmpbuf, dev->desc->params.pixels_per_line,
			     FALSE))
				return;

			dest = buf + channel - first;
			for (ctr = 0; ctr < dev->desc->params.pixels_per_line;
			     ctr++) {
				*dest = tmpbuf[ctr];
				dest += stride;
			}
	...
	for (i = 0; i < 8; i++) {
		for (channel = first; channel <= last; ++channel) {
			dev->CIS.powerOnDelay[channel] =
				(powerOnDelayLower[channel] +
				 powerOnDelayUpper[channel]) / 2;
		}
		Mustek_PP_1015_write_reg(dev, MA1015W_POWER_ON_DELAY, dev->CIS.powerOnDelay[1]);	/* Green */
		...
		DBG(4,
		    "cis_maximize_dynamic_range: power on delay %3d %3d %3d\n",
		    dev->CIS.powerOnDelay[0], dev->CIS.powerOnDelay[1],
		    dev->CIS.powerOnDelay[2]);
	}
	dev->CIS.dontMove = FALSE;
		     ...
		     int pixels, int first, int last)
{
	...
	memset((void *) &min, 255,
	       3 * MUSTEK_PP_CIS_MAX_H_PIXEL * sizeof(SANE_Byte));
	memset((void *) &max, 0,
	       3 * MUSTEK_PP_CIS_MAX_H_PIXEL * sizeof(SANE_Byte));
	memset((void *) &sum, 0,
	       3 * MUSTEK_PP_CIS_MAX_H_PIXEL * sizeof(int));
	...
	for (cnt = 0; cnt < MUSTEK_PP_CIS_AVERAGE_COUNT + 2; ++cnt) {
		for (channel = first; channel <= last; ++channel) {
			DBG(4,
			    "cis_measure_extremes: Reading line %d - channel %d\n",
			    cnt, channel);
			...
			for (p = 0; p < pixels; ++p) {
				...
				if (val < min[channel][p])
					min[channel][p] = val;
				if (val > max[channel][p])
					max[channel][p] = val;
				sum[channel][p] += val;
			}
		}
		...
		for (p = 0; p < pixels; p++) {
			...
			if (cal_hi <= cal_low) {
				if (cal_hi < 255) {
					...
					dev->calib_hi[channel][p] =
						cal_low + 1;
				} else {
					/* calib_low exists, else cal_low would be 0, < 255  */
					dev->calib_low[channel][p] =
						cal_hi - 1;
				}
			}
		}
...
cis_measure_delay(Mustek_PP_CIS_dev * dev)
{
	...
	for (d = 0; d < 75 /* 255 */  && dev->desc->state != STATE_CANCELLED;
	     d += 5) {
		dev->CIS.delay = d;
		...
		for (i = 0; i < 5; ++i) {
			CIS_INC_READ(dev);
			CIS_CLEAR_READ_BANK(dev);
			cis_read_line_low_level(dev, &buf[1][0], 2048, NULL,
						NULL, NULL);
			if (dev->desc->state == STATE_CANCELLED)
				return FALSE;
		}
...
cis_return_home(Mustek_PP_CIS_dev * dev, SANE_Bool nowait)
{
	...
	if (nowait == FALSE)
		Mustek_PP_1015_wait_bit(dev, MA1015R_MOTOR,
					MA1015B_MOTOR_HOME, TRUE, 1000);
}
...
cis_calibrate(Mustek_PP_CIS_dev * dev)
{
	...
	cis_return_home(dev, FALSE);	/* Wait till it's home */
...
/*   dev->CIS.skipsToOrigin = hw2max_vres(dev, dev->CIS.skipsToOrigin); */
	...
	if (dev->calib_mode) {
		...
		DBG(3,
		    "cis_calibrate: running in calibration mode. Returning home.\n");
		cis_return_home(dev, FALSE);	/* Wait till it's home */
	}
	   ...
	   SANE_Attach_Callback attach, int driverNo, int info)
{
	...
	if (status != SANE_STATUS_GOOD) {
		...
		if (strcmp(port, "0x378") == 0)
			altPort = "parport0";
		else if (strcmp(port, "0x278") == 0)
			altPort = "parport1";
		else if (strcmp(port, "0x3BC") == 0)
			altPort = "parport2";
		else
			return status;
		...
		if (altStatus != SANE_STATUS_GOOD) {
			DBG(2,
			    "cis_attach: couldn't attach to alternative port `%s' "
		...
		const char * name, SANE_Attach_Callback attach)
{
	...
	return cis_attach(port, name, attach, MUSTEK_PP_CIS600,
			  MUSTEK_PP_CIS600);
}
		 ...
		 const char * name, SANE_Attach_Callback attach)
{
	...
	return cis_attach(port, name, attach, MUSTEK_PP_CIS1200,
			  MUSTEK_PP_CIS1200);
}
		  ...
		  const char * name, SANE_Attach_Callback attach)
{
	...
	return cis_attach(port, name, attach, MUSTEK_PP_CIS1200PLUS,
			  MUSTEK_PP_CIS1200PLUS);
}
...
cis_drv_open(char * port, int caps, int * fd)
{
	...
	if (caps != CAP_NOTHING) {
		DBG(1,
		    "cis_drv_open: called with unknown capabilities (0x%02X)\n",
		    caps);
		return SANE_STATUS_INVAL;
	}
...
******************************************************************************/
void
cis_drv_setup(SANE_Handle hndl)
{
	...
	if (cisdev == NULL) {
		DBG(2,
		    "cis_drv_setup: not enough memory for device descriptor\n");
	...
	/* Default values for configurable parameters; configuration file
	   may override them. */
	cisdev->fast_skip = TRUE;
	cisdev->bw_limit = 127;
	cisdev->calib_mode = FALSE;
	       ...
	       const char * optval)
{
	...
	if (!strcmp(optname, "top_adjust")) {
		if (!optval) {
			DBG(1,
			    "cis_drv_config: missing value for option top_adjust\n");
			return SANE_STATUS_INVAL;
		}
		dvalue = atof(optval);
		...
		if (dvalue < -5.0) {
			DBG(1,
			    "cis_drv_config: value for option top_adjust too small: "
			    "%.2f < -5; limiting to -5 mm\n", dvalue);
			dvalue = -5.0;
		}
		if (dvalue > 5.0) {
			DBG(1,
			    "cis_drv_config: value for option top_adjust too large: "
			    "%.2f > 5; limiting to 5 mm\n", dvalue);
			dvalue = 5.0;
		}
		...
		if (optval) {
			DBG(1,
			    "cis_drv_config: unexpected value for option slow_skip\n");
			return SANE_STATUS_INVAL;
		}
		DBG(3, "cis_drv_config: disabling fast skipping\n");
		cisdev->fast_skip = FALSE;
	} else if (!strcmp(optname, "bw")) {
		if (!optval) {
			DBG(1,
			    "cis_drv_config: missing value for option bw\n");
			return SANE_STATUS_INVAL;
		}
		value = atoi(optval);
		if (value < 0 || value > 255) {
			DBG(1,
			    "cis_drv_config: value for option bw out of range: "
			    "%d < 0 or %d > 255\n", value, value);
			return SANE_STATUS_INVAL;
		}
		...
		if (optval) {
			DBG(1,
			    "cis_drv_config: unexpected value for option calibration_mode\n");
			return SANE_STATUS_INVAL;
		}
		DBG(3, "cis_drv_config: using calibration mode\n");
		cisdev->calib_mode = TRUE;
	} else if (!strcmp(optname, "engine_delay")) {
		if (!optval) {
			DBG(1,
			    "cis_drv_config: missing value for option engine_delay\n");
			return SANE_STATUS_INVAL;
		}
		value = atoi(optval);
		if (value < 0 || value > 100) {	/* 100 ms is already pretty slow */
			DBG(1,
			    "cis_drv_config: value for option engine_delay out of range: "
			    "%d < 0 or %d > 100\n", value, value);
			return SANE_STATUS_INVAL;
		}
...
******************************************************************************/
void
cis_drv_close(SANE_Handle hndl)
{
	Mustek_pp_Handle *dev = hndl;
	Mustek_PP_CIS_dev *cisdev = dev->priv;
	DBG(3, "cis_close: resetting device.\n");
	sanei_pa4s2_enable(dev->fd, TRUE);
	cis_reset_device(cisdev);
	DBG(3, "cis_close: returning home.\n");
	cis_return_home(cisdev, TRUE);	/* Don't wait */
	DBG(3, "cis_close: disabling fd.\n");
	sanei_pa4s2_enable(dev->fd, FALSE);
...
******************************************************************************/
SANE_Status
cis_drv_start(SANE_Handle hndl)
{
	Mustek_pp_Handle *dev = hndl;
	Mustek_PP_CIS_dev *cisdev = dev->priv;
	...
	cisdev->CIS.setParameters = FALSE;
	cisdev->CIS.use8KBank = TRUE;
	...
	if (dev->res > cisdev->CIS.hw_hres)
		cisdev->calib_pixels =
			(pixels * cisdev->CIS.hw_hres) / dev->res;
	else
	...
	cis_reset_device(cisdev);
	cis_return_home(cisdev, TRUE);	/* Don't wait here */
	...
	cis_reset_device(cisdev);
	cis_return_home(cisdev, TRUE);	/* no wait */
	...
	if (cisdev->tmpbuf == NULL) {
		sanei_pa4s2_enable(dev->fd, FALSE);
		DBG(2,
		    "cis_drv_start: not enough memory for temporary buffer\n");
		free(cisdev);
		dev->priv = NULL;
		return SANE_STATUS_NO_MEM;
	}
	...
	if (cisdev->calib_low[1] == NULL || cisdev->calib_hi[1] == NULL) {
		free(cisdev->calib_low[1]);
		cisdev->calib_low[1] = NULL;
		free(cisdev->calib_hi[1]);
		cisdev->calib_hi[1] = NULL;
		sanei_pa4s2_enable(dev->fd, FALSE);
		DBG(2,
		    "cis_drv_start: not enough memory for calibration buffer\n");
		free(cisdev->tmpbuf);
		cisdev->tmpbuf = NULL;
		free(cisdev);
		dev->priv = NULL;
	...
	if (dev->mode == MODE_COLOR) {
		...
		if ((cisdev->calib_low[0] == NULL)
		    || (cisdev->calib_low[2] == NULL)
		    || (cisdev->calib_hi[0] == NULL)
		    || (cisdev->calib_hi[2] == NULL)) {
			free(cisdev->calib_low[0]);
			cisdev->calib_low[0] = NULL;
			free(cisdev->calib_low[1]);
			cisdev->calib_low[1] = NULL;
			free(cisdev->calib_low[2]);
			cisdev->calib_low[2] = NULL;
			free(cisdev->calib_hi[0]);
			cisdev->calib_hi[0] = NULL;
			free(cisdev->calib_hi[1]);
			cisdev->calib_hi[1] = NULL;
			free(cisdev->calib_hi[2]);
			cisdev->calib_hi[2] = NULL;
			free(cisdev->tmpbuf);
			cisdev->tmpbuf = NULL;
			free(cisdev);
			...
			sanei_pa4s2_enable(dev->fd, FALSE);
			DBG(2,
			    "cis_drv_start: not enough memory for color calib buffer\n");
			return SANE_STATUS_NO_MEM;
		}
	}
...
/*   M1015_DISPLAY_REGS(dev, "after calibration"); */
	...
	cisdev->CIS.line_step =
		SANE_FIX((float) cisdev->CIS.hw_vres /
			 (float) cisdev->CIS.res);
...
******************************************************************************/
void
cis_drv_read(SANE_Handle hndl, SANE_Byte * buffer)
{
	Mustek_pp_Handle *dev = hndl;
	Mustek_PP_CIS_dev *cisdev = dev->priv;
