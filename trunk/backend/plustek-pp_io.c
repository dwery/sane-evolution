 ...
 */
static Byte
ioDataFromSPPFast(pScanData ps)
{
	Byte bData, tmp;
...
static Byte
ioDataFromSPPMiddle(pScanData ps)
{
	Byte bData, tmp;
...
static UChar
ioDataFromSPPSlow(pScanData ps)
{
	Byte bData, tmp;
...
static UChar
ioDataFromSPPSlowest(pScanData ps)
{
	Byte bData, tmp;
 ...
 */
static Bool
fnSPPRead(pScanData ps, pUChar pBuffer, ULong ulSize)
{
	switch (ps->IO.delay) {
 ...
 */
static Bool
fnEPPRead(pScanData ps, pUChar pBuffer, ULong ulSize)
{
	register ULong i;
 ...
 */
static Bool
fnBiDirRead(pScanData ps, pUChar pBuffer, ULong ulSize)
{
	UChar start, end;
 ...
 */
static void
ioSwitchToSPPMode(pScanData ps)
{
 ...
 */
static void
ioRestoreParallelMode(pScanData ps)
{
 ...
 */
_LOC void
ioP98001EstablishScannerConnection(pScanData ps, ULong delTime)
{
 ...
 */
static void
ioP98003EstablishScannerConnection(pScanData ps, ULong delTime)
{
	_OUTB_DATA(ps, _ID1ST);
	_DO_UDELAY(delTime);
 ...
 */
static Bool
ioP96OpenScanPath(pScanData ps)
{
 ...
 */
static Bool
ioP98OpenScanPath(pScanData ps)
{
	Byte tmp;
	ULong dw;
	ULong dwTime = 1;
	...
	if (0 == ps->IO.bOpenCount) {
		...
		for (dw = 10; dw; dw--) {
			...
			if (0x50 == (tmp & 0xf0)) {
				...
				if (ps->sCaps.AsicID ==
				    IODataFromRegister(ps, ps->RegAsicID)) {
					return _TRUE;
				}
				ps->IO.bOpenCount = 0;
			}
 ...
 */
static int
ioP98ReadWriteTest(pScanData ps)
{
	UChar tmp;
	ULong ul;
	pUChar buffer;
	...
	for (ul = 0; ul < _MEMTEST_SIZE; ul++) {
		if (buffer[ul] != buffer[ul + _MEMTEST_SIZE]) {
			DBG(DBG_HIGH,
			    "Error in memory test at pos %lu (%u != %u)\n",
 ...
 */
static void
ioSPPWrite(pScanData ps, pUChar pBuffer, ULong size)
{
 ...
 */
_LOC void
IOMoveDataToScanner(pScanData ps, pUChar pBuffer, ULong size)
{
#ifdef DEBUG
	if (0 == ps->IO.bOpenCount)
 ...
 */
_LOC void
IODownloadScanStates(pScanData ps)
{
	TimerDef timer;
#ifdef DEBUG
	if (0 == ps->IO.bOpenCount)
 ...
 */
_LOC void
IODataToScanner(pScanData ps, Byte bValue)
{
	ULong deltime = 4;
 ...
 */
_LOC void
IODataToRegister(pScanData ps, Byte bReg, Byte bData)
{
#ifdef DEBUG
	if (0 == ps->IO.bOpenCount)
 ...
 */
_LOC Byte
IODataFromRegister(pScanData ps, Byte bReg)
{
	IORegisterToScanner(ps, bReg);
 ...
 */
_LOC void
IORegisterToScanner(pScanData ps, Byte bReg)
{
#ifdef DEBUG
	if (0 == ps->IO.bOpenCount)
 ...
 */
_LOC void
IODataRegisterToDAC(pScanData ps, Byte bReg, Byte bData)
{
	ULong i;
 ...
 */
_LOC Byte
IODataRegisterFromScanner(pScanData ps, Byte bReg)
{
	Byte bData;
 ...
 */
_LOC void
IOCmdRegisterToScanner(pScanData ps, Byte bReg, Byte bData)
{
	ps->OpenScanPath(ps);
 ...
 */
_LOC void
IORegisterDirectToScanner(pScanData ps, Byte bReg)
{
 ...
 */
_LOC void
IOReadScannerImageData(pScanData ps, pUChar pBuf, ULong size)
{
	if (_ASIC_IS_98003 != ps->sCaps.AsicID)
		ps->OpenScanPath(ps);
...
_LOC void
IOOutDelayed(Byte data, UShort port)
{
