		    ...
		    SANE_Int format, SANE_Word total, unsigned char *image)
{
	...
	while (count == 0) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "simple_scan: failed to wait for data\n");
			return status;
		}
		if (count == 0) {
			status = sanei_rts88xx_read_reg(devnum, CONTROL_REG,
							&control);
			if (((control & 0x08) == 0)
	...
	while ((read < total) && (count != 0 || (control & 0x08) == 0x08)) {
		...
		if (count > 0) {
			...
			if (len > 0) {
				status = sanei_rts88xx_read_data(devnum, &len,
								 image +
								 read);
				if (status != SANE_STATUS_GOOD) {
					DBG(DBG_error,
...
rts8891_set_default_regs(SANE_Byte * scanner_regs)
{
	SANE_Byte default_75[RTS8891_MAX_REGISTERS] =
		{ 0xe5, 0x41, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x0a, 0x0a,
0x0a, 0x70,
		0x00, 0x00, 0x00, 0x00, 0x28, 0x3f, 0xff, 0x20, 0xf8, 0x28,
			0x07, 0x00,
		0xff, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00,
		0x00, 0x3a, 0xf2, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x10, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00,
		...
		
	};
...
read_data(struct Rts8891_Scanner *scanner, SANE_Byte * dest, SANE_Int length)
{
	...
	if (dev->read == 0) {
		do {
			status = sanei_rts88xx_data_count(dev->devnum,
							  &count);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
				    "read_data: failed to wait for data\n");
				return status;
			}
			if (count == 0) {
				sanei_rts88xx_read_reg(dev->devnum,
						       CONTROL_REG, &control);
		...
		while (count == 0);
	} else {		/* start of read for a new block */
		...
		if (count == 0) {
			sanei_rts88xx_read_reg(dev->devnum, CONTROL_REG,
					       &control);
			if ((control & 0x08) == 0 && (count == 0)) {
				DBG(DBG_error,
				    "read_data: scanner stopped being busy before data are available\n");
	...
	while (read < length && dev->read < dev->to_read
		...
		if (dev->read == 0) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
				    "read_data: failed to read data count\n");
				return status;
			}
		}
		...
		if (count > 0) {
			...
			if (len & 1) {
				DBG(DBG_io,
				    "read_data: round to next even number\n");
				len++;
			}
			...
			status = sanei_rts88xx_read_data(dev->devnum, &len,
							 dest + read);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
				    "read_data: failed to read from scanner\n");
				return status;
			}
	...
	if (dev->read >= dev->to_read) {
		...
		if (count > 0) {
			...
			while (count > 0) {
				...
				sanei_rts88xx_read_data(dev->devnum, &len,
							buffer);
				sanei_rts88xx_data_count(dev->devnum, &count);
			}
		}
