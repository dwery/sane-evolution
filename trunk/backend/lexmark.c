...
attachLexmark(SANE_String_Const devname)
{
	...
	DBG(2,
	    "attachLexmark: testing device `%s': 0x%04x:0x%04x, no variant\n",
	    devname, vendor, product);
	if (sanei_lexmark_low_assign_model
	    (lexmark_device, devname, vendor, product,
	     0) != SANE_STATUS_GOOD) {
		DBG(2,
		    "attachLexmark: unsupported device `%s': 0x%04x:0x%04x\n",
		    devname, vendor, product);
...
calc_parameters(Lexmark_Device * lexmark_device)
{
	...
	if (lexmark_device->val[OPT_MANUAL_GAIN].w == TRUE) {
		if (strcmp
		    (lexmark_device->val[OPT_MODE].s,
		     SANE_VALUE_SCAN_MODE_COLOR)
		    != 0) {
			lexmark_device->opt[OPT_GRAY_GAIN].cap &=
				~SANE_CAP_INACTIVE;
			lexmark_device->opt[OPT_RED_GAIN].cap |=
				SANE_CAP_INACTIVE;
			lexmark_device->opt[OPT_GREEN_GAIN].cap |=
				SANE_CAP_INACTIVE;
			lexmark_device->opt[OPT_BLUE_GAIN].cap |=
				SANE_CAP_INACTIVE;
		} else {
			lexmark_device->opt[OPT_GRAY_GAIN].cap |=
				SANE_CAP_INACTIVE;
		    ...
		    void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		if (lexmark_device->opt[option].constraint_type ==
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(2,
				    "SANE_CONTROL_OPTION: Bad value for range\n");
				return SANE_STATUS_INVAL;
			}
		}
	...
	case SANE_ACTION_GET_VALUE:
		...
		case OPT_BR_X:
		case OPT_BR_Y:
			*(int *) value = lexmark_device->val[option].w;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (strcmp
	    (lexmark_device->val[OPT_MODE].s, SANE_VALUE_SCAN_MODE_COLOR)
	    != 0) {
		channels = 1;
		bitsperchannel = 8;
	}
	...
	/* If not color there is only 1 channel */
	if (strcmp
	    (lexmark_device->val[OPT_MODE].s, SANE_VALUE_SCAN_MODE_LINEART)
	    == 0) {
		bitsperchannel = 1;
	}
	...
	lexmark_device->data_size =
		channels * device_params->pixels_per_line *
		device_params->lines;
	...
	if (device_params->last_frame == TRUE)
...
sane_start(SANE_Handle handle)
{
	...
	lexmark_device->device_cancelled = FALSE;
	lexmark_device->data_ctr = 0;
	lexmark_device->eof = FALSE;
	...
	sanei_lexmark_low_set_scan_regs(lexmark_device, 0, FALSE);
	...
	/* Set the shadow registers for scan with the options (resolution, mode, 
	   ...
	   start. */
	sanei_lexmark_low_set_scan_regs(lexmark_device, offset, TRUE);

	if (sanei_lexmark_low_start_scan(lexmark_device) == SANE_STATUS_GOOD) {
		DBG(2, "sane_start: scan started\n");
		return SANE_STATUS_GOOD;
	} else {
		lexmark_device->device_cancelled = TRUE;
		return SANE_STATUS_INVAL;
	}
}
