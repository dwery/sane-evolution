			  ...
			  Genesys_Register_Set * reg, size_t elems)
{
	...
	DBG(DBG_io,
	    "gl646_bulk_write_register: wrote %lu bytes, %lu registers\n",
...
gl646_asic_test(Genesys_Device * dev)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_asic_test: failed to write register: %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_asic_test: failed to read register: %s\n",
		    sane_strstatus(status));
		return status;
	}
	if (val != 0xde) {	/* value of register 0x38 */
		DBG(DBG_error,
		    "gl646_asic_test: register contains invalid value\n");
	...
	if (!data) {
		DBG(DBG_error,
		    "gl646_asic_test: could not allocate memory\n");
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_asic_test: failed to set buffer address: %s\n",
		    sane_strstatus(status));
		free(data);
		free(verify_data);
		return status;
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_asic_test: failed to bulk write data: %s\n",
		    sane_strstatus(status));
		free(data);
		free(verify_data);
		return status;
	...
	status = gl646_bulk_read_data(dev, 0x45, (u_int8_t *) verify_data,
				      verify_size);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_asic_test: failed to bulk read data: %s\n",
		    sane_strstatus(status));
		free(data);
		free(verify_data);
		return status;
	...
	for (i = 0; i < size; i++) {
		if (verify_data[i + 2] != data[i]) {
			DBG(DBG_error,
			    "gl646_asic_test: data verification error\n");
			free(data);
			free(verify_data);
			return SANE_STATUS_IO_ERROR;
		}
...
/* ST24: 0x30 0x00 0x31 0x0c 0x32 0x2a 0x33 0xf8 0x34 0x0c 0x35 0x01 0x36 0x00 0x37 0x00 0x38 0x2a 0x39 0xf8 */
	...
	dev->reg[reg_0x39].value =
		LOBYTE(dev->settings.exposure_time) /*0xf8 */ ;
		       ...
		       u_int16_t * slope_table, int steps)
{
	...
	status = gl646_bulk_write_data(dev, 0x3c, (u_int8_t *) table,
				       steps * 2);
	if (status != SANE_STATUS_GOOD) {
#ifdef WORDS_BIGENDIAN
		free(table);
...
gl646_set_fe(Genesys_Device * dev, u_int8_t set)
{
	...
	if (set == AFE_POWER_SAVE) {
		...
		if (status != SANE_STATUS_GOOD)
			DBG(DBG_error,
			    "gl646_set_fe: writing data failed: %s\n",
			    sane_strstatus(status));
		return status;
	}
	...
	/*if (dev->model->ccd_type!=CCD_HP2300 && dev->model->ccd_type!=CCD_HP2400) */
	{
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl646_set_fe: writing reg0 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl646_set_fe: writing reg2 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
	}
	...
	for (i = 0; i < 3; i++) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl646_set_fe: writing sign[%d] failed: %s\n", i,
			    sane_strstatus(status));
			return status;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl646_set_fe: writing gain[%d] failed: %s\n", i,
			    sane_strstatus(status));
			return status;
		}
	       ...
	       SANE_Bool check_stop)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_end_scan: Failed to write registers: %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (check_stop) {
		for (i = 0; i < 300; i++) {	/* do not wait longer than wait 30 seconds */
			...
			if ((!(val & REG41_MOTMFLG)) && (val & REG41_FEEDFSH)) {
				DBG(DBG_proc,
				    "gl646_end_scan: scanfeed finished\n");
...
gl646_slow_back_home(Genesys_Device * dev, SANE_Bool wait_until_home)
{
	...
	if (val & REG41_HOMESNR) {	/* is sensor at home? */
		DBG(DBG_info,
		    "gl646_slow_back_home: already at home, completed\n");
		dev->scanhead_position_in_steps = 0;
		return SANE_STATUS_GOOD;
	}
	...
	if (dev->model->motor_type == MOTOR_HP2300
		...
		exposure_time =
			sanei_genesys_exposure_time(dev, local_reg, dpi);
	} else
	...
	if (dev->model->motor_type == MOTOR_HP2300
		...
		local_reg[reg_0x02].value =
			(local_reg[reg_0x02].
			 value & ~REG02_STEPSEL) | REG02_HALFSTEP;
	...
	status = gl646_send_slope_table(dev, 0, slope_table0,
					local_reg[reg_0x21].value);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
	...
	if (wait_until_home) {
		...
		while (loop < 3) {	/* do not wait longer then 30 seconds */
			...
			if (val & 0x08) {	/* home sensor */
				DBG(DBG_info,
				    "gl646_slow_back_home: reached home position\n");
				DBG(DBG_proc,
				    "gl646_slow_back_home: finished\n");
				return SANE_STATUS_GOOD;
			}
		...
		SANE_Bool wait_until_home)
{
	...
	if (wait_until_home) {
		...
		while (loop < 300) {
			...
			if (val & REG41_HOMESNR) {
				DBG(DBG_info,
				    "gl646_park_head: reached home position\n");
			...
			if (!(val & REG41_MOTMFLG)) {
				DBG(DBG_info,
				    "gl646_park_head: restarting motor\n");
				status = sanei_genesys_start_motor(dev);
				if (status != SANE_STATUS_GOOD) {
					DBG(DBG_error,
...
gl646_search_start_position(Genesys_Device * dev)
{
	...
	status = gl646_begin_scan(dev, local_reg, TRUE);
	if (status != SANE_STATUS_GOOD) {
		free(data);
		DBG(DBG_error,
	...
	if (depth == 16) {
		if (DBG_LEVEL >= DBG_data)
			sanei_genesys_write_pnm_file("search_position16.pnm",
						     data, depth, 1, pixels,
						     dev->model->
						     search_lines);
		size /= 2;
	...
	status = gl646_end_scan(dev, local_reg, FALSE);
	if (status != SANE_STATUS_GOOD) {
		free(data);
		DBG(DBG_error,
	...
	status = sanei_genesys_search_reference_point(dev, data, start_pixel,
						      dpi, pixels,
						      dev->model->
						      search_lines);
	if (status != SANE_STATUS_GOOD) {
...
/* ST12:
	...
	dev->calib_reg[reg_0x04].value =
		(dev->calib_reg[reg_0x04].
		 value & ~REG04_LINEART) | REG04_BITSET;
...
/* ST12:
	...
	status = gl646_send_slope_table(dev, 0, slope_table,
	...
	status = gl646_bulk_write_register(dev, dev->calib_reg,
					   GENESYS_GL646_MAX_REGS);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
	...
	DBG(DBG_proc,
	    "gl646_init_register_for_coarse_calibration: completed\n");
...
gl646_init_regs_for_shading(Genesys_Device * dev)
{
	...
	exposure_time =
		sanei_genesys_exposure_time(dev, dev->calib_reg,
					    dev->settings.yres);
	DBG(DBG_info,
	...
	if (dev->model->motor_type != MOTOR_5345) {
		...
		steps_sum = sanei_genesys_create_slope_table(dev,
							     ...
							     (dev, dev->reg,
							      dpiset), 0,
							     dpiset, 0);
		move = (dev->model->shading_lines * dev->motor.base_ydpi) /
			dpiset;
		DBG(DBG_info,
		...
		move = (dev->model->shading_lines * exposure_time *
			(1 +
			 dummy_lines) * 2) / (step_parts * slope_table[0]);
	...
	dev->calib_reg[reg_0x1e].value =
		(dev->calib_reg[reg_0x1e].
		 value & ~REG1E_LINESEL) | dummy_lines;
...
gl646_init_regs_for_scan(Genesys_Device * dev)
{
	...
	read_factor = 0.0;
	pixels = (dev->settings.pixels * dev->sensor.optical_res) /
		dev->settings.xres;
	if (half_ccd)
		pixels = pixels / 2;
	...
	if (dpiset != 2 * dev->settings.xres
		...
		read_factor =
			(float) pixels / (float) (dev->settings.pixels * 2);
		DBG(DBG_info,
	...
	if ((dev->model->ccd_type == CCD_5345) && (channels == 1)
	    && (!half_ccd)) {
	...
	if ((dev->model->ccd_type == CCD_5345)
	    ...
	    || (dev->model->ccd_type == CCD_HP2400))
		exposure_time =
			sanei_genesys_exposure_time(dev, dev->reg,
						    dev->settings.xres);
	else
	...
	dev->current_setup.pixels =
		(pixels * dpiset) / dev->sensor.optical_res;
...
/* END TODO */
	...
	status = gl646_send_slope_table(dev, 0, dev->slope_table0,
					sanei_genesys_read_reg_from_set(dev->
									reg,
									0x21));
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
...
gl646_send_gamma_table(Genesys_Device * dev, SANE_Bool generic)
{
	...
	if (!generic
		...
		DBG(DBG_proc,
		    "gl646_send_gamma_table: nothing to send, skipping\n");
		return SANE_STATUS_GOOD;
	}
	...
	if (generic) {
		...
		for (i = 0; i < size; i++) {
			...
			gamma[i * 2 + 1] =
				dev->sensor.red_gamma_table[i] >> 8;
			gamma[i * 2 + size * 2] =
				dev->sensor.green_gamma_table[i] & 0xff;
			gamma[i * 2 + 1 + size * 2] =
				dev->sensor.green_gamma_table[i] >> 8;
			gamma[i * 2 + size * 4] =
				dev->sensor.blue_gamma_table[i] & 0xff;
			gamma[i * 2 + 1 + size * 4] =
				dev->sensor.blue_gamma_table[i] >> 8;
		}
	}
	...
	/* send data */
	status = gl646_bulk_write_data(dev, 0x3c, (u_int8_t *) gamma,
				       size * 2 * 3);
	if (status != SANE_STATUS_GOOD) {
		free(gamma);
		DBG(DBG_error,
...
/* ST24:           0x02 0x71           0x04 0x5f 0x05 0x50           0x08 0x0e 0x09 0x0c 0x0a 0x00 0x0b 0x0c */
	...
	if (dev->model->ccd_type == CCD_5345) {
		...
		total_size =
			(num_pixels * channels * 2 * dpi * lincnt) /
			dev->sensor.optical_res;
		dev->calib_reg[reg_0x35].value =
			LOBYTE(HIWORD(total_size / lincnt));
		dev->calib_reg[reg_0x36].value =
			HIBYTE(LOWORD(total_size / lincnt));
		dev->calib_reg[reg_0x37].value =
			LOBYTE(LOWORD(total_size / lincnt));
	}
...
/* ST24: 0x6a 0x40 0x6b 0xff 0x6c 0x00 0x6d 0x01 */
	...
	dev->calib_reg[reg_0x6a].value =
		dev->calib_reg[reg_0x6a].value | 0x02;
	if (dev->model->motor_type == MOTOR_5345) {
	...
	RIE(gl646_begin_scan(dev, dev->calib_reg, TRUE));
	RIE(sanei_genesys_read_data_from_scanner
	    (dev, first_line, total_size));
	RIE(gl646_end_scan(dev, dev->calib_reg, FALSE));
	...
	for (j = 0; j < channels; j++) {
		for (i = 0; i < num_pixels; i++) {
			val = first_line[i * 2 * channels + 2 * j + 1] * 256 +
	...
	for (j = 0; j < channels; j++) {
		...
		count = 0;
		for (i = 0; i < num_pixels; i++) {
			val = 256 * second_line[i * 2 * channels + j * 2 +
						1] +
		...
		case CCD_HP2300:
			if (j == 0)
				dev->frontend.offset[j] =
					offset + average / (1.2 * 256);
			else
				dev->frontend.offset[j] =
					offset + average / (1.0 * 256);
			break;
		}
		DBG(DBG_proc,
...
gl646_coarse_gain_calibration(Genesys_Device * dev, int dpi)
{
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("alternative_coarse.pnm", line,
					     16, channels, num_pixels,
					     lincnt);
	...
	for (j = 0; j < channels; j++) {
		...
		maximum = 0;
		for (i = 0; i < num_pixels; i++) {
			val = 256 * line[i * 2 * channels + 2 * j + 1] +
		...
		case CCD_HP2300:
			dev->frontend.gain[j] =
				(int) (((dev->sensor.gain_white_ref * 256) /
					average[j] - 1.0) / 0.4);
			break;
		case CCD_5345:
		default:
...
gl646_repark_head(Genesys_Device * dev)
{
	...
	local_reg[reg_0x01].value =
		local_reg[reg_0x01].
		value & ~REG01_DVDSET & ~REG01_FASTMOD & ~REG01_SCAN;
	local_reg[reg_0x02].value =
		(local_reg[reg_0x02].
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl646_repark_head: failed to start motor: %s\n",
		    sane_strstatus(status));
		return SANE_STATUS_IO_ERROR;
	}
...
gl646_init(Genesys_Device * dev)
{
	...
	if (dev->model->ccd_type == CCD_5345) {
		...
		status = dev->model->cmd_set->bulk_read_data(dev, 0x45, data,
							     64);
		if (status != SANE_STATUS_GOOD) {
			/* expected error */
			DBG(DBG_error,
			    "gl646_init: read stream reset ... %s\n",
			    sane_strstatus(status));
			return status;
		}
		/* restore timeout */
	...
	/* Move home */
	RIE(gl646_slow_back_home(dev, TRUE));
