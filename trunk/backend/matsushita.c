...
/*
   $Id: matsushita.c,v 1.10 2005-09-19 19:57:48 fzago-guest Exp $
 ...
 */
#ifdef unused_yet
static const int resolutions_list_200[4] = {
	3, 100, 150, 200
};
static const int resolutions_rounds_200[4] = {
	3, 0x100, 0x40, 0x20
};
#endif

static const int resolutions_list_300[5] = {
	4, 150, 200, 240, 300
};
static const int resolutions_rounds_300[5] = {
	4, 0x100, 0x40, 0x20, 0x80
};

static const int resolutions_list_400[8] = {
	7, 100, 150, 200, 240, 300, 360, 400
};
static const int resolutions_rounds_400[8] = {
...
static const struct scanners_supported scanners[] = {
	...
	/* Panasonic KV-SS50EX */
	{
	 ...
	 image_emphasis_list_5, image_emphasis_val_5,
	 MAT_CAP_AUTOMATIC_THRESHOLD | MAT_CAP_WHITE_LEVEL | MAT_CAP_GAMMA |
	 MAT_CAP_NOISE_REDUCTION | MAT_CAP_PAPER_DETECT |
	 MAT_CAP_MIRROR_IMAGE},
	...
	/* Panasonic KV-SS855 */
	{
	 ...
	 MAT_CAP_PAPER_DETECT | MAT_CAP_DETECT_DOUBLE_FEED |
	 MAT_CAP_MANUAL_FEED},
...
matsushita_identify_scanner(Matsushita_Scanner * dev)
{
	...
	for (i = 0; i < NELEMS(scanners); i++) {
		    ...
		    strcmp(dev->scsi_product, scanners[i].scsi_product) == 0)
		{
			...
			DBG(DBG_error,
			    "matsushita_identify_scanner: scanner supported\n");
	...
	DBG(DBG_proc,
	    "matsushita_identify_scanner: exit, device not supported\n");
...
static int
get_int_list_index(const int list[], const int value)
{
	int index;
...
static SANE_Status
matsushita_sense_handler(int scsi_fd, unsigned char *result,
			 void __sane_unused__ * arg)
{
	...
	DBG(DBG_sense,
	    "matsushita_sense_handler: sense=%d, ASC/ASCQ=%02x%02x\n",
	    sensekey, asc, ascq);
...
attach_scanner(const char *devicename, Matsushita_Scanner ** devp)
{
	...
	if (sanei_scsi_open(devicename, &sfd, matsushita_sense_handler, dev)
	    != 0) {
	...
	if (matsushita_identify_scanner(dev) == FALSE) {
		DBG(DBG_error,
...
matsushita_init_options(Matsushita_Scanner * dev)
{
	...
	dev->opt[OPT_FEEDER_MODE].constraint_type =
	...
	dev->opt[OPT_PAPER_SIZE].size =
		max_string_size(dev->paper_sizes_list);
	dev->opt[OPT_PAPER_SIZE].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	dev->opt[OPT_PAPER_SIZE].constraint.string_list =
		dev->paper_sizes_list;
	...
	dev->opt[OPT_AUTOMATIC_THRESHOLD].title =
		SANE_I18N("Automatic threshold");
	...
	dev->val[OPT_AUTOMATIC_THRESHOLD].s =
		strdup(automatic_threshold_list[0]);
	...
	dev->val[OPT_AUTOMATIC_SEPARATION].w = FALSE;
	...
	dev->opt[OPT_WHITE_LEVEL].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	...
	dev->opt[OPT_NOISE_REDUCTION].size =
		max_string_size(noise_reduction_list);
	dev->opt[OPT_NOISE_REDUCTION].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	...
	dev->opt[OPT_IMAGE_EMPHASIS].desc =
		SANE_I18N("Sets the image emphasis");
	...
	dev->opt[OPT_IMAGE_EMPHASIS].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
...
matsushita_set_window(Matsushita_Scanner * dev, int side)
{
	...
	/* Feeder mode */
	i = get_string_list_index(feeder_mode_list,
				  dev->val[OPT_FEEDER_MODE].s);
	...
	/* White level */
	i = get_string_list_index(white_level_list,
				  dev->val[OPT_WHITE_LEVEL].s);
	...
	if (dev->scan_mode == MATSUSHITA_BW ||
		...
		i = get_string_list_index(automatic_threshold_list,
					  dev->val[OPT_AUTOMATIC_THRESHOLD].
					  s);
...
matsushita_fill_image(Matsushita_Scanner * dev)
{
	...
	while (dev->real_bytes_left) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "sane_read: cannot read from the scanner\n");
			return status;
		}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_BR_X:
			...
			value = mmToIlu(SANE_UNFIX(*(int *) val));

			i = get_int_list_index(scanners[dev->scnum].
					       resolutions_list,
					       dev->val[OPT_RESOLUTION].w);

			if (value &
			    (scanners[dev->scnum].resolutions_round[i] - 1)) {
				value = (value |
					 (scanners[dev->scnum].
					  resolutions_round[i] - 1)) + 1;
				if (info) {
					*info |= SANE_INFO_INEXACT;
				}
			}

			*(int *) val = SANE_FIX(iluToMm(value));
		...
		case OPT_MODE:
			...
			dev->opt[OPT_HALFTONE_PATTERN].cap |=
				SANE_CAP_INACTIVE;
			dev->opt[OPT_AUTOMATIC_THRESHOLD].cap |=
				SANE_CAP_INACTIVE;
			dev->opt[OPT_AUTOMATIC_SEPARATION].cap |=
				SANE_CAP_INACTIVE;
			dev->opt[OPT_NOISE_REDUCTION].cap |=
				SANE_CAP_INACTIVE;
			dev->opt[OPT_GAMMA].cap |= SANE_CAP_INACTIVE;

			if (strcmp(dev->val[OPT_MODE].s, BLACK_WHITE_STR) ==
			    0) {
				dev->depth = 1;
			...
			} else if (strcmp(dev->val[OPT_MODE].s, GRAY4_STR) ==
				   0) {
				dev->scan_mode = MATSUSHITA_GRAYSCALE;
				dev->depth = 4;

				dev->opt[OPT_GAMMA].cap &= ~SANE_CAP_INACTIVE;
			} else if (strcmp(dev->val[OPT_MODE].s, GRAY8_STR) ==
				   0) {
				dev->scan_mode = MATSUSHITA_GRAYSCALE;
				dev->depth = 8;
		...
		case OPT_PAPER_SIZE:
			...
			value = 0;
			rc = sane_control_option(handle, OPT_TL_X,
						 SANE_ACTION_SET_VALUE,
						 &value, info);
			assert(rc == SANE_STATUS_GOOD);

			value = 0;
			rc = sane_control_option(handle, OPT_TL_Y,
						 SANE_ACTION_SET_VALUE,
						 &value, info);
			assert(rc == SANE_STATUS_GOOD);

			value = SANE_FIX(paper_sizes[i].width);
			rc = sane_control_option(handle, OPT_BR_X,
						 SANE_ACTION_SET_VALUE,
						 &value, info);
			assert(rc == SANE_STATUS_GOOD);

			value = SANE_FIX(paper_sizes[i].length);
			rc = sane_control_option(handle, OPT_BR_Y,
						 SANE_ACTION_SET_VALUE,
						 &value, info);
			assert(rc == SANE_STATUS_GOOD);
		...
		case OPT_AUTOMATIC_THRESHOLD:
			...
			if (strcmp
			    (dev->val[option].s,
			     automatic_threshold_list[0]) == 0) {
				dev->opt[OPT_WHITE_LEVEL].cap &=
					~SANE_CAP_INACTIVE;
				dev->opt[OPT_NOISE_REDUCTION].cap &=
					~SANE_CAP_INACTIVE;
				dev->opt[OPT_IMAGE_EMPHASIS].cap &=
					~SANE_CAP_INACTIVE;
				dev->opt[OPT_AUTOMATIC_SEPARATION].cap &=
					~SANE_CAP_INACTIVE;
				if (dev->scan_mode == MATSUSHITA_BW
...
sane_start(SANE_Handle handle)
{
	...
	if (!(dev->scanning)) {
		...
		if (sanei_scsi_open
		    (dev->devicename, &(dev->sfd), matsushita_sense_handler,
		     dev) != 0) {
