...
static char *dbg_scantype(char *text, int type);
...
static char *dbg_colour(char *text, int colour);
...
static void dbg_tiff_save(char *sFile, int width, int height,
			  ...
			  int res_y, SANE_Byte * buffer, int size);
static void dbg_autoref(struct st_scanparams *scancfg, SANE_Byte * pattern,
			int ser1, int ser2, int ler);

#ifdef developing
static void dbg_buffer(int level, char *title, SANE_Byte * buffer,
		       int size, int start);
		...
		for (colour = 0; colour < 3; colour++) {
			if (shadingfile[colour] != NULL) {
				for (a = 0; a < myCalib->shadinglength; a++)
					fprintf(shadingfile[colour],
						"%04i: %04x %04x\n", a,
						(unsigned int) myCalib->
						white_shading[colour][a],
						(unsigned int) myCalib->
						black_shading[colour][a]);
...
static char *
dbg_scantype(char *text, int type)
{
	if (text != NULL)
		switch (type) {
		case ST_NORMAL:
...
dbg_sensor(struct st_sensorcfg *sensor)
{
	if (sensor != NULL) {
		...
		DBG(DBG_FNC,
		    ...
		    sensor->rgb_order[1], sensor->rgb_order[2],
		    sensor->line_distance, sensor->evenodd_distance);
	} else
...
dbg_buttons(struct st_buttons *buttons)
{
	if (buttons != NULL) {
		DBG(DBG_FNC,
		    " -> count, btn1, btn2, btn3, btn4, btn5, btn6\n");
		DBG(DBG_FNC,
		    " -> -----, ----, ----, ----, ----, ----, ----\n");
		...
		for (a = 0; a < dev->scanmodes_count; a++) {
			...
			if (reg != NULL) {
				DBG(DBG_FNC,
				    ...
				    a, dbg_scantype(sdebug, reg->scantype),
				    dbg_colour(scm, reg->colormode),
				    reg->resolution, reg->timing,
				    reg->motorcurve, reg->samplerate,
				    reg->systemclock, reg->ctpc,
				    reg->motorbackstep,
				    reg->scanmotorsteptype, reg->dummyline,
				    reg->expt[0], reg->expt[1], reg->expt[2],
				    reg->mexpt[0], reg->mexpt[1],
				    reg->mexpt[2], reg->motorplus,
				    reg->multiexposurefor16bitmode,
				    reg->multiexposureforfullspeed,
				    reg->multiexposure, reg->mri, reg->msi,
				    reg->mmtir, reg->mmtirh,
				    reg->skiplinecount);
			}
		}
	}
...
dbg_motorcurves(struct st_device *dev)
{
	if (dev->mtrsetting != NULL) {
		...
		while (a < dev->mtrsetting_count) {
			...
			if (mtc != NULL) {
				DBG(DBG_FNC,
				    "mri=%i msi=%i skip=%i bckstp=%i\n",
				    mtc->mri, mtc->msi, mtc->skiplinecount,
				    mtc->motorbackstep);
...
dbg_hwdcfg(struct st_hwdconfig *params)
{
	if (params != NULL) {
		...
		DBG(DBG_FNC, " -> motor_direction       = %s\n",
		    (params->motor_direction ==
		     MTR_FORWARD) ? "FORWARD" : "BACKWARD");
...
static char *
dbg_colour(char *text, int colour)
{
	if (text != NULL)
		switch (colour) {
		case CM_COLOR:
...
dbg_motorcfg(struct st_motorcfg *motorcfg)
{
	if (motorcfg != NULL) {
		...
		DBG(DBG_FNC, " -> %4i, %4i, %4i, %5i, %4i, %4i, %4i, %6i\n",
		    motorcfg->type, motorcfg->resolution,
		    motorcfg->pwmfrequency, motorcfg->basespeedpps,
		    motorcfg->basespeedmotormove,
		...
		if (path != NULL) {
			...
			if (snprintf(filename, 512, "%s/%s", path, sFile) > 0) {
				...
				if ((image = TIFFOpen(filename, "w")) != NULL) {
					...
					TIFFWriteRawStrip(image, 0, buffer,
							  size);
					TIFFClose(image);
				}
			} else
				DBG(DBG_ERR,
				    "- dbg_tiff_save: Error generating filename\n");
		} else
			DBG(DBG_ERR,
	    ...
	    int ser1, int ser2, int ler)
{
	...
	if (img != NULL) {
		...
		for (c = 0;
		     c < (scancfg->coord.width * scancfg->coord.height);
		     c++) {
			value = *(pattern + c);
			*(img + (3 * c)) = value;
			*(img + (3 * c) + 1) = value;
		...
		dbg_tiff_save("post-autoref.tiff", scancfg->coord.width,
			      ...
			      scancfg->resolution_x, scancfg->resolution_y,
			      img,
...
static void
dbg_buffer(int level, char *title, SANE_Byte * buffer, int size,
	   int start)
{
	if (level <= DBG_LEVEL) {
		DBG(level, "%s    ", title);
