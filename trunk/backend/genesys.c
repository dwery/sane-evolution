			     ...
			     int channels, int pixels_per_line, int lines)
{
	...
	if (channels == 3) {
		int count = 0;
		for (count = 0; count < (pixels_per_line * lines * 3);
		     count++) {
			if (depth == 16)
				fputc(*(data + 1), out);
			fputc(*(data++), out);
...
SANE_Byte
sanei_genesys_read_reg_from_set(Genesys_Register_Set * reg, SANE_Byte address)
{
	int i;
...
SANE_Status
sanei_genesys_write_register(Genesys_Device * dev, u_int8_t reg, u_int8_t val)
{
	SANE_Status status;

	status = sanei_usb_control_msg(dev->dn, REQUEST_TYPE_OUT,
				       REQUEST_REGISTER, VALUE_SET_REGISTER,
				       INDEX, 1, &reg);
	...
	status = sanei_usb_control_msg(dev->dn, REQUEST_TYPE_OUT,
				       REQUEST_REGISTER, VALUE_WRITE_REGISTER,
				       INDEX, 1, &val);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
	...
	DBG(DBG_io,
	    "sanei_genesys_write_register (0x%02x, 0x%02x) completed\n", reg,
	    val);
			    ...
			    u_int8_t * val)
{
	...
	*val = 0;
	...
	status = sanei_usb_control_msg(dev->dn, REQUEST_TYPE_IN,
				       REQUEST_REGISTER, VALUE_READ_REGISTER,
				       INDEX, 1, val);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
	...
	DBG(DBG_io,
	    "sanei_genesys_read_register (0x%02x, 0x%02x) completed\n", reg,
	    *val);
...
genesys_dpiset(Genesys_Register_Set * reg)
{
	...
	dpiset = sanei_genesys_read_reg_from_set(reg,
						 0x2c) * 256 +
			     ...
			     unsigned int *used_steps, unsigned int *vfinal)
{
	...
	*used_steps = 0;
	...
	if (stop_at < vstart) {
		t2 = vstart;
		for (i = 0; i < steps && i < use_steps - 1 && i < max_steps;
		     i++, c++) {
		...
		if (t2 > stop_at) {
			DBG(DBG_warn,
			    "Can not reach target speed(%d) in %d steps.\n",
			    stop_at, use_steps);
			DBG(DBG_warn,
			    "Expect image to be distorted. "
...
genesys_create_slope_table4(Genesys_Device * dev,
			    ...
			    int step_type, int exposure_time,
			    SANE_Bool same_speed, double yres, int power_mode)
{
	unsigned int sum_time = 0;
	unsigned int vtarget;
	unsigned int vend;
			    ...
			    SANE_Bool same_speed, double yres, int power_mode)
{
	...
	/* final speed */
	vend = (exposure_time * yres) / (dev->motor.base_ydpi *
					 (1 << step_type));
...
sanei_genesys_init_shading_data(Genesys_Device * dev, int pixels_per_line)
{
	...
	DBG(DBG_proc,
	    "sanei_genesys_init_shading_data (pixels_per_line = %d)\n",
	    pixels_per_line);
	...
	status = genesys_send_offset_and_shading(dev, shading_data,
						 pixels_per_line * 4 *
						 channels);
	if (status != SANE_STATUS_GOOD)
		DBG(DBG_error,
...
sanei_genesys_test_buffer_empty(Genesys_Device * dev, SANE_Bool * empty)
{
	...
	if (dev->model->cmd_set->test_buffer_empty_bit(val)) {
		DBG(DBG_io2,
		    "sanei_genesys_test_buffer_empty: buffer is empty\n");
		*empty = TRUE;
		return SANE_STATUS_GOOD;
	}
				     ...
				     size_t size)
{
	...
	DBG(DBG_proc,
	    "sanei_genesys_read_data_from_scanner (size = %lu bytes)\n",
	    (u_long) size);
				     ...
				     int height)
{
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("laplace.pnm", image, 8, 1,
					     width, height);
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("xsobel.pnm", image, 8, 1, width,
					     height);
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("detected-xsobel.pnm", image, 8,
					     1, width, height);
	left = left / count;
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("ysobel.pnm", image, 8, 1, width,
					     height);
	...
	if (dev->model->ccd_type == CCD_5345
		...
		if (DBG_LEVEL >= DBG_data)
			sanei_genesys_write_pnm_file("detected-ysobel.pnm",
						     image, 8, 1, width,
						     height);
		top = top / count;
...
sanei_genesys_calculate_zmode(Genesys_Device * dev, u_int32_t exposure_time,
			      ...
			      u_int8_t tgtime, u_int32_t * z1, u_int32_t * z2)
{
	...
	*z1 = exposure_factor * ((steps_sum + fwdstep * last_speed) %
				 exposure_time);

	/* Z2 is for acceleration before scan */
	if (fastfed) {		/* two curve mode */
		*z2 = exposure_factor * ((steps_sum + scanfed * last_speed) %
					 exposure_time);
	} else {		/* one curve mode */

		*z2 = exposure_factor * ((steps_sum + feedl * last_speed) %
					 exposure_time);
	}
}
...
genesys_coarse_calibration(Genesys_Device * dev)
{
	...
	size = channels * 2 * SANE_UNFIX(dev->model->y_size) *
		dev->settings.xres / 25.4;
	...
	for (i = 0; i < 4; i++) {	/* read 4 lines */
		...
		if (i == 1) {	/* second line */
			...
			if (dev->settings.scan_method == 2)	/* Transparency */
				gain_white_ref =
					dev->sensor.fau_gain_white_ref * 256;
			else
				gain_white_ref =
					dev->sensor.gain_white_ref * 256;
			/* white and black are defined downwards */

			genesys_adjust_gain(dev, &applied_multi,
					    &dev->frontend.gain[0],
					    gain_white_ref / (white[0] -
							      dark[0]),
					    dev->frontend.gain[0]);
			genesys_adjust_gain(dev, &applied_multi,
					    &dev->frontend.gain[1],
					    gain_white_ref / (white[1] -
							      dark[1]),
					    dev->frontend.gain[1]);
			genesys_adjust_gain(dev, &applied_multi,
					    &dev->frontend.gain[2],
					    gain_white_ref / (white[2] -
							      dark[2]),
					    dev->frontend.gain[2]);
			...
			status = sanei_genesys_fe_write_data(dev, 0x28,
							     dev->frontend.
							     gain[0]);
			...
			status = sanei_genesys_fe_write_data(dev, 0x29,
							     dev->frontend.
							     gain[1]);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
			...
			status = sanei_genesys_fe_write_data(dev, 0x2a,
							     dev->frontend.
							     gain[2]);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
		...
		status = sanei_genesys_fe_write_data(dev, 0x20,
		...
		status = sanei_genesys_fe_write_data(dev, 0x21,
		...
		status = sanei_genesys_fe_write_data(dev, 0x22,
		...
		status = dev->model->cmd_set->begin_scan(dev, dev->calib_reg,
							 FALSE);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		status = sanei_genesys_read_data_from_scanner(dev,
							      calibration_data,
							      size);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		if (i == 3) {	/* last line */
			...
			for (count = 0; count < (unsigned int) (size * 4 / 2);
			     count++)
				all_data_8[count] = all_data[count * 2 + 1];
			status = sanei_genesys_write_pnm_file("coarse.pnm",
							      all_data_8, 8,
							      channels,
							      size / 6, 4);
			if (status != SANE_STATUS_GOOD) {
		...
		status = dev->model->cmd_set->end_scan(dev, dev->calib_reg,
						       TRUE);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		if (dev->settings.scan_mode == 4) {	/* single pass color */
			for (j = 0; j < 3; j++) {
				genesys_average_white(dev, 3, j,
						      calibration_data, size,
						      &white_average);
				white[i * 3 + j] = white_average;
				dark[i * 3 + j] =
					genesys_average_black(dev, j,
							      calibration_data,
							      black_pixels);
				DBG(DBG_info,
				    "genesys_coarse_calibration: white[%d]=%d, black[%d]=%d\n",
				    i * 3 + j, white[i * 3 + j], i * 3 + j,
				    dark[i * 3 + j]);
			}
		} else {	/* one color-component modes */

			genesys_average_white(dev, 1, 0, calibration_data,
					      size, &white_average);
			white[i * 3 + 0] = white[i * 3 + 1] =
				white[i * 3 + 2] = white_average;
			dark[i * 3 + 0] = dark[i * 3 + 1] = dark[i * 3 + 2] =
				genesys_average_black(dev, 0,
						      calibration_data,
						      black_pixels);
		}
		     ...
		     u_int16_t pixel_components_per_line)
{
	...
	for (x = 0; x < pixel_components_per_line; x++) {
		sum = 0;
		for (y = 0; y < lines; y++) {
			sum += calibration_data[(x +
						 y *
						 pixel_components_per_line) *
						2];
			sum += calibration_data[(x +
						 y *
						 pixel_components_per_line) *
...
genesys_dark_shading_calibration(Genesys_Device * dev)
{
	...
	dev->model->cmd_set->set_lamp_power(dev, dev->calib_reg, FALSE);
	dev->model->cmd_set->set_motor_power(dev->calib_reg, FALSE);
	...
	status = sanei_genesys_read_data_from_scanner(dev, calibration_data,
						      size);
	if (status != SANE_STATUS_GOOD) {
		free(calibration_data);
		DBG(DBG_error,
	...
	dev->model->cmd_set->set_motor_power(dev->calib_reg, FALSE);
	dev->model->cmd_set->set_lamp_power(dev, dev->calib_reg, TRUE);
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("black_shading.pnm",
					     calibration_data, 16, channels,
					     pixels_per_line,
...
genesys_dummy_dark_shading(Genesys_Device * dev)
{
	...
	for (x = skip + 1; x <= xend; x++) {
		...
		if (channels > 1) {
			dummy2 +=
				(dev->
				 white_average_data[channels * 2 * x + 2] +
				 256 * dev->white_average_data[channels * 2 *
							       x + 3]);
			dummy3 +=
				(dev->
				 white_average_data[channels * 2 * x + 4] +
				 256 * dev->white_average_data[channels * 2 *
							       x + 5]);
		}
	}
	...
	for (x = 0; x < pixels_per_line; x++) {
		...
		if (channels > 1) {
			dev->dark_average_data[channels * 2 * x + 2] =
				dummy2 & 0xff;
			dev->dark_average_data[channels * 2 * x + 3] =
				dummy2 >> 8;
			dev->dark_average_data[channels * 2 * x + 4] =
				dummy3 & 0xff;
			dev->dark_average_data[channels * 2 * x + 5] =
				dummy3 >> 8;
		}
	}
...
genesys_white_shading_calibration(Genesys_Device * dev)
{
	...
	dev->model->cmd_set->set_lamp_power(dev, dev->calib_reg, TRUE);
	dev->model->cmd_set->set_motor_power(dev->calib_reg, TRUE);
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("white_shading.pnm",
					     calibration_data, 16, channels,
					     pixels_per_line,
					     dev->model->shading_lines);
...
genesys_dark_white_shading_calibration(Genesys_Device * dev)
{
	...
	unsigned int x;
	int y;
	u_int32_t dark, white, dark_sum, white_sum, dark_count, white_count,
		col, dif;


	DBG(DBG_proc,
	    "genesys_black_white_shading_calibration (lines = %d)\n",
	    dev->model->shading_lines);
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("black_white_shading.pnm",
					     calibration_data, 16, channels,
					     pixels_per_line,
					     dev->model->shading_lines);
...
genesys_send_shading_coefficient(Genesys_Device * dev)
{
	...
	case CCD_5345:
		...
		for (x = 2; x < pixels_per_line; x++) {
			...
			val += 256 * dev->white_average_data[(x - 2) * 2 *
							     channels + 1] +
				dev->white_average_data[(x - 2) * 2 *
							channels];
			val -= (256 *
				dev->dark_average_data[(x - 2) * 2 *
						       channels + 1] +
				dev->dark_average_data[(x - 2) * 2 *
						       channels]);

			val += 256 * dev->white_average_data[(x - 1) * 2 *
							     channels + 1] +
				dev->white_average_data[(x - 1) * 2 *
							channels];
			val -= (256 *
				dev->dark_average_data[(x - 1) * 2 *
						       channels + 1] +
				dev->dark_average_data[(x - 1) * 2 *
						       channels]);

			val += 256 * dev->white_average_data[(x) * 2 *
							     channels + 1] +
			...
			if (channels > 1) {
				...
				val -= (256 *
					...
					dev->dark_average_data[(x - 2) * 2 *
							       channels + 2]);
				val += 256 * dev->white_average_data[(x - 1) *
								     2 *
								     channels
								     + 3] +
					dev->white_average_data[(x - 1) * 2 *
		...
		for (x = 0; x < pixels_per_line - o; x++) {
			...
			for (j = 0; j < channels; j++) {
				...
				if (br * target_dark > dk * target_bright)
					val = 0;
				else if (dk * target_bright -
					 br * target_dark >
					 65535 * (target_bright -
						  target_dark))
					val = 65535;
				else
					val = (dk * target_bright -
					       br * target_dark) /
...
/*fill all pixels, even if only the first one is relevant*/
				...
				if (65535 * val >
				    (target_bright - target_dark) * coeff)
					val = (coeff *
					       (target_bright -
						target_dark)) / val;
				else
...
*/
	...
	default:
		...
		for (x = 6; x < pixels_per_line; x++) {
			...
			val = 0;
			for (i = 0; i < 7; i++) {
				val += dev->dark_average_data[(x - i) * 2 *
							      channels];
				val += 256 * dev->dark_average_data[(x - i) *
								    2 *
								    channels +
								    1];
			}
			...
			/* average on 7 rows */
			for (i = 0; i < 7; i++) {
				val += 256 * dev->white_average_data[(x - i) *
								     2 *
								     channels
								     + 1] +
					dev->white_average_data[(x - i) * 2 *
								channels];
				val -= (256 *
					dev->dark_average_data[(x - i) * 2 *
							       channels + 1] +
					dev->dark_average_data[(x - i) * 2 *
							       channels]);
			}
...
genesys_flatbed_calibration(Genesys_Device * dev)
{
	...
	if (dev->model->flags & GENESYS_FLAG_OFFSET_CALIBRATION) {
		...
		status = dev->model->cmd_set->coarse_gain_calibration(dev,
								      yres);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		/* since we have 2 gain calibration proc, skip second if first one was
		   used. */
	{
		status = dev->model->cmd_set->
			init_regs_for_coarse_calibration(dev);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
...
/*calibrate afe again to match new exposure*/
		if (dev->model->flags & GENESYS_FLAG_OFFSET_CALIBRATION) {
			...
			status = dev->model->cmd_set->
				coarse_gain_calibration(dev, yres);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_error,
				    "genesys_flatbed_calibration: coarse gain calibration: %s\n",
				    sane_strstatus(status));
...
/*y_size * xres? y_size hopefully in inches, then*/
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "genesys_flatbed_calibration: failed to send shading "
		    "registers: %s\n", sane_strstatus(status));
		return status;
	}
...
genesys_warmup_lamp(Genesys_Device * dev)
{
	...
	do {
		...
		RIE(dev->model->cmd_set->
		    begin_scan(dev, local_reg, TRUE));
		do {
		...
		status = sanei_genesys_read_data_from_scanner(dev, first_line,
							      total_size);
		if (status != SANE_STATUS_GOOD) {
		...
		RIE(dev->model->cmd_set->
		    end_scan(dev, local_reg, FALSE));

		sleep(1);	/* sleep 1 s */
		seconds++;

		RIE(dev->model->cmd_set->
		    begin_scan(dev, local_reg, FALSE));
		do {
			sanei_genesys_test_buffer_empty(dev, &empty);
		}
		while (empty);
		...
		for (pixel = 0; pixel < total_size; pixel++) {
			if (dev->model->cmd_set->get_bitset_bit(local_reg)) {
				first_average +=
					(first_line[pixel] +
					 first_line[pixel + 1] * 256);
				second_average +=
					(second_line[pixel] +
					 second_line[pixel + 1] * 256);
				pixel++;
			} else {
		...
		if (dev->model->cmd_set->get_bitset_bit(local_reg)) {
			...
			second_average /= pixel;
			if (DBG_LEVEL >= DBG_data) {
				sanei_genesys_write_pnm_file("warmup1.pnm",
							     first_line, 8,
							     channels,
							     total_size / 2,
							     2);
				sanei_genesys_write_pnm_file("warmup2.pnm",
							     second_line, 8,
							     channels,
							     total_size / 2,
							     2);
...
/* disable power saving*/
	status = dev->model->cmd_set->save_power(dev, FALSE);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
	...
	if ((dev->model->flags & GENESYS_FLAG_SEARCH_START)
		...
		if (dev->model->flags & GENESYS_FLAG_USE_PARK)
			status = dev->model->cmd_set->park_head(dev, dev->reg,
								1);
		else
	...
	status = dev->model->cmd_set->bulk_write_register(dev, dev->reg,
							  dev->model->
							  cmd_set->
							  bulk_full_size());
	if (status != SANE_STATUS_GOOD) {
	...
	status = dev->model->cmd_set->begin_scan(dev, dev->reg, TRUE);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
...
genesys_fill_read_buffer(Genesys_Device * dev)
{
	...
	work_buffer_dst =
		sanei_genesys_buffer_get_write_pos(&(dev->read_buffer),
						   space);
...
/* size is already maxed to our needs. bulk_read_data
   will read as much data as requested. */
	status = dev->model->cmd_set->bulk_read_data(dev, 0x45,
			  ...
			  size_t * len)
{
	...
	if (dev->read_active != TRUE) {
		DBG(DBG_error,
		    "genesys_read_ordered_data: read not active!\n");
		*len = 0;
		return SANE_STATUS_INVAL;
	}
...
/*not implemented for depth == 1.*/
		...
		work_buffer_src =
			sanei_genesys_buffer_get_read_pos(src_buffer);
...
/*we are greedy. we work as much as possible*/
		...
		DBG(DBG_info,
		    "genesys_read_ordered_data: reordering %d lines\n",
		    dst_lines);
		...
		if (dst_lines != 0) {
			...
			if (channels == 3) {
				...
				case 1:	/* RGB, chunky, 16 bit */
					...
					status = genesys_reorder_components_endian_16(work_buffer_src, work_buffer_dst, dst_lines, src_pixels, 3);
					break;
				...
				case 2:	/* RGB, cis, 8 bit */
					status = genesys_reorder_components_cis_8(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				case 3:	/* RGB, cis, 16 bit */
					status = genesys_reorder_components_cis_16(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				case 4:	/* BGR, chunky, 8 bit */
					status = genesys_reorder_components_bgr_8(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				case 5:	/* BGR, chunky, 16 bit */
					status = genesys_reorder_components_bgr_16(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				case 6:	/* BGR, cis, 8 bit */
					status = genesys_reorder_components_cis_bgr_8(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				case 7:	/* BGR, cis, 16 bit */
					status = genesys_reorder_components_cis_bgr_16(work_buffer_src, work_buffer_dst, dst_lines, src_pixels);
					break;
				}
			} else {
#ifdef WORDS_BIGENDIAN
				if (depth == 16) {
					status = genesys_reorder_components_endian_16(work_buffer_src, work_buffer_dst, dst_lines, src_pixels, 1);
				} else {
					status = SANE_STATUS_GOOD;
				}
...
/*should not happen with depth == 1.*/
		if (depth == 1) {
			DBG(DBG_error,
			    "Can't reverse ccd for single bit data\n");
			return SANE_STATUS_INVAL;
		}
		...
		extra = (dev->current_setup.max_shift * src_pixels *
			 channels * depth) / 8;
...
/*we are greedy. we work as much as possible*/
		...
		DBG(DBG_info,
		    "genesys_read_ordered_data: un-ccd-ing %d lines\n",
		    dst_lines);
...
/*we are greedy. we work as much as possible*/
		...
		if (dst_lines >
		    (bytes * 8) / (dev->settings.pixels * channels * depth))
			dst_lines =
				(bytes * 8) / (dev->settings.pixels *
					       channels * depth);
		...
		DBG(DBG_info,
		    "genesys_read_ordered_data: shrinking %d lines\n",
		    dst_lines);

		if (dst_lines != 0) {

			if (depth == 1)
				status = genesys_shrink_lines_1
					(work_buffer_src, work_buffer_dst,
					 dst_lines, src_pixels,
					 dev->settings.pixels, channels);
			else if (depth == 8)
				status = genesys_shrink_lines_8
					(work_buffer_src, work_buffer_dst,
					 dst_lines, src_pixels,
					 dev->settings.pixels, channels);
			else
				status = genesys_shrink_lines_16
					(work_buffer_src, work_buffer_dst,
					 dst_lines, src_pixels,
					 dev->settings.pixels, channels);
...
/*we just consumed this many bytes*/
			bytes = (dst_lines * src_pixels * channels * depth) /
				8;
			RIE(sanei_genesys_buffer_consume(src_buffer, bytes));
...
/* move data to destination */
	...
	if (needs_reverse) {
		status = genesys_reverse_bits(work_buffer_src, destination,
					      bytes);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		*len = bytes;
		...
		if (depth != 8) {
			DBG(DBG_error,
			    "Cannot convert from 16bit to lineart\n");
			return SANE_STATUS_INVAL;
		}
/*lines in input*/
		...
		*len = dst_lines * channels *
			(dev->settings.pixels / 8 +
			 ((dev->settings.pixels % 8) ? 1 : 0));
	} else {
	...
	DBG(DBG_proc,
	    "genesys_read_ordered_data: completed, %lu bytes read\n",
	    (u_long) bytes);
	return SANE_STATUS_GOOD;
}
...
calc_parameters(Genesys_Scanner * s)
{
	...
	s->dev->settings.disable_interpolation =
		s->val[OPT_DISABLE_INTERPOLATION].w == TRUE;
	...
	/* threshold setting */
	s->dev->settings.threshold =
		2.55 * (SANE_UNFIX(s->val[OPT_THRESHOLD].w));
...
static SANE_Status
create_bpp_list(Genesys_Scanner * s, int * bpp)
{
	int count;
...
init_options(Genesys_Scanner * s)
{
	...
	s->opt[OPT_BIT_DEPTH].size = sizeof(int);
	...
	dpi_list = malloc((count + 1) * sizeof(int));
	if (!dpi_list)
		return SANE_STATUS_NO_MEM;
	dpi_list[0] = count;
	for (count = 0; model->ydpi_values[count] != 0; count++)
	...
	s->opt[OPT_DISABLE_INTERPOLATION].constraint_type =
		SANE_CONSTRAINT_NONE;
	s->val[OPT_DISABLE_INTERPOLATION].w = FALSE;
	...
	s->opt[OPT_COLOR_FILTER].constraint_type =
...
attach(SANE_String_Const devname, Genesys_Device ** devp, SANE_Bool may_wait)
{
	...
	for (dev = first_dev; dev; dev = dev->next) {
		if (strcmp(dev->file_name, devname) == 0) {
			if (devp)
				*devp = dev;
			DBG(DBG_info,
			    "attach: device `%s' was already in device list\n",
			    devname);
			return SANE_STATUS_GOOD;
		}
	}
	...
	for (i = 0; i < MAX_SCANNERS && genesys_usb_device_list[i].model != 0;
	     i++) {
...
attach_one_device(SANE_String_Const devname)
{
	...
	if (dev) {
		...
		if (new_dev_len >= new_dev_alloced) {
			...
			if (!new_dev) {
				DBG(DBG_error,
				    "attach_one_device: out of memory\n");
				return SANE_STATUS_NO_MEM;
			}
		}
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (!fp) {
		...
		DBG(DBG_warn,
		    "sane_init: couldn't open config file `%s': %s. Using "
		    "/dev/usb/scanner directly\n", GENESYS_CONFIG_FILE,
		    strerror(errno));
		attach("/dev/usb/scanner", 0, FALSE);
		return SANE_STATUS_GOOD;
	}
...
sane_open(SANE_String_Const devicename, SANE_Handle * handle)
{
	...
	if (devicename[0]) {
		...
		if (!dev) {
			DBG(DBG_info,
			    ...
			    devicename);
			RIE(attach(devicename, &dev, TRUE));
		} else
		...
		if (dev) {
			...
			DBG(DBG_info,
			    "sane_open: empty devicename, trying `%s'\n",
			    devicename);
		}
	}
	...
	if (dev->model->flags & GENESYS_FLAG_UNTESTED) {
		...
		DBG(DBG_error0,
		    "         scanner and what does (not) work.\n");
	}
	...
	*handle = s;
	...
	if (genesys_init_cmd_set(s->dev) != SANE_STATUS_GOOD) {
		DBG(DBG_error0,
		    "This device doesn't have a valid command set!!\n");
		return SANE_STATUS_IO_ERROR;
	}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (s->scanning) {
		DBG(DBG_warn,
		    "sane_control_option: don't call this function while "
		    "scanning (option = %s (%d))\n", s->opt[option].name,
		    option);
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_PREVIEW:
			...
			break;
		case OPT_SOURCE:
			if (strcmp(s->val[option].s, val) != 0) {	/* something changed */
				if (s->val[option].s)
					free(s->val[option].s);
				s->val[option].s = strdup(val);
				if (strcmp
				    (s->val[option].s,
				     "Transparency Adapter") == 0) {
		...
		case OPT_COLOR_FILTER:
			...
			break;
		case OPT_LAMP_OFF_TIME:
			if (*(int *) val != s->val[option].w) {
				s->val[option].w = *(int *) val;
				RIE(s->dev->model->cmd_set->
				    set_powersaving(s->dev,
						    s->val[option].w));
			}
			break;
			/* string options: */
	  ...
	  int * len)
{
	...
	*len = 0;
	...
	if (!s->scanning) {
		DBG(DBG_warn,
		    "sane_read: scan was cancelled, is over or has not been "
		    "initiated yet\n");
...
sane_cancel(SANE_Handle handle)
{
	...
	status = s->dev->model->cmd_set->end_scan(s->dev, s->dev->reg,
						  TRUE);
	if (status != SANE_STATUS_GOOD) {
	...
	if (s->dev->model->flags & GENESYS_FLAG_USE_PARK)
		status = s->dev->model->cmd_set->park_head(s->dev,
							   s->dev->reg, 1);
	else
...
/*enable power saving mode*/
	status = s->dev->model->cmd_set->save_power(s->dev, TRUE);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
...
sane_set_io_mode(SANE_Handle handle, SANE_Bool non_blocking)
{
	...
	DBG(DBG_proc, "sane_set_io_mode: handle = %p, non_blocking = %s\n",
	    handle, non_blocking == TRUE ? "true" : "false");
