...
static TState
ReadNextColorLine(PTInstance this)
{
	int iWrite, i;
	...
	do {
		...
		if (this->state.iLine > 2 * this->state.ySensorSkew) {
			bVisible = true;
			iOffsetR =
				(this->state.szOrder[0] -
				 '0') * this->state.cxMax;
			iOffsetG =
				(this->state.szOrder[1] -
				 '0') * this->state.cxMax;
			iOffsetB =
				(this->state.szOrder[2] -
				 '0') * this->state.cxMax;
			for (nInterpolator = 100, iWrite = 0, iRead = 0;
			     iRead < 3 * this->state.cxMax
			     && iWrite < this->state.cchLineOut; iRead++) {
		...
		for (i = this->state.cBacklog - 2; i >= 0; i--)
			this->state.ppchLines[i + 1] =
				this->state.ppchLines[i];
...
StartScanColor(TInstance * this)
{
	...
	case 200:
		this->state.ySensorSkew = 1;
		break;
	case 300:
		this->state.ySensorSkew = 2;
		break;
	case 600:
		this->state.ySensorSkew = 4;
		break;
	default:
		break;
	}
	 ...
	 */
	{
		unsigned char uchRegs[] = {
			0xFC /*!!R_SPOS!! */ , 0x00 /*R_SPOSH */ ,
			0x24 /*!!0x03!! */ ,
			0xB0 /*!!R_SWID!! */ , 0xC4 /*!!R_SWIDH!! */ ,
			1, 0,
			0xFF /*!!0x08!! */ , 0xFF /*!!0x09!! */ ,
			0x22 /*!!R_LEN!! */ , 0x07 /*!!R_LENH!! */ ,
			0x6D /*0x0C */ ,
		...
		RegWrite(this, R_SLEN, 2,
			 this->state.cyWindow +
			 (2 * this->state.ySensorSkew) * 600 /
			 this->param.res);
		this->state.szOrder = ORDER_BRG;
		RegWrite(this, R_CCAL, 3, this->calibration.rgbBias);
		INST_ASSERT();	/* 0xBBGGRR */
		switch (this->param.res) {
		case 75:
			RegWrite(this, R_XRES, 1, 0x20);	/* ups, can  do only 100 dpi horizontal */
			RegWrite(this, R_SWID, 2,
				 0xC000 | this->state.cxWindow);
