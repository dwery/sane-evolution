...
/* 
   $Id: sceptre.c,v 1.13 2005-09-19 19:57:48 fzago-guest Exp $
...
static const int resolutions_list[NUM_OF_RES + 1] = {
	NUM_OF_RES, 10, 25, 30, 45, 75, 90, 150, 300, 450, 600, 750, 900,
	1050,
	1125, 1200
};

static const int color_shift_list[NUM_OF_RES + 1] = {
...
sceptre_identify_scanner(Sceptre_Scanner * dev)
{
	...
	for (i = 0; i < NELEMS(scanners); i++) {
		    ...
		    strcmp(dev->scsi_product, scanners[i].scsi_product) == 0)
		{
			...
			DBG(DBG_error,
			    "sceptre_identify_scanner: scanner supported\n");
...
sceptre_get_status(Sceptre_Scanner * dev, size_t * data_left)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sceptre_get_status: cannot get buffer status\n");
		*data_left = 0;
	...
	for (raster = 0; raster < nb_rasters; raster++) {
		...
		if (dev->raster_num < dev->color_shift) {
			...
			if (colour) {
			   ...
			   dev->raster_real - dev->color_shift) {
			/* Blue */
			colour = 2;
			line = dev->line;
		} else if (dev->raster_num >=
			   dev->raster_real - 3 * dev->color_shift) {
			/* Green or Blue */
			colour = (dev->raster_real - dev->raster_num -
				  dev->color_shift) % 2 + 1;
			if (colour == 1) {
				/* Green */
...
static SANE_Status
sceptre_sense_handler(int scsi_fd, unsigned char __sane_unused__ * result,
...
attach_scanner(const char *devicename, Sceptre_Scanner ** devp)
{
	...
	if (sanei_scsi_open(devicename, &sfd, sceptre_sense_handler, dev) !=
	    0) {
	...
	if (sceptre_identify_scanner(dev) == FALSE) {
		DBG(DBG_error,
...
sceptre_fill_image(Sceptre_Scanner * dev)
{
	...
	while (dev->real_bytes_left) {
		...
		if ((status =
		     sceptre_get_status(dev,
					&data_left)) != SANE_STATUS_GOOD) {
			return (status);
		}
		...
		DBG(DBG_info,
		    "sceptre_fill_image: to read   = %ld bytes (bpl=%d)\n",
		    (long) size, dev->params.bytes_per_line);

		MKSCSI_READ_10(cdb, 0, 0, size);

		hexdump(DBG_info2, "sceptre_fill_image: READ_10 CDB",
			cdb.data, 10);
...
/* Start a scan. */
static const int gamma_init[GAMMA_LENGTH] = {
	
};
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_MODE:
			...
			dev->opt[OPT_GAMMA_VECTOR_G].cap |= SANE_CAP_INACTIVE;
			dev->opt[OPT_GAMMA_VECTOR_B].cap |= SANE_CAP_INACTIVE;
			dev->opt[OPT_HALFTONE_PATTERN].cap |=
				SANE_CAP_INACTIVE;
			...
			if (strcmp(dev->val[OPT_MODE].s, LINEART_STR) == 0) {
				...
				dev->opt[OPT_THRESHOLD].cap &=
					~SANE_CAP_INACTIVE;
			} else if (strcmp(dev->val[OPT_MODE].s, HALFTONE_STR)
				   == 0) {
				dev->scan_mode = SCEPTRE_HALFTONE;
				dev->opt[OPT_HALFTONE_PATTERN].cap &=
					~SANE_CAP_INACTIVE;
			} else if (strcmp(dev->val[OPT_MODE].s, GRAY_STR) ==
				   0) {
				dev->scan_mode = SCEPTRE_GRAYSCALE;
			} else if (strcmp(dev->val[OPT_MODE].s, COLOR_STR) ==
				   0) {
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!(dev->scanning)) {
		...
		case SCEPTRE_HALFTONE:
			...
			dev->params.lines =
				((dev->length * dev->resolution) / 600);
			if ((dev->params.lines) * 600 !=
			    (dev->length * dev->resolution)) {
				/* Round up lines to 2. */
				dev->params.lines &= ~1;
				dev->params.lines += 2;
			}
		...
		case SCEPTRE_COLOR:
			...
			if (dev->params.pixels_per_line & 1) {
				if ((dev->params.pixels_per_line * 600) ==
				    (dev->width * x_dpi)) {
					/* 2n */
					dev->params.pixels_per_line--;
				} else {
		...
		dev->bytes_left =
			dev->params.lines * dev->params.bytes_per_line;
	}
...
sane_start(SANE_Handle handle)
{
	...
	if (!(dev->scanning)) {
		...
		if (sanei_scsi_open
		    (dev->devicename, &(dev->sfd), sceptre_sense_handler,
		     dev) != 0) {
