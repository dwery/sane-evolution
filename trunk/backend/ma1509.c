...
#define MA1509_READ_LIMIT (1024 * 256)
	...
	if (status != SANE_STATUS_GOOD || size != MA1509_COMMAND_LENGTH) {
		DBG(5,
		    ...
		    sane_strstatus(status), (long int) size,
	...
	if (cmd[1] == 1) {
		...
		if (data && data_size && *data_size) {
			...
			DBG(5,
			    "ma1509_cmd: trying to receive %ld bytes of data\n",
			    (long int) *data_size);
			...
			while (status == SANE_STATUS_GOOD && bytes_left > 0) {
				...
				if (status != SANE_STATUS_GOOD) {
					DBG(1,
					    "ma1509_cmd: sanei_usb_read_bulk returned %s\n",
					    sane_strstatus(status));
					return status;
				}
				bytes_left -= size;
				DBG(5,
				    "ma1509_cmd: read %ld bytes, %ld bytes to go\n",
		...
		if (data && data_size && *data_size) {
			...
			while (status == SANE_STATUS_GOOD && bytes_left > 0) {
				...
				if (status != SANE_STATUS_GOOD) {
					DBG(1,
					    "ma1509_cmd: sanei_usb_write_bulk returned %s\n",
					    sane_strstatus(status));
					return status;
				}
				bytes_left -= size;
				DBG(5,
				    "ma1509_cmd: wrote %ld bytes, %ld bytes to go\n",
				    (long int) size, (long int) bytes_left);
			}
...
attach(SANE_String_Const devname, Ma1509_Device ** devp)
{
	...
	if (status == SANE_STATUS_UNSUPPORTED) {
		DBG(3,
		    "attach: can't detect vendor/product, trying anyway\n");
...
set_window(Ma1509_Scanner * s)
{
	...
	DBG(4,
	    "set_window: tlx=%d (%d mm); tly=%d (%d mm); width=%d (%d mm); "
...
calibration(Ma1509_Scanner * s)
{
	...
	for (color = 1; color < 4; color++) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "calibration: ma1509_cmd send data failed: %s\n",
			    sane_strstatus(status));
			free(buffer);
...
send_gamma(Ma1509_Scanner * s)
{
	...
	if (!buffer) {
		DBG(1,
		    "send_gamma: couldn't malloc %lu bytes for gamma  buffer\n",
		    (u_long) total_size);
		return SANE_STATUS_NO_MEM;
	}
	...
	for (color = 1; color < 4; color++) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "send_gamma: ma1509_cmd send data failed: %s\n",
			    sane_strstatus(status));
			free(buffer);
			return status;
		}
...
static SANE_Status
read_data(Ma1509_Scanner * s, SANE_Byte * buffer, int * size)
{
	...
	while (sanei_config_read(line, sizeof(line), fp)) {
		...
		if (strcmp(word, "option") == 0) {
			...
			if (strcmp(word, "warmup-time") == 0) {
				...
				if (end == word) {
					DBG(3,
					    "sane-init: config file line %d: warmup-time must "
					    "have a parameter; using default (%d)\n",
					    linenumber, warmup_time);
				} else if (errno) {
					DBG(3,
					    "sane-init: config file line %d: warmup-time `%s' "
	  ...
	  int * len)
{
	...
	*len = max_len;
	...
	DBG(4, "sane_read: read %d/%d bytes (%d bytes to go, %d total)\n",
	    *len, max_len, total_size - s->read_bytes, total_size);
