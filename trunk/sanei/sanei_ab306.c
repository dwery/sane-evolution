...
ab306_write(Port * p, const void *buf, size_t len)
{
	...
	if (cksum != 0xa5) {
		DBG(0,
		    "ab306_write: checksum error (%02x!=a5) when sending command!\n",
		    cksum);
...
sanei_ab306_open(const char *dev, int *fdp)
{
	...
	if (ioperm(AB306_CIO, 1, 1) != 0) {
		...
		if (port[i].port_fd < 0)
			return SANE_STATUS_IO_ERROR;
		for (j = 0; j < NELEMS(wakeup); ++j) {
			if (lseek(port[i].port_fd, AB306_CIO, SEEK_SET) !=
			    AB306_CIO)
				return SANE_STATUS_IO_ERROR;
			byte = wakeup[j];
			if (j == NELEMS(wakeup) - 1)
		...
		void *dst, size_t * dst_size)
{
	...
	if (dst && *dst_size > 0) {
		...
		DBG(3,
		    "sanei_ab306_cmd: waiting for scanner to be NOT ready %02x\n",
		...
		if ((cksum & 0xff) != 0) {
			DBG(0,
			    "sanei_ab306_cmd: checksum error (%2x!=0) when receiving "
			    "after command!\n", cksum);
			return SANE_STATUS_IO_ERROR;
		}
