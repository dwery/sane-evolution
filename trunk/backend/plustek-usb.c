 ...
 */
static SANE_Bool
usb_normFileName(char *fname, char *buffer, u_long max_len)
{
	char *src, *dst;
 ...
 */
static void
usb_CheckAndCopyAdjs(Plustek_Device * dev)
{
...
usb_initDev(Plustek_Device * dev, int idx, int handle, int vendor)
{
	...
	if (dev->adj.tpaShadingY >= 0)
		dev->usbDev.Caps.Positive.ShadingOriginY =
			dev->adj.tpaShadingY;

	if (dev->adj.negShadingY >= 0)
		dev->usbDev.Caps.Negative.ShadingOriginY =
			dev->adj.negShadingY;
	...
	for (i = 0; usbVendors[i].desc != NULL; i++) {
		...
		if (usbVendors[i].id == vendor) {
			...
			if (dev->usbDev.Caps.
			    workaroundFlag & _WAF_USE_ALT_DESC)
				if (usbVendors[i].desc_alt)
					dev->sane.vendor =
						usbVendors[i].desc_alt;
	...
	sprintf(tmp_str2, "%s-%s", dev->sane.vendor, dev->usbDev.ModelStr);
 ...
 */
static int
usb_CheckForPlustekDevice(int handle, Plustek_Device * dev)
{
	char tmp[50];
	char pcbStr[10];
	...
	/* get the PCB-ID */
	result = sanei_lm983x_read(handle, 0x59, reg59s, 3, TRUE);
	...
	result = sanei_lm983x_write(handle, 0x59, reg59, 3, TRUE);
	if (SANE_STATUS_GOOD != result) {
		sanei_usb_close(handle);
		return -1;
	}

	result = sanei_lm983x_read(handle, 0x02, &pcbID, 1, TRUE);
	if (SANE_STATUS_GOOD != result) {
		sanei_usb_close(handle);
		return -1;
	}
 ...
 */
static void
usbDev_shutdown(Plustek_Device * dev)
{
	int handle;
 ...
 */
static SANE_Bool
usb_IsDeviceInList(char *usbIdStr)
{
	int i;
 ...
 */
static int
usbDev_open(Plustek_Device * dev, DevList * devs, int keep_lock)
{
	char dn[512];
	char devStr[50];
	int result;
	...
	if (devs) {
		...
		if (!strcmp(dev->name, "auto")) {
			...
			for (tmp = devs; tmp; tmp = tmp->next) {
				if (tmp->vendor_id == vendor
				    && tmp->device_id == product) {
					if (!tmp->attached) {
						tmp->attached = TRUE;
						strcpy(dn, tmp->dev_name);
						break;
	...
	if (SANE_STATUS_GOOD == result) {
		...
		if (dev->usbId[0] != '\0') {
			...
			if (0 != strcmp(dev->usbId, devStr)) {
				DBG(_DBG_ERROR,
				    "Specified Vendor and Product ID "
		...
		DBG(_DBG_INFO,
		    "Can't get vendor & product ID from driver...\n");
		...
		if (dev->usbId[0] == '\0') {
			DBG(_DBG_ERROR,
			    "Cannot autodetect Vendor an Product ID, "
	...
	if ((version < 3) || (version > 4)) {
		DBG(_DBG_ERROR,
		    "This is not a LM9831 or LM9832 chip based scanner.\n");
		sanei_usb_close(handle);
		sanei_access_unlock(dev->sane.name);
		return -1;
	}
	...
	if (0x07B3 == vendor) {
		...
		for (i = 0; NULL != Settings[i].pIDString; i++) {
			...
			if (0 == strncmp(Settings[i].pIDString, devStr, lc)) {
				DBG(_DBG_INFO,
				    "Device description for >%s< found.\n",
				    devStr);
 ...
 */
static int
usbDev_close(Plustek_Device * dev)
{
 ...
 */
static int
usbDev_getCaps(Plustek_Device * dev)
{
 ...
 */
static int
usbDev_getCropInfo(Plustek_Device * dev, CropInfo * ci)
{
	WinInfo size;
	...
	if (ci->ImgDef.dwFlag & SCANFLAG_DWORDBoundary)
		ci->dwBytesPerLine =
			(ci->dwBytesPerLine + 3UL) & 0xfffffffcUL;
 ...
 */
static int
usbDev_setMap(Plustek_Device * dev, int * map,
...
usbDev_setScanEnv(Plustek_Device * dev, ScanInfo * si)
{
	...
	if ((si->ImgDef.dwFlag & SCANFLAG_Calibration) == 0) {
		...
		if (si->ImgDef.wDataType == COLOR_256GRAY) {
			...
			if (!(si->ImgDef.dwFlag & SCANDEF_Adf)
			    && !usb_IsCISDevice(dev)
			    && (caps->OpticDpi.x == 1200
				&& si->ImgDef.xyDpi.x <= 300)) {
	 ...
	 */
	usb_AdjustCISLampSettings(dev, TRUE);
	...
	if (scan->sParam.bSource == SOURCE_ADF) {
		...
		if (scan->dwFlag & SCANDEF_ContinuousScan)
			dev->usbDev.fLastScanIsAdf = TRUE;
		else
			dev->usbDev.fLastScanIsAdf = FALSE;
	}
...
/* HEINER: Preview currently not working correctly */
	...
	if ((scan->dwFlag & SCANDEF_Adf)
	    && (scan->dwFlag & SCANDEF_ContinuousScan)) {
		scan->fCalibrated = TRUE;
	} else {
		scan->fCalibrated = FALSE;
	}
	...
	if (dev->usbDev.Caps.workaroundFlag & _WAF_LOFF_ON_START) {
		if (usb_GetLampStatus(dev))
			usb_LampOn(dev, FALSE, TRUE);
	}

	usb_LampOn(dev, TRUE, TRUE);

	m_fStart = m_fFirst = TRUE;
	m_fAutoPark =
		(scan->
		 dwFlag & SCANFLAG_StillModule) ? FALSE : TRUE;
...
usbDev_Prepare(Plustek_Device * dev, SANE_Byte * buf)
{
	...
	if (!(scan->dwFlag & SCANFLAG_Scanning)) {
		...
		if (scaps->workaroundFlag & _WAF_BYPASS_CALIBRATION) {
			if (!usb_Wait4Warmup(dev)) {
				DBG(_DBG_INFO,
				    "usbDev_Prepare() - Cancel detected...\n");
				return 0;
			}
		}
		...
		scan->dwNumberOfScanBufs =
			_SCANBUF_SIZE / scan->dwBytesScanBuf;
		scan->dwLinesPerScanBufs =
			scan->dwNumberOfScanBufs * scan->dwLinesScanBuf;
		scan->pbScanBufEnd =
			scan->pbScanBufBegin +
			scan->dwLinesPerScanBufs *
			scan->sParam.Size.dwPhyBytes;
		...
		if (scan->sParam.bChannels == 3) {
			...
			if ((scan->sParam.bDataType == SCANDATATYPE_Color) &&
				...
				if (_WAF_RESET_SO_TO_RGB & scaps->
				    workaroundFlag) {
					if (scaps->bPCB != 0) {
						if (scan->sParam.PhyDpi.x >
						    scaps->bPCB) {
							so = SENSORORDER_rgb;
							DBG(_DBG_INFO,
							    "* Resetting sensororder to RGB\n");
						}
					}
				}
		...
		if (scan->sParam.UserDpi.y != scan->sParam.PhyDpi.y) {
			...
			if (scan->sParam.UserDpi.y < scan->sParam.PhyDpi.y) {
				...
				DBG(_DBG_INFO,
				    "SampleY Flag set (%u != %u, wSumY=%u)\n",
				    scan->sParam.UserDpi.y,
				    scan->sParam.PhyDpi.y, scan->wSumY);
			}
		}
	}
	...
	if (scan->dwFlag & SCANFLAG_BottomUp)
		scan->UserBuf.pb =
			buf + (scan->dwLinesUser - 1) * scan->dwBytesLine;
	else
		scan->UserBuf.pb = buf;
