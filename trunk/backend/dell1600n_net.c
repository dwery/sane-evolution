...
static void HexDump(int debugLevel, const unsigned char *buf, size_t bufSize);
...
*/
static struct DeviceRecord *ProcessFindResponse(unsigned char *pData,
						size_t size);
		 ...
		 SANE_Bool __sane_unused__ local_only)
{
	...
	if (fConfig) {
		while (!feof(fConfig)) {
			if (!sanei_config_read
			    (configBuf, sizeof(configBuf), fConfig))
				break;
			...
			if (!strncmp(pVal, "extra_scanner:", valLen)) {
				...
				pVal = sanei_config_skip_whitespace(pVal +
								    valLen);

				pDevice = malloc(sizeof(struct DeviceRecord));
				if (!pDevice) {
					DBG(1,
					    "sane_get_devices: memory allocation failure\n");
					break;
				}
	...
	while (select(sock + 1, &readFds, NULL, NULL, &selTimeVal)) {
		...
		if (iNextDevice >= MAX_SCANNERS) {
			DBG(1,
			    "sane_get_devices: more than %d devices, ignoring\n",
			    MAX_SCANNERS);
			break;
		}
...
sane_open(const char * devicename, SANE_Handle * handle)
{
	...
	DBG(5, "sane_open: connected to %s:%d as %s\n", devicename,
	    gScannerPort, gOpenScanners[iHandle]->m_regName);
...
SANE_Status
sane_control_option(SANE_Handle __sane_unused__ handle, int option,
		    SANE_Action action, void *value,
		    int __sane_unused__ * info)
{
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	/* fetch page info */
	memcpy(&pageInfo, gOpenScanners[iHandle]->m_pageInfo.m_pBuf,
	       sizeof(pageInfo));
	...
	DBG(5,
	    "sane_get_parameters: bytes remaining on this page: %d, num pages: %d, size: %dx%d\n",
	    pageInfo.m_bytesRemaining, gOpenScanners[iHandle]->m_numPages,
	    width, height);
...
sane_start(SANE_Handle handle)
{
	...
	if (gOpenScanners[iHandle]->m_imageData.m_used) {
		...
		/* remove empty page */
		PopFromComBuf(&gOpenScanners[iHandle]->m_pageInfo,
	...
	errorCheck |=
		AppendMessageToPacket(&buf, 0x22,
				      "std-scan-subscribe-user-name", 0x0b,
				      gOpenScanners[iHandle]->m_regName,
				      strlen(gOpenScanners[iHandle]->
					     m_regName));
	errorCheck |=
		AppendMessageToPacket(&buf, 0x22,
				      "std-scan-subscribe-ip-address", 0x0a,
				      &myAddr.sin_addr, 4);
	FinalisePacket(&buf);
	...
	while (!gOpenScanners[iHandle]->m_bFinish) {
		...
		/* read from socket */
		nread = read(gOpenScanners[iHandle]->m_udpFd, sockBuf,
			     sizeof(sockBuf));
		...
		if (ProcessUdpResponse
		    (sockBuf, nread, gOpenScanners[iHandle])) {
			status = SANE_STATUS_IO_ERROR;
			goto cleanup;
		}
	...
	if (gOpenScanners[iHandle]->m_bCancelled)
	  ...
	  int max_length, int * length)
{
	...
	*length = 0;
	...
	/* update counter */
	memcpy(gOpenScanners[iHandle]->m_pageInfo.m_pBuf, &pageInfo,
	       sizeof(pageInfo));
	...
	DBG(5,
	    ...
	    dataSize, gOpenScanners[iHandle]->m_bytesRead,
	    pageInfo.m_bytesRemaining,
	    gOpenScanners[iHandle]->m_imageData.m_used - dataSize,
	    pageInfo.m_width, pageInfo.m_height);
...
ClearKnownDevices()
{
	...
	for (i = 0; i < MAX_SCANNERS; ++i) {
		...
		if (gKnownDevices[i]) {
			if (gKnownDevices[i]->m_pName)
				free(gKnownDevices[i]->m_pName);
			if (gKnownDevices[i]->m_pModel)
				free(gKnownDevices[i]->m_pModel);
...
ProcessFindResponse(unsigned char *pData, size_t size)
{
	...
	while (pItem < pEnd) {
		...
		if (!strncmp("std-scan-discovery-ip", pName, nameSize)) {
			...
			snprintf(printerName, sizeof(printerName),
				 "%d.%d.%d.%d", (int) pValue[0],
				 (int) pValue[1], (int) pValue[2],
				 (int) pValue[3]);
			DBG(2, "%s\n", printerName);

		} else if (!strncmp
			   ("std-scan-discovery-model-name", pName,
			    nameSize)) {
...
ValidScannerNumber(int iHandle)
{
	...
	if ((iHandle < 0) || (iHandle >= MAX_SCANNERS)) {
		DBG(1, "ValidScannerNumber: invalid scanner index %d",
		    iHandle);
		return 0;
	}
		   ...
		   struct ScannerState *pState)
{
	...
	while (pItem < pEnd) {
		...
		/* process the item */
		if (!strncmp
		    ("std-scan-request-tcp-connection", pName, nameSize)) {
			...
			if (!
				...
				DBG(1,
				    "ProcessUdpResponse: error opening TCP socket\n");
				return 2;
			}
			...
			DBG(1,
			    "ProcessUdpResponse: opened TCP connection to scanner\n");
			...
			while (1) {
				...
				nread = read(pState->m_tcpFd, sockBuf,
					     sizeof(sockBuf));

				if (nread <= 0) {
					DBG(1,
					    "ProcessUdpResponse: TCP read returned %d\n",
					    nread);
					break;
				}
			...
			DBG(1,
			    "ProcessUdpResponse: closed TCP connection to scanner\n");
...
ProcessTcpResponse(struct ScannerState *pState, struct ComBuf *pTcpBuf)
{
	...
	if (!MessageIsComplete(pData, pTcpBuf->m_used)) {
		DBG(10,
		    "ProcessTcpResponse: incomplete message, returning\n");
		return 0;
	}
	...
	while (pItem < pEnd) {
		...
		if (!strncmp("std-scan-session-open", pName, nameSize)) {
			...
			send(pState->m_tcpFd, buf.m_pBuf, buf.m_used, 0);
			   ...
			   ("std-scan-getclientpref", pName, nameSize)) {
			...
			send(pState->m_tcpFd, buf.m_pBuf, buf.m_used, 0);
			   ...
			   ("std-scan-document-start", pName, nameSize)) {
			errorCheck |= InitPacket(&buf, 0x02);
			uiVal = 0;
			errorCheck |=
				AppendMessageToPacket(&buf, 0x22,
			...
			send(pState->m_tcpFd, buf.m_pBuf, buf.m_used, 0);
			   ...
			   ("std-scan-document-file-type", pName, nameSize)) {
			memcpy(&pState->m_fileType, pValue,
			       sizeof(pState->m_fileType));
			DBG(5, "File type: %x\n", ntohl(pState->m_fileType));
		} else if (!strncmp
			   ("std-scan-document-image-compression", pName,
			    nameSize)) {
			...
			DBG(5, "Compression: %x\n",
			   ...
			   ("std-scan-document-xresolution", pName,
			    nameSize)) {
			memcpy(&pState->m_xres, pValue,
			       sizeof(pState->m_xres));
			DBG(5, "X resolution: %d\n", ntohs(pState->m_xres));
		} else if (!strncmp
			   ("std-scan-document-yresolution", pName,
			    nameSize)) {
			memcpy(&pState->m_yres, pValue,
			       sizeof(pState->m_yres));
			DBG(5, "Y resolution: %d\n", ntohs(pState->m_yres));
		} else if (!strncmp
			   ("std-scan-page-widthpixel", pName, nameSize)) {
			if (1 || !pState->m_pixelWidth) {
				...
				DBG(5,
				    "Ignoring width (already have a value)\n");
			}
		} else if (!strncmp
			   ("std-scan-page-heightpixel", pName, nameSize)) {
			if (1 || !pState->m_pixelHeight) {
				...
				DBG(5,
				    "Ignoring height (already have a value)\n");
			}
			...
			shutdown(pState->m_tcpFd, SHUT_RDWR);
			   ...
			   ("std-scan-scandata-error", pName, nameSize)) {
			...
			/* append message to buffer */
			errorCheck |=
				AppendToComBuf(&pState->m_buf, pItem,
					       dataChunkSize);
...
ProcessPageData(struct ScannerState *pState)
{
		...
		/* decode as JPEG if appropriate */
	{
		...
		DBG(1,
		    "ProcessPageData: image dimensions: %d x %d, line size: %d\n",
		...
		pState->m_pixelHeight =
			htonl(jpegCinfo.m_cinfo.output_height);
		...
		while (jpegCinfo.m_cinfo.output_scanline
			...
			/* read scanline */
			jpeg_read_scanlines(&jpegCinfo.m_cinfo, &pJpegLine,
					    1);
		...
		pageInfo.m_totalSize =
			pageInfo.m_width * pageInfo.m_height * 3;
		pageInfo.m_bytesRemaining = pageInfo.m_totalSize;

		DBG(1,
		    "Process page data: page %d: JPEG image: %d x %d, %d bytes\n",
		    pState->m_numPages, pageInfo.m_width, pageInfo.m_height,
		    pageInfo.m_totalSize);

		ret |= AppendToComBuf(&pState->m_pageInfo,
		...
		/* CCITT Group 4 Fax data */
	{
		...
		if (!pTiff) {
			DBG(1,
			    "ProcessPageData: Error opening temp TIFF file");
		...
		TIFFSetField(pTiff, TIFFTAG_PLANARCONFIG,
			     PLANARCONFIG_CONTIG);
		...
		TIFFReadBufferSetup(pTiff, NULL,
				    width * height * sizeof(u_long));
		...
		DBG(1,
		    "Process page data: page %d: TIFF image: %d x %d, %d bytes\n",
...
/* Libjpeg decompression interface */
{
	struct JpegDataDecompState *pState =
		(struct JpegDataDecompState *) cinfo;
