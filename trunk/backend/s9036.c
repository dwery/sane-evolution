...
# define GET_READ_SIZES_READ_SIZE 24
	...
	status = sanei_scsi_cmd(fd, scsi_read, sizeof(scsi_read), result,
				&size);
...
/* This function sets and sends the window for scanning */
{
	...
	int contr_adjust =
		auto_contr ? (256 - s->val[OPT_CONTR_ADJUST]) % 256 : 0;
	...
	set_size(cmd.wd.wwidth, 2,
		 SANE_UNFIX(s->val[OPT_BR_X] - s->val[OPT_TL_X])
		 * pixels_per_mm + 0.5);
	set_size(cmd.wd.wlength, 2,
		 SANE_UNFIX(s->val[OPT_BR_Y] - s->val[OPT_TL_Y])
		 * pixels_per_mm + 0.5);
...
read_more_data(S9036_Scanner * s)
{
	...
	while (1) {
		...
		DBG(1, "Requesting %d lines, in scanner: %d, total: %d\n",
		    lines_read, s->lines_in_scanner, s->params.lines);

		status = sanei_scsi_cmd(s->fd, cmd, sizeof(cmd), s->buffer,
					&size);
		...
		if (size !=
		    (unsigned int) lines_read * s->params.bytes_per_line) {
			DBG(1,
			    "sanei_scsi_cmd(): got %lu bytes, expected %d\n",
			    (u_long) size,
			    lines_read * s->params.bytes_per_line);
			return SANE_STATUS_INVAL;
		}
...
init_options(S9036_Scanner * s)
{
	...
	s->opt[OPT_BRIGHT_ADJUST].desc =
		"Controls the automatic brightness of the "
	...
	s->opt[OPT_CONTR_ADJUST].desc =
		"Controls the automatic contrast of the "
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_BRIGHT_ADJUST:
		case OPT_CONTR_ADJUST:
			s->val[option] = *(int *) val;
			break;
		case OPT_BRIGHTNESS:
			if (SANE_OPTION_IS_ACTIVE
			    (s->opt[OPT_BRIGHT_ADJUST].cap)) {
				s->opt[OPT_BRIGHT_ADJUST].cap |=
					SANE_CAP_INACTIVE;
				if (info)
...
sane_start(SANE_Handle handle)
{
...

	{
		...
		status = get_read_sizes(s->fd, &lines_available,
					&bytes_per_line, &total_lines);
		if (status != SANE_STATUS_GOOD) {
		...
		s->params.pixels_per_line =
			bytes_per_line * (8 / s->params.depth);
...
static void
copy_buffer(S9036_Scanner * s, SANE_Byte ** buf, int * max_len,
	    int * len)
{
	  ...
	  int * len)
{
	...
	*len = 0;
	...
	while (max_len > (int) s->in_buffer
	       && s->lines_read < s->params.lines) {
		...
		if (s->in_buffer == 0) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "sane_read: read_more_data() failed (%s)\n",
				    sane_strstatus(status));
				do_cancel(s);
				return status;
			}
