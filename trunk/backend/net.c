...
# endif	/* ENABLE_IPV6 */
	...
	if (error) {
		...
		if (error) {
			DBG(1,
			    "add_device: error while getting address of host %s: %s\n",
			...
			for (resp = res; resp != NULL; resp = resp->ai_next) {
				...
				case AF_INET:
					sin = (struct sockaddr_in *) resp->
						ai_addr;
					sin->sin_port = sane_port;
					break;
#ifdef ENABLE_IPV6
				case AF_INET6:
					sin6 = (struct sockaddr_in6 *) resp->
						ai_addr;
	...
	if (!nd) {
		DBG(1,
		    "add_device: not enough memory for Net_Device struct\n");
...
add_device(const char *name, Net_Device ** ndp)
{
	...
	if (he->h_addrtype != AF_INET) {
		DBG(1,
		    "add_device: don't know how to deal with addr family %d\n",
		    he->h_addrtype);
...
connect_dev(Net_Device * dev)
{
	...
	for (addrp = dev->addr, i = 0;
	     (addrp != NULL) && (connected == FALSE);
	     addrp = addrp->ai_next, i++) {
# ifdef ENABLE_IPV6
		if ((addrp->ai_family != AF_INET)
		    && (addrp->ai_family != AF_INET6))
# else /* !ENABLE_IPV6 */
		if (addrp->ai_family != AF_INET)
# endif	/* ENABLE_IPV6 */
		{
			DBG(1,
			    "connect_dev: [%d] don't know how to deal with addr family %d\n",
			    i, addrp->ai_family);
			continue;
		}
		...
		if (dev->ctl < 0) {
			DBG(1,
			    "connect_dev: [%d] failed to obtain socket (%s)\n",
			    i, strerror(errno));
			dev->ctl = -1;
			continue;
		}
		...
		if (connect_timeout > 0) {
			...
			if (setsockopt
			    (dev->ctl, SOL_SOCKET, SO_SNDTIMEO, &tv,
			     sizeof(tv)) < 0) {
				DBG(1,
				    "connect_dev: [%d] failed to set SO_SNDTIMEO (%s)\n",
				    i, strerror(errno));
			}
		}
		...
		DBG(3, "connect_dev: [%d] connection succeeded (%s)\n", i,
		    (addrp->ai_family == AF_INET6) ? "IPv6" : "IPv4");
		dev->addr_used = addrp;
		connected = TRUE;
	}
	...
	if (connected != TRUE) {
		DBG(1,
		    "connect_dev: couldn't connect to host (see messages above)\n");
		return SANE_STATUS_IO_ERROR;
	}
#else /* !NET_USES_AF_INDEP */
...
connect_dev(Net_Device * dev)
{
	...
	if (dev->addr.sa_family != AF_INET) {
		DBG(1,
		    "connect_dev: don't know how to deal with addr family %d\n",
		    dev->addr.sa_family);
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (connect_timeout > 0) {
		...
		if (setsockopt
		    (dev->ctl, SOL_SOCKET, SO_SNDTIMEO, &tv,
		     sizeof(tv)) < 0) {
			DBG(1,
			    "connect_dev: failed to set SO_SNDTIMEO (%s)\n",
			    strerror(errno));
		}
	}
	...
	if (connect_timeout > 0) {
		...
		if (setsockopt
		    (dev->ctl, SOL_SOCKET, SO_SNDTIMEO, &tv,
		     sizeof(tv)) < 0) {
			DBG(1,
			    "connect_dev: failed to reset SO_SNDTIMEO (%s)\n",
			    strerror(errno));
		}
	}
#ifdef TCP_NODELAY
...
# else /* !SOL_TCP */
	/* Look up the protocol level in the protocols database. */
	{
		...
		if (p == 0)
			DBG(1,
			    "connect_dev: cannot look up `tcp' protocol number");
		else
			level = p->p_proto;
	}
# endif	/* SOL_TCP */
	...
	if (level == -1 ||
		...
		DBG(1,
		    "connect_dev: failed to put send socket in TCP_NODELAY mode (%s)",
		    strerror(errno));
#endif /* !TCP_NODELAY */
	...
	if (SANE_VERSION_MAJOR(version_code) != V_MAJOR) {
		DBG(1,
		    "connect_dev: major version mismatch: got %d, expected %d\n",
...
fetch_options(Net_Scanner * s)
{
	...
	if (s->opt.num_options) {
		DBG(2,
		    "fetch_options: %d option descriptors cached... freeing\n",
		    s->opt.num_options);
		...
		if (s->hw->wire.status) {
			DBG(1,
			    "fetch_options: failed to free old list (%s)\n",
	...
	if (s->hw->wire.status) {
		DBG(1,
		    "fetch_options: failed to get option descriptors (%s)\n",
		    strerror(s->hw->wire.status));
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (s->local_opt.num_options == 0) {
		DBG(3,
		    "fetch_options: creating %d local option descriptors\n",
		    s->opt.num_options);
		s->local_opt.desc =
			malloc(s->opt.num_options *
			       sizeof(s->local_opt.desc));
		if (!s->local_opt.desc) {
			DBG(1,
			    "fetch_options: couldn't malloc s->local_opt.desc\n");
			return SANE_STATUS_NO_MEM;
		}
		for (option_number = 0;
		     option_number < s->opt.num_options; option_number++) {
		...
		DBG(1,
		    "fetch_options: option number count changed during runtime?\n");
		return SANE_STATUS_INVAL;
	}
	...
	for (option_number = 0; option_number < s->opt.num_options;
	     option_number++) {
		memcpy(s->local_opt.desc[option_number],
		       s->opt.desc[option_number],
...
do_authorization(Net_Device * dev, char * resource)
{
	...
	memset(username, 0, sizeof(char) * SANE_MAX_USERNAME_LEN);
	memset(password, 0, sizeof(char) * SANE_MAX_PASSWORD_LEN);
	...
	if (net_resource != NULL) {
		...
		if (auth_callback) {
			DBG(2,
			    "do_authorization: invoking auth_callback, resource = %s\n",
			    net_resource);
			(*auth_callback) (net_resource, username, password);
		} else
			DBG(1,
			    "do_authorization: no auth_callback present\n");
		free(net_resource);
	} else {		/* Is this necessary? If we don't have these few bytes we will get
				   in trouble later anyway */
		DBG(1,
		    "do_authorization: not enough memory for net_resource\n");
		if (auth_callback) {
			DBG(2,
			    "do_authorization: invoking auth_callback, resource = %s\n",
			    resource);
...
sane_init(int *version_code, SANE_Auth_Callback authorize)
{
	...
	DBG(2, "sane_init: authorize = %p, version_code = %p\n",
	    (void *) authorize, (void *) version_code);
	...
	if (serv) {
		...
		DBG(1,
		    "sane_init: could not find `sane-port' service (%s); using default "
	...
	if (fp) {
		while (sanei_config_read
		       (device_name, sizeof(device_name), fp)) {
			...
			if (strstr(device_name, "connect_timeout") != NULL) {
				...
				optval = sanei_config_skip_whitespace
					(++optval);
				if ((optval != NULL) && (*optval != '\0')) {
					connect_timeout = atoi(optval);

					DBG(2,
					    "sane_init: connect timeout set to %d seconds\n",
					    connect_timeout);
				}
	...
	DBG(2,
	    "sane_init: evaluating environment variable SANE_NET_TIMEOUT\n");
	env = getenv("SANE_NET_TIMEOUT");
	if (env) {
		connect_timeout = atoi(env);
		DBG(2,
		    "sane_init: connect timeout set to %d seconds from env\n",
		    connect_timeout);
	}
	...
	for (dev = first_device; dev; dev = dev->next) {
		if (dev->ctl < 0) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "sane_get_devices: ignoring failure to connect to %s\n",
				    dev->name);
				continue;
			}
		}
		...
		if (reply.status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_get_devices: ignoring rpc-returned status %s\n",
			    sane_strstatus(reply.status));
			sanei_w_free(&dev->wire, (WireCodecFunc)
				     sanei_w_get_devices_reply, &reply);
			continue;
		}
		...
		for (i = 0; i < num_devs; ++i) {
			...
			len = strlen(dev->name) + 1 +
				strlen(reply.device_list[i]->name);
			...
			if (!mem) {
				DBG(1,
				    "sane_get_devices: not enough free memory\n");
				sanei_w_free(&dev->wire, (WireCodecFunc)
...
sane_open(const char * full_name, SANE_Handle * meta_handle)
{
	...
	if (full_name[0] == '[') {
		...
		if (!tmp_name) {
			DBG(1,
			    "sane_open: incorrect host address: missing matching ']'\n");
			return SANE_STATUS_INVAL;
		}
	} else
		tmp_name = full_name;
...
# ifdef ENABLE_IPV6
		if (v6addr == TRUE)
			nd_name =
				strndupa(full_name + 1,
					 dev_name - full_name - 2);
		else
...
# endif	/* ENABLE_IPV6 */
	...
	do {
		...
		if (need_auth) {
			...
			sanei_w_free(&dev->wire,
				     (WireCodecFunc) sanei_w_open_reply,
				     &reply);
...
sane_close(SANE_Handle handle)
{
	...
	if (s->opt.num_options) {
		...
		if (s->hw->wire.status)
			DBG(1,
			    "sane_close: couldn't free sanei_w_option_descriptor_array "
...
sane_get_option_descriptor(SANE_Handle handle, int option)
{
	...
	if (!s->options_valid) {
		DBG(3,
		    "sane_get_option_descripter: getting option descriptors\n");
		status = fetch_options(s);
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_get_option_descriptor: fetch_options failed (%s)\n",
			    sane_strstatus(status));
			return 0;
		}
	}

	if (((int) option >= s->opt.num_options) || (option < 0)) {
		    ...
		    SANE_Action action, void *value, int *info)
{
	...
	if (!s->options_valid) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_control_option: fetch_options failed (%s)\n",
			    sane_strstatus(status));
	...
	do {
		...
		if (need_auth) {
			...
			sanei_w_free(&s->hw->wire, (WireCodecFunc)
				     sanei_w_control_option_reply, &reply);
			...
			if (req.value_size > 0) {
				if ((int) req.value_size == reply.value_size)
					...
					DBG(1,
					    "sane_control_option: size changed from %d to %d\n",
					    s->opt.desc[option]->size,
					    reply.value_size);
			}
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!params) {
		DBG(1,
		    "sane_get_parameters: parameter params not supplied\n");
		return SANE_STATUS_INVAL;
	}
...
sane_start(SANE_Handle handle)
{
	...
	do {
		...
		if (reply.byte_order == 0x1234) {
			server_big_endian = 0;
			DBG(1,
			    "sane_start: server has little endian byte order\n");
		} else {
			server_big_endian = 1;
			DBG(1,
			    "sane_start: server has big endian byte order\n");
		}
		...
		if (need_auth) {
			...
			sanei_w_free(&s->hw->wire,
				     (WireCodecFunc) sanei_w_start_reply,
				     &reply);
...
sane_read(SANE_Handle handle, SANE_Byte * data, int max_length, int *length)
{
	...
	*length = 0;
	...
	if ((depth == 16) && (server_big_endian != client_big_endian)) {
		if (left_over > -1) {
			DBG(3,
			    "sane_read: left_over from previous call, return "
			    "immediately\n");
	...
	if (s->data < 0) {
		DBG(1,
		    "sane_read: data pipe doesn't exist, scan cancelled?\n");
		return SANE_STATUS_CANCELLED;
	}
	...
	if (s->bytes_remaining == 0) {
		...
		DBG(4, "sane_read: read %lu bytes, %d from 4 total\n",
		    (u_long) nread, s->reclen_buf_offset);
		...
		if (s->bytes_remaining == 0xffffffff) {
			...
			if (read(s->data, &ch, sizeof(ch)) != 1) {
				DBG(1,
				    "sane_read: failed to read error code\n");
