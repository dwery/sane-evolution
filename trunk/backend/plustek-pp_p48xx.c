 ...
 */
static void
p48xxSetMemoryBankForProgram(pScanData ps, Byte bBankAndSize)
{
	/* enter program mode */
...
p48xxDoTest(pScanData ps)
{
	...
	for (cntr = _BankAndSizeForTest;
		...
		if (ul != _TEST_SZ / sizeof(ULong)) {
			DBG(DBG_LOW,
			    "Bank not present, error at pos %lu (%lu)\n", ul,
 ...
 */
static void
p48xxSetAsicRegisters(pScanData ps)
{
 ...
 */
static int
p48xxCheck4800Memory(pScanData ps)
{
	int retval;
	ULong ul;
	pUChar buffer;
 ...
 */
static int
p48xxInitAllModules(pScanData ps)
{
	int result;
 ...
 */
static int
p48xxReadWriteTest(pScanData ps)
{
	int retval;
	...
	if (_NO_BASE == ps->sCaps.wIOBase) {
		...
		/* get copy of asic id */
		ps->sCaps.AsicID =
			IODataRegisterFromScanner(ps, ps->RegAsicID);
		...
		if (_ASIC_IS_96003 == ps->sCaps.AsicID) {
			...
			if (_ASIC_IS_96001 == ps->sCaps.AsicID) {
				DBG(DBG_LOW,
				    "Found a 96001 ASIC at Reg 0x%x\n",
				    ps->RegAsicID);
				ModelSet4800(ps);
			} else {
				DBG(DBG_LOW,
				    "Can't find your model, asic = 0x%x\n",
				    ps->sCaps.AsicID);
				return _E_NO_ASIC;
			}
		}
 ...
 */
static void
p48xxSetupScannerVariables(pScanData ps)
{
	UChar tmp;
	TimerDef timer;
	...
	if (_PORT_SPP != ps->IO.portMode) {
		...
		if (NULL != pBuf) {
			...
			do {
				IOReadScannerImageData(ps, pBuf,
						       (_BUF_SIZE_BASE_CONST *
							2));
 ...
 */
static void
p48xxSetGeneralRegister(pScanData ps)
{
 ...
 */
static void
p48xxSetupScanningCondition(pScanData ps)
{
	...
	if (MODEL_OP_A3I == ps->sCaps.Model)
		ps->wLinesPer64kTime =
			(UShort) (65555UL / ps->DataInf.dwAsicBytesPerPlane *
				  5UL);
	else
		ps->wLinesPer64kTime =
			(UShort) (65555UL / ps->DataInf.dwAsicBytesPerPlane *
	...
	ps->bMinReadFifo =
		(Byte) ((ps->DataInf.dwAsicBytesPerPlane + 511) / 512);
...
*/
	...
	ps->AsicReg.RD_Origin =
		(UShort) (ps->Offset70 + ps->Device.DataOriginX +
 ...
 */
static void
p48xxPutToIdleMode(pScanData ps)
{
