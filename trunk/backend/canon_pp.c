...
/* leftover from the last read */
static SANE_Bool force_nibble = FALSE;
 ...
 *************************************************************************/
SANE_Status
sane_init(int * vc, SANE_Auth_Callback cb)
{
	...
	for (s_tmp = first_dev; s_tmp != NULL; s_tmp = s_tmp->next) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(10,
			    "sane_init: Error detecting port mode on %s!\n",
		...
		if (tmp && (s_tmp->ieee1284_mode != M1284_NIBBLE)) {
			...
			sanei_canon_pp_set_ieee1284_mode(s_tmp->
							 ieee1284_mode);
			tmp = sanei_canon_pp_detect(s_tmp->params.port,
						    s_tmp->init_mode);
		}
...
sane_open(SANE_String_Const name, SANE_Handle * h)
{
	...
	if (!strlen(name)) {
		...
		if ((cs == NULL) || (cs->scanner_present != TRUE)) {
			cs = first_dev;
			while ((cs != NULL) &&
			       (cs->scanner_present == FALSE))
				cs = cs->next;
		}
	...
	if (tmp != 0) {
		...
		DBG(1, "sane_open: Can't contact scanner.  Try power "
		    "cycling scanner, and unplug any " "printers\n");
	...
	if (cs->weights_file != NULL)
		DBG(2, "sane_open: >> load_weights(%s, %p)\n",
		    cs->weights_file, (const void *) (&(cs->params)));
	else
...
sane_get_option_descriptor(SANE_Handle h, int opt)
{
 ...
 *************************************************************************/
SANE_Status
sane_control_option(SANE_Handle h, int opt, SANE_Action act,
		    void *val, int * info)
{
	CANONP_Scanner *cs = ((CANONP_Scanner *) h);
	int i = 0, tmp, maxresi;
	...
	if (cs->scanning == TRUE) {
		DBG(1,
		    "sane_control_option: That scanner (%p) is scanning!\n",
		    h);
		return SANE_STATUS_DEVICE_BUSY;
	}
	...
	case SANE_ACTION_GET_VALUE:
		...
		case OPT_COLOUR_MODE:
			strcpy((char *) val, cmodes[cs->vals[opt]]);
			break;
		case OPT_DEPTH:
			strcpy((char *) val, depths[cs->vals[opt]]);
			break;
		case OPT_RESOLUTION:
	...
	case SANE_ACTION_SET_VALUE:
		...
		case OPT_COLOUR_MODE:
			...
			while ((cmodes[cs->vals[opt]] != NULL)
			       && strcmp(cmodes[cs->vals[opt]], (char *) val)) {
				cs->vals[opt] += 1;
			}
			if (info != NULL)
		...
		case OPT_DEPTH:
			...
			while ((depths[cs->vals[opt]] != NULL)
			       && strcmp(depths[cs->vals[opt]], (char *) val)) {
				cs->vals[opt] += 1;
			}
			if (info != NULL)
				*info |= SANE_INFO_RELOAD_PARAMS;
		...
		case OPT_TL_Y:
		case OPT_BR_Y:
			if ((i < cs->opt[opt].constraint.range->min)
			    || (i > cs->opt[opt].constraint.range->max))
				return SANE_STATUS_INVAL;
			cs->vals[opt] = i;
			break;
		case OPT_CAL:
			/* Call the calibration code */
			if ((cs->weights_file == NULL) || cs->cal_readonly)
				DBG(2, ">> calibrate(x, " "NULL)\n");
			else
				DBG(2, ">> calibrate(x,"
				    "%s)\n", cs->weights_file);

			if (cs->cal_readonly)
				tmp = sanei_canon_pp_calibrate(&(cs->params),
							       NULL);
			else
				tmp = sanei_canon_pp_calibrate(&(cs->params),
							       cs->
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * params)
{
	...
	if (params->pixels_per_line < 64)
		params->pixels_per_line = 64;
	...
	if (params->lines > max_height)
		params->lines = max_height;
	...
	DBG(10,
	    "get_params: bytes_per_line=%d, pixels_per_line=%d, lines=%d\n"
...
sane_start(SANE_Handle h)
{
	...
	if (cs->scan.width > max_width)
		cs->scan.width = max_width;
	if (cs->scan.width + cs->scan.xoffset > max_width)
		cs->scan.xoffset = max_width - cs->scan.width;
	if (cs->scan.height > max_height)
		cs->scan.height = max_height;
 ...
 *************************************************************************/
SANE_Status
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * lenp)
{
	CANONP_Scanner *cs = ((CANONP_Scanner *) h);
	image_segment *is;
	...
	*lenp = 0;
	...
	DBG(10, "scanner setup: shw=%d xres=%d yres=%d %d %d id=%s\n",
	    ...
	    cs->params.max_yresolution, (cs->params.id_string) + 8);
	DBG(10, "scan_params->: width=%d, height=%d, xoffset=%d, "
	    ...
	    cs->scan.xresolution, cs->scan.yresolution, cs->scan.mode, lines);

	DBG(2, ">> read_segment(x, x, x, %d, %d, %d)\n",
	    lines, cs->cal_valid, cs->scan.height - cs->lines_scanned);
...
init_device(struct parport *pp)
{
	...
	cs->opt[OPT_COLOUR_MODE].constraint_type =
 ...
 ************************************************************************/
static SANE_Status
fix_weights_file(CANONP_Scanner * cs)
{
	...
	if (cs->weights_file[0] == '~') {
		...
		if ((myhome = getenv("HOME")) == NULL) {
			DBG(0,
			    "fix_weights_file: FATAL: ~ used, but $HOME not"
			    " set!\n");
			free(tmp);
			tmp = NULL;
			return SANE_STATUS_INVAL;
...
detect_mode(CANONP_Scanner * cs)
{
	...
	if (capabilities & CAP1284_ECP) {
		...
		DBG(0,
		    "detect_mode: No supported parport modes available!\n");
		ieee1284_release(cs->params.port);
		ieee1284_close(cs->params.port);
		return SANE_STATUS_IO_ERROR;
	}
