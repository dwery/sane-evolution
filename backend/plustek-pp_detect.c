 ...
 */
static int
detectScannerConnection(pScanData ps)
{
	...
	if (0x55 == data) {
		...
		if (0xAA == data) {
			 ...
			 */
			DBG(DBG_HIGH,
			    "Compare data=0x%x and status=0x%x, port=0x%x\n",
			...
			if (data != status) {
				 ...
				 */
				for (ps->IO.delay = 0; ps->IO.delay < 5;
				     ps->IO.delay++) {
 ...
 */
static int
detectSetupBuffers(pScanData ps)
{
...
*/
	...
	if (_ASIC_IS_98001 == ps->sCaps.AsicID) {
		...
		ps->dwShadow =
			(_DEF_BRIGHTEST_SKIP +
			 _DEF_DARKEST_SKIP) * 5400UL * 2UL * 3UL;
		...
		if (NULL != ps->Shade.pHilight) {
			...
			ps->pwShadow =
				(pUShort) ps->Shade.pHilight + ps->dwHilight;
			ps->Shade.dwDiv =
				32UL - _DEF_BRIGHTEST_SKIP -
				_DEF_DARKEST_SKIP;
		...
		ps->Bufs.TpaBuf.pb =
			&((pUChar) ps->Bufs.b2.pSumBuf)[_SizeShadingSumBuf];
 ...
 */
_LOC int
DetectScanner(pScanData ps, int mode)
{
	Byte asic;
	...
	if (0 == mode) {
		...
		if (_ASIC_IS_98001 == mode) {
			...
			DBG(DBG_HIGH,
			    "Starting Scanner-detection (ASIC 98001)\n");
			result = detectAsic98001(ps);
