 ...
 */
static void
tpaP98SubNoise(pScanData ps, pULong pdwSum, pUShort pwShading,
 ...
 */
static void
tpaP98ShadingWaveformSum(pScanData ps)
{
	DataPointer pd, pt;
	ULong dw;
	...
	if ((ps->DataInf.dwScanFlag & SCANDEF_TPA) ||
		...
		if (ps->Shade.pHilight) {
			...
			tpaP98SubNoise(ps,
				       (pULong) ps->pScanBuffer1 +
				       5400UL * 2UL,
				       (pUShort) ps->pScanBuffer1 +
				       5400UL * 2UL, ps->dwHilightCh * 2,
				       ps->dwShadowCh * 2);
		} else {
 ...
 */
static void
tpaP98GetNegativeTempRamData(pScanData ps)
{
	...
	ps->AsicReg.RD_ModelControl =
		_ModelDpi600 + _LED_CONTROL + _LED_ACTIVITY;
	ps->AsicReg.RD_Dpi = ps->PhysicalDpi;

	if (!ps->wNegAdjustX) {
		ps->AsicReg.RD_Origin =
			(UShort) (ps->dwOffset70 + ps->Device.DataOriginX +
	...
	for (dw1 = 16; dw1; dw1--) {
		...
		IOReadScannerImageData(ps,
				       (pUChar) (pNegativeTempRam + 960 * 2),
				       960);

		/* fillNegativeSum() */
		for (dw = 0; dw < 960 * 3; dw++)
			pdwNegativeSumTemp[dw] +=
				((pUShort) pNegativeTempRam)[dw];
	...
	/* NegativeAdd1() */
	if (!ps->wNegAdjustX) {
		dw1 = (ps->dwOffsetNegative +
		       _Negative96OriginOffsetX * 2 * 2) / 2;
	} else {
	...
	for (dw = 0; dw < 240; dw++, dw1 += 4) {
		pNegativeTempRam2[dw] = (UShort) ((((pUShort) ps->
						    pScanBuffer1)[dw1] +
						   ((pUShort) ps->
						    pScanBuffer1)[dw1 + 1] +
						   ((pUShort) ps->
	...
	/* NegativeAdd1() */
	if (!ps->wNegAdjustX)
		dw1 = (ps->dwOffsetNegative + 5400 * 2 +
		       _Negative96OriginOffsetX * 2 * 2) / 2;
	else
		dw1 = (ps->dwOffsetNegative + 5400 * 2 +
		       ps->wNegAdjustX * 2) / 2;
	...
	/* NegativeAdd1() */
	if (!ps->wNegAdjustX)
		dw1 = (ps->dwOffsetNegative + 5400 * 4 +
		       _Negative96OriginOffsetX * 2 * 2) / 2;
	else
		dw1 = (ps->dwOffsetNegative + 5400 * 4 +
		       ps->wNegAdjustX * 2) / 2;
	...
	if ((ps->bRedGainIndex +=
	     (Byte) ((wRedShadingTemp / wRedTemp) * 100 - 50) / 25) > 32)
		ps->bRedGainIndex = 31;

	if ((ps->bGreenGainIndex +=
	     (Byte) ((wGreenShadingTemp / wGreenTemp) * 100 - 50) / 25) > 32)
		ps->bGreenGainIndex = 31;

	if ((ps->bBlueGainIndex +=
 ...
 */
static void
tpaP98RecalculateNegativeShadingGain(pScanData ps)
{
	Byte b[3];
	UShort wSum, counter;
	UShort w, w1, w2;
 ...
 */
static void
tpaP98RecalculateShadingGainandData(pScanData ps)
{
	DataPointer p;
	ULong dw;
	UShort filmAdjustX;
	...
	wOldRedGain =
		a_wGainString[ps->bRedGainIndex] * 100 / ps->wReduceRedFactor;

	/* SearchNewGain() */
	for (ps->bRedGainIndex = 0; ps->bRedGainIndex < 32;
	     ps->bRedGainIndex++) {
	...
	wOldBlueGain =
		a_wGainString[ps->bBlueGainIndex] * 100 /
		ps->wReduceBlueFactor;

	/* SearchNewGain() */
	for (ps->bBlueGainIndex = 0; ps->bBlueGainIndex < 32;
	     ps->bBlueGainIndex++) {
 ...
 */
_LOC void
TPAP98001AverageShadingData(pScanData ps)
{
	...
	if ((ps->DataInf.wPhyDataType >= COLOR_256GRAY) &&
		...
		if (((ps->DataInf.dwScanFlag & SCANDEF_Negative)
		     && !ps->wNegAdjustX)
		    || ((ps->DataInf.dwScanFlag & SCANDEF_Transparency)
			&& !ps->wPosAdjustX)) {
			...
			DBG(DBG_LOW,
			    "_TPAPageWidth = %u, _NegativePageWidth = %lu\n"
			    "right = %ld, left = %ld --> right = %ld\n",
			    _TPAPageWidth, _NegativePageWidth, dwRight,
			    dwLeft, (((Long) dwRight - (Long) dwLeft) / 2));
 ...
 */
_LOC void
TPAP98003FindCenterPointer(pScanData ps)
{
	ULong i;
	ULong width;
	ULong left;
	...
	if ((right <= left) || ((right - left) < width)) {
		if (ps->DataInf.dwScanFlag & SCANDEF_Negative)
			ps->Scan.negBegin =
				ps->Device.DataOriginX + _NEG_ORG_OFFSETX * 2;
		else
			ps->Scan.posBegin =
 ...
 */
_LOC void
TPAP98003Reshading(pScanData ps)
{
...
/* CHECK: Why this ??? */
	...
	for (dwSum = 8; dwSum--;) {
		...
		IOReadOneShadingLine(ps, ps->Bufs.b1.pShadingMap,
				     _NEG_PAGEWIDTH600);
	...
	for (i = 16; i-- && !ps->Shade.fStop;) {
		...
		if (IOReadOneShadingLine
		    (ps, ps->Bufs.b1.pShadingMap, _NEG_PAGEWIDTH600)) {
			...
			if (!bHi[0] || !bHi[1] || !bHi[2]) {
				...
				DacP98003AdjustGain(ps, _CHANNEL_GREEN,
						    bHi[1]);
	...
	if (dwR && dwG && dwB) {
		...
		if (ps->Device.bDACType != _DA_ESIC
		    && ps->Device.bCCDID != _CCD_3799) {
			ps->Shade.DarkOffset.Colors.Red =
