/*
  ...
  $Id: niash.c,v 1.13 2006-02-04 10:28:50 ullsig-guest Exp $
*/
...
typedef struct
{
	...
	int aGammaTable[SANE_GAMMA_SIZE];	/* a 12-to-8 bit color lookup table */

	/* fCancelled needed to let sane issue the cancel message
	   instead of an error message */
	SANE_Bool fCancelled;	/* TRUE if scanning cancelled */
...
static void
_ConvertGammaTable(int * saneGamma, unsigned char *hwGamma)
{
	int i;
	int current = 0;
...
static const int startUpGamma = SANE_FIX(1.6);
...
{
	return (pixelsPerLine * BITS_PER_PIXEL_GRAY +
		BITS_PADDING) / BITS_PER_BYTE;
}
...
{
	static const int aMask[BITS_PER_BYTE] =
...
typedef struct tgModeParam
{
	...
	void (*adaptFormat) (unsigned char *rgbBuffer, int pixels,
			     int threshold);
...
_WarmUpLamp(TScanner * s, int iMode)
{
	...
	if (iMode == WARMUP_AFTERSTART) {
		fLampOn = FALSE;
		s->CalWhite[0] = s->CalWhite[1] = s->CalWhite[2] =
			(unsigned char) (-1);
	} else
...
_WaitForLamp(TScanner * s, unsigned char *pabCalibTable)
{
	...
	while (s->WarmUpTime) {
		...
		if (s->WarmUpTime && fHasCal) {
			SANE_Bool fOver = TRUE;
			for (i = 0; fOver && i < 3; ++i) {
				if (!s->CalWhite[i])
					fOver = FALSE;
				else if (CalWhite[iCurrent][i] <
					 s->CalWhite[i])
					fOver = FALSE;
			}
		...
		if (s->WarmUpTime && fHasCal && iCal) {
			...
			if (_TimeElapsed
			    (&s->WarmUpStarted, &now[iCurrent],
			     WARMUP_MAXTIME)) {
				/* stop idling */
				s->WarmUpTime = 0;
				DBG(DBG_MSG,
				    "_WaitForLamp: WARMUP_MAXTIME=%ds elapsed!\n",
				    WARMUP_MAXTIME);
			}
		}
		...
		if (s->WarmUpTime) {
			...
			if (fHasCal)
				DBG(DBG_MSG,
				    "_WaitForLamp: entering delay loop\r");
			else
				DBG(DBG_MSG,
				    "_WaitForLamp: delay loop %d        \r",
				    ++iDelay);
			sleep(1);
			fHasCal = FALSE;
			gettimeofday(&now[!iCurrent], 0);
		...
		if (s->WarmUpTime	/* did we have to wait at all */
		    ...
		    && _TimeElapsed(&s->WarmUpStarted, &now[!iCurrent],
				    s->WarmUpTime)
			...
			SimpleCalibExt(&s->HWParams, pabCalibTable,
				       CalWhite[iCurrent]);
			fHasCal = TRUE;
			...
			DBG(DBG_MSG,
			    "_WaitForLamp: recalibration #%d, deviation = %d%%\n",
			    iCal, dev);
...
_SetScalarGamma(int * aiGamma, int sfGamma)
{
	...
	for (j = 0; j < SANE_GAMMA_SIZE; j++) {
		int iData;
		iData = floor(256.0 *
			      pow(((double) j / (double) SANE_GAMMA_SIZE),
				  1.0 / fGamma));
		if (iData > 255)
			iData = 255;
		aiGamma[j] = iData;
...
static int
_ChangeCap(int * pCap, int cap, int isSet)
{
	int prevCap = *pCap;
	if (isSet) {
		*pCap |= cap;
	...
	for (i = optCount; i < optLast; i++) {
		...
		case optBRY:
			...
			pDesc->cap =
				SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
			pVal->w = 290
				/* have a bit reserve instaed of rangeYmm.max */
				;
			break;
		...
		case optLamp:
			...
			pDesc->desc =
				SANE_I18N("Switches the lamp on or off.");
		...
		case optThreshold:
			...
			pDesc->cap =
				SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT |
				SANE_CAP_INACTIVE | SANE_CAP_EMULATED;
			pVal->w = 50;
...
SANE_Status
sane_init(int * piVersion, SANE_Auth_Callback pfnAuth)
{
		    ...
		    void *pVal, int * pInfo)
{
	...
	case SANE_ACTION_GET_VALUE:
		...
		case optThreshold:
			DBG(DBG_MSG,
			    ...
			    n, (int) s->aValues[n].w);
			*(int *) pVal = s->aValues[n].w;
			break;

			/* Get options of type int array */
		case optGammaTable:
		...
		default:
			DBG(DBG_MSG,
			    "SANE_ACTION_GET_VALUE: Invalid option (%d)\n",
			    n);
		}
		break;
	...
	case SANE_ACTION_SET_VALUE:
		...
		case optBRY:
			...
			status = sanei_constrain_value(&s->aOptions[n], pVal,
						       &info);
			if (status != SANE_STATUS_GOOD) {
				DBG(DBG_ERR,
				    "Failed to constrain option %d (%s)\n", n,
				    s->aOptions[n].title);
				return status;
			}

			/* check values if they are equal */
			fSame = s->aValues[n].w == *(int *) pVal;

			/* set the values */
			s->aValues[n].w = *(int *) pVal;
			DBG(DBG_MSG,
		...
		case optGammaTable:
			...
			for (i = 0; i < SANE_GAMMA_SIZE; i++) {
				if ((SANE_GAMMA_SIZE / 16)
				    && (i % (SANE_GAMMA_SIZE / 16)) == 0) {
					strcat(szTable, "\n");
					DBG(DBG_MSG, szTable);
					strcpy(szTable, "");
				}
				/* test for number print */
				if ((SANE_GAMMA_SIZE / 64)
				    && (i % (SANE_GAMMA_SIZE / 64)) == 0) {
		...
		case optMode:
		{
			...
			if (strcmp((char const *) pVal, colorStr) == 0) {
				...
				fCapChanged =
					_ChangeCap(pCap, SANE_CAP_INACTIVE,
						   1);
			}
...
/*       SimpleCalib(&s->HWParams); */
		...
		default:
			DBG(DBG_ERR,
			    "SANE_ACTION_SET_VALUE: Invalid option (%d)\n",
			    n);
		}
		if (pInfo != NULL) {
			*pInfo |= info;
...
sane_start(SANE_Handle h)
{
	...
	s->ScanParams.iTop =
		MM_TO_PIXEL(s->aValues[optTLY].w + s->HWParams.iTopLeftY,
			    HW_LPI) - iLineCorr;
	s->ScanParams.iLeft =
		MM_TO_PIXEL(s->aValues[optTLX].w + s->HWParams.iTopLeftX,
			    HW_DPI);
	...
	if (s->HWParams.fReg07) {
		s->DataPipe.iSkipLines +=
			MM_TO_PIXEL(s->aValues[optTLY].w +
				    s->HWParams.iTopLeftY,
...
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * len)
{
	...
	if (s->iBytesLeft == 0) {
		...
		if (CircBufferGetLineEx
		    (s->HWParams.iXferHandle, p, p->pabLineBuf,
		     s->HWParams.iReversedHead, TRUE)) {
			pMode->adaptFormat(p->pabLineBuf, s->iPixelsPerLine,
					   s->aValues[optThreshold].w);
			s->iBytesLeft =
				pMode->bytesPerLine(s->iPixelsPerLine);
			s->iLinesLeft--;
		}
