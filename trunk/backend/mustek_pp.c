...
/* prototypes */
static void free_cfg_options(int *numoptions,
			     Mustek_pp_config_option ** options);
...
{
	sanei_pa4s2_enable(fd_to_release, FALSE);
	    ...
	    int info)
{
	...
	DBG(3,
	    "sane_attach: attaching device ``%s'' to port %s (driver %s v%s by %s)\n",
	...
	dev->func->capabilities(info, &dev->model, &dev->vendor, &dev->type,
				&dev->maxres, &dev->minres, &dev->maxhsize,
				&dev->maxvsize, &dev->caps);
...
init_options(Mustek_pp_Handle * hndl)
{
	...
	hndl->opt[OPT_PREVIEW].cap =
		SANE_CAP_SOFT_DETECT | SANE_CAP_SOFT_SELECT;
	hndl->val[OPT_PREVIEW].w = FALSE;
	...
	hndl->val[OPT_GRAY_PREVIEW].w = FALSE;
	...
	hndl->opt[OPT_DEPTH].size = sizeof(int);
	...
	hndl->x_range.max =
		SANE_FIX(PIXEL_TO_MM(hndl->dev->maxhsize, hndl->dev->maxres));
	...
	hndl->y_range.max =
		SANE_FIX(PIXEL_TO_MM(hndl->dev->maxvsize, hndl->dev->maxres));
	...
	hndl->opt[OPT_ENHANCEMENT_GROUP].constraint_type =
	...
	hndl->val[OPT_CUSTOM_GAMMA].w = FALSE;
	...
	hndl->opt[OPT_GAMMA_VECTOR].size = 256 * sizeof(int);
	...
	hndl->opt[OPT_GAMMA_VECTOR_R].size = 256 * sizeof(int);
	...
	hndl->opt[OPT_GAMMA_VECTOR_G].size = 256 * sizeof(int);
	...
	hndl->opt[OPT_GAMMA_VECTOR_B].size = 256 * sizeof(int);
	      ...
	      char * * port, char * * option_ta)
{
	...
	for (port_no = 0; ports[port_no] != NULL; port_no++) {
		for (driver_no = 0; driver_no < MUSTEK_PP_NUM_DRIVERS;
		     driver_no++) {
			if (strcasecmp
			    (Mustek_pp_Drivers[driver_no].driver,
			     *driver) == 0) {
				Mustek_pp_Drivers[driver_no].
					init((*option_ta ==
					      0 ? CAP_NOTHING : CAP_TA),
					     ports[port_no], *name,
					     sane_attach);
				found = 1;
				break;
	...
	if (found == 0) {
		...
		DBG(3,
		    "sane_init: either the driver name ``%s'' is invalid, or no scanner was detected\n",
		    *driver);
	}
...
sane_init(int * version_code, SANE_Auth_Callback authorize)
{
	...
	if (fp == NULL) {
		...
		for (device_no = 0; devices[device_no] != NULL; device_no++) {
			...
			for (driver_no = 0; driver_no < MUSTEK_PP_NUM_DRIVERS;
			     driver_no++) {
				Mustek_pp_Drivers[driver_no].init(CAP_NOTHING,
								  devices
								  [device_no],
								  Mustek_pp_Drivers
								  [driver_no].
				...
				Mustek_pp_Drivers[driver_no].init(CAP_TA,
								  devices
								  [device_no],
								  driver_name,
								  sane_attach);
	...
	while (sanei_config_read(config_line, 1023, fp)) {
		...
		if (strncmp(config_line_ptr, "scanner", 7) == 0) {
			...
			if (name) {
				...
				attach_device(&driver, &name, &port,
					      &option_ta);
			}

			config_line_ptr =
				sanei_config_skip_whitespace(config_line_ptr);
			if (!*config_line_ptr) {
				DBG(1,
				    "sane_init: parse error in line %d after ``scanner''\n",
				    line);
				continue;
			}

			config_line_ptr =
				sanei_config_get_string(config_line_ptr,
							&name);
			if ((name == NULL) || (!*name)) {
				DBG(1,
				    "sane_init: parse error in line %d after ``scanner''\n",
			...
			if (!*config_line_ptr) {
				DBG(1,
				    "sane_init: parse error in line %d after "
				    "``scanner %s''\n", line, name);
				free(name);
				name = 0;
				continue;
			...
			if (!*config_line_ptr) {
				DBG(1,
				    "sane_init: parse error in line %d after ``option''\n",
				    line);
				continue;
			}
			...
			if (!strcmp(optname, "no_epp")) {
				...
				if (name)
					DBG(2,
					    "sane_init: global option found in local scope, "
					    "executing anyway\n");
				free(optname);
				if (optval) {
					DBG(1,
					    "sane_init: unexpected value for option no_epp\n");
					free(optval);
					continue;
				}
				DBG(3, "sane_init: disabling mode EPP\n");
				sanei_pa4s2_options(&pa4s2_options,
						    FALSE);
				...
				DBG(1,
				    "sane_init: parse error in line %d: unexpected "
				    " ``option''\n", line);
				free(optname);
				if (optval)
					free(optval);
			...
			tmpoptions = realloc(cfgoptions,
					     (numcfgoptions +
					      1) * sizeof(cfgoptions[0]));
			if (!tmpoptions) {
				DBG(1,
				    "sane_init: not enough memory for device options\n");
				free_cfg_options(&numcfgoptions, &cfgoptions);
				return SANE_STATUS_NO_MEM;
			}
			...
			DBG(1,
			    "sane_init: parse error at beginning of line %d\n",
			    line);
			continue;
		}
		 ...
		 SANE_Bool local_only __UNUSED__)
{
	...
	if (devarray == NULL) {
		DBG(1,
		    "sane_get_devices: not enough memory for device list\n");
		return SANE_STATUS_NO_MEM;
	}
...
sane_open(const char * devicename, SANE_Handle * handle)
{
	...
	DBG(3, "sane_open: Using device ``%s'' (driver %s v%s by %s)\n",
	    dev->name, dev->func->driver, dev->func->version,
	    dev->func->author);
	...
	if ((status =
	     dev->func->open(dev->port, dev->caps,
			     &fd)) != SANE_STATUS_GOOD) {
	...
	for (i = 0; i < dev->numcfgoptions; ++i) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "sane_open: could not set option %s for device (%s)\n",
...
sane_get_option_descriptor(SANE_Handle handle, int option)
{
	...
	if ((unsigned) option >= NUM_OPTIONS) {
		DBG(2,
		    "sane_get_option_descriptor: option %d doesn't exist\n",
		    option);
		return NULL;
	}
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_GAMMA_VECTOR_B:
			...
			memcpy(val, hndl->val[option].wa,
			       hndl->opt[option].size);
			return SANE_STATUS_GOOD;
		...
		if (!SANE_OPTION_IS_SETTABLE(cap)) {
			DBG(2,
			    "sane_control_option: option can't be set (%s)\n",
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(2,
			    "sane_control_option: constrain_value failed (%s)\n",
			    sane_strstatus(status));
			return status;
		}
		...
		case OPT_CUSTOM_GAMMA:
			...
			if (w == TRUE) {
				...
				if (strcmp(mode, "Grayscale") == 0)
					hndl->opt[OPT_GAMMA_VECTOR].cap &=
						~SANE_CAP_INACTIVE;
				else if (strcmp(mode, "Color") == 0) {
					hndl->opt[OPT_GAMMA_VECTOR].cap &=
						~SANE_CAP_INACTIVE;
					hndl->opt[OPT_GAMMA_VECTOR_R].cap &=
						~SANE_CAP_INACTIVE;
					hndl->opt[OPT_GAMMA_VECTOR_G].cap &=
						~SANE_CAP_INACTIVE;
					hndl->opt[OPT_GAMMA_VECTOR_B].cap &=
						~SANE_CAP_INACTIVE;
				}
			} else {
				hndl->opt[OPT_GAMMA_VECTOR].cap |=
		...
		case OPT_MODE:
		{
			...
			if (strcmp(val, "Lineart") != 0)
				hndl->opt[OPT_CUSTOM_GAMMA].cap &=
					~SANE_CAP_INACTIVE;

			if (hndl->val[OPT_CUSTOM_GAMMA].w == TRUE) {
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (hndl->state != STATE_SCANNING) {
		...
		if ((hndl->dev->caps & CAP_GAMMA_CORRECT)
		    && (hndl->val[OPT_CUSTOM_GAMMA].w == TRUE))
			hndl->do_gamma = TRUE;
		else
			hndl->do_gamma = FALSE;
		...
		if (hndl->dev->caps & CAP_SPEED_SELECT) {
			...
			for (ctr = SPEED_SLOWEST; ctr <= SPEED_FASTEST; ctr++)
				if (strcmp
				    (mustek_pp_speeds[ctr],
				     hndl->val[OPT_SPEED].s) == 0)
					hndl->speed = ctr;
		...
		if (hndl->val[OPT_PREVIEW].w == TRUE) {
			...
			if (hndl->val[OPT_GRAY_PREVIEW].w == TRUE)
				hndl->mode = MODE_GRAYSCALE;
			else {
				hndl->mode = MODE_COLOR;
			}
		...
		hndl->topX = MIN((int)
				 (MM_TO_PIXEL
				  (SANE_UNFIX(hndl->val[OPT_TL_X].w),
				   hndl->dev->maxres) + 0.5),
				 hndl->dev->maxhsize);
		hndl->topY = MIN((int)
				 (MM_TO_PIXEL
				  (SANE_UNFIX(hndl->val[OPT_TL_Y].w),
				   hndl->dev->maxres) + 0.5),
				 hndl->dev->maxvsize);

		hndl->bottomX = MIN((int)
				    (MM_TO_PIXEL
				     (SANE_UNFIX(hndl->val[OPT_BR_X].w),
				      hndl->dev->maxres) + 0.5),
				    hndl->dev->maxhsize);
		hndl->bottomY = MIN((int)
				    (MM_TO_PIXEL
				     (SANE_UNFIX(hndl->val[OPT_BR_Y].w),
				      hndl->dev->maxres) + 0.5),
				    hndl->dev->maxvsize);
		...
		hndl->params.pixels_per_line =
			(hndl->bottomX -
			 hndl->topX) * hndl->res / hndl->dev->maxres;
		...
		hndl->params.last_frame = TRUE;

		hndl->params.lines =
			(hndl->bottomY -
			 hndl->topY) * hndl->res / hndl->dev->maxres;
	} else
		DBG(2,
		    "sane_get_parameters: can't set parameters while scanning\n");
	...
	while (*len < max_len) {
		...
		if (nread < 0) {
			...
			if (errno == EAGAIN) {
				...
				if (*len == 0)
					DBG(3,
					    "sane_read: no data at the moment\n");
				else
