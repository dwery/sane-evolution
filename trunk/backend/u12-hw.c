 ...
 */
static void
u12hw_SelectLampSource(U12_Device * dev)
{
 ...
 */
static void
u12hw_PutToIdleMode(U12_Device * dev)
{
 ...
 */
static void
u12hw_ProgramCCD(U12_Device * dev)
{
	SANE_Byte *reg_val;
 ...
 */
static void
u12hw_ButtonSetup(U12_Device * dev, SANE_Byte numButtons)
{
 ...
 */
static void
u12hw_InitiateComponentModel(U12_Device * dev)
{
 ...
 */
static SANE_Status
u12hw_InitAsic(U12_Device * dev, SANE_Bool shading)
{
	SANE_Byte rb[6];
	int c;
	...
	DBG(_DBG_INFO,
	    "* PCB-ID=0x%02x, CCD-ID=0x%02x, DAC-TYPE=0x%02x\n",
	...
	u12io_DataToRegister(dev, REG_MODEL1CONTROL,
			     dev->regs.RD_Model1Control);
#else
	c = 0;
 ...
 */
static void
u12hw_ControlLampOnOff(U12_Device * dev)
{
	SANE_Byte lampStatus;

	dev->warmupNeeded = TRUE;
 ...
 */
static void
u12hw_SetGeneralRegister(U12_Device * dev)
{
	DBG(_DBG_INFO, "u12hw_SetGeneralRegister()\n");

	dev->scan.motorBackward = FALSE;
	...
	dev->regs.RD_Motor0Control =
		_MotorOn | _MotorHQuarterStep | _MotorPowerEnable;
 ...
 */
static void
u12hw_SetupPreviewCondition(U12_Device * dev)
{
	int i, c;
	u_long channel;
	SANE_Byte rb[100];
	...
	dev->regs.RD_Origin =
		(u_short) (dev->adj.leftNormal * 2 + _DATA_ORIGIN_X);
	...
	_SET_REG(rb, c, REG_EXTENDEDLINECONTROL, dev->regs.RD_ExtLineControl);
 ...
 */
static void
u12hw_SetupScanningCondition(U12_Device * dev)
{
	TimerDef timer;
	u_long channel;
	int c;
	...
	dev->regs.RD_Motor0Control = _FORWARD_MOTOR;
	dev->regs.RD_StepControl = _MOTOR0_SCANSTATE;
	dev->regs.RD_ModeControl =
		_ModeScan /*(_ModeScan | _ModeFifoGSel) */ ;
	...
	if (dev->DataInf.wPhyDataType == COLOR_BW)
		dev->regs.RD_Pixels =
			(u_short) dev->DataInf.dwAsicBytesPerPlane;
	else
		dev->regs.RD_Pixels =
			(u_short) dev->DataInf.dwAppPixelsPerLine;
	...
	for (channel = _NUMBER_OF_SCANSTEPS, pState =
	     dev->bufs.b1.pReadBuf; channel;
	     channel -= dev->scan.dwInterval) {
		*pState = state;
 ...
 */
static SANE_Status
u12hw_CheckDevice(U12_Device * dev)
{
#ifndef _FAKE_DEVICE
	SANE_Byte tmp;
	SANE_Byte rb[8];
 ...
 */
static void
u12hw_CancelSequence(U12_Device * dev)
{
	int c = 0;
	SANE_Byte rb[6];
 ...
 */
static SANE_Status
u12hw_WarmupLamp(U12_Device * dev)
{
	TimerDef timer;
...
usb_LampTimerIrq(int sig)
{
	...
	if (-1 == dev_xxx->fd) {
		...
		if (SANE_STATUS_GOOD ==
		    sanei_usb_open(dev_xxx->sane.name, &handle)) {
			dev_xxx->fd = handle;
		}
	}
 ...
 */
static void
u12hw_StartLampTimer(U12_Device * dev)
{
 ...
 */
static void
u12hw_StopLampTimer(U12_Device * dev)
{
	sigset_t block, pause_mask;
