...
usb_low_init(ma1017 ** chip_address)
{
	...
	*chip_address = chip;
	...
	for (i = 0; i < 32; i++)
		chip->is_transfer_table[i] = FALSE;
		      ...
		      SANE_Bool is_move_motor, SANE_Bool is_transfer)
{
	...
	if ((unsigned int) index > 31) {
		DBG(7, "usb_low_set_cmt_table: CMT index (%d) exceed 31",
		    index);
		return SANE_STATUS_INVAL;
	}
...
usb_low_start_cmt_table(ma1017 * chip)
{
	...
	data_field[0] =
		0x02 | chip->append | chip->test_sram | chip->fix_pattern;
	data_field[1] = 2;
	...
	if (status != SANE_STATUS_GOOD || n != 2) {
		DBG(3,
		    "usb_low_start_cmt_table: can't write, wanted 2 bytes, "
		    "wrote %lu bytes\n", (unsigned long int) n);
...
usb_low_stop_cmt_table(ma1017 * chip)
{
	...
	data_field[0] =
		0x01 | chip->append | chip->test_sram | chip->fix_pattern;
	data_field[1] = 2;
	data_field[1] |= 0x80;
	n = 2;
	status = sanei_usb_write_bulk(chip->fd, data_field, &n);
	if (status != SANE_STATUS_GOOD || n != 2) {
		DBG(3,
		    "usb_low_stop_cmt_table: couldn't write, wanted 2 bytes, wrote "
		    "%lu bytes\n", (unsigned long int) n);
		return SANE_STATUS_IO_ERROR;
	}
	chip->total_write_urbs++;
	...
	if (status != SANE_STATUS_GOOD || n != 1) {
		DBG(3,
		    "usb_low_stop_cmt_table: couldn't read, wanted 1 byte, got %lu "
		    "bytes\n", (unsigned long int) n);
		return SANE_STATUS_IO_ERROR;
	}
...
usb_low_set_cmt_table_length(ma1017 * chip, SANE_Byte table_length)
{
	...
	chip->cmt_table_length_word = (int) table_length;
...
usb_low_set_cmt_second_position(ma1017 * chip, SANE_Byte position)
{
	...
	if (chip->is_rowing) {
		DBG(3,
		    "usb_low_set_cmt_second_position: stop rowing first\n");
		return SANE_STATUS_INVAL;
	}
	if (position > 31) {
		DBG(3,
		    "usb_low_set_cmt_second_position: length: %d exceeds 31\n",
		    (int) position);
		return SANE_STATUS_INVAL;
	}

	chip->cmt_second_pos = position;
	chip->cmt_second_pos_word = (int) (position);
...
SANE_Status
usb_low_set_ccd_width(ma1017 * chip, int ccd_width)
{
	SANE_Status status;
	SANE_Byte data, reg_no;
...
SANE_Status
usb_low_set_dummy(ma1017 * chip, int dummy)
{
	SANE_Status status;
	SANE_Byte data, reg_no;
...
usb_low_get_a12(ma1017 * chip, SANE_Byte * value)
{
	...
	chip->byte_width =
		(chip->byte_width & 0x3f00) + ((int) pattern);
	chip->soft_resample =
		(chip->soft_resample == 0) ? 1 : chip->soft_resample;
	chip->get_row = (chip->soft_resample == 1)
...
SANE_Status
usb_low_set_image_byte_width(ma1017 * chip, int row_size)
{
	SANE_Byte data, reg_no;
	SANE_Status status;
...
SANE_Status
usb_low_set_soft_resample(ma1017 * chip, int soft_resample)
{
	SANE_Byte data, reg_no;
	SANE_Status status;
...
/* A14 + A30W */
SANE_Status
usb_low_set_cmt_loop_count(ma1017 * chip, int loop_count)
{
	SANE_Byte data, reg_no;
	SANE_Status status;
...
usb_low_enable_motor(ma1017 * chip, SANE_Bool is_enable)
{
	...
	data = chip->motor_enable | chip->motor_movement
		| chip->motor_direction | chip->motor_signal | chip->
		motor_home;
	reg_no = 15;
	RIE(usb_low_write_reg(chip, reg_no, data));
...
usb_low_start_rowing(ma1017 * chip)
{
	...
	if (chip->loop_count == 0) {
		DBG(3,
		    "usb_low_start_rowing loop_count hasn't been set yet\n");
		return SANE_STATUS_INVAL;
	}
	if (chip->cmt_table_length_word == 0) {
		DBG(3,
		    "usb_low_start_rowing: cmt_table_length_word hasn't been set "
		    "yet\n");
		return SANE_STATUS_INVAL;
	}
	if (chip->cmt_table_length_word <= chip->cmt_second_pos_word) {
		DBG(3,
		    "usb_low_start_rowing: cmt_second_pos_word cannot be larger "
	...
	chip->total_lines =
		((int) (chip->loop_count - 1)) * line_of_second +
		line_of_first;
...
SANE_Status
usb_low_get_row(ma1017 * chip, SANE_Byte * data, int * lines_left)
{
	SANE_Status status;
			 ...
			 int * lines_left)
{
	...
	if (chip->lines_left <= 1) {
		RIE(usb_low_read_rows
		    (chip, resample_buffer, chip->byte_width));

		if ((chip->sensor == ST_CANON600)
		    && (chip->pixel_depth == 0x20)) {
			pixel_temp =
				(int *) malloc(6 * 1024 *
						     sizeof(int));
			...
			for (i = 0; i < j; i += chip->soft_resample * 2) {
				data[k] =
					(SANE_Byte) (pixel_temp[i] & 0x00ff);
				k++;
				data[k] =
					(SANE_Byte) ((pixel_temp[i] & 0x0f00)
						     >> 4);
				data[k] |=
					(SANE_Byte) ((pixel_temp[i + 2] &
						      0x0f00) >> 8);
				k++;
				data[k] =
					(SANE_Byte) (pixel_temp[i + 2] &
			...
			for (i = 0; i < chip->byte_width;
			     i += chip->soft_resample)
...
SANE_Status
usb_low_read_rows(ma1017 * chip, SANE_Byte * data, int byte_count)
{
	...
	if (!(chip->is_opened)) {
		DBG(3, "usb_low_read_rows: is_opened==FALSE\n");
		return SANE_STATUS_INVAL;
	}
	if (!(chip->is_rowing)) {
		DBG(3, "usb_low_read_rows: is_rowing==FALSE\n");
		return SANE_STATUS_INVAL;
	}
	...
	while ((int) bytes_total < byte_count) {
		status = sanei_usb_read_bulk(chip->fd,
					     (SANE_Byte *) (data +
							    bytes_total), &n);
		if (status != SANE_STATUS_GOOD) {
			DBG(7,
			    "usb_low_read_rows: problems during read: %s -- exiting\n",
			    sane_strstatus(status));
			return status;
		}
		...
		bytes_total += n;
		if ((int) bytes_total != byte_count) {
			DBG(7, "usb_low_read_rows:  wanted %d, got %d "
			    "bytes (%d in total) -- retrying\n", byte_count,
			    (int) n, (int) bytes_total);
		}
		n = MIN((byte_count - (int) bytes_total),
			chip->max_block_size);
	}
...
usb_low_write_reg(ma1017 * chip, SANE_Byte reg_no, SANE_Byte data)
{
	...
	if (status != SANE_STATUS_GOOD || n != 2) {
		DBG(3,
		    "usb_low_write_reg: couldn't write, tried to write %d, "
	...
	if (status == SANE_STATUS_GOOD) {
		if (devvendor == 0x055f) {
			...
			default:
				...
				DBG(3,
				    "usb_low_identify_scanner: unknown product id: "
				    "0x%04x\n", devproduct);
				return SANE_STATUS_INVAL;
				break;
			}
			...
			if (scanner_type)
				*scanner_type = devtype;
			DBG(3,
			    "usb_low_identify_scanner: unknown vendor id: 0x%04d\n",
			    devvendor);
			return SANE_STATUS_INVAL;
		}
	}
...
usb_low_open(ma1017 * chip, const char * devname)
{
	...
	if (status == SANE_STATUS_GOOD) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(3,
			    "usb_low_open: device `%s' doesn't look like a supported "
			    "scanner\n", devname);
			...
			if (scanner_type == MT_UNKNOWN) {
				DBG(3,
				    "usb_low_open: device `%s' can't be identified\n",
				    devname);
			} else if (scanner_type != chip->scanner_type) {
				DBG(3,
				    "usb_low_open: device `%s' is supported but"
...
usb_low_close(ma1017 * chip)
{
	...
	if (chip->fd >= 0) {
		...
		if ((chip->total_write_urbs % 2) == 1)
			usb_low_set_fix_pattern(chip, FALSE);
		sanei_usb_close(chip->fd);
		chip->fd = -1;
	}
	chip->is_opened = FALSE;
