...
check_gt6801(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-6801 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	...
	result = usb_control_msg(handle, 0x40, 0x01, 0x2010, 0x3f40, req, 64,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
	...
	result = usb_control_msg(handle, 0xc0, 0x01, 0x2011, 0x3f00, req, 64,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
...
check_gt6816(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_PER_INTERFACE)
		...
		if (verbose > 2)
			printf("    this is not a GT-6816 (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x110) {
		if (verbose > 2)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-6816 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-6816 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[1].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[1].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[1].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[1].bInterval);
		return 0;
	...
	result = usb_control_msg(handle, 0x40, 0x04, 0x2012, 0x3f40, req, 64,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
			printf("    Couldn't send write control message (%s)\n", strerror(errno));
...
check_gt8911(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_PER_INTERFACE)
		...
		USB_CLASS_VENDOR_SPEC)) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 1, bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x110) {
		if (verbose > 2)
	...
	if (dev->descriptor.bDeviceSubClass != 0x00) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 3, bDeviceSubClass = 0x%x)\n", dev->descriptor.bDeviceSubClass);
		return 0;
	}
	if (dev->descriptor.bDeviceProtocol != 0x00) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 4, bDeviceProtocol = 0x%x)\n", dev->descriptor.bDeviceProtocol);
		return 0;
	}
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 6, bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
	     bEndpointAddress != 0x02)
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 7, bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[1].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[1].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[1].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[1].bInterval);
		return 0;
	...
	if ((dev->config[0].interface[1].altsetting[2].endpoint[0].
		...
		bInterval != 0x01)) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 10, bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[1].altsetting[2].endpoint[0].bEndpointAddress, dev->config[0].interface[1].altsetting[2].endpoint[0].bmAttributes, dev->config[0].interface[1].altsetting[2].endpoint[0].wMaxPacketSize, dev->config[0].interface[1].altsetting[2].endpoint[0].bInterval);
		return 0;
	}
	if ((dev->config[0].interface[1].altsetting[2].endpoint[1].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 11, bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[1].altsetting[2].endpoint[1].bEndpointAddress, dev->config[0].interface[1].altsetting[2].endpoint[1].bmAttributes, dev->config[0].interface[1].altsetting[2].endpoint[1].wMaxPacketSize, dev->config[0].interface[1].altsetting[2].endpoint[1].bInterval);
		return 0;
	...
	result = usb_control_msg(handle, 0xc0, 0x10, 0x41, 0x0000, req, 64,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 12, couldn't send read control message (%s))\n", strerror(errno));
		finish_interface(handle);
		return 0;
	}
	result = usb_control_msg(handle, 0xc0, 0x10, 0x05, 0x0000, req, 64,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
			printf("    this is not a GT-8911 (check 13, couldn't send read control message (%s)\n", strerror(errno));
		finish_interface(handle);
		return 0;
	}
	/* tested on model hardware version 0xffffffc0, firmware version 0x10)) */
	if (verbose > 2)
		printf("    Check 14, control message (hardware version %0x / firmware version %0x)\n", req[0], req[1]);
...
check_ma1017(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_PER_INTERFACE)
		...
		if (verbose > 2)
			printf("    this is not a MA-1017 (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a MA-1017 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[2].
		...
		bInterval != 0x01)) {
		if (verbose > 2)
			printf("    this is not a MA-1017 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[2].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[2].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[2].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[2].bInterval);
		return 0;
	}
...
check_ma1015(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a MA-1015 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
	     bEndpointAddress != 0x02)
...
check_ma1509(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		wMaxPacketSize != 0x40)) {
		if (verbose > 2)
			printf("    this is not a MA-1509 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
	     bEndpointAddress != 0x02)
		...
		wMaxPacketSize != 0x08)) {
		if (verbose > 2)
			printf("    this is not a MA-1509 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[1].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[1].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[1].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].endpoint[2].
	     bEndpointAddress != 0x83)
		...
		bInterval != 0x10)) {
		if (verbose > 2)
			printf("    this is not a MA-1509 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[2].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[2].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[2].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[2].bInterval);
		return 0;
	}
...
check_merlin(struct usb_device *dev)
{
	...
	if (((dev->descriptor.bDeviceClass != USB_CLASS_VENDOR_SPEC)
		...
		USB_CLASS_VENDOR_SPEC)) {
		if (verbose > 2)
			printf("    this is not a LM983x (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if ((dev->descriptor.bcdUSB != 0x110)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x10)) {
		if (verbose > 2)
			printf("    this is not a LM983x (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
gl646_write_reg(usb_dev_handle * handle, unsigned char reg, unsigned char val)
{
	...
	result = usb_control_msg(handle, 0x00, 0x00, 0x83, 0x00,
				 (char *) &reg, 0x01, TIMEOUT);
	if (result < 0)
		return 0;

	result = usb_control_msg(handle, 0x00, 0x00, 0x85, 0x00,
				 (char *) &val, 0x01, TIMEOUT);
	if (result < 0)
		return 0;
...
gl646_read_reg(usb_dev_handle * handle, unsigned char reg, unsigned char *val)
{
	...
	result = usb_control_msg(handle, 0x80, 0x00, 0x84, 0x00, (char *) val,
				 0x01, TIMEOUT);
	if (result < 0)
		return 0;
...
check_gl646(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_PER_INTERFACE)
		...
		0x10)) {
		if (verbose > 2)
			printf("    this is not a GL646 (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x110) {
		if (verbose > 2)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x0)) {
		if (verbose > 2)
			printf("    this is not a GL646 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_gl646_hp(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != 0xff)
		...
		0xff)) {
		if (verbose > 2)
			printf("    this is not a GL646_HP (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x110) {
		if (verbose > 2)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x0)) {
		if (verbose > 2)
			printf("    this is not a GL646_HP (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_gl660_gl646(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_VENDOR_SPEC)
		...
		USB_CLASS_PER_INTERFACE)) {
		if (verbose > 2)
			printf("    this is not a GL660+GL646 (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x0)) {
		if (verbose > 2)
			printf("    this is not a GL660+GL646 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_gl841(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_VENDOR_SPEC)
		...
		USB_CLASS_VENDOR_SPEC)) {
		if (verbose > 2)
			printf("    this is not a GL841 (bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x200) {
		if (verbose > 2)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x0)) {
		if (verbose > 2)
			printf("    this is not a GL841 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_icm532b(struct usb_device *dev)
{
	...
	if ((dev->descriptor.bDeviceClass != USB_CLASS_VENDOR_SPEC)
		...
		0xff)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 1, bDeviceClass = %d, bInterfaceClass = %d)\n", dev->descriptor.bDeviceClass, dev->config[0].interface[0].altsetting[0].bInterfaceClass);
		return 0;
	}
	if (dev->descriptor.bcdUSB != 0x110) {
		if (verbose > 2)
	...
	if (dev->descriptor.bDeviceSubClass != 0xff) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 3, bDeviceSubClass = 0x%x)\n", dev->descriptor.bDeviceSubClass);
		return 0;
	}
	if (dev->descriptor.bDeviceProtocol != 0xff) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 4, bDeviceProtocol = 0x%x)\n", dev->descriptor.bDeviceProtocol);
		return 0;
	}
	...
	if (dev->config[0].interface[0].altsetting[0].endpoint[0].
	    bmAttributes != 0x01) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 7, bEndpointAddress = %d)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[0].bAlternateSetting !=
	     0x00)
	    || (dev->config[0].interface[0].altsetting[0].endpoint[0].
		wMaxPacketSize != 0x00)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 8, bAlternateSetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[0].bAlternateSetting, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize);
		return 0;
	}

	if ((dev->config[0].interface[0].altsetting[1].bAlternateSetting !=
	     0x01)
	    || (dev->config[0].interface[0].altsetting[1].endpoint[0].
		wMaxPacketSize != 0x100)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 9, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[1].bAlternateSetting, dev->config[0].interface[0].altsetting[1].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[2].bAlternateSetting !=
	     0x02)
	    || (dev->config[0].interface[0].altsetting[2].endpoint[0].
		wMaxPacketSize != 0x180)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 10, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[2].bAlternateSetting, dev->config[0].interface[0].altsetting[2].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[3].bAlternateSetting !=
	     0x03)
	    || (dev->config[0].interface[0].altsetting[3].endpoint[0].
		wMaxPacketSize != 0x200)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 11, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[3].bAlternateSetting, dev->config[0].interface[0].altsetting[3].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[4].bAlternateSetting !=
	     0x04)
	    || (dev->config[0].interface[0].altsetting[4].endpoint[0].
		wMaxPacketSize != 0x280)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 12, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[4].bAlternateSetting, dev->config[0].interface[0].altsetting[4].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[5].bAlternateSetting !=
	     0x05)
	    || (dev->config[0].interface[0].altsetting[5].endpoint[0].
		wMaxPacketSize != 0x300)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 13, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[5].bAlternateSetting, dev->config[0].interface[0].altsetting[5].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[6].bAlternateSetting !=
	     0x06)
	    || (dev->config[0].interface[0].altsetting[6].endpoint[0].
		wMaxPacketSize != 0x380)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 14, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[6].bAlternateSetting, dev->config[0].interface[0].altsetting[6].endpoint[0].wMaxPacketSize);
		return 0;
	}
	if ((dev->config[0].interface[0].altsetting[7].bAlternateSetting !=
	     0x07)
	    || (dev->config[0].interface[0].altsetting[7].endpoint[0].
		wMaxPacketSize != 0x3ff)) {
		if (verbose > 2)
			printf("    this is not a ICM532B (check 15, bAlternatesetting = 0x%x, wMaxPacketSize = 0x%x)\n", dev->config[0].interface[0].altsetting[7].bAlternateSetting, dev->config[0].interface[0].altsetting[7].endpoint[0].wMaxPacketSize);
		return 0;
	}
...
check_pv8630_lm9830(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a PV8630/LM9830 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	/* Endpoint 1 */
	if ((dev->config[0].interface[0].altsetting[0].endpoint[1].
	...
	result = usb_control_msg(handle, 0x40, 0x01, 0x38, 0x01, NULL, 0,
				 TIMEOUT);
	if (result < 0) {
		if (verbose > 2)
	...
	result = usb_control_msg(handle, 0x40, 0x01, 0x0f, 0x00, NULL, 0,
				 TIMEOUT);
	if (result < 0) {
		if (verbose > 2)
	...
	result = usb_control_msg(handle, 0xc0, 0x00, 0, 0x00, &data, 1,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
...
check_m011(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a M011 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	...
	result = usb_control_msg(handle, 0x40, 0x08, 0x34, 0x00, &data, 1,
				 TIMEOUT);
	if (result < 0) {
		if (verbose > 2)
	...
	result = usb_control_msg(handle, 0xc0, 0x00, 0x34, 0x00, &data, 1,
				 TIMEOUT);
	if (result <= 0) {
		if (verbose > 2)
...
static int
rts88xx_read_reg(usb_dev_handle * handle, unsigned char *req,
		 unsigned char *res, int size)
{
	int result;

	result = usb_bulk_write(handle, 0x02, (char *) req, 0x04, TIMEOUT);
	if (result < 0)
		return 0;

	result = usb_bulk_read(handle, 0x81, (char *) res, size, TIMEOUT);
	if (result < 0)
		return 0;
...
check_rts8858c(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a rts8858c (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_rts88x1(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a rts8801/rts8891 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
...
check_sq113(struct usb_device *dev)
{
	...
	if (dev->config[0].interface[0].altsetting[0].bInterfaceSubClass !=
	    255) {
		if (verbose > 2)
			printf("    this is not a SQ113 (bInterfaceSubClass = %d)\n", dev->config[0].interface[0].altsetting[0].bInterfaceSubClass);
		return 0;
	}
	if (dev->config[0].interface[0].altsetting[0].bInterfaceProtocol !=
	    255) {
		if (verbose > 2)
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a SQ113 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	...
	result = usb_control_msg(handle, 0x40, 0x01, 0xb0, 0, (char *) buffer,
				 4, TIMEOUT);
	if (result < 0) {
		if (verbose > 2)
	...
	buffer[2] = 0x8b;
	buffer[3] = 0x8b;
	result = usb_control_msg(handle, 0x40, 0x01, 0x04, 0x8b,
	...
	result = usb_control_msg(handle, 0xc0, 0x01, 0x07, 0, (char *) buffer,
				 4, TIMEOUT);
	if (result < 0) {
		if (verbose > 2)
			printf("    Couldn't read register (%s)\n",
...
check_rts8822(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a RTS8822 (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	...
	result = usb_control_msg(handle, 0xc0, 0x04, 0xfe11, 0x100, data,
				 0x02, TIMEOUT);
...
check_hp5590(struct usb_device *dev)
{
	...
	if ((dev->config[0].interface[0].altsetting[0].endpoint[0].
		...
		bInterval != 0x00)) {
		if (verbose > 2)
			printf("    this is not a HP5550/5590/7650 chipset (bEndpointAddress = 0x%x, bmAttributes = 0x%x, " "wMaxPacketSize = 0x%x, bInterval = 0x%x)\n", dev->config[0].interface[0].altsetting[0].endpoint[0].bEndpointAddress, dev->config[0].interface[0].altsetting[0].endpoint[0].bmAttributes, dev->config[0].interface[0].altsetting[0].endpoint[0].wMaxPacketSize, dev->config[0].interface[0].altsetting[0].endpoint[0].bInterval);
		return 0;
	}
	...
	if (status != 0x01) {
		if (verbose > 2)
			printf("    Didn't get correct confirmation for USB-in-USB command " "(needed: 0x01, got: 0x%02x\n", status);
		finish_interface(handle);
		return NULL;
	}
	...
	while (len) {
		...
		result = usb_control_msg(handle,
					 ...
					 0x04, 0x90, 0x00, (char *) ptr,
					 next_packet_size, TIMEOUT);
		if (result < 0) {
			if (verbose > 2)
