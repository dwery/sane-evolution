...
init_dc210(DC210 * camera)
{
	...
	if ((camera->fd = open(camera->tty_name, O_RDWR)) == -1) {
		DBG(2,
		    "init_dc210: error: could not open %s for read/write\n",
		    camera->tty_name);
		return -1;
	}
	/*
...
# elif defined(TCSBRK)
	...
	if (send_pck(camera->fd, init_pck) == -1) {
		...
		for (speed_index = NELEMS(speeds) - 1; speed_index > 0;
		     speed_index--) {
			int x;
			...
			if (tcsetattr(camera->fd, TCSANOW, &tty_new) == -1) {
				DBG(2,
				    "init_dc210: error: could not set attributes\n");
				return -1;
			}
			for (x = 0; x < 3; x++)
				if (send_pck(camera->fd, init_pck) != -1)
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		case DC210_OPT_IMAGE_NUMBER:
			Camera.current_picture_number = *(int *) value;
			myinfo |= SANE_INFO_RELOAD_PARAMS;
			/* get the image's resolution */

			if (Camera.
			    Pictures[Camera.current_picture_number -
				     1].low_res) {
				parms.bytes_per_line = 640 * 3;
				parms.pixels_per_line = 640;
				parms.lines = 480;
		...
		case DC210_OPT_THUMBS:
			dc210_opt_thumbnails = !!*(int *) value;
			myinfo |= SANE_INFO_RELOAD_PARAMS;
 ...
 */
	...
	case SANE_ACTION_GET_VALUE:
		...
		case DC210_OPT_THUMBS:
			*(int *) value = dc210_opt_thumbnails;
			break;

		case DC210_OPT_SNAP:
			*(int *) value = dc210_opt_snap;
			break;

		case DC210_OPT_LOWRES:
			*(int *) value = dc210_opt_lowres;
			break;

		case DC210_OPT_ERASE:
			*(int *) value = dc210_opt_erase;
			break;
...
sane_start(SANE_Handle handle)
{
	...
	if (dc210_opt_thumbnails) {
		...
		thumb_pck[3] =
			(unsigned char) Camera.current_picture_number - 1;
		thumb_pck[4] = 1;
	...
	if (dc210_opt_thumbnails) {
		...
		*length = 0;
		if (!(bytes_in_buffer - bytes_read_from_buffer)) {
			if (read_data
			    (Camera.fd, (unsigned char *) buffer,
			     1024) == -1) {
		...
		if (cinfo.output_scanline >= cinfo.output_height) {
			...
			if (end_of_data(Camera.fd) == -1) {
				DBG(2,
				    "sane_read: error: end_of_data returned -1\n");
				return SANE_STATUS_INVAL;
			}
			if (dc210_opt_erase) {
