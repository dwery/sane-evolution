...
/*
   $Id: teco3.c,v 1.8 2005-09-19 19:57:48 fzago-guest Exp $
...
teco_query_sense(Teco_Scanner * dev)
{
	...
	status = sanei_scsi_cmd2(dev->sfd, cdb.data, cdb.len, NULL, 0, buf,
				 &size);
...
teco_fill_image(Teco_Scanner * dev)
{
	...
	if (dev->scan_mode == TECO_COLOR) {
		memmove(dev->image, dev->image + dev->image_begin,
			dev->raster_ahead);
	}
	...
	while (dev->real_bytes_left) {
		...
		if (dev->raster_ahead + dev->image_end +
		    dev->params.bytes_per_line > dev->image_size) {
