...
static SANE_Status sense_handler(int scsi_fd, unsigned char *result,
				 void *arg);
...
request_sense_parse(unsigned char *sensed_data)
{
	...
	case 0x2:
		if ((0x4 == asc) && (0x1 == ascq)) {
			DBG(10,
			    "\t%d/%d/%d: Logical unit is in process of becomming ready\n",
			    sense, asc, ascq);
	...
	case 0x5:
		if ((0x00 == asc) && (0x5 == ascq))
			...
			DBG(1,
			    "\t%d/%d/%d: Invalid Field in Parameter List\n",
			    sense, asc, ascq);
			...
			DBG(1,
			    "\t%d/%d/%d: Saving Parameters Not Supported\n",
			    sense, asc, ascq);
		else if ((0x3d == asc) && (0x00 == ascq))
			DBG(1,
			    "\t%d/%d/%d: Invalid Bits In Identify Message\n",
			    sense, asc, ascq);
		else
			DBG(1, "\tUnknown - Sense=%d, ASC=%d, ASCQ=%d\n",
			    sense, asc, ascq);
	...
	case 0x6:
		if ((0x29 == asc) && (0x0 == ascq))
			DBG(1,
			    "\t%d/%d/%d: Power On, Reset, or Bus Device Reset Occured\n",
			    sense, asc, ascq);
	...
	case 0xb:
		if ((0x43 == asc) && (0x0 == ascq))
			...
			DBG(1,
			    "\t%d/%d/%d: Initiator Detected Error Message Received\n",
			    sense, asc, ascq);
...
wait_scanner(Coolscan_t * s)
{
	...
	while (ret != 0) {
		...
		if (ret == SANE_STATUS_DEVICE_BUSY) {
			...
			if (cnt++ > 40) {	/* 20 sec. max (prescan takes up to 15 sec. */
				DBG(1,
				    "wait_scanner: scanner does NOT get ready\n");
				return -1;
			}
...
coolscan_grab_scanner(Coolscan_t * s)
{
	...
	wait_scanner(s);	/* wait for scanner ready, if not print 
				   sense and return 1 */
	ret = do_scsi_cmd(s->sfd, reserve_unit.cmd, reserve_unit.size, NULL,
			  0);
	if (ret)
		return ret;
...
coolscan_autofocus_LS30(Coolscan_t * s)
{
	...
	do_scsi_cmd(s->sfd, s->buffer, autofocusLS30.size + 9, NULL, 0);
	/* Trashes when used in combination with scsi-driver AM53C974.o  */
	do_scsi_cmd(s->sfd, command_c1.cmd, command_c1.size, NULL, 0);
...
coolscan_object_discharge(Coolscan_t * s)
{
	...
	ret = do_scsi_cmd(s->sfd, s->buffer, object_position.size, NULL, 0);
	wait_scanner(s);
...
coolscan_set_window_param_LS20(Coolscan_t * s, int prescan)
{
	...
	if (prescan) {
		...
		if (1 == s->LS) {	/* Analog gamma reserved on LS-1000 */
	...
	DBG(10,
	    "\tnegative=%d, dropoutcolor=%d, preview=%d, transfermode=%d, gammasel=%d\n",
	...
	ret = do_scsi_cmd(s->sfd, s->buffer, set_window.size +
			  window_parameter_data_block.size +
			  window_descriptor_block.size, NULL, 0);
	DBG(10, "window set.\n");
	return ret;
}
...
coolscan_set_window_param_LS30(Coolscan_t * s, int wid, int prescan)
{
	...
	wait_scanner(s);
	memset(buffer_r, '\0', max_WDB_size);	/* clear buffer */
	memcpy(buffer_r, window_descriptor_block_LS30.cmd, window_descriptor_block_LS30.size);	/* copy preset data */
	...
	switch (wid) {
	case 1:
		set_gain_LS30(buffer_r, (s->exposure_R * s->pretv_r) / 50);
		break;
	case 2:
		set_gain_LS30(buffer_r, (s->exposure_G * s->pretv_g) / 50);
		break;
	case 3:
		set_gain_LS30(buffer_r, (s->exposure_B * s->pretv_b) / 50);
		break;
	}
	...
	ret = do_scsi_cmd(s->sfd, s->buffer, set_window.size +
			  window_parameter_data_block.size +
			  window_descriptor_block_LS30.size, NULL, 0);
	DBG(10, "window set.\n");
	return ret;
}
...
coolscan_set_window_param(Coolscan_t * s, int prescan)
{
	...
	if (s->LS < 2) {	/* distinquish between old and new scanners */
		...
		do_scsi_cmd(s->sfd, commande1.cmd, commande1.size, s->buffer,
			    0x0d);
		wait_scanner(s);
		wait_scanner(s);
...
coolscan_get_window_param_LS30(Coolscan_t * s, int wid, int prescanok)
{
	...
	if (prescanok) {
		switch (wid) {
		case 1:
			s->pretv_r = get_gain_LS30(buf);
			break;
		case 2:
			s->pretv_g = get_gain_LS30(buf);
			break;
		case 3:
			s->pretv_b = get_gain_LS30(buf);
			break;
		}
	}
...
do_prescan_now(Coolscan_t * scanner)
{
	...
	if (scanner->scanning == TRUE)
		return SANE_STATUS_DEVICE_BUSY;

	if (scanner->sfd < 0) {	/* first call */
	...
	if (coolscan_check_values(scanner) != 0) {	/* Verify values */
...
static int
send_one_LUT(Coolscan_t * s, int * LUT, int reg)
{
	int i;
	short lutval;
	short bytesperval;
	...
	return do_scsi_cmd(s->sfd, gamma,
			   send.size + s->lutlength * bytesperval, NULL, 0);
}
...
static int
coolscan_read_data_block(Coolscan_t * s, unsigned int datatype,
			 unsigned int length)
{
	int r;
...
coolscan_identify_scanner(Coolscan_t * s)
{
	...
	if (get_inquiry_periph_devtype(s->buffer) !=
	...
	if (get_inquiry_additional_length(s->buffer) >= 0x1f) {
		...
		for (i = 0; i < known_scanners; i++) {
			if (!strncmp
			    ((char *) product, scanner_str[i],
			     strlen(scanner_str[i]))) {
				s->LS = i;
				return 0;
			}
		}
...
coolscan_check_values(Coolscan_t * s)
{
	...
	if (s->asf != 0) {
		if (s->autofeeder == 0) {
			DBG(1,
			    "ERROR: ASF-MODE NOT SUPPORTED BY SCANNER, ABORTING\n");
			return (1);
		}
	}
...
get_inquiery_part_LS30(Coolscan_t * s, unsigned char part)
{
	...
	size = 4;
	set_inquiry_return_size(inquiry.cmd, size);
	ret = do_scsi_cmd(s->sfd, inquiry.cmd, inquiry.size, s->buffer, size);
...
coolscan_read_var_data_block(Coolscan_t * s, int datatype)
{
	...
	r = do_scsi_cmd(s->sfd, sread.cmd, sread.size, s->buffer, size);
	size = s->buffer[5];
	set_R_xfer_length(sread.cmd, size);
	r = do_scsi_cmd(s->sfd, sread.cmd, sread.size, s->buffer, size);
	return ((r != 0) ? -1 : size);
...
get_feeder_type_LS30(Coolscan_t * s)
{
	...
	if (s->feeder == STRIP_FEEDER) {
		...
		if (size >= 4) {
			s->numima = s->buffer[3];
			if (s->numima > 6)
				s->numima = 6;	/* limit to 6 images for now */
			if (s->numima > (size - 4) / 16)
				s->numima = (size - 4) / 16;
			ptr = s->buffer + 4;
...
coolscan_get_inquiry_values(Coolscan_t * s)
{
	...
	if (s->inquiry_len < 36) {
		DBG(1,
		    "WARNING: inquiry return block is unexpected short (%d instead of 36).\n",
		    s->inquiry_len);
	}
...
coolscan_initialize_values(Coolscan_t * s)
{
	...
	if (coolscan_test_little_endian() == TRUE) {
...
attach_scanner(const char *devicename, Coolscan_t ** devp)
{
	...
	for (dev = first_dev; dev; dev = dev->next) {
		if (strcmp(dev->sane.name, devicename) == 0) {
			...
			DBG(5,
			    "attach_scanner: scanner already attached (is ok)!\n");
			return SANE_STATUS_GOOD;
		}
	}
...
/*  static int irmulr= -34*25; */
	...
	for (i = 0; i < s->lutlength; i++) {
		if (s->gamma_bind) {
			uselutr = uselutg = uselutb = useluti =
				s->gamma[i] / div;
		} else {
		...
		s->lutr[uselutr] =
			(int) (irmulr * pow((double) i, (double) 0.333333));
		s->lutg[uselutg] =
			(int) (irmulg * pow((double) i, (double) 0.333333));
		s->lutb[uselutb] =
			(int) (irmulb * pow((double) i, (double) 0.333333));
		s->luti[useluti] =
			(int) (irmuli * pow((double) i, (double) 0.333333));
		if (uselutr < 255) {
			if (s->lutr[uselutr + 1] == 0)
				s->lutr[uselutr + 1] = s->lutr[uselutr];
		}
		if (uselutg < 255) {
			if (s->lutg[uselutg + 1] == 0)
				s->lutg[uselutg + 1] = s->lutg[uselutg];
		}
		if (uselutb < 255) {
			if (s->lutb[uselutb + 1] == 0)
				s->lutb[uselutb + 1] = s->lutb[uselutb];
		}
		if (useluti < 255) {
			if (s->luti[useluti + 1] == 0)
				s->luti[useluti + 1] = s->luti[useluti];
		}
	}
	/* DEBUG
	   for(i=0;i<255;i++)
...
RGBIfix(Coolscan_t * scanner,
	...
	int size, int *lutr, int *lutg, int *lutb, int *luti)
{
...
static int
rgb2g(unsigned char *rgbimat, unsigned char *gomat, int size)
{
...
#if 0
static int
RGBIfix1(unsigned char *rgbimat, unsigned char *orgbimat,
	 int size, int *lutr, int *lutg, int *lutb, int *luti)
{
	unsigned char *pr, *pg, *pb, *pi;
	unsigned char *opr, *opg, *opb, *opi;
	...
	DBG(2,
	    "Regression: size:%d I=%d/1024*R b:%d s:%d sr:%e si:%e sii:%e sri:%e  srr:%e\n",
	    size, cc.mr, cc.br, cc.sum, cc.sumr, cc.sumi, cc.sumii, cc.sumri,
	    cc.sumrr);
	for (x = 0; x < size; x++) {
...
reader_process(void *data)
{
	...
	do {
		...
		if (scanner->colormode == RGBI) {	/* Correct Infrared Channel */
			if (scanner->bits_per_color > 8) {
				RGBIfix16(scanner,
					  (unsigned short *) scanner->buffer,
					  (unsigned short *) scanner->obuffer,
					  data_to_read / 8, scanner->lutr,
					  scanner->lutg, scanner->lutb,
					  scanner->luti);
			} else {
				RGBIfix(scanner, scanner->buffer,
					scanner->obuffer, data_to_read / 4,
					scanner->lutr, scanner->lutg,
					scanner->lutb, scanner->luti);
			}
		} else if ((scanner->colormode == GREYSCALE) && (scanner->LS >= 2)) {	/* Convert to Grey */
			data_to_write /= 3;
			rgb2g(scanner->buffer, scanner->obuffer,
			      data_to_write);
		} else {	/* or just copy */
			memcpy(scanner->obuffer, scanner->buffer,
			       data_to_read);
		}
		if ((!scanner->low_byte_first)
		...
		DBG(10,
		    "reader_process: buffer of %d bytes read; %d bytes to go\n",
...
init_options(Coolscan_t * scanner)
{
	...
	if (scanner->LS < 2) {
		scanner->opt[OPT_MODE].size =
			max_string_size(scan_mode_list_LS20);
		scanner->opt[OPT_MODE].constraint_type =
			SANE_CONSTRAINT_STRING_LIST;
		scanner->opt[OPT_MODE].constraint.string_list =
			scan_mode_list_LS20;
	} else {
		scanner->opt[OPT_MODE].size =
			max_string_size(scan_mode_list_LS30);
		scanner->opt[OPT_MODE].constraint_type =
			SANE_CONSTRAINT_STRING_LIST;
		scanner->opt[OPT_MODE].constraint.string_list =
			scan_mode_list_LS30;
	...
	scanner->opt[OPT_PRESCAN_NOW].desc = "Perform a prescan now";
	...
	scanner->opt[OPT_PRESCAN_NOW].cap =
		SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT;
	...
	scanner->opt[OPT_RESOLUTION].constraint_type =
		SANE_CONSTRAINT_WORD_LIST;
	...
	scanner->opt[OPT_PREVIEW_RESOLUTION].constraint_type =
		SANE_CONSTRAINT_WORD_LIST;
	scanner->opt[OPT_PREVIEW_RESOLUTION].constraint.word_list =
		resolution_list;
	...
	scanner->opt[OPT_R_BRIGHTNESS].constraint_type =
	...
	scanner->opt[OPT_G_BRIGHTNESS].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->opt[OPT_B_BRIGHTNESS].constraint_type =
		SANE_CONSTRAINT_RANGE;
	...
	scanner->opt[OPT_AUTOFOCUS].size =
		max_string_size(autofocus_mode_list);
	scanner->opt[OPT_AUTOFOCUS].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	scanner->opt[OPT_AUTOFOCUS].constraint.string_list =
		autofocus_mode_list;
...
/*
   static void
   worddump(char *comment, int * p, int l)
   {
   int i;
   char line[128];
   char *ptr;
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		switch (option) {
			/* word options: */
		case OPT_TL_X:
			*(int *) val = SANE_FIX(iluToMm(scanner->tlx));
			return SANE_STATUS_GOOD;

		case OPT_TL_Y:
			*(int *) val = SANE_FIX(iluToMm(scanner->tly));
			return SANE_STATUS_GOOD;

		case OPT_BR_X:
			*(int *) val = SANE_FIX(iluToMm(scanner->brx));
			return SANE_STATUS_GOOD;

		case OPT_BR_Y:
			*(int *) val = SANE_FIX(iluToMm(scanner->bry));
			return SANE_STATUS_GOOD;

		case OPT_PREVIEW:
			*(int *) val = scanner->preview;
			return SANE_STATUS_GOOD;
		...
		case OPT_RESOLUTION:
			*(int *) val = resDivToVal(scanner->x_nres);
			return SANE_STATUS_GOOD;

		case OPT_PREVIEW_RESOLUTION:
			*(int *) val = resDivToVal(scanner->x_p_nres);
			return SANE_STATUS_GOOD;

		case OPT_BIT_DEPTH:
			*(int *) val = scanner->bits_per_color;
			return SANE_STATUS_GOOD;

		case OPT_CONTRAST:
			*(int *) val = scanner->contrast - 128;
			return SANE_STATUS_GOOD;

		case OPT_R_CONTRAST:
			*(int *) val = scanner->contrast_R - 128;
			return SANE_STATUS_GOOD;

		case OPT_G_CONTRAST:
			*(int *) val = scanner->contrast_G - 128;
			return SANE_STATUS_GOOD;

		case OPT_B_CONTRAST:
			*(int *) val = scanner->contrast_B - 128;
			return SANE_STATUS_GOOD;

		case OPT_BRIGHTNESS:
			*(int *) val = scanner->brightness - 128;
			return SANE_STATUS_GOOD;

		case OPT_R_BRIGHTNESS:
			*(int *) val = scanner->brightness_R - 128;
			return SANE_STATUS_GOOD;

		case OPT_G_BRIGHTNESS:
			*(int *) val = scanner->brightness_G - 128;
			return SANE_STATUS_GOOD;

		case OPT_B_BRIGHTNESS:
			*(int *) val = scanner->brightness_B - 128;
			return SANE_STATUS_GOOD;

		case OPT_EXPOSURE:
			*(int *) val = scanner->exposure_R * 2;
			return SANE_STATUS_GOOD;
		...
		case OPT_G_EXPOSURE:
			*(int *) val = scanner->exposure_G * 2;
			return SANE_STATUS_GOOD;

		case OPT_B_EXPOSURE:
			*(int *) val = scanner->exposure_B * 2;
			return SANE_STATUS_GOOD;

		case OPT_R_SHIFT:
			*(int *) val = scanner->shift_R - 128;
			return SANE_STATUS_GOOD;

		case OPT_G_SHIFT:
			*(int *) val = scanner->shift_G - 128;
			return SANE_STATUS_GOOD;

		case OPT_B_SHIFT:
			*(int *) val = scanner->shift_B - 128;
			return SANE_STATUS_GOOD;


		case OPT_IRED_RED:
			*(int *) val = scanner->ired_red;
			return SANE_STATUS_GOOD;
		...
		case OPT_TYPE:
			strcpy(val,
			       ((scanner->
				 negative) ? negativeStr : positiveStr));
			return SANE_STATUS_GOOD;
		...
		case OPT_RGB_CONTROL:
			*(int *) val =
				(scanner->
				 rgb_control) ? TRUE : FALSE;
			return SANE_STATUS_GOOD;

		case OPT_GAMMA_BIND:
			*(int *) val =
				(scanner->
				 gamma_bind) ? TRUE : FALSE;
			return SANE_STATUS_GOOD;

		case OPT_ANALOG_GAMMA:
			*(int *) val =
				(scanner->
				 analog_gamma_r) ? TRUE : FALSE;
			return SANE_STATUS_GOOD;

		case OPT_AVERAGING:
			*(int *) val =
				(scanner->averaging) ? TRUE : FALSE;
			return SANE_STATUS_GOOD;

		case OPT_GAMMA_VECTOR:
			memcpy(val, scanner->gamma,
			       scanner->opt[option].size);
			return SANE_STATUS_GOOD;
		case OPT_GAMMA_VECTOR_R:
			memcpy(val, scanner->gamma_r,
			       scanner->opt[option].size);
			return SANE_STATUS_GOOD;
		case OPT_GAMMA_VECTOR_G:
			memcpy(val, scanner->gamma_g,
			       scanner->opt[option].size);
			return SANE_STATUS_GOOD;
		case OPT_GAMMA_VECTOR_B:
			memcpy(val, scanner->gamma_b,
			       scanner->opt[option].size);
			return SANE_STATUS_GOOD;
		...
		case OPT_BIT_DEPTH:
			scanner->bits_per_color = (*(int *) val);
			if (info)
				*info |= SANE_INFO_RELOAD_PARAMS;
			return SANE_STATUS_GOOD;
		...
		case OPT_RGB_CONTROL:
			...
			if (scanner->rgb_control) {
				...
				scanner->opt[OPT_CONTRAST].cap |=
					SANE_CAP_INACTIVE;
				scanner->opt[OPT_EXPOSURE].cap |=
					SANE_CAP_INACTIVE;

				scanner->opt[OPT_R_BRIGHTNESS].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_G_BRIGHTNESS].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_B_BRIGHTNESS].cap &=
					~SANE_CAP_INACTIVE;

				scanner->opt[OPT_R_CONTRAST].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_G_CONTRAST].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_B_CONTRAST].cap &=
					~SANE_CAP_INACTIVE;

				scanner->opt[OPT_R_EXPOSURE].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_G_EXPOSURE].cap &=
					~SANE_CAP_INACTIVE;
				scanner->opt[OPT_B_EXPOSURE].cap &=
					~SANE_CAP_INACTIVE;
		...
		case OPT_CONTRAST:
			scanner->contrast = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_R_CONTRAST:
			scanner->contrast_R = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_G_CONTRAST:
			scanner->contrast_G = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_B_CONTRAST:
			scanner->contrast_B = *(int *) val + 128;
			return SANE_STATUS_GOOD;

		case OPT_BRIGHTNESS:
			scanner->brightness = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_R_BRIGHTNESS:
			scanner->brightness_R = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_G_BRIGHTNESS:
			scanner->brightness_G = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_B_BRIGHTNESS:
			scanner->brightness_B = *(int *) val + 128;
			return SANE_STATUS_GOOD;

		case OPT_EXPOSURE:
			scanner->exposure_R = *(int *) val / 2;
			scanner->exposure_G = *(int *) val / 2;
			scanner->exposure_B = *(int *) val / 2;
			return SANE_STATUS_GOOD;
		case OPT_R_EXPOSURE:
			scanner->exposure_R = *(int *) val / 2;
			return SANE_STATUS_GOOD;
		...
		case OPT_R_SHIFT:
			scanner->shift_R = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_G_SHIFT:
			scanner->shift_G = *(int *) val + 128;
			return SANE_STATUS_GOOD;
		case OPT_B_SHIFT:
			scanner->shift_B = *(int *) val + 128;
			return SANE_STATUS_GOOD;

		case OPT_IRED_RED:
			scanner->ired_red = *(int *) val;
			return SANE_STATUS_GOOD;
