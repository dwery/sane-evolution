 ...
 *
 * @param - none
 * @return the function returns TRUE if a cancel condition has been
 *  detected, if not, it returns FALSE
 */
static SANE_Bool
 ...
 * @param fOn    - TRUE means motor on, FALSE means motor off
 * @return always TRUE
 */
static SANE_Bool
...
usb_WaitPos(Plustek_Device * dev, u_long to, SANE_Bool stay)
{
	...
	if ((hw->dHighSpeed == 0.0) || (dev->adj.disableSpeedup != 0)) {
		...
		min_ffs =
			(u_short) (CRYSTAL_FREQ /
				   ((u_long) mclk_div * 32UL * (u_long) mch *
				    hw->dHighSpeed * hw->wMotorDpi));
		maxf = (ffs - min_ffs) / 4;
		if (maxf > 100.0)
			maxf = 100.0;
		if (maxf < 5.0)
	...
	for (;;) {
		...
		if (min_ffs != 0xffff) {
			...
			if (ffs >= min_ffs) {
				...
				r[0] = (u_char) (ffs >> 8);
				r[1] = (u_char) (ffs & 0xFF);
				sanei_lm983x_write(dev->fd, 0x48, r, 2,
						   TRUE);
				if (ffs == min_ffs)
					ffs = 0;
			} else {
...
usb_ModuleMove(Plustek_Device * dev, u_char action, u_long dwStep)
{
	...
	DBG(_DBG_INFO2, "MOTOR: "
	    ...
	    regs[0x56], regs[0x57], regs[0x45], regs[0x48], regs[0x49]);
	...
	_UIO(sanei_lm983x_write(dev->fd, 0x48, &regs[0x48], 2, TRUE));
	_UIO(sanei_lm983x_write(dev->fd, 0x4A, &regs[0x4A], 2, TRUE));
	...
	if (usbio_WriteReg(dev->fd, 0x07, reg7)) {
		...
		if (action == MOVE_ToPaperSensor) {
			...
			retval = usb_WaitPos(dev, 200, TRUE);
		}
...
usb_ModuleToHome(Plustek_Device * dev, SANE_Bool fWait)
{
	...
	if (value & 1) {
		dev->usbDev.fModFirstHome = FALSE;
		return TRUE;
	}
	...
	if (value != 2) {
		...
		wFastFeedStepSize =
			(u_short) (CRYSTAL_FREQ /
				   (mclk_div * 8 * 1 * hw->dMaxMotorSpeed *
				    4 * hw->wMotorDpi));
		regs[0x48] = (u_char) (wFastFeedStepSize >> 8);
		regs[0x49] = (u_char) (wFastFeedStepSize & 0xFF);
		regs[0x4a] = 0;
		...
		DBG(_DBG_INFO2,
		    "MotorDPI=%u, MaxMotorSpeed=%.3f, FFStepSize=%u\n",
		    hw->wMotorDpi, hw->dMaxMotorSpeed, wFastFeedStepSize);
		DBG(_DBG_INFO,
		    "MOTOR: " "PWM=0x%02x, PWM_DUTY=0x%02x 0x45=0x%02x "
		    "0x48=0x%02x, 0x49=0x%02x\n", regs[0x56], regs[0x57],
		...
		_UIO(sanei_lm983x_write
		     (dev->fd, 0x48, &regs[0x48], 4, TRUE));
		_UIO(sanei_lm983x_write
		     (dev->fd, 0x56, &regs[0x56], 3, TRUE));
...
usb_GetLampStatus(Plustek_Device * dev)
{
	...
	if (NULL == hw) {
		DBG(_DBG_ERROR,
		    "NULL-Pointer detected: usb_GetLampStatus()\n");
		return -1;
	}
	...
	if (_WAF_MISC_IO_LAMPS & sc->workaroundFlag) {
		...
		if (0 == reg) {
			...
			DBG(_DBG_INFO2,
			    "LAMP-REG[0x%02x] = 0x%02x (msk=0x%02x)\n", reg,
			    val, msk);
			if (val & msk)
				iLampStatus |= DEV_LampReflection;
			...
			if (sc->wFlags & DEVCAPSFLAG_TPA) {
				...
				usb_GetLampRegAndMask(_GET_TPALAMP
						      (sc->misc_io), &reg,
						      &msk);
				if (reg) {
					usbio_ReadReg(dev->fd, reg, &val);
					DBG(_DBG_INFO2,
					    "TPA-REG[0x%02x] = 0x%02x (msk=0x%02x)\n",
					    reg, val, msk);
					if (val & msk)
			...
			if ((dev->usbDev.vendor == 0x04A9)
			    && (dev->usbDev.product == 0x2208)) {
				sanei_lm983x_read(dev->fd, 0x29, &regs[0x29],
						  3, TRUE);
				DBG(_DBG_INFO,
				    "[29]=0x%02x, [2A]=0x%02x, [2B]=0x%02x\n",
				    regs[0x29], regs[0x2a], regs[0x2b]);
			}
		}
	} else {
		sanei_lm983x_read(dev->fd, 0x29, &regs[0x29], 0x37 - 0x29 + 1,
				  TRUE);
...
/* HEINER: BETTER define register to check ! */
			...
			if (!_IS_PLUSTEKMOTOR(hw->motorModel)) {
				...
				if ((regs[0x2e] * 256 + regs[0x2f]) >
				    hw->wLineEnd)
					iLampStatus |= DEV_LampReflection;

				if ((regs[0x36] * 256 + regs[0x37]) >
				    hw->wLineEnd)
					iLampStatus |= DEV_LampTPA;
			}
		}
...
usb_switchLampX(Plustek_Device * dev, SANE_Bool on, SANE_Bool tpa)
{
	...
	if (0 == reg)
		return FALSE;	/* no need to switch something */
...
usb_switchLamp(Plustek_Device * dev, SANE_Bool on)
{
	...
	if ((dev->scanning.sParam.bSource == SOURCE_Negative) ||
		...
		result = usb_switchLampX(dev, on, TRUE);
	} else {
		result = usb_switchLampX(dev, on, FALSE);
	}
...
/* HEINER: CHECK WHY THIS has been disabled*/
	...
	if (dev->initialized >= 0) {
		...
		sanei_lm983x_read(dev->fd, 0x59, &regs[0x59], 3, TRUE);
		...
		if (usb_IsCISDevice(dev)) {
			...
			sanei_lm983x_write(dev->fd, 0x59, &regs[0x59], 3,
					   TRUE);
		}
	}
	...
	if (value & 1) {
		...
		if (value) {
			...
			sanei_lm983x_write(dev->fd, 0x58,
					   &hw->bReg_0x58, 0x5b - 0x58 + 1,
					   TRUE);
			usbio_ReadReg(dev->fd, 2, &value);
			usbio_ReadReg(dev->fd, 2, &value);
		}

		usb_MotorOn(dev, FALSE);
		return TRUE;
	}
...
usb_LampSwitch(Plustek_Device * dev, SANE_Bool sw)
{
	...
	if (-1 == dev->fd) {
		...
		if (SANE_STATUS_GOOD ==
		    sanei_usb_open(dev->sane.name, &handle)) {
			dev->fd = handle;
		}
	}

	/* needs to be recalibrated */
	dev->scanning.fCalibrated = FALSE;

	if (-1 != dev->fd)
		usb_LampOn(dev, sw, FALSE);
...
usb_HasTPA(Plustek_Device * dev)
{
	...
	if (dev->usbDev.vendor == 0x04B8) {	/* the EPSON section */
		...
		usb_switchLampX(dev, FALSE, TRUE);
		usbio_WriteReg(dev->fd, 0x58, 0x1d);
		usbio_WriteReg(dev->fd, 0x59, 0x49);
		usbio_ReadReg(dev->fd, 0x02, &val);
		usbio_WriteReg(dev->fd, 0x58,
			       dev->usbDev.HwSetting.bReg_0x58);
		usbio_WriteReg(dev->fd, 0x59,
			       dev->usbDev.HwSetting.bReg_0x59);
		...
		if ((dev->usbDev.product == 0x0050)
		    || (dev->usbDev.product == 0x0060)) {
			...
			usbio_WriteReg(dev->fd, 0x58,
				       dev->usbDev.HwSetting.bReg_0x58);
			usbio_WriteReg(dev->fd, 0x5a,
				       dev->usbDev.HwSetting.bReg_0x5a);
			usbio_WriteReg(dev->fd, 0x5b,
				       dev->usbDev.HwSetting.bReg_0x5b);
...
usb_UpdateButtonStatus(Plustek_Scanner * s)
{
	...
	if (val == 0) {
		...
		if ((dev->usbDev.vendor == 0x07B3)
		    || (dev->usbDev.vendor == 0x0458)) {
			...
			if (caps->bButtons == 2 || caps->bButtons == 5) {
				...
				switch (val) {
				case 1:
					s->val[OPT_BUTTON_1].w = 1;
					break;
				case 2:
					s->val[OPT_BUTTON_0].w = 1;
					break;
				case 3:
					s->val[OPT_BUTTON_2].w = 1;
					break;
				case 4:
					s->val[OPT_BUTTON_3].w = 1;
					break;
				case 6:
					s->val[OPT_BUTTON_4].w = 1;
					break;
				}
			...
			for (i = 0; i < 3; i++) {
				...
				for (j = 0; j < 2; j++) {
					...
					if ((mio[i] & mask) == 0) {
						DBG(_DBG_INFO2,
						    "* port %u configured as input,"
						    " status: %s (%u)\n",
						    (i * 2) + j + 1,
						    ((val & 1) ? "PRESSED" :
						     "RELEASED"),
						    (OPT_BUTTON_0 + bc));
						s->val[OPT_BUTTON_0 + bc].w =
							val & 1;
						bc++;
					}
					val >>= 1;
