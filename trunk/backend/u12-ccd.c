 ...
 */
static void
fnCCDInitWolfson3797(U12_Device * dev)
{
	if (dev->shade.intermediate & _ScanMode_Mono)
		dev->shade.pCcdDac->DarkDAC.Colors.Green = 0xcc;
	else if (dev->shade.intermediate & _ScanMode_AverageOut)
 ...
 */
static void
fnCCDInitSamsung3797(U12_Device * dev)
{
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_TPA)) {
		...
		if (!(dev->shade.intermediate & _ScanMode_AverageOut)) {
			...
			if (dev->PCBID == _OPTICWORKS2000) {
				...
				dev->shade.pCcdDac->DarkDAC.Colors.Green =
					0x40;
				dev->shade.pCcdDac->DarkDAC.Colors.Blue =
					0x40;
				dev->shade.pCcdDac->DarkCmpHi.Colors.Red =
					0x48;
				dev->shade.pCcdDac->DarkCmpHi.Colors.Green =
					0x40;
				dev->shade.pCcdDac->DarkCmpHi.Colors.Blue =
					0x40;
				dev->shade.pCcdDac->DarkCmpLo.Colors.Red =
					0x38;
				dev->shade.pCcdDac->DarkCmpLo.Colors.Green =
					0x30;
				dev->shade.pCcdDac->DarkCmpLo.Colors.Blue =
					0x30;
				dev->shade.pCcdDac->DarkOffSub.Colors.Red =
					0x48;
				dev->shade.pCcdDac->DarkOffSub.Colors.Green =
					0x38;
				dev->shade.pCcdDac->DarkOffSub.Colors.Blue =
					0x40;
			} else {
				dev->shade.pCcdDac->GainResize.Colors.Red =
					99;
 ...
 */
static void
fnCCDInitWolfson3799(U12_Device * dev)
{
 ...
 */
static void
fnCCDInitWolfson548(U12_Device * dev)
{
	if (!(dev->shade.intermediate & _ScanMode_AverageOut)) {
 ...
 */
static void
fnCCDInitSamsung3777(U12_Device * dev)
{
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_Negative)) {
 ...
 */
static void
fnCCDInitSamsung3799(U12_Device * dev)
{
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_TPA)) {
 ...
 */
static void
fnCCDInitESIC3799(U12_Device * dev)
{
	if (!(dev->DataInf.dwScanFlag & _SCANDEF_Negative)) {
...
{
	if (dev->shade.DarkOffset.wColors[dwCh] >
	    sTbl->DarkOffSub.wColors[dwCh])
		dev->shade.DarkOffset.wColors[dwCh] -=
			sTbl->DarkOffSub.wColors[dwCh];
	else
		dev->shade.DarkOffset.wColors[dwCh] = 0;
}
 ...
 */
static void
fnDACDarkWolfson(U12_Device * dev, ShadingVarDef * sTbl,
		 u_long dwCh, u_short wDarkest)
{
	...
	if (wDarkest > sTbl->DarkCmpHi.wColors[dwCh]) {
		...
		if ((wDarkest < sTbl->DarkCmpLo.wColors[dwCh]) &&
			...
			if (wDarkest)
				w = (u_short) dev->shade.DarkDAC.
					bColors[dwCh] - 2U;
			else
				w = (u_short) dev->shade.DarkDAC.
					bColors[dwCh] -
					dev->shade.wDarkLevels;
 ...
 */
static void
fnDACDarkSamsung(U12_Device * dev, ShadingVarDef * sTbl,
		 u_long dwCh, u_short wDarkest)
{
	u_short w;
	...
	if (wDarkest > sTbl->DarkCmpHi.wColors[dwCh]) {
		...
		if ((wDarkest < sTbl->DarkCmpLo.wColors[dwCh]) &&
			...
			if (wDarkest)
				...
				w = dev->shade.wDarkLevels +
					(u_short) dev->shade.DarkDAC.
					bColors[dwCh];
 ...
 */
static void
u12ccd_InitCCDandDAC(U12_Device * dev, SANE_Bool shading)
{
	u_short w;
...
/* CHECK: org was to  fnDarkOffsetWolfson3797 */
	...
	for (w = 0; w < dev->numDACRegs; w++) {
		...
		u12io_DataToRegister(dev, REG_ADCSERIALOUT,
				     dev->DACRegs[w].val);
#endif
	}
