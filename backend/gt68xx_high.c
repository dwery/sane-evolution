			 ...
			 GT68xx_Scan_Request * request)
{
	...
	if (req.use_ta) {
		req.lamp = SANE_FALSE;
		status = gt68xx_device_lamp_control(scanner->dev, SANE_FALSE,
						    SANE_TRUE);
	} else {
		req.lamp = SANE_TRUE;
		status = gt68xx_device_lamp_control(scanner->dev, SANE_TRUE,
						    SANE_FALSE);
	}
			  ...
			  GT68xx_Scan_Parameters * params)
{
	...
	if (request->use_ta) {
		gt68xx_device_lamp_control(scanner->dev, SANE_FALSE,
					   SANE_TRUE);
		request->lamp = SANE_FALSE;
	} else {
		gt68xx_device_lamp_control(scanner->dev, SANE_TRUE,
					   SANE_FALSE);
...
gt68xx_afe_ccd_adjust_offset_gain(SANE_String_Const color_name,
				  ...
				  SANE_Byte * old_pga)
{
	SANE_Int black_low = values->coarse_black, black_high =
		black_low + 10;
	SANE_Int white_high = values->coarse_white, white_low =
		white_high - 10;
...
gt68xx_afe_cis_auto(GT68xx_Scanner * scanner)
{
	...
	RIE(gt68xx_afe_cis_read_lines
	    (&values, scanner, SANE_FALSE, SANE_FALSE, r_gbuffer, g_gbuffer,
	     b_gbuffer));
	...
	do {
		...
		RIE(gt68xx_afe_cis_read_lines
		    (&values, scanner, SANE_FALSE, first, r_obuffer,
		     g_obuffer, b_obuffer));
		RIE(gt68xx_afe_cis_read_lines
		    (&values, scanner, SANE_TRUE, SANE_FALSE, r_gbuffer,
		     g_gbuffer, b_gbuffer));
