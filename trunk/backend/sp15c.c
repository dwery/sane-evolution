static const char RCSid[] =
	"$Header: /org/alioth.debian.org/chroot/cvsroot/sane/sane-backends/backend/sp15c.c,v 1.15 2007-11-18 10:59:18 ellert-guest Exp $";
...
static const SANE_Range brightness_range = { 0, 255, 32 };
static const SANE_Range threshold_range = { 0, 255, 4 };
static const SANE_Range x_range = { 0, SANE_FIX(216), 1 };
static const SANE_Range y_range_fb = { 0, SANE_FIX(295), 1 };
static const SANE_Range y_range_adf = { 0, SANE_FIX(356), 1 };
		    ...
		    SANE_Action action, void *val, int * info)
{
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case OPT_X_RES:
			*(int *) val = scanner->x_res;
			return SANE_STATUS_GOOD;

		case OPT_Y_RES:
			*(int *) val = scanner->y_res;
			return SANE_STATUS_GOOD;
		...
		case OPT_TL_X:
			*(int *) val = SANE_FIX(iluToMm(scanner->tl_x));
			return SANE_STATUS_GOOD;

		case OPT_TL_Y:
			*(int *) val = SANE_FIX(iluToMm(scanner->tl_y));
			return SANE_STATUS_GOOD;

		case OPT_BR_X:
			*(int *) val = SANE_FIX(iluToMm(scanner->br_x));
			return SANE_STATUS_GOOD;

		case OPT_BR_Y:
			*(int *) val = SANE_FIX(iluToMm(scanner->br_y));
			return SANE_STATUS_GOOD;
		...
		case OPT_BRIGHTNESS:
			*(int *) val = scanner->brightness;
			return SANE_STATUS_GOOD;

		case OPT_THRESHOLD:
			*(int *) val = scanner->threshold;
			return SANE_STATUS_GOOD;
		...
		case OPT_SOURCE:
			if (strcmp(val, "ADF") == 0) {
				if (scanner->use_adf == TRUE)
					return SANE_STATUS_GOOD;
				scanner->use_adf = TRUE;
				scanner->opt[OPT_TL_Y].constraint.range =
					&y_range_adf;
				scanner->opt[OPT_BR_Y].constraint.range =
					&y_range_adf;
				apply_constraints(scanner, OPT_TL_Y,
						  &scanner->tl_y, info);
				apply_constraints(scanner, OPT_BR_Y,
						  &scanner->br_y, info);
			} else if (strcmp(val, "FB") == 0) {
				if (scanner->use_adf == FALSE)
					return SANE_STATUS_GOOD;
				scanner->use_adf = FALSE;
				scanner->opt[OPT_TL_Y].constraint.range =
					&y_range_fb;
				scanner->opt[OPT_BR_Y].constraint.range =
					&y_range_fb;
		...
		case OPT_BRIGHTNESS:
			scanner->brightness = *(int *) val;
			return SANE_STATUS_GOOD;

		case OPT_THRESHOLD:
			scanner->threshold = *(int *) val;
			return SANE_STATUS_GOOD;
...
sane_start(SANE_Handle handle)
{
	...
	if ((ret = sp15c_check_values(scanner)) != 0) {	/* Verify values */
...
{
	DBG(10, "sane_close\n");
	if (((struct sp15c *) handle)->scanning == TRUE)
		do_cancel(handle);
}				/* sane_close */
...
/************** "Mode" group: **************/
	...
	scanner->opt[OPT_PREVIEW_RES].constraint_type =
...
{
	if (s->use_adf == TRUE && s->autofeeder == 0) {
...
sp15c_free_scanner(struct sp15c *s)
{
	...
	ret = do_scsi_cmd(s->sfd, release_unitB.cmd, release_unitB.size, NULL,
			  0);
	if (ret)
		return ret;
...
sp15c_grab_scanner(struct sp15c *s)
{
	...
	ret = do_scsi_cmd(s->sfd, reserve_unitB.cmd, reserve_unitB.size, NULL,
			  0);
	if (ret)
		return ret;
...
sp15c_media_check(struct sp15c *s)
{
	...
	ret = do_scsi_cmd(s->sfd, media_checkB.cmd, media_checkB.size,
			  s->buffer, 1);
...
sp15c_set_window_param(struct sp15c *s, int prescan)
{
	...
	memcpy(buffer_r, window_descriptor_blockB.cmd, window_descriptor_blockB.size);	/* copy preset data */
	...
	set_WD_adf(buffer_r, s->use_adf == TRUE ? 1 : 0);
	...
	case WD_comp_LA:
	case WD_comp_HT:
		set_WD_line_width(buffer_r,
				  ((s->br_x -
				    s->tl_x) * s->x_res / 1200) / 8);
		set_WD_line_count(buffer_r,
				  (s->br_y - s->tl_y) * s->y_res / 1200);
		goto b_and_w;
	case WD_comp_G4:
		set_WD_line_width(buffer_r,
				  ((s->br_x -
				    s->tl_x) * s->x_res / 1200) / 2);
		set_WD_line_count(buffer_r,
				  (s->br_y - s->tl_y) * s->y_res / 1200);
		goto grayscale;
	case WD_comp_G8:
		set_WD_line_width(buffer_r,
				  (s->br_x - s->tl_x) * s->x_res / 1200);
		set_WD_line_count(buffer_r,
				  (s->br_y - s->tl_y) * s->y_res / 1200);
	      grayscale:
	...
	memcpy((s->buffer + set_windowB.size +
		window_parameter_data_blockB.size), buffer_r,
	       window_descriptor_blockB.size);

	set_SW_xferlen(s->buffer,
		       (window_parameter_data_blockB.size + WDB_size_Color));
	set_WDPB_wdblen((s->buffer + set_windowB.size), WDB_size_Color);
	set_WD_parm_length((s->buffer
			    + set_windowB.size
			    + window_parameter_data_blockB.size), 9);

	DBG(10, "\tx_res=%d, y_res=%d\n", s->x_res, s->y_res);
	DBG(10, "\tupper left-x=%d, upper left-y=%d\n", s->tl_x, s->tl_y);
	DBG(10, "\twindow width=%d, length=%d\n",
	    s->br_x - s->tl_x, s->br_y - s->tl_y);
...
reader_process(void *data)
{
	...
	data_left = bytes_per_line(scanner) * lines_per_scan(scanner);
	...
	if (status != SANE_STATUS_GOOD) {
		if (SANE_CONSTRAINT_RANGE ==
			...
			if (*target < scanner->opt[opt].constraint.range->min) {
				*target =
					scanner->opt[opt].constraint.range->
					min;
				return SANE_STATUS_GOOD;
			} else if (scanner->opt[opt].constraint.range->max <
				   *target) {
				*target =
					scanner->opt[opt].constraint.range->
					max;
