		      ...
		      int white_level, GT68xx_Calibrator ** cal_return)
{
	...
	*cal_return = 0;
	...
	if (!cal) {
		DBG(5,
		    "gt68xx_calibrator_new: no memory for GT68xx_Calibrator\n");
	...
	if (!cal->k_white || !cal->k_black | !cal->white_line
	    || !cal->black_line) {
		DBG(5,
		    "gt68xx_calibrator_new: no memory for calibration data\n");
...
#ifdef TUNE_CALIBRATOR
	DBG(4,
	    "gt68xx_calibrator_free: min_clip_count=%d, max_clip_count=%d\n",
...
gt68xx_calibrator_eval_white(GT68xx_Calibrator * cal, double factor)
{
	...
	for (i = 0; i < width; ++i) {
		cal->white_line[i] =
			cal->white_line[i] / cal->white_count * factor;
	}
...
gt68xx_calibrator_eval_black(GT68xx_Calibrator * cal, double factor)
{
	...
	for (i = 0; i < width; ++i) {
		cal->black_line[i] =
			cal->black_line[i] / cal->black_count - factor;
	}
...
gt68xx_calibrator_process_line(GT68xx_Calibrator * cal, unsigned int *line)
{
	...
	for (i = 0; i < width; ++i) {
		...
		if (src_value > black) {
			value = (src_value -
				 black) * white_level / cal->k_white[i];
			if (value > 0xffff) {
				value = 0xffff;
#ifdef TUNE_CALIBRATOR
				cal->max_clip_count++;
...
gt68xx_scanner_wait_for_positioning(GT68xx_Scanner * scanner)
{
	...
	while (TRUE) {
		...
		if (status == SANE_STATUS_GOOD) {
			...
			if (status_count > 9) {
				DBG(1,
				    "gt68xx_scanner_wait_for_positioning: error count too high!\n");
				return status;
			}
			status_count++;
			DBG(3,
			    "gt68xx_scanner_wait_for_positioning: ignored error\n");
		}
		usleep(100000);
	}
...
gt68xx_scanner_internal_start_scan(GT68xx_Scanner * scanner)
{
	...
	for (repeat_count = 0; repeat_count < 30 * 100; ++repeat_count) {
		status = gt68xx_device_read_scanned_data(scanner->dev,
							 &ready);
		if (status != SANE_STATUS_GOOD) {
			DBG(5,
				   ...
				   GT68xx_Scan_Parameters * params)
{
	...
	status = gt68xx_device_setup_scan(scanner->dev, request, action,
					  params);
	if (status != SANE_STATUS_GOOD) {
		DBG(5,
	...
	if (scanner->dev->model->is_cis
		...
		status = gt68xx_device_set_exposure_time(scanner->dev,
							 scanner->dev->
							 exposure);
		if (status != SANE_STATUS_GOOD) {
			DBG(5,
					...
					GT68xx_Scan_Parameters * params)
{
	...
	if (!scanner->cal_r) {
		status = gt68xx_scanner_create_calibrator(params,
							  &scanner->cal_r);
		if (status != SANE_STATUS_GOOD)
			return status;
	}
...
gt68xx_scanner_calibrate_gray_white_line(GT68xx_Scanner * scanner,
					 unsigned int **buffer_pointers)
{
	gt68xx_calibrator_add_white_line(scanner->cal_gray,
					 buffer_pointers[0]);
	return SANE_STATUS_GOOD;
}
...
gt68xx_scanner_calibrate_gray_black_line(GT68xx_Scanner * scanner,
					 unsigned int **buffer_pointers)
{
	gt68xx_calibrator_add_black_line(scanner->cal_gray,
					 buffer_pointers[0]);
	return SANE_STATUS_GOOD;
}
			 ...
			 GT68xx_Scan_Request * request)
{
	...
	if (scanner->auto_afe) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(5,
			    "gt68xx_scanner_calibrate: gt68xx_afe_*_auto failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		req.mbs = FALSE;
	...
	DBG(3, "afe 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
	    afe->r_offset, afe->r_pga, afe->g_offset, afe->g_pga,
	...
	if (req.use_ta) {
		req.lamp = FALSE;
		status = gt68xx_device_lamp_control(scanner->dev, FALSE,
						    TRUE);
	} else {
		req.lamp = TRUE;
		status = gt68xx_device_lamp_control(scanner->dev, TRUE,
						    FALSE);
	}

	status = gt68xx_scanner_start_scan_extended(scanner, &req,
						    SA_CALIBRATE, &params);
	if (status != SANE_STATUS_GOOD) {
		DBG(5,
	...
	if (params.color) {
		status = gt68xx_scanner_create_color_calibrators(scanner,
								 &params);
	} else {
		status = gt68xx_scanner_create_gray_calibrators(scanner,
								&params);
	}
	...
	for (i = 0; i < params.pixel_ys; ++i) {
		status = gt68xx_line_reader_read(scanner->reader,
						 buffer_pointers);
		if (status != SANE_STATUS_GOOD) {
			DBG(5,
		...
		if (params.color)
			status = gt68xx_scanner_calibrate_color_white_line
				(scanner, buffer_pointers);
		else
			status = gt68xx_scanner_calibrate_gray_white_line
				(scanner, buffer_pointers);

		if (status != SANE_STATUS_GOOD) {
			DBG(5,
			    "gt68xx_scanner_calibrate: calibration failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
	}
	...
	for (i = 0; i < params.pixel_ys; ++i) {
		...
		if (params.color)
			status = gt68xx_scanner_calibrate_color_black_line
				(scanner, buffer_pointers);
		else
			status = gt68xx_scanner_calibrate_gray_black_line
				(scanner, buffer_pointers);
...
gt68xx_scanner_start_scan(GT68xx_Scanner * scanner,
			  ...
			  GT68xx_Scan_Parameters * params)
{
	request->mbs = FALSE;	/* don't go home before real scan */
	request->mds = TRUE;
	request->mas = FALSE;
	if (request->use_ta) {
		gt68xx_device_lamp_control(scanner->dev, FALSE,
					   TRUE);
		request->lamp = FALSE;
	} else {
		gt68xx_device_lamp_control(scanner->dev, TRUE,
					   FALSE);
		request->lamp = TRUE;
	}
	if (!scanner->dev->model->is_cis)
			 ...
			 unsigned int **buffer_pointers)
{
	...
	if (scanner->calib) {
		if (scanner->reader->params.color) {
			...
			if (scanner->dev->model->is_cis
			    && !(scanner->dev->model->
				 flags & GT68XX_FLAG_CIS_LAMP)) {
				if (strcmp
				    (scanner->val[OPT_GRAY_MODE_COLOR].s,
				     GT68XX_COLOR_BLUE) == 0)
					gt68xx_calibrator_process_line
						(scanner->cal_b,
						 buffer_pointers[0]);
				else if (strcmp
					 (scanner->val[OPT_GRAY_MODE_COLOR].s,
				...
				gt68xx_calibrator_process_line(scanner->
							       cal_gray,
							       buffer_pointers
							       [0]);
			}
...
gt68xx_afe_ccd_adjust_offset_gain(const char * color_name,
				  ...
				  SANE_Byte * old_pga)
{
	int black_low = values->coarse_black, black_high =
		black_low + 10;
	int white_high = values->coarse_white, white_low =
		white_high - 10;
	SANE_Bool done = TRUE;
	...
	*old_offset = *offset;
	...
	DBG(4,
	    "%5s white=%3d, black=%3d, offset=%2d, gain=%2d, old offs=%2d, "
...
gt68xx_wait_lamp_stable(GT68xx_Scanner * scanner,
			...
			GT68xx_Scan_Request * request,
			unsigned int *buffer_pointers[3],
			GT68xx_Afe_Values * values, SANE_Bool dont_move)
{
	SANE_Status status = SANE_STATUS_GOOD;
	int last_white = 0;
	SANE_Bool first = TRUE;
	SANE_Bool message_printed = FALSE;
	...
	do {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(3,
			    "gt68xx_wait_lamp_stable: gt68xx_line_reader_read failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		gt68xx_scanner_stop_scan(scanner);
		...
		if (scanner->val[OPT_AUTO_WARMUP].w == TRUE) {
	...
	DBG(3,
	    "gt68xx_wait_lamp_stable: Lamp is stable after %d secs (%d secs total)\n",
		    ...
		    GT68xx_Scan_Request * orig_request)
{
	...
	SANE_Bool gray_done = FALSE;
	SANE_Bool red_done = FALSE, green_done = FALSE, blue_done =
		FALSE;
	...
	values.max_width =
		(params.pixel_xs * scanner->dev->model->optical_xdpi) /
		params.xdpi;
	if (orig_request->use_ta)
	...
	DBG(5, "gt68xx_afe_ccd_auto: scan_dpi=%d, calwidth=%d, max_width=%d, "
	    "start_black=%.1f mm\n", values.scan_dpi,
	    values.calwidth, values.max_width,
	    SANE_UNFIX(values.start_black));

	status = gt68xx_line_reader_read(scanner->reader, buffer_pointers);
	if (status != SANE_STATUS_GOOD) {
		DBG(3,
		    "gt68xx_afe_ccd_auto: gt68xx_line_reader_read failed: %s\n",
		    sane_strstatus(status));
		return status;
	}
	gt68xx_scanner_stop_scan(scanner);
	...
	status = gt68xx_wait_lamp_stable(scanner, &params, &request,
					 buffer_pointers, &values,
					 FALSE);

	if (status != SANE_STATUS_GOOD) {
		DBG(1,
		    "gt68xx_afe_ccd_auto: gt68xx_wait_lamp_stable failed %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	do {
		...
		if (params.color) {
			...
			if (!green_done)
				green_done =
					gt68xx_afe_ccd_adjust_offset_gain
					("green", &values, buffer_pointers[1],
			  ...
			  unsigned int *black_buffer)
{
	...
	for (i = start_black; i < end_black; ++i) {
		...
		for (j = 0; j < values->callines; j++)
			avg_black +=
				(*(black_buffer + i + j * values->calwidth) >>
				 8);
			  ...
			  unsigned int *white_buffer)
{
	...
	for (i = start_white; i < end_white; ++i) {
		...
		for (j = 0; j < values->callines; j++) {
			avg_white +=
				(*(white_buffer + i + j * values->calwidth) >>
				 8);
			values->total_white +=
				(*(white_buffer + i + j * values->calwidth));
		}
				  ...
				  GT68xx_AFE_Parameters * old_afe)
{
	...
	*old_offset = *offset;
	...
	DBG(4,
	    "%5s white=%3d, black=%3d, offset=0x%02X, gain=0x%02X, old offs=0x%02X, "
	    "old gain=0x%02X, total_white=%5d %s\n", color_name,
	    values->white, values->black, o, g, *offset, *gain,
	    values->total_white, done ? "DONE " : "");
			       ...
			       int * exposure_time)
{
	...
	if (values->white < border) {
		...
		DBG(4,
		    ...
		    color_name, values->white, values->total_white,
		    exposure_change, *exposure_time);
		return FALSE;
		...
		DBG(4,
		    "%5s: white = %3d, total_white=%5d (exposure ok=0x%03x)\n",
		    color_name, values->white, values->total_white,
		    *exposure_time);
	}
	return TRUE;
			  ...
			  unsigned int *g_buffer, unsigned int *b_buffer)
{
	...
	if (first && (scanner->dev->model->flags & GT68XX_FLAG_CIS_LAMP)) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "gt68xx_afe_cis_read_lines: gt68xx_wait_lamp_stable failed %s\n",
			    sane_strstatus(status));
			return status;
		}
		request.mbs = FALSE;
	...
	if (r_buffer && g_buffer && b_buffer)
		for (line = 0; line < values->callines; line++) {
			...
			memcpy(r_buffer + values->calwidth * line,
			       buffer_pointers[0],
			       values->calwidth * sizeof(unsigned int));
			memcpy(g_buffer + values->calwidth * line,
			       buffer_pointers[1],
			       values->calwidth * sizeof(unsigned int));
			memcpy(b_buffer + values->calwidth * line,
			       buffer_pointers[2],
			       values->calwidth * sizeof(unsigned int));
		}
...
gt68xx_afe_cis_auto(GT68xx_Scanner * scanner)
{
	...
	RIE(gt68xx_afe_cis_read_lines
	    (&values, scanner, FALSE, FALSE, r_gbuffer, g_gbuffer,
	     b_gbuffer));

	r_gbuffer =
		malloc(values.calwidth * values.callines *
		       sizeof(unsigned int));
	g_gbuffer =
		malloc(values.calwidth * values.callines *
		       sizeof(unsigned int));
	...
	total_count = 0;
	red_done = green_done = blue_done = FALSE;
	...
	do {
		...
		RIE(gt68xx_afe_cis_read_lines
		    (&values, scanner, FALSE, first, r_obuffer,
		     g_obuffer, b_obuffer));
		RIE(gt68xx_afe_cis_read_lines
		    (&values, scanner, TRUE, FALSE, r_gbuffer,
		     g_gbuffer, b_gbuffer));

		if (!red_done)
			red_done =
				gt68xx_afe_cis_adjust_gain_offset("red",
								  &values,
								  r_obuffer,
								  r_gbuffer,
								  afe,
		...
		if (!green_done)
			green_done =
				gt68xx_afe_cis_adjust_gain_offset("green",
								  &values,
								  g_obuffer,
								  g_gbuffer,
								  afe,
		...
		if (!blue_done)
			blue_done =
				gt68xx_afe_cis_adjust_gain_offset("blue",
								  &values,
								  b_obuffer,
								  b_gbuffer,
								  afe,
	...
	do {
		...
		if (!red_done)
			red_done =
				gt68xx_afe_cis_adjust_exposure("red", &values,
							       r_gbuffer, 248,
							       &exposure->
							       r_time);
		if (!green_done)
			green_done =
				gt68xx_afe_cis_adjust_exposure("green",
							       &values,
							       g_gbuffer, 248,
							       &exposure->
							       g_time);
		if (!blue_done)
			blue_done =
				gt68xx_afe_cis_adjust_exposure("blue",
							       &values,
							       b_gbuffer, 248,
							       &exposure->
							       b_time);
	...
	while ((!red_done || !green_done || !blue_done)
	       && exposure_count < 50);
