...
#define add_sense_qual sense_buffer[13]
	...
	DBG(10, "sense code: %02x %02x %02x %02x %02x %02x %02x %02x "
	    ...
	    sense_buffer[0], sense_buffer[1], sense_buffer[2],
	    sense_buffer[3], sense_buffer[4], sense_buffer[5],
	    sense_buffer[6], sense_buffer[7], sense_buffer[8],
	    sense_buffer[9], sense_buffer[10], sense_buffer[11],
	    sense_buffer[12], sense_buffer[13], sense_buffer[14],
	    sense_buffer[15]);
	...
	if (sense_buffer[7] >= 5) {
		if (sdat->model == JX610) {
			...
			case 0x08:
				DBG(5,
				    "error: scanner communication failure (time out?)\n");
				return SANE_STATUS_IO_ERROR;
			case 0x1A:
				DBG(10,
			...
			case 0x39:
				DBG(10,
				    "error: saving parameter not supported\n");
				return SANE_STATUS_IO_ERROR;
			case 0x47:
			...
			case 0x02:	/* not ready */
				...
				case 0x80:
					...
					case 0:
						DBG(1,
						    "Scanner not ready: ADF cover open\n");
						if (sdat->
						    complain_on_errors &
					...
					case 1:
						DBG(1,
						    "Scanner not ready: ADF maintenance "
						    "cover open\n");
						if (sdat->
						    complain_on_errors &
						    COMPLAIN_ON_ADF_ERROR)
					...
					default:
						DBG(5,
						    "Scanner not ready: undocumented reason\n");
						return SANE_STATUS_IO_ERROR;
					}
				case 0x81:
					...
					case 0:
						DBG(1,
						    "Scanner not ready: FSU cover open\n");
						if (sdat->
						    complain_on_errors &
						    COMPLAIN_ON_FSU_ERROR)
							return SANE_STATUS_COVER_OPEN;
						else
							return SANE_STATUS_GOOD;
					case 1:
						DBG(1,
						    "Scanner not ready: FSU light dispersion "
						    "error\n");
						if (sdat->
						    complain_on_errors &
						    COMPLAIN_ON_FSU_ERROR) {
			...
			case 0x03:	/* medium error */
				...
				default:
					DBG(5,
					    "medium error: undocumented reason\n");
					return SANE_STATUS_IO_ERROR;
				}
			case 0x04:	/* hardware error */
				...
				case 0x08:
					DBG(1,
					    "hardware error: scanner communication failed\n");
					return SANE_STATUS_IO_ERROR;
				case 0x60:
					DBG(1,
					    "hardware error: lamp failure\n");
					return SANE_STATUS_IO_ERROR;
				case 0x62:
					DBG(1,
					    "hardware error: scan head positioning failed\n");
					return SANE_STATUS_IO_ERROR;
				default:
			...
			case 0x06:	/* unit attention */
				...
				case 0x29:
					DBG(5,
					    "unit attention: reset occured\n");
					return SANE_STATUS_GOOD;
				case 0x2a:
					DBG(5,
					    "unit attention: parameter changed by "
					    "another initiator\n");
					return SANE_STATUS_IO_ERROR;
				default:
					DBG(5,
					    "unit attention: exact reason not documented\n");
					return SANE_STATUS_IO_ERROR;
				}
...
static SANE_Status
mode_sense(int fd, void *modeparam_buf, size_t * modeparam_size, int page)
{
	static u_char cmd[6];
	SANE_Status status;
	DBG(11, "<< mode_sense ");
...
get_window(int fd, void *buf, size_t * buf_size)
{
	...
	static u_char cmd[10] =
		{ GET_WINDOW, 0, 0, 0, 0, 0, 0, 0, WINDOW_LEN, 0 };
	SANE_Status status;
	DBG(11, "<< get_window ");
	...
	for (i = 0; i < max_queue; i++) {
		...
		if (bytes_to_queue) {
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "reader_process: read command failed: %s",
	...
	while (s->bytes_to_read > 0) {
		...
		if (bc->shm_status == SHM_BUSY) {
			...
			if (status == SANE_STATUS_DEVICE_BUSY && retries) {
				...
				retries--;
				DBG(11,
				    "reader: READ command returned BUSY\n");
			...
			DBG(1, "btr: %i btq: %i nreq: %i nrcv: %i\n",
			    s->bytes_to_read, bytes_to_queue, bc->nreq,
			    bc->used);
#endif
			bc->start = 0;
		...
		if (bytes_to_queue) {
			...
			while (buf_status(bc) != SHM_EMPTY) {
				...
				if (cancel_requested(s)) {
#ifdef HAVE_SANEI_SCSI_OPEN_EXTENDED
					sanei_scsi_req_flush_all_extended(s->
									  fd);
#else
					sanei_scsi_req_flush_all();
#endif
...
get_max_scan_size(int fd, SHARP_Device * dev, int mode)
{
	...
	dev->info.tl_x_ranges[mode].max = SANE_FIX(PIX_TO_MM((m_subdev.
							      max_x[0] << 24)
							     +
							     (m_subdev.
							      max_x[1] << 16)
	...
	dev->info.br_x_ranges[mode].min =
		SANE_FIX(PIX_TO_MM(1, dev->info.mud));
	dev->info.br_x_ranges[mode].max =
		SANE_FIX(PIX_TO_MM
			 ((m_subdev.max_x[0] << 24) +
			  (m_subdev.max_x[1] << 16) +
			  (m_subdev.max_x[2] << 8) + m_subdev.max_x[3],
			  dev->info.mud));
	...
	if ((dev->sensedat.model != JX250 && dev->sensedat.model != JX350) ||
	    mode != SCAN_WITH_FSU)
		dev->info.tl_y_ranges[mode].max =
			SANE_FIX(PIX_TO_MM
				 ((m_subdev.max_y[0] << 24) +
				  (m_subdev.max_y[1] << 16) +
				  (m_subdev.max_y[2] << 8) +
				  m_subdev.max_y[3] - 1, dev->info.mud));
	else
	...
	dev->info.br_y_ranges[mode].min =
		SANE_FIX(PIX_TO_MM(1, dev->info.mud));
	dev->info.br_y_ranges[mode].max =
		SANE_FIX(PIX_TO_MM
			 ((m_subdev.max_y[0] << 24) +
			  (m_subdev.max_y[1] << 16) +
			  (m_subdev.max_y[2] << 8) + m_subdev.max_y[3],
			  dev->info.mud));
...
attach(const char *devnam, SHARP_Device ** devp)
{
	...
	{
		int bufsize = 4096;
		status = sanei_scsi_open_extended(devnam, &fd, &sense_handler,
						  &sensedat, &bufsize);
		if (status != SANE_STATUS_GOOD) {
...
set_gamma_caps(SHARP_Scanner * s)
{
	...
	if (strcmp(s->val[OPT_MODE].s, M_LINEART) == 0
		...
		s->opt[OPT_CUSTOM_GAMMA].cap &= ~SANE_CAP_INACTIVE;
		if (s->val[OPT_CUSTOM_GAMMA].w == FALSE) {
			s->opt[OPT_GAMMA].cap &= ~SANE_CAP_INACTIVE;
			s->opt[OPT_GAMMA_VECTOR].cap |= SANE_CAP_INACTIVE;
		} else {
			s->opt[OPT_GAMMA].cap |= SANE_CAP_INACTIVE;
...
init_options(SHARP_Scanner * s)
{
...
#endif
	init_string_option(s, SANE_NAME_HALFTONE_PATTERN,
			   SANE_TITLE_HALFTONE_PATTERN,
			   SANE_DESC_HALFTONE " (JX-330 only)", halftone_list,
			   OPT_HALFTONE, 0);
	...
	if (s->dev->info.adf_fsu_installed
	    && (s->dev->sensedat.model == JX330))
	...
	if (s->dev->sensedat.model == JX610) {
		...
		s->opt[OPT_PAPER].constraint_type =
	...
	if (s->dev->sensedat.model == JX610)
		init_string_option(s, "Paper size", "Paper size",
				   "Paper size", paper_list_jx610, OPT_PAPER,
				   1);
	else
		init_string_option(s, "Paper size", "Paper size",
				   "Paper size", paper_list_jx330, OPT_PAPER,
				   0);
	...
	init_string_option(s, "Gamma", "Gamma", "Gamma", gamma_list,
			   OPT_GAMMA, 1);
	...
	if (s->dev->sensedat.model == JX610 || s->dev->sensedat.model == JX330
	    ...
	    || s->dev->sensedat.model == JX320)
		init_string_option(s, "ResolutionList", "ResolutionList",
				   "ResolutionList", resolution_list_jx610,
				   OPT_RESOLUTION_LIST, RESOLUTION_MAX_JX610);
	else
		init_string_option(s, "ResolutionList", "ResolutionList",
				   "ResolutionList", resolution_list_jx250,
				   OPT_RESOLUTION_LIST, RESOLUTION_MAX_JX250);
#endif
	/* x resolution */
	...
	s->opt[OPT_TL_X].constraint.range =
		&s->dev->info.tl_x_ranges[default_source];
	...
	s->opt[OPT_TL_Y].constraint.range =
		&s->dev->info.tl_y_ranges[default_source];
	...
	s->opt[OPT_BR_X].constraint.range =
		&s->dev->info.br_x_ranges[default_source];
	...
	s->opt[OPT_BR_Y].constraint.range =
		&s->dev->info.br_y_ranges[default_source];
	...
	s->opt[OPT_EDGE_EMPHASIS].constraint_type =
		SANE_CONSTRAINT_STRING_LIST;
	...
	s->opt[OPT_THRESHOLD_R].constraint.range =
		&s->dev->info.threshold_range;
	...
	s->opt[OPT_THRESHOLD_G].constraint.range =
		&s->dev->info.threshold_range;
	...
	s->opt[OPT_THRESHOLD_B].constraint.range =
		&s->dev->info.threshold_range;
...
static int stop_on_fsu_error[2] =
	  ...
	  SANE_Auth_Callback __sane_unused__ authorize)
{
	...
	if (!fp) {
		...
		dp->info.complain_on_errors =
			COMPLAIN_ON_ADF_ERROR | COMPLAIN_ON_FSU_ERROR;
			...
			if (word[0] != '#') {
				if (strcmp(word, "option") == 0) {
					...
					if (strcmp(word, "buffers") == 0) {
						...
						if (end == word) {
							DBG(1,
							    "error in config file, line %i: number expected:\n",
							    linecount);
							DBG(1, "%s\n", line);
						} else if (i > 2)
							buffers[opt_index] =
							...
							buffers[opt_index] =
								2;
					} else if (strcmp(word, "buffersize")
						   == 0) {
						free(word);
						word = 0;
						sanei_config_get_string(lp,
						...
						if (strcmp(word, "auto") == 0)
							default_scan_mode
								[opt_index] =
								-1;
						else if (strcmp(word, "fsu")
							 == 0)
							default_scan_mode
								[opt_index] =
								SCAN_WITH_FSU;
						else if (strcmp(word, "adf")
							 == 0)
							default_scan_mode
								[opt_index] =
								SCAN_WITH_ADF;
						...
						DBG(1,
						    "error in config file, line %i: unknown option\n",
						    linecount);
						DBG(1, "%s\n", line);
					}
				} else {
		...
		case OPT_MODE:
			if (strcmp(val, M_LINEART) == 0) {
				s->opt[OPT_THRESHOLD].cap &=
					~SANE_CAP_INACTIVE;
#ifdef USE_COLOR_THRESHOLD
				s->opt[OPT_THRESHOLD_R].cap |=
					SANE_CAP_INACTIVE;
				s->opt[OPT_THRESHOLD_G].cap |=
					SANE_CAP_INACTIVE;
				s->opt[OPT_THRESHOLD_B].cap |=
					SANE_CAP_INACTIVE;
#endif
				if (s->dev->sensedat.model == JX330)
					s->opt[OPT_HALFTONE].cap &=
						~SANE_CAP_INACTIVE;
		...
		case OPT_RESOLUTION_LIST:
			if (info)
				*info |= SANE_INFO_RELOAD_OPTIONS |
					SANE_INFO_RELOAD_PARAMS;
			for (i = 0;
			     s->opt[OPT_RESOLUTION_LIST].constraint.
			     string_list[i]; i++) {
				if (strcmp
				    (val,
				     s->opt[OPT_RESOLUTION_LIST].constraint.
...
static SANE_Status
send_binary_g_table(SHARP_Scanner * s, int * a, int dtq)
{
	SANE_Status status;
	int i;
...
send_threshold_data(SHARP_Scanner * s)
{
	...
	len = sprintf((char *) &cmd[10], "%i/%i/%i/%i",
		      ...
		      s->val[OPT_THRESHOLD_B].w, s->val[OPT_THRESHOLD].w);
	cmd[8] = len;
...
sane_start(SANE_Handle handle)
{
...
#ifdef USE_FORK
	{
		...
		s->shmid = shmget(IPC_PRIVATE, sizeof(SHARP_rdr_ctl)
				  + s->dev->info.buffers *
				  (sizeof(SHARP_shmem_ctl) +
				   s->dev->info.bufsize), IPC_CREAT | 0600);
		if (s->shmid == -1) {
			free(s->buffer);
			s->buffer = 0;
			sanei_scsi_close(s->fd);
		...
		for (n = 0; n < s->dev->info.buffers; n++) {
			s->rdr_ctl->buf_ctl[n].buffer =
				(SANE_Byte *) & s->rdr_ctl->buf_ctl[s->dev->
								    info.
								    buffers]
	...
	if (s->dev->sensedat.model != JX610
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(10,
			    "sane_start: mode_select_adf_fsu failed: %s\n",
			    sane_strstatus(status));
			sanei_scsi_close(s->fd);
			s->fd = -1;
			return (status);
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(10,
			    "sane_start: MODE_SENSE/subdevice page failed\n");
			sanei_scsi_close(s->fd);
			s->fd = -1;
			return (status);
		}
	...
	wp.wdb.eletu =
		(!s->speed << 2) + (s->edge << 6) + (s->lightcolor << 4);
...
#if 0
	{
		...
		for (i = 0;
		     i <
		     sizeof(wp.wdb) + +sizeof(wp.wdbx330) +
		     sizeof(wp.wdbx250); i += 16) {
			DBG(1,
	...
	DBG(1,
	    "%d pixels per line, %d bytes, %d lines high, total %lu bytes, "
...
static SANE_Status
sane_read_direct(SANE_Handle handle, SANE_Byte * dst_buf, int max_len,
		 int * len)
{
	SHARP_Scanner *s = handle;
	SANE_Status status;
...
static SANE_Status
sane_read_shuffled(SANE_Handle handle, SANE_Byte * dst_buf, int max_len,
	...
	*len = 0;
	...
	while (max_len > 0 && s->bytes_to_read > 0) {
		if (eight_bit_data) {
			...
			bytes_per_line_in *= 3;
			max_line =
				s->params.bytes_per_line + bytes_per_line_in;
		...
		if (nread != ntest) {
			 ...
			 */
			DBG(1,
			    "Warning: could not read an integral number of scan lines\n");
...
SANE_Status
sane_read(SANE_Handle handle, SANE_Byte * dst_buf, int max_len,
	  int * len)
{
	SHARP_Scanner *s = handle;
	SANE_Status status;
