...
typedef struct
{
	int maximum_start_speed;	/* maximum speed allowed when accelerating from standstill. Unit: pixeltime/step */
	int maximum_speed;	/* maximum speed allowed. Unit: pixeltime/step */
	int minimum_steps;	/* number of steps used for default curve */
...
typedef struct
{
	int base_ydpi;	/* motor base steps. Unit: 1/" */
	int optical_ydpi;	/* maximum resolution in y-direction. Unit: 1/"  */
	int max_step_type;	/* maximum step type. 0-2 */
	int power_mode_count;	/* number of power modes */
...
typedef struct Genesys_Model
{
	...
	int ccd_type;	/* which SENSOR type do we have ? */
	int dac_type;	/* which DAC do we have ? */
	int gpo_type;	/* General purpose output type */
	int motor_type;	/* stepper motor type */
	int flags;	/* Which hacks are needed for this scanner? */
	/*@} */
	int shading_lines;	/* how many lines are used for shading calibration */
	int search_lines;	/* how many lines are used to search start position */
} Genesys_Model;
...
typedef struct Genesys_Current_Setup
{
	...
	int stagger;
	int max_shift;	/* max shift of any ccd component, including staggered pixels */
...
struct Genesys_Device
{
	...
	int scanhead_position_in_steps;
	int lamp_off_time;
