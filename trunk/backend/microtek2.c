...
sane_exit(void)
{
	...
	while (md_first_dev != NULL) {
		...
		for (i = 0; i < 4; i++) {
			if (md_first_dev->custom_gamma_table[i]) {
				DBG(100,
				    "free md_first_dev->custom_gamma_table[%d] at %p\n",
				    i,
				    (void *) md_first_dev->
				    custom_gamma_table[i]);
	...
	sane_get_devices(NULL, FALSE);	/* free list of SANE_Devices */
...
sane_get_devices(const SANE_Device *** device_list, SANE_Bool local_only)
{
	...
	DBG(100, "sane_get_devices: sd_list=%p, malloc'd %lu bytes\n",
	    (void *) sd_list,
	    (u_long) ((md_num_devices + 1) * sizeof(SANE_Device **)));
	...
	*device_list = sd_list;
	...
	while (md) {
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(10,
			    "sane_get_devices: test_unit_ready status '%s'\n",
			    sane_strstatus(status));
			md = md->next;
			continue;
		}
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!ms->scanning) {	/* get an estimate for the params */
		...
		get_scan_mode_and_depth(ms, &mode, &depth, &bits_pp_in,
					&bits_pp_out);
		...
		case MS_MODE_COLOR:
			if (mi->onepass) {
				...
				ms->params.last_frame = TRUE;
			} else {
				ms->params.format = SANE_FRAME_RED;
				ms->params.last_frame = FALSE;
			}
			break;
		case MS_MODE_GRAY:
		...
		if (val[OPT_RESOLUTION_BIND].w == TRUE) {
		...
		height_pixel =
			fabs(SANE_UNFIX(ms->val[OPT_BR_Y].w) *
			     y_pixel_per_mm - y1_pixel) + 0.5;
		ms->params.lines = (int) height_pixel;

		x1_pixel = SANE_UNFIX(ms->val[OPT_TL_X].w) * x_pixel_per_mm;
		width_pixel =
			fabs(SANE_UNFIX(ms->val[OPT_BR_X].w) *
			     x_pixel_per_mm - x1_pixel) + 0.5;
	...
	DBG(30, "sane_get_parameters: depth=%d, ppl=%d, bpl=%d\n",
	    ms->params.depth, ms->params.pixels_per_line,
	    ms->params.bytes_per_line);
...
sane_init(int *version_code, SANE_Auth_Callback authorize)
{
	...
	if (fp == NULL)
		...
		while (md_config_temp) {
			sanei_config_attach_matching_devices(md_config_temp->
							     device,
							     attach_one);
...
sane_open(SANE_String_Const name, SANE_Handle * handle)
{
	...
	*handle = NULL;
	...
	ms->fp = NULL;
	ms->gamma_table = NULL;
	ms->buf.src_buf = ms->buf.src_buffer[0] = ms->buf.src_buffer[1] =
		NULL;
...
sane_read(SANE_Handle handle, SANE_Byte * buf, int maxlen, int *len)
{
	...
	*len = 0;
	...
	if (!ms->scanning || ms->cancelled) {
		...
		DBG(15,
		    "sane_read: scan cancelled or scanner not scanning->cleanup\n");
		cleanup_scanner(ms);
		return status;
	}
...
add_device_list(SANE_String_Const dev_name, Microtek2_Device ** mdev)
{
	...
	while (md) {
		if (strcmp(hdev, md->name) == 0) {
			DBG(30,
			    "add_device_list: device '%s' already in list\n",
			    hdev);
...
attach(Microtek2_Device * md)
{
	...
	for (source_info = 1; source_info < 5; ++source_info)
		memcpy(&md->info[source_info],
		       &md->info[MD_SOURCE_FLATBED], sizeof(Microtek2_Info));
...
check_option(const char *cp, Config_Options * co)
{
	...
	if (strncmp(cp, "dump", 4) == 0 && isspace(cp[4])) {
		...
		if (*cp) {
			...
			if (*cp) {
				...
				co->strip_height = 14.0;
				DBG(30,
				    "check_option: option value wrong: %f\n",
				    co->strip_height);
			}
		}
	...
	} else if (strncmp(cp, "lightlid-35", 11) == 0 && isspace(cp[11])) {
	...
	} else if (strncmp(cp, "toggle-lamp", 11) == 0 && isspace(cp[11])) {
		cp = sanei_config_skip_whitespace(cp + 11);
		if (strncmp(cp, "on", 2) == 0) {
			cp = sanei_config_skip_whitespace(cp + 2);
			co->toggle_lamp = "on";
...
check_inquiry(Microtek2_Device * md, SANE_String * model_string)
{
	...
	if (mi->scsi_version != MI_SCSI_II_VERSION) {
		DBG(1,
		    "check_inquiry: Device is not a SCSI-II device, but 0x%02x\n",
		    mi->scsi_version);
		return SANE_STATUS_IO_ERROR;
	}
	...
	if (strncasecmp("MICROTEK", mi->vendor, INQ_VENDOR_L) != 0
		...
		DBG(1,
		    "check_inquiry: Device is not a Microtek, but '%.*s'\n",
	...
	case 0xa0:
		*model_string = "Phantom 330cx / Phantom 336cx / SlimScan C3";
		...
		md->model_flags |= MD_NO_SLIDE_MODE | MD_NO_GAMMA
#ifndef NO_PHANTOMTYPE_SHADING
			| MD_PHANTOM336CX_TYPE_SHADING
#endif
			| MD_READ_CONTROL_BIT | MD_NO_ENHANCEMENTS;
		md->opt_backend_calib_default = TRUE;
		md->opt_no_backtrack_default = TRUE;
	...
	case 0x9a:
		*model_string = "Phantom 636cx / C6";
		...
		md->model_flags |= MD_NO_SLIDE_MODE
			| MD_READ_CONTROL_BIT | MD_NO_GAMMA | MD_PHANTOM_C6;
		md->opt_backend_calib_default = TRUE;
		md->opt_no_backtrack_default = TRUE;
	...
	case 0xb0:
		*model_string = "ScanMaker X12USL";
		md->opt_backend_calib_default = TRUE;
		md->model_flags |= MD_16BIT_TRANSFER | MD_CALIB_DIVISOR_600;
		break;
	case 0xb3:
	...
	case 0xde:
		*model_string = "ScanMaker 9800XL";
		md->model_flags |= MD_NO_GAMMA | MD_16BIT_TRANSFER;
		md->opt_backend_calib_default = TRUE;
		md->opt_no_backtrack_default = TRUE;
		break;
	default:
...
do_authorization(char *ressource)
{
	...
	do {
		...
		if (!ferror(fp) && !feof(fp)) {
			...
			*p = '\0';
			user = linep;
			if (strncmp(user, username, SANE_MAX_USERNAME_LEN) !=
			    0)
...
dump_attributes(Microtek2_Info * mi)
{
	...
	switch (mi->model_code) {
	case 0x80:
		DBG(1, "Redondo 2000XL / ArtixScan 2020\n");
		break;
	case 0x81:
		DBG(1, "ScanMaker 4 / Aruba\n");
		break;
	case 0x82:
		DBG(1, "Bali\n");
		break;
	...
	case 0x84:
		...
		break;
	case 0x85:
		DBG(1, "ScanMaker V300 / Phantom parallel / TR3\n");
		break;
	case 0x86:
		DBG(1, "CCP\n");
		break;
	...
	case 0x88:
		...
		break;
	case 0x89:
		DBG(1, "ScanMaker 6400XL / A3-400\n");
		break;
	case 0x8a:
		DBG(1, "ScanMaker 9600XL / A3-600\n");
		break;
	case 0x8b:
		DBG(1, "Watt\n");
		break;
	case 0x8c:
		DBG(1, "ScanMaker V600 / TR6\n");
		break;
	case 0x8d:
		DBG(1, "ScanMaker V310 / Tr3 10-bit\n");
		break;
	case 0x8e:
		DBG(1, "CCB\n");
		break;
	case 0x8f:
		...
		break;
	case 0x90:
		DBG(1, "ScanMaker E3+ 10-bit\n");
		break;
	case 0x91:
		DBG(1, "ScanMaker X6 / Phantom 636\n");
		break;
	case 0x92:
		DBG(1, "ScanMaker E3+ / Vobis Highscan\n");
		break;
	case 0x93:
		DBG(1, "ScanMaker V310\n");
		break;
	case 0x94:
		DBG(1, "SlimScan C3 / Phantom 330cx / 336cx\n");
		break;
	case 0x95:
		DBG(1, "ArtixScan 1010\n");
		break;
	case 0x97:
		DBG(1, "ScanMaker V636\n");
		break;
	case 0x98:
		DBG(1, "ScanMaker X6EL\n");
		break;
	case 0x99:
		DBG(1, "ScanMaker X6 / X6USB\n");
		break;
	case 0x9a:
		DBG(1, "SlimScan C6 / Phantom 636cx\n");
		break;
	case 0x9d:
		DBG(1, "AGFA DuoScan T1200\n");
		break;
	case 0xa0:
		DBG(1, "SlimScan C3 / Phantom 336cx\n");
		break;
	case 0xac:
		DBG(1, "ScanMaker V6UL\n");
		break;
	case 0xa3:
		DBG(1, "ScanMaker V6USL\n");
		break;
	case 0xaf:
		DBG(1, "SlimScan C3 / Phantom 336cx\n");
		break;
	case 0xb0:
		DBG(1, "ScanMaker X12USL\n");
		break;
	case 0xb3:
		DBG(1, "ScanMaker 3600\n");
		break;
	case 0xb4:
		DBG(1, "ScanMaker 4700\n");
		break;
	case 0xb6:
		DBG(1, "ScanMaker V6UPL\n");
		break;
	case 0xb8:
		DBG(1, "ScanMaker 3700\n");
		break;
	case 0xde:
		DBG(1, "ScanMaker 9800XL\n");
		break;
	default:
		DBG(1, "Unknown\n");
		break;
...
parse_config_file(FILE * fp, Config_Temp ** ct)
{
	...
	*ct = hct1 = NULL;
	...
	while (sanei_config_read(s, sizeof(s), fp)) {
		...
		if (strncmp(sanei_config_skip_whitespace(s), "option ", 7) ==
		    0
		    || strncmp(sanei_config_skip_whitespace(s), "option\t",
			       7) == 0) {
			DBG(100,
			    "parse_config_file: found global option %s\n", s);
	...
	while (!feof(fp) && !ferror(fp)) {
		...
		if (strncmp(sanei_config_skip_whitespace(s), "option ", 7) ==
			...
			DBG(100,
			    "parse_config_file: found device option %s\n", s);
			...
			if (hct2 == NULL) {
				DBG(1,
				    "parse_config_file: malloc() failed\n");
				return;
			}
...
init_options(Microtek2_Scanner * ms, u_int8_t current_scan_source)
{
	...
	for (color = 0; color < 4; color++) {
		...
		DBG(100,
		    "init_options: md->custom_gamma_table[%d]=%p, malloc'd %lu bytes\n",
		    color, (void *) md->custom_gamma_table[color],
		    (u_long) (tablesize * sizeof(int)));
		if (md->custom_gamma_table[color] == NULL) {
			DBG(1,
			    "init_options: malloc for custom gamma table failed\n");
			return SANE_STATUS_NO_MEM;
		}
	...
	/* special options */
	val[OPT_RESOLUTION_BIND].w = TRUE;
	...
	val[OPT_LIGHTLID35].w = FALSE;

	/* auto adjustment of threshold during a lineart scan */
	val[OPT_AUTOADJUST].w = FALSE;
	...
	if (first_call) {
		...
		sod[OPT_HALFTONE].size =
			max_string_size(md->halftone_mode_list);
		sod[OPT_HALFTONE].cap |= SANE_CAP_INACTIVE;
		sod[OPT_HALFTONE].constraint_type =
			SANE_CONSTRAINT_STRING_LIST;
		sod[OPT_HALFTONE].constraint.string_list =
			md->halftone_mode_list;
		...
		sod[OPT_GEOMETRY_GROUP].constraint_type =
		...
		sod[OPT_ENHANCEMENT_GROUP].constraint_type =
			SANE_CONSTRAINT_NONE;
		...
		sod[OPT_GAMMA_MODE].size =
			max_string_size(md->gammamode_list);
		sod[OPT_GAMMA_MODE].constraint_type =
			SANE_CONSTRAINT_STRING_LIST;
		sod[OPT_GAMMA_MODE].constraint.string_list =
			md->gammamode_list;
		...
		sod[OPT_GAMMA_SCALAR].constraint.range =
			&md->scalar_gamma_range;
		...
		sod[OPT_GAMMA_SCALAR_R].constraint.range =
			&md->scalar_gamma_range;
		...
		sod[OPT_GAMMA_SCALAR_G].constraint.range =
			&md->scalar_gamma_range;
		...
		sod[OPT_GAMMA_SCALAR_B].constraint.range =
			&md->scalar_gamma_range;
		...
		sod[OPT_GAMMA_CUSTOM].size = option_size * sizeof(int);
		sod[OPT_GAMMA_CUSTOM].constraint.range =
			&md->custom_gamma_range;
		...
		sod[OPT_GAMMA_CUSTOM_R].size = option_size * sizeof(int);
		sod[OPT_GAMMA_CUSTOM_R].constraint.range =
			&md->custom_gamma_range;
		...
		sod[OPT_GAMMA_CUSTOM_G].size = option_size * sizeof(int);
		sod[OPT_GAMMA_CUSTOM_G].constraint.range =
			&md->custom_gamma_range;
		...
		sod[OPT_GAMMA_CUSTOM_B].size = option_size * sizeof(int);
		sod[OPT_GAMMA_CUSTOM_B].constraint.range =
			&md->custom_gamma_range;
		...
		sod[OPT_CHANNEL].constraint_type =
			SANE_CONSTRAINT_STRING_LIST;
		...
		sod[OPT_RESOLUTION_BIND].constraint_type =
			SANE_CONSTRAINT_NONE;
		...
		sod[OPT_DISABLE_BACKTRACK].constraint_type =
			SANE_CONSTRAINT_NONE;
			...
			Option_Value * val)
{
	...
	val[OPT_MIDTONE_R].w = val[OPT_MIDTONE_G].w = val[OPT_MIDTONE_B].w
		= MD_MIDTONE_DEFAULT;
	val[OPT_HIGHLIGHT_R].w = val[OPT_HIGHLIGHT_G].w =
		val[OPT_HIGHLIGHT_B].w = MD_HIGHLIGHT_DEFAULT;
	val[OPT_EXPOSURE_R].w = val[OPT_EXPOSURE_G].w =
		val[OPT_EXPOSURE_B].w = MD_EXPOSURE_DEFAULT;
		    ...
		    SANE_Action action, void *value, int *info)
{
	...
	case SANE_ACTION_SET_VALUE:	/* set option values */
		if (!SANE_OPTION_IS_SETTABLE(sod[option].cap)) {
			DBG(100,
			    "sane_control_option: option %d; action %d \n",
			    option, action);
			DBG(10,
			    "sane_control_option: trying to set unsettable option\n");
			return SANE_STATUS_INVAL;
		}
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(10,
			    "sane_control_option: invalid option value\n");
			return status;
		}
		/*  } */
		...
		case SANE_TYPE_BOOL:
			...
			if (!((*(SANE_Bool *) value == TRUE)
			      || (*(SANE_Bool *) value == FALSE))) {
				DBG(10,
				    "sane_control_option: invalid BOOL option value\n");
				return SANE_STATUS_INVAL;
			}
		...
		case SANE_TYPE_STRING:
			...
			if (val[option].s == NULL) {
				DBG(1,
				    "sane_control_option: strdup failed\n");
				return SANE_STATUS_NO_MEM;
			}
			break;
		...
		case OPT_BITDEPTH:
			...
			if (md->model_flags & MD_NO_GAMMA) {
				...
				md->custom_gamma_range.max =
					(int) max_gamma_value;
				sod[OPT_GAMMA_CUSTOM].size =
					size * sizeof(int);
				sod[OPT_GAMMA_CUSTOM_R].size =
					size * sizeof(int);
				sod[OPT_GAMMA_CUSTOM_G].size =
					size * sizeof(int);
				sod[OPT_GAMMA_CUSTOM_B].size =
					size * sizeof(int);
		...
		case OPT_SOURCE:
			...
			if (strcmp(val[option].s, MD_SOURCESTRING_FLATBED) ==
			    0)
				md->scan_source = MD_SOURCE_FLATBED;
			else if (strcmp(val[option].s, MD_SOURCESTRING_TMA) ==
				 0)
				md->scan_source = MD_SOURCE_TMA;
			else if (strcmp(val[option].s, MD_SOURCESTRING_ADF) ==
				 0)
				md->scan_source = MD_SOURCE_ADF;
			else if (strcmp(val[option].s, MD_SOURCESTRING_STRIPE)
				 == 0)
				md->scan_source = MD_SOURCE_STRIPE;
			else if (strcmp(val[option].s, MD_SOURCESTRING_SLIDE)
				 == 0)
				md->scan_source = MD_SOURCE_SLIDE;
			else {
				DBG(1,
				    "sane_control_option: unsupported option %s\n",
				    val[option].s);
				return SANE_STATUS_UNSUPPORTED;
			}
		...
		case OPT_CHANNEL:
			...
			if (strcmp(val[option].s, MD_CHANNEL_MASTER) == 0) {
				...
				sod[OPT_HIGHLIGHT_B].cap |= SANE_CAP_INACTIVE;
				sod[OPT_EXPOSURE_B].cap |= SANE_CAP_INACTIVE;
			} else if (strcmp(val[option].s, MD_CHANNEL_GREEN) ==
				   0) {
				sod[OPT_SHADOW].cap |= SANE_CAP_INACTIVE;
				sod[OPT_MIDTONE].cap |= SANE_CAP_INACTIVE;
				sod[OPT_HIGHLIGHT].cap |= SANE_CAP_INACTIVE;
				...
				sod[OPT_HIGHLIGHT_B].cap |= SANE_CAP_INACTIVE;
				sod[OPT_EXPOSURE_B].cap |= SANE_CAP_INACTIVE;
			} else if (strcmp(val[option].s, MD_CHANNEL_BLUE) ==
				   0) {
				sod[OPT_SHADOW].cap |= SANE_CAP_INACTIVE;
				sod[OPT_MIDTONE].cap |= SANE_CAP_INACTIVE;
				sod[OPT_HIGHLIGHT].cap |= SANE_CAP_INACTIVE;
		...
		case OPT_RESOLUTION_BIND:
			if (ms->val[option].w == FALSE) {
				ms->sod[OPT_Y_RESOLUTION].cap &=
					~SANE_CAP_INACTIVE;
			} else {
				ms->sod[OPT_Y_RESOLUTION].cap |=
					SANE_CAP_INACTIVE;
		...
		case OPT_BALANCE_FW:
			val[OPT_BALANCE_R].w = SANE_FIX((u_int8_t)
							((float) mi->
							 balance[0] / 2.55));
			val[OPT_BALANCE_G].w = SANE_FIX((u_int8_t)
							((float) mi->
							 balance[1] / 2.55));
			val[OPT_BALANCE_B].w = SANE_FIX((u_int8_t)
							((float) mi->
							 balance[2] / 2.55));
			if (info)
				*info |= SANE_INFO_RELOAD_OPTIONS;
...
sane_get_option_descriptor(SANE_Handle handle, int n)
{
	Microtek2_Scanner *ms = handle;
...
restore_gamma_options(SANE_Option_Descriptor * sod, Option_Value * val)
{
	...
	if (strcmp(val[OPT_MODE].s, MD_MODESTRING_COLOR) == 0) {
		...
		if (strcmp(val[OPT_GAMMA_MODE].s, MD_GAMMAMODE_LINEAR) == 0) {
			...
			sod[OPT_GAMMA_CUSTOM_G].cap |= SANE_CAP_INACTIVE;
			sod[OPT_GAMMA_CUSTOM_B].cap |= SANE_CAP_INACTIVE;
		} else if (strcmp(val[OPT_GAMMA_MODE].s, MD_GAMMAMODE_SCALAR)
			   == 0) {
			sod[OPT_GAMMA_BIND].cap &= ~SANE_CAP_INACTIVE;
			if (val[OPT_GAMMA_BIND].w == TRUE) {
				sod[OPT_GAMMA_SCALAR].cap &=
					~SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_SCALAR_R].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_SCALAR_G].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_SCALAR_B].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_CUSTOM].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_CUSTOM_R].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_CUSTOM_G].cap |=
					SANE_CAP_INACTIVE;
				sod[OPT_GAMMA_CUSTOM_B].cap |=
					SANE_CAP_INACTIVE;
			} else {
				sod[OPT_GAMMA_SCALAR].cap |=
					SANE_CAP_INACTIVE;
		...
		} else if (strcmp(val[OPT_GAMMA_MODE].s, MD_GAMMAMODE_CUSTOM)
			   == 0) {
			sod[OPT_GAMMA_BIND].cap &= ~SANE_CAP_INACTIVE;
			if (val[OPT_GAMMA_BIND].w == TRUE) {
				sod[OPT_GAMMA_CUSTOM].cap &=
...
calculate_sane_params(Microtek2_Scanner * ms)
{
	...
	if (!mi->onepass && ms->mode == MS_MODE_COLOR) {
		if (ms->current_pass == 1)
			...
			DBG(1,
			    "calculate_sane_params: invalid pass number %d\n",
			    ms->current_pass);
			return SANE_STATUS_IO_ERROR;
		}
...
get_scan_parameters(Microtek2_Scanner * ms)
{
	...
	if (ms->val[OPT_DISABLE_BACKTRACK].w == TRUE)
		ms->no_backtracking = 1;
	else
		ms->no_backtracking = 0;

	/* turn off the lamp during a scan */
	if (ms->val[OPT_LIGHTLID35].w == TRUE)
		ms->lightlid35 = 1;
	else
		ms->lightlid35 = 0;

	/* automatic adjustment of threshold */
	if (ms->val[OPT_AUTOADJUST].w == TRUE)
		ms->auto_adjust = 1;
	else
		ms->auto_adjust = 0;

	/* color calibration by backend */
	if (ms->val[OPT_CALIB_BACKEND].w == TRUE)
		ms->calib_backend = 1;
	else
		ms->calib_backend = 0;
	...
	if (ms->mode == MS_MODE_HALFTONE) {
		i = 0;
		while (strcmp
		       (md->halftone_mode_list[i], ms->val[OPT_HALFTONE].s))
			++i;
	...
	DBG(30,
	    "get_scan_parameters: mode=%d, depth=%d, bpp_in=%d, bpp_out=%d\n",
...
/*    ms->y1_dots -= 50;*/
	...
	DBG(30,
	    "get_scan_parameters: yres=%d, x1=%d, width=%d, y1=%d, height=%d\n",
	    ms->y_resolution_dpi, ms->x1_dots, ms->width_dots, ms->y1_dots,
	    ms->height_dots);

	/* Preview mode */
	if (ms->val[OPT_PREVIEW].w == TRUE) {
		ms->fastscan = TRUE;
		ms->quality = FALSE;
	} else {
		ms->fastscan = FALSE;
	...
	ms->brightness_m = (u_int8_t) (SANE_UNFIX(ms->val[OPT_BRIGHTNESS].w)
				       /
				       SANE_UNFIX(md->percentage_range.max) *
				       254.0) + 1;
	ms->brightness_r = ms->brightness_g = ms->brightness_b =
		ms->brightness_m;
...
get_scan_mode_and_depth(Microtek2_Scanner * ms,
			int *mode,
			int *depth,
			int *bits_per_pixel_in, int *bits_per_pixel_out)
{
...
scsi_send_gamma(Microtek2_Scanner * ms)
{
	...
	if ((3 * ms->lut_size_bytes) <= 0xffff) {	/*send Gamma with one command */
		...
		memcpy(cmd + SG_CMD_L, ms->gamma_table,
		       3 * ms->lut_size_bytes);
...
scsi_read_attributes(Microtek2_Info * pmi, char *device, u_int8_t scan_source)
{
	...
	if (md_dump >= 2)
		dump_area2(readattributes, sizeof(readattributes),
			   "scannerattributes");
...
scsi_read_control_bits(Microtek2_Scanner * ms)
{
	...
	if (md_dump >= 2)
		dump_area2(ms->control_bytes,
			   ms->n_control_bytes, "readcontrolbitsresult");
...
static SANE_Status
scsi_set_window(Microtek2_Scanner * ms, int n)
{				/* n windows, not yet */
	/* implemented */
	SANE_Status status;
	u_int8_t *setwindow;
		...
		int bytes_per_pixel)
{
	...
	if (buffer && (ms->dev->model_flags & MD_PHANTOM_C6) && endiantype) {
		...
		default:
			DBG(1,
			    "scsi_read_image: Unexpected bytes_per_pixel=%d\n",
			    bytes_per_pixel);
		}
	}
...
scsi_read_image_status(Microtek2_Scanner * ms)
{
	...
	/*    mi->new_image_status = TRUE;  *//* for testing */
	...
	status = sanei_scsi_cmd(ms->sfd, cmd, sizeof(cmd), &dummy,
				&dummy_length);
...
scsi_read_shading(Microtek2_Scanner * ms, u_int8_t * buffer, u_int32_t length)
{
	...
	DBG(30,
	    "scsi_read_shading: pos=%p, size=%d, word=%d, color=%d, dark=%d\n",
	...
	if (md_dump > 3)
		dump_area2(buffer, size, "readshadingresult");
...
scsi_send_shading(Microtek2_Scanner * ms,
		  u_int8_t * shading_data, u_int32_t length, u_int8_t dark)
{
	SANE_Bool endiantype;
	SANE_Status status;
	size_t size;
	...
	DBG(30,
	    "scsi_send_shading: pos=%p, size=%d, word=%d, color=%d, dark=%d\n",
	    shading_data, length, ms->word, ms->current_color, dark);
	...
	if (cmd == NULL) {
		DBG(1,
		    "scsi_send_shading: Couldn't get buffer for shading table\n");
		return SANE_STATUS_NO_MEM;
	}
...
scsi_read_system_status(Microtek2_Device * md, int fd)
{
	...
	if (fd == -1) {
		status = sanei_scsi_open(md->name, &sfd, scsi_sense_handler,
					 0);
		if (status != SANE_STATUS_GOOD) {
...
scsi_sense_handler(int fd, u_char * sense, void *arg)
{
	...
	case RQS_SENSEKEY_VENDOR:
		if (asc == 0x4a && ascq == 0x00)
			...
			DBG(5,
			    "scsi_sense_handler: Command sequence error\n");
		else if (asc == 0x4b && ascq == 0x00)
			DBG(5, "scsi_sense_handler: Data phase error\n");
		else if (asc == 0x40) {
			DBG(5,
			    "scsi_sense_handler: Hardware diagnostic failure:\n");
			switch (ascq) {
			...
			case RQS_ASCQ_DCOFF:
				DBG(5,
				    "scsi_sense_handler: DC Offset error\n");
				break;
			case RQS_ASCQ_GAIN:
			...
			case RQS_ASCQ_POS:
				DBG(5,
				    "scsi_sense_handler: Positoning error\n");
				break;
			default:
				DBG(5,
				    "scsi_sense_handler: Unknown combination of ASC"
			...
			DBG(5,
			    "scsi_sense_handler: Invalid bit in IDENTIFY\n");
		else if (asc == 0x2c && ascq == 0x02)
/* Ok */
			DBG(5,
			    "scsi_sense_handler: Invalid comb. of windows specfied\n");
		else if (asc == 0x20 && ascq == 0x00)
/* Ok */
			DBG(5,
			    "scsi_sense_handler: Invalid command opcode\n");
		else if (asc == 0x24 && ascq == 0x00)
/* Ok */
			DBG(5, "scsi_sense_handler: Invalid field in CDB\n");
		else if (asc == 0x26 && ascq == 0x00)
			DBG(5,
			    "scsi_sense_handler: Invalid field in the param list\n");
			...
			DBG(5,
			    "scsi_sense_handler: Unsupported logic. unit\n");
		else if (asc == 0x53 && ascq == 0x00) {
			DBG(5,
			    "scsi_sense_handler: ADF paper jam or no paper\n");
			...
			DBG(5,
			    "scsi_sense_handler: Scan Job stopped or cancelled\n");
			return SANE_STATUS_CANCELLED;
		} else if (asc == 0x3a && ascq == 0x00) {
			DBG(5,
			    "scsi_sense_handler: Media (ADF or TMA) not available\n");
			return SANE_STATUS_NO_DOCS;
			...
			DBG(5,
			    "scsi_sense_handler:  No additional sense information\n");
/* Ok */
		else if (asc == 0x1a && ascq == 0x00)
			DBG(5,
			    "scsi_sense_handler: Parameter list length error\n");
		else if (asc == 0x26 && ascq == 0x02)
			DBG(5,
			    "scsi_sense_handler: Parameter value invalid\n");
		else if (asc == 0x03 && ascq == 0x00)
			DBG(5,
			    "scsi_sense_handler: Peripheral device write fault - "
			    "Firmware Download Error\n");
		else if (asc == 0x2c && ascq == 0x01)
			DBG(5,
			    "scsi_sense_handler: Too many windows specified\n");
			...
			DBG(5,
			    "scsi_sense_handler: Unknown combination of SENSE KEY "
...
sane_start(SANE_Handle handle)
{
	...
	if (md->model_flags & MD_READ_CONTROL_BIT) {
		...
		DBG(100,
		    "sane_start: ms->control_bytes=%p, malloc'd %lu bytes\n",
		    ms->control_bytes, (u_long) ms->n_control_bytes);
		if (ms->control_bytes == NULL) {
			DBG(1,
			    "sane_start: malloc() for control bits failed\n");
...
/* with this line on some scanners (X6, 0x91) the Flamp goes on  */
		...
		DBG(100,
		    "sane_start: ms->gamma_table=%p, malloc'd %d bytes\n",
...
prepare_buffers(Microtek2_Scanner * ms)
{
	...
	strip_lines =
		(int) ((double) ms->y_resolution_dpi * md->opts.strip_height);
	if (strip_lines == 0)
		strip_lines = 1;
...
#else
	ms->src_max_lines =
		MIN(sanei_scsi_max_request_size / ms->bpl, strip_lines);
#endif
	...
	if (ms->mode == MS_MODE_COLOR
		...
		for (i = 0; i < 2; i++) {
			...
			ms->buf.src_buffer[i] =
				(u_int8_t *) malloc(ms->src_buffer_size +
						    extra_buf_size);
			DBG(100,
			    "prepare_buffers: ms->buf.src_buffer[%d]=%p,"
			    "malloc'd %d bytes\n", i, ms->buf.src_buffer[i],
			    ms->src_buffer_size + extra_buf_size);
			if (ms->buf.src_buffer[i] == NULL) {
				DBG(1,
				    "sane_start: malloc for scan buffer failed\n");
				status = SANE_STATUS_NO_MEM;
				goto cleanup;
			}
		}
		...
		DBG(100,
		    "sane_start: ms->buf.src_buf=%p, malloc'd %d bytes\n",
	...
	if (ms->auto_adjust == 1) {
		ms->temporary_buffer =
			(u_int8_t *) malloc(ms->remaining_bytes);
		DBG(100,
		    "sane_start: ms->temporary_buffer=%p, malloc'd %d bytes\n",
		    ms->temporary_buffer, ms->remaining_bytes);
		if (ms->temporary_buffer == NULL) {
			DBG(1,
			    "sane_start: malloc() for temporary buffer failed\n");
			status = SANE_STATUS_NO_MEM;
			goto cleanup;
		}
	} else
...
condense_shading(Microtek2_Scanner * ms)
{
	...
	if (md->shading_table_w == NULL) {
		DBG(1,
		    "condense shading: no shading table found, skip shading\n");
		return SANE_STATUS_GOOD;
	}
	...
	if (md->shading_table_d != NULL) {
		...
		if (ms->condensed_shading_d == NULL) {
			DBG(1,
			    "condense_shading: malloc for dark table failed\n");
			return SANE_STATUS_NO_MEM;
		}
	}
	...
	for (lfd_bit = 0;
	     (lfd_bit < mi->geo_width) && (count < (int) ms->ppl);
	     ++lfd_bit) {
			...
			for (color = 0; color < 3; ++color) {
				if ((ms->mode == MS_MODE_COLOR)
					...
					if (csh_offset > cond_length) {
						DBG(1,
						    "condense_shading: wrong control bits data, ");
						DBG(1,
						    "csh_offset (%d) > cond_length(%d)\n",
	...
	if (md_dump >= 3) {
		dump_area2(ms->condensed_shading_w, cond_length,
			   "condensed_shading_w");
		if (ms->condensed_shading_d != NULL)
...
read_shading_image(Microtek2_Scanner * ms)
{
		...
		/* ~~~~~~~~~~~~~~~~~~~~~~~ */
	{
		...
		if (ms->shading_image == NULL) {
			DBG(1,
			    "read_shading_image: malloc for buffer failed\n");
			return SANE_STATUS_NO_MEM;
		}
		...
		while (ms->src_remaining_lines > 0) {
			lines_to_read =
				MIN(max_lines, ms->src_remaining_lines);
			...
			if (status != SANE_STATUS_GOOD) {
				DBG(1,
				    "read_shading_image: read image failed: '%s'\n",
				    sane_strstatus(status));
				return status;
			}
		...
		status = prepare_shading_data(ms, lines,
					      &md->shading_table_d);
		if (status != SANE_STATUS_GOOD)
			return status;
...
/*    md->status.reserved17 &= ~MD_RESERVED17_ON; */
	...
	ms->shading_image = malloc(ms->bpl * ms->src_remaining_lines);
	DBG(100,
	    "read shading image: ms->shading_image=%p, malloc'd %d bytes\n",
	    ms->shading_image, ms->bpl * ms->src_remaining_lines);
	if (ms->shading_image == NULL) {
...
static SANE_Status
prepare_shading_data(Microtek2_Scanner * ms, u_int32_t lines,
		     u_int8_t ** data)
{
...
#define MICROTEK2_CALIB_USE_MEDIAN
	...
	if (*data == NULL) {
		*data = (u_int8_t *) malloc(length);
		...
		if (*data == NULL) {
			DBG(1,
			    "prepare_shading_data: malloc for shading table failed\n");
			return SANE_STATUS_NO_MEM;
		}
	}
#ifdef  MICROTEK2_CALIB_USE_MEDIAN
	...
	if (sortbuf == NULL) {
		DBG(1,
		    "prepare_shading_data: malloc for sort buffer failed\n");
		return SANE_STATUS_NO_MEM;
	}
#endif
	...
	case MI_DATAFMT_LPLCONCAT:
		if (ms->lut_entry_size == 1) {
			DBG(1,
			    "prepare_shading_data: wordsize == 1 unsupported\n");
			return SANE_STATUS_UNSUPPORTED;
		}
		for (color = 0; color < 3; color++) {
			for (i = 0; i < (mi->geo_width / mi->calib_divisor);
			     i++) {
				value = 0;
...
/*  use a median filter to get the shading data -- should be better */
	...
	default:
		DBG(1,
		    "prepare_shading_data: Unsupported data format 0x%02x\n",
		    mi->data_format);
...
read_cx_shading_image(Microtek2_Scanner * ms)
{
	...
	if (ms->shading_image == NULL) {
		DBG(1,
		    "read_cx_shading: malloc for cx_shading buffer failed\n");
		return SANE_STATUS_NO_MEM;
	}
...
calc_cx_shading_line(Microtek2_Scanner * ms)
{
	...
	if (ms->dark == 0) {	/* white shading data  */
		...
		DBG(100,
		    "calc_cx_shading: md->shading_table_w=%p, malloc'd %d bytes\n",
		    md->shading_table_w, shading_line_bytes);
		if (md->shading_table_w == NULL) {
			DBG(100,
			    "calc_cx_shading: malloc for white shadingtable failed\n");
			status = SANE_STATUS_NO_MEM;
			cleanup_scanner(ms);
		}
		...
		DBG(100,
		    "calc_cx_shading: md->shading_table_d=%p, malloc'd %d bytes\n",
		    md->shading_table_d, shading_line_bytes);

		if (md->shading_table_d == NULL) {
			DBG(1,
			    "calc_cx_shading: malloc for dark shading table failed\n");
			status = SANE_STATUS_NO_MEM;
			cleanup_scanner(ms);
		}
		...
		for (i = 0; i < shading_line_pixels; i++) {
			...
			for (line = 0; line < md->shading_length; line++) {
				...
				if (ms->word == 1) {
					current_byte =
						buf +
						(line * shading_data_bytes)
						+ color_offset +
						shading_line_pixels + i;
...
calculate_gamma(Microtek2_Scanner * ms, u_int8_t * pos, int color, char *mode)
{
	...
	if (strcmp(mode, MD_GAMMAMODE_SCALAR) == 0) {
		...
		for (i = 0; i < ms->lut_size; i++) {
			if (ms->lut_entry_size == 2)
				*((u_int16_t *) pos + i) =
					(u_int16_t) (src[i] / factor);
			else
				*((u_int8_t *) pos + i) =
					(u_int8_t) (src[i] / factor);
		}
...
/*    mi = &md->info[MD_SOURCE_FLATBED]; */
	...
	for (color = 0; color < 3; color++) {
		for (i = 0; i < (mi->geo_width / mi->calib_divisor); i++) {
			...
			default:
				DBG(1,
				    "Unsupported shading transfer function 0x%02x\n",
				    mi->shtrnsferequ);
				break;
			}
		}
...
set_exposure(Microtek2_Scanner * ms)
{
	...
	if (ms->lut_entry_size == 1) {
		DBG(1,
		    "set_exposure: 1 byte gamma output tables currently ignored\n");
		return;
	}
	...
	for (byte = 0; byte < ms->lut_size; byte++) {
		for (color = 0; color < 3; color++) {
			val32 = (u_int32_t) * ((u_int16_t *) from +
					       color * size + byte);
			val32 = MIN(val32 +
				    val32 * (2 * (u_int32_t) exposure / 100),
				    (u_int32_t) maxval);
			*((u_int16_t *) from + color * size + byte) =
				(u_int16_t) val32;
		}
	}
	...
	for (color = 0; color < 3; color++) {
		for (byte = 0; byte < size; byte++) {
			...
			val32 = MIN(val32 +
				    val32 * (2 *
					     (u_int32_t) exposure_rgb[color] /
					     100), (u_int32_t) maxval);
			*((u_int16_t *) from + color * size + byte) =
				(u_int16_t) val32;
		}
	}
...
reader_process(void *data)
{
	...
	while (ms->src_remaining_lines > 0) {
		...
		DBG(30,
		    "reader_process: transferlength=%d, lines=%d, linelength=%d, "
		    "real_bpl=%d, srcbuf=%p\n", ms->transfer_length,
		    ms->src_lines_to_read, ms->bpl, ms->real_bpl,
		    ms->buf.src_buf);
		...
		case MS_MODE_COLOR:
			...
			{
				DBG(1,
				    "reader_process: 3 pass not yet supported\n");
				return SANE_STATUS_IO_ERROR;
			} else {
				switch (mi->data_format) {
				case MI_DATAFMT_CHUNKY:
		...
		case MS_MODE_LINEARTFAKE:
			if (ms->auto_adjust == 1)
				status = auto_adjust_proc_data(ms,
							       &temp_current);
			else
...
segreg_proc_data(Microtek2_Scanner * ms)
{
	...
	pad = (int) ceil((double) (ms->ppl * ms->bits_per_pixel_in) / 8.0) %
		2;
	...
	DBG(30,
	    "segreg_proc_data: lines=%d, bpl=%d, ppl=%d, bpf=%d, bpp=%d,\n"
	    "depth=%d, pad=%d, freelines=%d, calib_backend=%d\n",
	    ms->src_lines_to_read, ms->bpl, ms->ppl, bpf, bpp, ms->depth, pad,
	    ms->buf.free_lines, ms->calib_backend);

	/* determine how many planes of each color are in the source buffer */
	from = ms->buf.src_buf;
	for (frame = 0; frame < 3 * ms->src_lines_to_read;
	     frame++, from += bpf) {
		switch (*from) {
		case 'R':
		...
		default:
			DBG(1,
			    "segreg_proc_data: unknown color indicator (1) "
			    "0x%02x\n", *from);
			return SANE_STATUS_IO_ERROR;
		}
	}
	...
	lines_to_deliver =
		ms->buf.planes[0][colseq2] + ms->buf.planes[1][colseq2];
...
segreg_copy_pixels(Microtek2_Scanner * ms)
{
	...
	DBG(100,
	    "segreg_copy_pixels: buffer 0x%p, right_to_left=%d, depth=%d\n",
	...
	for (pixel = 0; pixel < ms->ppl; pixel++) {
		for (color = 0; color < 3; color++) {
			...
			if (ms->depth > 8)
				...
				DBG(1,
				    "segreg_copy_pixels: Unknown depth %d\n",
				    ms->depth);
				return SANE_STATUS_IO_ERROR;
			}
...
lplconcat_copy_pixels(Microtek2_Scanner * ms,
		      u_int8_t ** from,
		      int right_to_left, int gamma_by_backend)
{
	Microtek2_Device *md;
	Microtek2_Info *mi;
	u_int32_t pixel;
	...
	if ((md->model_flags & MD_READ_CONTROL_BIT) && ms->calib_backend) {
		shading_factor =
			(float) pow(2.0,
				    (double) (md->shading_depth - ms->depth));
	...
	for (pixel = 0; pixel < ms->ppl; pixel++) {
		for (color = 0; color < 3; color++) {
			if (ms->depth > 8)
				...
				DBG(1,
				    "lplconcat_copy_pixels: Unknown depth %d\n",
				    ms->depth);
				return SANE_STATUS_IO_ERROR;
			}
...
wordchunky_proc_data(Microtek2_Scanner * ms)
{
	...
	from = ms->buf.src_buf;
	for (line = 0; line < (u_int32_t) ms->src_lines_to_read; line++) {
		status = wordchunky_copy_pixels(from, ms->ppl, ms->depth,
						ms->fp);
		if (status != SANE_STATUS_GOOD)
			return status;
		from += ms->bpl;
...
static SANE_Status
wordchunky_copy_pixels(u_int8_t * from, u_int32_t pixels, int depth,
		       FILE * fp)
{
	u_int32_t pixel;
	int color;
...
gray_proc_data(Microtek2_Scanner * ms)
{
	...
	do {
		status = gray_copy_pixels(ms,
					  from,
					  right_to_left, gamma_by_backend);
		if (status != SANE_STATUS_GOOD)
			return status;
...
gray_copy_pixels(Microtek2_Scanner * ms,
		 u_int8_t * from, int right_to_left, int gamma_by_backend)
{
	Microtek2_Device *md;
	u_int32_t pixel;
	u_int16_t val16;
	...
	if (ms->depth >= 8) {
		...
		pixel = 0;
		while (pixel < ms->ppl) {
			fputc((char) (((*from >> 4) & 0x0f) | (*from & 0xf0)),
			      ms->fp);
			++pixel;
			if (pixel < ms->ppl)
				fputc((char)
...
lineartfake_copy_pixels(Microtek2_Scanner * ms,
			u_int8_t * from,
			u_int32_t pixels,
			u_int8_t threshold, int right_to_left, FILE * fp)
{
	Microtek2_Device *md;
	u_int32_t pixel;
	u_int32_t bit;
	...
	DBG(30,
	    "lineartfake_copy_pixels: from=%p,pixels=%d,threshold=%d,file=%p\n",
	...
	for (pixel = 0; pixel < pixels; pixel++) {
		if ((md->model_flags & MD_READ_CONTROL_BIT)
		    && ms->calib_backend && (ms->condensed_shading_w != NULL))
			/* apply shading by backend */
		{
			get_cshading_values(ms,
					    0,
...
auto_adjust_proc_data(Microtek2_Scanner * ms, u_int8_t ** temp_current)
{
	...
	*temp_current += ms->transfer_length;
	...
	if (ms->src_remaining_lines == 0) {	/* we have read all the image data, *//* calculate threshold value */
...
get_cshading_values(Microtek2_Scanner * ms,
		    ...
		    int right_to_left, float *s_d, float *s_w)
{
	Microtek2_Device *md;
		...
		/* condensed shading is 8 bit data */
	{
		*s_w = (float) *(ms->condensed_shading_w + csh_offset);
		if (ms->condensed_shading_d != NULL)
			*s_d = (float) *(ms->condensed_shading_d +
					 csh_offset);
		else
			*s_d = 0.0;
	}
