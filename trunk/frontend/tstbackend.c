...
static void test_options(SANE_Device * device, int can_do_recursive);
 ...
 */
#ifdef __GNUC__
static int check(enum message_level, int condition, const char *format, ...)
...
#define GUARD1 ((int)0x5abf8ea5)
#define GUARD2 ((int)0xa58ebf5a)
...
/* Set the guards */
static void
guards_set(void *ptr, size_t size)
{
	int *p;

	p = (int *) (((unsigned char *) ptr) - GUARDS_SIZE);
	*p = GUARD1;

	p = (int *) (((unsigned char *) ptr) + size);
	*p = GUARD2;
}

/* Check that the guards have not been tampered with. */
static void
guards_check(void *ptr, size_t size)
{
	int *p;
...
test_parameters(SANE_Device * device, SANE_Parameters * params)
{
	...
	check(FATAL, ((p.last_frame == FALSE) ||
		      (p.last_frame == TRUE)),
	      "parameter last_frame is neither FALSE or TRUE (%d)",
	      p.last_frame);
		       ...
		       int can_do_recursive)
{
	...
	check(FATAL, (opt->type == SANE_TYPE_INT ||
	      ...
	      "type must be SANE_TYPE_INT or SANE_TYPE_FIXED (%d)",
	      opt->type);
	...
	for (i = 1; i < opt->constraint.word_list[0]; i++) {
		...
		status = sane_control_option(device, option_num,
					     SANE_ACTION_SET_VALUE, &val_int,
					     &info);

		check(FATAL, (status == SANE_STATUS_GOOD),
		      "cannot set a settable option (status=%s)",
...
test_options(SANE_Device * device, int can_do_recursive)
{
	...
	for (option_num = 0; option_num < num_dev_options; option_num++) {
		...
		check(WRN,
		      ((opt->
			cap & ~(SANE_CAP_SOFT_SELECT | SANE_CAP_HARD_SELECT |
				SANE_CAP_SOFT_DETECT | SANE_CAP_EMULATED |
				SANE_CAP_AUTOMATIC | SANE_CAP_INACTIVE |
				SANE_CAP_ADVANCED | SANE_CAP_ALWAYS_SETTABLE))
		       == 0), "invalid capabilities for option [%d, %s] (%x)",
		      option_num, opt->name, opt->cap);
		check(WRN, (opt->title != NULL),
		      "option [%d, %s] must have a title", option_num,
		      opt->name);
		check(WRN, (opt->desc != NULL),
		      "option [%d, %s] must have a description", option_num,
		      opt->name);
		...
		if (opt->type == SANE_TYPE_GROUP) {
			check(INF, (opt->name == NULL || *opt->name == 0),
			      "option [%d, %s] has a name", option_num,
			      opt->name);
		} else {
			if (option_num == 0) {
				check(ERR,
		...
		switch (opt->type) {
		case SANE_TYPE_BOOL:
			check(WRN, (opt->size == sizeof(int)),
			      "size of option %s is incorrect", opt->name);
			optval = guards_malloc(opt->size);
			optsize = opt->size;
			check(WRN,
			      (opt->constraint_type == SANE_CONSTRAINT_NONE),
			      "invalid constraint type for option [%d, %s] (%d)",
			      option_num, opt->name, opt->constraint_type);
			break;
		...
		case SANE_TYPE_STRING:
			check(WRN, (opt->size >= 1),
			      "size of option [%d, %s] must be at least 1 for the NUL terminator",
			      option_num, opt->name);
			check(INF, (opt->unit == SANE_UNIT_NONE),
			      "unit of option [%d, %s] is not SANE_UNIT_NONE",
			      option_num, opt->name);
			check(WRN,
			      (opt->constraint_type ==
		...
		case SANE_TYPE_GROUP:
			check(INF, (opt->unit == SANE_UNIT_NONE),
			      "option [%d, %s], unit is not SANE_UNIT_NONE",
			      option_num, opt->name);
			check(INF, (opt->size == 0),
			      "option [%d, %s], size is not 0", option_num,
			      opt->name);
			check(WRN,
			      (opt->constraint_type == SANE_CONSTRAINT_NONE),
			      "invalid constraint type for option [%d, %s] (%d)",
		...
		if (optval) {
			...
			if (SANE_OPTION_IS_GETTABLE(opt->cap)) {
				check(ERR, (status == SANE_STATUS_GOOD),
				      "cannot get option [%d, %s] value, although it is active (%s)",
				      option_num, opt->name,
				      sane_strstatus(status));
			} else {
				check(ERR, (status == SANE_STATUS_INVAL),
				      "was able to get option [%d, %s] value, although it is not active",
				      option_num, opt->name);
			}
			...
			if (SANE_OPTION_IS_SETTABLE(opt->cap)
				...
				check(ERR, (status == SANE_STATUS_GOOD),
				      "cannot set option [%d, %s] value, although it is active and settable (%s)",
				      option_num, opt->name,
				      sane_strstatus(status));
			} else {
				check(ERR, (status == SANE_STATUS_INVAL),
				      "was able to set option [%d, %s] value, although it is not active or settable",
				      option_num, opt->name);
			}
			...
			check(ERR, ((info == (int) 0xdeadbeef)
				    || (info == 0)),
			      "when getting option [%d, %s], info was set to %x",
			      option_num, opt->name, info);
			...
			if (SANE_OPTION_IS_SETTABLE(opt->cap)
				...
				check(ERR, ((info & ~(SANE_INFO_INEXACT |
					    ...
					    == 0),
				      "sane_control_option set some wrong bit in info (%d)",
				      info);
			...
			if (SANE_OPTION_IS_SETTABLE(opt->cap) &&
				...
				check(ERR, (status == SANE_STATUS_GOOD),
				      "cannot set the option [%d, %s] automatically.",
				      option_num, opt->name);
			} else {
				check(ERR, (status != SANE_STATUS_GOOD),
				      "was able to automatically set option [%d, %s], although it is not active or settable or automatically settable",
				      option_num, opt->name);
			}
			if (info & SANE_INFO_RELOAD_PARAMS) {
				test_parameters(device, NULL);
		...
		check(ERR,
		      ((opt->
			cap & (SANE_CAP_HARD_SELECT | SANE_CAP_SOFT_SELECT))
		       != (SANE_CAP_HARD_SELECT | SANE_CAP_SOFT_SELECT)),
		      "option [%d, %s], SANE_CAP_HARD_SELECT and SANE_CAP_SOFT_SELECT are mutually exclusive",
		      option_num, opt->name);
		if (opt->cap & SANE_CAP_SOFT_SELECT) {
			check(ERR, ((opt->cap & SANE_CAP_SOFT_DETECT) != 0),
			      "option [%d, %s], SANE_CAP_SOFT_DETECT must be set if SANE_CAP_SOFT_SELECT is set",
			      option_num, opt->name);
		}
		if ((opt->cap & (SANE_CAP_SOFT_SELECT |
			...
			check(ERR, (!SANE_OPTION_IS_SETTABLE(opt->cap)),
			      "option [%d, %s], must not be settable",
			      option_num, opt->name);
		}
		...
		if (opt->type == SANE_TYPE_STRING) {
			...
			check(FATAL, (status == SANE_STATUS_GOOD),
			      "cannot get option [%d, %s] value", option_num,
			      opt->name);
			...
			guards_check(optstr, opt->size);
			check(ERR, (status == SANE_STATUS_GOOD),
			      "cannot set option [%d, %s] value", option_num,
			      opt->name);
			...
			check(FATAL, (strcmp(optstr, val_string2) != 0),
			      "sane_control_option did not copy the string parameter for option [%d, %s]",
			      option_num, opt->name);
		...
		if (opt->type == SANE_TYPE_BOOL) {
			...
			status = sane_control_option(device, option_num,
						     ...
						     &org_v, &info);
			check(ERR, (status == SANE_STATUS_GOOD),
			      "cannot get boolean option [%d, %s] value (%s)",
			      option_num, opt->name, sane_strstatus(status));
			...
			status = sane_control_option(device, option_num,
						     ...
						     &v, &info);
			check(ERR, (status == SANE_STATUS_GOOD),
			      "cannot set boolean option [%d, %s] value (%s)",
			      option_num, opt->name, sane_strstatus(status));
			check(ERR, (v != org_v),
		...
		switch (opt->type) {
		case SANE_TYPE_BOOL:{
			int v;	/* should be SANE_Bool instead */

			v = -1;	/* invalid value. must be FALSE or TRUE */
			status = sane_control_option(device, option_num,
						     SANE_ACTION_SET_VALUE,
						     &v, NULL);
			check(ERR, (status != SANE_STATUS_GOOD),
			      "was able to set an invalid value for boolean option [%d, %s]",
			      option_num, opt->name);

			v = 2;	/* invalid value. must be FALSE or TRUE */
			status = sane_control_option(device, option_num,
						     SANE_ACTION_SET_VALUE,
						     &v, NULL);
			check(ERR, (status != SANE_STATUS_GOOD),
		...
		case SANE_TYPE_INT:{
			...
			if (opt->constraint_type == SANE_CONSTRAINT_RANGE) {
				...
				status = sane_control_option(device,
							     option_num,
							     SANE_ACTION_SET_VALUE,
							     v, &info);
				...
				check(ERR,
				      ...
				      "incorrect return when setting an invalid range value for option [%d, %s] (status %s, info %x)",
				      option_num, opt->name,
				      sane_strstatus(status), info);
				...
				check(ERR,
				      (status == SANE_STATUS_GOOD
				       && !(info & SANE_INFO_INEXACT)),
				      "incorrect return when setting a valid range value for option [%d, %s] (status %s, info %x)",
				      option_num, opt->name,
				      sane_strstatus(status), info);
			}
		...
		case SANE_CONSTRAINT_WORD_LIST:
			check(FATAL, (opt->constraint.word_list != NULL),
			      "no constraint list for option [%d, %s]",
			      option_num, opt->name);
			test_options_word_list(device, option_num, opt,
					       can_do_recursive);
			break;
		...
		case SANE_CONSTRAINT_STRING_LIST:
			check(FATAL, (opt->constraint.string_list != NULL),
			      "no constraint list for option [%d, %s]",
			      option_num, opt->name);
			test_options_string_list(device, option_num, opt,
						 can_do_recursive);
			break;

		case SANE_CONSTRAINT_RANGE:
			check(FATAL, (opt->constraint.range != NULL),
			      "no constraint range for option [%d, %s]",
			      option_num, opt->name);
			check(FATAL,
		...
		case SANE_CONSTRAINT_NONE:
			check(INF, (opt->constraint.range == NULL),
			      "option [%d, %s] has some constraint value set",
			      option_num, opt->name);
	...
	check(ERR, (opt == NULL),
	      "was able to get option descriptor for option %d",
	      num_dev_options + 1);
...
static void
set_min_value(SANE_Handle device, int option_num,
	      const SANE_Option_Descriptor * opt)
{
	SANE_Status status;
	SANE_String val_string;
	...
	case SANE_CONSTRAINT_STRING_LIST:
		...
		status = sane_control_option(device, option_num,
					     ...
					     val_string, NULL);
		check(ERR, (status == SANE_STATUS_GOOD),
		      "cannot set option %s to [%s] (%s)", opt->name,
		      val_string, sane_strstatus(status));
		free(val_string);
		break;
...
static void
set_max_value(SANE_Handle device, int option_num,
	      const SANE_Option_Descriptor * opt)
{
	SANE_Status status;
	SANE_String val_string;
	...
	case SANE_CONSTRAINT_WORD_LIST:
		...
		if (!rc)
			return;
		val_int =
			opt->constraint.word_list[opt->constraint.
						  word_list[0]];
		status = sane_control_option(device, option_num,
					     SANE_ACTION_SET_VALUE, &val_int,
					     NULL);
...
static void
set_random_value(SANE_Handle device, int option_num,
		 const SANE_Option_Descriptor * opt)
{
	SANE_Status status;
	SANE_String val_string;
...
get_option_value(SANE_Handle device, const char *option_name)
{
	...
	if (opt) {
		...
		if (status == SANE_STATUS_GOOD) {
			...
			case SANE_TYPE_INT:
				sprintf(str, "%d", *(int *) optval);
				break;

			case SANE_TYPE_FIXED:{
				int i;
				i = SANE_UNFIX(*(int *) optval);
				sprintf(str, "%d", i);
			}
				break;
...
static char *
display_scan_parameters(SANE_Handle device)
{
	static char str[150];
	char *p = str;
	...
	*p = 0;
	...
	p += sprintf(p, "scan mode=[%s] ",
		     get_option_value(device, SANE_NAME_SCAN_MODE));
	p += sprintf(p, "resolution=[%s] ",
		     get_option_value(device, SANE_NAME_SCAN_RESOLUTION));
	...
	p += sprintf(p, "br_y=[%s] ",
		     get_option_value(device, SANE_NAME_SCAN_BR_Y));
...
test_scan(SANE_Handle device)
{
	...
	if (opt)
		set_min_value(device, option_num, opt);
	...
	if (opt)
		set_max_value(device, option_num, opt);
...
#define IMAGE_SIZE (512 * 1024)
	...
	status = sane_set_io_mode(device, FALSE);
	check(ERR, (status == SANE_STATUS_INVAL),
	      "it is possible to sane_set_io_mode outside of a scan");
	status = sane_set_io_mode(device, TRUE);
	check(ERR, (status == SANE_STATUS_INVAL ||
	...
	if (test_level > 2) {
		...
		check(MSG, 0, "TEST: scan byte per byte - %s",
		      display_scan_parameters(device));

		test_parameters(device, &params);

		status = sane_start(device);
		rc = check(ERR, (status == SANE_STATUS_GOOD),
			   "cannot start the scan (%s)",
			   sane_strstatus(status));
		if (!rc)
			goto the_end;

		/* sane_set_io_mode with FALSE is always supported. */
		status = sane_set_io_mode(device, FALSE);
		check(ERR, (status == SANE_STATUS_GOOD),
		      "sane_set_io_mode with FALSE must return SANE_STATUS_GOOD");

		/* test sane_set_io_mode with TRUE. */
		status = sane_set_io_mode(device, TRUE);
		check(ERR, (status == SANE_STATUS_GOOD ||
			    status == SANE_STATUS_UNSUPPORTED),
		      "sane_set_io_mode with TRUE returned an invalid status (%s)",
		      sane_strstatus(status));
	 ...
	 * Do a partial scan 
	 */
	check(MSG, 0, "TEST: partial scan - %s",
	      display_scan_parameters(device));
	 ...
	 */
	check(MSG, 0, "TEST: scan random length - %s",
	      display_scan_parameters(device));
	 ...
	 */
	check(MSG, 0, "TEST: scan with a big max_len - %s",
	      display_scan_parameters(device));
...
test_scans(SANE_Device * device)
{
	...
	scan_mode_opt =
		get_optdesc_by_name(device, SANE_NAME_SCAN_MODE,
				    &scan_mode_optnum);
	if (scan_mode_opt) {

		rc = check(INF, (scan_mode_opt->type == SANE_TYPE_STRING),
			   "option [%s] is not a SANE_TYPE_STRING - skipping test",
			   SANE_NAME_SCAN_MODE);
		if (!rc)
			return;
		rc = check(INF,
			   (scan_mode_opt->constraint_type ==
			    SANE_CONSTRAINT_STRING_LIST),
			   "constraint for option [%s] is not SANE_CONSTRAINT_STRING_LIST - skipping test",
			   SANE_NAME_SCAN_MODE);
		if (!rc)
			return;
		rc = check(INF, (SANE_OPTION_IS_SETTABLE(scan_mode_opt->cap)),
			   "option [%s] is not settable - skipping test",
			   SANE_NAME_SCAN_MODE);
		if (!rc)
			return;
	}
	...
	resolution_mode_opt =
		get_optdesc_by_name(device, SANE_NAME_SCAN_RESOLUTION,
				    &resolution_mode_optnum);
	if (resolution_mode_opt) {
		rc = check(INF,
			   (SANE_OPTION_IS_SETTABLE
			    (resolution_mode_opt->cap)),
			   "option [%s] is not settable - skipping test",
	...
	if (scan_mode_opt) {
		...
		for (i = 0; scan_mode_opt->constraint.string_list[i] != NULL;
		     i++) {

			val_string =
				strdup(scan_mode_opt->constraint.
				       string_list[i]);
			assert(val_string);
...
main(int argc, char **argv)
{
	...
	for (i = 0; device_list[i] != NULL; i++) {
		...
		check(INF, ((strcmp(dev->type, "flatbed scanner") == 0) ||
			    ...
			    (strcmp(dev->type, "multi-function peripheral") ==
			     0)
			    || (strcmp(dev->type, "sheetfed scanner") == 0)),
		      "unknown device type [%s]. Update SANE doc section \"Type Strings\"",
		      dev->type);

		check(INF, ((strcmp(dev->vendor, "AGFA") == 0) ||
		      ...
		      "unknown device vendor [%s]. Update SANE doc section \"Vendor Strings\"",
		      dev->vendor);
	}
	...
	rc = check(ERR, (devname != NULL), "no SANE devices found");
	if (!rc)
		goto the_exit;
	...
	for (i = 0; i < 10; i++) {
		...
		rc = check(ERR, (status == SANE_STATUS_GOOD),
			   "sane_open failed with %s for device %s",
