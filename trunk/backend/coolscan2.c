	...
	if (device_list)
		...
		if (config) {
			...
			while (sanei_config_read(line, sizeof(line), config)) {
				...
				if (strlen(p) && (p[0] != '\n')
				    && (p[0] != '#'))
					cs2_open(line, CS2_INTERFACE_UNKNOWN,
						 NULL);
			}
			fclose(config);
		} else {
	...
	*h = (SANE_Handle) s;
	...
	for (i_option = 0; i_option < CS2_N_OPTIONS; i_option++) {
		...
		case CS2_OPTION_PREVIEW_RESOLUTION:
			...
			o.constraint_type = SANE_CONSTRAINT_WORD_LIST;
			word_list =
				(int *) cs2_xmalloc((s->resx_n_list + 1)
							  *
							  sizeof(int));
			if (!word_list)
				alloc_failed = 1;
		...
		case CS2_OPTION_RESY:
			...
			o.constraint_type = SANE_CONSTRAINT_WORD_LIST;
			word_list =
				(int *) cs2_xmalloc((s->resy_n_list + 1)
							  *
							  sizeof(int));
			if (!word_list)
				alloc_failed = 1;
...
SANE_Status
sane_control_option(SANE_Handle h, int n, SANE_Action a, void *v,
		    int * i)
{
	cs2_t *s = (cs2_t *) h;
	int flags = 0;
	...
	case SANE_ACTION_GET_VALUE:
		...
		case CS2_OPTION_PREVIEW:
			*(int *) v = s->preview;
			break;
		case CS2_OPTION_EXPOSURE:
			*(int *) v = SANE_FIX(s->exposure);
			break;
		case CS2_OPTION_EXPOSURE_R:
			*(int *) v = SANE_FIX(s->exposure_r);
			break;
		case CS2_OPTION_EXPOSURE_G:
			*(int *) v = SANE_FIX(s->exposure_g);
			break;
		case CS2_OPTION_EXPOSURE_B:
			*(int *) v = SANE_FIX(s->exposure_b);
			break;
		case CS2_OPTION_LUT_R:
			if (!(s->lut_r))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				((int *) v)[pixel] = s->lut_r[pixel];
			break;
		case CS2_OPTION_LUT_G:
			if (!(s->lut_g))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				((int *) v)[pixel] = s->lut_g[pixel];
			break;
		case CS2_OPTION_LUT_B:
			if (!(s->lut_b))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				((int *) v)[pixel] = s->lut_b[pixel];
			break;
		case CS2_OPTION_EJECT:
			break;
		case CS2_OPTION_LOAD:
		...
		default:
			DBG(4,
			    "Error: sane_control_option(): Unknown option (bug?).\n");
			return SANE_STATUS_INVAL;
		}
		break;
...
/* XXXXXXXXXXXXXXXXX do this for all elements of arrays */
		...
		case CS2_OPTION_PREVIEW:
			...
			break;
		case CS2_OPTION_EXPOSURE:
			s->exposure = SANE_UNFIX(*(int *) v);
			break;
		case CS2_OPTION_EXPOSURE_R:
			s->exposure_r = SANE_UNFIX(*(int *) v);
			break;
		case CS2_OPTION_EXPOSURE_G:
			s->exposure_g = SANE_UNFIX(*(int *) v);
			break;
		case CS2_OPTION_EXPOSURE_B:
			s->exposure_b = SANE_UNFIX(*(int *) v);
			break;
		case CS2_OPTION_LUT_R:
			if (!(s->lut_r))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				s->lut_r[pixel] = ((int *) v)[pixel];
			break;
		case CS2_OPTION_LUT_G:
			if (!(s->lut_g))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				s->lut_g[pixel] = ((int *) v)[pixel];
			break;
		case CS2_OPTION_LUT_B:
			if (!(s->lut_b))
				return SANE_STATUS_INVAL;
			for (pixel = 0; pixel < s->n_lut; pixel++)
				s->lut_b[pixel] = ((int *) v)[pixel];
			break;
		case CS2_OPTION_LOAD:
			cs2_load(s);
			break;
		...
		case CS2_OPTION_YMAX:
			...
			break;
		case CS2_OPTION_FOCUS_ON_CENTRE:
			s->focus_on_centre = *(int *) v;
			if (s->focus_on_centre) {
				s->option_list[CS2_OPTION_FOCUSX].cap |=
					SANE_CAP_INACTIVE;
				s->option_list[CS2_OPTION_FOCUSY].cap |=
					SANE_CAP_INACTIVE;
			} else {
				s->option_list[CS2_OPTION_FOCUSX].cap &=
					~SANE_CAP_INACTIVE;
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * p)
{
	...
	if (s->infrared_stage == CS2_INFRARED_OUT) {
		...
		p->bytes_per_line =
			s->n_colour_out * s->logical_width *
			s->bytes_per_pixel;
	}
	p->last_frame = TRUE;
...
sane_read(SANE_Handle h, SANE_Byte * buf, int maxlen, int * len)
{
	...
	if (s->infrared_stage == CS2_INFRARED_OUT) {
		...
		memcpy(buf, &(s->infrared_buf[s->xfer_position]),
		       xfer_len_out);
	...
	if ((s->type == CS2_TYPE_LS50) || (s->type == CS2_TYPE_LS5000)) {
		...
		if (i != 0)
			DBG(1,
			    "BUG: sane_read(): Read size is not a multiple of 64. (0x%06lx)\n",
			    (long) i);
	}
					...
					s8 = (u_int8_t *) & (s->
							     line_buf[s->
								      ...
								      *
								      index +
								      colour]);
				*s8 = s->recv_buf[colour * s->logical_width +
						  (colour +
						   1) * s->odd_padding +
						  index];
				break;
...
cs2_open(const char *device, cs2_interface_t interface, cs2_t ** sp)
{
	...
	case CS2_INTERFACE_USB:
		...
		DBG(6,
		    "cs2_open(): Trying to open %s, assuming USB interface ...\n",
		    device);
	...
	if (s->type != CS2_TYPE_UNKOWN)
		DBG(10,
		    "cs2_open(): Device identified as coolscan2 type #%i.\n",
		    s->type);
	else {
	...
	if (sp)
		*sp = s;
		...
		line = (char *) cs2_xmalloc(strlen(device) + strlen(prefix) +
					    1);
		if (!line)
			alloc_failed = 1;
		else {
...
cs2_pack_byte(cs2_t * s, SANE_Byte byte)
{
	while (s->send_buf_size <= s->n_send) {
		s->send_buf_size += 16;
		s->send_buf =
			(SANE_Byte *) cs2_xrealloc(s->send_buf,
						   s->send_buf_size);
		if (!s->send_buf)
			return SANE_STATUS_NO_MEM;
	}
	...
	for (i = 0; i < strlen(text); i += 2)
		if (text[i] == ' ')
			...
			if ((!isxdigit(text[i])) || (!isxdigit(text[i + 1])))
				DBG(1,
				    "BUG: cs2_parse_cmd(): Parser got invalid character.\n");
			c = 0;
			for (j = 0; j < 2; j++) {
				h = tolower(text[i + j]);
				if ((h >= 'a') && (h <= 'f'))
...
cs2_issue_cmd(cs2_t * s)
{
	...
	DBG(20,
	    "cs2_issue_cmd(): opcode = 0x%02x, n_send = %lu, n_recv = %lu.\n",
	...
	if (!s->n_cmd)
		...
		default:
			DBG(1,
			    "BUG: cs2_issue_cmd(): Unknown command opcode 0x%02x.\n",
			    s->send_buf[0]);
			break;
		}
	...
	case CS2_INTERFACE_USB:
		...
		if (status != SANE_STATUS_GOOD) {
			DBG(1,
			    "Error: cs2_issue_cmd(): Could not write command.\n");
			return SANE_STATUS_IO_ERROR;
		}
		...
		case CS2_PHASE_OUT:
			if (s->n_send - s->n_cmd < n_data || !n_data) {
				DBG(4,
				    "Error: cs2_issue_cmd(): Unexpected data out phase.\n");
				return SANE_STATUS_IO_ERROR;
			}
		...
		case CS2_PHASE_IN:
			if (s->n_recv < n_data || !n_data) {
				DBG(4,
				    "Error: cs2_issue_cmd(): Unexpected data in phase.\n");
				return SANE_STATUS_IO_ERROR;
			}
		...
		case CS2_PHASE_NONE:
			DBG(4,
			    "Error: cs2_issue_cmd(): No command received!\n");
			return SANE_STATUS_IO_ERROR;
		default:
			if (n_data) {
				DBG(4,
...
cs2_scanner_ready(cs2_t * s, int flags)
{
	...
	do {
		...
		if (status)
			if (--retry < 0)
				return status;
		if (++count > 240) {	/* 120s timeout */
			DBG(4,
			    "Error: cs2_scanner_ready(): Timeout expired.\n");
			status = SANE_STATUS_IO_ERROR;
			break;
		}
	}
...
cs2_page_inquiry(cs2_t * s, int page)
{
	...
	if (status) {
		DBG(4,
		    "Error: cs2_page_inquiry(): Inquiry of page failed: %s.\n",
		    sane_strstatus(status));
		return status;
	}
...
cs2_full_inquiry(cs2_t * s)
{
	...
	s->n_lut <<= s->maxbits;
	s->lut_r =
		(cs2_pixel_t *) cs2_xrealloc(s->lut_r,
					     s->n_lut * sizeof(cs2_pixel_t));
	s->lut_g =
		(cs2_pixel_t *) cs2_xrealloc(s->lut_g,
					     s->n_lut * sizeof(cs2_pixel_t));
	s->lut_b =
		(cs2_pixel_t *) cs2_xrealloc(s->lut_b,
					     s->n_lut * sizeof(cs2_pixel_t));
	s->lut_neutral =
...
cs2_get_exposure(cs2_t * s)
{
	...
	for (i_colour = 0; i_colour < 3; i_colour++) {	/* XXXXXXXXXXXXX CCCCCCCCCCCCC */
		cs2_scanner_ready(s, CS2_STATUS_NO_DOCS);
		...
		DBG(6,
		    "cs2_get_exposure(): exposure for colour %i: %li * 10ns\n",
...
cs2_convert_options(cs2_t * s)
{
	...
	if (s->preview)
		...
		if (s->infrared) {
			s->n_colour_in++;
			s->n_infrared_buf =
				s->bytes_per_pixel * s->logical_width *
				s->logical_height;
			infrared_buf_new =
				(SANE_Byte *) cs2_xrealloc(s->infrared_buf,
							   s->n_infrared_buf);
			if (infrared_buf_new)
...
cs2_scan(cs2_t * s, cs2_scan_t type)
{
	...
	case CS2_SCAN_NORMAL:
		...
		for (i_colour = 0; i_colour < s->n_colour_in; i_colour++) {
			...
			for (pixel = 0; pixel < s->n_lut; pixel++) {	/* XXXXXXXXXXXXXXX 2 bytes per point */
	...
	for (i_colour = 0; i_colour < s->n_colour_in; i_colour++) {
		...
		if (i_colour == 3)	/* infrared */
			...
			cs2_pack_byte(s,
				      s->
				      real_exposure[cs2_colour_list[i_colour]]
				      & 0xff);
		}
		status = cs2_issue_cmd(s);
		if (status)
	...
	if (s->status == CS2_STATUS_REISSUE) {
		...
		if ((s->type == CS2_TYPE_LS50)
			...
			if (status)
				return status;
			if ((s->recv_buf[11] != 0x08)
			    || (s->recv_buf[12] != 0x00))
				DBG(1,
				    "BUG: cs2_scan(): Unexpected block_padding position.\n");
			s->block_padding =
				256 * s->recv_buf[19] + s->recv_buf[20];
			cs2_init_buffer(s);
			switch (s->n_colour_in) {
			case 3:
				cs2_parse_cmd(s,
...
cs2_xmalloc(size_t size)
{
	...
	if (!value)
		DBG(0,
		    "Error: cs2_xmalloc(): Failed to malloc() %lu bytes.\n",
		    (unsigned long) size);
...
cs2_xrealloc(void *p, size_t size)
{
	...
	if (!value)
		DBG(0,
		    "Error: cs2_xrealloc(): Failed to realloc() %lu bytes.\n",
		    (unsigned long) size);
