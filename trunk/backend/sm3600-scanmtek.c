...
$Id: sm3600-scanmtek.c,v 1.11 2004-04-15 21:06:58 eichholz-guest Exp $
...
__SM3600EXPORT__ TState
WaitWhileBusy(TInstance * this, int cSecs)
{
...
__SM3600EXPORT__ TState
WaitWhileScanning(TInstance * this, int cSecs)
{
	int cTimeOut = cSecs * 10;
	int value;
	INST_ASSERT();
	...
	return SetError(this, SANE_STATUS_IO_ERROR,
			"Timeout while waiting for CSTAT");
}
...
__SM3600EXPORT__
	TState UploadGammaTable(TInstance * this, int iByteAddress,
				int * pnGamma)
{
	...
	if (!puchGamma)
		return SetError(this, SANE_STATUS_NO_MEM, "gamma buffer");
	...
	for (i = 0; rc == SANE_STATUS_GOOD && i < 0x2000; i += 0x1000)
		rc = MemWriteArray(this, (i + iByteAddress) >> 1, 0x1000,
				   puchGamma + i);
	free(puchGamma);
	return rc;
}
...
{
	{
		...
		for (i = iOff; i < MAX_PIXEL_PER_SCANLINE; i++) {
			...
			aGain[i - iOff].uchLow =
				(unsigned char) (uwGain & 0xFF);
			aGain[i - iOff].uchHigh =
				(unsigned char) (uwGain >> 8);
		}
		for (i = 0; i < 0x4000; i += 0x1000)
			MemWriteArray(this, (iTableOffset + i) >> 1, 0x1000,
				      ((unsigned char *) aGain) + i);
	}
	return SANE_STATUS_GOOD;
}
