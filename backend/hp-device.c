...
/* #define HP_TEST_SIMULATE */
	...
	if (!(sclsupport->checked))
		return SANE_STATUS_INVAL;
	if (!(sclsupport->is_supported))
		return SANE_STATUS_UNSUPPORTED;

	if (minval)
		*minval = sclsupport->minval;
	if (maxval)
		*maxval = sclsupport->maxval;
...
sanei_hp_device_support_probe(HpScsi scsi)
{
	...
	for (k = 0; k < (int) (sizeof(sclprobe) / sizeof(sclprobe[0])); k++) {
		...
		if (((sclprobe[k] == SCL_BRIGHTNESS)
		     || (sclprobe[k] == SCL_CONTRAST))
		    && (sanei_hp_device_probe(&compat, scsi) ==
			SANE_STATUS_GOOD) && (compat & HP_COMPAT_OJ_1150C)) {
			sclsupport->is_supported = 0;
		}

		if (sclsupport->is_supported) {
			DBG(1,
			    "hp_device_support_probe: %d supported (%d..%d, %d)\n",
			    inqid + HP_SCL_INQID_MIN, sclsupport->minval,
			    sclsupport->maxval, val);
		} else {
			    ...
			    int *model_num, const char **model_name)
{
	static struct
	{
		...
		{
		SCL_HP_MODEL_10, 10, "PhotoSmart Photo Scanner",
				HP_COMPAT_PS}, {
		SCL_HP_MODEL_11, 11, "OfficeJet 1150C", HP_COMPAT_OJ_1150C},
		{
		SCL_HP_MODEL_12, 12, "OfficeJet 1170C or later",
				HP_COMPAT_OJ_1170C}, {
	...
	if (last_device != NULL) {	/* Look if we already probed the device */
		if (strcmp(last_device, sanei_hp_scsi_devicename(scsi)) == 0) {
			DBG(3,
			    "probe_scanner: use cached compatibility flags\n");
			*compat = last_compat;
			if (model_num)
				*model_num = last_model_num;
			if (model_name)
...
hp_nonscsi_device_new(HpDevice * newp, const char *devname, HpConnect connect)
{
	...
	status = sanei_hp_device_probe_model(&(this->compat), scsi, 0,
					     &model_name);
	if (!FAILED(status)) {
