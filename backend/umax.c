...
{
	if ((scanner->val[OPT_RGB_BIND].w == SANE_FALSE) && (strcmp(scanner->val[OPT_MODE].s, COLOR_STR) == 0)) {	/* enable rgb options */
	...
	if ((scanner->device->inquiry_exposure_adj)
		...
		if ((scanner->val[OPT_RGB_BIND].w == SANE_FALSE) && (!scanner->device->exposure_time_rgb_bind) && (strcmp(scanner->val[OPT_MODE].s, COLOR_STR) == 0)) {	/* enable rgb exposure time options */
...
*/
	...
	scanner->val[OPT_CALIB_MODE].s =
		(SANE_Char *) strdup(calibration_list[0]);
	...
	if (action == SANE_ACTION_GET_VALUE) {
		...
		case SANE_TYPE_FIXED:
			DBG(DBG_sane_option, "set %s [#%d] to %f\n", name,
			    option, SANE_UNFIX(*(SANE_Word *) val));
			break;
		...
		case OPT_RESOLUTION_BIND:
		{
			if (scanner->val[option].w != *(SANE_Word *) val) {
				...
				if (scanner->val[option].w == SANE_FALSE) {	/* don't bind */
		...
		case OPT_SELECT_CAL_EXPOSURE_TIME:
		{
			if (scanner->val[option].w != *(SANE_Word *) val) {
				...
				if (scanner->val[OPT_SELECT_EXPOSURE_TIME].
					...
					if ((strcmp(scanner->val[OPT_MODE].s, COLOR_STR) != 0) || (scanner->val[OPT_RGB_BIND].w == SANE_TRUE) || (scanner->device->exposure_time_rgb_bind)) {	/* RGB bind */
						if (scanner->
						    val
						    [OPT_SELECT_CAL_EXPOSURE_TIME].
						    w) {
