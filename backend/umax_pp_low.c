...
static void
sanei_outsb(unsigned int port, const unsigned char *addr, unsigned long count)
{
	_outps8(port, (unsigned char *) addr, count);
}

static void
sanei_outsl(unsigned int port, const unsigned char *addr, unsigned long count)
{
...
static int ggamma[256] =
	...
	0x1F,
	0x21, 0x23, 0x24, 0x26, 0x28, 0x2A, 0x2B, 0x2D, 0x2E, 0x30, 0x31,
	0x33,
	0x34, 0x36, 0x37, 0x39, 0x3A, 0x3B, 0x3D, 0x3E, 0x40, 0x41, 0x42,
	0x43,
	0x45, 0x46, 0x47, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4F, 0x50, 0x51,
	0x52,
	0x53, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5E, 0x5F,
	0x60,
	...
	,
...
sanei_parport_find_device(void)
{
	...
	while (devices[i] != NULL) {
		...
		if (file < 0) {
			...
			case EACCES:
				DBG(16,
				    "current user cannot use existing %s device ...\n",
				    devices[i]);
				break;
			default:
				perror(devices[i]);
...
sanei_umax_pp_initPort(int port, char *name)
{
	...
	if ((name == NULL) || ((name != NULL) && (strlen(name) < 4))) {
		DBG(0,
		    "sanei_umax_pp_InitPort cannot use direct hardware access\n");
			...
			if (ioctl(fd, PPCLAIM)) {
				...
				if (ioctl(fd, PPGETMODES, &modes)) {
					...
					if (modes & PARPORT_MODE_PCSPP)
						sprintf(strmodes,
							"%s\t\tPARPORT_MODE_PCSPP\n",
							strmodes);
					if (modes & PARPORT_MODE_TRISTATE)
						sprintf(strmodes,
							"%s\t\tPARPORT_MODE_TRISTATE\n",
							strmodes);
...
/* the ppi device let user access to parallel port on freebsd */
	...
	if (port < 0x400) {
		if (sanei_ioperm(port, 8, 1) != 0) {
			DBG(1,
			    "sanei_ioperm() could not gain access to 0x%X\n",
			    port);
			return 0;
		}
...
waitFifoEmpty(void)
{
	...
	if (i == FIFO_WAIT) {
		DBG(0,
		    "waitFifoEmpty failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	return 1;
...
waitFifoNotEmpty(void)
{
	...
	if (i == FIFO_WAIT) {
		DBG(0,
		    "waitFifoNotEmpty failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	return 1;
...
waitFifoFull(void)
{
	...
	if (i == FIFO_WAIT) {
		DBG(0,
		    "waitFifoFull failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	return 1;
...
PS2Something(int reg)
{
	...
	if (breg != 0x08) {
		DBG(0,
		    "PS2Something failed, expecting 0x08, got 0x%02X (%s:%d)\n",
...
ECPregisterWrite(int reg, int value)
{
	...
	if (waitFifoEmpty() == 0) {
		DBG(0,
		    "ECPregisterWrite failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return;
	}
	breg = Inb(ECR);
...
ECPSetBuffer(int size)
{
	...
	if (waitFifoEmpty() == 0) {
		DBG(0,
		    "ECPSetBuffer failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return;
	}
	breg = Inb(ECR);
...
ECPbufferRead(int size, unsigned char *dest)
{
	...
	if (waitFifoEmpty() == 0) {
		DBG(0,
		    "ECPbufferRead failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return idx;
	}
	breg = Inb(ECR);
	...
	while (remain > 0) {
		if (waitFifoNotEmpty() == 0) {
			DBG(0,
			    "ECPbufferRead failed, FIFO time-out (%s:%d)\n",
			    __FILE__, __LINE__);
		}
...
ECPbufferWrite(int size, unsigned char *source)
{
	...
	if (waitFifoEmpty() == 0) {
		DBG(0,
		    "ECPWriteBuffer failed, time-out waiting for FIFO (%s:%d)\n",
		    __FILE__, __LINE__);
		return;
	}
	breg = Inb(ECR);
...
sync610p(void)
{
	...
	if (status != 0x38) {
		DBG(0,
		    "sync610p failed (got 0x%02X expected 0x38)! (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	Outb(CONTROL, 0x07);
	...
	if (status != 0xF8) {
		DBG(0,
		    "sync610p failed (got 0x%02X expected 0xF8)! (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	Outb(CONTROL, 0x05);
...
EPPcmdSync610p(int cmd)
{
	...
	if (cmd == 0xC2) {
		...
		if (status != 0xC0) {
			DBG(0,
			    "EPPcmdSync610p: Found 0x%X expected 0xC0 (%s:%d)\n",
			    status, __FILE__, __LINE__);
			/*return 0; */
		}
	}
...
disconnect610p(void)
{
	...
	for (i = 0; i < 41; i++) {
		...
		if (control != 0x04) {
			DBG(0,
			    "disconnect610p failed (idx %d=%02X)! (%s:%d)\n",
	...
	if (control != 0x0C) {
		DBG(0,
		    "disconnect610p failed expected 0x0C got %02X (%s:%d)\n",
...
sendWord1220P(int *cmd)
{
	...
	if ((reg != 0xC0) && (reg != 0xD0)) {
		DBG(0,
		    "sendWord failed  got 0x%02X instead of 0xC0 or 0xD0 (%s:%d)\n",
		    reg, __FILE__, __LINE__);
	...
	reg = reg & 0x10;
	if ((reg != 0x10) && (scannerStatus != 0x68)
	    && (scannerStatus != 0xA8)) {
	...
	if (try) {
		DBG(0,
		    "sendWord retry success (retry %d time%s) ... (%s:%d)\n",
		    try, (try > 1) ? "s" : "", __FILE__, __LINE__);
	}
	return 1;
}
...
SPPsendWord610p(int *cmd)
{
	...
	if (status != 0x88) {
		DBG(0,
		    "SPPsendWord610p found 0x%02X expected 0x88  (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	Outb(CONTROL, 0x04);
	...
	if (cmd[3] == 0xC2) {
		...
		if (tmp != 0xFF) {
			DBG(0,
			    "SPPsendWord610p found 0x%X expected 0xFF  (%s:%d)\n",
			    tmp, __FILE__, __LINE__);
			return 0;
		}
		status = Inb(STATUS) & 0xF8;
		...
		if (status != 0xC0) {
			DBG(0,
			    "SPPsendWord610p found 0x%X expected 0xC0  (%s:%d)\n",
			    status, __FILE__, __LINE__);
			return 0;
		}
		Outb(DATA, 0xFF);
	...
	do {
		...
		if (status != count) {
			...
			DBG(2, "status %d doesn't match count 0x%X! %s:%d\n",
			    status, count, __FILE__, __LINE__);
			Outb(CONTROL, control);
			Outb(DATA, data);
			return 0;
		}
...
sendLength(int *cmd, int len)
{
	...
	if (((reg == 0xC0) || (reg == 0xD0)) && (i != len)) {
		DBG(0,
		    "sendLength failed: sent only %d bytes out of %d (%s:%d)\n",
	...
	if ((reg != 0x10) && (scannerStatus != 0x68)
	    && (scannerStatus != 0xA8)) {
		DBG(0,
		    "sendLength failed: acknowledge not received (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	if (try) {
		DBG(0,
		    "sendLength retry success (retry %d time%s) ... (%s:%d)\n",
		    try, (try > 1) ? "s" : "", __FILE__, __LINE__);
	}
	return 1;
}
...
sendData610p(int *cmd, int len)
{
	...
	if (i < len) {
		DBG(0,
		    "sendData610p failed: sent only %d bytes out of %d (%s:%d)\n",
		    i, len, __FILE__, __LINE__);
		return 0;
	}
	return 1;
...
sendData(int *cmd, int len)
{
	...
	if ((reg != 0xC0) && (reg != 0xD0)) {
		DBG(0,
		    "sendData failed  got 0x%02X instead of 0xC0 or 0xD0 (%s:%d)\n",
		    reg, __FILE__, __LINE__);
		DBG(0, "Blindly going on .....\n");
	}
	...
	if (((reg == 0xC0) || (reg == 0xD0)) && (i < len)) {
		DBG(0,
		    "sendData failed: sent only %d bytes out of %d (%s:%d)\n",
		    i, len, __FILE__, __LINE__);
		return 0;
	}
...
pausedReadData(int size, unsigned char *dest)
{
	...
	if ((reg != 0xC0) && (reg != 0xD0)) {
		DBG(0,
		    "Unexpected reg19: 0x%02X instead of 0xC0 or 0xD0 (%s:%d)\n",
		    reg, __FILE__, __LINE__);
		return 0;
	}
	if (gMode == UMAX_PP_PARPORT_ECP) {
...
receiveData610p(int *cmd, int len)
{
	...
	if (status != 0xC0) {
		DBG(0,
		    "receiveData610p failed  got 0x%02X instead of 0xC0 (%s:%d)\n",
		    status, __FILE__, __LINE__);
		DBG(0, "Blindly going on .....\n");
	}
...
receiveData(int *cmd, int len)
{
	...
	if ((reg != 0x10) && (scannerStatus != 0x68)
	    && (scannerStatus != 0xA8)) {
		DBG(0,
		    "receiveData failed: acknowledge not received (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	return 1;
...
cmdSetDataBuffer(int *data)
{
	...
	int cmd2[] =
		...
		0xC1, 0x80,
		0x00, 0x20, 0x02, 0x00, 0x16, 0x41, 0xE0, 0xAC, 0x03, 0x03,
		0x00, 0x00,
...
initScanner610p(int recover)
{
	int first, rc, x;
	int cmd55AA[9] =
		{ 0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA, -1 };
	int cmd02[17] = { 0x02, 0x80, 0x00, 0x40, 0x30, 0x00, 0xC0, 0x2F,
		0x2F, 0x07, 0x00, 0x00, 0x00, 0x80, 0xF0, 0x00, -1
	};
	int op01[17] =
		{ 0x01, 0x00, 0x32, 0x70, 0x00, 0x00, 0xC0, 0x2F, 0x17, 0x05,
		0x00, 0x00,
		0x00, 0x80, 0xA4, 0x00, -1
	};
	int op11[17] =
		{ 0x01, 0x80, 0x0C, 0x70, 0x00, 0x00, 0xC0, 0x2F, 0x17, 0x01,
		0x00, 0x00,
		0x00, 0x80, 0xA4, 0x00, -1
	};
	int op21[17] =
		{ 0x01, 0x00, 0x01, 0x40, 0x30, 0x00, 0xC0, 0x2F, 0x17, 0x05,
		0x00, 0x00,
		0x00, 0x80, 0xF4, 0x00, -1
	};
	int op31[17] =
		{ 0x01, 0x00, 0x39, 0x73, 0x00, 0x00, 0xC0, 0x2F, 0x17, 0x05,
		0x00, 0x00,
...
sanei_umax_pp_initScanner(int recover)
{
	...
	int sentcmd[17] =
		{ 0x02, 0x80, 0x00, 0x70, 0x00, 0x00, 0x00, 0x2F, 0x2F, 0x07,
		0x00,
	...
	/* head homing needed ? */
	if ((readcmd[34] != 0x1A) || (recover == 1)) {	/* homing needed, readcmd[34] should be 0x48 */
		int op01[17] =
			{ 0x01, 0x00, 0x32, 0x70, 0x00, 0x00, 0x60, 0x2F,
			0x17, 0x05, 0x00,
			0x00, 0x00, 0x80, 0xE4, 0x00, -1
		};
		int op05[17] =
			{ 0x01, 0x00, 0x01, 0x70, 0x00, 0x00, 0x60, 0x2F,
			0x13, 0x05, 0x00,
			0x00, 0x00, 0x80, 0xF0, 0x00, -1
		};
		int op02[37] =
			...
			0x00, 0x04, 0x40,
			0x01, 0x00, 0x20, 0x02, 0x00, 0x16, 0x00, 0x70, 0x9F,
			0x06, 0x00,
			0x00, 0xF6, 0x4D, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9,
			0x68, 0xDF,
			0x0B, 0x1A, 0x00, -1
		};
		int op04[37] =
			{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C,
			0x00, 0x03, 0xC1,
			0x80, 0x00, 0x20, 0x02, 0x00, 0x16, 0x80, 0x15, 0x78,
			0x03, 0x03,
			0x00, 0x00, 0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9,
			0x68, 0xDF,
			0x0B, 0x1A, 0x00, -1
		};
		int op03[9] =
			{ 0x00, 0x00, 0x00, 0xAA, 0xCC, 0xEE, 0xFF, 0xFF,
			-1
		};
		...
		if (!recover) {
			...
			do {
				...
				CMDSYNC(0xC2);
				DBG(16,
				    "PARKING polling status is 0x%02X   (%s:%d)\n",
				    sanei_umax_pp_scannerStatus(), __FILE__,
				    __LINE__);
			}
			while (sanei_umax_pp_scannerStatus() == 0x90);
		}
...
{				/* ECP OK !! */
	...
	if (reg != gEPAT) {
		DBG(16,
		    "Error! expected reg0B=0x%02X, found 0x%02X! (%s:%d) \n",
	...
	if (reg != 0x00) {
		if (reg != 0x1C) {
			DBG(0,
			    "Warning! expected reg0A=0x00, found 0x%02X! (%s:%d) \n",
			    reg, __FILE__, __LINE__);
		} else {
...
probeEPP(unsigned char *dest)
{
	...
	for (i = 0; i < 10; i++) {
		...
		for (j = 0; j < 512; j++) {
			if (dest[2 * j] != (j % 256)) {
				DBG(0,
				    "Loop %d, char %d bufferRead failed! (%s:%d)\n",
				    i, j * 2, __FILE__, __LINE__);
				return 0;
			}
			if (dest[2 * j + 1] != (0xFF - (j % 256))) {
				DBG(0,
				    "Loop %d, char %d bufferRead failed! (%s:%d)\n",
				    i, j * 2 + 1, __FILE__, __LINE__);
				return 0;
			}
		}
		DBG(16, "Loop %d: bufferRead(0x400,dest) passed... (%s:%d)\n",
...
/* reset sequence */
	...
	if (breg != 0x48) {
		DBG(0,
		    "probeECP() failed, status=0x%02X, expected 0x48 (%s:%d)\n",
		    breg, __FILE__, __LINE__);
		return 0;
	}
	Outb(CONTROL, 0x04);
	...
	if (breg != 0xC8) {
		DBG(0,
		    "probeECP() failed, status=0x%02X, expected 0xC8 (%s:%d)\n",
		    breg, __FILE__, __LINE__);
		return 0;
	}
...
/* routine C */
	...
	if (breg != 0x0B) {
		DBG(0,
		    "probeECP() failed, reg10=0x%02X, expected 0x0B (%s:%d)\n",
		    breg, __FILE__, __LINE__);
		/* return 0; */
	}
	...
	if (breg != 0x04) {
		DBG(0,
		    "Warning! expected reg0C=0x04, found 0x%02X! (%s:%d) \n",
		    breg, __FILE__, __LINE__);
	}
	...
	for (i = 0; i < 3; i++) {	/* will go in a function */
		ClearRegister(0);
		if (waitAck() != 1) {
			DBG(0,
			    "probeECP failed because of waitAck() (%s:%d) \n",
			    __FILE__, __LINE__);
...
sanei_umax_pp_probeScanner(int recover)
{
	...
	if (!tmp) {
		...
		if (!tmp) {
			DBG(1,
			    "No scanner detected by 'ringScanner(5,0)'...\n");
	...
	if (!tmp) {
		DBG(1,
		    "No 1220P/2000P scanner detected by 'ringScanner()'...\n");
	}
	...
	if (testVersion(1) == 0) {	/* software doesn't do it for model 0x07 *//* but it works ..                       */
	...
	if ((model != 0x1F) && (model != 0x07)) {
		...
		DBG(0,
		    "There is a new scanner revision in town, or a bug ....\n");
	}
	...
	if (probePS2(dest)) {	/* PS2 mode works */
	...
	if (reg != 0x1C) {
		DBG(0,
		    "Warning! expected reg0A=0x1C, found 0x%02X! (%s:%d) \n",
		    reg, __FILE__, __LINE__);
	}
	DBG(16, "(%s:%d) passed \n", __FILE__, __LINE__);
	...
	/* if (model == 0x07) */
	{
		...
		if (reg != 0x00) {
			DBG(0,
			    "Warning! expected reg13=0x00, found 0x%02X! (%s:%d) \n",
			    reg, __FILE__, __LINE__);
		}
...
/* end of countdown */
	...
	for (i = 0; i < 3; i++) {
		if (cmdSetDataBuffer(initbuf) != 1) {
			DBG(0,
			    "cmdSetDataBuffer(initbuf) failed ! (%s:%d) \n",
			    __FILE__, __LINE__);
			return 0;
		}
		DBG(16,
		    "Loop %d: cmdSetDataBuffer(initbuf) passed... (%s:%d)\n",
		    i, __FILE__, __LINE__);
		if (cmdSetDataBuffer(voidbuf) != 1) {
			DBG(0,
			    "Loop %d: cmdSetDataBuffer(voidbuf) failed ! (%s:%d) \n",
			    i, __FILE__, __LINE__);
			return 0;
		}
	}
...
EPPcmdGet610p(int cmd, int len, int *val)
{
	...
	if (status != 0xC8) {
		DBG(1,
		    "EPPcmdGet610p: loop %d, found 0x%02X expected 0xC8  (%s:%d)\n",
...
EPPcmdSet610p(int cmd, int len, int *val)
{
	...
	if ((status != 0xC8) && (status != 0xC0)) {
		DBG(0,
		    "EPPcmdSet610p: Found 0x%X expected 0xC0 or 0xC8  (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	status = EPPputByte610p(0xAA);
	...
	if ((status != 0xC8) && (status != 0xC0)) {
		DBG(0,
		    "EPPcmdSet610p: Found 0x%02X expected 0xC0 or 0xC8  (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	...
	if ((status != 0xC8) && (status != 0xC0)) {
		DBG(0,
		    "EPPcmdSet610p: loop %d, found 0x%02X expected 0xC0 or 0xC8  (%s:%d)\n",
		    i, status, __FILE__, __LINE__);
		return 0;
	}
	...
	if (status != 0xC8) {
		DBG(0,
		    "EPPcmdSet610p: loop %d, found 0x%02X expected 0xC8 (%s:%d)\n",
		    i, status, __FILE__, __LINE__);
		return 0;
	}
...
EPPcmdGetBuffer610p(int cmd, int len, unsigned char *buffer)
{
	...
	if ((status != 0xD0) && (status != 0xC8)) {
		DBG(0,
		    "EPPcmdGetBuffer610p: Found 0x%X expected 0xC8 or 0xD0 (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
	status = EPPputByte610p(0xAA);
	if ((status != 0xD0) && (status != 0xC8)) {
		DBG(0,
		    "EPPcmdGetBuffer610p: Found 0x%02X expected 0xC8 or 0xD0 (%s:%d)\n",
		    status, __FILE__, __LINE__);
		return 0;
	}
...
cmdGetBuffer(int cmd, int len, unsigned char *buffer)
{
	...
	if (reg != 0x04) {
		DBG(0,
		    "cmdGetBuffer failed: unexpected status 0x%02X  ...(%s:%d)\n",
		    reg, __FILE__, __LINE__);
		return 0;
	}
	...
	while (read < len) {
		...
		if (gMode == UMAX_PP_PARPORT_ECP) {
			...
			DBG(16,
			    "ECPbufferRead(%d,buffer+read) passed (%s:%d)\n",
			    needed, __FILE__, __LINE__);
			REGISTERWRITE(0x1A, 0x84);
		} else {
		...
		if (tmp < needed) {
			DBG(64,
			    "cmdGetBuffer only got %d bytes out of %d ...(%s:%d)\n",
		...
		read += tmp;
		DBG(16,
		    "Read %d bytes out of %d (last block is %d bytes) (%s:%d)\n",
		...
		if (read < len) {
			...
			if ((reg & 0x08) == 0x08) {
				...
				while ((pass < 32768)
				       && ((reg & 0x08) == 0x08));
				DBG(64,
				    "Status after waiting is 0x%02X (pass=%d) (%s:%d)\n",
...
cmdGetBuffer32(int cmd, int len, unsigned char *buffer)
{
	...
	if ((reg != 0xC0) && (reg != 0xD0)) {
		DBG(0,
		    "cmdGetBuffer32 failed: unexpected status 0x%02X  ...(%s:%d)\n",
		    reg, __FILE__, __LINE__);
		return 0;
	}
	reg = registerRead(0x0C);
...
decodeDC(int *motor)
{
	DBG(0, "DC (R,G,B)=(%d,%d,%d)\n",
	    (motor[11] & 0xF0) >> 4, (motor[12] & 0x3C) >> 2,
	    motor[13] & 0x0F);
}
...
decodeVGA(int *motor)
{
	if (sanei_umax_pp_getastra() > 610) {
		DBG(0, "VGA (R,G,B)=(%d,%d,%d)\n",
		    (motor[10] & 0xF0) >> 4, (motor[10] & 0x0F),
		    (motor[11] & 0x0F));
	} else {
		DBG(0, "VGA (R,G,B)=(%d,%d,%d)\n",
		    (motor[11] & 0x0F), (motor[10] & 0xF0) >> 4,
		    (motor[10] & 0x0F));
	}
}
...
encodeCoefficient(int color, int dpi, int *calibration)
{
	 ...
	 */
	int color1220p[5][2] =
		{ {0x80, 0xAA}, {0x88, 0xFF}, {0xAA, 0xFF}, {0xFF, 0xFF},
	{0xFF, 0xFF}
	};
	int gray1220p[5][2] =
		{ {0x80, 0x88}, {0x88, 0xAA}, {0xAA, 0xFF}, {0xFF, 0xFF},
	{0xFF, 0xFF}
	};
...
bloc2Decode(int *op)
{
	...
	if (op[14] & 0x04) {
		...
		DBG(0,
		    "\t->move and scan (head doesn't stop at each row)    \n");
	}
	DBG(0, "\n");
}
...
static void
computeCalibrationData(int color, int width, unsigned char *source, int *data)
{
	int p, i, l;
	int sum;
	...
	/* 0 -> 5099 */
	for (i = 0; i < width; i++) {	/* red calibration data */
		if (color >= RGB_MODE) {
			/* compute average */
			sum = 0;
			for (l = 0; l < 66; l++)
...
sanei_umax_pp_park(void)
{
	...
	int body610[35] = {
		0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x03,
		0xC1, 0x80,
		0x00, 0x20, 0x02, 0x00, 0x16, 0x80, 0x15, 0x78, 0x03, 0x03,
		0x00, 0x00,
	...
	int body[37] =
		{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x03,
		0xC1, 0x80,
		0x00, 0x00, 0x04, 0x00, 0x16, 0x80, 0x15, 0x78, 0x03, 0x03,
		0x00, 0x00,
		0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x1B,
		0x1A, 0x00,
		-1
	};
...
{
	int opsc32[17] =
		{ 0x4A, 0x00, 0x00, 0x70, 0x00, 0x00, 0x60, 0x00, 0x17, 0x05,
		0xA5, 0x08,
	...
	int opsc41[37] =
		...
		0x40, 0x01,
		0x00, 0x00, 0x04, 0x00, 0x6E, 0x90, 0xD0, 0x47, 0x06, 0x00,
		0x00, 0xC4,
		0x5C, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x93,
		0x1B, 0x00,
		-1
	};
	int opscnb[37] =
		...
		0x00, 0x00, 0x04, 0x00, 0x6E, 0x90, 0xD0, 0x47, 0x06, 0x00,
		0x00, 0xEC,
		0x54, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x93,
		0x1A, 0x00,
		-1
	};
	int opsc04[9] =
		{ 0x06, 0xF4, 0xFF, 0x81, 0x1B, 0x00, 0x00, 0x00, -1 };
	int commit[9] =
		   ...
		   int gain, int offset)
{
	...
	rc = sanei_umax_pp_startScan
		(x, y - dataoffset, width, height + dataoffset, dpi, color,
		 gain, offset, &bpp, &tw, &th);
	if (rc == 1) {
		...
		DBG(8, "Getting buffer %d*%d*%d=%ld=0x%lX    (%s:%d)  \n",
		    bpp, tw, th, somme, somme, __FILE__, __LINE__);
		...
		if (buffer == NULL) {
			DBG(0,
			    "Failed to allocate %ld bytes, giving up....\n",
			    blocksize + reserve);
			DBG(0,
			    "Try to scan at lower resolution, or a smaller area.\n");
			gCancel = 1;
		}
		...
		while (read < dataoffset) {
			if (read == 0)
				...
				len = dataoffset - read;
			len = sanei_umax_pp_readBlock(len, tw, dpi, 0,
						      buffer + read);
			if (len == 0) {
				DBG(0,
		...
		while ((read < somme) && (!gCancel)) {
			...
			if (somme - read > blocksize - remain)
				...
				len = somme - read;
			len = sanei_umax_pp_readBlock(len, tw, dpi,
						      (len < blocksize),
						      buffer + reserve +
						      remain);
			if (len == 0) {
		...
		DBG(8, "%ld bytes transfered in %f seconds ( %.2f Kb/s)\n",
		    somme, elapsed, (somme / elapsed) / 1024.0);
#endif
			...
			int *rtw, int *rth)
{
	...
	int ccd[37] =
		...
		0x3C, 0x71,
		0x0F, 0x00, 0x04, 0x00, 0x6E, 0x61, 0xA1, 0x24, 0xC4, 0x7E,
		0x00, 0xAE,
		0x41, 0xA0, 0x0A, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x33,
		0x1A, 0x00,
		-1
	};
	...
	if (sanei_umax_pp_getastra() <= 610) {
		if (leftShadingCalibration610p
		    (color, dcRed, dcGreen, dcBlue, vgaRed, vgaGreen, vgaBlue,
		     calibration) == 0) {
			DBG(0,
			    "leftShadingCalibration610p failed !!! (%s:%d)\n",
			    __FILE__, __LINE__);
			return 0;
		}
		DBG(16,
...
sanei_umax_pp_checkModel(void)
{
	...
	int opsc35[37] =
		{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x03,
		0xC1, 0x80,
		0x00, 0x00, 0x04, 0x00, 0x16, 0x41, 0xE0, 0xAC, 0x03, 0x03,
		0x00, 0x00,
		0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x13,
		0x1A, 0x00,
		-1
	};
	...
	for (i = 0; i < 256; i++) {
		if ((dest[2 * i] != i)
			...
			DBG(0,
			    ...
			    i, i, dest[i * 2], dest[i * 2 + 1], __FILE__,
			    __LINE__);
			err = 0;
		}
	}
...
loadDefaultTables(void)
{
	...
	for (i = 0; i < 256; i++) {
		if ((buffer[2 * i] != i)
			...
			DBG(0,
			    "Error data altered: expected %d=(0x%02X,0x04), found (0x%02X,0x%02X) !    (%s:%d)\n",
			    i, i, buffer[i * 2], buffer[i * 2 + 1], __FILE__,
			    __LINE__);
			err = 1;
		}
	}
...
offsetCalibration1220p(int color, int *offRed, int *offGreen, int *offBlue)
{
	...
	int opsc04[9] =
		{ 0x06, 0xF4, 0xFF, 0x81, 0x1B, 0x00, 0x00, 0x00, -1 };
	int opsc10[9] =
		{ 0x06, 0xF4, 0xFF, 0x81, 0x1B, 0x00, 0x08, 0x00, -1 };
	int opsc38[37] =
		{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x04,
		0x40, 0x01,
		0x00, 0x00, 0x04, 0x00, 0x6E, 0x18, 0x10, 0x03, 0x06, 0x00,
		0x00, 0x00,
		0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x13,
		0x1A, 0x00,
		-1
...
offsetCalibration(int color, int *dcRed, int *dcGreen, int *dcBlue)
{
	if (sanei_umax_pp_getastra() <= 610) {
		...
		DBG(16,
		    "offsetCalibration610p(%d=>%d,%d,%d) passed ... (%s:%d)\n",
		    color, *dcRed, *dcGreen, *dcBlue, __FILE__, __LINE__);
	} else {
		if (offsetCalibration1220p(color, dcRed, dcGreen, dcBlue) ==
		    0) {
		...
		DBG(16,
		    "offsetCalibration1220p(%d=>%d,%d,%d) passed ... (%s:%d)\n",
		    color, *dcRed, *dcGreen, *dcBlue, __FILE__, __LINE__);
	}
	return 1;
}
	...
	if (color >= RGB_MODE) {
		...
		for (*vgaBlue = 0x0F; *vgaBlue > 0; (*vgaBlue)--) {
			...
			if ((max <= targetCode)
				...
				break;
			DBG(32,
			    "coarseGainCalibration610p, blue: min=%d, max=%d\n",
			    min, max);
		}
		...
		for (*vgaRed = 0x0F; *vgaRed > 0; (*vgaRed)--) {
			...
			if ((max <= targetCode)
				...
				break;
			DBG(32,
			    "coarseGainCalibration610p, red: min=%d, max=%d\n",
			    min, max);
		}
	} else {
		*vgaRed = 0x0F;
			   ...
			   int *vgaBlue)
{
	...
	int opsc39[37] =
		{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x04,
		0x40, 0x01,
		0x00, 0x00, 0x04, 0x00, 0x6E, 0x41, 0x20, 0x24, 0x06, 0x00,
		0x00, 0x00,
		0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x13,
		0x1A, 0x00,
		-1
	...
	int opsc40[37] =
		{ 0x00, 0x00, 0x04, 0x00, 0x02, 0x00, 0x00, 0x0C, 0x00, 0x04,
		0x40, 0x01,
		0x00, 0x00, 0x04, 0x00, 0x6E, 0x41, 0x60, 0x4F, 0x06, 0x00,
		0x00, 0x00,
		0x46, 0xA0, 0x00, 0x8B, 0x49, 0x2A, 0xE9, 0x68, 0xDF, 0x93,
		0x1A, 0x00,
		-1
	};
	int motor[17] =
	...
	if (sanei_umax_pp_getastra() <= 610) {
		if (coarseGainCalibration610p
		    (color, dcRed, dcGreen, dcBlue, vgaRed, vgaGreen,
		     vgaBlue) == 0) {
			DBG(0,
			    "coarseGainCalibration610p failed !!! (%s:%d)\n",
			    __FILE__, __LINE__);
			return 0;
		}
		DBG(16,
		...
		if (coarseGainCalibration1220p
		    (color, dcRed, dcGreen, dcBlue, vgaRed, vgaGreen,
		     vgaBlue) == 0) {
			DBG(0,
			    "coarseGainCalibration1220p failed !!! (%s:%d)\n",
			    __FILE__, __LINE__);
			return 0;
		}
		DBG(16,
		       ...
		       int *calibration)
{
	...
	if (data == NULL) {
		DBG(0,
		    "shadingCalibration610p: failed to allocate memory (%s:%d)\n",
		    __FILE__, __LINE__);
		return 0;
	}
	...
	if (color < RGB_MODE) {
		...
		for (x = 4; x < w; x++) {
			...
			coeff = (256.0 * (targetCode / avg - 1.0)) /
				((avg * 3.5) / targetCode);
			if (coeff < 0)
				coeff = 0;
			if (coeff > 255)
...
{
	if (sanei_umax_pp_getastra() < 1220)
		return shadingCalibration610p(color, dcRed, dcGreen, dcBlue,
