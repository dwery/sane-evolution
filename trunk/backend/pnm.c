...
static int status_none = TRUE;
static int status_eof = FALSE;
static int status_jammed = FALSE;
static int status_nodocs = FALSE;
static int status_coveropen = FALSE;
static int status_ioerror = FALSE;
static int status_nomem = FALSE;
static int status_accessdenied = FALSE;
static int test_option = 0;

static SANE_Fixed bright = 0;
static int res = 75;
static SANE_Fixed contr = 0;
static SANE_Bool gray = FALSE;
static SANE_Bool usegamma = FALSE;
static int gamma[4][256];
static enum
{
	ppm_bitmap,
	ppm_greyscale,
...
static SANE_Option_Descriptor sod[] = {
	...
	{			/* opt_default_enhancements */
	 ...
	 SANE_I18N("Defaults"),
	 SANE_I18N
	 ("Set default values for enhancement controls (brightness & "
	  "contrast)."),
	 SANE_TYPE_BUTTON,
	 SANE_UNIT_NONE,
	 0,
	...
	{			/* opt_status_eof */
	 ...
	 SANE_I18N
	 ("Force the backend to return the status code SANE_STATUS_EOF "
		    ...
		    SANE_Action action, void *value, int * info)
{
	...
	if (option < 0 || option >= NELEMS(sod)) {
		DBG(1,
		    "sane_control_option: option %d < 0 or >= number of options\n",
		    option);
	...
	case SANE_ACTION_SET_AUTO:
		...
		status = sanei_constrain_value(sod + option, (void *) &v,
					       &myinfo);
	...
	case SANE_ACTION_SET_VALUE:
		...
		case opt_grayify:
			...
			break;
		case opt_three_pass:
			three_pass = !!*(int *) value;
			myinfo |= SANE_INFO_RELOAD_PARAMS;
			break;
		case opt_hand_scanner:
			hand_scanner = !!*(int *) value;
			myinfo |= SANE_INFO_RELOAD_PARAMS;
			break;
		...
		case opt_custom_gamma:
			...
			break;
		case opt_gamma:
			memcpy(&gamma[0][0], (int *) value,
			       256 * sizeof(int));
			myinfo |= SANE_INFO_RELOAD_OPTIONS;
			break;
		case opt_gamma_r:
			memcpy(&gamma[1][0], (int *) value,
			       256 * sizeof(int));
			myinfo |= SANE_INFO_RELOAD_OPTIONS;
			break;
		case opt_gamma_g:
			memcpy(&gamma[2][0], (int *) value,
			       256 * sizeof(int));
			myinfo |= SANE_INFO_RELOAD_OPTIONS;
			break;
		case opt_gamma_b:
			memcpy(&gamma[3][0], (int *) value,
			       256 * sizeof(int));
			myinfo |= SANE_INFO_RELOAD_OPTIONS;
			break;
			/* status */
		case opt_status:
			status_none = *(int *) value;
			if (status_none) {
				status_eof = FALSE;
				status_jammed = FALSE;
				status_nodocs = FALSE;
				status_coveropen = FALSE;
				status_ioerror = FALSE;
				status_nomem = FALSE;
				status_accessdenied = FALSE;
			}
			myinfo |= SANE_INFO_RELOAD_OPTIONS;
			break;
		case opt_status_eof:
			status_eof = *(int *) value;
			if (status_eof) {
				status_none = FALSE;
				status_jammed = FALSE;
				status_nodocs = FALSE;
	...
	case SANE_ACTION_GET_VALUE:
		...
		case opt_grayify:
			*(int *) value = gray;
			break;
		case opt_three_pass:
			*(int *) value = three_pass;
			break;
		case opt_hand_scanner:
			*(int *) value = hand_scanner;
			break;
		case opt_read_only:
			*(int *) value = test_option;
			break;
		case opt_custom_gamma:
			*(int *) value = usegamma;
			break;
		case opt_gamma:
			memcpy((int *) value, &gamma[0][0],
			       256 * sizeof(int));
			break;
		case opt_gamma_r:
			memcpy((int *) value, &gamma[1][0],
			       256 * sizeof(int));
			break;
		case opt_gamma_g:
			memcpy((int *) value, &gamma[2][0],
			       256 * sizeof(int));
			break;
		case opt_gamma_b:
			memcpy((int *) value, &gamma[3][0],
			       256 * sizeof(int));
			break;
		case opt_status:
			*(int *) value = status_none;
			break;
		case opt_status_eof:
			*(int *) value = status_eof;
			break;
		case opt_status_jammed:
			*(int *) value = status_jammed;
			break;
		case opt_status_nodocs:
			*(int *) value = status_nodocs;
			break;
		case opt_status_coveropen:
			*(int *) value = status_coveropen;
			break;
		case opt_status_ioerror:
			*(int *) value = status_ioerror;
			break;
		case opt_status_nomem:
			*(int *) value = status_nomem;
			break;
		case opt_status_accessdenied:
			*(int *) value = status_accessdenied;
			break;
		default:
			return SANE_STATUS_INVAL;
		}
	  ...
	  int max_length, int * length)
{
	...
	*length = 0;
	...
	if (status_jammed == TRUE)
		return SANE_STATUS_JAMMED;
	if (status_eof == TRUE)
		return SANE_STATUS_EOF;
	if (status_nodocs == TRUE)
		return SANE_STATUS_NO_DOCS;
	if (status_coveropen == TRUE)
		return SANE_STATUS_COVER_OPEN;
	if (status_ioerror == TRUE)
		return SANE_STATUS_IO_ERROR;
	if (status_nomem == TRUE)
		return SANE_STATUS_NO_MEM;
	if (status_accessdenied == TRUE)
	...
	if (ppm_type == ppm_color && (gray || three_pass)) {
		...
		if (gray) {
			...
			for (p = (rgbbuf + (pass + 1) % 3); p < rgbend;
			     p += 3)
				*q++ = *p;
		}
