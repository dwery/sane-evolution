...
select_source(pixma_t * s)
{
	...
	if (s->cfg->pid == MP830_PID) {
		...
		case PIXMA_SOURCE_TPU:
			PDBG(pixma_dbg
			     (1,
			      "BUG:select_source(): unsupported source %d\n",
			      s->param->source));
			/* fall through */
...
send_scan_param(pixma_t * s)
{
	...
	if (mp->generation <= 2) {
		...
		data[0x18] = ((s->param->channels != 1)
			      || is_ccd_grayscale(s)) ? 0x08 : 0x04;
		...
		data[0x00] =
			(s->param->source == PIXMA_SOURCE_ADF) ? 0x02 : 0x01;
		data[0x01] = 0x01;
		data[0x02] = 0x01;
...
init_ccd_3(pixma_t * s)
{
	...
	if (error >= 0) {
		data = pixma_newcmd(&mp->cb, cmd_end_calibrate_ccd_3, 0,
				    status_len);
		error = pixma_exec(s, &mp->cb);
		if (error >= 0) {
			memcpy(mp->current_status, data, status_len);
...
query_status(pixma_t * s)
{
	...
	if (error >= 0) {
		memcpy(mp->current_status, data, status_len);
		PDBG(pixma_dbg
		     (3, "Current status: paper=%u cal=%u lamp=%u busy=%u\n",
...
read_image_block(pixma_t * s, uint8_t * header, uint8_t * data)
{
	...
	if ((mp->last_block & 0x20) == 0)
		pixma_set_be32((IMAGE_BLOCK_SIZE / 65536) * 65536 + 8,
			       cmd + 0xc);
	else
	...
	if (datalen >= hlen) {
		...
		data += datalen;
		if (mp->cb.reslen == 512) {
			error = pixma_read(s->io, data,
					   IMAGE_BLOCK_SIZE - 512 + hlen);
			if (error < 0)
				return error;
			datalen += error;
		}
...
wait_until_ready(pixma_t * s)
{
	...
	while (!is_calibrated(s)) {
		...
		if (--tmo == 0) {
			PDBG(pixma_dbg
			     (1,
			      "WARNING:Timed out in wait_until_ready()\n"));
			PDBG(query_status(s));
...
mp150_scan(pixma_t * s)
{
	...
	if (error == PIXMA_EBUSY || error == PIXMA_ETIMEDOUT) {
		...
		PDBG(pixma_dbg
		     (2, "Scanner hangs? Sending abort_session command.\n"));
		drain_bulk_in(s);
		abort_session(s);
		pixma_sleep(500000);
		error = start_session(s);
...
static uint8_t *
shift_colors(uint8_t * dptr, uint8_t * sptr, unsigned w, int sr, int sg,
	     int sb)
{
	unsigned i;
...
mp150_fill_buffer(pixma_t * s, pixma_imagebuf_t * ib)
{
	...
	do {
		...
		error = read_image_block(s, header,
					 mp->imgbuf + mp->data_left_len);
		if (error < 0) {
		...
		if ((header[8] & ~0x38) != 0) {
			PDBG(pixma_dbg
			     (1, "WARNING: Unexpected result header\n"));
		...
		mp->data_left_ofs =
			mp->imgbuf + mp->data_left_len + bytes_received;
...
mp150_finish_scan(pixma_t * s)
{
	...
	case state_warmup:
		...
		if (error < 0)
			PDBG(pixma_dbg
			     (1, "WARNING:abort_session() failed %d\n",
			      error));
		/* fall through */
	case state_finished:
		mp->state = state_idle;
...
mp150_get_status(pixma_t * s, pixma_device_status_t * status)
{
	...
	status->cal =
		(is_calibrated(s)) ? PIXMA_CALIBRATION_OK :
