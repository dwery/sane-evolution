		      ...
		      u_int8_t * data, size_t len)
{
	...
	while (len) {
		...
		status = sanei_usb_control_msg(dev->dn, REQUEST_TYPE_OUT,
					       REQUEST_BUFFER, VALUE_BUFFER,
					       INDEX, sizeof(outdata),
					       outdata);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		     ...
		     u_int8_t * data, size_t len)
{
	...
	status = sanei_usb_control_msg(dev->dn, REQUEST_TYPE_OUT,
				       REQUEST_BUFFER, VALUE_BUFFER, INDEX,
...
gl841_set_buffer_address_gamma(Genesys_Device * dev, u_int32_t addr)
{
	...
	DBG(DBG_io,
	    "gl841_set_buffer_address_gamma: setting address to 0x%05x\n",
	    addr & 0xfffffff0);
...
#if 0
	fastmode =
		(sanei_genesys_read_reg_from_set(reg, 0x01) & REG01_FASTMOD) ?
		1 : 0;
#endif
	...
	for (i = 0; i < GENESYS_GL841_MAX_REGS; i++) {
		...
		case 0x05:
			DBG(DBG_info, "%s, %s, %s, %s, %s, %s",
			    ((v & REG05_DPIHW) >> 6) ==
			    0 ? "opt. sensor dpi: 600" : ((v & REG05_DPIHW) >>
							  6) ==
			    1 ? "opt. sensor dpi: 1200 dpi"
			    : ((v & REG05_DPIHW) >> 6) ==
			    2 ? "opt. sensor dpi: 2400 dpi" :
			    "opt. sensor dpi: reserved", "lamp time out",
			    ((v & REG05_MTLLAMP) >> 4) <
			    2 ? (((v & REG05_MTLLAMP) >> 4) ==
				 0 ? "1*LAMPTIM" : "2*LAMPTIM")
			    : ((v & REG05_MTLLAMP) >> 4) ==
			    2 ? "4*LAMPTIM" : "reserved",
			    (v & REG05_GMMENB) ? "gamma correction on" :
			    "gamma correction off",
			    "pixes number under each system pixel time: ",
			    (v & REG05_MTLBASE) < 2 ? ((v & REG05_MTLBASE) ==
						       0 ? "1" : "2") : (v &
									 REG05_MTLBASE)
			    == 2 ? "3" : "4");
		...
		case 0x06:
			DBG(DBG_info,
			    "12 clk/pixel normal for scanning, %s, %s, %s, %s",
			    ((v & REG06_SCANMOD >> 5) >
			     5) ? ((v & REG06_SCANMOD >> 5) ==
				   4) ? " 6 clk/pixel fast mode  " : ((v &
								       REG06_SCANMOD
								       >> 5)
								      ==
								      5) ?
			    ...
			    : ((v & REG06_SCANMOD >> 5) <
			       2) ? ((v & REG06_SCANMOD >> 5) ==
				     0) ? "12 clk/pixel normal for scanning" :
			    "12 clk/pixel bypass for scanning" : " reserved",
			    (v & REG06_PWRBIT) ? "turn power on" :
			    "don't turn power on",
			    (v & REG06_GAIN4) ?
			    "digital shading gain=4 times system" :
			    "digital shading gain=8 times system",
			    (v & REG06_OPTEST) ==
			    0 ? "ASIC test mode: off" : (v & REG06_OPTEST) ==
			    1 ? "ASIC test mode: simulation, motorgo" : (v &
									 REG06_OPTEST)
			    ==
			    2 ? "ASIC test mode: image, pixel count" : (v &
									REG06_OPTEST)
			    ==
			    3 ? "ASIC test mode: image, line count" : (v &
								       REG06_OPTEST)
			    ==
			    4 ? "ASIC test mode: simulation, counter + adder"
			    : (v & REG06_OPTEST) ==
			    5 ? "ASIC test mode: CCD TG" :
			    "ASIC test mode: reserved");
			break;
		case 0x07:
			DBG(DBG_info, "%s, %s, %s, %s",
			    ...
			    "MPU access for DRAM",
			    (v & REG07_DMARDWR) ? "DMA read DRAM" :
			    "DMA write DRAM");
			break;
		case 0x08:
			DBG(DBG_info, "%s, %s, %s, %s, %s, %s, %s",
			    (v & REG08_DECFLAG) ?
			    "gamma table is decrement type" :
		...
		case 0x0d:
			DBG(DBG_info, "scanner command: %s",
			    (v & REG0D_CLRLNCNT) ?
			    "clear SCANCNT(Reg4b,Reg4c,Reg4d)" :
			    "don't clear SCANCNT");
			break;
		case 0x0e:
		...
		case 0x11:
			DBG(DBG_info, "red exposure time lo (total: 0x%x)",
			    v + 256 * sanei_genesys_read_reg_from_set(reg,
								      0x10));
			break;
		case 0x12:
		...
		case 0x16:
			DBG(DBG_info, "%s, %s, %s, %s, %s, %s, %s, %s",
			    ...
			    (v & REG16_TOSHIBA) ?
			    "image sensor is TOSHIBA CIS" : " ",
			    (v & REG16_TGINV) ? "inverse CCD TG" :
			    "normal CCD TG",
			    ...
			    "normal CCD CP & RS",
			    (v & REG16_CKDIS) ?
			    "CCD TG position clock 1/2 signal off" :
		...
		case 0x17:
			DBG(DBG_info, "%s, CCD TG width: %0x",
			    ((v & REG17_TGMODE) >> 6) ==
			    0x00 ? "CCD TG without dummy line"
			    : ((v & REG17_TGMODE) >> 6) ==
			    0x01 ? "CCD TG with dummy line" :
			    "CCD TG reserved", (v & REG17_TGW));
			break;
		case 0x18:
			DBG(DBG_info,
			    "%s, %d time CCD clock speed for dummy line, %s, "
		...
		case 0x19:
			DBG(DBG_info,
			    "dummy line exposure time (0x%x pixel times)",
			    v * 256);
			break;
		case 0x1a:
...
sanei_gl841_asic_test(Genesys_Device * dev)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_gl841_asic_test: failed to write register: %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_gl841_asic_test: failed to read register: %s\n",
		    sane_strstatus(status));
		return status;
	}
	...
	if (!data) {
		DBG(DBG_error,
		    "sanei_gl841_asic_test: could not allocate memory\n");
...
/*  status = gl841_bulk_write_data (dev, 0x3c, data, size);
  if (status != SANE_STATUS_GOOD)
	...
	status = gl841_bulk_read_data(dev, 0x45, (u_int8_t *) verify_data,
				      verify_size);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "sanei_gl841_asic_test: failed to bulk read data: %s\n",
		    sane_strstatus(status));
		free(data);
		free(verify_data);
		return status;
	...
	for (i = 0; i < size; i++) {
		if (verify_data[i] != data[i]) {
			DBG(DBG_error,
			    "sanei_gl841_asic_test: data verification error\n");
			DBG(DBG_info,
			    "0x%.8x: got %.2x %.2x %.2x %.2x, expected %.2x %.2x %.2x %.2x\n",
			    i, verify_data[i], verify_data[i + 1],
			    verify_data[i + 2], verify_data[i + 3], data[i],
...
gl841_init_registers(Genesys_Device * dev)
{
	...
	dev->reg[reg_0x1e].value |= 1 << REG1ES_WDTIME;
	 ...
	 /*SCANFED*/ dev->reg[reg_0x1f].value = 0x01;

	 /*BUFSEL*/ dev->reg[reg_0x20].value = 0x20;

	 /*LAMPPWM*/ dev->reg[reg_0x29].value = 0xff;

	 /*BWHI*/ dev->reg[reg_0x2e].value = 0x80;

	 /*BWLOW*/ dev->reg[reg_0x2f].value = 0x80;

	 /*LPERIOD*/ dev->reg[reg_0x38].value = 0x4f;
	dev->reg[reg_0x39].value = 0xc1;

	 /*VSMPW*/ dev->reg[reg_0x58].value |= 3 << REG58S_VSMPW;

	 /*BSMPW*/ dev->reg[reg_0x59].value |= 3 << REG59S_BSMPW;

	 /*RLCSEL*/ dev->reg[reg_0x5a].value |= REG5A_RLCSEL;

	 /*STOPTIM*/ dev->reg[reg_0x5e].value |= 0x2 << REG5ES_STOPTIM;
...
/*#else
  table = (u_int8_t*)slope_table;
	...
	status = sanei_genesys_set_buffer_address(dev,
						  start_address +
						  table_nr * 0x200);
	if (status != SANE_STATUS_GOOD) {
/*#ifdef WORDS_BIGENDIAN*/
...
/*#endif*/
	...
	status = gl841_bulk_write_data(dev, 0x3c, (u_int8_t *) table,
				       steps * 2);
	if (status != SANE_STATUS_GOOD) {
/*#ifdef WORDS_BIGENDIAN*/
		free(table);
...
gl841_set_fe(Genesys_Device * dev, u_int8_t set)
{
	...
	if ((dev->reg[reg_0x04].value & REG04_FESET) != 0x00) {
		DBG(DBG_proc,
		    "gl841_set_fe(): unsupported frontend type %d\n",
	...
	if (set == AFE_INIT) {
		...
		if (status != SANE_STATUS_GOOD) {
			...
			if (dev->model->ccd_type == CCD_HP2300
				...
				val = 0x07;
				status = sanei_usb_control_msg(dev->dn,
							       REQUEST_TYPE_OUT,
							       REQUEST_REGISTER,
							       GPIO_OUTPUT_ENABLE,
							       INDEX, 1,
	...
	if (set == AFE_POWER_SAVE) {
		...
		if (status != SANE_STATUS_GOOD)
			DBG(DBG_error,
			    "gl841_set_fe: writing data failed: %s\n",
			    sane_strstatus(status));
		return status;
	}
	...
	/*if (dev->model->ccd_type!=CCD_HP2300 && dev->model->ccd_type!=CCD_HP2400) */
	{
		...
		status = sanei_genesys_fe_write_data(dev, 0x00,
						     dev->frontend.reg[0]);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl841_set_fe: writing reg0 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
		status = sanei_genesys_fe_write_data(dev, 0x02,
						     dev->frontend.reg[2]);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
			    "gl841_set_fe: writing reg2 failed: %s\n",
			    sane_strstatus(status));
			return status;
		}
	}
	...
	status = sanei_genesys_fe_write_data(dev, 0x06,
					     dev->frontend.reg2[0]);
	if (status != SANE_STATUS_GOOD) {
	...
	status = sanei_genesys_fe_write_data(dev, 0x08,
					     dev->frontend.reg2[1]);
	if (status != SANE_STATUS_GOOD) {
	...
	status = sanei_genesys_fe_write_data(dev, 0x09,
					     dev->frontend.reg2[2]);
	if (status != SANE_STATUS_GOOD) {
	...
	for (i = 0; i < 3; i++) {
		...
		status = sanei_genesys_fe_write_data(dev, 0x28 + i,
						     dev->frontend.gain[i]);
		if (status != SANE_STATUS_GOOD) {
			DBG(DBG_error,
		...
		status = sanei_genesys_fe_write_data(dev, 0x20 + i,
...
static SANE_Status
gl841_init_motor_regs_off(Genesys_Device * dev, Genesys_Register_Set * reg, unsigned int scan_lines	/*lines, scan resolution */
	)
{
	unsigned int feedl;
	Genesys_Register_Set *r;
...
static SANE_Status
gl841_init_motor_regs(Genesys_Device * dev, Genesys_Register_Set * reg, unsigned int feed_steps,	/*1/base_ydpi */
...
/*number of scan lines to add in a scan_lines line*/
	...
	DBG(DBG_proc,
	    "gl841_init_motor_regs : feed_steps=%d, action=%d, flags=%x\n",
...
/* FEED and GO_HOME can use fastest slopes available */
		...
		DBG(DBG_info,
		    "gl841_init_motor_regs : fast_exposure=%d pixels\n",
		    fast_exposure);
	}
...
static SANE_Status
gl841_init_motor_regs_scan(Genesys_Device * dev, Genesys_Register_Set * reg, unsigned int scan_exposure_time,	/*pixel */
			   ...
			   int scan_power_mode, unsigned int flags)
{
	...
	DBG(DBG_info,
	    "gl841_init_motor_regs_scan : fast_exposure=%d pixels\n",
	    fast_exposure);
...
/* we need to shorten fast_slope_steps here. */
	...
	DBG(DBG_info,
	    "gl841_init_motor_regs_scan: Maximum allowed slope steps for fast slope: %d\n",
	    fast_slope_steps);
	...
	if (feed_steps <
		...
		use_fast_fed = 0;
		DBG(DBG_info,
		    "gl841_init_motor_regs_scan: feed too short, slow move forced.\n");
	} else {
...
/*we use full steps as base unit here*/
		...
		slow_time =
			...
			dev->motor.base_ydpi * (feed_steps -
						(slow_slope_steps >>
						 scan_step_type))
			+ slow_slope_time;

		DBG(DBG_info,
		    "gl841_init_motor_regs_scan: Time for slow move: %d\n",
		    slow_time);
		DBG(DBG_info,
		    "gl841_init_motor_regs_scan: Time for fast move: %d\n",
		    fast_time);
 ...
 */
	...
	if (use_fast_fed) {
		status = gl841_send_slope_table(dev, 3, fast_slope_table,
						256);
	...
	if (flags & MOTOR_FLAG_AUTO_GO_HOME) {
		status = gl841_send_slope_table(dev, 4, fast_slope_table,
						256);
...
static SANE_Status
gl841_init_optical_regs_off(Genesys_Device * dev, Genesys_Register_Set * reg)
{
	Genesys_Register_Set *r;
...
gl841_get_led_exposure(Genesys_Device * dev)
{
	...
	r = dev->sensor.regs_0x10_0x1d[1] | (dev->sensor.
					     regs_0x10_0x1d[0] << 8);
	g = dev->sensor.regs_0x10_0x1d[3] | (dev->sensor.
					     regs_0x10_0x1d[2] << 8);
	b = dev->sensor.regs_0x10_0x1d[5] | (dev->sensor.
					     regs_0x10_0x1d[4] << 8);
 ...
 */
static SANE_Status
gl841_init_scan_regs(Genesys_Device * dev, Genesys_Register_Set * reg, float xres,	/*dpi */
		     float yres,	/*dpi */
		     ...
		     int color_filter, unsigned int flags)
{
	...
	DBG(DBG_info,
	    ...
	    startx, starty, depth, channels, flags);
...
/* start */
	/* add x coordinates */
	start = ((dev->sensor.CCD_start_xoffset + startx) * used_res) /
...
/* exposure_time */
	...
	exposure_time = sanei_genesys_exposure_time2(dev, slope_dpi, scan_step_type, start + used_pixels,	/*+tgtime? currently done in sanei_genesys_exposure_time2 with tgtime = 32 pixel */
						     led_exposure,
						     scan_power_mode);

	while (scan_power_mode + 1 < dev->motor.power_mode_count) {
		exposure_time2 = sanei_genesys_exposure_time2(dev, slope_dpi, scan_step_type, start + used_pixels,	/*+tgtime? currently done in sanei_genesys_exposure_time2 with tgtime = 32 pixel */
							      led_exposure,
							      scan_power_mode
							      + 1);
...
/*  move = ((move + dummy) / (dummy + 1)) * (dummy + 1);
	...
	if (flags & SCAN_FLAG_SINGLE_LINE)
		...
		status = gl841_init_motor_regs_scan(dev,
						    ...
						    dev->model->
						    is_cis ? lincnt *
						    channels : lincnt, dummy,
						    move, scan_power_mode,
						    (flags &
...
/* words_per_line */
	...
	RIE(sanei_genesys_buffer_alloc
	    (&(dev->read_buffer), read_buffer_size));

	RIE(sanei_genesys_buffer_free(&(dev->lines_buffer)));
	RIE(sanei_genesys_buffer_alloc
	    (&(dev->lines_buffer), read_buffer_size));
	...
	if (depth == 1 || dev->settings.scan_mode == 0)
		dev->total_bytes_to_read =
			((dev->settings.pixels * dev->settings.lines) / 8 +
			 (((dev->settings.pixels * dev->settings.lines) %
			   8) ? 1 : 0)
			) * channels;
	else
		dev->total_bytes_to_read =
...
/* final state: GPIO8 disabled, GPIO9 enabled, GPIO17 disabled, 
   GPIO18 disabled*/
			...
			sanei_genesys_write_register(dev, 0x6B,
						     val & ~REG6B_GPO17);
...
static SANE_Status
gl841_set_powersaving(Genesys_Device * dev, int delay /* in minutes */ )
{
	SANE_Status status;
	...
	exposure_time =
		(u_int32_t) (time * 32000.0 /
			     (24.0 * 64.0 *
			      (local_reg[1].value & REG03_LAMPTIM) * 1024.0) +
			     0.5);
		 ...
		 SANE_Bool start_motor)
{
	...
	local_reg[0].value =
		sanei_genesys_read_reg_from_set(reg, 0x03) | REG03_LAMPPWR;
	       ...
	       SANE_Bool check_stop)
{
	...
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
		    "gl841_end_scan: Failed to write registers: %s\n",
		    sane_strstatus(status));
		return status;
	}
...
/* when scanhead is moving then wait until scanhead stops or timeout */
	...
	for (i = 400; i > 0; i--) {	/* do not wait longer than 40 seconds, count down to get i = 0 when busy */
		...
		if (!(val & REG41_MOTORENB)) {	/* motor is off */
			DBG(DBG_info, "gl841_feed: motor is off\n");
		...
		SANE_Bool wait_until_home)
{
	...
	if (wait_until_home) {
		...
		while (loop < 300) {
			...
			if (!(val & REG41_MOTORENB)) {
				DBG(DBG_info,
				    "gl841_park_head: restarting motor\n");
				status = sanei_genesys_start_motor(dev);
				if (status != SANE_STATUS_GOOD) {
					DBG(DBG_error,
		...
		DBG(DBG_info, "gl841_park_head: exiting while moving home\n");
		return SANE_STATUS_GOOD;
	}
...
gl841_search_start_position(Genesys_Device * dev)
{
	...
	status = gl841_init_scan_regs(dev, local_reg, dpi, dpi, 0, 0,	/*we should give a small offset here~60 steps */
	...
	status = gl841_begin_scan(dev, local_reg, TRUE);
	if (status != SANE_STATUS_GOOD) {
		free(data);
		DBG(DBG_error,
	...
	if (DBG_LEVEL >= DBG_data)
		sanei_genesys_write_pnm_file("search_position.pnm", data, 8,
					     1, pixels,
					     dev->model->search_lines);

	status = gl841_end_scan(dev, local_reg, TRUE);
	if (status != SANE_STATUS_GOOD) {
		free(data);
		DBG(DBG_error,
...
/*TODO: find out where sanei_genesys_search_reference_point 
  stores information, and use that correctly*/
	status = sanei_genesys_search_reference_point(dev, data, 0, dpi,
						      pixels,
						      dev->model->
						      search_lines);
	if (status != SANE_STATUS_GOOD) {
...
gl841_init_regs_for_coarse_calibration(Genesys_Device * dev)
{
	...
	status = gl841_bulk_write_register(dev, dev->calib_reg,
					   GENESYS_GL841_MAX_REGS);
	if (status != SANE_STATUS_GOOD) {
		DBG(DBG_error,
...
gl841_init_regs_for_shading(Genesys_Device * dev)
{
	...
	status = gl841_init_scan_regs(dev,
				      ...
				      (dev->sensor.sensor_pixels *
				       dev->settings.xres) /
				      dev->sensor.optical_res,
				      dev->model->shading_lines, 16, channels,
				      dev->settings.color_filter,
				      SCAN_FLAG_DISABLE_SHADING |
				      SCAN_FLAG_DISABLE_GAMMA |
...
gl841_send_gamma_table(Genesys_Device * dev, SANE_Bool generic)
{
	...
	/* send data */
	status = gl841_bulk_write_data_gamma(dev, 0x28, (u_int8_t *) gamma,
					     size * 2 * 3);
	if (status != SANE_STATUS_GOOD) {
		free(gamma);
		DBG(DBG_error,
...
gl841_led_calibration(Genesys_Device * dev)
{
	...
	status = gl841_init_scan_regs(dev,
				      ...
				      (dev->sensor.sensor_pixels *
				       dev->settings.xres) /
				      dev->sensor.optical_res, 1, 16,
				      channels, dev->settings.color_filter,
				      SCAN_FLAG_DISABLE_SHADING |
				      SCAN_FLAG_DISABLE_GAMMA |
				      SCAN_FLAG_SINGLE_LINE |
				      SCAN_FLAG_IGNORE_LINE_DISTANCE |
 ...
 */
	...
	expr = (dev->sensor.regs_0x10_0x1d[0] << 8) | dev->sensor.
		regs_0x10_0x1d[1];
	expg = (dev->sensor.regs_0x10_0x1d[2] << 8) | dev->sensor.
		regs_0x10_0x1d[3];
	expb = (dev->sensor.regs_0x10_0x1d[4] << 8) | dev->sensor.
		regs_0x10_0x1d[5];
	...
	do {
		...
		RIE(gl841_begin_scan(dev, dev->calib_reg, TRUE));
...
*/
		...
		RIE(gl841_end_scan(dev, dev->calib_reg, TRUE));
	...
	DBG(DBG_info,
	    "gl841_led_calibration: acceptable exposure: %d,%d,%d\n", expr,
	    expg, expb);

	/* cleanup before return */
	free(line);

	gl841_slow_back_home(dev, TRUE);
 ...
 */
		...
		for (j = 0; j < channels; j++) {
			...
			for (i = 0; i < num_pixels; i++) {
				if (dev->model->is_cis)
					...
					val = first_line[i * 2 * channels +
							 2 * j + 1] * 256 +
						first_line[i * 2 * channels +
							   2 * j];
				if (val < 10)
					cmin[j]++;
				if (val > 65525)
	...
	DBG(DBG_info,
	    "gl841_offset_calibration: acceptable offsets: %d,%d,%d\n",
	    off[0], off[1], off[2]);
	...
	for (j = 0; j < channels; j++) {
		...
		for (i = 0; i < num_pixels; i++) {
			if (dev->model->is_cis)
				val = first_line[i * 2 + j * 2 * num_pixels +
						 1] * 256 + first_line[i * 2 +
								       j * 2 *
								       num_pixels];
			else
				val = first_line[i * 2 * channels + 2 * j +
						 1] * 256 +
		...
		for (j = 0; j < channels; j++) {
			...
			for (i = 0; i < num_pixels; i++) {
				if (dev->model->is_cis)
					...
					val = second_line[i * 2 * channels +
							  2 * j + 1] * 256 +
						second_line[i * 2 * channels +
							    2 * j];
				if (val < 10)
					cmin[j]++;
				if (val > 65525)
	...
	for (j = 0; j < channels; j++) {
		...
		for (i = 0; i < num_pixels; i++) {
			if (dev->model->is_cis)
				val = second_line[i * 2 + j * 2 * num_pixels +
						  1] * 256 +
					second_line[i * 2 +
						    j * 2 * num_pixels];
			else
				val = second_line[i * 2 * channels + 2 * j +
						  1] * 256 +
	...
	DBG(DBG_info,
	    "gl841_offset_calibration: first set: %d/%d,%d/%d,%d/%d\n",
	    off1[0], min1[0], off1[1], min1[1], off1[2], min1[2]);

	DBG(DBG_info,
	    "gl841_offset_calibration: second set: %d/%d,%d/%d,%d/%d\n",
...
/*TODO: try to avoid this*/
			DBG(DBG_warn,
			    "gl841_offset_calibration: difference too small\n");
			if (mintgt * (off1[j] - off2[j]) + min1[j] * off2[j] -
			    min2[j] * off1[j] >= 0)
				off[j] = 0x0000;
			else
				off[j] = 0xffff;
		} else
			off[j] = -(mintgt * (off1[j] - off2[j]) +
				   min2[j] * off1[j] -
				   min1[j] * off2[j]) / (min2[j] - min1[j]);
		if (off[j] > 255)
			off[j] = 255;
		if (off[j] < 0)
			off[j] = 0;
...
gl841_coarse_gain_calibration(Genesys_Device * dev, int dpi)
{
	...
	black_pixels =
		(dev->sensor.CCD_start_xoffset * dpi) /
		dev->sensor.optical_res;
	...
	for (j = 0; j < channels; j++) {
		...
		for (i = 0; i < num_pixels; i++) {
			if (dev->model->is_cis)
				val = line[i * 2 + j * 2 * num_pixels +
					   1] * 256 + line[i * 2 +
							   j * 2 *
							   num_pixels];
			else
				val = line[i * 2 * channels + 2 * j +
					   1] * 256 + line[i * 2 * channels +
							   2 * j];
	...
	for (j = 0; j < channels; j++) {
		if (gain[j] > 10) {
			...
			DBG(DBG_error0,
			    "****                                      ****\n");
			DBG(DBG_error0,
			    "****  Extremely low Brightness detected.  ****\n");
			DBG(DBG_error0,
			    "****  Check the scanning head is          ****\n");
			DBG(DBG_error0,
			    "****  unlocked and moving.                ****\n");
			DBG(DBG_error0,
			    "****                                      ****\n");
...
gl841_init(Genesys_Device * dev)
{
	...
	/* Move home */
	RIE(gl841_slow_back_home(dev, TRUE));
	...
	if (dev->sensor.green_gamma_table == NULL) {
		dev->sensor.green_gamma_table =
			(u_int16_t *) malloc(2 * size);
		if (dev->sensor.red_gamma_table == NULL) {
			DBG(DBG_error,
			    "gl841_init: could not allocate memory for gamma table\n");
			return SANE_STATUS_NO_MEM;
	...
	DBG(DBG_info, "gl841_init: starting dummy data reading\n");
	RIE(gl841_begin_scan(dev, dev->calib_reg, TRUE));
