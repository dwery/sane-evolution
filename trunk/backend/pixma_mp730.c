...
handle_interrupt(pixma_t * s, int timeout)
{
	...
	case MP390_PID:
		if (len != 16) {
			PDBG(pixma_dbg
			     (1,
			      "WARNING:unexpected interrupt packet length %d\n",
			      len));
			return PIXMA_EPROTO;
		}
		if (buf[12] & 0x40)
	...
	default:
		PDBG(pixma_dbg
		     (1, "WARNING:unknown interrupt, please report!\n"));
...
mp730_scan(pixma_t * s)
{
	...
	buf = (uint8_t *) malloc((n + 1) * s->param->line_size +
				 IMAGE_BLOCK_SIZE);
	if (!buf)
		return PIXMA_ENOMEM;
	mp->buf = buf;
	...
	do {
		...
		if (n != 0) {
			if (s->param->channels != 1) {
				/* color */
				pack_rgb(mp->imgbuf, n, mp->raw_width,
					 mp->lbuf);
			} else {
				/* grayscale */
				memcpy(mp->lbuf, mp->imgbuf,
				       n * s->param->line_size);
			}
			block_size = n * s->param->line_size;
			mp->imgbuf_len -= block_size;
			memcpy(mp->imgbuf, mp->imgbuf + block_size,
			       mp->imgbuf_len);
		}
	}
	while (n == 0);
