...
issue(struct req *req)
{
				      ...
				      void **idp)
	{
		...
		if (sg_version < 30000) {
			...
			memcpy(&req->sgdata.cdb.data[cmd_size], src,
			       src_size);
			...
			if (dst_size && *dst_size) {
				...
				memcpy(&req->sgdata.sg3.data[MAX_CDB], src,
				       src_size);
...
#define SCSI_IOCTL_GET_IDLUN 0x5382
	  ...
	  lx_chk_id(int dev_fd, int host, int channel, int id, int lun)
	{
...
#endif
		{
			...
			my_idlun;
			if (ioctl(dev_fd, SCSI_IOCTL_GET_IDLUN, &my_idlun) >=
			    0) {
			   ...
			   int host, int channel, int id, int lun)
	{
		...
		for (k = 0, missed = 0; (missed < SCAN_MISSES) && (k < 255);
			...
			if ((dev_fd =
			     lx_mk_devicename(k, name, name_len)) >= 0) {
				missed = 0;
	...
	static int		/* Returns 1 if match, else 0 */
		...
		lx_chk_devicename(int guess_devnum, char *name,
				  size_t name_len, int host, int channel,
				  int id, int lun)
	{
		int dev_fd;
		...
		if (0 != lx_devfs) {	/* simple mapping if we have devfs */
			...
			snprintf(name, name_len, DEVFS_MSK, host, channel, id,
				 lun);
			dev_fd = open(name, O_RDWR | O_NONBLOCK);
			if (dev_fd >= 0) {
				close(dev_fd);
		...
		if ((dev_fd = lx_mk_devicename(guess_devnum, name, name_len)) < -1) {	/* no candidate sg device file name found, try /dev/sg0,1 */
			if ((dev_fd =
			     lx_mk_devicename(0, name, name_len)) < -1) {
				if ((dev_fd =
				     lx_mk_devicename(1, name,
						      name_len)) < -1)
...
/* if mismatch then call scan algorithm */
		if (lx_scan_sg
		    (guess_devnum, name, name_len, host, channel, id, lun)) {
			DBG(1,
...
#define FOUND_ALL     255
		...
		size_t findvendor_len = 0, findmodel_len = 0, findtype_len =
			0;
		...
		while (!feof(proc_fp)) {
			...
			definedd = 0;
			if ((!findvendor
			     || strncmp(vendor, findvendor,
					findvendor_len) == 0)
			    && (!findmodel
				|| strncmp(model, findmodel,
					   findmodel_len) == 0)
			    && (!findtype
