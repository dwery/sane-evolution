	  ...
	  int * len)
{
	...
	int bytecounter, linebufcounter, ctlbytes;
	...
	if ((unsigned int) s->image_counter >= s->bytes_to_read) {
		*len = 0;
		if (s->scanning) {
			read(s->as6e_params.ctlinpipe, &written,
			     sizeof(written));
			if (written != -1)
				DBG(3, "pipe error\n");
	...
	if (s->scan_buffer_count > 0) {	/*there are leftover bytes from the last call */
		if (s->scan_buffer_count <= max_len) {
			...
			if (s->scan_buffer_count == max_len) {
				...
				DBG(3,
				    "returning %d bytes from the carryover buffer\n",
				    *len);
			...
			for (bytecounter = max_len;
			     bytecounter < s->scan_buffer_count;
			     bytecounter++)
	...
	while (*len < max_len) {
		...
		DBG(3, "bytes written = %d, ctlbytes =%d\n", written,
		    ctlbytes);
		fflush(stdout);
		if ((s->cancelled) && (written == 0)) {	/*first clear -1 from pipe */
		...
		while (linebufcounter < written) {
			...
			bytes_read =
				read(s->as6e_params.datapipe,
				     linebuffer + linebufcounter, maxbytes);
			linebufcounter += bytes_read;
			maxbytes -= bytes_read;
			DBG(3, "bytes_read = %d linebufcounter = %d\n",
			    bytes_read, linebufcounter);
		}
		DBG(3, "written =%d max_len =%d  len =%d\n", written, max_len,
		    *len);
		if (written <= (max_len - *len)) {
			for (bytecounter = 0; bytecounter < written;
			     bytecounter++) {
				buf[bytecounter + buffer_offset] =
					linebuffer[bytecounter];
				(*len)++;
			}
			buffer_offset += written;
			DBG(3, "buffer offset = %d\n", buffer_offset);
		} else if (max_len > *len) {	/*there's still room to send data */
			for (bytecounter = 0; bytecounter < (max_len - *len);
			     bytecounter++)
				buf[bytecounter + buffer_offset] =
					linebuffer[bytecounter];
			DBG(3, "topping off buffer\n");
			for (bytecounter = (max_len - *len);
			     bytecounter < written; bytecounter++) {
			...
			*len = max_len;
		} else {	/*everything goes into the carryover buffer */
			for (bytecounter = 0; bytecounter < written;
			     bytecounter++)
				s->scan_buffer[s->scan_buffer_count +
					       bytecounter]
...
sane_start(SANE_Handle handle)
{
	...
	numbytes =
		write(s->as6e_params.ctloutpipe, scan_params,
		      sizeof(scan_params));
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (!s->scanning) {
		...
		s->as6e_params.startpos =
			as6e_unit_convert(s->value[OPT_TL_X].w);
		s->as6e_params.stoppos =
			as6e_unit_convert(s->value[OPT_BR_X].w);
		s->as6e_params.startline =
			as6e_unit_convert(s->value[OPT_TL_Y].w);
		s->as6e_params.stopline =
			as6e_unit_convert(s->value[OPT_BR_Y].w);
		...
		s->as6e_params.startpos =
			(s->as6e_params.startpos / divisor) * divisor;
		s->as6e_params.stoppos =
			(s->as6e_params.stoppos / divisor) * divisor;
		s->as6e_params.startline =
			(s->as6e_params.startline / divisor) * divisor;
		s->as6e_params.stopline =
			(s->as6e_params.stopline / divisor) * divisor;
		s->sane_params.pixels_per_line =
			(s->as6e_params.stoppos -
			 s->as6e_params.startpos) *
			s->as6e_params.resolution / 300;
		s->sane_params.lines =
			(s->as6e_params.stopline -
			 s->as6e_params.startline) *
			s->as6e_params.resolution / 300;
...
/*else*/ if ((strcmp(mode, "Gray") == 0)
	     || (strcmp(mode, "Lineart") == 0)) {
			...
			s->sane_params.bytes_per_line =
				3 * s->sane_params.pixels_per_line;
			s->sane_params.depth = 8;
		}		/*color frame */
		s->bytes_to_read =
			s->sane_params.lines * s->sane_params.bytes_per_line;
		s->sane_params.last_frame = TRUE;
	}
	/*!scanning */
	if (params)
...
/*      status = sanei_constrain_value (s->options_list[option], val, info);*/
		...
		case OPT_TL_X:
		case OPT_TL_Y:
			if (info && s->value[option].w != *(int *) val)
				*info |= SANE_INFO_RELOAD_PARAMS;
			/* fall through */
		case OPT_NUM_OPTS:
		case OPT_CONTRAST:
		case OPT_BRIGHTNESS:
			s->value[option].w = *(int *) val;
...
as6e_open(AS6E_Scan * s)
{
	...
	if ((pipe(ctloutpipe) == 0) && (pipe(ctlinpipe) == 0)
		...
		if (fork_result == 0) {	/*in child */
			...
			exec_result =
				execlp("as6edriver", "as6edriver", "-s",
				       inpipe_desc, outpipe_desc,
				       datapipe_desc, (char *) 0);
			DBG(1,
			...
			write(ctlinpipe[WRITEPIPE], &exec_result,
			      sizeof(exec_result));
			exit(-1);
		} else {	/*parent process */
			data_processed =
				read(ctlinpipe[READPIPE], &as6e_status,
				     sizeof(as6e_status));
			DBG(1, "%d - read %d status = %d\n", getpid(),
			    data_processed, as6e_status);
			if (as6e_status == -2) {
...
initialize_options_list(AS6E_Scan * s)
{
	...
	for (option = 0; option < NUM_OPTIONS; ++option) {
		s->options_list[option].size = sizeof(int);
	...
	s->options_list[OPT_NUM_OPTS].size = sizeof(int);
	...
	s->options_list[OPT_MODE].constraint_type =
	...
	s->options_list[OPT_RESOLUTION].constraint_type =
		SANE_CONSTRAINT_WORD_LIST;
	s->options_list[OPT_RESOLUTION].constraint.word_list =
		resolution_list;
	...
	s->value[OPT_TL_X].w =
		s->options_list[OPT_TL_X].constraint.range->min;
	...
	s->value[OPT_TL_Y].w =
		s->options_list[OPT_TL_Y].constraint.range->min;
	...
	s->value[OPT_BR_X].w =
		s->options_list[OPT_BR_X].constraint.range->max;
	...
	s->value[OPT_BR_Y].w =
		s->options_list[OPT_BR_Y].constraint.range->max;
