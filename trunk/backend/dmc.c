...
/* $Id: dmc.c,v 1.4 2003-09-12 16:10:33 hmg-guest Exp $ */
...
static int ValidASAs[] = { 3, 25, 50, 100 };
...
DMCAttach(char const *devname, DMC_Device ** devp)
{
	...
	if (result[0] != 6 ||
		...
		sanei_scsi_close(fd);
		DBG(1,
		    "DMCAttach: Device does not look like a Polaroid DMC\n");
	...
	if (status != SANE_STATUS_GOOD ||
		...
		DBG(1,
		    "DMCAttach: Couldn't read exposure calculation results (%s)\n",
		    sane_strstatus(status));
		sanei_scsi_close(fd);
		if (status == SANE_STATUS_GOOD)
	...
	if (status != SANE_STATUS_GOOD ||
		...
		DBG(1,
		    "DMCAttach: Couldn't read user interface settings (%s)\n",
		    sane_strstatus(status));
		sanei_scsi_close(fd);
		if (status == SANE_STATUS_GOOD)
			status = SANE_STATUS_IO_ERROR;
...
DMCInitOptions(DMC_Camera * c)
{
	...
	c->opt[OPT_IMAGE_MODE].desc =
		"Selects image mode: 800x600 full frame, 270x201 viewfinder mode, 1599x600 \"raw\" image, 80x60 thumbnail image or 1599x1200 \"super-resolution\" image";
	...
	c->opt[OPT_SHUTTER_SPEED].constraint.range =
		&c->hw->shutterSpeedRange;
	...
	c->opt[OPT_WHITE_BALANCE].constraint_type =
...
DMCSetASA(int fd, unsigned int asa)
{
	...
	for (i = 1; i <= ASA_100 + 1; i++) {
		if (asa == (unsigned int) ValidASAs[i])
			break;
	}
...
DMCSetWhiteBalance(int fd, int mode)
{
	...
	if (status != SANE_STATUS_GOOD)
		return status;
	if (len < sizeof(userInterfaceSettings))
		return SANE_STATUS_IO_ERROR;
...
DMCReadTwoSuperResolutionLines(DMC_Camera * c, SANE_Byte * buf, int lastLine)
{
	...
	if (lastLine) {
		memcpy(buf + BYTES_PER_RAW_LINE * 3, buf,
		       BYTES_PER_RAW_LINE * 3);
		return SANE_STATUS_GOOD;
	}
	redCoeff = 3;
	greenCoeff = 1;
...
sane_get_parameters(SANE_Handle handle, SANE_Parameters * params)
{
	...
	if (c->fd < 0) {
		...
		c->params.depth = 8;
		c->params.last_frame = TRUE;
		switch (c->imageMode) {
		case IMAGE_SUPER_RES:
		case IMAGE_MFI:
		case IMAGE_THUMB:
			c->params.format = SANE_FRAME_RGB;
			c->params.bytes_per_line =
				c->params.pixels_per_line * 3;
			break;
		case IMAGE_RAW:
		case IMAGE_VIEWFINDER:
...
sane_start(SANE_Handle handle)
{
	...
	if ((unsigned int) c->val[OPT_SHUTTER_SPEED].w != c->hw->shutterSpeed) {
		status = DMCSetShutterSpeed(c->fd,
					    c->val[OPT_SHUTTER_SPEED].w);
		if (status != SANE_STATUS_GOOD) {
			DMCCancel(c);
			return status;
	...
	if (c->imageMode != IMAGE_VIEWFINDER && c->inViewfinderMode) {
		status = sanei_scsi_cmd(c->fd, no_viewfinder,
					sizeof(no_viewfinder), NULL, NULL);
		if (status != SANE_STATUS_GOOD) {
			DMCCancel(c);
			return status;
		}
	  ...
	  int * len)
{
	...
	if (c->imageMode == IMAGE_SUPER_RES) {
		...
		if (max_len == 0)
			return SANE_STATUS_INVAL;
		if ((unsigned int) max_len > c->bytes_to_read)
			max_len = c->bytes_to_read;
	...
	if (c->imageMode == IMAGE_MFI || c->imageMode == IMAGE_RAW) {
		...
		max_len =
			(max_len / c->params.bytes_per_line) *
			c->params.bytes_per_line;
		...
		status = DMCRead(c->fd, 0x00, c->imageMode, buf, max_len,
				 &size);
		*len = size;
		return status;
	}
	...
	status = DMCRead(c->fd, 0x00, c->imageMode,
			 (SANE_Byte *) c->readBuffer, c->bytes_to_read,
			 &size);
	*len = size;
	if (status != SANE_STATUS_GOOD)
		return status;
	if ((unsigned int) *len != c->bytes_to_read)
		return SANE_STATUS_IO_ERROR;
