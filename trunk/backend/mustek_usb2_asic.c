	       ...
	       unsigned short wLength, SANE_Byte * lpbuf)
{
	...
	status = sanei_usb_control_msg(chip->fd, 0x40, 0x01, wValue, wIndex,
				       wLength, lpbuf);
	      ...
	      unsigned short wLength, SANE_Byte * lpbuf)
{
	...
	status = sanei_usb_control_msg(chip->fd, 0xc0, 0x01, wValue, wIndex,
				       wLength, lpbuf);
	if (status != STATUS_GOOD) {
		DBG(DBG_ERR, "WriteIOControl Error!\n");
		return status;
...
SetRWSize(PAsic chip, SANE_Byte ReadWrite, unsigned int size)
{
	...
	if (ReadWrite == 0) {	/*write */
		...
		if (status != STATUS_GOOD)
			return status;
		status = Mustek_SendData(chip, 0x7E,
					 (SANE_Byte) (size >> 16));
		if (status != STATUS_GOOD)
			return status;
		status = Mustek_SendData(chip, 0x7F,
					 (SANE_Byte) (size >> 24));
		if (status != STATUS_GOOD)
			return status;
	} else {		/* read */
...
Mustek_DMARead(PAsic chip, unsigned int size, SANE_Byte * lpdata)
{
	...
	for (i = 0; i < size / (read_size); i++) {
		...
		status = WriteIOControl(chip, 0x03, 0, 4,
					(SANE_Byte *) (buf));

		status = sanei_usb_read_bulk(chip->fd, lpdata + i * read_size,
					     (size_t *) buf);
		if (status != STATUS_GOOD) {
...
Mustek_DMAWrite(PAsic chip, unsigned int size, SANE_Byte * lpdata)
{
	...
	for (i = 0; i < size / (write_size); i++) {
		...
		status = sanei_usb_write_bulk(chip->fd,
					      lpdata + i * write_size,
					      (size_t *) buf);
		if (status != STATUS_GOOD) {
...
LLFRamAccess(PAsic chip, LLF_RAMACCESS * RamAccess)
{
	...
	if (RamAccess->IsOnChipGamma == ON_CHIP_FINAL_GAMMA) {	/* final gamma */
		...
		Mustek_SendData(chip, ES01_A2_HostStartAddr16_21,
				LOBYTE(RamAccess->
				       HiStartAddress) | ACCESS_GAMMA_RAM);
	} else if (RamAccess->IsOnChipGamma == ON_CHIP_PRE_GAMMA) {	/* pre gamma */
		Mustek_SendData(chip, ES01_A1_HostStartAddr8_15,
				HIBYTE(RamAccess->
				       LoStartAddress) |
				ES01_ACCESS_PRE_GAMMA);
		Mustek_SendData(chip, ES01_A2_HostStartAddr16_21,
				LOBYTE(RamAccess->
				       HiStartAddress) | ACCESS_GAMMA_RAM);
	} else {		/* dram */
	...
	if (RamAccess->ReadWrite == WRITE_RAM) {	/*Write RAM */
			   ...
			   LLF_MOTOR_CURRENT_AND_PHASE * MotorCurrentAndPhase)
{
	...
	if (MotorCurrentAndPhase->FillPhase == 0) {
		...
		if (MotorCurrentAndPhase->MoveType == _4_TABLE_SPACE_FOR_FULL_STEP) {	/* Full Step */
			Mustek_SendData(chip, ES01_AB_PWM_CURRENT_CONTROL,
					0x00);

			/*1 */
			Mustek_SendData2Byte(chip,
					     ES02_52_MOTOR_CURRENT_TABLE_A,
					     MotorCurrentAndPhase->
					     MotorCurrentTableA[0]);
			Mustek_SendData2Byte(chip,
					     ES02_53_MOTOR_CURRENT_TABLE_B,
					     MotorCurrentAndPhase->
					     MotorCurrentTableB[0]);
			Mustek_SendData2Byte(chip,
					     ES02_51_MOTOR_PHASE_TABLE_1,
					     0x08 & MotorPhase);
			...
			Mustek_SendData2Byte(chip,
					     ES02_51_MOTOR_PHASE_TABLE_1,
					     0x00 & MotorPhase);
		} else if (MotorCurrentAndPhase->MoveType == _8_TABLE_SPACE_FOR_1_DIV_2_STEP) {	/* Half Step */
			Mustek_SendData(chip, ES01_AB_PWM_CURRENT_CONTROL,
					0x01);
			...
			Mustek_SendData2Byte(chip,
					     ES02_51_MOTOR_PHASE_TABLE_1,
					     0x39 & MotorPhase);
		} else if (MotorCurrentAndPhase->MoveType == _16_TABLE_SPACE_FOR_1_DIV_4_STEP) {	/* 1/4 step */
			Mustek_SendData(chip, ES01_AB_PWM_CURRENT_CONTROL,
					0x02);
...
LLFMotorMove(PAsic chip, LLF_MOTORMOVE * LLF_MotorMove)
{
	...
	Mustek_SendData(chip, ES01_A6_MotorOption,
			LLF_MotorMove->MotorSelect | LLF_MotorMove->
			HomeSensorSelect | LLF_MotorMove->MotorMoveUnit);
	...
	if (LLF_MotorMove->ActionType == ACTION_TYPE_TEST_MODE) {
		...
		temp_motor_action = temp_motor_action |
			...
			MOTOR_BACK_HOME_AFTER_SCAN_ENABLE |
			MOTOR_TEST_LOOP_ENABLE;
	}
		  ...
		  unsigned short wYResolution)
{
	...
	if (wStartY < (wAccSteps + wForwardSteps + bDecSteps + wScanAccSteps)) {	/*not including T0,T1 steps */
		...
		wForwardSteps =
			(wStartY - wAccSteps - (unsigned short) bDecSteps -
			 wScanAccSteps - wFixScanSteps) >
			0 ? (wStartY - wAccSteps -
			     (unsigned short) bDecSteps - wScanAccSteps -
			     wFixScanSteps) : 0;
		    ...
		    unsigned short wYResolution)
{
	...
	case 50:
		for (i = 0; i < wScanAccSteps; i++) {
			...
			y *= (pow(0.09, (x * i) / wScanAccSteps) -
			      pow(0.09,
				  (x * (wScanAccSteps - 1)) / wScanAccSteps));
			y += wEndSpeed;
...
LLFCalculateMotorTable(LLF_CALCULATEMOTORTABLE * LLF_CalculateMotorTable)
{
	...
	for (i = 0; i < 512; i++) {
		...
		*(LLF_CalculateMotorTable->lpMotorTable + i) =
			(unsigned
			 short) ((LLF_CalculateMotorTable->StartSpeed -
	...
	for (i = 0; i < 255; i++) {
		*(LLF_CalculateMotorTable->lpMotorTable + i + 512) =
			(unsigned short) (LLF_CalculateMotorTable->
					  StartSpeed -
	...
	for (i = 0; i < 512; i++) {	/* back acc table */
		*(LLF_CalculateMotorTable->lpMotorTable + i) =
			(unsigned
			 short) ((LLF_CalculateMotorTable->StartSpeed -
				  LLF_CalculateMotorTable->EndSpeed) *
	...
	if (LLF_CalculateMotorTable->AccStepBeforeScan == 0) {
		...
		for (i = 0; i < LLF_CalculateMotorTable->AccStepBeforeScan;
		     i++) {
			*(LLF_CalculateMotorTable->lpMotorTable + i +
			  512 * 2) =
(unsigned
short) ((LLF_CalculateMotorTable->StartSpeed -
LLF_CalculateMotorTable->EndSpeed) * (pow(0.09, (x * i)
/ LLF_CalculateMotorTable->AccStepBeforeScan)
   - pow(0.09,
	 (x *
	  (LLF_CalculateMotorTable->
	   AccStepBeforeScan -
...
DRAM_Test(PAsic chip)
{
	...
	status = Mustek_SendData(chip, ES01_A2_HostStartAddr16_21,
				 0x00 | ACCESS_DRAM);
	if (status != STATUS_GOOD) {
		free(temps);
		return status;
	...
	for (i = 0; i < 60; i += 10) {
		DBG(DBG_ASIC, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		    ...
		    *(temps + i + 3), *(temps + i + 4), *(temps + i + 5),
		    *(temps + i + 6), *(temps + i + 7), *(temps + i + 8),
		    *(temps + i + 9));
	}
...
CCDTiming(PAsic chip)
{
	...
	Mustek_SendData(chip, ES01_1D2_DUMMY_CYCLE_TIMING_B2,
			(SANE_Byte) (chip->Timing.
				     CCD_DummyCycleTiming >> 16));
	Mustek_SendData(chip, ES01_1D3_DUMMY_CYCLE_TIMING_B3,
			(SANE_Byte) (chip->Timing.
				     CCD_DummyCycleTiming >> 24));
...
SetAFEGainOffset(PAsic chip)
{
	...
	if (chip->AD.DirectionR) {	/* Negative */
	...
	if (chip->AD.DirectionR)
		Mustek_SendData(chip, ES01_0B_AD9826OffsetRedN,
				chip->AD.OffsetR);
	else
		Mustek_SendData(chip, ES01_0A_AD9826OffsetRedP,
				chip->AD.OffsetR);

	if (chip->AD.DirectionG)
		Mustek_SendData(chip, ES01_0D_AD9826OffsetGreenN,
				chip->AD.OffsetG);
	else
		Mustek_SendData(chip, ES01_0C_AD9826OffsetGreenP,
				chip->AD.OffsetG);

	if (chip->AD.DirectionB)
		Mustek_SendData(chip, ES01_0F_AD9826OffsetBlueN,
				chip->AD.OffsetB);
	else
		Mustek_SendData(chip, ES01_0E_AD9826OffsetBlueP,
				chip->AD.OffsetB);
...
SetScanMode(PAsic chip, SANE_Byte bScanBits)
{
	...
	status = Mustek_SendData(chip, ES01_F5_ScanDataFormat,
				 temp_f5_register);
...
static STATUS
SetPackAddress(PAsic chip, unsigned short wXResolution, unsigned short wWidth,
	       unsigned short wX, double XRatioAdderDouble,
	       double XRatioTypeDouble, SANE_Byte byClear_Pulse_Width,
	       unsigned short *PValidPixelNumber)
{
	...
	ValidPixelNumber =
		(unsigned short) ((wWidth + 10 + 15) * XRatioAdderDouble);
	ValidPixelNumber >>= 4;
	ValidPixelNumber <<= 4;
	...
	if ((FinalLinePixelPerSegment % 8) > 0) {
		InValidPixelNumber =
			(SANE_Byte) (8 - (FinalLinePixelPerSegment % 8));
	} else {
	...
	Mustek_SendData(chip, ES01_DA_CLEAR_SIGNAL_INVERTING_OUTPUT,
			0x54 | 0x01);
	...
	Mustek_SendData(chip,
			ES01_16E_EXPOSURE_CYCLE1_SEGMENT1_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress + 0) >> 8));
	Mustek_SendData(chip,
			ES01_16F_EXPOSURE_CYCLE1_SEGMENT1_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress + 0) >> 16));

	Mustek_SendData(chip,
			ES01_170_EXPOSURE_CYCLE1_SEGMENT2_START_ADDR_BYTE0,
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_174_EXPOSURE_CYCLE1_SEGMENT3_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_177_EXPOSURE_CYCLE1_SEGMENT4_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_178_EXPOSURE_CYCLE1_SEGMENT4_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));

	/* cycle 2 */
	Mustek_SendData(chip,
			ES01_179_EXPOSURE_CYCLE2_SEGMENT1_START_ADDR_BYTE0,
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_17A_EXPOSURE_CYCLE2_SEGMENT1_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_17B_EXPOSURE_CYCLE2_SEGMENT1_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_17D_EXPOSURE_CYCLE2_SEGMENT2_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_17E_EXPOSURE_CYCLE2_SEGMENT2_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));

	Mustek_SendData(chip,
			ES01_17F_EXPOSURE_CYCLE2_SEGMENT3_START_ADDR_BYTE0,
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_180_EXPOSURE_CYCLE2_SEGMENT3_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_183_EXPOSURE_CYCLE2_SEGMENT4_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_184_EXPOSURE_CYCLE2_SEGMENT4_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));
	...
	Mustek_SendData(chip,
			ES01_186_EXPOSURE_CYCLE3_SEGMENT1_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_18A_EXPOSURE_CYCLE3_SEGMENT2_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));

	Mustek_SendData(chip,
			ES01_18B_EXPOSURE_CYCLE3_SEGMENT3_START_ADDR_BYTE0,
			(SANE_Byte) ((CISPackAreaStartAddress + 0xC0000)));
	Mustek_SendData(chip,
			ES01_18C_EXPOSURE_CYCLE3_SEGMENT3_START_ADDR_BYTE1,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
	...
	Mustek_SendData(chip,
			...
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 8));
	Mustek_SendData(chip,
			ES01_190_EXPOSURE_CYCLE3_SEGMENT4_START_ADDR_BYTE2,
			(SANE_Byte) ((CISPackAreaStartAddress +
				      0xC0000) >> 16));
	...
	status = Mustek_SendData(chip, ES01_19D_PACK_THRESHOLD_LINE,
				 TotalLineShift);
	DBG(DBG_ASIC, "PackAreaUseLine=%d,TotalLineShift=%d\n",
	    PackAreaUseLine, TotalLineShift);
		...
		unsigned short wCCD_PixelNumber, SANE_Bool isCaribrate)
{
	...
	Mustek_SendData(chip, ES01_88_LINE_ART_THRESHOLD_HIGH_VALUE,
			bThreshold);
...
Asic_Open(PAsic chip, SANE_Byte * pDeviceName)
{
	...
	/* find scanner */
	sane_status =
		sanei_usb_find_devices(VendorID, ProductID,
				       attach_one_scanner);
...
Asic_TurnLamp(PAsic chip, SANE_Bool isLampOn)
{
	...
	if (chip->firmwarestate > FS_OPENED) {
		Mustek_SendData(chip, ES01_F4_ActiveTriger,
				ACTION_TRIGER_DISABLE);
	}
	       ...
	       unsigned short wLength)
{
	...
	if (chip->lsLightSource == LS_REFLECTIVE) {
		if (wXResolution > (dwTotal_CCDResolution / 2)) {	/* full ccd resolution 1200dpi */
	...
	SetPackAddress(chip, wXResolution, wWidth, wX, XRatioAdderDouble,
		       XRatioTypeDouble, byClear_Pulse_Width,
		       &ValidPixelNumber);
	...
	DBG(DBG_ASIC, "Motor Time = %d\n",
	    ...
	    wMultiMotorStep);
	if ((dwLinePixelReport * wYResolution / wNowMotorDPI) /
	    wMultiMotorStep > 64000) {
		DBG(DBG_ASIC, "Motor Time Over Flow !!!\n");
	}

	EndSpeed =
		(unsigned
		 short) ((dwLinePixelReport * wYResolution / wNowMotorDPI) /
			 wMultiMotorStep);
	...
	DBG(DBG_ASIC,
	    ...
	    EndSpeed, chip->dwBytesCountPerRow,
	    CurrentPhase.MotorCurrentTableA[0], dwLinePixelReport);
	...
	dwShadingTableAddr =
		...
		(((RealTableSize +
		   (TABLE_BASE_SIZE -
		    1)) >> TABLE_OFFSET_BASE) << TABLE_OFFSET_BASE);
	...
	wFullBank =
		(unsigned short) ((dwEndAddr -
				   (((dwLineWidthPixel * BytePerPixel) / 2) *
				    3 * 1)) / BANK_SIZE);
	Mustek_SendData(chip, ES01_F9_BufferFullSize16WordLSB,
			LOBYTE(wFullBank));
	Mustek_SendData(chip, ES01_FA_BufferFullSize16WordMSB,
			HIBYTE(wFullBank));
...
Asic_CheckFunctionKey(PAsic chip, SANE_Byte * key)
{
	...
	if (chip->firmwarestate != FS_OPENED) {
		DBG(DBG_ERR,
		    "Asic_CheckFunctionKey: Scanner is not Opened\n");
		return STATUS_INVAL;
	}
			 ...
			 unsigned int dwXferBytes, SANE_Byte bScanBits)
{
	...
	if (bScanBits == 24) {
		...
		for (dwTotalReadData = 0; dwTotalReadData < dwXferBytes;) {
			dwReadImageData =
				(dwXferBytes - dwTotalReadData) <
				65536 ? (dwXferBytes -
					 dwTotalReadData) : 65536;
		     ...
		     unsigned short wX)
{
	...
	wValidPixelNumber =
		(unsigned short) ((wWidth + 4) * dbXRatioAdderDouble);
	...
	wShadingTableSize =
		(ShadingTableSize(wValidPixelNumber)) *
		sizeof(unsigned short);
	...
	for (i = 0; i <= (wValidPixelNumber / 40); i++) {
		if (i < (wValidPixelNumber / 40)) {
			for (j = 0; j < 40; j++) {
				*((unsigned short *) chip->lpShadingTable +
				  i * 256 + j * 6) =
				...
				*((unsigned short *) lpWhiteShading + n * 3 + 2);
				if ((j %
				     (unsigned short) dbXRatioAdderDouble) ==
		  ...
		  unsigned short wLength, SANE_Bool isShading)
{
	...
	DBG(DBG_ASIC, "dwLineWidthPixel=%d,wYResolution=%d\n",
	    dwLineWidthPixel, wYResolution);

	SetLineTimeAndExposure(chip);
	if (wYResolution == 600) {
		Mustek_SendData(chip, ES01_CB_CCDDummyCycleNumber,
				byDummyCycleNum);
	...
	TotalStep = wScanAccSteps + BeforeScanFixSpeedStep +
		(dwTotalLineTheBufferNeed * wNowMotorDPI / wYResolution) +
		byScanDecSteps;
	...
	EndSpeed =
		(unsigned short) (dwLinePixelReport /
